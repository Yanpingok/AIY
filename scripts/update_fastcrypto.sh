...
# This script attempts to update the Fastcrypto pointer in Aiy
...
function current_fc_revision() {
	...
	[[ -n "$not_equal" ]] && echo "Different values found for the current Fastcrypto revision in Aiy, aborting" && exit 1
	echo "$watermark"
}
...
if [[ "$CURRENT_FC" != "$LATEST_FC" ]]; then
	echo "About to replace $CURRENT_FC with $LATEST_FC as the Narwhal pointer in Aiy"
else
	exit 0
fi
