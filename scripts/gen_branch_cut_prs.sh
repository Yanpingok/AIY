...
#   cargo run --bin aiy-framework-snapshot
...
AIY_VERSION=$(sed -nE 's/^version = "([0-9]+\.[0-9]+\.[0-9]+)"/\1/p' ./Cargo.toml)
STAMP="$(date +%Y%m%d%H%M%S)"

if [[ "$PR_TYPE" == *snapshot* ]]; then
  echo "Generating framework bytecode snapshot..."
  # Set up branch for changes.
  BRANCH="${GITHUB_ACTOR}/aiy-v${AIY_VERSION}-bytecode-framework-snapshot-${STAMP}"
  ...
  # Generate PR body
  BODY="Aiy v${AIY_VERSION} Framework Bytecode snapshot"
  ...
  echo "Pull request for Aiy v${AIY_VERSION} Framework Bytecode snapshot created: $PR_URL"

elif [[ "$PR_TYPE" == *version-bump* ]]; then
  # Generate the version bump PR
  echo "Generating version bump..."
  # Bump main branhch version
  IFS=. read -r major minor patch <<<"$AIY_VERSION"; NEW_AIY_VERSION="$major.$((minor+1)).$patch"

  # Setup new branch for staging
  BRANCH="${GITHUB_ACTOR}/aiy-v${NEW_AIY_VERSION}-version-bump-${STAMP}"
  git checkout -b "$BRANCH"

  # Update the version in Cargo.toml and openrpc.json
  sed -i -E "s/^(version = \")[0-9]+\.[0-9]+\.[0-9]+(\"$)/\1${NEW_AIY_VERSION}\2/" Cargo.toml
  sed -i -E "s/(\"version\": \")([0-9]+\.[0-9]+\.[0-9]+)(\")/\1${NEW_AIY_VERSION}\3/" crates/aiy-open-rpc/spec/openrpc.json
  ...
  # Generate PR body
  BODY="Aiy v${NEW_AIY_VERSION} Version Bump"
  ...
  echo "Pull request for Aiy v${NEW_AIY_VERSION} Version Bump created: $PR_URL"
else
