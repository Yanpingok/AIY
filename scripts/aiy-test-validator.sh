...
echo "aiy-test-validator binary has been deprecated in favor of aiy start, which is a more powerful command that allows you to start the local network with more options.
This script offers backward compatibiltiy, but ideally, you should migrate to aiy start instead. Use aiy start --help to see all the flags and options. 

To recreate the exact basic functionality of aiy-test-validator, you must use the following options:
...
# Basic command that replicates the command line arguments of aiy-test-validator
cmd="aiy start --with-faucet --force-regenesis"
