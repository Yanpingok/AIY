...
CLEAN=0
LOG_LEVEL="info"
AIY_RUN_PATH="/opt/aiy"
VERBOSE=""

function cleanup {
    echo "Performing exit cleanup..."
    [ ! -z $AIY_NODE_PID ] && kill $AIY_NODE_PID && echo "Shutdown aiynode process running on pid $AIY_NODE_PID"
}
...
while getopts "hvn:e:p:t:" OPT; do
    case $OPT in
        ...
        h)
            ...
            >&2 echo ""
            >&2 echo "Options:"
            >&2 echo " -p                 Path to aiy binary to run. If unspecified, will build from source."
...
[ ! -d "${AIY_RUN_PATH}/aiydb" ] && mkdir -p ${AIY_RUN_PATH}/aiydb
...
if [[ ! -f "${AIY_RUN_PATH}/genesis.blob" ]]; then
    echo "Copying genesis.blob for ${NETWORK}"
    curl -fLJO https://github.com/MystenLabs/sui-genesis/raw/main/${NETWORK}/genesis.blob
    mv ./genesis.blob ${AIY_RUN_PATH}/genesis.blob
    echo "Done"
fi

if [[ ! -f "${AIY_RUN_PATH}/fullnode.yaml" ]]; then
    echo "Generating fullnode.yaml at ${AIY_RUN_PATH}/fullnode.yaml"
    cp crates/aiy-config/data/fullnode-template.yaml ${AIY_RUN_PATH}/fullnode.yaml
    sed -i "s|genesis.blob|${AIY_RUN_PATH}/genesis.blob|g" ${AIY_RUN_PATH}/fullnode.yaml
    sed -i "s|aiydb|${AIY_RUN_PATH}/aiydb|g" ${AIY_RUN_PATH}/fullnode.yaml

    if [[ $NETWORK != "devnet" ]]; then
        cat >> "${AIY_RUN_PATH}/fullnode.yaml" <<- EOM

p2p-config:
  seed-peers:
    - address: /dns/ewr-tnt-ssfn-00.testnet.aiy.io/udp/8084
      peer-id: df8a8d128051c249e224f95fcc463f518a0ebed8986bbdcc11ed751181fecd38
    - address: /dns/lax-tnt-ssfn-00.testnet.aiy.io/udp/8084
      peer-id: f9a72a0a6c17eed09c27898eab389add704777c03e135846da2428f516a0c11d
    - address: /dns/lhr-tnt-ssfn-00.testnet.aiy.io/udp/8084
      peer-id: 9393d6056bb9c9d8475a3cf3525c747257f17c6a698a7062cbbd1875bc6ef71e
    - address: /dns/mel-tnt-ssfn-00.testnet.aiy.io/udp/8084
...
if [[ -z $AIY_BIN_PATH ]]; then
    echo "Building aiy..."
    cargo build --release --bin aiy-node
    AIY_BIN_PATH="target/release/aiy-node"
    echo "Done"
fi


echo "Starting aiynode..."
RUST_LOG=$LOG_LEVEL $AIY_BIN_PATH --config-path ${AIY_RUN_PATH}/fullnode.yaml &
AIY_NODE_PID=$!
