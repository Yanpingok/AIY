...
def parse_args():
    ...
    generate_lib = subparsers.add_parser(
        "generate-lib",
        help=(
            "Generate `aiy-execution/src/lib.rs` based on the current set of "
...
def do_cut(args):
    ...
    update_toml(args.feature, Path() / "aiy-execution" / "Cargo.toml")
    generate_impls(args.feature, impl_module)

    with open(Path() / "aiy-execution" / "src" / "lib.rs", mode="w") as lib:
        generate_lib(lib)
...
def do_generate_lib(args):
    if args.dry_run:
        ...
        lib_path = Path() / "aiy-execution" / "src" / "lib.rs"
...
def cut_command(f):
    ...
    return [
        ...
        *[repo_root()],
        *["-d", f"aiy-execution/latest:aiy-execution/{f}:-latest"],
        *["-d", f"external-crates/move:external-crates/move/move-execution/{f}"],
        *["-p", "aiy-adapter-latest"],
        *["-p", "aiy-move-natives-latest"],
        *["-p", "aiy-verifier-latest"],
...
def cut_directories(f):
    ...
    aiy_base = Path() / "aiy-execution"
    external = Path() / "external-crates"

    crates = [
        aiy_base / f / "aiy-adapter",
        aiy_base / f / "aiy-move-natives",
        aiy_base / f / "aiy-verifier",
    ]
...
def impl(feature):
    ...
    return Path() / "aiy-execution" / "src" / (feature.replace("-", "_") + ".rs")
...
def update_toml(feature, toml_path):
    """Add dependencies for 'feature' to aiy-execution's manifest."""
...
def generate_impls(feature, copy):
    ...
    orig = Path() / "aiy-execution" / "src" / "latest.rs"
...
def generate_lib(output_file: TextIO):
    ...
    Generates the contents of aiy-execution/src/lib.rs to assign a numeric
    ...
    template_path = Path() / "aiy-execution" / "src" / "lib.template.rs"
    cuts = discover_cuts()
...
# Modules in `aiy-execution` that don't count as "cuts" (they are
...
def discover_cuts():
    ...
    Finds all modules within the `aiy-execution` crate that count as
