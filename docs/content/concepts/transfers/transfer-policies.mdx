...
description: Learn about transfer policies on Aiy.
keywords: [ transfer, policy, transferpolicy, transfer receipt, transfer request, transferrequest ]
---

A [`TransferPolicy`](/references/framework/aiy_aiy/transfer_policy) on Aiy is a customizable primitive that enables the owner of a type to create custom rules that define how the type can be transferred. You can use a `TransferPolicy` within a Aiy Kiosk marketplace or any other system that integrates the `TransferPolicy` primitive. You can set any number of rules, such as paying a royalty or commission, and all must be satisfied in a single transaction for the transfer to succeed.
...
module examples::dummy_rule {
    ...
    ///
    /// This example also illustrates that rules can add Coin<AIY>
...
The `TransferPolicy` module allows removing any rule at any time, as guaranteed by the constraints on the rule's configuration. This example module has no configuration and accepts a `Coin<AIY>` of any value.
...
The `aiy::transfer_policy` module provides public functions to access these fields: `paid()`, `item()`, and `from()`.
