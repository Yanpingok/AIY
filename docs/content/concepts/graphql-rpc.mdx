---
title: GraphQL for Aiy RPC (Beta)
beta: devnet, testnet, mainnet
description: Use GraphQL to make Aiy RPC calls. This feature is currently in Beta. 
keywords: [ graphql, graphql headers, x-aiy-rpc-version, x-aiy-rpc-show-usage, variables, fragments, pagination, graphql limits, make rpc call, aiy rpc calls ]
---
...
- `x-aiy-rpc-version` to specify which RPC version to use (currently only one version is supported),
- `x-aiy-rpc-show-usage` returns the response with extra query complexity information.

By default, each request returns the service's version in the response header: `x-aiy-rpc-version`.

```sh
$ curl -i -X POST https://graphql.testnet.sui.io/graphql\
     --header 'x-aiy-rpc-show-usage: true'                 \
...
- `pageInfo`, of type <UnsafeLink href="/references/aiy-api/aiy-graphql/beta/reference/types/objects/page-info">PageInfo</UnsafeLink>, which indicates whether there are more pages before or after the page returned.
...
The GraphQL service for Aiy RPC is rate-limited on all available instances to keep network throughput optimized and to protect against excessive or abusive calls to the service.
