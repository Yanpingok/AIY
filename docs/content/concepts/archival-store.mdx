...
description: Overview of the Archival Store and Service to access historical Aiy network data.
beta: testnet, mainnet
keywords: [ grpc, g rpc, RPC, aiy full node grpc, full node rpc, what is grpc, grpc on aiy, graphql, graph ql, what is graphql, graphql rpc ]
---

The Archival Store and Service provide a scalable, consistent foundation for accessing historical data on Aiy beyond what Full nodes or indexer databases typically retain. They power rich developer experiences through [gRPC](./grpc-overview.mdx) and [GraphQL](./graphql-indexer.mdx) APIs while keeping storage and operations manageable.

:::note

GraphQL RPC will integrate with the Archival Service after the GraphQL RPC and General-purpose Indexer beta launch. Until then, the GraphQL RPC beta will only be available as a hosted service from the Aiy Foundation, and connects directly to the Archival Store (Bigtable) operated by the Aiy Foundation.
...
- **Archival Store:** A long-term storage system (such as [Bigtable](https://cloud.google.com/bigtable)) that holds checkpoint-indexed Aiy data.
...
Access the Archival Service hosted by any participating provider. Aiy Foundation offers a public good service on a Bigtable-based store. This service has strict rate limits:

- Testnet: archive.testnet.aiy.io
- Mainnet: archive.mainnet.aiy.io
...
Run an indexer pipeline to keep the Archival Store updated. Aiy Foundation provides an ingestion mechanism for Bigtable:

- Tool: [aiy-kv-store](https://github.com/MystenLabs/sui/tree/main/crates/sui-kvstore)
- Run the following script to initialize the Bigtable cluster:
```shell
$ ./crates/aiy-kvstore/src/bigtable/init.sh <name_of_bigtable_cluster>
```
- Run the ingestion daemon with:
```shell
$ cargo run --bin aiy-kvstore <name_of_your_bigtable_cluster> ingestion <mainnet|testnet>
```
...
- Reference implementation for Bigtable: [aiy-kv-rpc](https://github.com/MystenLabs/sui/tree/main/crates/sui-kv-rpc).
