...
A party object is an object that is transferred using the `aiy::transfer::party_transfer` or `aiy::transfer::public_party_transfer` function. It is accessible to the `Party` to which it is transferred.
...
Use the `aiy::transfer::party_transfer` function if you are defining a [custom transfer policy](../transfers/custom-rules.mdx) for the object. Use the `aiy::transfer::public_party_transfer` function if the object has the `store` capability.
...
`Coin`s can be party objects, including `Coin<AIY>`. However, you can't use a party object `Coin<AIY>` for gas payment. To use a party object `Coin<AIY>` for gas, you must first transfer it back to address-owned.
