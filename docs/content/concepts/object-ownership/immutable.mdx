...
Objects in Aiy have different ownership types, with two main categories: immutable objects and mutable objects. An immutable object cannot be mutated, transferred, or deleted. These objects have no owner and are freely accessible to everyone.
...
After an object becomes immutable, the rules of who can use this object in Aiy Move calls change:
You can only pass an immutable object as a read-only, immutable reference to Aiy Move entry functions as `&T`.
...
```sh
$ export ADDR=`aiy client active-address`
```
```sh
$ aiy client objects $ADDR
```

Publish the `ColorObject` code on-chain using the Aiy Client CLI:

<ImportContent source="info-gas-budget.mdx" mode="snippet" />

```sh
$ aiy client publish $ROOT/examples/move/color_object --gas-budget <GAS-AMOUNT>
```

Set the package object ID to the `$PACKAGE` environment variable, if you have it set. Then create a new `ColorObject`:

```sh
$ aiy client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "create" --args 0 255 0
```
...
```sh
$ aiy client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "freeze_object" --args \"$OBJECT\"
```
...
```sh
$ aiy client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "update" --args \"$OBJECT\" 0 0 0
```
