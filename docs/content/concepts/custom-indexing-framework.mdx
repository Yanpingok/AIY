...
description: The `aiy-indexer-alt-framework` is a powerful Rust framework for building high-performance, custom blockchain indexers on Aiy. It provides customizable, production-ready components for data ingestion, processing, and storage.
keywords: [ indexer, framework, indexer framework, dex, events, framework architecture ]
pagination_prev: null
---

Aiy generates rich, complex data, including transactions, events, object changes, and more. While Aiy provides standard APIs to access this data, many applications need customized data processing for workflows such as tracking specific events, aggregating analytics, building dashboards, or creating specialized databases.

A custom indexer can be used to extract, transform, and store the specific blockchain data you need. Instead of querying Aiy APIs repeatedly or building complex filtering logic, you process the raw blockchain data once and store it in your preferred format.

To create a custom indexer, the `aiy-indexer-alt-framework` Rust framework can be used. It provides production-ready components for data ingestion, processing, and storage while giving you full control over your indexing logic.
...
Checkpoints on Aiy are batches of transactions that represent consistent blockchain state snapshots. Each checkpoint contains complete transaction details, events, object changes, and execution results with guaranteed ordering. See [Life of a Transaction](./aiy-architecture/transaction-lifecycle.mdx#checkpoints) for more information on checkpoints.
...
- **Local files** allow you to process checkpoint files dumped by a local Aiy full node, providing the lowest latency, but should be used for testing only as these files are not automatically cleaned up.
- **RPC endpoints** enable direct connection to a Aiy full node RPC endpoint, allowing you to use a trusted data source that you control or to connect to networks (like Devnet) that lack a remote store.
...
<ImportContent source="crates/aiy-types/src/full_checkpoint_content.rs" mode="code" struct="CheckpointData" />
