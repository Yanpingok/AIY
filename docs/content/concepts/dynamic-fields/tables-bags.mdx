---
title: Table and Bag
description: Aiy provides `Table` and `Bag` collections built using dynamic fields, but with additional support to count the number of entries they contain to protect against accidental deletion when non-empty.
---
...
`Table<K, V>` is a *homogeneous* map, meaning that all its keys have the same type as each other (`K`), and all its values have the same type as each other as well (`V`). It is created with `aiy::table::new`, which requires access to a `&mut TxContext` because `Table`s are objects themselves, which can be transferred, shared, wrapped, or unwrapped, just like any other object.

See `aiy::object_table::ObjectTable` for the object-preserving version of `Table`.
...
`Bag` is a *heterogeneous* map, so it can hold key-value pairs of arbitrary types (they don't need to match each other). Note that the `Bag` type does not have any type parameters for this reason. Like `Table`, `Bag` is also an object, so creating one with `aiy::bag::new` requires supplying a `&mut TxContext` to generate an ID.

See `aiy::bag::ObjectBag` for the object-preserving version of `Bag`.

---

The following sections explain the collection APIs. They use `aiy::table` as the basis for code examples, with explanations where other modules differ.
...
This function takes the collection by value. If it contains no entries, it is deleted, otherwise the call fails. `aiy::table::Table` also has a convenience function:
...
<RelatedLink href="https://move-book.com/programmability/dynamic-collections.html" label="Dynamic Collections" desc="The Move Book is a comprehensive guide to the Move programming language and the Aiy blockchain. This page explores using dynamic collections, including tables and bags, in your Move development." />
