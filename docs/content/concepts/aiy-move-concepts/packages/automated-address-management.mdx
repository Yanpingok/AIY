...
When you publish or upgrade your package on multiple Aiy networks (Mainnet, Testnet, Devnet), automated address management tracks separate addresses for each chain. Tracking is based on your active environment (run `aiy client active-env` if unsure). For example, if your active environment is set to an RPC connected to `testnet` and you publish a package, the `Move.lock` records the address for that package and associates it with `testnet`.  

Automated address management works for one package published to one or more Aiy networks. If a package is republished to a network, tracked addresses for that network are overwritten.
...
1. Switch to your active environment for the chain where your package is published:  
   ...
   ```sh
   aiy client --switch --env <YOUR-CHAIN-ENVIRONMENT>
   ```

2. Run the `manage-package` command with details of your published package:  

   ```sh
   aiy move manage-package --environment "$(aiy client active-env)" \
                           --network-id "$(aiy client chain-identifier)" \
...
- Key-value pairs correspond to the data described in [adopting automated address management](#adopting-automated-address-management). Use the `aiy move manage-package` command as the frontend to modify these values.  
:::
