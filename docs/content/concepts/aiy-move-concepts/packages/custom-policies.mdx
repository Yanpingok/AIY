...
To address the security risk of single-key upgrade ownership while still providing the opportunity to [upgrade live packages](./upgrade.mdx), Aiy offers _custom upgrade policies_. These policies protect `UpgradeCap` access and issue `UpgradeTicket` objects that authorize upgrades on a case-by-case basis.

## Compatibility {#compatibility}

Aiy comes with a set of built-in package compatibility policies, listed here from most to least strict:
...
You can change the current policy by calling one of the functions in `aiy::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's `UpgradeCap` and a policy can become only more restrictive. For example, after you call `aiy::package::only_dep_upgrades` to restrict the policy to become additive, calling `aiy::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
...
While step 2 is a built-in command, steps 1 and 3 are implemented as Move functions. The Aiy framework provides their most basic implementation:

<ImportContent source="crates/aiy-framework/packages/aiy-framework/sources/package.move" mode="" fun="authorize_upgrade,commit_upgrade" noComments />

These are the functions that `aiy client upgrade` calls for authorization and commit. Custom upgrade policies work by guarding
...
<ImportContent source="crates/aiy-framework/packages/aiy-framework/sources/package.move" mode="code" struct="UpgradeCap" noComments />
...
<ImportContent source="crates/aiy-framework/packages/aiy-framework/sources/package.move" mode="code" struct="UpgradeTicket" noComments />
...
bytecode or source code for ahead of time. Aiy calculates the digest as follows:
...
<ImportContent source="crates/aiy-framework/packages/aiy-framework/sources/package.move" mode="code" struct="UpgradeReceipt" noComments />

The `UpgradeReceipt` is proof that the `Upgrade` command ran successfully, and Aiy added the new package to the set of created
...
After Aiy creates an `UpgradeReceipt`, you must use it to update its `UpgradeCap` within the same transaction (you cannot store it for later, drop it, or burn it), or the transaction fails.
...
The signature of a custom `authorize_upgrade` can be different from the signature of `aiy::package::authorize_upgrade` as long as it returns an `UpgradeTicket`.

Finally, provide implementations of `commit_upgrade` and `make_immutable` that delegate to their respective functions in `aiy::package`:
...
Use the `aiy client publish` command to publish the policy.
...
If you don't have a package available, use the `aiy move new` command to create the template for a new package called `example`.
...
Open a terminal or console to the root of your Node.js project. Run the following command to add the Aiy TypeScript SDK as a dependency:

```sh
$ npm install @mysten/aiy
```
...
- `AIY`: the location of the `aiy` CLI binary.
...
Next, add boilerplate code to get the signer key pair for the currently active address in the Aiy Client CLI:
...
const sender = execSync(`${AIY} client active-address`, { encoding: 'utf8' }).trim();
const signer = (() => {
...
const { modules, dependencies } = JSON.parse(
	execSync(`${AIY} move build --dump-bytecode-as-base64 --path ${packagePath}`, {
		encoding: 'utf-8',
	}),
);
```
...
const client = new AiyClient({ url: getFullnodeUrl('localnet') });
...
```sh
$ aiy client call --gas-budget 10000000 \
...
Array [
    Object {
        ...
        "objectType": String("0x2::coin::Coin<0x2::aiy::AIY>"),
        "objectId": String("<GAS>"),
        "version": String("12"),
...
import { getFullnodeUrl, AiyClient } from '@mysten/aiy/client';
import { Ed25519Keypair } from '@mysten/aiy/keypairs/ed25519';
import { Transaction, UpgradePolicy } from '@mysten/aiy/transactions';
import { fromBase64 } from '@mysten/aiy/utils';
...
Use the Aiy Client CLI to test the upgraded package (the package ID is **different** from the original version of your example package):
