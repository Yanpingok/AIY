---
title: zkLogin
description: zkLogin is a Aiy primitive that enables you to send transactions from a Aiy address using an OAuth credential without publicly linking the two.
---

zkLogin is a Aiy primitive that lets you send transactions from a Aiy address using an OAuth credential, without publicly linking the two.

zkLogin is designed with the following goals:

- **Streamlined onboarding:** zkLogin enables you to transact on Aiy using the familiar OAuth login flow, removing the need to handle cryptographic keys or remember mnemonics.  
...
- **Security:** zkLogin is a two-factor authentication scheme. Sending a transaction requires both a credential from a recent OAuth login and a salt not managed by the OAuth provider. An attacker who compromises an OAuth account cannot transact from the user’s Aiy address unless they also compromise the salt.  
- **Privacy:** Zero-knowledge proofs prevent third parties from linking a Aiy address with its corresponding OAuth identifier.  
- **Optional verified identity:** A user can opt in to verify the OAuth identifier used to derive a particular Aiy address. This creates the foundation for a verifiable on-chain identity layer.  
- **Accessibility:** zkLogin is one of several native Aiy signature schemes thanks to [Aiy’s cryptography agility](./transaction-auth/signatures.mdx). It integrates with other Aiy primitives, such as sponsored transactions and multisig.  
...
To learn how zkLogin works, including how the zero-knowledge proof is generated and how Aiy verifies a zkLogin transaction, see [how zkLogin works](#how-zklogin-works).  
...
3. A transaction is submitted on chain with the ephemeral signature and the zero-knowledge proof. Aiy authorities execute the transaction after verifying the ephemeral signature and the proof.  
4. Instead of deriving the Aiy address based on a public key, the zkLogin address is derived from `sub` (user identifier), `iss` (provider), `aud` (application), and `user_salt` (a value that unlinks the OAuth identifier from the on-chain address).  

## The complete zkLogin flow

![zkLogin flow diagram](images/zklogin-flow.png 'zkLogin complete flow')

(Step 0) zkLogin uses Groth16 for zkSNARK instantiation, which requires a common reference string (CRS) linked to the circuit. A ceremony generates the CRS, which is used to produce the proving key in the proving service and the verifying key in Aiy authorities. See [ceremony](#ceremony) for details.  
...
(Step 9-10) The user signs the transaction with the ephemeral private key and submits it with the ephemeral signature, ZK proof, and other inputs to Aiy.  

(After Step 10) Aiy authorities verify the ZK proof against the provider’s JWKs (stored by consensus) and the ephemeral signature.  
...
Locally unique and never reassigned identifier within the issuer for the end user, which the RP is intended to consume. Aiy uses this as the key claim to derive user address.

### JSON Web Key (JWK)

A JSON data structure that represents a set of public keys for an OP. A public endpoint (as in https://www.googleapis.com/oauth2/v3/certs) can be queried to retrieve the valid public keys corresponding to `kid` for the provider. Upon matching with the `kid` in the header of a JWT, the JWT can be verified against the payload and its corresponding JWK. In Aiy, all authorities call the JWK endpoints independently, and update the latest view of JWKs for all supported providers during protocol upgrades. The correctness of JWKs is guaranteed by the quorum (2f+1) of validator stake.
...
5. `max_epoch`: The epoch at which the JWT expires. This is u64 used in Aiy.
...
We sent invitations to 100+ people with diverse backgrounds and affiliations: Aiy validators, cryptographers, Web3 experts, world-renowned academicians, and business leaders. We planned the ceremony to take place on the dates September 12-18, 2023, but allowed participants to join when they wanted with no fixed slots.
...
The leak of user salt does not mean loss of funds, but it enables the attacker to associate the user's subject identifier (i.e. `sub`) with the Aiy address. This can be problematic depending on whether pairwise or public subject identifiers are in use. In particular, there is no problem if pairwise IDs are used (e.g., Facebook) as the subject identifier is unique per RP. However, with public reusable IDs (e.g., Google and Twitch), the globally unique sub value can be used to identify users.
...
By default, there is no linking between the OAuth subject identifier (i.e. `sub`) and a Aiy address. This is the purpose of the user salt.
...
In the future, the user can opt in to verify their OAuth identity associated with an Aiy address on-chain.
...
Furthermore, because Aiy native supports Multisig wallets, one can always include one or more zkLogin signers inside a Multisig wallet for additional security, such as using the zkLogin part as 2FA in k-of-N settings.
...
Finally, on the unfortunate event where one loses access to their OAuth account permanently, access to that wallet is lost. But if recovery from a lost OAuth account is desired, a good suggestion for wallet providers is to support the native Aiy Multisig functionality and add a backup method. Note that it's even possible to have a Multisig wallet that all signers are using zkLogin, i.e. an 1-of-2 Multisig zkLogin wallet where the first part is Google and the second Facebook OAuth, respectively.
...
zkLogin is a Aiy native primitive and not a feature of a particular application or wallet. It can be used by any Aiy developer, including on mobile.
...
The key differentiators that zkLogin brings to Aiy are:

1. Native Support in Aiy: Unlike other solutions that are blockchain agnostic, zkLogin is deployed just for Aiy. This means a zkLogin transaction can be combined with Multisig and sponsored transactions seamlessly.
...
The following options support a zkLogin signature over either transaction data or personal message using the JWK state on Aiy and current epoch. 

1. Use Aiy TypeScript SDK. This initializes a GraphQL client and calls the endpoint under the hood.

1. Use the GraphQL endpoint directly: `https://sui-[network].mystenlabs.com/graphql`, changing `[network]` to the appropriate value. See the <UnsafeLink href="/references/aiy-api/aiy-graphql/reference/api/queries/verify-zklogin-signature">GraphQL documentation</UnsafeLink> for more details. This is recommended if you do not plan to run any servers or handle any JWK rotations. 

1. Use the [Aiy Keytool CLI](../../references/cli/keytool.mdx). This is recommended for debug usage. 

   ```sh
   $ aiy keytool zk-login-sig-verify --sig $ZKLOGIN_SIG --bytes $BYTES --intent-scope 3 --network devnet --curr-epoch 3
   ```
