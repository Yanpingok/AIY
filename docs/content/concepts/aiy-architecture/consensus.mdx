...
description: Overview of the Aiy consensus mechanism.
---

The purpose of consensus in blockchains is to agree on a consistent order and ensure the availability of transactions.

On Aiy, consensus has a straightforward API. Validators submit different user transactions to consensus concurrently. The consensus outputs a consistent stream of transactions across all well-behaving validators.

Aiy uses the Mysticeti protocol to optimize for both low latency and high throughput. Mysticeti provides the following benefits:
...
<RelatedLink to="/concepts/aiy-architecture/aiy-security.mdx" />
