...
description: The GraphQL RPC Beta service offers a structured way for your clients to interact with data on the Aiy blockchain. It accesses data processed by a general-purpose indexer and can connect to an archival store for historical network state.
beta: devnet, testnet, mainnet
keywords: [ indexer, archival store, graphql, custom indexer ]
---

The GraphQL and `aiy-indexer-alt` (Indexer) stack are part of the [Aiy data access infrastructure](../guides/developer/getting-started/data-serving.mdx). The stack provides access to on-chain data through a high-performance GraphQL service backed by a scalable and general-purpose indexer. This stack is optimized for developers who need flexible queries, structured output, and historical access to data (with configurable retention) across the Aiy network.
...
The general-purpose indexer includes configurable checkpoint-based pipelines that extract data from the Aiy remote checkpoint store and full nodes. The pipelines write processed data to a database optimized for GraphQL query access.
...
- **General-purpose Indexer:** Ingests and transforms Aiy checkpoint data using configurable and parallel pipelines, and writes it into a Postgres-compatible database. Can be configured to use the Aiy remote checkpoint store and a full node as its sources.
...
- **GraphQL service:** Serves structured queries over indexed data. Follows the [GraphQL specification](https://graphql.org/) and the supported schema is documented in the [GraphQL API reference](/references/aiy-graphql). Also take a look at the [getting started guide](/guides/developer/getting-started/graphql-rpc).
...
The GraphQL schema is defined in the [GraphQL reference](/references/aiy-graphql). You can explore supported types and fields there, use the GraphiQL IDE to test queries, and read documentation on the up-to-date schema.
...
<ImportContent source="crates/aiy-indexer-alt-schema/src/schema.rs" mode="code" />
</details>
...
Current implementation supports [GCP Bigtable](https://cloud.google.com/bigtable) which is a highly scalable and performant data store. If you plan to operate your own archival store, refer to `aiy-kvstore` and `aiy-kv-rpc` for indexer setup and RPC service implementation respectively. For the indexer setup, make sure to use the [custom indexing framework](../guides/developer/advanced/custom-indexer). If you're interested in contributing support for other scalable data stores, reach out on GitHub by creating a new issue.

<details>
<summary>
`main.rs` in `aiy-kvstore`
</summary>
<ImportContent source="crates/aiy-kvstore/src/main.rs" mode="code" />
</details>

<details>
<summary>
`main.rs` in `aiy-kv-rpc`
</summary>
<ImportContent source="crates/aiy-kv-rpc/src/main.rs" mode="code" />
</details>
...
- Consider offering different tiers of service to meet different developer needs. For example:
    ...
    - A premium tier with full GraphQL / gRPC + Archival Service access, aiyted to apps that need historical lookups.
