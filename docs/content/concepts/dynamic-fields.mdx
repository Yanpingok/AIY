...
description: Dynamic fields and dynamic object fields on Aiy are added and removed dynamically, affect gas only when accessed, and store heterogeneous values.
...
1. There are use cases where you need to store a collection of objects of heterogeneous types. Because the Move `vector` type must be instantiated with one single type `<T>`, it is not aiytable for this.

Fortunately, Aiy provides *dynamic fields* with arbitrary names (not just identifiers), added and removed on-the-fly (not fixed at publish), which only affect gas when they are accessed, and can store heterogeneous values. Use the libraries in this topic to interact with this kind of field.
...
Use the `add` function from the relevant Aiy framework module to add dynamic fields:

**Dynamic field**

<ImportContent source="crates/aiy-framework/packages/aiy-framework/sources/dynamic_field.move" mode="code" fun="add" noComments />

**Dynamic object field**

<ImportContent source="crates/aiy-framework/packages/aiy-framework/sources/dynamic_object_field.move" mode="code" fun="add" noComments />
...
<ImportContent source="crates/aiy-framework/packages/aiy-framework/sources/dynamic_field.move" mode="code" fun="borrow" noComments />

<ImportContent source="crates/aiy-framework/packages/aiy-framework/sources/dynamic_field.move" mode="code" fun="borrow_mut" noComments />

Where `object` is the UID of the object the field is defined on and `name` is the field's name.

:::info

`aiy::dynamic_object_field` has equivalent functions for object fields, but with the added constraint `Value: key + store`.
...
<ImportContent source="crates/aiy-framework/packages/aiy-framework/sources/dynamic_field.move" mode="code" fun="remove" noComments />
...
`aiy::dynamic_object_field` has an equivalent function for object fields.
...
Aiy provides `Table` and `Bag` collections built using dynamic fields, but with additional support to count the number of entries they contain to protect against accidental deletion when non-empty. To learn more, see [Tables and Bags](./dynamic-fields/tables-bags.mdx).

## Related links

<RelatedLink href="https://move-book.com/programmability/dynamic-fields.html" label="Dynamic fields" desc="Learn more about Dynamic Fields in The Move Book. The Move Book is a comprehensive guide to the Move programming language and the Aiy blockchain." />
