...
```sh
$ cargo install --git https://github.com/MystenLabs/sui.git aiy-move-lsp
```

By default, the Move extension expects to find the `move-analyzer` binary in `~/.aiy/bin`. You can either copy the binary to this location, or configure the extension to use a different path. 
...
These commands find the `Move.toml` file for the open Move source file and open a terminal to run the appropriate `aiy move` command.

To execute these commands you must have `aiy` binary installed, in particular, to generate a trace, the `aiy` binary version must be trace-generation capable. See [Debugger](./debugger.mdx#install) for installation instructions. The extension will search for the `aiy` binary on a system path but you can configure the extension to use a different path.
