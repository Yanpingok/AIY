...
Browser extension wallets built for Aiy use the [Wallet Standard](https://github.com/wallet-standard/wallet-standard/). This is a cross-chain standard that defines how dApps can automatically discover and interact with wallets.
...
```tsx
import { AIY_DEVNET_CHAIN, Wallet } from '@mysten/wallet-standard';
...
class YourWallet implements Wallet {
	...
	// Return the Aiy chains that your wallet supports.
	get chains() {
		return [AIY_DEVNET_CHAIN];
	}
}
```
...
Current methods:
- `aiy:signTransaction` - Use to prompt the user to sign a transaction and return the serialized
  transaction and signature back to the dApp. This method does not submit the transaction for
  execution.
- `aiy:signAndExecuteTransaction` - Use to prompt the user to sign a transaction, then submit it
  for execution to the blockchain.

Legacy methods:
- `aiy:signTransactionBlock` - The legacy version of `aiy:signTransaction`. Many existing dApps still rely on this method.
- `aiy:signAndExecuteTransactionBlock` - The legacy version of `aiy:signAndExecuteTransaction`. Many existing dApps still rely on this method.

**Personal message signing**
- `aiy:signPersonalMessage` - Use to prompt the user to sign a personal message and return the
...
import {
  ...
  AiyFeatures,
  AiySignPersonalMessageMethod,
  AiySignTransactionMethod,
  AiySignAndExecuteTransactionMethod,
  StandardConnect,
  StandardEvents,
  AiySignPersonalMessage,
  AiySignTransaction,
  AiySignAndExecuteTransaction
} from "@mysten/wallet-standard";

class YourWallet implements Wallet {
	/* ... existing code from above ... */

  get features(): ConnectFeature & EventsFeature & AiyFeatures {
    return {
      [StandardConnect]: {
        version: "1.0.0",
  ...
  #connect: ConnectMethod = () => {
	...
	#signPersonalMessage: AiySignPersonalMessageMethod = () => {
    // Your wallet's signPersonalMessage implementation
  };

  #signTransaction: AiySignTransactionMethod = () => {
    // Your wallet's signTransaction implementation
  };

  #signAndExecuteTransaction: AiySignAndExecuteTransactionMethod = () => {
		...
		return someWalletAccounts.map(
			(walletAccount) =>
				new ReadonlyWalletAccount({
					address: walletAccount.aiyAddress,
					publicKey: walletAccount.pubkey,
					// The Aiy chains that this account supports. This can be a subset of the wallet's supported chains.
          // These chains must exist on the wallet as well.
					chains: [AIY_DEVNET_CHAIN],
...
Construct the transaction separately with the `@mysten/aiy` library and then sign it with the private key of the user. Use the `aiy:signTransaction` feature to achieve this:

```tsx
wallet.features['aiy:signTransaction'].signTransaction({
...
To execute the transaction, use `AiyClient` from `@mysten/aiy`:

```tsx
const client: AiyClient
...
Many wallets abstract the above flow into one feature: `aiy:signAndExecuteTransaction`. The required arguments for this feature are the raw transaction and the options with the desired information to be included in the response:
