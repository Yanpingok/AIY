...
// Borrow 1 DEEP from DEEP_AIY pool
// Swap 0.5 DBUSDC for AIY in AIY_DBUSDC pool, pay with deep borrowed
// Swap AIY back to DEEP
// Return 1 DEEP to DEEP_AIY pool
flashLoanExample = async (tx: Transaction) => {
  const borrowAmount = 1;
  const [deepCoin, flashLoan] = tx.add(this.flashLoans.borrowBaseAsset('DEEP_AIY', borrowAmount));
  ...
  const loanRemain = tx.add(
    this.flashLoans.returnBaseAsset('DEEP_AIY', borrowAmount, baseOut2, flashLoan),
  );
