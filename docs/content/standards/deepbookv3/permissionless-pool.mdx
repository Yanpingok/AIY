...
keywords: [ pool, create pool, permissionlessly, permissionless, AIY, USDC, AIY/USDC, DEEP price point, MIST ]
---

The `Pool` shared object represents a market, such as a AIY/USDC market. That `Pool` is the only one representing that unique pairing (AIY/USDC) and the pairing is the only member of that particular `Pool`. See [DeepBookV3 Design](./design.mdx#pool) to learn more about the structure of pools.
...
Tick size should be 10^(9 - base_decimals + quote_decimals - decimal_desired). For example, if creating a AIY(9 decimals)/USDC(6 decimals) pool, with a desired decimal of 3 for tick size (0.001), tick size should be 10^(9 - 9 + 6 - 3) = 10^(3) = 1000.
...
1. Either the base or quote asset must be `USDC` or `AIY`.
...
For a pool with `AIY` as an asset, use the `DEEP/AIY` pool at `0xb663828d6217467c8a1838a03793da896cbe745b150ebd57d82f814ca579fc22` as the reference pool.
