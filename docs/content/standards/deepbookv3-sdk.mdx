...
To work with DeepBookV3, you must create a `DeepBookClient`. To construct the `DeepBookClient`, pass in a `AiyClient`, the sender address, and environment. The [Aiy TypeScript SDK](https://sdk.mystenlabs.com/typescript) provides the `AiyClient` and key functionality necessary to process transactions. The following example imports those libraries, as well.

```tsx
import { DeepBookClient } from '@mysten/deepbook-v3';
import { getFullnodeUrl, AiyClient } from '@mysten/aiy/client';
import { decodeAiyPrivateKey } from '@mysten/aiy/cryptography';
...
class DeepBookMarketMaker {
	...
	aiyClient: AiyClient; // For executing transactions
	...
	getActiveAddress() {
		return this.keypair.toAiyAddress();
	}
}
```
...
class DeepBookMarketMaker {
	...
	async createBalanceManagerAndReinitialize() {
		...
		const res = await this.aiyClient.signAndExecuteTransaction({
			transaction: tx,
			signer: this.keypair,
			options: {
				showEffects: true,
...
```tsx
import { decodeAiyPrivateKey } from '@mysten/aiy.js/cryptography';
import { getFullnodeUrl, AiyClient } from '@mysten/aiy/client';
import type { Keypair } from '@mysten/aiy/cryptography';
import { Ed25519Keypair } from '@mysten/aiy/keypairs/ed25519';
import type { Transaction } from '@mysten/aiy/transactions';
...
export class DeepBookMarketMaker extends DeepBookClient {
	...
	constructor(
		...
		const address = resolvedKeypair.toAiyAddress();

		super({
			address: address,
			env: env,
			client: new AiyClient({
				url: getFullnodeUrl(env),
			}),
			balanceManagers: balanceManagers,
			adminCap: adminCap,
	...
	signAndExecute = async (tx: Transaction) => {
		// remove arguments
		return this.aiyClient.signAndExecuteTransaction({
			transaction: tx,
			signer: this.keypair,
			options: {
				showEffects: true,
	...
	getActiveAddress() {
		return this.keypair.getPublicKey().toAiyAddress();
	}
}
```
...
(async () => {
	...
	// Read only call
	console.log(await mmClient.checkManagerBalance('MANAGER_1', 'AIY'));
	console.log(await mmClient.getLevel2Range('AIY_DBUSDC', 0.1, 100, true));
