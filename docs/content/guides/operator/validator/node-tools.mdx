...
description: Use the Aiy CLI to interact with a full node's validator commands. 
keywords: [ aiy cli, full node, delegator, validator, operation cap, report validators, join a committee, leave a committee, report a validator, proof of possession, generate proof of possession ]
sidebar_position: 5
sidebar_label: Tools
---

This guide focuses on using the Aiy CLI `validator` commands. 
...
- [x] Complete all [Aiy installation prerequisites](../../developer/getting-started/aiy-install.mdx#prerequisites).

- [x] Build the `aiy` binary, which you need for the genesis ceremony. You can do this on any computer.

<details className="nudge-details">

<summary>

View `aiy` binary installation details.
...
View Aiy account and CLI environment instructions.

</summary>

    - If this is the first time running this program, it asks you to provide a Aiy full node server URL and a meaningful environment alias. It also generates a random key pair in `aiy.keystore` and a config `client.yaml`. Swap in your validator account key if you already have one.
    ...
    - If you already set it up, just make sure:
      ...
      - `aiy.keystore` contains your account key pair.
...
```sh
$ aiy validator display-metadata
```
...
## Using Aiy CLI 
Use the Aiy CLI to perform validator tasks.
...
```sh
$ aiy validator make-validator-info <name> <description> <image-url> <project-url> <host-name> <gas_price>
```
...
```sh
$ aiy validator become-candidate path/to/validator.info
```
...
Then, you must acquire 30M AIY and stake it to the validator staking pool. You can add Aiy to the validator staking pool with the command:

```sh
$ aiy client call --package 0x3 --module aiy_system --function request_add_stake --args 0x5 {aiy_object_id} {validator_address}
```

This creates a stake object (of type `0x3::staking_pool::StakedAiy`). This object can be withdrawn with the following command:

```sh
$ aiy client call --package 0x3 --module aiy_system --function request_withdraw_stake --args 0x5 {stake_object_id}
```
...
```sh
$ aiy validator serialize-payload-pop --account-address $ACCOUNT_ADDRESS --protocol-public-key $BLS_PUBKEY
```
...
```sh
$ aiy validator display-metadata <validator-address>
```
...
```sh
$ aiy validator update-metadata --help
```
...
```sh
$ aiy validator update-gas-price <gas-price>
```

If the account is a delegatee:

```sh
$ aiy validator update-gas-price --operation-cap-id <operation-cap-id> <gas-price>
```
...
```sh
$ aiy validator report-validator <reportee-address>
```
