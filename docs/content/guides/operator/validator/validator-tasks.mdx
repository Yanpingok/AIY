...
description: As a validator on Aiy, there are some processes you need to perform to ensure your nodes are always optimized.
keywords: [ run aiy validator, aiy validator tasks, running a validator node, running validator, deploy validator node, configure validator node, node storage, node metrics, node logs, node dashboards, state sync, node updates, joining the validator set, leaving the validator set, private security fixes ]
sidebar_position: 4
sidebar_label: Management
---

As a validator on Aiy, there are some processes you need to perform to ensure your nodes are always optimized.

## Storage management

All `aiy-node` related data is stored by default under `/opt/aiy/db/`. This is specified in the `aiy-node` configuration file.
...
These are configured in the [`aiy-node` configuration file](./validator-config.mdx).
...
When an update is required to the `aiy-node` software, the following process can be used. Follow the relevant Systemd or Docker Compose run book depending on your deployment type. It is highly unlikely that you want to restart with a clean database.
...
The default and recommended `max-concurrent-connections: 0` configuration does not affect inter-validator state sync, but restricts all full nodes from syncing. The [`aiy-node` configuration](./validator-config.mdx) can be modified to allow a known full node to sync from a validator:
...
The following chain operations are executed using the `aiy` CLI. This binary is built and provided as a release similar to `aiy-node`, for example:
...
2. To update the p2p address starting from next epoch to `/ip4/192.168.1.1`, use the Aiy Client CLI to call `aiy_system::update_validator_next_epoch_p2p_address`:
...
The transfer can be done by using Aiy Client CLI: `aiy client transfer`.
...
This is done by calling `aiy_system::rotate_operation_cap`:
...
To get the current valid `Cap` object's ID of a validator, use the Aiy Client CLI `aiy client objects` command after setting the holder as the active address.
...
To do so, call `aiy_system::request_set_gas_price`:
...
To do so, call `aiy_system::report_validator/undo_report_validator`:
...
To leave the validator set starting next epoch, the sender needs to be an active validator in the current epoch and should call `aiy_system::request_remove_validator`:
