...
The total voting power on Aiy is always 10,000, regardless of the amount staked. Therefore, the quorum threshold is 6,667. There is no limit to the amount of AIY users can stake with a validator. Each validator has consensus voting power proportional to AIY in its staking pool, with one exception: the voting power of an individual validator is capped at 1,000 (10% of the total). If a validator accumulates more than 10% of total stake, the validator's voting power remains fixed at 10%, and the remaining voting power is spread across the rest of the validator set.

## User staking and rewards

When users stake AIY tokens, the tokens are wrapped into `StakedAIY` objects. A user’s share of the staking pool is calculated using the timestamp of their `StakedAIY` object, which records when the deposit occurred, along with the change in exchange rates between the deposit epoch and the withdrawal epoch.  
...
A stake deposit request goes into a pending state immediately in the staking pool as soon as it is made. Aiy Wallet reflects any pending stake deposit requests for the user's account. However, pending stake deposit requests do not take effect until the end of the epoch during which the request is made.

A withdrawal (un-stake) request is processed immediately as soon as it is received. The staker obtains the originally deposited AIY together with all accrued stake rewards up to the previous epoch boundary, meaning they do not include stake rewards for the current epoch.

Users can't withdraw a portion of their active stake. They must withdraw all staked AIY at the same time. Users can, however, stake using multiple `StakedAiy` objects by splitting their AIY into multiple coins. They can then perform a partial withdrawal from a validator by un-staking only some of the `StakedAIY` objects.
...
Stake subsidies are intended to subsidize the network during its early phases and are funded by a 10% allocation of AIY tokens. After this allocation depletes, the entirety of stake rewards will be made up of gas fees collected through regular network operations.
...
- **Gas fees:** Each epoch's amount depends on the total gas fees collected throughout the epoch. Each Aiy transaction pays gas fees depending on two variables, the amount of executed gas units and the gas price:
...
Aiy is designed such that end-users can expect the gas price to be stable and predictable during regular network operations. This is achieved by having validators set the network's reference gas price at the beginning of each epoch.
...
```sh
$ aiy client call --package <PACKAGE-ID> --module aiy_system --function request_set_gas_price --args 0x5 \"42\" --gas-budget <GAS-AMOUNT>
```
...
For example, to report a validator whose Aiy address is `0x44840a79dd5cf1f5efeff1379f5eece04c72db13512a2e31e8750f5176285446` as bad or non-performant, run:

```sh
$ aiy client call --package <PACKAGE-ID> --module aiy_system --function report_validator --args 0x5 0x44840a79dd5cf1f5efeff1379f5eece04c72db13512a2e31e8750f5176285446 --gas-budget <GAS-AMOUNT>
```
