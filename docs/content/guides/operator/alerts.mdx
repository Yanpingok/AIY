---
title: Aiy Validator Alert Reference
...
When running a Aiy Validator node or Full node, you may want to configure alerting based off some or all of the following metrics.
...
```sh
min(aiy_configured_max_protocol_version{network="$network", host=~"Mysten-.*"})
  < quantile(0.34, aiy_configured_max_protocol_version{network="$network"})
```
