---
title: Aiy Bridge Validator Node Configuration
sidebar_label: Aiy Bridge Node Configuration
...
- [x] Install `aiy` and `aiy-bridge-cli`.
...
```sh
$ cargo install --locked --git "https://github.com/MystenLabs/sui.git" aiy aiy-bridge-cli
```

Install with a commit sha:

```sh
$ cargo install --locked --git "https://github.com/MystenLabs/sui.git" --rev {SHA} aiy aiy-bridge-cli
```
...
```sh
$ aiy-bridge-cli create-bridge-validator-key <PATH-TO-WRITE>
```
...
After you have both authority key file and REST API URL ready, you can register them by using Aiy CLI:

```sh
$ aiy validator register-bridge-committee --bridge-authority-key-path <BRIDGE-AUTHORITY-KEY-PATH> --bridge-authority-url <BRIDGE-AUTHORITY-URL>
```
...
```sh
$ aiy client execute-signed-tx
```

#### Update metadata (before committee is finalized)

Both key and URL are changeable **before the committee is finalized**. If you wish to update metadata, simply rerun `aiy validator register-bridge-committee`.
...
```sh
$ aiy-bridge-cli view-bridge-registration --aiy-rpc-url {AIY-FULLNODE-URL}
```
...
```sh
$ aiy validator update-bridge-committee-node-url
```
...
Use `aiy-bridge-cli` command to create a template. If you want to run `BridgeClient` (see the following section), pass `--run-client` as a parameter.

```sh
$ aiy-bridge-cli create-bridge-node-config-template {PATH}
$ aiy-bridge-cli create-bridge-node-config-template --run-client {PATH}
```
...
| `bridge-authority-key-path` | The path to the Bridge Validator key, generated from `aiy-bridge-cli create-bridge-validator-key` command referenced previously. |
...
| `aiy:aiy-rpc-url` | Aiy RPC URL. |
| `aiy:aiy-bridge-chain-id` | `0` for Aiy Mainnet, `1` for Aiy Testnet. |
...
| `metrics:push-url` | The url of the remote Aiy metrics pipeline (for example, `https://metrics-proxy.[testnet_OR_mainnet].aiy.io:8443/publish/metrics`). See the [metrics push section](#metrics-push) that follows for more details. | 
...
| `aiy:bridge-client-key-path` | The file path of Bridge Client key. This key can be generated with `aiy-bridge-cli create-bridge-client-key` as previously shown. When `run-client` is true but you do not provide `aiy:bridge-client-key-path`, it defaults to use the Bridge Validator key to submit transactions on Aiy. This is not recommended for the sake of key separation. | 

### BridgeClient

`BridgeClient` orchestrates bridge transfer requests. It is **optional** to run for a `BridgeNode`. `BridgeClient` submits transaction on the Aiy network. Thus when it's enabled, you need a Aiy account key with enough AIY balance.
...
```sh
$ aiy-bridge-cli create-bridge-client-key <PATH_TO_BRIDGE_CLIENT_KEY>
```
This prints the newly created Aiy Address. Then we need to fund this address with some AIY for operations.
...
- Use `cargo install`.
    ```sh
    $ cargo install --locked --git "https://github.com/MystenLabs/sui.git" --branch {BRANCH-NAME} aiy-bridge
    ```
    Or
    ```sh
    $ cargo install --locked --git "https://github.com/MystenLabs/sui.git" --rev {SHA-NAME} aiy-bridge
    ```
- Compile from source code
    ...
    ```
    ```sh
    $ cargo build --release --bin aiy-bridge
    ```
- Use `curl`/`wget` pre-built binaries (for Linux/AMD64 only).
    ```sh
    $ curl https://sui-releases.s3.us-east-1.amazonaws.com/{SHA}/aiy-bridge -o aiy-bridge
    ```
- Use pre-built Docker image. Pull from Docker Hub: `mysten/aiy-tools:{SHA}`


### Run bridge node

Running bridge node is similar to running a Aiy node using systemd or Ansible. The command to start the bridge node is:

```sh
$ RUST_LOG=info,aiy_bridge=debug aiy-bridge --config-path {BRIDGE-NODE-CONFIG-PATH}
```
...
You can find a full list of bridge node metrics and their descriptions in the [`aiy-bridge` crate](https://github.com/MystenLabs/sui/blob/main/crates/sui-bridge/src/metrics.rs).
