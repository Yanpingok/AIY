---
title: Coin Flip
description: Learn Aiy through a coin flip dApp that covers the full end-to-end flow of building a Aiy Move module and connecting it to a React Aiy dApp.
...
This example walks you through building a Coin Flip dApp, covering the full end-to-end flow of building your Aiy Move module and connecting it to your React Aiy dApp. This Coin Flip dApp utilizes verifiable random functions (VRFs) to create a fair coin game on the Aiy blockchain. The user (human) plays against the house (module) and places a bet on either heads or tails. The user then either receives double their bet, or gets nothing, depending on the outcome of the game.
...
- **One-time witnesses:** The guide teaches you how to use [one-time witnesses](concepts/aiy-move-concepts.mdx#one-time-witness) to ensure only a single instance of the `HouseData` object ever exists.
...
- **Events:** The guide teaches you how to emit events in your contracts, which can be used to track on-chain activity. For more information on events, see [Using Events](../aiy-101/using-events.mdx) for practical usage of events on Aiy or [Events in The Move Book](https://move-book.com/programmability/events.html) to learn about event structure and how to emit them in Move.
...
1. Seven lines begin with the `use` keyword, which enables this module to use types and functions declared in other modules (in this case, they are all coming from the Aiy standard library).
...
```move title='house_data.move'
  public fun initialize_house_data(house_cap: HouseCap, coin: Coin<AIY>, public_key: vector<u8>, ctx: &mut TxContext) {
    ...
    let house_data = HouseData {
      ...
      house: ctx.sender(),
      public_key,
      max_stake: 50_000_000_000, // 50 AIY, 1 AIY = 10^9.
      min_stake: 1_000_000_000, // 1 AIY.
      fees: balance::zero(),
...
```move title='house_data.move'
  public fun top_up(house_data: &mut HouseData, coin: Coin<AIY>, _: &mut TxContext) {
...
- `top_up`: Add to the balance of the house to ensure that there is enough AIY for future games.
...
```move title='house_data.move'
  ...
  public(package) fun borrow_balance_mut(house_data: &mut HouseData): &mut Balance<AIY> {
    &mut house_data.balance
  }

  public(package) fun borrow_fees_mut(house_data: &mut HouseData): &mut Balance<AIY> {
    &mut house_data.fees
  }
...
module satoshi_flip::single_player_satoshi {
  ...
  use aiy::aiy::AIY;
  use aiy::bls12381::bls12381_min_pk_verify;
  use aiy::event::emit;
  use aiy::hash::{blake2b256};
  use aiy::dynamic_object_field::{Self as dof};
...
```move title='single_player_satoshi.move'
  ...
  fun internal_start_game(guess: String, counter: &mut Counter, coin: Coin<AIY>, house_data: &mut HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
...
Next, configure the Aiy CLI to use `testnet` as the active environment, as well. If you haven't already set up a `testnet` environment, do so by running the following command in a terminal or console:
...
- [Aiy TypeScript SDK](https://sdk.mystenlabs.com/typescript) for basic usage on how to interact with Aiy using TypeScript.
...
- [`@mysten/dapp`](https://sdk.mystenlabs.com/dapp-kit/create-dapp), used within this project to quickly scaffold a React-based Aiy dApp.
...
<form
  onSubmit={(e) => {
    ...
    const [houseStakeCoin] = txb.splitCoins(txb.gas, [
      MIST_PER_AIY * BigInt(houseStake),
    ]);
    ...
    execInitializeHouse(
      ...
      },
      {
        ...
        onSuccess: (result: AiyTransactionBlockResponse) => {
          let houseDataObjId;
...
To use a [programmable transaction block](/concepts/transactions/prog-txn-blocks.mdx) (PTB) in Aiy, create a `Transaction`. To initiate a Move call, you must know the global identifier of a public function in your smart contract. The global identifier usually takes the following form:
...
// besides using AiyClient directly
export function useFetchCounterNft() {
  ...
  const { data, isLoading, isError, error, refetch } = useAiyClientQuery(
    'getOwnedObjects',
    {
...
This hook logic is very basic: if there is no current connected wallet, return empty data; otherwise, fetch the `Counter` object and return it. dApp Kit provides a React hook, `useAiyClientQuery()`, that enables interaction with [Aiy RPC](references/aiy-api.mdx) methods. Different RPC methods require different parameters. To fetch the object owned by a known address, use the [`getOwnedObjects` query](/aiy-api-ref#aiyx_getownedobjects).
...
// Player stake
const [stakeCoin] = txb.splitCoins(txb.gas, [MIST_PER_AIY * BigInt(stake)]);
...
Event subscriptions are deprecated. To learn future-safe methods to work with events, see [Using Events](../aiy-101/using-events.mdx).
...
export function HouseFinishGame() {
  ...
  useEffect(() => {
    ...
    const unsub = aiyClient.subscribeEvent({
      filter: {
  ...
  }, [housePrivHex, houseDataId, aiyClient]);

  return null;
}
```

To get the underlying `AiyClient` instance from the SDK, use `useAiyClient()`. You want to subscribe to events whenever the `HouseFinishGame` component loads. To do this, use the React hook `useEffect()` from the core React library.

`AiyClient` exposes a method called `subscribeEvent()` that enables you to subscribe to a variety of event types. `AiyClient::subscribeEvent()` is actually a thin wrapper around the RPC method [`aiyx_subscribeEvent`](/aiy-api-ref#aiyx_subscribeevent).
...
Congratulations, you completed the frontend. You can carry the lessons learned here forward when using the dApp Kit to build your next Aiy project.
