...
This example walks you through building a basic distributed counter app, covering the full end-to-end flow of building your Aiy Move module and connecting it to your React Aiy dApp. The app allows users to create counters that anyone can increment, but only the owner can reset. 
...
Your smart contract is complete. You should be able to run the `aiy move build` command from `react-e2e-counter/move/counter` and receive a response similar to the following:
```sh
...
BUILDING counter
```
You always run `aiy move build` at the same level as your `Move.toml` file. After a successful build, you now have a `build` folder inside `react-e2e-counter/move/counter`.
...
The `create` function now creates a new Aiy `Transaction` and calls the `create` function from your Move module. The PTB is then signed and executed via the `useSignAndExecuteTransaction` hook. The `onCreated` callback is called with the new counter's ID when the transaction is successful.
...
```tsx
  ...
  import { isValidAiyObjectId } from "@mysten/aiy/utils";
  ...
  function App() {
    ...
    const [counterId, setCounter] = useState(() => {
      ...
      return isValidAiyObjectId(hash) ? hash : null;
    });
...
Add the following code to your `src/Counter.tsx` file:
  ...
  ```tsx
    ...
    export function Counter({ id }: { id: string }) {
      const currentAccount = useCurrentAccount();
      const { data, isPending, error, refetch } = useAiyClientQuery("getObject", {
        id,
        options: {
          showContent: true,
          showOwner: true,
    ...
    function getCounterFields(data: AiyObjectData) {
