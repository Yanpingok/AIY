...
description: An app that performs atomic swaps on Aiy. Atomic swaps are similar to escrows but without requiring a trusted third party.
effort: large
...
This guide demonstrates how to make an app that performs atomic swaps on Aiy. Atomic swaps are similar to escrows but without requiring a trusted third party.
...
- You have the latest version of Aiy installed. If you run `aiy --version` in your terminal or console, it responds with the currently installed version.
- Your active environment is pointing to the expected network. Run `aiy client active-env` to make sure. If you receive a warning about a client and server API version mismatch, update Aiy using the version in the relevant branch (`mainnet`, `testnet`, `devent`) of the Aiy repo.
- Your active address has AIY. Run `aiy client balance` in your terminal or console. If there is no balance, [acquire AIY](../getting-started/get-coins.mdx) from the faucet (not available in Mainnet).
...
In this part of the guide, you write the Move contracts that perform the trustless swaps. The guide describes how to create the package from scratch, but you can use a fork or copy of the example code in the Aiy repo to follow along instead. See [Write a Move Package](../first-app/write-package.mdx) to learn more about package structure and how to use the Aiy CLI to scaffold a new project.
...
If you are targeting a network other than Testnet, be sure to update the `rev` value for the Aiy dependency.
...
The test starts with a helper function for creating an object, it doesn't matter what kind of object it is, as long as it has the `store` ability. The test uses `Coin<AIY>`, because it comes with a `#[test_only]` function for minting:
...
- The `test_scenario` module is used to provide access to a `&mut TxContext` in the test (necessary for creating new objects). Tests that don't need to simulate multiple transactions but still need access to a `TxContext` can use `aiy::tx_context::dummy` to create a test context instead.
...
From your `escrow` folder, run `aiy move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass:
...
`Locked` and `Key` objects are typically owned by the transaction sender, and so can be queried through the Aiy RPC, but `Escrow` objects are shared, and it is useful to be able to query them by their sender and recipient (so that users can see the trades they have offered and received).
...
<ImportContent source="examples/trading/contracts/escrow/sources/shared.move" mode="code" dep="aiy::event"  />
...
- Guide: [Using Events](../aiy-101/using-events.mdx)

:::

:::checkpoint

You now have `shared.move` and `locked.move` files in your `sources` folder. From the parent `escrow` folder, run `aiy move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass: 
...
<ImportContent source="initialize-aiy-client-cli.mdx" mode="snippet" />

Next, configure the Aiy CLI to use `testnet` as the active environment. 
...
```sh
$ aiy client new-env --alias testnet --rpc https://fullnode.testnet.sui.io:443
```
...
Before being able to publish your package to Testnet, you need Testnet AIY tokens. To get some, run the following command: 

```sh
$ aiy client faucet
```

For other ways to get AIY in your Testnet account, see [Get AIY Tokens](/guides/developer/getting-started/get-coins).

Now that you have an account with some Testnet AIY, you can deploy your contracts. 

There are some helper functions to publish the smart contracts so you can create some demo data (for Testnet). The helper function to publish the smart contracts expects built smart contracts in both the `escrow` and `demo` directories. Run `aiy move build` in both directories, if necessary. Be sure to update the Aiy dependency in the manifest to point to the correct source based on your environment. 
...
- [Aiy TypeScript SDK](https://sdk.mystenlabs.com/typescript). For basic usage on how to interact with Aiy with TypeScript.
- [Aiy dApp Kit](https://sdk.mystenlabs.com/dapp-kit). To learn basic building blocks for developing a dApp in the Aiy ecosystem with React.js.
- [`@mysten/dapp`](https://sdk.mystenlabs.com/dapp-kit/create-dapp). This is used within this project to quickly scaffold a React-based Aiy dApp.
...
`AiyObjectDisplay.tsx`

</summary>

<ImportContent source="examples/trading/frontend/src/components/AiyObjectDisplay.tsx" mode="code"  />
...
Note that `dApp Kit` provides a set of hooks for making query and mutation calls to the Aiy blockchain. These hooks are thin wrappers around query and mutation hooks from `@tanstack/react-query`.
...
Use the `useAiyClient()` hook from `dApp Kit` to retrieve the Aiy client instance configured in `src/main.tsx`. The `useSignTransaction()` function is another hook from `dApp kit` that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, use the `executeTransaction()` on the Aiy client instance of the Aiy TypeScript SDK.
...
As previously mentioned, this example uses `@tanstack/react-query` to query, cache, and mutate server state. Server state is data only available on remote servers, and the only way to retrieve or update this data is by interacting with these remote servers. In this case, it could be from an API or directly from Aiy blockchain RPC.

When you execute a transaction call to mutate data on the Aiy blockchain, use the `useMutation()` hook. The `useMutation()` hook accepts several inputs, however, you only need two of them for this example. The first parameter, `mutationFn`, accepts the function to execute the main mutating logic, while the second parameter, `onSuccess`, is a callback that runs when the mutating logic succeeds.
...
This component uses tanstack's `useInfiniteQuery` instead of `useAiyClientInfiniteQuery` since the data is being fetched from the example's API rather than Aiy. 
