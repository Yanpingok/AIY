---
title: Blackjack
description: Learn Aiy using an example implementation of the popular casino game Blackjack.
...
The following documentation goes through an example implementation of the popular casino game Blackjack on Aiy. This guide walks through its components, providing a detailed look at the module's functions, structures, constants, and their significance in the overall gameplay mechanism.
...
For more details on building a backend and deploying to Aiy, check out the [Coin Flip app example](./coin-flip.mdx).
...
- [`services/`](https://github.com/MystenLabs/blackjack-sui/blob/main/app/src/app/api/services): The core logic of the backend, that signs and executes the transactions on the Aiy blockchain.
...
An interesting aspect of developing a dApp on Aiy, that is coupled to using a full node with/without a load balancer, and requires attention by the developer, is the occurrence of read-after-write and write-after-write cases.
...
- **Blockchain-based logic:** Both games are built on Aiy, leveraging its capabilities for decentralized applications. The core game logic for each resides in Move modules, ensuring secure and verifiable gameplay.
...
The complete app example can be found in [the `blackjack-aiy` repo](https://github.com/MystenLabs/blackjack-sui).
