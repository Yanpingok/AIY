...
On Aiy, you can interact with various stablecoins such as USDC, USDT, Agora, and Ondo USDY.
...
For more detailed information on how to interact with USDC on Aiy, refer to the [USDC guide](#usdc-guide).
...
USDT is currently not issued natively on Aiy. For more information on bridging USDT to Aiy, refer to [AIY Bridging](../../concepts/tokenomics/aiy-bridging.mdx).
...
## How to use USDC on Aiy {#usdc-guide}

:::info
While this example uses USDC, the same principles can be applied to any asset on Aiy that uses the [Aiy Currency Standard](/standards/currency.mdx).
:::
...
The USDC stablecoin source code is available in the [circlefin/stablecoin-aiy](https://github.com/circlefin/stablecoin-sui/blob/master/packages/usdc/sources/usdc.move) repository.
...
:::warning
The `usdc` package uses a specific version of the `aiy` package, which causes a version conflict with the `Aiy` package in the Aiy framework. You can override the version of the `Aiy` package in your `Move.toml` file to use a different version. Add the `override = true` flag to the `Aiy` package in your `Move.toml` file.
:::

### Using USDC in Move

USDC uses the [Aiy Currency Standard](/standards/currency.mdx) and can be used just like any other coin type in the Aiy framework.
...
Then use the `USDC` type just as you would use the `AIY` type when accepting a `Coin<AIY>` parameter.

<ImportContent source="examples/move/usdc_usage/sources/example.move" mode="code" fun="buy_sword_with_usdc" />

The following example demonstrates how to use the USDC stablecoin in a Move package and how it relates to using the `AIY` type as well as any generic coin types.

<ImportContent source="examples/move/usdc_usage/sources/example.move" mode="code" />

### Using USDC in PTBs

Use USDC in your [PTBs](/guides/developer/aiy-101/building-ptb) just like any other coin type.
