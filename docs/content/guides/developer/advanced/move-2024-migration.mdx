...
1. Delete your existing `Move.lock` file (if one exists) to make sure you're using the newest `aiy-framework` version.
1. Perform one of the following:
   - Run `aiy move migrate` in the root of your Move project. See [Automatic migration](#automatic-migration).
...
See the getting started guide on [Move IDEs and plugins](../../../references/contribute/aiy-environment.mdx#move-ides-and-plugins) for more information.
...
```move
use aiy::{balance, coin::{Self, Coin}};
```
...
Move 2024 revises how paths and namespaces work compared to legacy Move, toward easing `enum` aliasing in the future. Consider the following snippet from a test annotation in the `aiy_system` library:

```move
use aiy_system::aiy_system;
...
#[expected_failure(abort_code = aiy_system::validator_set::EInvalidCap)]
```

Legacy Move would always treat a three-part name as an address(`aiy_system`), module(`validator_set`), and module member (`EInvalidCap`). Move 2024 respects scope for `use`, so `aiy_system` in the attribute resolves to the module, producing a name resolution error overall.
