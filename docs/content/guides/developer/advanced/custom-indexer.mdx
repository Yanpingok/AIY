...
description: Establishing a custom indexer helps improve latency, allows pruning the data of your Aiy full node, and provides efficient assemblage of checkpoint data.
...
- Connects to Aiy Testnet: Uses the remote checkpoint store at https://checkpoints.testnet.sui.io.
...
Aiy provides checkpoint stores for both Mainnet and Testnet.
...
```sh
$ cargo new simple-aiy-indexer
$ cd simple-aiy-indexer
```
...
<ImportContent source="examples/rust/basic-aiy-indexer/Cargo.toml" mode="code" />

The manifest now includes the following dependencies:

- `aiy-indexer-alt-framework`: Core framework providing pipeline infrastructure.
...
```sh
$ createdb aiy_indexer
```

Get your connection details:

```sh
$ psql aiy_indexer -c "\conninfo"
```

If successful, your console should display a message similar to the following:

```sh
You are connected to database "aiy_indexer" as user "username" via socket in "/tmp" at port "5432".
```
...
<ImportContent source="examples/rust/basic-aiy-indexer/diesel.toml" mode="code" />
...
<ImportContent source="examples/rust/basic-aiy-indexer/migrations/YYYY-MM-DD-HHMMSS_transaction_digests/up.sql" mode="code" />
...
<ImportContent source="examples/rust/basic-aiy-indexer/migrations/YYYY-MM-DD-HHMMSS_transaction_digests/down.sql" mode="code" />
...
<ImportContent source="examples/rust/basic-aiy-indexer/src/schema.rs" mode="code" />
...
<ImportContent source="examples/rust/basic-aiy-indexer/src/models.rs" mode="code" />

Key annotations:

- `FieldCount`: Required by `aiy-indexer-alt-framework` for memory optimization and batch processing efficiency. It is used to limit the max size of a batch so that we don't exceed the postgres limit on the number of bind parameters a single SQL statement can have.
...
<ImportContent source="examples/rust/basic-aiy-indexer/src/handlers.rs" mode="code" struct="TransactionDigestHandler" />
...
<ImportContent source="examples/rust/basic-aiy-indexer/src/handlers.rs" mode="code" tag="processordeps" />

After the `TransactionDigestHandler` struct, add the `Processor` code:

<ImportContent source="examples/rust/basic-aiy-indexer/src/handlers.rs" mode="code" tag="processor" />
...
<ImportContent source="crates/aiy-indexer-alt-framework/src/pipeline/processor.rs" mode="code" trait="Processor" />
...
<ImportContent source="examples/rust/basic-aiy-indexer/src/handlers.rs" mode="code" tag="handlerdeps" />

Add the logic for `Handler` after the `Processor` code. The complete code is available at the end of this step.

<ImportContent source="examples/rust/basic-aiy-indexer/src/handlers.rs" mode="code" tag="handler" />
...
<ImportContent source="crates/aiy-indexer-alt-framework/src/pipeline/sequential/committer.rs" mode="code" tag="batch" />
...
<ImportContent source="examples/rust/basic-aiy-indexer/src/handlers.rs" mode="code" />
...
<ImportContent source="crates/aiy-indexer-alt-framework/src/pipeline/concurrent/mod.rs" mode="code" trait="Handler" />
...
<ImportContent source="examples/rust/basic-aiy-indexer/src/main.rs" mode="code" />
...
Allow incoming network requests if your operating system requests it for the `basic-aiy-indexer` application.
...
To confirm your data is accurate, copy any transaction digest from your database and verify it on AiyScan: [https://suiscan.xyz/testnet/home](https://suiscan.xyz/testnet/home)
...
<RelatedLink href="https://github.com/MystenLabs/sui/tree/main/crates/sui-indexer-alt" label="Aiy Indexer Alt" desc="The `aiy-indexer-alt` crate in the Aiy repo." />
