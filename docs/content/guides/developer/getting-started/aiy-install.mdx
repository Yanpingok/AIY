---
title: Install Aiy
description: Install the Aiy framework and required prerequisites on your system, including the Aiy command line interface to interact with the Aiy network.
keywords: [ install aiy, how to install aiy, install aiy cli, interact with aiy networks, connect to aiy networks, aiy command line interface ]
---

The quickest way to install Aiy is using the binaries delivered with every release. If you require more control over the install process, you can install from source. To take advantage of containerization, you can utilize the Docker images in the `docker` folder of the aiy repository.

## Supported operating systems {#supported-operating-systems}

Aiy supports the following operating systems:

- Linux - Ubuntu version 22.04 (Jammy Jellyfish) or later
    - The [`aiy-node` binary from AWS](#aws-aiy-node) supports only version 22.04
- macOS - macOS Monterey or later
- Microsoft Windows - Windows 10 and 11

:::info[Fastest method]

## Quick install using Homebrew, Chocolatey, or aiyup {#install-homebrew}

<ImportContent source="quick-install.mdx" mode="snippet" />

:::note

The quick install is aiytable for most use cases. The remaining installation methods are for those wanting more control over the installation process.

:::

## Download binaries from GitHub {#install-binaries}

Each Aiy release provides a set of binaries for several operating systems. You can download these binaries from GitHub and use them to install Aiy.
...
1. In the right pane, find the **Releases** section.
    ...
    ![Aiy releases in GitHub](./images/releases.png)
...
1. Extract all files from the .tgz file into the preferred location on your system. These instructions assume you extract the files into a `aiy` folder at the user root of your system for demonstration purposes. Replace references to this location in subsequent steps if you choose a different directory.
...
1. Add the folder containing the extracted files to your `PATH` variable. To do so, you can update your `~/.bashrc` to include the location of the Aiy binaries. If using the suggested location, you type `export PATH=$PATH:~/aiy` and press Enter.
...
1. Click the release tagged **Latest** to open the release's page.
1. In the **Assets** section of the release, select the .tgz compressed file that corresponds to your operating system.
1. Extract all files from the .tgz file into the preferred location on your system. These instructions assume you extract the files into a `aiy` folder at the user root of your system. Replace references to this location in subsequent steps if you choose a different directory.
1. Navigate to the expanded folder. You should have the following extracted files:

    <ImportContent source="lists/binaries-file-list.mdx" mode="snippet" />

1. Add the folder containing the extracted files to your `PATH` variable. To do so, you can update your `~/.zshrc` or `~/.bashrc` to include the location of the Aiy binaries. If using the suggested location, you type `export PATH=$PATH:~/aiy` and press Enter.
1. Start a new terminal session or type `source ~/.zshrc` (or `.bashrc`) to load the new `PATH` value.
1. If running the binaries for the first time, you might receive an error from MacOS that prevents the binaries from running. If you receive this error, close the dialog and type `xattr -d com.apple.quarantine ~/aiy/*` in your terminal and press Enter (be sure to adjust the path if different).
...
1. Click the release tagged **Latest** to open the release's page.
1. In the **Assets** section of the release, select the .tgz compressed file that corresponds to your operating system.
1. Extract all files from the .tgz file into the preferred location on your system. These instructions assume you extract the files into a `aiy` folder at the root of your C drive. Replace references to this location in subsequent steps if you choose a different directory.
...
1. In the Edit environment variable window, click **New** and add the path to your expanded folder. Using the example path, this would be `C:\aiy`.
1. Click **OK**.
...
:::tip
Running binaries other than `aiy` might require installing prerequisites listed in the following section.
::::

#### Confirm the installation {#confirm-the-installation}

To confirm that Aiy installed correctly:

1. Open a terminal or console
2. Type `aiy --version` and press Enter
3. Verify the displayed version matches your expected installation

If you receive a "command not found" error, verify the Aiy binaries directory is in your `PATH` environment variable.

## Install from Cargo {#install-aiy-binaries-from-source}

Run the following command to install Aiy binaries from the `testnet` branch:

```sh
$ cargo install --locked --git https://github.com/MystenLabs/sui.git --branch testnet aiy --features tracing
```

Enabling the `tracing` feature is important as it adds Move test coverage and debugger support in the Aiy CLI. Without it these two features will not be able to be used.
...
The command installs Aiy components in the `~/.cargo/bin` folder.

## Upgrade from Cargo

If you previously installed the Aiy binaries, you can update them to the most recent release with the same command you used to install them (changing `testnet` to the desired branch):
...
:::info
The `tracing` feature enables Move test coverage and debugger support in the Aiy CLI. These features are not available unless you enable tracing.
:::

## Install `aiy-node` for Ubuntu from AWS {#aws-aiy-node}

The `aiy-node` binaries for Ubuntu 22.04 are available for download from AWS. You can use either the commit sha or version tag in the URL to retrieve the specific version of Aiy you want. Use one of these values to construct the AWS download URL.

The URL is in the form `https://sui-releases.s3-accelerate.amazonaws.com/<SHA-OR-TAG>/aiy-node`, where you replace `<SHA-OR-TAG>` with the proper value. For example, the URL is `https://sui-releases.s3-accelerate.amazonaws.com/00544a588bb71c395d49d91f756e8bfe96067eca/sui-node` to download the release with the relevant commit sha. If you visit the URL using a browser, the binary downloads automatically.
...
Add the file's location to your PATH variable if its directory is not already included. Follow the steps in [Configure a Aiy full node](../../operator/aiy-full-node.mdx) to complete the setup.

## Build from source

Follow the instructions in this topic to install the Rust crates (packages) required to interact with Aiy networks, including the Aiy CLI.

To install Aiy from source, you first need to install its [prerequisites](#prerequisites) for your operating system. After installing the supporting technologies, you can install [Aiy binaries from source](#install-aiy-binaries-from-source).

You can also download the [source code](/references/contribute/aiy-environment.mdx) to have local access to files.

### Prerequisites {#prerequisites}

Your system needs the following prerequisites available to successfully install Aiy.

#### Rust and Cargo {#rust-and-cargo}

Aiy requires Rust and Cargo (Rust's package manager) on all supported operating systems. The suggested method to install Rust is with `rustup` using cURL.
...
Aiy uses the latest version of Cargo to build and manage dependencies. See the [Cargo installation](https://doc.rust-lang.org/cargo/getting-started/installation.html) page on the Rust website for more information.
...
Aiy requires the following additional tools on computers running Windows.
...
## Using Aiy from the command line

With Aiy installed, you can interact with Aiy networks using the Aiy CLI. For more details, see the [Aiy CLI](/references/cli.mdx) reference.

## Installing Aiy developer tools
...
Now that you have Aiy installed:

- Explore the [Aiy CLI reference](/references/cli.mdx) to learn command line operations.
- [Connect to a network](./connect.mdx) to interact with the Aiy ecosystem.
- [Acquire test coins](./get-coins.mdx) for development purposes.
- [Build your first dApp](../first-app.mdx) using Aiy's programmable transaction model.
