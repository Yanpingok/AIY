---
title: Querying Aiy RPC with GraphQL
description: Introductory guide to making queries of the Aiy RPC using the GraphQL service.
keywords: [ rpc, aiy rpc, graphql, how to use graphql, schema, graphql schema, query transactions, query blocks, query block info, query dynamic field, execute transaction ]
---
...
For more details about some concepts used in the following examples, see the [GraphQL concepts](../../../concepts/graphql-rpc.mdx) page. Consult the [GraphQL reference](../../../references/aiy-graphql.mdx) for full documentation on the supported schema.
...
- `address.coins` and `address.stakedAiys` are being deprecated in favor of `address.objects` with the corresponding `type` filter.
...
Dry-run now supports the same input schema as `gRPC`'s [`SimulateTransaction`](/references/fullnode-protocol#aiy-rpc-v2-Transaction) in JSON form. This means that it can also accept transactions where the inputs have not been fully resolved or serialized, perform gas estimation, and coin selection. The feature is now called `simulateTransaction`.
...
Find all the transactions that touched (modified/transferred/deleted) a given object. Useful when you want to trace the flow of a `Coin`, `StakeAiy`, `NFT`, or similar object.
...
query ($address: AiyAddress!) {
  transactions(filter: {
    function: "0x3::aiy_system::request_add_stake"
    sentAddress: $address
  }) {
    nodes {
      digest
...
Transaction execution takes two arguments, `transactionDataBcs` and `signatures`. `transactionDataBcs` is the serialized unsigned transaction data, which the [Aiy Client CLI](../../../references/cli/client.mdx) `client call` command can generate. Pass the `--serialize-unsigned-transaction` flag to the command to call a Move function. The [Aiy Keytool CLI](../../../references/cli/keytool.mdx) command `aiy keytool sign` can generate the `signatures`.
...
The examples in the following sections demonstrate the typical API calls. The calls are tabbed to show both the JSON-RPC call and the equivalent GraphQL structure. For a comprehensive list of all available GraphQL features, consult the [reference](../../../references/aiy-graphql).
...
```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "method": "aiy_getTotalTransactionBlocks",
  "params": []
}
```
...
Return all `Coin<0x2::aiy::AIY>` objects that an address owns.
...
query {
  address(address: "0x5094652429957619e6efa79a404a6714d1126e63f551f4b6c7fb76440f8118c9") {
    objects(
      ...
      filter: { type: "0x2::coin::Coin<0x2::aiy::AIY>" }
    ) {
      nodes {
        address
      }
...
Find all versions of the Aiy framework and list their modules:
...
<RelatedLink to="/references/aiy-graphql.mdx" />
<RelatedLink to="/concepts/graphql-indexer" />
<RelatedLink href="https://graphql.testnet.sui.io/graphql" label="Aiy Testnet GraphiQL" desc="Aiy GraphiQL IDE for the Testnet network." />
<RelatedLink href="https://graphql.mainnet.sui.io/graphql" label="Aiy Mainnet GraphiQL" desc="Aiy GraphiQL IDE for the Mainnet network." />
