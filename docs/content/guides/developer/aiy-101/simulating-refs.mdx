...
description: Use the borrow module in the Aiy framework to include objects by reference in your programmable transaction blocks.
keywords: [ borrow module, aiy framework, objects, immutable reference, mutable reference, Aiy API, references, simulate a reference, simulate references ]
---

Everything on the Aiy blockchain is an object. When you develop Move packages for the Aiy network, you're typically manipulating or using on-chain objects in some way through functionality available in the Aiy API. For most API functions, you provide an object by reference.

References are a key construct when programming in Move and on Aiy. Most of the functionality available in the Aiy API takes objects by reference.
