...
description: zkLogin can be integrated into applications deployed on Aiy.
keywords: [ zklogin, how to use zklogin, generate ephemeral key pair, fetch JWT, decode JWT, generate salt, generate user Aiy address, fetch ZK proof, assemble zkLogin signature, oauth, openid ]
---
...
1. The wallet obtains a unique user salt based on a JWT. Use the OAuth subject identifier and salt to compute the zkLogin Aiy address.
...
```sh npm2yarn
$ npm install @mysten/aiy@experimental
```

## Get JWT

1. Generate an ephemeral key pair. Follow the same process as you would generating a key pair in a traditional wallet. See [Aiy SDK](https://sdk.mystenlabs.com/typescript/cryptography/keypairs) for details.
...
```typescript
import { generateNonce, generateRandomness } from '@mysten/aiy/zklogin';

const FULLNODE_URL = 'https://fullnode.devnet.sui.io'; // replace with the RPC URL you want to use
const aiyClient = new AiyClient({ url: FULLNODE_URL });
const { epoch, epochDurationMs, epochStartTimestampMs } = await aiyClient.getLatestAiySystemState();
...
zkLogin uses the user salt to compute the zkLogin Aiy address (see [definition](../../../concepts/cryptography/zklogin.mdx#address-definition)). The salt must be a 16-byte value or a integer smaller than `2n**128n`. There are several options for the application to maintain the user salt:
...
```typescript
import { jwtToAddress } from '@mysten/aiy/zklogin';
...
```typescript
import { getExtendedEphemeralPublicKey } from '@mysten/aiy/zklogin';
...
1. Download the [Groth16 proving key zkey file](https://docs.circom.io/getting-started/proving-circuits/). There are zkeys available for all Aiy networks. See [the Ceremony section](#ceremony) for more details on how the main proving key is generated.
...
const client = new AiyClient({ url: '<YOUR_RPC_URL>' });
...
```typescript
import { genAddressSeed, getZkLoginSignature } from '@mysten/aiy/zklogin';
