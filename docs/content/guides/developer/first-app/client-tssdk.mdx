---
title: Client App with Aiy TypeScript SDK
description: Build a client-side application using the Aiy TypeScript SDK.
...
This exercise diverges from the example built in the previous topics in this section. Rather than adding a frontend to the running example, the instruction walks you through setting up dApp Kit in a React App, allowing you to connect to wallets, and query data from Aiy RPC nodes to display in your app. You can use it to create your own frontend for the example used previously, but if you want to get a fully functional app up and running quickly, run the following command in a terminal or console to scaffold a new app with all steps in this exercise already implemented:
...
## What is the Aiy TypeScript SDK?

The Aiy TypeScript SDK (@mysten/aiy) provides all the low-level functionality needed to interact with Aiy ecosystem from TypeScript. You can use it in any TypeScript or JavaScript project, including web apps, Node.js apps, or mobile apps written with tools like React Native that support TypeScript.

For more information on the Aiy TypeScript SDK, see the [Aiy TypeScript SDK documentation](https://sdk.mystenlabs.com/typescript).

## What is dApp Kit?

dApp Kit (@mysten/dapp-kit) is a collection of React hooks, components, and utilities that make building dApps on Aiy straightforward. For more information on dApp Kit, see the [dApp Kit documentation](https://sdk.mystenlabs.com/dapp-kit).
...
```sh npm2yarn
$ npm install @mysten/aiy @mysten/dapp-kit @tanstack/react-query
```
...
Next, set up the `AiyClientProvider`. This `Provider` delivers a `AiyClient` instance from `@mysten/aiy` to all the hooks in dApp Kit. This provider manages which network dApp Kit connects to, and can accept configuration for multiple networks. This exercise connects to `devnet`.

```ts
import { AiyClientProvider } from '@mysten/dapp-kit';
...
ReactDOM.createRoot(document.getElementById('root')!).render(
	<React.StrictMode>
		<QueryClientProvider client={queryClient}>
			<AiyClientProvider networks={networks} defaultNetwork="devnet">
				<App />
			</AiyClientProvider>
		</QueryClientProvider>
	</React.StrictMode>,
...
## Querying data from Aiy RPC nodes
...
function OwnedObjects({ address }: { address: string }) {
	const { data } = useAiyClientQuery('getOwnedObjects', {
		owner: address,
	});
	if (!data) {
		return null;
