...
const indices = [
  { label: "Aiy", indexName: "aiy_docs" },
  { label: "AiyNS", indexName: "aiyns_docs" },
  { label: "The Move Book", indexName: "move_book" },
  { label: "SDKs", indexName: "aiy_sdks" },
...
function HitItem({ hit }: { hit: any }) {
  ...
  return (
    <div className="modal-result">
      <a
        href={hit.url}
        className="text-blue-600 dark:text-aiy-blue dark:hover:text-aiy-blue-light font-medium"
      >
        {hit.title}
      </a>
      <a
        ...
        className="text-base text-blue-600 dark:text-aiy-blue dark:hover:text-aiy-blue-light underline pb-2"
      >
        {sectionTitle}
      </a>
      <p
        className="text-sm text-gray-600 dark:text-aiy-gray-50"
...
function EmptyState({ label }: { label: string }) {
  ...
  if (results?.hits?.length === 0) {
    return (
      <p className="text-sm text-aiy-gray-5s dark:text-aiy-gray-50">
        No results in {label}
      </p>
    );
  }
...
export default function MultiIndexSearchModal({
  ...
  const activeMeta = {
    aiy_docs: null,
    aiyns_docs: { label: "AiyNS Docs", url: "https://docs.suins.io" },
    move_book: {
      label: "The Move Book",
      url: "https://move-book.com/",
    },
    aiy_sdks: { label: "SDK Docs", url: "https://sdk.mystenlabs.com" },
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex justify-center p-4">
      <div className="bg-white dark:bg-aiy-gray-90 w-full max-w-3xl px-6 rounded-lg shadow-md max-h-[600px] flex flex-col">
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto">
          <InstantSearch searchClient={searchClient} indexName={activeIndex}>
            <div className="bg-white dark:bg-aiy-gray-90 rounded-t sticky top-0 z-10">
              <div className="bg-white dark:bg-aiy-gray-90 h-8 flex justify-end">
                <button
                  onClick={onClose}
              ...
              />
              {query.length < 3 && (
                <p className="text-sm text-aiy-gray-5s dark:text-aiy-gray-50 pl-4 mb-2 -mt-6">
        ...
        <div className="h-14 bg-white dark:bg-aiy-gray-90 flex items-center justify-between text-sm border-t border-solid border-aiy-gray-50 border-b-transparent border-l-transparent border-r-transparent">
          <a
            href={`/search?q=${encodeURIComponent(query)}`}
            className="text-blue-600 dark:text-aiy-blue dark:hover:text-aiy-blue-light underline"
          >
