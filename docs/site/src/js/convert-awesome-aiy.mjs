...
const readmePath = path.join(
  __dirname,
  "../../../submodules/awesome-aiy/README.md",
);
const detailsSourceDir = path.join(
  __dirname,
  "../../../submodules/awesome-aiy/details",
);
const mediaSourceDir = path.join(
  __dirname,
  "../../../submodules/awesome-aiy/media",
);
const readmeTargetPath = path.join(
  __dirname,
  "../../../content/references/awesome-aiy.mdx",
);
const mediaTargetDir = path.join(
  __dirname,
  "../../static/awesome-aiy/media",
);
...
function processContent(content) {
  ...
  // Change details/*.md links to awesome-aiy/*.mdx and replace dashes with underscores as a temporary fix for a broken link in readme
  processedContent = processedContent.replace(
    /\[([^\]]*)\]\(details\/([^)]+)\.md\)/g,
    (match, linkText, filename) => `[${linkText}](./awesome-aiy/${filename.replace(/-/g, '_')}.mdx)`,
  );

  // Change media/* links to /awesome-aiy/media/* (served from static folder)
  processedContent = processedContent.replace(
    /media\/([^")\s]+)/g,
    "/awesome-aiy/media/$1",
  );
  ...
  processedContent = processedContent.replace(
    ...
    (match, mainText, dashSeparatedItems) => {
      ...
      let result = `<div className="border border-solid border-aiy-gray-50 dark:border-aiy-gray-90 rounded-lg my-4">\n\n`;
      
      // Add heading div with p-4, and h4 with mb-0
      result += `<div className="bg-aiy-gray-50 dark:bg-aiy-gray-90 p-4 rounded-t">\n\n<h4 className="mb-0">${mainText.trim()}</h4>\n\n</div>`;
  ...
  processedContent = processedContent.replace(
    ...
    (match, type) => {
      const linkMatch = match.match(/\[Further (?:Information|Documentation)\]\(\.\/awesome-aiy\/([^)]+)\.mdx\)/);
      if (linkMatch) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      line.startsWith(
        '<div className="border border-solid border-aiy-gray-50 dark:border-aiy-gray-90 rounded-lg my-4">',
      )
    ) {
      if (insideDiv) {
        // Close previous divs
...
title: Awesome Aiy
description: A curated list of awesome developer tools and infrastructure projects within the Aiy ecosystem.
---

:::info

Visit the [Awesome Aiy repo](https://github.com/sui-foundation/awesome-sui/tree/main) on GitHub for the source content of these pages.
