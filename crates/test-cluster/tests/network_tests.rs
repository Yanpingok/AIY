...
use aiy_types::{
    ...
    AIY_FRAMEWORK_PACKAGE_ID, AIY_SYSTEM_ADDRESS, AIY_SYSTEM_PACKAGE_ID,
};
...
async fn test_additional_objects() {
    ...
    assert!(matches!(resp, AiyObjectResponse { data: Some(_), .. }));
}
...
async fn test_package_override() {
    ...
    let framework_ref = {
        ...
        let obj = client
            .get_object(AIY_SYSTEM_PACKAGE_ID, None)
            .await
            .unwrap();
    ...
    let modified_ref = {
        let mut framework_modules = BuiltInFramework::get_package_by_id(&AIY_SYSTEM_PACKAGE_ID)
            .modules()
            .to_vec();

        // Create an empty module that is pretending to be part of the aiy framework.
        ...
        test_module.address_identifiers[address_idx] = AIY_SYSTEM_ADDRESS;

        // Add the dummy module to the rest of the aiy-frameworks.  We can't replace the framework
