...
use aiy_types::base_types::{ConsensusObjectSequenceKey, ObjectID, ObjectRef, AiyAddress};
use aiy_types::execution_config_utils::to_binary_config;
use aiy_types::object::{Object, Owner};
use aiy_types::storage::WriteKind;
use aiy_types::transaction::{CallArg, ObjectArg, TransactionData, TEST_ONLY_GAS_UNIT_FOR_PUBLISH};
use aiy_types::{Identifier, AIY_FRAMEWORK_ADDRESS};
...
impl SurferState {
    ...
    #[tracing::instrument(skip_all, fields(surfer_id = self.id))]
    async fn process_tx_effects(&mut self, effects: &AiyTransactionBlockEffects) {
        for (owned_ref, write_kind) in effects.all_changed_objects() {
            ...
            match owned_ref.owner {
                ...
                // TODO: Implement full support for ConsensusAddressOwner objects in aiy-surfer.
...
fn is_type_tx_context(ty: &Type) -> bool {
    match ty {
        Type::Reference(_, inner) => match inner.as_ref() {
            Type::Datatype(dt) => {
                dt.module.address == AIY_FRAMEWORK_ADDRESS
