...
mod tests {
            ...
            vec![S::Expr(E {
                alternates: vec![
                    ...
                    C {
                        root: Some(L::Vector(Box::new(Vector {
                            type_: Some(
                                TypeTag::from_str("0x2::coin::Coin<0x2::aiy::AIY>").unwrap()
                            ),
                            elements: vec![],
                        }))),
                        accessors: vec![],
    ...
    fn test_metering_datatypes() {
        let (nodes, strands) = nodes(
            r#"{ 0x1::string::String(42u64, 'foo', vector[1u256, 2u256, 3u256])
               | 0x2::coin::Coin<0x2::aiy::AIY>::Foo#1 { balance: 100u64 } }"#,
        );
                ...
                alternates: vec![
                    ...
                    C {
                        root: Some(L::Enum(Box::new(Enum {
                            type_: StructTag::from_str("0x2::coin::Coin<0x2::aiy::AIY>").unwrap(),
    ...
    fn test_struct_positional_literals() {
        assert_snapshot!(strands(
            "{ 0x1::string::String(42u64, 'foo', vector[1u256, 2u256, 3u256]) \
             | 0x2::coin::Coin<0x2::aiy::AIY> (true, 100u32,) }"
        ));
    }
    ...
    fn test_struct_named_literals() {
        assert_snapshot!(strands(
            "{ 0x1::string::String { length: 42u64, value: 'foo', data: vector[1u128, 2u128, 3u128], } \
             | 0x2::coin::Coin<0x2::aiy::AIY> { is_locked: true, amount: 100u32 } }"
        ));
    }
    ...
    fn test_struct_literal_whitespace() {
        assert_snapshot!(strands(r#"{0x1 :: coin :: Coin<0x2::aiy::AIY>()}"#));
    }
    ...
    fn test_nested_datatype() {
        assert_snapshot!(strands(
            r#"{
                0x1::option::Option<0x2::coin::Coin<0x2::aiy::AIY>>::Some#1(
