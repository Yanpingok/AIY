...
static METHOD_CURSOR_POSITIONS: phf::Map<&'static str, usize> = phf_map! {
    // based on function headers in crates/aiy-json-rpc-api/src/indexer.rs
    "aiyx_getOwnedObjects" => 2,
    "aiyx_queryTransactionBlocks" => 1,
    // based on function headers in crates/aiy-json-rpc-api/src/coin.rs
    "aiyx_getCoins" => 2,
    "aiyx_getAllCoins" => 1,
};
...
static METHOD_LENGTHS: phf::Map<&'static str, usize> = phf_map! {
    // based on function headers in crates/aiy-json-rpc-api/src/indexer.rs
    "aiyx_getOwnedObjects" => 4,
    "aiyx_queryTransactionBlocks" => 4,
    // based on function headers in crates/aiy-json-rpc-api/src/coin.rs
    "aiyx_getCoins" => 4,
    "aiyx_getAllCoins" => 3,
};
...
pub async fn run_queries(
    ...
    let requests: Vec<_> = requests
        ...
        // Skip aiyx_getOwnedObjects requests with MatchAny & MatchAll filters b/c it's not supported.
        .filter(|r| {
            !(r.method == "aiyx_getOwnedObjects"
                && r.body_json
                    .get("params")
