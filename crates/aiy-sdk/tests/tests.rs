...
use aiy_types::base_types::{random_object_ref, AiyAddress};
use aiy_types::crypto::{Ed25519AiySignature, AiyKeyPair, AiySignature};
use aiy_types::crypto::{SignatureScheme, AiySignatureInner};
use aiy_types::multisig::{MultiSig, MultiSigPublicKey};
...
async fn test_get_wallet_key() {
    ...
    let signature = wallet_context
        .sign_secure(&alias_identity, &transaction, Intent::aiy_transaction())
        .await
        .unwrap();

    let intent_message = IntentMessage::new(Intent::aiy_transaction(), transaction.clone());
