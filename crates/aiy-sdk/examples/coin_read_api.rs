...
// and add 1 AIY to the active address.
// By default, the example will use the Aiy testnet network (fullnode.testnet.aiy.io:443).
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // use `None` for the default `Coin<AIY>` which is represented as
    // "0x2::aiy::AIY"
    let coin_type = Some("0x2::aiy::AIY".to_string());
    let coins = aiy
        .coin_read_api()
    ...
    let coins_stream = aiy.coin_read_api().get_coins_stream(active_address, None);
    ...
    // Select coins based on the provided coin type (AIY in this example). Use `None` for the default Aiy coin
    let select_coins = aiy
        .coin_read_api()
    ...
    // or if None is passed, it will use Coin<AIY> as the coin type
    let balance = aiy
        .coin_read_api()
    ...
    let total_balance = aiy.coin_read_api().get_all_balances(active_address).await?;
    println!(" *** Balance + Total Balance *** ");
    println!("Balance: {:?}", balance);
    println!("Total Balance: {:?}", total_balance);
    println!(" *** Balance + Total Balance ***\n ");

    // Return the coin metadata for the Coin<AIY>
