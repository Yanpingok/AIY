...
use aiy_sdk::rpc_types::{
    AiyGetPastObjectRequest, AiyObjectDataOptions, AiyTransactionBlockResponseOptions,
};
use aiy_sdk::types::base_types::ObjectID;
...
async fn main() -> Result<(), anyhow::Error> {
    let (aiy, active_address, _) = setup_for_write().await?;
    ...
    let object_data = object
        .data
        .as_ref()
        .unwrap_or_else(|| panic!("No object data for this AiyObjectResponse {:?}", object));
    let object_id = object_data.object_id;
    let version = object_data.version;

    let aiy_data_options = AiyObjectDataOptions {
        show_type: true,
        show_owner: true,
    ...
    let past_object = aiy
        .read_api()
        .try_get_parsed_past_object(object_id, version, aiy_data_options.clone())
        .await?;
    println!(" *** Past Object *** ");
    println!("{:?}", past_object);
    println!(" *** Past Object ***\n");

    let aiy_get_past_object_request = past_object.clone().into_object()?;
    let multi_past_object = aiy
        .read_api()
        .try_multi_get_parsed_past_object(
            vec![AiyGetPastObjectRequest {
                object_id: aiy_get_past_object_request.object_id,
                version: aiy_get_past_object_request.version,
            }],
            aiy_data_options.clone(),
        )
        .await?;
    ...
    // Object with options
    let object_with_options = aiy
        .read_api()
        .get_object_with_options(aiy_get_past_object_request.object_id, aiy_data_options)
        .await?;
    ...
    println!("{:?}", aiy.read_api().get_chain_identifier().await?);
    println!(" *** Chain identifier ***\n ");

    println!(" *** Protocol Config *** ");
    println!("{:?}", aiy.read_api().get_protocol_config(None).await?);
    println!(" *** Protocol Config ***\n ");

    // we make a dummy transaction which returns a transaction digest
    let tx_digest = split_coin_digest(&aiy, &active_address).await?;
    ...
    let tx_blocks = aiy.read_api().get_total_transaction_blocks().await?;
