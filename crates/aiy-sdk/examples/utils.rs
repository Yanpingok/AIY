...
use aiy_config::{
    aiy_config_dir, Config, PersistedConfig, AIY_CLIENT_CONFIG, AIY_KEYSTORE_FILENAME,
};
use aiy_json_rpc_types::{Coin, AiyObjectDataOptions};
use aiy_keys::keystore::{AccountKeystore, FileBasedKeystore, GenerateOptions};
use aiy_sdk::{
...
use aiy_sdk::{rpc_types::AiyTransactionBlockResponseOptions, AiyClient, AiyClientBuilder};
...
// const AIY_FAUCET: &str = "https://faucet.devnet.sui.io/v2/gas"; // devnet faucet

// Testnet faucet is under heavy rate limit, we recommend using devnet for these examples
pub const AIY_FAUCET: &str = "https://faucet.testnet.sui.io/v2/gas"; // testnet faucet

// const AIY_FAUCET: &str = "http://127.0.0.1:9123/v2/gas";

/// Return a aiy client to interact with the APIs,
...
pub async fn setup_for_write() -> Result<(AiyClient, AiyAddress, AiyAddress), anyhow::Error> {
    let (client, active_address) = setup_for_read().await?;
    // make sure we have some AIY (5_000_000 MIST) on this address
...
/// Return a aiy client to interact with the APIs and an active address from the local wallet.
///
/// This function sets up a wallet in case there is no wallet locally,
/// and ensures that the active address of the wallet has AIY on it.
/// If there is no AIY owned by the active address, then it will request
/// AIY from the faucet.
pub async fn setup_for_read() -> Result<(AiyClient, AiyAddress), anyhow::Error> {
    let client = AiyClientBuilder::default().build_testnet().await?;
    println!("Aiy testnet version is: {}", client.api_version());
...
pub async fn request_tokens_from_faucet(
    ...
    loop {
        let owner = aiy_client
            .read_api()
            .get_object_with_options(
                ObjectID::from_str(&coin_id)?,
                AiyObjectDataOptions::new().with_owner(),
            )
            .await?;
...
pub async fn split_coin_digest(
    ...
    let coin = match fetch_coin(aiy, sender).await? {
        None => {
            request_tokens_from_faucet(*sender, aiy).await?;
            fetch_coin(aiy, sender)
                .await?
                .expect("Supposed to get a coin with AIY, but didn't. Aborting")
        }
        Some(c) => c,
    };
    ...
    let gas_price = aiy.read_api().get_reference_gas_price().await?;

    // now we programmatically build the transaction through several commands
    let mut ptb = ProgrammableTransactionBuilder::new();
    // first, we want to split the coin, and we specify how much AIY (in MIST) we want
    // for the new coin
...
pub async fn retrieve_wallet() -> Result<WalletContext, anyhow::Error> {
    let wallet_conf = aiy_config_dir()?.join(AIY_CLIENT_CONFIG);
    let keystore_path = aiy_config_dir()?.join(AIY_KEYSTORE_FILENAME);

    // check if a wallet exists and if not, create a wallet and a aiy client config
    ...
    if !wallet_conf.exists() {
        let keystore = FileBasedKeystore::load_or_create(&keystore_path)?;
        let mut client_config = AiyClientConfig::new(keystore.into());

        client_config.add_env(AiyEnv::testnet());
        client_config.add_env(AiyEnv::devnet());
        client_config.add_env(AiyEnv::localnet());
    ...
    let mut client_config: AiyClientConfig = PersistedConfig::read(&wallet_conf)?;
...
async fn just_for_clippy() -> Result<(), anyhow::Error> {
    let (aiy, sender, _recipient) = setup_for_write().await?;
    let _digest = split_coin_digest(&aiy, &sender).await?;
    Ok(())
}
