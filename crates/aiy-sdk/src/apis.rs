...
use aiy_json_rpc_types::{
    ...
    ProtocolConfigResponse, AiyCoinMetadata, AiyCommittee, AiyEvent, AiyGetPastObjectRequest,
    AiyMoveNormalizedModule, AiyObjectDataOptions, AiyObjectResponse, AiyObjectResponseQuery,
    AiyPastObjectResponse, AiyTransactionBlockEffects, AiyTransactionBlockResponse,
    AiyTransactionBlockResponseOptions, AiyTransactionBlockResponseQuery, TransactionBlocksPage,
    TransactionFilter,
};
use aiy_types::balance::Supply;
use aiy_types::base_types::{ObjectID, SequenceNumber, AiyAddress, TransactionDigest};
use aiy_types::dynamic_field::DynamicFieldName;
...
use aiy_types::transaction::{Transaction, TransactionData, TransactionKind};
...
impl ReadApi {
    ...
    ///     let aiy = AiyClientBuilder::default().build_localnet().await?;
    ///     let address = AiyAddress::from_str("0x0000....0000")?;
    ///     let owned_objects = aiy
    ///         .read_api()
    ...
    pub async fn get_owned_objects(
        &self,
        address: AiyAddress,
        query: Option<AiyObjectResponseQuery>,
        cursor: Option<ObjectID>,
        limit: Option<usize>,
    ) -> AiyRpcResult<ObjectsPage> {
        Ok(self
            .api
            .http
    ...
    ///         "No object data for this AiyObjectResponse {:?}",
    ///         object
    ///     ));
    ///     let object_id = object_data.object_id;
    ///     let dynamic_fields = aiy
    ///         .read_api()
    ...
    ) -> AiyRpcResult<AiyObjectResponse> {
        Ok(self
            .api
            .http
    ...
    /// use aiy_sdk::AiyClientBuilder;
    /// use aiy_types::base_types::{ObjectID, AiyAddress};
    /// use aiy_json_rpc_types::AiyObjectDataOptions;
    /// use std::str::FromStr;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), anyhow::Error> {
    ...
    ) -> AiyRpcResult<AiyPastObjectResponse> {
        Ok(self
            .api
            .http
    ...
    /// Return a list of [AiyPastObjectResponse] objects, or an error upon failure.
    ///
    ...
    /// use aiy_sdk::AiyClientBuilder;
    /// use aiy_types::base_types::{ObjectID, AiyAddress};
    /// use aiy_json_rpc_types::{AiyObjectDataOptions, AiyGetPastObjectRequest};
    /// use std::str::FromStr;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), anyhow::Error> {
    ...
    ///     let multi_past_object = aiy
    ///         .read_api()
    ///         .try_multi_get_parsed_past_object(
    ///             vec![AiyGetPastObjectRequest {
    ...
    pub async fn try_multi_get_parsed_past_object(
        &self,
        past_objects: Vec<AiyGetPastObjectRequest>,
        options: AiyObjectDataOptions,
    ) -> AiyRpcResult<Vec<AiyPastObjectResponse>> {
        Ok(self
            .api
            .http
    ...
    /// Return a [AiyObjectResponse] based on the provided [ObjectID] and [AiyObjectDataOptions], or an error upon failure.
    ///
    /// The [AiyObjectResponse] contains two fields:
    /// 1) `data` for the object's data (see [AiyObjectData](aiy_json_rpc_types::AiyObjectData)),
    /// 2) `error` for the error (if any) (see [AiyObjectResponseError](aiy_types::error::AiyObjectResponseError)).
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///     ));
    ///     let object_id = object_data.object_id;
    ///     let object = aiy.read_api().get_object_with_options(object_id,
    ///             AiyObjectDataOptions {
    ///                 show_type: true,
    ///                 show_owner: true,
    ///                 show_previous_transaction: true,
    ...
    /// Return a list of [AiyObjectResponse] from the given vector of [ObjectID]s and [AiyObjectDataOptions], or an error upon failure.
    ///
    ...
    ///     let object = aiy.read_api().multi_get_object_with_options(object_ids,
    ///             AiyObjectDataOptions {
    ///                 show_type: true,
    ///                 show_owner: true,
    ///                 show_previous_transaction: true,
    ...
    pub async fn multi_get_object_with_options(
        ...
        options: AiyObjectDataOptions,
    ) -> AiyRpcResult<Vec<AiyObjectResponse>> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_move_object_bcs(&self, object_id: ObjectID) -> AiyRpcResult<Vec<u8>> {
        let resp = self
            .get_object_with_options(object_id, AiyObjectDataOptions::default().with_bcs())
            .await?
            .into_object()
            .map_err(|e| {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let aiy = AiyClientBuilder::default().build_localnet().await?;
    ///     let total_transaction_blocks = aiy
    ///         .read_api()
    ...
    /// }
    /// ```
    pub async fn get_total_transaction_blocks(&self) -> AiyRpcResult<u64> {
        Ok(*self.api.http.get_total_transaction_blocks().await?)
    }

    /// Return a transaction and its effects in a [AiyTransactionBlockResponse] based on its
    /// [TransactionDigest], or an error upon failure.
    pub async fn get_transaction_with_options(
        &self,
        digest: TransactionDigest,
        options: AiyTransactionBlockResponseOptions,
    ) -> AiyRpcResult<AiyTransactionBlockResponse> {
        Ok(self
            .api
            .http
    ...
    /// Return a list of [AiyTransactionBlockResponse] based on the given vector of [TransactionDigest], or an error upon failure.
    ///
    ...
    pub async fn multi_get_transactions_with_options(
        ...
        options: AiyTransactionBlockResponseOptions,
    ) -> AiyRpcResult<Vec<AiyTransactionBlockResponse>> {
        Ok(self
            .api
            .http
    ...
    /// Return the [AiyCommittee] information for the provided `epoch`, or an error upon failure.
    ///
    /// The [AiyCommittee] contains the validators list and their information (name and stakes).
    ///
    ...
    pub async fn query_transaction_blocks(
        &self,
        query: AiyTransactionBlockResponseQuery,
        cursor: Option<TransactionDigest>,
        limit: Option<usize>,
        descending_order: bool,
    ) -> AiyRpcResult<TransactionBlocksPage> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_chain_identifier(&self) -> AiyRpcResult<String> {
        Ok(self.api.http.get_chain_identifier().await?)
    }

    /// Return a checkpoint, or an error upon failure.
    ///
    /// A Aiy checkpoint is a sequence of transaction sets that a quorum of validators
    /// agree upon as having been executed within the Aiy system.
    pub async fn get_checkpoint(&self, id: CheckpointId) -> AiyRpcResult<Checkpoint> {
    ...
    pub async fn get_latest_checkpoint_sequence_number(
        &self,
    ) -> AiyRpcResult<CheckpointSequenceNumber> {
        Ok(*self
            .api
            .http
    ...
    /// Return a stream of [AiyTransactionBlockResponse], or an error upon failure.
    pub fn get_transactions_stream(
        &self,
        query: AiyTransactionBlockResponseQuery,
        cursor: Option<TransactionDigest>,
        descending_order: bool,
    ) -> impl Stream<Item = AiyTransactionBlockResponse> + '_ {
        stream::unfold(
    ...
    ) -> AiyRpcResult<impl Stream<Item = AiyRpcResult<AiyTransactionBlockEffects>>> {
        let Some(c) = &self.api.ws else {
            return Err(Error::Subscription(
                "Subscription only supported by WebSocket client.".to_string(),
            ));
        };
        let subscription: Subscription<AiyTransactionBlockEffects> =
    ...
    pub async fn get_normalized_move_modules_by_package(
        &self,
        package: ObjectID,
    ) -> AiyRpcResult<BTreeMap<String, AiyMoveNormalizedModule>> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_reference_gas_price(&self) -> AiyRpcResult<u64> {
    ...
    pub async fn dry_run_transaction_block(
        &self,
        tx: TransactionData,
    ) -> AiyRpcResult<DryRunTransactionBlockResponse> {
        Ok(self
            .api
            .http
    ...
    ) -> AiyRpcResult<DevInspectResults> {
        Ok(self
            .api
            .http
    ...
    ) -> AiyRpcResult<ProtocolConfigResponse> {
    ...
    /// Verify a zkLogin signature against bytes that is parsed using intent_scope, and the aiy address.
    pub async fn verify_zklogin_signature(
        ...
        address: AiyAddress,
    ) -> AiyRpcResult<ZkLoginVerifyResult> {
        Ok(self
            .api
            .http
...
/// Coin Read API provides the functionality needed to get information from the Aiy network regarding the coins owned by an address.
#[derive(Debug, Clone)]
...
impl CoinReadApi {
    ...
    /// or use `None` for the default `Coin<AIY>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///
    /// The coins can be filtered by `coin_type` (e.g., 0x168da5bf1f48dafc111b0a488fa454aca95e0b5e::usdc::USDC)
    /// or use `None` to use the default `Coin<AIY>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// it will use the default `Coin<AIY>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// }
    /// ```
    pub async fn get_all_balances(&self, owner: AiyAddress) -> AiyRpcResult<Vec<Balance>> {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let aiy = AiyClientBuilder::default().build_localnet().await?;
    ///     let coin_metadata = aiy
    ///         .coin_read_api()
    ///         .get_coin_metadata("0x2::aiy::AIY".to_string())
    ///         .await?;
    ///     Ok(())
    /// }
    /// ```
    pub async fn get_coin_metadata(
        &self,
        coin_type: String,
    ) -> AiyRpcResult<Option<AiyCoinMetadata>> {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let aiy = AiyClientBuilder::default().build_localnet().await?;
    ///     let total_supply = aiy
    ///         .coin_read_api()
    ///         .get_total_supply("0x2::aiy::AIY".to_string())
    ///         .await?;
    ///     Ok(())
    /// }
    /// ```
    pub async fn get_total_supply(&self, coin_type: String) -> AiyRpcResult<Supply> {
        Ok(self.api.http.get_total_supply(coin_type).await?)
    }
}

/// Event API provides the functionality to fetch, query, or subscribe to events on the Aiy network.
#[derive(Clone)]
pub struct EventApi {
    api: Arc<RpcClient>,
}
...
impl EventApi {
    ...
    ///         .ws_url("wss://rpc.mainnet.aiy.io:443")
    ///         .build("https://fullnode.mainnet.sui.io:443")
    ///         .await?;
    ///     let mut subscribe_all = aiy
    ///         .event_api()
    ...
    pub async fn subscribe_event(
        &self,
        filter: EventFilter,
    ) -> AiyRpcResult<impl Stream<Item = AiyRpcResult<AiyEvent>>> {
        match &self.api.ws {
            Some(c) => {
                let subscription: Subscription<AiyEvent> = c.subscribe_event(filter).await?;
                Ok(subscription.map(|item| Ok(item?)))
            }
    ...
    pub async fn get_events(&self, digest: TransactionDigest) -> AiyRpcResult<Vec<AiyEvent>> {
...
impl GovernanceApi {
    ...
    pub async fn get_stakes(&self, owner: AiyAddress) -> AiyRpcResult<Vec<DelegatedStake>> {
        Ok(self.api.http.get_stakes(owner).await?)
    }

    /// Return the [AiyCommittee] information for the given `epoch`, or an error upon failure.
    ///
    ...
    /// Return the latest AIY system state object on-chain, or an error upon failure.
    ///
    ...
    /// and much more. See the [AiySystemStateSummary] for all the available fields.
    pub async fn get_latest_aiy_system_state(&self) -> AiyRpcResult<AiySystemStateSummary> {
        Ok(self.api.http.get_latest_aiy_system_state().await?)
    }
