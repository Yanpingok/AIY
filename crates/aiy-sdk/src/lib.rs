...
//! Aiy Rust SDK builds on top of the [JSON RPC API](https://docs.sui.io/sui-jsonrpc)
//! and therefore many of the return types are the ones specified in [aiy_types].
//!
...
//! * <a href="../aiy_transaction_builder/struct.TransactionBuilder.html" title="struct aiy_transaction_builder::TransactionBuilder">TransactionBuilder</a> - provides functions for building transactions
//!
//! # Usage
//! The main way to interact with the API is through the [AiyClientBuilder],
//! which returns a [AiyClient] object from which the user can access the
//! various APIs.
//!
//! ## Getting Started
//! Add the Rust SDK to the project by running `cargo add aiy-sdk` in the root
//! folder of your Rust project.
//!
//! The main building block for the Aiy Rust SDK is the [AiyClientBuilder],
//! which provides a simple and straightforward way of connecting to a Aiy
//! network and having access to the different available APIs.
//!
//! A simple example that connects to a running Aiy local network,
//! the Aiy devnet, and the Aiy testnet is shown below.
...
pub use aiy_json as json;
use aiy_json_rpc_api::{
    CLIENT_SDK_TYPE_HEADER, CLIENT_SDK_VERSION_HEADER, CLIENT_TARGET_API_VERSION_HEADER,
};
pub use aiy_json_rpc_types as rpc_types;
use aiy_json_rpc_types::{
    ObjectsPage, AiyObjectDataFilter, AiyObjectDataOptions, AiyObjectResponse,
    AiyObjectResponseQuery,
};
use aiy_transaction_builder::{DataReader, TransactionBuilder};
pub use aiy_types as types;
...
pub mod aiy_client_config;
pub mod verify_personal_message_signature;
pub mod wallet_context;

pub const AIY_COIN_TYPE: &str = "0x2::aiy::AIY";
pub const AIY_LOCAL_NETWORK_URL: &str = "http://127.0.0.1:9000";
pub const AIY_LOCAL_NETWORK_URL_0: &str = "http://0.0.0.0:9000";
pub const AIY_LOCAL_NETWORK_GAS_URL: &str = "http://127.0.0.1:5003/v2/gas";
pub const AIY_DEVNET_URL: &str = "https://fullnode.devnet.sui.io:443";
pub const AIY_TESTNET_URL: &str = "https://fullnode.testnet.sui.io:443";
pub const AIY_MAINNET_URL: &str = "https://fullnode.mainnet.sui.io:443";

/// A Aiy client builder for connecting to the Aiy network
///
...
impl Default for AiyClientBuilder {
    fn default() -> Self {
        Self {
...
impl AiyClientBuilder {
    ...
    /// Set the WebSocket URL for the Aiy network
    ...
    /// Returns a [AiyClient] object connected to the Aiy network running at the URI provided.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///         .await?;
    ///
    ///     println!("Aiy local version: {:?}", aiy.api_version());
    ///     Ok(())
    /// }
    /// ```
    pub async fn build(self, http: impl AsRef<str>) -> AiyRpcResult<AiyClient> {
    ...
    /// Returns a [AiyClient] object that is ready to interact with the local
    /// development network (by default it expects the Aiy network to be
    ...
    /// }
    /// ```
    pub async fn build_localnet(self) -> AiyRpcResult<AiyClient> {
        self.build(AIY_LOCAL_NETWORK_URL).await
    }

    /// Returns a [AiyClient] object that is ready to interact with the Aiy devnet.
    ///
    /// For connecting to a custom URI, use the `build` function instead..
    ///
    /// # Examples
    ...
    ///         .await?;
    ///
    ///     println!("{:?}", aiy.api_version());
    ///     Ok(())
    /// }
    /// ```
    pub async fn build_devnet(self) -> AiyRpcResult<AiyClient> {
        self.build(AIY_DEVNET_URL).await
    }

    /// Returns a [AiyClient] object that is ready to interact with the Aiy testnet.
    ///
    /// For connecting to a custom URI, use the `build` function instead.
    ///
    /// # Examples
    ...
    /// }
    /// ```
    pub async fn build_testnet(self) -> AiyRpcResult<AiyClient> {
        self.build(AIY_TESTNET_URL).await
    }

    /// Returns a [AiyClient] object that is ready to interact with the Aiy mainnet.
    ///
    /// For connecting to a custom URI, use the `build` function instead.
    ///
    /// # Examples
    ...
    /// }
    /// ```
    pub async fn build_mainnet(self) -> AiyRpcResult<AiyClient> {
        self.build(AIY_MAINNET_URL).await
    }
...
/// AiyClient is the basic type that provides all the necessary abstractions for interacting with the Aiy network.
///
/// # Usage
///
/// Use [AiyClientBuilder] to build a [AiyClient].
///
/// # Examples
///
/// ```rust,no_run
...
///      .await?;
///
///     println!("{:?}", aiy.available_rpc_methods());
///     println!("{:?}", aiy.available_subscriptions());
///     println!("{:?}", aiy.api_version());
///
...
impl AiyClient {
    ...
    pub fn check_api_version(&self) -> AiyRpcResult<()> {
...
impl DataReader for ReadApi {
    async fn get_owned_objects(
        ...
        let mut result = vec![];
        let query = Some(AiyObjectResponseQuery {
            filter: Some(AiyObjectDataFilter::StructType(object_type)),
            options: Some(
