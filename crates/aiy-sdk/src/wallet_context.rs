...
use crate::aiy_client_config::{AiyClientConfig, AiyEnv};
use crate::AiyClient;
...
use std::sync::Arc;
use aiy_config::{Config, PersistedConfig};
use aiy_json_rpc_types::{
    AiyObjectData, AiyObjectDataFilter, AiyObjectDataOptions, AiyObjectResponse,
    AiyObjectResponseQuery, AiyTransactionBlockResponse, AiyTransactionBlockResponseOptions,
};
...
use aiy_types::transaction::{Transaction, TransactionData, TransactionDataAPI};
use tokio::sync::RwLock;

pub struct WalletContext {
    pub config: PersistedConfig<AiyClientConfig>,
    request_timeout: Option<std::time::Duration>,
    client: Arc<RwLock<Option<AiyClient>>>,
...
impl WalletContext {
    pub fn new(config_path: &Path) -> Result<Self, anyhow::Error> {
        let config: AiyClientConfig = PersistedConfig::read(config_path).map_err(|err| {
            anyhow!(
    ...
    pub fn new_for_tests(
        ...
        let mut config = AiyClientConfig::new(keystore)
    ...
    pub async fn get_client(&self) -> Result<AiyClient, anyhow::Error> {
    ...
    // TODO: Ger rid of mut
    pub fn active_address(&mut self) -> Result<AiyAddress, anyhow::Error> {
    ...
    pub async fn gas_objects(
        &self,
        address: AiyAddress,
    ) -> Result<Vec<(u64, AiyObjectData)>, anyhow::Error> {
        let client = self.get_client().await?;
            ...
            let response = client
                ...
                .get_owned_objects(
                    ...
                    Some(AiyObjectResponseQuery::new(
                        ...
                        Some(AiyObjectDataOptions::full_content()),
                    )),
                    cursor,
                    None,
                )
    ...
    pub async fn get_object_owner(&self, id: &ObjectID) -> Result<AiyAddress, anyhow::Error> {
        let client = self.get_client().await?;
        let object = client
            .read_api()
            .get_object_with_options(*id, AiyObjectDataOptions::new().with_owner())
            .await?
            .into_object()?;
        Ok(object
            .owner
    ...
    pub async fn try_get_object_owner(
        &self,
        id: &Option<ObjectID>,
    ) -> Result<Option<AiyAddress>, anyhow::Error> {
        if let Some(id) = id {
    ...
    pub async fn infer_sender(&mut self, gas: &[ObjectID]) -> Result<AiyAddress, anyhow::Error> {
        if gas.is_empty() {
    ...
    pub async fn gas_for_owner_budget(
    ...
    ) -> Result<(u64, AiyObjectData), anyhow::Error> {
        ...
        Err(anyhow!(
            "No non-argument gas objects found for this address with value >= budget {budget}. Run aiy client gas to check for gas objects."
        ))
    }
    ...
    pub async fn get_one_account(&self) -> anyhow::Result<(AiyAddress, Vec<ObjectRef>)> {
    ...
    pub async fn get_one_gas_object(&self) -> anyhow::Result<Option<(AiyAddress, ObjectRef)>> {
    ...
    pub async fn get_all_accounts_and_gas_objects(
        &self,
    ) -> anyhow::Result<Vec<(AiyAddress, Vec<ObjectRef>)>> {
        let mut result = vec![];
        for address in self.get_addresses() {
            let objects = self
                .gas_objects(address)
    ...
    /// Add an account
    pub async fn add_account(&mut self, alias: Option<String>, keypair: AiyKeyPair) {
