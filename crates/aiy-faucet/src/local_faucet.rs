...
use aiy_sdk::{
    rpc_types::{AiyTransactionBlockResponse, AiyTransactionBlockResponseOptions},
...
use aiy_sdk::rpc_types::AiyTransactionBlockEffectsAPI;
...
impl LocalFaucet {
    ...
    pub async fn local_request_execute_tx(
        ...
        ptb.pay_aiy(recipients, amounts)
    ...
    async fn execute_txn(
        ...
        let signature = self
            ...
            .config
            .keystore
            .sign_secure(&self.active_address, &tx_data, Intent::aiy_transaction())
            .await
            .map_err(FaucetError::internal)?;
        ...
        Ok(client
            ...
            .map_err(|e| {
                FaucetError::internal(format!(
                    "Failed to execute PayAiy transaction for coin {:?}, with err {:?}",
                    coin_id, e
                ))
            })?)
    }
...
async fn find_gas_coins_and_address(
    ...
    config: &FaucetConfig,
) -> Result<(Vec<GasCoin>, AiyAddress), FaucetError> {
...
mod tests {
    ...
    async fn test_local_faucet_execute_txn() {
        ...
        let client = cluster.aiy_client().clone();
