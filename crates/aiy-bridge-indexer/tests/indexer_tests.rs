...
use aiy_bridge::e2e_tests::test_utils::{
    initiate_bridge_eth_to_aiy, BridgeTestCluster, BridgeTestClusterBuilder,
};
use aiy_bridge_indexer::config::IndexerConfig;
use aiy_bridge_indexer::create_aiy_indexer;
use aiy_bridge_indexer::metrics::BridgeIndexerMetrics;
use aiy_bridge_indexer::postgres_manager::get_connection_pool;
use aiy_bridge_indexer::storage::PgBridgePersistent;
use aiy_bridge_schema::models::{GovernanceAction, TokenTransfer, TokenTransferStatus};
use aiy_bridge_schema::{schema, MIGRATIONS};
use aiy_data_ingestion_core::DataIngestionMetrics;
use aiy_indexer::database::Connection;
use aiy_indexer_builder::indexer_builder::IndexerProgressStore;
use aiy_pg_db::temp::TempDb;
...
async fn test_indexing_transfer() {
    ...
    let indexer = create_aiy_indexer(pool.clone(), metrics.clone(), ingestion_metrics, &config)
        .await
        .unwrap();
    ...
    // transfer eth to aiy
    initiate_bridge_eth_to_aiy(&cluster, 1000, 0).await.unwrap();
...
async fn setup_bridge_env(with_eth_env: bool) -> (IndexerConfig, BridgeTestCluster, TempDb) {
    ...
    let config = IndexerConfig {
        remote_store_url: format!("{}/rest", bridge_test_cluster.aiy_rpc_url()),
        ...
        concurrency: 10,
        aiy_bridge_genesis_checkpoint: 0,
        eth_bridge_genesis_block: 0,
        eth_aiy_bridge_contract_address: bridge_test_cluster.aiy_bridge_address(),
        metric_port: 9001,
        eth_only: false,
    };
