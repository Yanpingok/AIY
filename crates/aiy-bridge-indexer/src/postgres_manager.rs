...
use aiy_bridge_schema::models::AiyProgressStore;
use aiy_bridge_schema::schema::governance_actions;
use aiy_bridge_schema::schema::aiy_progress_store::txn_digest;
use aiy_bridge_schema::schema::{aiy_error_transactions, token_transfer_data};
use aiy_bridge_schema::{schema, schema::token_transfer};
...
const AIY_PROGRESS_STORE_DUMMY_KEY: i32 = 1;
...
pub async fn update_aiy_progress_store(
    pool: &PgPool,
    tx_digest: TransactionDigest,
) -> Result<(), anyhow::Error> {
    let mut conn = pool.get().await?;
    diesel::insert_into(schema::aiy_progress_store::table)
        .values(&AiyProgressStore {
            id: AIY_PROGRESS_STORE_DUMMY_KEY,
            txn_digest: tx_digest.inner().to_vec(),
        })
        .on_conflict(schema::aiy_progress_store::dsl::id)
        .do_update()
...
pub async fn read_aiy_progress_store(pool: &PgPool) -> anyhow::Result<Option<TransactionDigest>> {
    let mut conn = pool.get().await?;
    let val: Option<AiyProgressStore> = schema::aiy_progress_store::dsl::aiy_progress_store
        .select(AiyProgressStore::as_select())
        .first(&mut conn)
        .await
        .optional()?;
    match val {
