...
use aiy_bridge::error::BridgeError;
use aiy_bridge::eth_client::EthClient;
use aiy_bridge::eth_syncer::EthSyncer;
use aiy_bridge::metered_eth_provider::MeteredEthHttpProvier;
use aiy_bridge::retry_with_max_elapsed_time;
use aiy_indexer_builder::Task;
...
use aiy_bridge::types::{EthEvent, RawEthLog};
use aiy_bridge_schema::models::GovernanceActionType;
use aiy_indexer_builder::indexer_builder::{DataMapper, DataSender, Datasource};
...
impl EthSubscriptionDatasource {
    pub async fn new(
        eth_aiy_bridge_contract_addresses: Vec<EthAddress>,
        eth_client: Arc<EthClient<MeteredEthHttpProvier>>,
        eth_ws_url: String,
        metrics: Box<dyn IndexerMetricProvider>,
        genesis_block: u64,
        ...
        Ok(Self {
            addresses: eth_aiy_bridge_contract_addresses,
            eth_client,
            eth_ws_url,
            metrics,
            genesis_block,
...
impl EthFinalizedSyncDatasource {
    pub async fn new(
        ...
        Ok(Self {
            bridge_addresses: eth_aiy_bridge_contract_addresses,
            eth_http_url,
            eth_client,
            metrics,
            bridge_metrics,
...
impl DataMapper<RawEthData, ProcessedTxnData> for EthDataMapper {
    fn map(
        ...
        match bridge_event {
            EthBridgeEvent::EthAiyBridgeEvents(bridge_event) => match &bridge_event {
                EthAiyBridgeEvents::TokensDepositedFilter(bridge_event) => {
                    info!(
                    ...
                    processed_txn_data.push(ProcessedTxnData::TokenTransfer(TokenTransfer {
                        ...
                        data: Some(TokenTransferData {
                            ...
                            amount: bridge_event.aiy_adjusted_amount,
                            is_finalized,
                        }),
                    }));
                }
                EthAiyBridgeEvents::TokensClaimedFilter(bridge_event) => {
                    info!(
                ...
                EthAiyBridgeEvents::EmergencyOperationFilter(f) => {
                    info!(
                ...
                EthAiyBridgeEvents::ContractUpgradedFilter(f) => {
                    info!(
                        "Observed Eth AiyBridge Upgrade at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
                ...
                | EthAiyBridgeEvents::PausedFilter(_)
                | EthAiyBridgeEvents::UnpausedFilter(_)
            ...
            EthBridgeEvent::EthBridgeConfigEvents(bridge_event) => match &bridge_event {
                ...
                EthBridgeConfigEvents::TokenAddedFilter(_) => {
                    info!(
                        "Observed Eth AddAiyTokens at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
