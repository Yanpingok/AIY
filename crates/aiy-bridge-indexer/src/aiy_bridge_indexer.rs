...
use aiy_bridge_schema::models::GovernanceActionType;
use aiy_indexer_builder::indexer_builder::DataMapper;
use aiy_indexer_builder::aiy_datasource::CheckpointTxnData;
use aiy_types::effects::TransactionEffectsAPI;
use aiy_types::event::Event;
use aiy_types::execution_status::ExecutionStatus;
use aiy_types::full_checkpoint_content::CheckpointTransaction;
use aiy_types::{BRIDGE_ADDRESS, AIY_BRIDGE_OBJECT_ID};

use crate::metrics::BridgeIndexerMetrics;
use crate::{
    BridgeDataSource, GovernanceAction, ProcessedTxnData, AiyTxnError, TokenTransfer,
    TokenTransferData, TokenTransferStatus,
};

/// Data mapper impl
#[derive(Clone)]
pub struct AiyBridgeDataMapper {
    pub metrics: BridgeIndexerMetrics,
}

impl DataMapper<CheckpointTxnData, ProcessedTxnData> for AiyBridgeDataMapper {
    fn map(
        &self,
        (data, checkpoint_num, timestamp_ms): CheckpointTxnData,
    ) -> Result<Vec<ProcessedTxnData>, Error> {
        self.metrics.total_aiy_bridge_transactions.inc();
        if !data
            .input_objects
            .iter()
            .any(|obj| obj.id() == AIY_BRIDGE_OBJECT_ID)
        {
            return Ok(vec![]);
        }
        ...
        match &data.events {
            Some(events) => {
                let token_transfers = events.data.iter().try_fold(vec![], |mut result, ev| {
                    if let Some(data) = process_aiy_event(ev, &data, checkpoint_num, timestamp_ms)?
                    {
                        result.push(data);
                    }
                ...
                if !token_transfers.is_empty() {
                    info!(
                        "AIY: Extracted {} bridge token transfer data entries for tx {}.",
                        token_transfers.len(),
            ...
            None => {
                if let ExecutionStatus::Failure { error, command } = data.effects.status() {
                    Ok(vec![ProcessedTxnData::Error(AiyTxnError {
...
fn process_aiy_event(
    ...
    Ok(if ev.type_.address == BRIDGE_ADDRESS {
        match ev.type_.name.as_str() {
            "TokenDepositedEvent" => {
                info!("Observed Aiy Deposit {:?}", ev);
                // todo: metrics.total_aiy_token_deposited.inc();
                ...
                Some(ProcessedTxnData::TokenTransfer(TokenTransfer {
                    ...
                    data_source: BridgeDataSource::AIY,
                    is_finalized: true,
                    data: Some(TokenTransferData {
                        ...
                        amount: move_event.amount_aiy_adjusted,
                        is_finalized: true,
                    }),
                }))
            }
            "TokenTransferApproved" => {
                info!("Observed Aiy Approval {:?}", ev);
                // todo: metrics.total_aiy_token_transfer_approved.inc();
            ...
            "TokenTransferClaimed" => {
                info!("Observed Aiy Claim {:?}", ev);
                // todo: metrics.total_aiy_token_transfer_claimed.inc();
            ...
            "UpdateRouteLimitEvent" => {
                info!("Observed Aiy Route Limit Update {:?}", ev);
            ...
            "EmergencyOpEvent" => {
                info!("Observed Aiy Emergency Op {:?}", ev);
            ...
            "BlocklistValidatorEvent" => {
                info!("Observed Aiy Blocklist Validator {:?}", ev);
            ...
            "TokenRegistrationEvent" => {
                info!("Observed Aiy Token Registration {:?}", ev);
                ...
                Some(ProcessedTxnData::GovernanceAction(GovernanceAction {
                    ...
                    sender: ev.sender.to_vec(),
                    timestamp_ms,
                    action: GovernanceActionType::AddAiyTokens,
                    data: serde_json::to_value(event)?,
                }))
            }
            "UpdateTokenPriceEvent" => {
                info!("Observed Aiy Token Price Update {:?}", ev);
            ...
            "NewTokenEvent" => {
                info!("Observed Aiy New token event {:?}", ev);
            ...
            _ => {
                // todo: metrics.total_aiy_bridge_txn_other.inc();
