...
use crate::aiy_bridge_indexer::AiyBridgeDataMapper;
...
use aiy_bridge_schema::models::{
    BridgeDataSource, GovernanceAction as DBGovernanceAction, TokenTransferStatus,
};
use aiy_bridge_schema::models::{GovernanceActionType, TokenTransferData as DBTokenTransferData};
use aiy_bridge_schema::models::{AiyErrorTransactions, TokenTransfer as DBTokenTransfer};
use aiy_data_ingestion_core::DataIngestionMetrics;
use aiy_indexer_builder::indexer_builder::{BackfillStrategy, Datasource, Indexer, IndexerBuilder};
use aiy_indexer_builder::metrics::IndexerMetricProvider;
use aiy_indexer_builder::progress::{
...
pub mod storage;
pub mod aiy_transaction_handler;
pub mod aiy_transaction_queries;
pub mod types;

pub mod eth_bridge_indexer;
pub mod aiy_bridge_indexer;
...
impl AiyTxnError {
    fn to_db(&self) -> AiyErrorTransactions {
        AiyErrorTransactions {
...
pub async fn create_aiy_indexer(
    pool: PgPool,
    metrics: BridgeIndexerMetrics,
    ingestion_metrics: DataIngestionMetrics,
    config: &IndexerConfig,
) -> anyhow::Result<
    Indexer<PgBridgePersistent, AiyCheckpointDatasource, AiyBridgeDataMapper>,
    anyhow::Error,
> {
    ...
    let aiy_checkpoint_datasource = AiyCheckpointDatasource::new(
        ...
        config.aiy_bridge_genesis_checkpoint,
        ingestion_metrics,
        metrics.clone().boxed(),
    );
...
async fn get_eth_bridge_contract_addresses(
    ...
    let bridge_address = EthAddress::from_str(&config.eth_aiy_bridge_contract_address)?;
