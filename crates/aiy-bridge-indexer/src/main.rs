...
use std::str::FromStr;
use std::sync::Arc;
use aiy_bridge::eth_client::EthClient;
use aiy_bridge::metered_eth_provider::{new_metered_eth_provider, MeteredEthHttpProvier};
use aiy_bridge::aiy_bridge_watchdog::Observable;
use aiy_bridge::aiy_client::AiyBridgeClient;
use aiy_bridge::utils::get_eth_contract_addresses;
...
use aiy_bridge::aiy_bridge_watchdog::{
    eth_bridge_status::EthBridgeStatus,
    eth_vault_balance::{EthereumVaultBalance, VaultAsset},
    metrics::WatchdogMetrics,
    aiy_bridge_status::AiyBridgeStatus,
    BridgeWatchDog,
};
use aiy_bridge_indexer::config::IndexerConfig;
use aiy_bridge_indexer::metrics::BridgeIndexerMetrics;
use aiy_bridge_indexer::postgres_manager::{get_connection_pool, read_aiy_progress_store};
use aiy_bridge_indexer::aiy_transaction_handler::handle_aiy_transactions_loop;
use aiy_bridge_indexer::aiy_transaction_queries::start_aiy_tx_polling_task;
use aiy_bridge_indexer::{
    create_eth_subscription_indexer, create_eth_sync_indexer, create_aiy_indexer,
};
...
async fn main() -> Result<()> {
    ...
    let eth_bridge_proxy_address = EthAddress::from_str(&config.eth_aiy_bridge_contract_address)?;
    let mut tasks = vec![];
    ...
    if !config.eth_only {
        let indexer =
            create_aiy_indexer(pool, indexer_meterics, ingestion_metrics, &config).await?;
        tasks.push(spawn_logged_monitored_task!(indexer.start()));
    }

    let aiy_bridge_client =
        Arc::new(AiyBridgeClient::new(&config.aiy_rpc_url, bridge_metrics.clone()).await?);
    start_watchdog(
        config,
...
async fn start_watchdog(
    ...
    let aiy_bridge_status =
        AiyBridgeStatus::new(aiy_client, watchdog_metrics.aiy_bridge_paused.clone());
...
#[allow(unused)]
async fn start_processing_aiy_checkpoints_by_querying_txns(
    aiy_rpc_url: String,
    db_url: String,
    ...
    let (tx, rx) = channel(
        ...
        &mysten_metrics::get_metrics()
            .unwrap()
            .channel_inflight
            .with_label_values(&["aiy_transaction_processing_queue"]),
    );
    let mut handles = vec![];
    let cursor = read_aiy_progress_store(&pg_pool)
        .await
        .expect("Failed to read cursor from aiy progress store");
    let aiy_client = AiyClientBuilder::default().build(aiy_rpc_url).await?;
    handles.push(spawn_logged_monitored_task!(
        start_aiy_tx_polling_task(aiy_client, cursor, tx),
        "start_aiy_tx_polling_task"
    ));
    handles.push(spawn_logged_monitored_task!(
        handle_aiy_transactions_loop(pg_pool.clone(), rx, indexer_metrics.clone()),
        "handle_aiy_transcations_loop"
    ));
    Ok(handles)
}
