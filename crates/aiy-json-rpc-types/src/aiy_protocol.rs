...
use aiy_protocol_config::{ProtocolConfig, ProtocolConfigValue, ProtocolVersion};
use aiy_types::aiy_serde::Readable;
use aiy_types::aiy_serde::{AsProtocolVersion, BigInt};

#[serde_as]
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema, PartialEq)]
#[serde(rename_all = "camelCase", rename = "ProtocolConfigValue")]
pub enum AiyProtocolConfigValue {
    U16(
...
impl From<ProtocolConfigValue> for AiyProtocolConfigValue {
    fn from(value: ProtocolConfigValue) -> Self {
        match value {
            ProtocolConfigValue::u16(y) => AiyProtocolConfigValue::U16(y),
            ProtocolConfigValue::u32(y) => AiyProtocolConfigValue::U32(y),
            ProtocolConfigValue::u64(x) => AiyProtocolConfigValue::U64(x),
            ProtocolConfigValue::bool(z) => AiyProtocolConfigValue::Bool(z),
        }
    }
}
...
pub struct ProtocolConfigResponse {
    ...
    pub attributes: BTreeMap<String, Option<AiyProtocolConfigValue>>,
}
    ...
    fn from(config: ProtocolConfig) -> Self {
        ProtocolConfigResponse {
            ...
            attributes: config
                .attr_map()
                .into_iter()
                .map(|(k, v)| (k, v.map(AiyProtocolConfigValue::from)))
                .collect(),
