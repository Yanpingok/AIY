...
use aiy_package_resolver::{PackageStore, Resolver};
use tabled::{
...
use aiy_json::{primitive_type, AiyJsonValue};
use aiy_types::accumulator_event::AccumulatorEvent;
use aiy_types::authenticator_state::ActiveJwk;
use aiy_types::base_types::{
    EpochId, ObjectID, ObjectRef, SequenceNumber, AiyAddress, TransactionDigest,
};
use aiy_types::crypto::AiySignature;
use aiy_types::digests::Digest;
use aiy_types::digests::{
    AdditionalConsensusStateDigest, CheckpointDigest, ConsensusCommitDigest, ObjectDigest,
    TransactionEventsDigest,
};
use aiy_types::effects::{
    AccumulatorOperation, AccumulatorValue, TransactionEffects, TransactionEffectsAPI,
    TransactionEvents,
};
use aiy_types::error::{ExecutionError, AiyError, AiyResult};
use aiy_types::execution_status::ExecutionStatus;
use aiy_types::gas::GasCostSummary;
use aiy_types::layout_resolver::{get_layout_from_struct_tag, LayoutResolver};
use aiy_types::messages_checkpoint::CheckpointSequenceNumber;
use aiy_types::messages_consensus::ConsensusDeterminedVersionAssignments;
use aiy_types::object::Owner;
use aiy_types::parse_aiy_type_tag;
use aiy_types::quorum_driver_types::ExecuteTransactionRequestType;
use aiy_types::signature::GenericSignature;
use aiy_types::storage::{DeleteKind, WriteKind};
use aiy_types::aiy_serde::Readable;
use aiy_types::aiy_serde::{
    BigInt, SequenceNumber as AsSequenceNumber, AiyTypeTag as AsAiyTypeTag,
};
...
use aiy_types::AIY_FRAMEWORK_ADDRESS;

use crate::balance_changes::BalanceChange;
use crate::object_changes::ObjectChange;
use crate::aiy_transaction::GenericSignature::Signature;
use crate::{Filter, Page, AiyEvent, AiyMoveAbort, AiyObjectRef};

// similar to EpochId of aiy-types but BigInt
pub type AiyEpochId = BigInt<u64>;
...
pub struct AiyTransactionBlockResponseQuery {
    /// If None, no filter will be applied
    pub filter: Option<TransactionFilter>,
    /// config which fields to include in the response, by default only digest is included
    pub options: Option<AiyTransactionBlockResponseOptions>,
}

impl AiyTransactionBlockResponseQuery {
    pub fn new(
...
pub type TransactionBlocksPage = Page<AiyTransactionBlockResponse, TransactionDigest>;
...
pub struct AiyTransactionBlockResponseOptions {
...
impl AiyTransactionBlockResponseOptions {
    pub fn new() -> Self {
        Self::default()
    }
...
pub struct AiyTransactionBlockResponse {
    pub digest: TransactionDigest,
    /// Transaction input data
    #[serde(skip_serializing_if = "Option::is_none")]
    pub transaction: Option<AiyTransactionBlock>,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub effects: Option<AiyTransactionBlockEffects>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub events: Option<AiyTransactionBlockEvents>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl AiyTransactionBlockResponse {
...
impl PartialEq for AiyTransactionBlockResponse {
...
impl Display for AiyTransactionBlockResponse {
...
pub enum AiyTransactionBlockKind {
    /// A system transaction that will update epoch information on-chain.
    ChangeEpoch(AiyChangeEpoch),
    /// A system transaction used for initializing the initial state of the chain.
    Genesis(AiyGenesisTransaction),
    /// A system transaction marking the start of a series of transactions scheduled as part of a
    /// checkpoint
    ConsensusCommitPrologue(AiyConsensusCommitPrologue),
    /// A series of transactions where the results of one transaction can be used in future
    /// transactions
    ProgrammableTransaction(AiyProgrammableTransactionBlock),
    /// A transaction which updates global authenticator state
    AuthenticatorStateUpdate(AiyAuthenticatorStateUpdate),
    /// A transaction which updates global randomness state
    RandomnessStateUpdate(AiyRandomnessStateUpdate),
    /// The transaction which occurs only at the end of the epoch
    EndOfEpochTransaction(AiyEndOfEpochTransaction),
    ConsensusCommitPrologueV2(AiyConsensusCommitPrologueV2),
    ConsensusCommitPrologueV3(AiyConsensusCommitPrologueV3),
    ConsensusCommitPrologueV4(AiyConsensusCommitPrologueV4),

    ProgrammableSystemTransaction(AiyProgrammableTransactionBlock),
    // .. more transaction types go here
}

impl Display for AiyTransactionBlockKind {
    ...
    fn try_from_inner(tx: TransactionKind) -> Result<Self, anyhow::Error> {
        Ok(match tx {
            ...
            TransactionKind::ConsensusCommitPrologueV2(p) => {
                Self::ConsensusCommitPrologueV2(AiyConsensusCommitPrologueV2 {
                    epoch: p.epoch,
                    round: p.round,
                    commit_timestamp_ms: p.commit_timestamp_ms,
            ...
            TransactionKind::ConsensusCommitPrologueV3(p) => {
                Self::ConsensusCommitPrologueV3(AiyConsensusCommitPrologueV3 {
                    epoch: p.epoch,
                    round: p.round,
            ...
            TransactionKind::ConsensusCommitPrologueV4(p) => {
                Self::ConsensusCommitPrologueV4(AiyConsensusCommitPrologueV4 {
                    epoch: p.epoch,
                    round: p.round,
                    sub_dag_index: p.sub_dag_index,
                    commit_timestamp_ms: p.commit_timestamp_ms,
            ...
            TransactionKind::AuthenticatorStateUpdate(update) => {
                Self::AuthenticatorStateUpdate(AiyAuthenticatorStateUpdate {
                    epoch: update.epoch,
                    round: update.round,
                    new_active_jwks: update
                        .new_active_jwks
                        .into_iter()
                        .map(AiyActiveJwk::from)
                        .collect(),
                })
            }
            TransactionKind::RandomnessStateUpdate(update) => {
                Self::RandomnessStateUpdate(AiyRandomnessStateUpdate {
                    epoch: update.epoch,
            ...
            TransactionKind::EndOfEpochTransaction(end_of_epoch_tx) => {
                Self::EndOfEpochTransaction(AiyEndOfEpochTransaction {
                    transactions: end_of_epoch_tx
                        .into_iter()
                        .map(|tx| match tx {
                            EndOfEpochTransactionKind::ChangeEpoch(e) => {
                                AiyEndOfEpochTransactionKind::ChangeEpoch(e.into())
                            }
                            EndOfEpochTransactionKind::AuthenticatorStateCreate => {
                                AiyEndOfEpochTransactionKind::AuthenticatorStateCreate
                            }
                            EndOfEpochTransactionKind::AuthenticatorStateExpire(expire) => {
                                AiyEndOfEpochTransactionKind::AuthenticatorStateExpire(
                                    AiyAuthenticatorStateExpire {
                                        min_epoch: expire.min_epoch,
                                    },
                                )
                            }
                            EndOfEpochTransactionKind::RandomnessStateCreate => {
                                AiyEndOfEpochTransactionKind::RandomnessStateCreate
                            }
                            EndOfEpochTransactionKind::DenyListStateCreate => {
                                AiyEndOfEpochTransactionKind::CoinDenyListStateCreate
                            }
                            EndOfEpochTransactionKind::BridgeStateCreate(chain_id) => {
                                AiyEndOfEpochTransactionKind::BridgeStateCreate(
                                    (*chain_id.as_bytes()).into(),
                                )
                            }
                            EndOfEpochTransactionKind::BridgeCommitteeInit(
                                bridge_shared_version,
                            ) => AiyEndOfEpochTransactionKind::BridgeCommitteeUpdate(
                                bridge_shared_version,
                            ),
                            EndOfEpochTransactionKind::StoreExecutionTimeObservations(_) => {
                                AiyEndOfEpochTransactionKind::StoreExecutionTimeObservations
                            }
                            EndOfEpochTransactionKind::AccumulatorRootCreate => {
                                AiyEndOfEpochTransactionKind::AccumulatorRootCreate
                            }
                            EndOfEpochTransactionKind::CoinRegistryCreate => {
                                AiyEndOfEpochTransactionKind::CoinRegistryCreate
                            }
                        })
                        .collect(),
                })
    ...
    fn try_from_with_module_cache(
        ...
        match tx {
            TransactionKind::ProgrammableTransaction(p) => Ok(Self::ProgrammableTransaction(
                AiyProgrammableTransactionBlock::try_from_with_module_cache(p, module_cache)?,
            )),
        ...
        match tx {
            TransactionKind::ProgrammableSystemTransaction(p) => {
                Ok(Self::ProgrammableSystemTransaction(
                    AiyProgrammableTransactionBlock::try_from_with_package_resolver(
                        p,
                        package_resolver,
                    )
                    .await?,
...
impl From<ChangeEpoch> for AiyChangeEpoch {
...
#[enum_dispatch(AiyTransactionBlockEffectsAPI)]
#[serde(
    rename = "TransactionBlockEffects",
    rename_all = "camelCase",
    tag = "messageVersion"
)]
pub enum AiyTransactionBlockEffects {
    V1(AiyTransactionBlockEffectsV1),
}

#[enum_dispatch]
pub trait AiyTransactionBlockEffectsAPI {
    fn status(&self) -> &AiyExecutionStatus;
    fn into_status(self) -> AiyExecutionStatus;
    fn shared_objects(&self) -> &[AiyObjectRef];
    fn created(&self) -> &[OwnedObjectRef];
    fn mutated(&self) -> &[OwnedObjectRef];
    fn unwrapped(&self) -> &[OwnedObjectRef];
    fn deleted(&self) -> &[AiyObjectRef];
    fn unwrapped_then_deleted(&self) -> &[AiyObjectRef];
    fn wrapped(&self) -> &[AiyObjectRef];
    ...
    fn all_deleted_objects(&self) -> Vec<(&AiyObjectRef, DeleteKind)>;

    fn accumulator_events(&self) -> Vec<AiyAccumulatorEvent>;
}
...
pub struct AiyTransactionBlockEffectsModifiedAtVersions {
    object_id: ObjectID,
...
pub struct AiyAccumulatorEvent {
    pub accumulator_obj: ObjectID,
    pub address: AiyAddress,
    pub ty: AiyTypeTag,
    pub operation: AiyAccumulatorOperation,
    pub value: AiyAccumulatorValue,
}

impl From<AccumulatorEvent> for AiyAccumulatorEvent {
...
pub enum AiyAccumulatorOperation {
    Merge,
    Split,
}

impl From<AccumulatorOperation> for AiyAccumulatorOperation {
...
pub enum AiyAccumulatorValue {
    Integer(u64),
    IntegerTuple(u64, u64),
    EventDigest(u64 /* event index in the transaction */, Digest),
}

impl From<AccumulatorValue> for AiyAccumulatorValue {
...
pub struct AiyTransactionBlockEffectsV1 {
    /// The status of the execution
    pub status: AiyExecutionStatus,
    ...
    pub modified_at_versions: Vec<AiyTransactionBlockEffectsModifiedAtVersions>,
    /// The object references of the shared objects used in this transaction. Empty if no shared objects were used.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub shared_objects: Vec<AiyObjectRef>,
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub deleted: Vec<AiyObjectRef>,
    /// Object refs of objects previously wrapped in other objects but now deleted.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub unwrapped_then_deleted: Vec<AiyObjectRef>,
    /// Object refs of objects now wrapped in other objects.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub wrapped: Vec<AiyObjectRef>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub accumulator_events: Vec<AiyAccumulatorEvent>,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub abort_error: Option<AiyMoveAbort>,
}

// TODO move additional error info here

impl AiyTransactionBlockEffectsAPI for AiyTransactionBlockEffectsV1 {
    fn status(&self) -> &AiyExecutionStatus {
        &self.status
    }
    fn into_status(self) -> AiyExecutionStatus {
...
impl AiyTransactionBlockEffects {
    pub fn new_for_testing(
        transaction_digest: TransactionDigest,
        status: AiyExecutionStatus,
    ) -> Self {
        Self::V1(AiyTransactionBlockEffectsV1 {
            transaction_digest,
            status,
            gas_object: OwnedObjectRef {
                owner: Owner::AddressOwner(AiyAddress::random_for_testing_only()),
                reference: aiy_types::base_types::random_object_ref().into(),
            },
            executed_epoch: 0,
...
impl TryFrom<TransactionEffects> for AiyTransactionBlockEffects {
    type Error = AiyError;

    fn try_from(effect: TransactionEffects) -> Result<Self, Self::Error> {
        Ok(AiyTransactionBlockEffects::V1(
            AiyTransactionBlockEffectsV1 {
                ...
                modified_at_versions: effect
                    ...
                    .map(|(object_id, sequence_number)| {
                        AiyTransactionBlockEffectsModifiedAtVersions {
                            object_id,
                            sequence_number,
                        }
                    })
                ...
                shared_objects: to_aiy_object_ref(
                    effect
                ...
                deleted: to_aiy_object_ref(effect.deleted().to_vec()),
                unwrapped_then_deleted: to_aiy_object_ref(effect.unwrapped_then_deleted().to_vec()),
                wrapped: to_aiy_object_ref(effect.wrapped().to_vec()),
                gas_object: OwnedObjectRef {
                ...
                abort_error: effect
                    .move_abort()
                    .map(|(abort, code)| AiyMoveAbort::new(abort, code)),
                accumulator_events: effect
                    .accumulator_events()
                    .into_iter()
                    .map(AiyAccumulatorEvent::from)
                    .collect(),
            },
        ))
    }
...
fn objref_string(obj: &AiyObjectRef) -> String {
    format!(
        " ┌──\n │ ID: {} \n │ Version: {} \n │ Digest: {}\n └──",
        obj.object_id,
...
impl Display for AiyTransactionBlockEffects {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut builder = TableBuilder::default();
...
pub struct DryRunTransactionBlockResponse {
    pub effects: AiyTransactionBlockEffects,
    pub events: AiyTransactionBlockEvents,
    pub object_changes: Vec<ObjectChange>,
    pub balance_changes: Vec<BalanceChange>,
    pub input: AiyTransactionBlockData,
...
pub struct AiyTransactionBlockEvents {
    pub data: Vec<AiyEvent>,
}

impl AiyTransactionBlockEvents {
    pub fn try_from(
        ...
        Ok(Self {
            data: events
                ...
                .map(|(seq, event)| {
                    ...
                    AiyEvent::try_from(event, tx_digest, seq as u64, timestamp_ms, layout)
                })
                .collect::<Result<_, _>>()?,
        })
    }
...
impl Display for AiyTransactionBlockEvents {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
pub struct DevInspectArgs {
    ...
    pub gas_sponsor: Option<AiyAddress>,
...
pub struct DevInspectResults {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub results: Option<Vec<AiyExecutionResult>>,
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename = "AiyExecutionResult", rename_all = "camelCase")]
pub struct AiyExecutionResult {
    /// The value of any arguments that were mutably borrowed.
    /// Non-mut borrowed values are not included
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub mutable_reference_outputs: Vec<(/* argument */ AiyArgument, Vec<u8>, AiyTypeTag)>,
    /// The return values from the transaction
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub return_values: Vec<(Vec<u8>, AiyTypeTag)>,
}
...
impl DevInspectResults {
    pub fn new(
                    ...
                    srvs.into_iter()
                        .map(|srv| {
                            ...
                            let mutable_reference_outputs = mutable_reference_outputs
                                .into_iter()
                                .map(|(a, bytes, tag)| (a.into(), bytes, AiyTypeTag::from(tag)))
                                .collect();
                            let return_values = return_values
                                .into_iter()
                                .map(|(bytes, tag)| (bytes, AiyTypeTag::from(tag)))
                                .collect();
                            AiyExecutionResult {
        ...
        Ok(Self {
            ...
            events: AiyTransactionBlockEvents::try_from(events, tx_digest, None, resolver)?,
            results,
            error,
            raw_txn_data,
            raw_effects,
...
#[derive(Eq, PartialEq, Clone, Debug, Serialize, Deserialize, JsonSchema)]
pub enum AiyTransactionBlockBuilderMode {
    /// Regular Aiy Transactions that are committed on chain
    Commit,
...
impl Display for AiyExecutionStatus {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
...
impl AiyExecutionStatus {
    pub fn is_ok(&self) -> bool {
        matches!(self, AiyExecutionStatus::Success { .. })
    }
    pub fn is_err(&self) -> bool {
        matches!(self, AiyExecutionStatus::Failure { .. })
    }
}

impl From<ExecutionStatus> for AiyExecutionStatus {
...
fn to_aiy_object_ref(refs: Vec<ObjectRef>) -> Vec<AiyObjectRef> {
    refs.into_iter().map(AiyObjectRef::from).collect()
}
...
pub struct AiyGasData {
    pub payment: Vec<AiyObjectRef>,
    pub owner: AiyAddress,
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub price: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[derive(Debug, Deserialize, Serialize, JsonSchema, Clone, PartialEq, Eq)]
#[enum_dispatch(AiyTransactionBlockDataAPI)]
#[serde(
    rename = "TransactionBlockData",
    rename_all = "camelCase",
    tag = "messageVersion"
)]
pub enum AiyTransactionBlockData {
    V1(AiyTransactionBlockDataV1),
}

#[enum_dispatch]
pub trait AiyTransactionBlockDataAPI {
    fn transaction(&self) -> &AiyTransactionBlockKind;
    fn sender(&self) -> &AiyAddress;
    fn gas_data(&self) -> &AiyGasData;
}

#[derive(Debug, Deserialize, Serialize, JsonSchema, Clone, PartialEq, Eq)]
#[serde(rename = "TransactionBlockDataV1", rename_all = "camelCase")]
pub struct AiyTransactionBlockDataV1 {
    pub transaction: AiyTransactionBlockKind,
    pub sender: AiyAddress,
    pub gas_data: AiyGasData,
}

impl AiyTransactionBlockDataAPI for AiyTransactionBlockDataV1 {
    fn transaction(&self) -> &AiyTransactionBlockKind {
        &self.transaction
    }
    fn sender(&self) -> &AiyAddress {
...
impl AiyTransactionBlockData {
    pub fn move_calls(&self) -> Vec<&AiyProgrammableMoveCall> {
        match self {
            Self::V1(data) => match &data.transaction {
                AiyTransactionBlockKind::ProgrammableTransaction(pt) => pt
                    .commands
                    .iter()
                    .filter_map(|command| match command {
                        AiyCommand::MoveCall(c) => Some(&**c),
                        _ => None,
                    })
                    .collect(),
                _ => vec![],
    ...
    fn try_from_inner(
        ...
        transaction: AiyTransactionBlockKind,
    ) -> Result<Self, anyhow::Error> {
        let message_version = data.message_version();
        let sender = data.sender();
        let gas_data = AiyGasData {
            payment: data
                .gas()
                .iter()
                .map(|obj_ref| AiyObjectRef::from(*obj_ref))
                .collect(),
        ...
        match message_version {
            1 => Ok(AiyTransactionBlockData::V1(AiyTransactionBlockDataV1 {
                transaction,
                sender,
                gas_data,
            })),
    ...
    pub fn try_from_with_module_cache(
        ...
        let transaction = AiyTransactionBlockKind::try_from_with_module_cache(
    ...
    pub async fn try_from_with_package_resolver(
        ...
        let transaction = AiyTransactionBlockKind::try_from_with_package_resolver(
            data.clone().into_kind(),
            package_resolver,
        )
        .await?;
...
impl Display for AiyTransactionBlockData {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::V1(data) => {
...
pub struct AiyTransactionBlock {
    pub data: AiyTransactionBlockData,
    pub tx_signatures: Vec<GenericSignature>,
}

impl AiyTransactionBlock {
    pub fn try_from(
        data: SenderSignedData,
        module_cache: &impl GetModule,
    ) -> Result<Self, anyhow::Error> {
        Ok(Self {
            data: AiyTransactionBlockData::try_from_with_module_cache(
    ...
    // TODO: the AiyTransactionBlock `try_from` can be removed after cleaning up indexer v1, so are the related
    // `try_from` methods for nested structs like AiyTransactionBlockData etc.
    pub async fn try_from_with_package_resolver(
        data: SenderSignedData,
        package_resolver: &Resolver<impl PackageStore>,
    ) -> Result<Self, anyhow::Error> {
        Ok(Self {
            data: AiyTransactionBlockData::try_from_with_package_resolver(
                data.intent_message().value.clone(),
                package_resolver,
            )
            .await?,
...
impl Display for AiyTransactionBlock {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut builder = TableBuilder::default();
        ...
        for tx_sig in &self.tx_signatures {
            builder.push_record(vec![format!(
                ...
                match tx_sig {
                    ...
                    _ => Base64::from_bytes(tx_sig.as_ref()).encoded(), // the signatures for multisig and zklogin are not aiyted to be parsed out. they should be interpreted as a whole
                }
            )]);
        }
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyGenesisTransaction {
    pub objects: Vec<ObjectID>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyConsensusCommitPrologue {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyConsensusCommitPrologueV2 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyConsensusCommitPrologueV3 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyConsensusCommitPrologueV4 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyAuthenticatorStateUpdate {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
    ...
    pub new_active_jwks: Vec<AiyActiveJwk>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyRandomnessStateUpdate {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyEndOfEpochTransaction {
    pub transactions: Vec<AiyEndOfEpochTransactionKind>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub enum AiyEndOfEpochTransactionKind {
    ChangeEpoch(AiyChangeEpoch),
    AuthenticatorStateCreate,
    AuthenticatorStateExpire(AiyAuthenticatorStateExpire),
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyAuthenticatorStateExpire {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub min_epoch: u64,
}
...
impl From<ActiveJwk> for AiyActiveJwk {
...
pub enum AiyInputObjectKind {
    // A Move package, must be immutable.
    MovePackage(ObjectID),
    // A Move object, either immutable, or owned mutable.
    ImmOrOwnedMoveObject(AiyObjectRef),
...
/// commands
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyProgrammableTransactionBlock {
    /// Input objects or primitive values
    pub inputs: Vec<AiyCallArg>,
    ...
    pub commands: Vec<AiyCommand>,
}

impl Display for AiyProgrammableTransactionBlock {
...
impl AiyProgrammableTransactionBlock {
    fn try_from_with_module_cache(
        ...
        Ok(AiyProgrammableTransactionBlock {
            inputs: inputs
                .into_iter()
                .zip(input_types)
                .map(|(arg, layout)| AiyCallArg::try_from(arg, layout.as_ref()))
                .collect::<Result<_, _>>()?,
            commands: commands.into_iter().map(AiyCommand::from).collect(),
        })
    }
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename = "AiyTransaction")]
pub enum AiyCommand {
    /// A call to either an entry or a public Move function
    MoveCall(Box<AiyProgrammableMoveCall>),
    /// `(Vec<forall T:key+store. T>, address)`
    /// It sends n-objects to the specified address. These objects must have store
    /// (public transfer) and either the previous owner must be an address or the object must
    /// be newly created.
    TransferObjects(Vec<AiyArgument>, AiyArgument),
    /// `(&mut Coin<T>, Vec<u64>)` -> `Vec<Coin<T>>`
    /// It splits off some amounts into a new coins with those amounts
    SplitCoins(AiyArgument, Vec<AiyArgument>),
    /// `(&mut Coin<T>, Vec<Coin<T>>)`
    /// It merges n-coins into the first coin
    MergeCoins(AiyArgument, Vec<AiyArgument>),
    /// Publishes a Move package. It takes the package bytes and a list of the package's transitive
    /// dependencies to link against on-chain.
    Publish(Vec<ObjectID>),
    /// Upgrades a Move package
    Upgrade(Vec<ObjectID>, ObjectID, AiyArgument),
    /// `forall T: Vec<T> -> vector<T>`
    /// Given n-values of the same type, it constructs a vector. For non objects or an empty vector,
    /// the type tag must be specified.
    MakeMoveVec(Option<String>, Vec<AiyArgument>),
}

impl Display for AiyCommand {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MoveCall(p) => {
                write!(f, "MoveCall({p})")
...
impl From<Command> for AiyCommand {
    fn from(value: Command) -> Self {
        match value {
            Command::MoveCall(m) => AiyCommand::MoveCall(Box::new((*m).into())),
            Command::TransferObjects(args, arg) => AiyCommand::TransferObjects(
                args.into_iter().map(AiyArgument::from).collect(),
                arg.into(),
            ),
            Command::SplitCoins(arg, args) => AiyCommand::SplitCoins(
                arg.into(),
                args.into_iter().map(AiyArgument::from).collect(),
            ),
            Command::MergeCoins(arg, args) => AiyCommand::MergeCoins(
                arg.into(),
                args.into_iter().map(AiyArgument::from).collect(),
            ),
            Command::Publish(_modules, dep_ids) => AiyCommand::Publish(dep_ids),
            Command::MakeMoveVec(tag_opt, args) => AiyCommand::MakeMoveVec(
                tag_opt.map(|tag| tag.to_string()),
                args.into_iter().map(AiyArgument::from).collect(),
            ),
            Command::Upgrade(_modules, dep_ids, current_package_id, ticket) => {
                AiyCommand::Upgrade(dep_ids, current_package_id, AiyArgument::from(ticket))
            }
        }
    }
}
...
impl From<Argument> for AiyArgument {
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct AiyProgrammableMoveCall {
...
impl Display for AiyProgrammableMoveCall {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let Self {
            package,
            module,
...
impl From<ProgrammableMoveCall> for AiyProgrammableMoveCall {
    fn from(value: ProgrammableMoveCall) -> Self {
        ...
        Self {
            ...
            arguments: arguments.into_iter().map(AiyArgument::from).collect(),
        }
    }
}
...
impl From<InputObjectKind> for AiyInputObjectKind {
...
impl TryInto<TypeTag> for AiyTypeTag {
    type Error = anyhow::Error;
    fn try_into(self) -> Result<TypeTag, Self::Error> {
        parse_aiy_type_tag(&self.0)
    }
}

impl From<TypeTag> for AiyTypeTag {
...
pub struct MoveCallParams {
    ...
    pub function: String,
    #[serde(default)]
    pub type_arguments: Vec<AiyTypeTag>,
    pub arguments: Vec<AiyJsonValue>,
}
...
pub struct TransactionBlockBytes {
    ...
    pub gas: Vec<AiyObjectRef>,
    /// objects to be used in this transaction
    pub input_objects: Vec<AiyInputObjectKind>,
}
...
pub enum AiyCallArg {
    ...
    FundsWithdrawal(AiyFundsWithdrawalArg),
}
...
impl AiyCallArg {
    pub fn try_from(
        ...
        Ok(match value {
            CallArg::Pure(p) => AiyCallArg::Pure(AiyPureValue {
                value_type: layout.map(|l| l.into()),
                value: AiyJsonValue::from_bcs_bytes(layout, &p)?,
            }),
            CallArg::Object(ObjectArg::ImmOrOwnedObject((id, version, digest))) => {
                AiyCallArg::Object(AiyObjectArg::ImmOrOwnedObject {
                    object_id: id,
                    version,
                    digest,
                })
            ...
            CallArg::Object(ObjectArg::SharedObject {
                ...
                initial_shared_version,
                mutable,
            }) => AiyCallArg::Object(AiyObjectArg::SharedObject {
                object_id: id,
                initial_shared_version,
                mutable,
            }),
            CallArg::Object(ObjectArg::Receiving((object_id, version, digest))) => {
                AiyCallArg::Object(AiyObjectArg::Receiving {
                    object_id,
                    version,
                    digest,
                })
            }
            CallArg::FundsWithdrawal(arg) => AiyCallArg::FundsWithdrawal(AiyFundsWithdrawalArg {
                reservation: match arg.reservation {
                    Reservation::EntireBalance => AiyReservation::EntireBalance,
                    Reservation::MaxAmountU64(amount) => AiyReservation::MaxAmountU64(amount),
                },
                type_arg: match arg.type_arg {
                    WithdrawalTypeArg::Balance(type_input) => {
                        AiyWithdrawalTypeArg::Balance(type_input.to_type_tag()?.into())
                    }
                },
                withdraw_from: match arg.withdraw_from {
                    WithdrawFrom::Sender => AiyWithdrawFrom::Sender,
                },
            }),
        })
    }

    pub fn pure(&self) -> Option<&AiyJsonValue> {
        match self {
            AiyCallArg::Pure(v) => Some(&v.value),
            _ => None,
        }
    }
    ...
    pub fn object(&self) -> Option<&ObjectID> {
        match self {
            AiyCallArg::Object(AiyObjectArg::SharedObject { object_id, .. })
            | AiyCallArg::Object(AiyObjectArg::ImmOrOwnedObject { object_id, .. })
            | AiyCallArg::Object(AiyObjectArg::Receiving { object_id, .. }) => Some(object_id),
            _ => None,
        }
    }
}
...
#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct AiyPureValue {
    #[schemars(with = "Option<String>")]
    #[serde_as(as = "Option<AsAiyTypeTag>")]
    value_type: Option<TypeTag>,
    value: AiyJsonValue,
}

impl AiyPureValue {
    pub fn value(&self) -> AiyJsonValue {
        self.value.clone()
    }
...
#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub enum AiyWithdrawalTypeArg {
    Balance(AiyTypeTag),
}

#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub enum AiyWithdrawFrom {
    Sender,
}

#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct AiyFundsWithdrawalArg {
    pub reservation: AiyReservation,
    pub type_arg: AiyWithdrawalTypeArg,
    pub withdraw_from: AiyWithdrawFrom,
}
...
impl From<EffectsWithInput> for AiyTransactionBlockEffects {
...
pub enum TransactionFilter {
    ...
    FromAddress(AiyAddress),
    /// Query by recipient address.
    ToAddress(AiyAddress),
    /// Query by sender and recipient address.
    FromAndToAddress { from: AiyAddress, to: AiyAddress },
    /// CURRENTLY NOT SUPPORTED. Query txs that have a given address as sender or recipient.
    FromOrToAddress { addr: AiyAddress },
