...
use aiy_types::base_types::{
    ObjectDigest, ObjectID, ObjectInfo, ObjectRef, ObjectType, SequenceNumber, AiyAddress,
    TransactionDigest,
};
use aiy_types::error::{
    ExecutionError, AiyError, AiyObjectResponseError, AiyResult, UserInputError, UserInputResult,
};
use aiy_types::gas_coin::GasCoin;
use aiy_types::messages_checkpoint::CheckpointSequenceNumber;
use aiy_types::move_package::{MovePackage, TypeOrigin, UpgradeInfo};
use aiy_types::object::{Data, MoveObject, Object, ObjectInner, ObjectRead, Owner};
use aiy_types::aiy_serde::BigInt;
use aiy_types::aiy_serde::SequenceNumber as AsSequenceNumber;
use aiy_types::aiy_serde::AiyStructTag;

use crate::{Page, AiyMoveStruct, AiyMoveValue};

#[derive(Serialize, Deserialize, Debug, JsonSchema, Clone, PartialEq, Eq)]
pub struct AiyObjectResponse {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub data: Option<AiyObjectData>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub error: Option<AiyObjectResponseError>,
}

impl AiyObjectResponse {
    pub fn new(data: Option<AiyObjectData>, error: Option<AiyObjectResponseError>) -> Self {
        Self { data, error }
    }

    pub fn new_with_data(data: AiyObjectData) -> Self {
        Self {
            data: Some(data),
            error: None,
        }
    ...
    pub fn new_with_error(error: AiyObjectResponseError) -> Self {
        Self {
            data: None,
            error: Some(error),
        }
...
impl Ord for AiyObjectResponse {
    fn cmp(&self, other: &Self) -> Ordering {
        match (&self.data, &other.data) {
            ...
            // In this ordering those with data will come before AiyObjectResponses that are errors.
            (Some(_), None) => Ordering::Less,
            (None, Some(_)) => Ordering::Greater,
            // AiyObjectResponses that are errors are just considered equal.
            _ => Ordering::Equal,
        }
    }
}

impl PartialOrd for AiyObjectResponse {
...
impl AiyObjectResponse {
    pub fn move_object_bcs(&self) -> Option<&Vec<u8>> {
        match &self.data {
            Some(AiyObjectData {
                bcs: Some(AiyRawData::MoveObject(obj)),
                ..
            }) => Some(&obj.bcs_bytes),
            _ => None,
        }
    ...
    pub fn object_id(&self) -> Result<ObjectID, anyhow::Error> {
        match (&self.data, &self.error) {
            ...
            (None, Some(AiyObjectResponseError::NotExists { object_id })) => Ok(*object_id),
            (
                None,
                Some(AiyObjectResponseError::Deleted {
                    object_id,
                    version: _,
                    digest: _,
                }),
            ) => Ok(*object_id),
            _ => Err(anyhow!("Could not get object_id, something went wrong with AiyObjectResponse construction.")),
        }
    }
...
impl TryFrom<AiyObjectResponse> for ObjectInfo {
    type Error = anyhow::Error;

    fn try_from(value: AiyObjectResponse) -> Result<Self, Self::Error> {
        let AiyObjectData {
            object_id,
            version,
            digest,
...
pub struct AiyObjectData {
    ...
    /// The type of the object. Default to be None unless AiyObjectDataOptions.showType is set to true
    ...
    /// The owner of this object. Default to be None unless AiyObjectDataOptions.showOwner is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    pub owner: Option<Owner>,
    /// The digest of the transaction that created or last mutated this object. Default to be None unless
    /// AiyObjectDataOptions.showPreviousTransaction is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// The Display metadata for frontend UI rendering, default to be None unless AiyObjectDataOptions.showContent is set to true
    /// This can also be None if the struct type does not have Display defined
    /// See more details in <https://forums.sui.io/t/nft-object-display-proposal/4872>
    #[serde(skip_serializing_if = "Option::is_none")]
    pub display: Option<DisplayFieldsResponse>,
    /// Move object content or package content, default to be None unless AiyObjectDataOptions.showContent is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    pub content: Option<AiyParsedData>,
    /// Move object content or package content in BCS, default to be None unless AiyObjectDataOptions.showBcs is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    pub bcs: Option<AiyRawData>,
}
...
impl AiyObjectData {
    ...
    pub fn try_into_object(
        ...
        let data = match self.bcs {
            Some(AiyRawData::MoveObject(o)) => Data::Move(unsafe {
            ...
            Some(AiyRawData::Package(p)) => Data::Package(MovePackage::new(
                p.id,
                self.version,
                p.module_map,
            ...
            _ => Err(anyhow!(
                "BCS data is required to convert AiyObjectData to Object"
            ))?,
        };
        Ok(ObjectInner {
            data,
            owner: self
                .owner
                .ok_or_else(|| anyhow!("Owner is required to convert AiyObjectData to Object"))?,
            previous_transaction: self.previous_transaction.ok_or_else(|| {
                anyhow!("previous_transaction is required to convert AiyObjectData to Object")
            })?,
            storage_rebate: self.storage_rebate.ok_or_else(|| {
                anyhow!("storage_rebate is required to convert AiyObjectData to Object")
            })?,
        }
        .into())
    }
...
impl Display for AiyObjectData {
...
impl TryFrom<&AiyObjectData> for GasCoin {
    type Error = anyhow::Error;
    fn try_from(object: &AiyObjectData) -> Result<Self, Self::Error> {
        match &object
            .content
            .as_ref()
            .ok_or_else(|| anyhow!("Expect object content to not be empty"))?
        {
            AiyParsedData::MoveObject(o) => {
                if GasCoin::type_() == o.type_ {
                    return GasCoin::try_from(&o.fields);
                }
            }
            AiyParsedData::Package(_) => {}
        }
...
impl TryFrom<&AiyMoveStruct> for GasCoin {
    type Error = anyhow::Error;
    fn try_from(move_struct: &AiyMoveStruct) -> Result<Self, Self::Error> {
        match move_struct {
            AiyMoveStruct::WithFields(fields) | AiyMoveStruct::WithTypes { type_: _, fields } => {
                if let Some(AiyMoveValue::String(balance)) = fields.get("balance") {
                    if let Ok(balance) = balance.parse::<u64>() {
                        if let Some(AiyMoveValue::UID { id }) = fields.get("id") {
...
pub struct AiyObjectDataOptions {
...
impl TryFrom<(ObjectRead, AiyObjectDataOptions)> for AiyObjectResponse {
    type Error = anyhow::Error;

    fn try_from(
        (object_read, options): (ObjectRead, AiyObjectDataOptions),
    ) -> Result<Self, Self::Error> {
        match object_read {
            ObjectRead::NotExists(id) => Ok(AiyObjectResponse::new_with_error(
                AiyObjectResponseError::NotExists { object_id: id },
            )),
            ObjectRead::Exists(object_ref, o, layout) => {
                let data = (object_ref, o, layout, options).try_into()?;
                Ok(AiyObjectResponse::new_with_data(data))
            }
            ObjectRead::Deleted((object_id, version, digest)) => Ok(
                AiyObjectResponse::new_with_error(AiyObjectResponseError::Deleted {
                    object_id,
                    version,
                    digest,
                }),
...
impl TryFrom<(ObjectInfo, AiyObjectDataOptions)> for AiyObjectResponse {
    type Error = anyhow::Error;

    fn try_from(
        (object_info, options): (ObjectInfo, AiyObjectDataOptions),
    ) -> Result<Self, Self::Error> {
        let AiyObjectDataOptions {
            show_type,
            show_owner,
        ...
        Ok(Self::new_with_data(AiyObjectData {
    ...
    )> for AiyObjectData
{
    ...
    fn try_from(
        ...
        let content: Option<AiyParsedData> = if show_content {
            let data = match o.data {
                Data::Move(m) => {
                    let layout = layout.ok_or_else(|| {
                        anyhow!("Layout is required to convert Move object to json")
                    })?;
                    AiyParsedData::try_from_object(m, layout)?
                }
                Data::Package(p) => AiyParsedData::try_from_package(p)?,
            };
            Some(data)
        } else {
            None
    ...
    )> for AiyObjectData
{
    ...
    fn try_from(
        ...
        let mut data: AiyObjectData = (object_ref, o, layout, options).try_into()?;
        if show_display {
...
impl AiyObjectResponse {
    ...
    pub fn object(&self) -> Result<&AiyObjectData, AiyObjectResponseError> {
        if let Some(data) = &self.data {
            Ok(data)
            ...
            Err(AiyObjectResponseError::Unknown)
        }
    }

    /// Returns the object value if there is any, otherwise an Err if
    /// the object does not exist or is deleted.
    pub fn into_object(self) -> Result<AiyObjectData, AiyObjectResponseError> {
        match self.object() {
...
impl TryInto<Object> for AiyObjectData {
    type Error = anyhow::Error;
...
impl From<ObjectRef> for AiyObjectRef {
...
pub enum AiyRawData {
    ...
    MoveObject(AiyRawMoveObject),
    Package(AiyRawMovePackage),
}

impl AiyData for AiyRawData {
    type ObjectType = AiyRawMoveObject;
    type PackageType = AiyRawMovePackage;
...
pub enum AiyParsedData {
    // Manually handle generic schema generation
    MoveObject(AiyParsedMoveObject),
    Package(AiyMovePackage),
}

impl AiyData for AiyParsedData {
    type ObjectType = AiyParsedMoveObject;
    type PackageType = AiyMovePackage;

    fn try_from_object(
        object: MoveObject,
        layout: MoveStructLayout,
    ) -> Result<Self, anyhow::Error> {
        Ok(Self::MoveObject(AiyParsedMoveObject::try_from_layout(
            object, layout,
        )?))
    }
    ...
    fn try_from_package(package: MovePackage) -> Result<Self, anyhow::Error> {
        ...
        for bytecode in package.serialized_module_map().values() {
            ...
            let module = move_binary_format::CompiledModule::deserialize_with_defaults(bytecode)
                .map_err(|error| AiyError::ModuleDeserializationFailure {
            ...
            let d = move_disassembler::disassembler::Disassembler::from_module_with_max_size(
                ...
                *aiy_types::move_package::MAX_DISASSEMBLED_MODULE_SIZE,
            )
            .map_err(|e| AiyError::ObjectSerializationError {
                error: e.to_string(),
            })?;
            let bytecode_str = d
                .disassemble()
        ...
        Ok(Self::Package(AiyMovePackage { disassembled }))
    }
...
impl Display for AiyParsedData {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut writer = String::new();
        match self {
            AiyParsedData::MoveObject(o) => {
...
pub struct AiyParsedMoveObject {
    #[serde(rename = "type")]
    #[serde_as(as = "AiyStructTag")]
    #[schemars(with = "String")]
    pub type_: StructTag,
    pub has_public_transfer: bool,
    pub fields: AiyMoveStruct,
}

impl AiyMoveObject for AiyParsedMoveObject {
    fn try_from_layout(
        object: MoveObject,
        layout: MoveStructLayout,
    ) -> Result<Self, anyhow::Error> {
        ...
        Ok(
            if let AiyMoveStruct::WithTypes { type_, fields } = move_struct {
                AiyParsedMoveObject {
                    type_,
                    has_public_transfer: object.has_public_transfer(),
                    fields: AiyMoveStruct::WithFields(fields),
                }
            } else {
                AiyParsedMoveObject {
...
impl AiyParsedMoveObject {
    pub fn try_from_object_read(object_read: ObjectRead) -> Result<Self, anyhow::Error> {
        let parsed_data = AiyParsedData::try_from_object_read(object_read)?;
        match parsed_data {
            AiyParsedData::MoveObject(o) => Ok(o),
            AiyParsedData::Package(_) => Err(anyhow::anyhow!("Object is not a Move object")),
        }
    }
}
...
) -> AiyResult<(StructTag, serde_json::Value)> {
    match event_data.into() {
        AiyMoveValue::Struct(move_struct) => match &move_struct {
            AiyMoveStruct::WithTypes { type_, .. } => {
                Ok((type_.clone(), move_struct.clone().to_json_value()))
            }
            _ => Err(AiyError::ObjectDeserializationError {
                error: "Found non-type AiyMoveStruct in MoveValue event".to_string(),
            }),
        },
        AiyMoveValue::Variant(v) => Ok((v.type_.clone(), v.clone().to_json_value())),
        AiyMoveValue::Vector(_)
        | AiyMoveValue::Number(_)
        | AiyMoveValue::Bool(_)
        | AiyMoveValue::Address(_)
        | AiyMoveValue::String(_)
        | AiyMoveValue::UID { .. }
        | AiyMoveValue::Option(_) => Err(AiyError::ObjectDeserializationError {
...
impl From<MoveObject> for AiyRawMoveObject {
...
impl AiyMoveObject for AiyRawMoveObject {
    fn try_from_layout(
        object: MoveObject,
...
impl From<MovePackage> for AiyRawMovePackage {
...
pub enum AiyPastObjectResponse {
    /// The object exists and is found with this version
    VersionFound(AiyObjectData),
    /// The object does not exist
    ObjectNotExists(ObjectID),
    /// The object is found to be deleted with this version
    ObjectDeleted(AiyObjectRef),
...
impl AiyPastObjectResponse {
    /// Returns a reference to the object if there is any, otherwise an Err
    pub fn object(&self) -> UserInputResult<&AiyObjectData> {
        match &self {
    ...
    pub fn into_object(self) -> UserInputResult<AiyObjectData> {
        match self {
            Self::ObjectDeleted(oref) => Err(UserInputError::ObjectDeleted {
                object_ref: oref.to_object_ref(),
            }),
...
pub struct AiyMovePackage {
    pub disassembled: BTreeMap<String, Value>,
}

pub type QueryObjectsPage = Page<AiyObjectResponse, CheckpointedObjectID>;
pub type ObjectsPage = Page<AiyObjectResponse, ObjectID>;
...
pub struct AiyGetPastObjectRequest {
...
pub enum AiyObjectDataFilter {
    MatchAll(Vec<AiyObjectDataFilter>),
    MatchAny(Vec<AiyObjectDataFilter>),
    MatchNone(Vec<AiyObjectDataFilter>),
...
impl AiyObjectDataFilter {
    ...
    pub fn matches(&self, object: &ObjectInfo) -> bool {
        match self {
            AiyObjectDataFilter::MatchAll(filters) => !filters.iter().any(|f| !f.matches(object)),
            AiyObjectDataFilter::MatchAny(filters) => filters.iter().any(|f| f.matches(object)),
            AiyObjectDataFilter::MatchNone(filters) => !filters.iter().any(|f| f.matches(object)),
            AiyObjectDataFilter::StructType(s) => {
                ...
                // e.g. `0x2::coin::Coin` can match `0x2::coin::Coin<0x2::aiy::AIY>`
            ...
            AiyObjectDataFilter::MoveModule { package, module } => {
                matches!(&object.type_, ObjectType::Struct(s) if &ObjectID::from(s.address()) == package
                        && s.module() == module.as_ident_str())
            }
            AiyObjectDataFilter::Package(p) => {
                matches!(&object.type_, ObjectType::Struct(s) if &ObjectID::from(s.address()) == p)
            }
            AiyObjectDataFilter::AddressOwner(a) => {
                matches!(object.owner, Owner::AddressOwner(addr) if &addr == a)
            }
            AiyObjectDataFilter::ObjectOwner(o) => {
                matches!(object.owner, Owner::ObjectOwner(addr) if addr == AiyAddress::from(*o))
            }
            AiyObjectDataFilter::ObjectId(id) => &object.object_id == id,
            AiyObjectDataFilter::ObjectIds(ids) => ids.contains(&object.object_id),
            AiyObjectDataFilter::Version(v) => object.version.value() == *v,
        }
    }
}
...
pub struct AiyObjectResponseQuery {
    /// If None, no filter will be applied
    pub filter: Option<AiyObjectDataFilter>,
    /// config which fields to include in the response, by default only digest is included
    pub options: Option<AiyObjectDataOptions>,
}

impl AiyObjectResponseQuery {
    pub fn new(filter: Option<AiyObjectDataFilter>, options: Option<AiyObjectDataOptions>) -> Self {
        Self { filter, options }
    }

    pub fn new_with_filter(filter: AiyObjectDataFilter) -> Self {
        Self {
            filter: Some(filter),
            options: None,
        }
    ...
    pub fn new_with_options(options: AiyObjectDataOptions) -> Self {
        Self {
            filter: None,
