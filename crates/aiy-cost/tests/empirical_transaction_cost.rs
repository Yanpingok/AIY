...
use aiy_test_transaction_builder::publish_basics_package_and_make_counter;
use aiy_test_transaction_builder::TestTransactionBuilder;
use aiy_types::base_types::{FullObjectRef, ObjectRef, AiyAddress};
use aiy_types::coin::PAY_JOIN_FUNC_NAME;
use aiy_types::coin::PAY_MODULE_NAME;
use aiy_types::coin::PAY_SPLIT_VEC_FUNC_NAME;
...
pub enum CommonTransactionCosts {
    ...
    SplitCoin(usize),
    TransferWholeCoin,
    TransferWholeAiyCoin,
    TransferPortionAiyCoin,
    SharedCounterCreate,
...
// 1. Run `cargo insta test --review` under `./aiy-cost`.
...
async fn create_txes(
    ...
    //
    // Transfer Whole Aiy Coin and Transfer Portion of Aiy Coin
    //
    let whole_aiy_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_aiy(None, AiyAddress::default())
            .build();
    let partial_aiy_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_aiy(Some(10), AiyAddress::default())
            .build();
    ret.insert(
        CommonTransactionCosts::TransferWholeAiyCoin,
        whole_aiy_coin_tx,
    );
    ret.insert(
        CommonTransactionCosts::TransferPortionAiyCoin,
        partial_aiy_coin_tx,
    );
