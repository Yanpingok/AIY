...
use aiy_types::error::ErrorCategory;
use tonic::Code;

use crate::proto::google::rpc::{BadRequest, ErrorInfo, RetryInfo};
pub use aiy_rpc::proto::aiy::rpc::v2beta2::ErrorReason;
...
impl From<aiy_types::storage::error::Error> for RpcError {
    fn from(value: aiy_types::storage::error::Error) -> Self {
        Self {
            code: Code::Internal,
...
impl From<aiy_types::aiy_sdk_types_conversions::SdkTypeConversionError> for RpcError {
    fn from(value: aiy_types::aiy_sdk_types_conversions::SdkTypeConversionError) -> Self {
        Self {
            code: Code::Internal,
            message: Some(value.to_string()),
            details: None,
...
impl From<aiy_types::quorum_driver_types::QuorumDriverError> for RpcError {
    fn from(error: aiy_types::quorum_driver_types::QuorumDriverError) -> Self {
        ...
        match error {
            InvalidUserSignature(err) => {
                let message = {
                    let err = match err {
                        AiyError::UserInputError { error } => error.to_string(),
                        _ => err.to_string(),
                    };
...
impl ObjectNotFoundError {
    pub fn new(object_id: aiy_sdk_types::Address) -> Self {
        Self {
            object_id,
            version: None,
        }
...
impl CheckpointNotFoundError {
    ...
    pub fn digest(digest: aiy_sdk_types::Digest) -> Self {
        Self {
