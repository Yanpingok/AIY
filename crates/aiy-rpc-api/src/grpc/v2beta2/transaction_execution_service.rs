...
use aiy_rpc::proto::aiy::rpc::v2beta2::transaction_execution_service_server::TransactionExecutionService;
use aiy_rpc::proto::aiy::rpc::v2beta2::transaction_finality::Finality;
use aiy_rpc::proto::aiy::rpc::v2beta2::ExecuteTransactionRequest;
use aiy_rpc::proto::aiy::rpc::v2beta2::ExecuteTransactionResponse;
use aiy_rpc::proto::aiy::rpc::v2beta2::ExecutedTransaction;
use aiy_rpc::proto::aiy::rpc::v2beta2::Object;
use aiy_rpc::proto::aiy::rpc::v2beta2::Transaction;
use aiy_rpc::proto::aiy::rpc::v2beta2::TransactionEffects;
use aiy_rpc::proto::aiy::rpc::v2beta2::TransactionEvents;
use aiy_rpc::proto::aiy::rpc::v2beta2::UserSignature;
use aiy_sdk_types::Address;
use aiy_types::balance_change::derive_balance_changes;
...
pub async fn execute_transaction(
    ...
    let transaction = request
        ...
        .pipe(aiy_sdk_types::Transaction::try_from)
        .map_err(|e| {
    ...
    let signatures = request
        ...
        .enumerate()
        .map(|(i, signature)| {
            aiy_sdk_types::UserSignature::try_from(signature).map_err(|e| {
    ...
    let signed_transaction = aiy_sdk_types::SignedTransaction {
    ...
    let aiy_types::quorum_driver_types::ExecuteTransactionResponseV3 {
        effects:
            aiy_types::quorum_driver_types::FinalizedEffects {
                effects,
                finality_info,
            },
        events,
    ...
    let finality = {
        let finality = match finality_info {
            aiy_types::quorum_driver_types::EffectsFinalityInfo::Certified(sig) => {
                Finality::Certified(aiy_sdk_types::ValidatorAggregatedSignature::from(sig).into())
            }
            aiy_types::quorum_driver_types::EffectsFinalityInfo::Checkpointed(
                _epoch,
                checkpoint,
            ) => Finality::Checkpointed(checkpoint),
            aiy_types::quorum_driver_types::EffectsFinalityInfo::QuorumExecuted(_) => {
                Finality::QuorumExecuted(())
            }
        };
        let mut message = aiy_rpc::proto::aiy::rpc::v2beta2::TransactionFinality::default();
        ...
        read_mask.subtree(ExecuteTransactionResponse::TRANSACTION_FIELD.name)
    {
        ...
        let input_objects = input_objects
            .into_iter()
            .map(aiy_sdk_types::Object::try_from)
            .collect::<Result<Vec<_>, _>>()?;
        ...
        let effects = aiy_sdk_types::TransactionEffects::try_from(effects)?;
        let effects = mask
                    ...
                    for changed_object in effects.changed_objects.iter_mut() {
                            ...
                            .find(|o| o.object_id() == object_id)
                        {
                            changed_object.object_type = Some(match object.object_type() {
                                aiy_sdk_types::ObjectType::Package => "package".to_owned(),
                                aiy_sdk_types::ObjectType::Struct(struct_tag) => {
