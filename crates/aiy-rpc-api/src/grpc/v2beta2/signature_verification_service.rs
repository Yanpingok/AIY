...
use aiy_rpc::proto::aiy::rpc::v2beta2::signature_verification_service_server::SignatureVerificationService;
use aiy_rpc::proto::aiy::rpc::v2beta2::VerifySignatureRequest;
use aiy_rpc::proto::aiy::rpc::v2beta2::VerifySignatureResponse;
...
fn verify_signature(
    ...
    let signature = request
        ...
        .pipe(aiy_sdk_types::UserSignature::try_from)
        .map_err(|e| {
    ...
    let signing_digest = {
        ...
        match bcs.name() {
            "TransactionData" => bcs
                .deserialize::<aiy_sdk_types::Transaction>()?
                .signing_digest(),
            "PersonalMessage" => bcs
                .deserialize::<&[u8]>()
                .map(|slice| aiy_sdk_types::PersonalMessage(slice.into()))?
                .signing_digest(),
            _ => {
                ...
                } else if let Ok(transaction) = bcs.deserialize::<aiy_sdk_types::Transaction>() {
    ...
    if let Some(address) = request
        .address
        .map(|address| address.parse::<aiy_sdk_types::Address>())
        .transpose()
        .map_err(|e| {
    ...
    {
        //TODO add function in aiy_sdk_types crate to do this
        let derived_addresses = match &signature {
            aiy_sdk_types::UserSignature::Simple(simple_signature) => match simple_signature {
                aiy_sdk_types::SimpleSignature::Ed25519 { public_key, .. } => {
                    [Some(public_key.derive_address()), None]
                }
                aiy_sdk_types::SimpleSignature::Secp256k1 { public_key, .. } => {
                    [Some(public_key.derive_address()), None]
                }
                aiy_sdk_types::SimpleSignature::Secp256r1 { public_key, .. } => {
                    [Some(public_key.derive_address()), None]
                }
                _ => {
            ...
            aiy_sdk_types::UserSignature::Multisig(multisig) => {
                [Some(multisig.committee().derive_address()), None]
            }
            aiy_sdk_types::UserSignature::ZkLogin(z) => {
            ...
            aiy_sdk_types::UserSignature::Passkey(p) => {
    ...
    let jwks = {
        let mut jwks = request
            ...
            .enumerate()
            .map(|(i, jwk)| {
                let jwk = aiy_sdk_types::ActiveJwk::try_from(jwk).map_err(|e| {
            ...
            if let Some(authenticator_state) = service.reader.get_authenticator_state()? {
                jwks.extend(
                    authenticator_state
                        .active_jwks
                        .into_iter()
                        .map(aiy_sdk_types::ActiveJwk::from)
    ...
    let mut zklogin_verifier = match service.chain_id().chain() {
        aiy_protocol_config::Chain::Mainnet => aiy_crypto::zklogin::ZkloginVerifier::new_mainnet(),
        aiy_protocol_config::Chain::Testnet | aiy_protocol_config::Chain::Unknown => {
            aiy_crypto::zklogin::ZkloginVerifier::new_dev()
        }
    };
    *zklogin_verifier.jwks_mut() = jwks;
    let mut verifier = aiy_crypto::UserSignatureVerifier::new();
