...
// This module contains conversion functions from aiy-package-resolver types to their protobuf representations.
...
use aiy_package_resolver::{DataDef, FunctionDef, MoveData, VariantDef};
...
pub(crate) fn convert_error(e: aiy_package_resolver::error::Error) -> RpcError {
...
pub(crate) fn convert_module(
    module_name: &str,
    resolved_module: &aiy_package_resolver::Module,
    package_id: &ObjectID,
) -> Result<Module> {
...
fn convert_open_signature(sig: &aiy_package_resolver::OpenSignature) -> OpenSignature {
    let reference = sig.ref_.map(|r| match r {
        aiy_package_resolver::Reference::Immutable => Reference::Immutable as i32,
        aiy_package_resolver::Reference::Mutable => Reference::Mutable as i32,
    });
...
fn convert_open_signature_body(
    body: &aiy_package_resolver::OpenSignatureBody,
) -> OpenSignatureBody {
    let (type_enum, type_name, type_params, type_param_idx) = match body {
        aiy_package_resolver::OpenSignatureBody::Bool => (SignatureType::Bool, None, vec![], None),
        aiy_package_resolver::OpenSignatureBody::U8 => (SignatureType::U8, None, vec![], None),
        aiy_package_resolver::OpenSignatureBody::U16 => (SignatureType::U16, None, vec![], None),
        aiy_package_resolver::OpenSignatureBody::U32 => (SignatureType::U32, None, vec![], None),
        aiy_package_resolver::OpenSignatureBody::U64 => (SignatureType::U64, None, vec![], None),
        aiy_package_resolver::OpenSignatureBody::U128 => (SignatureType::U128, None, vec![], None),
        aiy_package_resolver::OpenSignatureBody::U256 => (SignatureType::U256, None, vec![], None),
        aiy_package_resolver::OpenSignatureBody::Address => {
            (SignatureType::Address, None, vec![], None)
        }
        aiy_package_resolver::OpenSignatureBody::Vector(inner) => {
            let inner_body = convert_open_signature_body(inner.as_ref());
            (SignatureType::Vector, None, vec![inner_body], None)
        }
        aiy_package_resolver::OpenSignatureBody::Datatype(key, args) => {
        ...
        aiy_package_resolver::OpenSignatureBody::TypeParameter(idx) => {
