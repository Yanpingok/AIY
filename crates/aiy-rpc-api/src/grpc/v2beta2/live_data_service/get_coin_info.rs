...
use aiy_rpc::proto::aiy::rpc::v2beta2::coin_metadata::MetadataCapState as ProtoMetadataCapState;
use aiy_rpc::proto::aiy::rpc::v2beta2::coin_treasury::SupplyState as RpcSupplyState;
use aiy_rpc::proto::aiy::rpc::v2beta2::regulated_coin_metadata::CoinRegulatedState as ProtoCoinRegulatedState;
use aiy_rpc::proto::aiy::rpc::v2beta2::CoinMetadata;
use aiy_rpc::proto::aiy::rpc::v2beta2::CoinTreasury;
use aiy_rpc::proto::aiy::rpc::v2beta2::GetCoinInfoRequest;
use aiy_rpc::proto::aiy::rpc::v2beta2::GetCoinInfoResponse;
use aiy_rpc::proto::aiy::rpc::v2beta2::RegulatedCoinMetadata;
use aiy_sdk_types::{Address, StructTag};
use aiy_types::base_types::{ObjectID as AiyObjectID, AiyAddress};
use aiy_types::coin_registry::{
    Currency, MetadataCapState, RegulatedState as CurrencyRegulatedState, SupplyState,
};
use aiy_types::aiy_sdk_types_conversions::struct_tag_sdk_to_core;

const AIY_COIN_TREASURY: CoinTreasury = {
    let mut treasury = CoinTreasury::const_default();
    treasury.total_supply = Some(aiy_types::gas_coin::TOTAL_SUPPLY_MIST);
...
fn get_coin_info_from_registry(
    ...
    let treasury = if aiy_types::gas_coin::GAS::is_gas(core_coin_type) {
        Some(AIY_COIN_TREASURY)
    } else {
...
fn get_coin_info_from_index(
    service: &RpcService,
    indexes: &dyn aiy_types::storage::RpcIndexes,
    coin_type: &StructTag,
    core_coin_type: &move_core_types::language_storage::StructTag,
) -> Result<Option<GetCoinInfoResponse>> {
    ...
    let metadata = if let Some(coin_metadata_object_id) = coin_info.coin_metadata_object_id {
        service
            ...
            .map(aiy_types::coin::CoinMetadata::try_from)
            .transpose()
            .map_err(|_| {
                RpcError::new(
                    tonic::Code::Internal,
    ...
    } else if aiy_types::gas_coin::GAS::is_gas(core_coin_type) {
        Some(AIY_COIN_TREASURY)
    } else {
        None
    };
        ...
        coin_info.regulated_coin_metadata_object_id
    {
        service
            ...
            .map(aiy_types::coin::RegulatedCoinMetadata::try_from)
            .transpose()
            .map_err(|_| {
                RpcError::new(
                    tonic::Code::Internal,
...
fn get_treasury_cap_info(
    service: &RpcService,
    treasury_object_id: AiyObjectID,
) -> Option<CoinTreasury> {
    let obj = service.reader.inner().get_object(&treasury_object_id)?;

    // Treasury caps owned by 0x0 indicate fixed supply
    let supply_state = if obj.owner == aiy_types::object::Owner::AddressOwner(AiyAddress::ZERO) {
        RpcSupplyState::Fixed
    } else {
        RpcSupplyState::Unknown
    };

    aiy_types::coin::TreasuryCap::try_from(obj)
        .ok()
        .map(|treasury| {
