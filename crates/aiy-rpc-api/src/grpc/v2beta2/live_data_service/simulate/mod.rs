...
use aiy_rpc::proto::aiy::rpc::v2beta2::Bcs;
use aiy_rpc::proto::aiy::rpc::v2beta2::CommandOutput;
use aiy_rpc::proto::aiy::rpc::v2beta2::CommandResult;
...
use aiy_rpc::proto::aiy::rpc::v2beta2::SimulateTransactionResponse;
...
pub fn simulate_transaction(
    ...
    let mut transaction = match aiy_sdk_types::Transaction::try_from(transaction_proto) {
        Ok(transaction) => aiy_types::transaction::TransactionData::try_from(transaction)?,
        ...
        if transaction.gas_data().payment.is_empty() {
            let input_objects = transaction
                ...
                .flat_map(|obj| match obj {
                    aiy_types::transaction::InputObjectKind::ImmOrOwnedMoveObject((id, _, _)) => {
                        Some(*id)
                    }
                    _ => None,
                })
    ...
    let transaction = if let Some(submask) = read_mask.subtree("transaction") {
        ...
        let transaction = aiy_sdk_types::Transaction::try_from(transaction)?;
        ...
        message.events = submask
            ...
            .and_then(|mask| {
                events.map(|events| {
                    aiy_sdk_types::TransactionEvents::try_from(events)
...
fn to_command_output(
    service: &RpcService,
    arg: Option<aiy_types::transaction::Argument>,
    bcs: Vec<u8>,
    ty: aiy_types::TypeTag,
) -> CommandOutput {
    let json = service
        .reader
        .inner()
        ...
        .flatten()
        .and_then(|layout| {
            aiy_types::proto_value::ProtoVisitorBuilder::new(
...
fn estimate_gas_budget_from_gas_cost(
    gas_cost_summary: &aiy_types::gas::GasCostSummary,
