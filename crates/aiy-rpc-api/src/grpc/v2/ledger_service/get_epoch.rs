...
use aiy_protocol_config::ProtocolConfigValue;
...
fn get_protocol_config(
    version: u64,
    chain: aiy_protocol_config::Chain,
) -> Result<ProtocolConfig, ProtocolVersionNotFoundError> {
    let config =
        aiy_protocol_config::ProtocolConfig::get_for_version_if_supported(version.into(), chain)
            .ok_or_else(|| ProtocolVersionNotFoundError::new(version))?;
    Ok(protocol_config_to_proto(config))
}

pub fn protocol_config_to_proto(config: aiy_protocol_config::ProtocolConfig) -> ProtocolConfig {
