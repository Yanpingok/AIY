...
use aiy_rpc::proto::aiy::rpc::v2::BatchGetTransactionsRequest;
use aiy_rpc::proto::aiy::rpc::v2::BatchGetTransactionsResponse;
use aiy_rpc::proto::aiy::rpc::v2::Event;
use aiy_rpc::proto::aiy::rpc::v2::ExecutedTransaction;
use aiy_rpc::proto::aiy::rpc::v2::GetTransactionRequest;
use aiy_rpc::proto::aiy::rpc::v2::GetTransactionResponse;
use aiy_rpc::proto::aiy::rpc::v2::GetTransactionResult;
...
use aiy_rpc::proto::timestamp_ms_to_proto;
use aiy_sdk_types::{Address, Digest};
                ...
                for changed_object in effects.changed_objects.iter_mut() {
                    ...
                    if let Some(ty) = object_types.get(&object_id.into()) {
                        changed_object.object_type = Some(match ty {
                            aiy_types::base_types::ObjectType::Package => "package".to_owned(),
                            aiy_types::base_types::ObjectType::Struct(struct_tag) => {
...
pub(crate) fn render_clever_error(service: &RpcService, effects: &mut TransactionEffects) {
    use aiy_rpc::proto::aiy::rpc::v2::clever_error;
    use aiy_rpc::proto::aiy::rpc::v2::execution_error::ErrorDetails;
    use aiy_rpc::proto::aiy::rpc::v2::CleverError;
    use aiy_rpc::proto::aiy::rpc::v2::MoveAbort;
    ...
    fn render(service: &RpcService, move_abort: &MoveAbort) -> Option<CleverError> {
        ...
        let package_id = location.package().parse::<aiy_sdk_types::Address>().ok()?;
        let module = location.module();

        let package = {
            let object = service.reader.inner().get_object(&package_id.into())?;
            aiy_package_resolver::Package::read_from_object(&object).ok()?
        };
        ...
        match clever_error.error_info {
            aiy_package_resolver::ErrorConstants::None => {}
            aiy_package_resolver::ErrorConstants::Rendered {
                identifier,
                constant,
            } => {
            ...
            aiy_package_resolver::ErrorConstants::Raw { identifier, bytes } => {
                clever_error_message.constant_name = Some(identifier);
