...
use aiy_sdk_types::{Address, Object, Version};
use aiy_sdk_types::{CheckpointSequenceNumber, EpochId, SignedTransaction, ValidatorCommittee};
use aiy_types::balance_change::BalanceChange;
use aiy_types::base_types::{ObjectID, ObjectType};
use aiy_types::storage::error::{Error as StorageError, Result};
use aiy_types::storage::ObjectKey;
use aiy_types::storage::RpcStateReader;
use aiy_types::storage::{ObjectStore, TransactionInfo};
use tap::Pipe;
...
impl StateReader {
    ...
    #[tracing::instrument(skip(self))]
    pub fn get_system_state(&self) -> Result<aiy_types::aiy_system_state::AiySystemState> {
        aiy_types::aiy_system_state::get_aiy_system_state(self.inner())
    ...
    pub fn get_system_state_summary(
        &self,
    ) -> Result<aiy_types::aiy_system_state::aiy_system_state_summary::AiySystemStateSummary> {
        use aiy_types::aiy_system_state::AiySystemStateTrait;

        let system_state = self.get_system_state()?;
        let summary = system_state.into_aiy_system_state_summary();
    ...
    pub fn get_authenticator_state(
        &self,
    ) -> Result<Option<aiy_types::authenticator_state::AuthenticatorStateInner>> {
        aiy_types::authenticator_state::get_authenticator_state(self.inner())
            .map_err(StorageError::custom)
    }
    ...
    pub fn get_transaction(
        &self,
        digest: aiy_sdk_types::Digest,
    ) -> crate::Result<(
        aiy_sdk_types::SignedTransaction,
        aiy_sdk_types::TransactionEffects,
        Option<aiy_sdk_types::TransactionEvents>,
    )> {
    ...
    pub fn get_transaction_info(
        &self,
        digest: &aiy_types::digests::TransactionDigest,
...
pub struct TransactionRead {
    pub digest: aiy_sdk_types::Digest,
    pub transaction: aiy_sdk_types::Transaction,
    pub signatures: Vec<aiy_sdk_types::UserSignature>,
    pub effects: aiy_sdk_types::TransactionEffects,
    pub events: Option<aiy_sdk_types::TransactionEvents>,
...
pub struct CheckpointTransactionsIter {
    ...
    checkpoint: Option<(
        aiy_types::messages_checkpoint::CheckpointSummary,
        aiy_types::messages_checkpoint::CheckpointContents,
    )>,
}
...
impl Iterator for CheckpointTransactionsIter {
    type Item = Result<(CursorInfo, aiy_types::digests::TransactionDigest)>;
...
#[derive(Debug)]
pub struct TransactionNotFoundError(pub aiy_sdk_types::Digest);
