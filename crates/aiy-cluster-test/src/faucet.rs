...
use std::env;
use std::sync::Arc;
use aiy_faucet::{FaucetConfig, FaucetResponse, LocalFaucet, RequestStatus};
...
pub trait FaucetClient {
    async fn request_aiy_coins(&self, request_address: AiyAddress) -> FaucetResponse;
}
...
impl FaucetClient for RemoteFaucetClient {
    /// Request test AIY coins from faucet.
