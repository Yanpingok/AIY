...
use aiy_json_rpc_types::{Balance, AiyTransactionBlockResponseOptions};
use aiy_move_build::test_utils::compile_managed_coin_package;
...
impl TestCaseImpl for CoinIndexTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        ctx.get_aiy_from_faucet(None).await;
        ...
        assert_eq!(
            ...
            "total_balance: {}, old_total_balance: {}, aiy_balance_change.amount: {}",
            total_balance,
            old_total_balance,
        ...
        let args = vec![
            AiyJsonValue::from_object_id(cap.0),
            AiyJsonValue::new(json!("10000"))?,
            AiyJsonValue::new(json!(account))?,
        ];
        let txn = client
            .transaction_builder()
            .move_call(
        ...
        let aiy_balance_change = balance_changes
            .iter()
            .find(|b| b.coin_type.to_string().contains("AIY"))
            .unwrap();
        ...
        assert_eq!(aiy_balance_change.owner, Owner::AddressOwner(account));
        ...
        assert_eq!(
            total_balance,
            (old_total_balance as i128 + aiy_balance_change.amount) as u128,
            "total_balance: {}, old_total_balance: {}, aiy_balance_change.amount: {}",
            total_balance,
            old_total_balance,
            aiy_balance_change.amount
        );
        old_coin_object_count = coin_object_count;
        ...
        let args = vec![AiyJsonValue::from_object_id(envelope.0)];
        let txn = client
            .transaction_builder()
            .move_call(
                account,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    AiyJsonValue::from_object_id(cap.0),
                    AiyJsonValue::from_object_id(envelope.0),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    AiyJsonValue::from_object_id(cap.0),
                    AiyJsonValue::from_object_id(managed_coin_id_10k),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let aiy_coins = client
            .coin_read_api()
            .get_coins(account, Some(aiy_type_str.into()), None, None)
            .await
            .unwrap()
            .data;
        ...
        assert_eq!(
            // this is only AIY coins at the moment
            aiy_coins,
            client
                .coin_read_api()
        ...
        let aiy_balance = client
            .coin_read_api()
            .get_balance(account, None)
            .await
            .unwrap();
        assert_eq!(
            aiy_balance.total_balance,
            aiy_coins.iter().map(|c| c.balance as u128).sum::<u128>()
        );
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    AiyJsonValue::from_object_id(cap.0),
                    AiyJsonValue::new(json!("5"))?,  // balance = 5
                    AiyJsonValue::new(json!("40"))?, // num = 40
                    AiyJsonValue::new(json!(account))?,
                ],
                None,
                rgp * 2_000_000,
        ...
        assert_eq!(
            ...
            client
                .coin_read_api()
                .get_coins(account, None, None, Some(aiy_coins.len() + 1))
                .await
                .unwrap()
                .data,
        );
        ...
        assert_eq!(aiy_coins.len() + managed_coins.len(), total_coins,);

        let aiy_coins_with_managed_coin_1 = client
            .coin_read_api()
            .get_all_coins(account, None, Some(aiy_coins.len() + 1))
            .await
            .unwrap();
        assert_eq!(
            aiy_coins_with_managed_coin_1.data.len(),
            aiy_coins.len() + 1
        );
        assert!(aiy_coins_with_managed_coin_1.has_next_page);
        let cursor = aiy_coins_with_managed_coin_1.next_cursor;
...
async fn add_to_envelope(
    ...
    let txn = client
        ...
        .move_call(
            ...
            vec![],
            vec![
                AiyJsonValue::from_object_id(envelope),
                AiyJsonValue::from_object_id(coin),
            ],
            None,
            rgp * 2_000_000,
            None,
