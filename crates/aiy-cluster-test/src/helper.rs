...
use aiy_json_rpc_types::{BalanceChange, AiyData, AiyObjectData, AiyObjectDataOptions};
use aiy_sdk::AiyClient;
use aiy_types::error::AiyObjectResponseError;
use aiy_types::gas_coin::GasCoin;
use aiy_types::{base_types::ObjectID, object::Owner, parse_aiy_type_tag};
use tracing::{debug, trace};

/// A util struct that helps verify Aiy Object.
...
/// `check_into_gas_coin` expect to get a `AiyObjectData` and `GasCoin`
/// respectfully.
#[derive(Debug)]
...
impl ObjectChecker {
    ...
    pub fn is_aiy_coin(mut self, is_aiy_coin: bool) -> Self {
        self.is_aiy_coin = Some(is_aiy_coin);
        self
    }

    pub async fn check_into_gas_coin(self, client: &AiyClient) -> GasCoin {
        if self.is_aiy_coin == Some(false) {
            panic!("'check_into_gas_coin' shouldn't be called with 'is_aiy_coin' set as false");
        }
        self.is_aiy_coin(true)
            .check(client)
            .await
            .unwrap()
            .into_gas_coin()
    ...
    pub async fn check_into_object(self, client: &AiyClient) -> AiyObjectData {
        self.check(client).await.unwrap().into_object()
    }

    pub async fn check(self, client: &AiyClient) -> Result<CheckerResultObject, anyhow::Error> {
        debug!(?self);
        ...
        match (object_info.data, object_info.error) {
            (None, Some(AiyObjectResponseError::NotExists { object_id })) => {
                panic!(
            ...
            (
                None,
                Some(AiyObjectResponseError::DynamicFieldNotFound {
            ...
            (None, Some(AiyObjectResponseError::DisplayError { error })) => {
                panic!("Display Error: {error:?}");
            }
            (None, None) | (None, Some(AiyObjectResponseError::Unknown)) => {
...
pub struct CheckerResultObject {
    ...
    object: Option<AiyObjectData>,
}

impl CheckerResultObject {
    pub fn new(gas_coin: Option<GasCoin>, object: Option<AiyObjectData>) -> Self {
        Self { gas_coin, object }
    }
    pub fn into_gas_coin(self) -> GasCoin {
        self.gas_coin.unwrap()
    }
    pub fn into_object(self) -> AiyObjectData {
        self.object.unwrap()
    }
}
...
impl BalanceChangeChecker {
    ...
    pub fn coin_type(mut self, coin_type: &str) -> Self {
        self.coin_type = Some(parse_aiy_type_tag(coin_type).unwrap());
        self
    }
