...
use aiy_config::{PersistedConfig, AIY_KEYSTORE_FILENAME, AIY_NETWORK_CONFIG};
use aiy_graphql_rpc::config::{ConnectionConfig, ServiceConfig};
use aiy_graphql_rpc::test_infra::cluster::start_graphql_server_with_fn_rpc;
...
use aiy_types::crypto::{get_key_pair, AccountKeyPair};
...
impl Cluster for LocalNewCluster {
    async fn start(options: &ClusterTestOpt) -> Result<Self, anyhow::Error> {
        ...
        if let Some(config_dir) = options.config_dir.clone() {
            ...
            let network_config_path = config_dir.join(AIY_NETWORK_CONFIG);
        ...
        let faucet_address = AiyAddress::from(faucet_key.public());
...
pub async fn new_wallet_context_from_cluster(
    ...
    let address: AiyAddress = key_pair.public().into();
    keystore
        .import(None, AiyKeyPair::Ed25519(key_pair))
        .await
        .unwrap();
    AiyClientConfig {
        keystore,
