...
public fun create_party(ctx: &mut TxContext) {
    let p = Pub { id: object::new(ctx) };
    transfer::public_party_transfer(p, aiy::party::single_owner(@0))
}

public fun pub_party(obj: Pub, p: aiy::party::Party) {
    transfer::public_party_transfer(obj, p)
}

public fun priv_party(obj: Priv, p: aiy::party::Party) {
...
//> 0: aiy::party::single_owner(Input(1));
//> aiy::transfer::public_party_transfer<ex::m::Pub>(Input(0), Result(0))
