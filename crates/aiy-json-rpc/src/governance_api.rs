...
use aiy_json_rpc_api::{GovernanceReadApiOpenRpc, GovernanceReadApiServer, JsonRpcMetrics};
use aiy_json_rpc_types::{DelegatedStake, Stake, StakeStatus};
use aiy_json_rpc_types::{AiyCommittee, ValidatorApy, ValidatorApys};
...
impl GovernanceReadApi {
    ...
    async fn get_staked_aiy(&self, owner: AiyAddress) -> Result<Vec<StakedAiy>, Error> {
        let state = self.state.clone();
        let result =
            spawn_monitored_task!(async move { state.get_staked_aiy(owner).await }).await??;

        self.metrics
            .get_stake_aiy_result_size
            .observe(result.len() as f64);
        self.metrics
            .get_stake_aiy_result_size_total
    ...
    async fn get_stakes_by_ids(
        ...
        let mut stakes: Vec<(StakedAiy, bool)> = vec![];
        for stake in stakes_read.into_iter() {
            match stake {
                ObjectRead::Exists(_, o, _) => stakes.push((StakedAiy::try_from(&o)?, true)),
                ObjectRead::Deleted(oref) => {
                    match self
                        ...
                        .await?
                    {
                        Some(o) => stakes.push((StakedAiy::try_from(&o)?, false)),
                        None => Err(AiyRpcInputError::UserInputError(
                ...
                ObjectRead::NotExists(id) => Err(AiyRpcInputError::UserInputError(
                    UserInputError::ObjectNotFound {
                        object_id: id,
                        version: None,
                    },
    ...
    async fn get_stakes(&self, owner: AiyAddress) -> Result<Vec<DelegatedStake>, Error> {
        let timer = self.metrics.get_stake_aiy_latency.start_timer();
        let stakes = self.get_staked_aiy(owner).await?;
        if stakes.is_empty() {
            return Ok(vec![]);
        }
        drop(timer);

        let _timer = self.metrics.get_delegated_aiy_latency.start_timer();
    ...
    async fn get_delegated_stakes(
        ...
        let system_state_summary: AiySystemStateSummary =
            system_state.clone().into_aiy_system_state_summary();
    ...
    fn get_system_state(&self) -> Result<AiySystemState, Error> {
...
impl GovernanceReadApiServer for GovernanceReadApi {
    ...
    async fn get_stakes_by_ids(
        ...
        with_tracing!(async move { self.get_stakes_by_ids(staked_aiy_ids).await })
    }
    ...
    #[instrument(skip(self))]
    async fn get_committee_info(&self, epoch: Option<BigInt<u64>>) -> RpcResult<AiyCommittee> {
    ...
    async fn get_validators_apy(&self) -> RpcResult<ValidatorApys> {
        ...
        let system_state_summary: AiySystemStateSummary =
            self.get_latest_aiy_system_state().await?;
...
async fn exchange_rates(
    ...
    let system_state_summary: AiySystemStateSummary = system_state.into_aiy_system_state_summary();
    ...
    for df in state.get_dynamic_fields(
        ...
        let pool_id: ID =
            bcs::from_bytes(&df.1.bcs_name).map_err(|e| AiyError::ObjectDeserializationError {
                error: e.to_string(),
            })?;
    ...
    for (address, pool_id, exchange_rates_id, exchange_rates_size, active) in tables {
        let mut rates = state
            ...
            .map(|df| {
                ...
                Ok::<_, AiyError>((epoch, exchange_rate))
            })
            .collect::<Result<Vec<_>, _>>()?;
...
impl AiyRpcModule for GovernanceReadApi {
