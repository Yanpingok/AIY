...
use aiy_json_rpc_api::{MoveUtilsOpenRpc, MoveUtilsServer};
use aiy_json_rpc_types::{
    MoveFunctionArgType, ObjectValueKind, AiyMoveNormalizedFunction, AiyMoveNormalizedModule,
    AiyMoveNormalizedStruct,
};
use aiy_open_rpc::Module;
use aiy_types::base_types::ObjectID;
use aiy_types::move_package::normalize_modules;
use aiy_types::object::{Data, ObjectRead};
use tap::TapFallible;
...
impl MoveUtilsInternalTrait for MoveUtilsInternal {
    ...
    async fn get_move_module(
        ...
        Ok(match normalized.remove(&module_name) {
            ...
            None => Err(AiyRpcInputError::GenericNotFound(format!(
        ...
        match object_read {
            ObjectRead::Exists(_obj_ref, object, _layout) => {
                match object.into_inner().data {
                    ...
                    _ => Err(AiyRpcInputError::GenericInvalid(format!(
                        "Object is not a package with ID {}",
                        package
                    )))?,
                }
            }
            _ => Err(AiyRpcInputError::GenericNotFound(format!(
...
impl AiyRpcModule for MoveUtils {
    ...
    async fn get_normalized_move_struct(
        ...
        with_tracing!(async move {
            ...
            let identifier = Identifier::new(struct_name.as_str())
                .map_err(|e| AiyRpcInputError::GenericInvalid(format!("{e}")))?;
...
mod tests {
    ...
    mod get_normalized_move_module_tests {
        ...
        async fn test_success_response() {
            ...
            let expected_module: AiyMoveNormalizedModule = normalized_module.into();
        ...
        async fn test_no_module_found() {
            ...
            let expected_error =
                Error::AiyRpcInputError(AiyRpcInputError::GenericNotFound(error_string.clone()));
            mock_internal
                .expect_get_move_module()
