...
use aiy_json_rpc_api::{TRANSACTION_EXECUTION_CLIENT_ERROR_CODE, TRANSIENT_ERROR_CODE};
use aiy_name_service::NameServiceError;
use aiy_types::committee::{QUORUM_THRESHOLD, TOTAL_VOTING_POWER};
use aiy_types::error::{ErrorCategory, AiyError, AiyObjectResponseError, UserInputError};
use aiy_types::quorum_driver_types::QuorumDriverError;
use thiserror::Error;
...
pub enum Error {
    ...
    #[error(transparent)]
    AiyObjectResponseError(#[from] AiyObjectResponseError),

    #[error(transparent)]
    AiyRpcInputError(#[from] AiyRpcInputError),
...
impl From<AiyError> for Error {
    fn from(e: AiyError) -> Self {
        match e {
            AiyError::UserInputError { error } => Self::UserInputError(error),
            AiyError::AiyObjectResponseError { error } => Self::AiyObjectResponseError(error),
            AiyError::UnsupportedFeatureError { error } => Self::UnsupportedFeature(error),
            AiyError::IndexStoreNotAvailable => Self::UnsupportedFeature(
                "Required indexes are not available on this node".to_string(),
            ),
            other => Self::AiyError(other),
        }
    }
}
...
impl From<Error> for ErrorObjectOwned {
    ...
    fn from(e: Error) -> ErrorObjectOwned {
        match e {
            ...
            Error::AiyObjectResponseError(err) => match err {
                AiyObjectResponseError::NotExists { .. }
                | AiyObjectResponseError::DynamicFieldNotFound { .. }
                | AiyObjectResponseError::Deleted { .. }
                | AiyObjectResponseError::DisplayError { .. } => invalid_params(err),
                _ => failed(err),
            },
            ...
            Error::AiyRpcInputError(err) => invalid_params(err),
            Error::AiyError(aiy_error) => match aiy_error {
                AiyError::TransactionNotFound { .. }
                | AiyError::TransactionsNotFound { .. }
                | AiyError::TransactionEventsNotFound { .. } => invalid_params(aiy_error),
                _ => failed(aiy_error),
            },
                        ...
                        let new_errors: Vec<String> = errors
                            ...
                            .filter_map(|(err, _, _)| {
                                match &err {
                                    ...
                                    AiyError::UserInputError { error } => Some(error.to_string()),
                                    _ => {
...
pub enum AiyRpcInputError {
    ...
    #[error("{0}")]
    CannotParseAiyStructTag(String),
...
impl From<AiyRpcInputError> for RpcError {
    fn from(e: AiyRpcInputError) -> Self {
        RpcError::Call(invalid_params(e))
    }
}

impl From<AiyRpcInputError> for ErrorObjectOwned {
    fn from(e: AiyRpcInputError) -> Self {
        invalid_params(e)
    }
}
...
mod tests {
    ...
    use aiy_types::base_types::AuthorityName;
    ...
    use aiy_types::crypto::AuthorityPublicKey;
    ...
    mod match_quorum_driver_error_tests {
        ...
        fn test_invalid_user_signature() {
            let quorum_driver_error =
                QuorumDriverError::InvalidUserSignature(AiyError::InvalidSignature {
        ...
        fn test_objects_double_used() {
            use aiy_types::crypto::VerifyingKey;
        ...
        fn test_non_recoverable_transaction_error_with_transient_errors() {
            let quorum_driver_error = QuorumDriverError::NonRecoverableTransactionError {
                errors: vec![
                    ...
                    (
                        AiyError::RpcError("Hello".to_string(), "Testing".to_string()),
                        0,
                        vec![],
                    ),
                ],
        ...
        fn test_quorum_driver_internal_error() {
            let quorum_driver_error = QuorumDriverError::QuorumDriverInternalError(
                AiyError::UnexpectedMessage("test".to_string()),
            );
        ...
        fn test_system_overload() {
            let quorum_driver_error = QuorumDriverError::SystemOverload {
                overloaded_stake: 10,
                errors: vec![(AiyError::UnexpectedMessage("test".to_string()), 0, vec![])],
            };
