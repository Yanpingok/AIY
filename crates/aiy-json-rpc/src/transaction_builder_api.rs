...
use aiy_json_rpc_api::{TransactionBuilderOpenRpc, TransactionBuilderServer};
use aiy_json_rpc_types::{RPCTransactionRequestParams, AiyObjectDataFilter};
use aiy_json_rpc_types::{
    AiyObjectDataOptions, AiyObjectResponse, AiyTransactionBlockBuilderMode, AiyTypeTag,
    TransactionBlockBytes,
};
use aiy_open_rpc::Module;
use aiy_transaction_builder::{DataReader, TransactionBuilder};
use aiy_types::base_types::ObjectInfo;
use aiy_types::base_types::{ObjectID, AiyAddress};
use aiy_types::aiy_serde::BigInt;

use crate::authority_state::StateRead;
use crate::AiyRpcModule;
...
impl TransactionBuilderServer for TransactionBuilderApi {
    ...
    async fn pay_all_aiy(
        ...
        let data = self
            .0
            .pay_all_aiy(signer, input_coins, recipient, *gas_budget)
            .await
            .map_err(crate::Error::from)?;
        Ok(TransactionBlockBytes::from_data(data).map_err(crate::Error::from)?)
    }
    ...
    async fn move_call(
        ...
        rpc_arguments: Vec<AiyJsonValue>,
        gas: Option<ObjectID>,
        gas_budget: BigInt<u64>,
        _txn_builder_mode: Option<AiyTransactionBlockBuilderMode>,
    ) -> RpcResult<TransactionBlockBytes> {
    ...
    async fn request_withdraw_stake(
        ...
        Ok(TransactionBlockBytes::from_data(
            self.0
                .request_withdraw_stake(signer, staked_aiy, gas, *gas_budget)
                .await
                .map_err(crate::Error::from)?,
        )
        .map_err(crate::Error::from)?)
...
impl AiyRpcModule for TransactionBuilderApi {
