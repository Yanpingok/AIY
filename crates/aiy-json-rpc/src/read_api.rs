...
use aiy_types::storage::ObjectKey;
use tap::TapFallible;
...
use aiy_json_rpc_types::{
    ...
    ObjectChange, ProtocolConfigResponse, AiyEvent, AiyGetPastObjectRequest, AiyObjectDataOptions,
    AiyObjectResponse, AiyPastObjectResponse, AiyTransactionBlock, AiyTransactionBlockEvents,
...
use aiy_types::base_types::{ObjectID, SequenceNumber, TransactionDigest};
use aiy_types::crypto::AggregateAuthoritySignature;
use aiy_types::display::DisplayVersionUpdatedEvent;
use aiy_types::effects::{TransactionEffects, TransactionEffectsAPI, TransactionEvents};
use aiy_types::error::{AiyError, AiyObjectResponseError};
use aiy_types::messages_checkpoint::{
    CheckpointContents, CheckpointSequenceNumber, CheckpointSummary, CheckpointTimestamp,
};
use aiy_types::object::{Object, ObjectRead, PastObjectRead};
use aiy_types::aiy_serde::BigInt;
use aiy_types::transaction::TransactionDataAPI;
use aiy_types::transaction::{Transaction, TransactionData};

use crate::authority_state::{StateRead, StateReadError, StateReadResult};
use crate::error::{Error, RpcInterimResult, AiyRpcInputError};
use crate::{
    get_balance_changes_from_effect, get_object_changes, ObjectProviderCache, AiyRpcModule,
};
...
struct IntermediateTransactionResponse {
    ...
    events: Option<AiyTransactionBlockEvents>,
...
impl ReadApi {
    ...
    async fn multi_get_transaction_blocks_internal(
        ...
        opts: Option<AiyTransactionBlockResponseOptions>,
    ) -> Result<Vec<AiyTransactionBlockResponse>, Error> {
        trace!("start");

        let num_digests = digests.len();
        if num_digests > *QUERY_MAX_RESULT_LIMIT {
            Err(AiyRpcInputError::SizeLimitExceeded(
        ...
        if temp_response.len() < num_digests {
            Err(AiyRpcInputError::ContainsDuplicates)?
        }
            ...
            for (_, cache_entry) in temp_response.iter_mut() {
                    ...
                    cache_entry.effects.as_ref().and_then(|e| e.events_digest())
                {
                    match events.next() {
                        Some(Some(ev)) => {
                            cache_entry.events =
                                Some(to_aiy_transaction_events(self, cache_entry.digest, ev)?)
                        }
                        None | Some(None) => {
                    ...
                    cache_entry.events = Some(AiyTransactionBlockEvents::default());
                }
            }
        }
        ...
        if opts.show_balance_changes || opts.show_object_changes {
            ...
            for resp in temp_response.values() {
                let effects = resp.effects.as_ref().ok_or_else(|| {
                    AiyRpcInputError::GenericNotFound(
...
impl ReadApiServer for ReadApi {
    ...
    async fn get_object(
        &self,
        object_id: ObjectID,
        options: Option<AiyObjectDataOptions>,
    ) -> RpcResult<AiyObjectResponse> {
                ...
                ObjectRead::Exists(object_ref, o, layout) => {
                    ...
                    if options.show_display {
                            ...
                            .await
                        {
                            ...
                            Err(e) => {
                                return Ok(AiyObjectResponse::new(
                                    Some((object_ref, o, layout, options, None).try_into()?),
                                    Some(AiyObjectResponseError::DisplayError {
                                        error: e.to_string(),
                                    }),
                                ));
                            }
                    ...
                    Ok(AiyObjectResponse::new_with_data(
    ...
    async fn multi_get_objects(
        ...
        options: Option<AiyObjectDataOptions>,
    ) -> RpcResult<Vec<AiyObjectResponse>> {
        with_tracing!(async move {
            if object_ids.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                let objects_result: Result<Vec<AiyObjectResponse>, String> = results
                    .into_iter()
    ...
    async fn try_get_past_object(
        ...
        options: Option<AiyObjectDataOptions>,
    ) -> RpcResult<AiyPastObjectResponse> {
        with_tracing!(async move {
            let state = self.state.clone();
            ...
            match past_read {
                PastObjectRead::ObjectNotExists(id) => {
                    Ok(AiyPastObjectResponse::ObjectNotExists(id))
                }
                PastObjectRead::VersionFound(object_ref, o, layout) => {
                    ...
                    Ok(AiyPastObjectResponse::VersionFound(
                        (object_ref, o, layout, options, display_fields).try_into()?,
                    ))
                }
                PastObjectRead::ObjectDeleted(oref) => {
                    Ok(AiyPastObjectResponse::ObjectDeleted(oref.into()))
                }
                PastObjectRead::VersionNotFound(id, seq_num) => {
                    Ok(AiyPastObjectResponse::VersionNotFound(id, seq_num))
                }
                PastObjectRead::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                } => Ok(AiyPastObjectResponse::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                }),
    ...
    async fn try_get_object_before_version(
        ...
        self.try_get_past_object(
            object_id,
            version,
            Some(AiyObjectDataOptions::bcs_lossless()),
        )
        .await
    }
    ...
    async fn try_multi_get_past_objects(
        ...
        options: Option<AiyObjectDataOptions>,
    ) -> RpcResult<Vec<AiyPastObjectResponse>> {
        with_tracing!(async move {
            if past_objects.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                if !errors.is_empty() {
                    let error_string = errors
                        ...
                        .collect::<Vec<String>>()
                        .join("; ");
                    Err(anyhow!("{error_string}").into()) // Collects errors not related to AiyPastObjectResponse variants
                } else {
                    Ok(success)
                }
            } else {
        ...
        with_tracing!(async move {
            ...
            if opts.show_events && temp_response.effects.is_some() {
                ...
                let events = spawn_monitored_task!(async move {
                    ...
                    .flatten();
                match events {
                    None => temp_response.events = Some(AiyTransactionBlockEvents::default()),
                    Some(events) => match to_aiy_transaction_events(self, digest, events) {
    ...
    async fn multi_get_transaction_blocks(
        ...
        digests: Vec<TransactionDigest>,
        opts: Option<AiyTransactionBlockResponseOptions>,
    ) -> RpcResult<Vec<AiyTransactionBlockResponse>> {
        with_tracing!(async move {
    ...
    #[instrument(skip(self))]
    async fn get_events(&self, transaction_digest: TransactionDigest) -> RpcResult<Vec<AiyEvent>> {
        with_tracing!(async move {
            let state = self.state.clone();
            let transaction_kv_store = self.transaction_kv_store.clone();
            spawn_monitored_task!(async move{
            ...
            Ok(match events {
                Some(events) => events
                    ...
                    .map(|(seq, e)| {
                        ...
                        AiyEvent::try_from(e, transaction_digest, seq as u64, None, layout)
                    })
                    .collect::<Result<Vec<_>, _>>()
                    .map_err(Error::AiyError)?,
                None => vec![],
            })
        }).await.map_err(Error::from)?
        })
        ...
        with_tracing!(async move {
            version
                .map(|v| {
                    ...
                    .ok_or(AiyRpcInputError::ProtocolVersionUnsupported(
    ...
    async fn verify_zklogin_signature(
        ...
        let zklogin_env_native = match self
            ...
            .chain()
        {
            aiy_protocol_config::Chain::Mainnet | aiy_protocol_config::Chain::Testnet => {
                ZkLoginEnv::Prod
            }
            _ => ZkLoginEnv::Test,
        };
        let GenericSignature::ZkLoginAuthenticator(zklogin_sig) =
            ...
            return Err(AiyRpcInputError::GenericNotFound(
            ...
            ZkLoginIntentScope::TransactionData => {
                let tx_data: TransactionData =
                    bcs::from_bytes(&Base64::decode(&bytes).map_err(Error::from)?)
                        .map_err(Error::from)?;
                let intent_msg = IntentMessage::new(Intent::aiy_transaction(), tx_data.clone());
...
#[instrument(skip_all)]
fn to_aiy_transaction_events(
    fullnode_api: &ReadApi,
    tx_digest: TransactionDigest,
    events: TransactionEvents,
) -> Result<AiyTransactionBlockEvents, Error> {
    ...
    Ok(AiyTransactionBlockEvents::try_from(
        events,
        tx_digest,
        None,
...
async fn get_display_fields(
    ...
    let format = match Format::parse(MAX_DISPLAY_NESTED_LEVEL, &display_object.fields) {
        ...
        Err(e) => {
            return Ok(DisplayFieldsResponse {
                data: None,
                error: Some(AiyObjectResponseError::DisplayError {
                    error: e.to_string(),
                }),
            });
        }
    ...
    Ok(DisplayFieldsResponse {
        ...
        error: (!errors.is_empty()).then(|| AiyObjectResponseError::DisplayError {
            error: errors.join("; "),
        }),
    })
}
...
fn convert_to_response(
    ...
    opts: &AiyTransactionBlockResponseOptions,
    module_cache: &impl GetModule,
) -> RpcInterimResult<AiyTransactionBlockResponse> {
    let mut response = AiyTransactionBlockResponse::new(cache.digest);
    ...
    if let Some(transaction) = cache.transaction {
        ...
        if opts.show_input {
            response.transaction = Some(AiyTransactionBlock::try_from(
