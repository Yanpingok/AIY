...
use aiy_core::execution_cache::ObjectCacheRead;
use aiy_core::jsonrpc_index::TotalBalance;
use aiy_core::subscription_handler::SubscriptionHandler;
use aiy_json_rpc_types::{
    Coin as AiyCoin, DevInspectResults, DryRunTransactionBlockResponse, EventFilter, AiyEvent,
    AiyObjectDataFilter, TransactionFilter,
};
...
use aiy_types::base_types::{
    MoveObjectType, ObjectID, ObjectInfo, ObjectRef, SequenceNumber, AiyAddress,
};
use aiy_types::bridge::Bridge;
use aiy_types::committee::{Committee, EpochId};
use aiy_types::digests::{ChainIdentifier, TransactionDigest};
use aiy_types::dynamic_field::DynamicFieldInfo;
...
use aiy_types::storage::{BackingPackageStore, ObjectStore, WriteKind};
...
pub trait StateRead: Send + Sync {
    ...
    // governance_api
    async fn get_staked_aiy(&self, owner: AiyAddress) -> StateReadResult<Vec<StakedAiy>>;
    fn get_system_state(&self) -> StateReadResult<AiySystemState>;
...
impl StateRead for AuthorityState {
    ...
    async fn get_staked_aiy(&self, owner: AiyAddress) -> StateReadResult<Vec<StakedAiy>> {
        Ok(self
            .get_move_objects(owner, MoveObjectType::staked_aiy())
            .await?)
    }
    fn get_system_state(&self) -> StateReadResult<AiySystemState> {
        Ok(self
    ...
    async fn get_balance(
        ...
        Ok(tokio::task::spawn_blocking(move || {
            indexes
                .as_ref()
                .ok_or(AiyError::IndexStoreNotAvailable)?
                .get_balance(owner, coin_type)
        })
        .await
        .map_err(|e: JoinError| AiyError::ExecutionError(e.to_string()))??)
    }
...
pub enum StateReadError {
    // aiy_json_rpc::Error will do the final conversion to generic error message
    #[error(transparent)]
...
impl From<AiyError> for StateReadError {
    fn from(e: AiyError) -> Self {
        match e {
            AiyError::IndexStoreNotAvailable
            | AiyError::TransactionNotFound { .. }
            | AiyError::UnsupportedFeatureError { .. }
            | AiyError::UserInputError { .. }
            | AiyError::WrongMessageVersion { .. } => StateReadError::Client(e.into()),
