...
use aiy_core::jsonrpc_index::TotalBalance;
use tap::TapFallible;
use tracing::{debug, instrument};

use mysten_metrics::spawn_monitored_task;
use aiy_core::authority::AuthorityState;
use aiy_json_rpc_api::{cap_page_limit, CoinReadApiOpenRpc, CoinReadApiServer, JsonRpcMetrics};
use aiy_json_rpc_types::Balance;
use aiy_json_rpc_types::{CoinPage, AiyCoinMetadata};
...
use aiy_types::coin::{CoinMetadata, TreasuryCap};
use aiy_types::coin_registry::{Currency, SupplyState};
use aiy_types::effects::TransactionEffectsAPI;
use aiy_types::gas_coin::{GAS, TOTAL_SUPPLY_MIST};
...
use crate::{with_tracing, AiyRpcModule};

pub fn parse_to_struct_tag(coin_type: &str) -> Result<StructTag, AiyRpcInputError> {
    parse_aiy_struct_tag(coin_type)
        .map_err(|e| AiyRpcInputError::CannotParseAiyStructTag(format!("{e}")))
}

pub fn parse_to_type_tag(coin_type: Option<String>) -> Result<TypeTag, AiyRpcInputError> {
...
impl AiyRpcModule for CoinReadApi {
...
impl CoinReadApiServer for CoinReadApi {
            ...
            let cursor = match cursor {
                Some(c) => {
                    let decoded = CoinCursor::decode(&c).ok_or_else(|| {
                        AiyRpcInputError::GenericInvalid("invalid cursor".to_string())
                    })?;

                    if coin_type_tag.to_string() != decoded.coin_type {
                        return Err(
                            AiyRpcInputError::GenericInvalid("invalid cursor".to_string()).into(),
                        );
                    }
                    (
                        decoded.coin_type,
    ...
    #[instrument(skip(self))]
    async fn get_all_balances(&self, owner: AiyAddress) -> RpcResult<Vec<Balance>> {
        with_tracing!(async move {
    ...
    #[instrument(skip(self))]
    async fn get_coin_metadata(&self, coin_type: String) -> RpcResult<Option<AiyCoinMetadata>> {
        with_tracing!(async move {
...
async fn find_package_object_id(
    ...
    spawn_monitored_task!(async move {
        ...
        Err(AiyRpcInputError::GenericNotFound(format!(
...
mod tests {
    ...
    use aiy_storage::key_value_store::{
    ...
    use aiy_types::coin::TreasuryCap;
    use aiy_types::digests::{ObjectDigest, TransactionDigest};
    ...
    use aiy_types::messages_checkpoint::{CheckpointDigest, CheckpointSequenceNumber};
    use aiy_types::object::MoveObject;
    use aiy_types::object::Object;
    use aiy_types::object::Owner;
    use aiy_types::utils::create_fake_transaction;
    use aiy_types::{parse_aiy_struct_tag, TypeTag};
    ...
    mock! {
        ...
        impl TransactionKeyValueStoreTrait for KeyValueStore {
            ...
            ) -> AiyResult<KVStoreTransactionData>;
            ...
            ) -> AiyResult<KVStoreCheckpointData>;
            ...
            async fn get_object(&self, object_id: ObjectID, version: SequenceNumber) -> AiyResult<Option<Object>>;
            async fn multi_get_objects(&self, object_keys: &[aiy_types::storage::ObjectKey]) -> AiyResult<Vec<Option<Object>>>;
            ...
            async fn multi_get_events_by_tx_digests(&self,digests: &[TransactionDigest]) -> AiyResult<Vec<Option<TransactionEvents>>>;
        }
    }
    ...
    fn get_test_coin_type_tag(coin_type: String) -> TypeTag {
        TypeTag::Struct(Box::new(parse_aiy_struct_tag(&coin_type).unwrap()))
    }
    ...
    fn get_test_treasury_cap_peripherals(
        ...
        let input_coin_struct = parse_aiy_struct_tag(&coin_name).expect("should not fail");
    ...
    fn create_currency_object(currency: Currency) -> Object {
        use aiy_types::object::{Data, ObjectInner};
    ...
    mock! {
        pub ObjectStore {}
        impl aiy_types::storage::ObjectStore for ObjectStore {
    ...
    mod get_coins_tests {
        ...
        async fn test_coin_no_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_aiy_struct_tag(&coin.coin_type).unwrap()));
            let mut mock_state = MockStateRead::new();
            mock_state
                .expect_get_owned_coins()
                .with(
        ...
        async fn test_coin_with_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_aiy_struct_tag(&coins[0].coin_type).unwrap()));
            let mut mock_state = MockStateRead::new();
            mock_state
                .expect_get_owned_coins()
                .with(
        ...
        async fn test_unrecognized_token() {
            ...
            expected.assert_eq(&error_object.code().to_string());
            let expected =
                expect!["Invalid struct type: 0x2::aiy:🤵. Got error: unrecognized token: :🤵"];
            expected.assert_eq(error_object.message());
        }
            ...
            mock_state
                ...
                .returning(move |_, _, _, _| {
                    Err(StateReadError::Client(
                        AiyError::IndexStoreNotAvailable.into(),
                    ))
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
        ...
        async fn test_get_coins_iterator_typed_store_error() {
            ...
            mock_state
                .expect_get_owned_coins()
                .returning(move |_, _, _, _| {
                    Err(AiyError::Storage("mock rocksdb error".to_string()).into())
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_coins(owner, Some(coin_type.to_string()), None, None)
    ...
    mod get_balance_tests {
        ...
        async fn test_get_balance_execution_error() {
            ...
            let mut mock_state = MockStateRead::new();
            mock_state.expect_get_balance().returning(move |_, _| {
                Err(AiyError::ExecutionError("mock db error".to_string()).into())
            });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_balance(owner, Some(coin_type.to_string()))
    ...
    mod get_coin_metadata_tests {
        ...
        async fn test_valid_coin_metadata_object() {
            ...
            let metadata = AiyCoinMetadata::try_from(coin_metadata_object.clone()).unwrap();
        ...
        #[tokio::test]
        async fn test_find_package_object_not_aiy_coin_metadata() {
            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let input_coin_struct = parse_aiy_struct_tag(&coin_name).expect("should not fail");
            let coin_metadata_struct = CoinMetadata::type_(input_coin_struct.clone());
        ...
        async fn test_coin_metadata_with_registry_data() {
            use aiy_types::coin_registry::{self, Currency, MetadataCapState, SupplyState};
            use aiy_types::collection_types::VecMap;

            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let coin_struct = parse_aiy_struct_tag(&coin_name).expect("should not fail");
