...
use aiy_core::traffic_controller::TrafficController;
use aiy_types::traffic_control::PolicyConfig;
...
pub use aiy_config::node::ServerType;
use aiy_open_rpc::{Module, Project};
...
pub fn aiy_rpc_doc(version: &str) -> Project {
    Project::new(
        version,
        "Aiy JSON-RPC",
        "Aiy JSON-RPC API for interaction with Aiy Full node. Make RPC calls using https://fullnode.NETWORK.sui.io:443, where NETWORK is the network you want to use (testnet, devnet, mainnet). By default, local networks use port 9000.",
        "Mysten Labs",
...
impl JsonRpcServerBuilder {
    ...
    pub fn register_module<T: AiyRpcModule>(&mut self, module: T) -> Result<(), Error> {
    ...
    pub async fn start(
        ...
        info!(local_addr =? addr, "Aiy JSON-RPC server listening on {addr}");
        Ok(handle)
    }
}
