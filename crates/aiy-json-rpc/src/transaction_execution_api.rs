...
use crate::error::{Error, AiyRpcInputError};
use crate::{
...
use aiy_json_rpc_api::{JsonRpcMetrics, WriteApiOpenRpc, WriteApiServer};
use aiy_json_rpc_types::{
    DevInspectArgs, DevInspectResults, DryRunTransactionBlockResponse, AiyTransactionBlock,
...
use aiy_types::storage::PostExecutionPackageResolver;
...
impl TransactionExecutionApi {
    ...
    > {
        ...
        let transaction = if opts.show_input {
            ...
            Some(AiyTransactionBlock::try_from(
                txn.data().clone(),
    ...
    ) -> Result<AiyTransactionBlockResponse, Error> {
        let request_type =
    ...
    async fn handle_post_orchestration(
        ...
        transaction: Option<AiyTransactionBlock>,
        ...
        let events = if opts.show_events {
            ...
            Some(AiyTransactionBlockEvents::try_from(
    ...
    pub fn prepare_dry_run_transaction_block(
        &self,
        tx_bytes: Base64,
    ) -> Result<(TransactionData, TransactionDigest, Vec<InputObjectKind>), AiyRpcInputError> {
        let tx_data: TransactionData = self.convert_bytes(tx_bytes)?;
...
impl AiyRpcModule for TransactionExecutionApi {
