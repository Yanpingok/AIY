...
use crate::{consumer::ProtobufDecoder, peers::AiyNodeProvider};
use axum::{
    body::Body,
    body::Bytes,
...
/// we expect aiy-node to send us an http header content-length encoding.
...
/// we expect aiy-node to send us an http header content-type encoding.
...
/// we expect that calling aiy-nodes are known on the blockchain and we enforce
/// their pub key tls creds here
pub async fn expect_valid_public_key(
    Extension(allower): Extension<Arc<AiyNodeProvider>>,
