...
/// AiyNodeProvider queries the aiy blockchain and keeps a record of known validators based on the response from
/// aiy_getValidators.  The node name, public key and other info is extracted from the chain and stored in this
...
#[derive(Debug, Clone)]
pub struct AiyNodeProvider {
    aiy_nodes: AllowedPeers,
...
impl AiyNodeProvider {
    pub fn new(
        ...
        let aiy_nodes = Arc::new(RwLock::new(HashMap::new()));
    ...
    pub fn get(&self, key: &Ed25519PublicKey) -> Option<AllowedPeer> {
        ...
        // check aiy validators
        if let Some(v) = self.aiy_nodes.read().unwrap().get(key) {
            return Some(AllowedPeer {
                name: v.name.to_owned(),
                public_key: v.public_key.to_owned(),
            });
    ...
    /// Get a mutable reference to the allowed aiy validator map
    pub fn get_aiy_mut(&mut self) -> &mut AllowedPeers {
        &mut self.aiy_nodes
    }

    /// get_validators will retrieve known validators
    async fn get_validators(url: String) -> Result<AiySystemStateSummary> {
        let rpc_method = "aiyx_getLatestAiySystemState";
        let observe = || {
    ...
    async fn get_bridge_validators(url: String) -> Result<BridgeSummary> {
        let rpc_method = "aiyx_getLatestBridge";
    ...
    async fn update_aiy_validator_set(&self) {
        match Self::get_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => {
                let validators = extract(summary);
                let mut allow = self.aiy_nodes.write().unwrap();
                allow.clear();
                allow.extend(validators);
                info!(
                    "{} aiy validators managed to make it on the allow list",
                    allow.len()
                );
            }
            Err(error) => {
    ...
    async fn update_bridge_validator_set(&self, metrics_keys: MetricsPubKeys) {
        let aiy_system = match Self::get_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => summary,
            Err(error) => {
                JSON_RPC_STATE
                    .with_label_values(&["update_bridge_peer_count", "failed"])
                    .inc();
                error!("unable to get aiy system state: {error}");
                return;
            }
        };
        match Self::get_bridge_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => {
                let names = aiy_system
                    .active_validators
                    .into_iter()
                    .map(|v| (v.aiy_address, v.name))
                    .collect();
            ...
            Err(error) => {
                JSON_RPC_STATE
                    .with_label_values(&["update_bridge_peer_count", "failed"])
                    .inc();
                error!("unable to refresh aiy bridge peer list: {error}");
            }
        };
    }
    ...
    pub fn poll_peer_list(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                cloned_self.update_aiy_validator_set().await;
                cloned_self
...
/// extract will get the network pubkey bytes from a AiyValidatorSummary type.  This type comes from a
...
fn extract(
    ...
    summary.active_validators.into_iter().filter_map(|vm| {
        match Ed25519PublicKey::from_bytes(&vm.network_pubkey_bytes) {
            Ok(public_key) => {
                debug!(
                    "adding public key {:?} for aiy validator {:?}",
                    public_key, vm.name
                );
                Some((
                    public_key.clone(),
            ...
            Err(error) => {
                error!(
                    "unable to decode public key for name: {:?} aiy_address: {:?} error: {error}",
                    vm.name, vm.aiy_address
                );
...
async fn extract_bridge(
    ...
    names: Arc<BTreeMap<AiyAddress, String>>,
    ...
    let results: Vec<_> = stream::iter(committee_members)
        .filter_map(|(_, cm)| {
            ...
            async move {
                ...
                let bridge_name = names.get(&cm.aiy_address).cloned().unwrap_or_else(|| {
                    warn!(
                        address =% cm.aiy_address,
                        "Bridge node not found in aiy committee, using base URL as the name",
                    );
...
mod tests {
    ...
    use serde::Serialize;
    use aiy_types::base_types::AiyAddress;
    use aiy_types::bridge::{BridgeCommitteeSummary, BridgeSummary, MoveTypeCommitteeMember};
    use aiy_types::aiy_system_state::aiy_system_state_summary::{
        AiySystemStateSummary, AiyValidatorSummary,
    };

    /// creates a test that binds our proxy use case to the structure in aiy_getLatestAiySystemState
    ...
    #[test]
    fn depend_on_aiy_aiy_system_state_summary() {
        let CertKeyPair(_, client_pub_key) = generate_self_cert("aiy".into());
        // all fields here just satisfy the field types, with exception to active_validators, we use
        // some of those.
        let depends_on = AiySystemStateSummary {
            active_validators: vec![AiyValidatorSummary {
        ...
        let r = serde_json::to_string(&ResponseBody { result: depends_on })
            .expect("expected to serialize ResponseBody{AiySystemStateSummary}");

        let deserialized = serde_json::from_str::<ResponseBody>(&r)
            .expect("expected to deserialize ResponseBody{AiySystemStateSummary}");
