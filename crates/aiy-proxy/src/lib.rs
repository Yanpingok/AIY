...
mod tests {
    ...
    use crate::{admin::CertKeyPair, config::RemoteWriteConfig, peers::AiyNodeProvider};
    ...
    use aiy_tls::{ClientCertVerifier, TlsAcceptor};
    ...
    /// aiy-node client that posts data to the proxy using the protobuf format.  The server processes this
    ...
    async fn test_axum_acceptor() {
        ...
        let CertKeyPair(client_priv_cert, client_pub_key) = admin::generate_self_cert("aiy".into());
        ...
        let mut allower = AiyNodeProvider::new("".into(), Duration::from_secs(30), vec![]);
        ...
        allower.get_aiy_mut().write().unwrap().insert(
