...
use aiy_types::base_types::{ExecutionDigests, ObjectID, SequenceNumber, TransactionDigest};
use aiy_types::bridge::{BridgeChainId, BRIDGE_CREATE_FUNCTION_NAME, BRIDGE_MODULE_NAME};
...
use aiy_types::crypto::{
    ...
    AuthoritySignature, DefaultHash, AiyAuthoritySignature,
};
use aiy_types::deny_list_v1::{DENY_LIST_CREATE_FUNC, DENY_LIST_MODULE};
use aiy_types::digests::ChainIdentifier;
use aiy_types::effects::{TransactionEffects, TransactionEffectsAPI, TransactionEvents};
use aiy_types::epoch_data::EpochData;
use aiy_types::execution_params::ExecutionOrEarlyError;
use aiy_types::gas::AiyGasStatus;
use aiy_types::gas_coin::GasCoin;
use aiy_types::governance::StakedAiy;
use aiy_types::id::UID;
use aiy_types::in_memory_storage::InMemoryStorage;
...
impl Builder {
    ...
    pub fn add_validator_signature(mut self, keypair: &AuthorityKeyPair) -> Self {
        ...
        let checkpoint_signature = {
            let intent_msg = IntentMessage::new(
                Intent::aiy_app(IntentScope::CheckpointSummary),
                checkpoint.clone(),
            );
        ...
        let token_distribution_schedule =
            if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
                ...
                TokenDistributionSchedule::new_for_validators_with_default_allocation(
                    validators.iter().map(|v| v.info.aiy_address()),
                )
            };
    ...
    fn committee(objects: &[Object]) -> Committee {
        let aiy_system_object =
            get_aiy_system_state(&objects).expect("Aiy System State object must always exist");
        aiy_system_object
    ...
    fn validate_inputs(&self) -> anyhow::Result<(), anyhow::Error> {
        ...
        if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
            ...
            token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
                self.validators.values().map(|v| v.info.aiy_address()),
            );
        }
    ...
    fn validate_output(&self) {
        ...
        let system_state = match unsigned_genesis.aiy_system_object() {
            AiySystemState::V1(inner) => inner,
            AiySystemState::V2(_) => unreachable!(),
            #[cfg(msim)]
            _ => {
            ...
            .zip(system_state.validators.active_validators.iter())
        {
            ...
            assert!(address_to_pool_id
                .insert(metadata.aiy_address, onchain_validator.staking_pool.id)
                .is_none());
            assert_eq!(validator.info.aiy_address(), metadata.aiy_address);
            assert_eq!(validator.info.protocol_key(), metadata.aiy_pubkey_bytes());
        ...
        let mut gas_objects: BTreeMap<ObjectID, (&Object, GasCoin)> = unsigned_genesis
            ...
            .collect();
        let mut staked_aiy_objects: BTreeMap<ObjectID, (&Object, StakedAiy)> = unsigned_genesis
            .objects()
            .iter()
            .filter_map(|o| StakedAiy::try_from(o).ok().map(|s| (o.id(), (o, s))))
            .collect();
        ...
        for allocation in token_distribution_schedule.allocations {
            if let Some(staked_with_validator) = allocation.staked_with_validator {
                ...
                let staked_aiy_object_id = staked_aiy_objects
                    .iter()
                    .find(|(_k, (o, s))| {
                ...
                let staked_aiy_object = staked_aiy_objects.remove(&staked_aiy_object_id).unwrap();
                assert_eq!(
                    staked_aiy_object.0.owner,
                    Owner::AddressOwner(allocation.recipient_address)
                );
                assert_eq!(staked_aiy_object.1.principal(), allocation.amount_mist);
                assert_eq!(staked_aiy_object.1.pool_id(), staking_pool_id);
                assert_eq!(staked_aiy_object.1.activation_epoch(), 0);
            } else {
        ...
        if !self.parameters.allow_insertion_of_extra_objects {
            ...
            assert!(staked_aiy_objects.is_empty());
        }
...
fn get_genesis_protocol_config(version: ProtocolVersion) -> ProtocolConfig {
    ...
    // AIY_PROTOCOL_CONFIG_CHAIN_OVERRIDE if necessary
...
fn build_unsigned_genesis_data(
    ...
    token_distribution_schedule.validate();
    token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
        genesis_validators.iter().map(|v| v.aiy_address),
    );
    ...
    let mut system_packages =
        aiy_framework_snapshot::load_bytecode_snapshot(parameters.protocol_version.as_u64())
...
// on `aiy_system` which is mocked in some tests, but would be in the loader
    ...
    let genesis_transaction = {
        let genesis_objects = objects
            ...
            .map(|mut object| {
                ...
                aiy_types::transaction::GenesisObject::RawObject {
                    data: object.data,
                    owner: object.owner,
                }
            })
        ...
        aiy_types::transaction::VerifiedTransaction::new_genesis_transaction(genesis_objects)
            .into_inner()
    };
...
pub fn generate_genesis_system_object(
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        // Step 1: Create the AiySystemState UID
        let aiy_system_state_uid = builder.programmable_move_call(
            AIY_FRAMEWORK_ADDRESS.into(),
            ident_str!("object").to_owned(),
            ident_str!("aiy_system_state").to_owned(),
            vec![],
            vec![],
        );
        ...
        if protocol_config.enable_bridge() {
            let bridge_uid = builder
                .input(CallArg::Pure(UID::new(AIY_BRIDGE_OBJECT_ID).to_bcs_bytes()))
                .unwrap();
            // TODO(bridge): this needs to be passed in as a parameter for next testnet regenesis
            // Hardcoding chain id to AiyCustom
            let bridge_chain_id = builder.pure(BridgeChainId::AiyCustom).unwrap();
        ...
        // Step 4: Mint the supply of AIY.
        let aiy_supply = builder.programmable_move_call(
            AIY_FRAMEWORK_ADDRESS.into(),
            ident_str!("aiy").to_owned(),
        ...
        // Step 5: Run genesis.
        // The first argument is the system state uid we got from step 1 and the second one is the AIY supply we
        // got from step 3.
        let mut arguments = vec![aiy_system_state_uid, aiy_supply];
    ...
    {
        let object = written.get_mut(&aiy_types::AIY_CLOCK_OBJECT_ID).unwrap();
        object
            .data
            .try_as_move_mut()
            .unwrap()
...
mod test {
    ...
    use fastcrypto::traits::KeyPair;
    use aiy_config::genesis::*;
    use aiy_config::local_ip_utils;
    use aiy_config::node::DEFAULT_COMMISSION_RATE;
    use aiy_config::node::DEFAULT_VALIDATOR_GAS_PRICE;
    ...
    fn allocation_csv() {
        let schedule = TokenDistributionSchedule::new_for_validators_with_default_allocation([
            AiyAddress::random_for_testing_only(),
            AiyAddress::random_for_testing_only(),
        ]);
    ...
    fn ceremony() {
        ...
        let validator = ValidatorInfo {
            ...
            account_address: AiyAddress::from(account_key.public()),
