...
use aiy_bridge_schema::models::{BridgeDataSource, GovernanceAction};
use aiy_bridge_schema::schema;
...
use aiy_indexer_alt_framework::types::BRIDGE_ADDRESS;
use tracing::info;
            ...
            for ev in tx.events.iter().flat_map(|e| &e.data) {
                ...
                let (action, data) = match &ev.type_ {
                    ...
                    t if t == &self.emergency_op_event_type => {
                        ...
                        self.metrics
                            .governance_actions_total
                            .with_label_values(&["emergency_operation", "aiy"])
                            .inc();
                        self.metrics
                            .bridge_pause_status
                    ...
                    t if t == &self.blocklist_event_type => {
                        info!(?ev, "Observed Aiy Blocklist Validator");
                    ...
                    t if t == &self.token_reg_event_type => {
                        info!(?ev, "Observed Aiy Token Registration");
                        let event: MoveTokenRegistrationEvent = bcs::from_bytes(&ev.contents)?;
                        (AddAiyTokens, serde_json::to_value(event)?)
                    }
                    t if t == &self.update_price_event_type => {
                        info!(?ev, "Observed Aiy Token Price Update");
                    ...
                    t if t == &self.new_token_event_type => {
                        info!(?ev, "Observed Aiy New token event");
