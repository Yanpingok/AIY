...
use std::sync::Arc;
use aiy_bridge::events::MoveTokenDepositedEvent;
use aiy_bridge_schema::models::TokenTransferData;
use aiy_bridge_schema::schema::token_transfer_data;
        ...
        for tx in &checkpoint.transactions {
            ...
            for ev in tx.events.iter().flat_map(|e| &e.data) {
                ...
                results.push(TokenTransferData {
                    ...
                    amount: event.amount_aiy_adjusted as i64,
                    is_finalized: true,
...
impl Handler for TokenTransferDataHandler {
    ...
    ) -> aiy_indexer_alt_framework::Result<usize> {
