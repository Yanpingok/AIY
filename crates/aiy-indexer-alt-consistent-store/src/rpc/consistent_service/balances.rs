...
use aiy_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha as grpc;
use aiy_indexer_alt_framework::types::{base_types::AiyAddress, TypeTag};
...
/// Parse the owner's `AiyAddress` from a string. Addresses must start with `0x` followed by
/// between 1 and 64 hexadecimal characters.
///
/// TODO: Switch to using `aiy_sdk_types::Address`, once the indexing framework is ported to the
/// new SDK.
fn owner(input: &str) -> Result<AiyAddress, Error> {
    ...
    AiyAddress::from_str(s.as_ref()).map_err(|_| Error::InvalidOwner(s.to_string()))
}
