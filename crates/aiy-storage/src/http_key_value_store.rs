...
trait IntoAiyResult<T> {
    fn into_aiy_result(self) -> AiyResult<T>;
}

impl<T, E> IntoAiyResult<T> for Result<T, E>
where
    E: std::error::Error,
{
    fn into_aiy_result(self) -> AiyResult<T> {
        self.map_err(|e| AiyError::Storage(e.to_string()))
    }
}
...
impl HttpKVStore {
    pub fn new_kv(
        ...
        cache_size: u64,
        metrics: Arc<KeyValueStoreMetrics>,
    ) -> AiyResult<TransactionKeyValueStore> {
    ...
    pub fn new(
        ...
        let base_url = Url::parse(&base_url).into_aiy_result()?;
    ...
    fn get_url(&self, key: &Key) -> AiyResult<Url> {
        let (digest, item_type) = key.to_path_elements();
        let joined = self
            .base_url
            .join(&format!("{}/{}", digest, item_type))
            .into_aiy_result()?;
        Url::from_str(joined.as_str()).into_aiy_result()
    }

    async fn multi_fetch(&self, uris: Vec<Key>) -> Vec<AiyResult<Option<Bytes>>> {
    ...
    async fn fetch(&self, key: Key) -> AiyResult<Option<Bytes>> {
        ...
        if resp.status().is_success() {
            let bytes = resp.bytes().await.into_aiy_result()?;
...
fn map_fetch<'a, K>(fetch: (&'a AiyResult<Option<Bytes>>, &'a K)) -> Option<(&'a Bytes, &'a K)>
where
    K: std::fmt::Debug,
{
    let (fetch, key) = fetch;
