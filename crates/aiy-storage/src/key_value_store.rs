...
use aiy_types::digests::{CheckpointDigest, TransactionDigest};
...
impl TransactionKeyValueStore {
    ...
    ) -> AiyResult<Vec<Option<CertifiedCheckpointSummary>>> {
    ...
    pub async fn multi_get_checkpoints_contents(
        &self,
        keys: &[CheckpointSequenceNumber],
    ) -> AiyResult<Vec<Option<CheckpointContents>>> {
        self.multi_get_checkpoints(&[], keys, &[])
            .await
    ...
    ) -> AiyResult<Vec<Option<Transaction>>> {
    ...
    pub async fn multi_get_fx_by_tx_digest(
        &self,
        keys: &[TransactionDigest],
    ) -> AiyResult<Vec<Option<TransactionEffects>>> {
    ...
    pub async fn get_tx(&self, digest: TransactionDigest) -> AiyResult<Transaction> {
        self.multi_get_tx(&[digest])
            .await?
            .into_iter()
            .next()
            .flatten()
            .ok_or(AiyError::TransactionNotFound { digest })
    }
    ...
    ) -> AiyResult<CertifiedCheckpointSummary> {
