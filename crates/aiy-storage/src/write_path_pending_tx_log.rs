...
use aiy_types::crypto::EmptySignInfo;
use aiy_types::error::{AiyError, AiyResult};
use aiy_types::message_envelope::TrustedEnvelope;
use aiy_types::transaction::{SenderSignedData, VerifiedTransaction};
...
impl WritePathPendingTransactionLog {
    ...
    pub fn finish_transaction(&self, tx: &TransactionDigest) -> AiyResult {
        let mut transactions_set = self.transactions_set.lock();
        ...
        write_batch.write().map_err(AiyError::from)?;
        transactions_set.remove(tx);
        Ok(())
    }

    pub fn load_all_pending_transactions(&self) -> AiyResult<Vec<VerifiedTransaction>> {
        let mut transactions_set = self.transactions_set.lock();
