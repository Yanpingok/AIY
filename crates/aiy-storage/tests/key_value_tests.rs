...
use aiy_types::digests::{CheckpointContentsDigest, CheckpointDigest, TransactionDigest};
...
use aiy_storage::http_key_value_store::*;
...
mod simtests {
    ...
    async fn test_server(data: Arc<Mutex<HashMap<String, Vec<u8>>>>) {
        let handle = aiy_simulator::runtime::Handle::current();
