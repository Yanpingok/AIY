...
use crate::validator_commands::{
    get_validator_summary, AiyValidatorCommand, AiyValidatorCommandResponse,
};
use anyhow::Ok;
...
use aiy_types::{base_types::AiyAddress, crypto::Signature, transaction::Transaction};
use test_cluster::TestClusterBuilder;

#[tokio::test]
async fn test_print_raw_rgp_txn() -> Result<(), anyhow::Error> {
    let test_cluster = TestClusterBuilder::new().build().await;
    let keypair: &AiyKeyPair = test_cluster
        .swarm
        .config()
        .validator_configs
        .first()
        ...
        .account_key_pair
        .keypair();
    let validator_address: AiyAddress = AiyAddress::from(&keypair.public());
    let mut context = test_cluster.wallet;
    let aiy_client = context.get_client().await?;
    let (_, summary) = get_validator_summary(&aiy_client, validator_address)
        .await?
        .unwrap();
    let operation_cap_id = summary.operation_cap_id;

    // Execute the command and get the serialized transaction data.
    let response = AiyValidatorCommand::DisplayGasPriceUpdateRawTxn {
    ...
    .execute(&mut context)
    .await?;
    let AiyValidatorCommandResponse::DisplayGasPriceUpdateRawTxn {
        data,
        serialized_data,
    } = response
    else {
    ...
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::aiy_transaction(), deserialized_data),
        keypair,
    );
