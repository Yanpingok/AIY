...
use aiy_types::crypto::{
    get_authority_key_pair, EncodeDecodeBase64, Signature, SignatureScheme, AiyKeyPair,
    ZkLoginPublicIdentifier,
};
use aiy_types::crypto::{DefaultHash, PublicKey};
use aiy_types::error::AiyResult;
use aiy_types::multisig::{MultiSig, MultiSigPublicKey, ThresholdUnit, WeightUnit};
use aiy_types::multisig_legacy::{MultiSigLegacy, MultiSigPublicKeyLegacy};
use aiy_types::signature::{GenericSignature, VerifyParams};
use aiy_types::signature_verification::VerifiedDigestCache;
use aiy_types::transaction::{TransactionData, TransactionDataAPI};
...
pub enum KeyToolCommand {
    ...
    /// encoded 33 byte flag || private key starting with "aiyprivkey").
    /// Hex private key format import and export are both deprecated in
    /// Aiy Wallet and Aiy CLI Keystore. Use `aiy keytool import` if you
    /// wish to import a key to Aiy Keystore.
    ...
    ///
    /// Use `aiy client new-address` if you want to generate and save the key into aiy.keystore.
    Generate {
    ...
    /// Add a new key to Aiy CLI Keystore using either the input mnemonic phrase or a Bech32 encoded 33-byte
    /// `flag || privkey` starting with "aiyprivkey", the key scheme flag {ed25519 | secp256k1 | secp256r1}
    ...
    /// Output the private key of the given key identity in Aiy CLI Keystore as Bech32
    /// encoded string starting with `aiyprivkey`.
    Export {
        #[clap(long)]
        key_identity: KeyIdentity,
    },
    /// List all keys by its Aiy address, Base64 encoded public key, key scheme name in
    /// aiy.keystore.
    List {
        /// Sort by alias
        #[clap(long, short = 's')]
    ...
    /// [enum AiyKeyPair] (Base64 encoded of 33-byte `flag || privkey`) or `type AuthorityKeyPair`
    ...
    /// To MultiSig Aiy Address. Pass in a list of all public keys `flag || pk` in Base64.
    ...
    /// result can be used as signature field for `aiy client execute-signed-tx`. The sum
    ...
    Show { file: PathBuf },
    /// Create signature using the private key for the given address (or its alias) in aiy keystore.
    ...
    /// This takes [enum AiyKeyPair] of Base64 encoded of 33-byte `flag || privkey`). It
    /// outputs the keypair into a file at the current directory where the address is the filename,
    /// and prints out its Aiy address, Base64 encoded public key, the key scheme, and the key scheme flag.
    ...
    ZkLoginEnterToken {
        ...
        kp_bigint: String,
        #[clap(long)]
        ephemeral_key_identifier: AiyAddress,
        #[clap(long, default_value = "devnet")]
        network: String,
        #[clap(long, default_value = "false")]
        test_multisig: bool,
    ...
    /// Example request: aiy keytool zk-login-sig-verify --sig $SERIALIZED_ZKLOGIN_SIG --bytes $BYTES --intent-scope 0 --network devnet --curr-epoch 10
    ZkLoginSigVerify {
    ...
    /// e.g. aiy keytool zk-login-insecure-sign-personal-message --data "hello" --max-epoch 5
...
pub struct ConvertOutput {
    bech32_with_flag: String, // latest Aiy Keystore and Aiy Wallet import/export format
    base64_with_flag: String, // Aiy Keystore storage format
    hex_without_flag: String, // Legacy Aiy Wallet format
    scheme: String,
}
...
pub struct SignData {
    ...
    // serialized Aiy signature to be send for executing the transaction.
    aiy_signature: String,
}
    ...
    pub async fn execute(self, keystore: &mut Keystore) -> Result<CommandOutput, anyhow::Error> {
        let cmd_result = Ok(match self {
            ...
            KeyToolCommand::DecodeMultiSig {
                ...
                let address = AiyAddress::from(multisig.get_pk());
            ...
            KeyToolCommand::Generate {
                ...
                SignatureScheme::BLS12381 => {
                    let (aiy_address, kp) = get_authority_key_pair();
                    let file_name = format!("bls-{aiy_address}.key");
                ...
                _ => {
                    let (aiy_address, skp, _scheme, phrase) =
                        generate_new_key(key_scheme, derivation_path, word_length)?;
                    let file = format!("{aiy_address}.key");
            ...
            KeyToolCommand::Import {
                ...
                if Hex::decode(&input_string).is_ok() {
                    return Err(anyhow!(
                        "Aiy Keystore and Aiy Wallet no longer support importing 
                    private key as Hex, if you are sure your private key is encoded in Hex, use 
                    `aiy keytool convert $HEX` to convert first then import the Bech32 encoded 
                    private key starting with `aiyprivkey`."
                    ));
                }

                match AiyKeyPair::decode(&input_string) {
                    Ok(skp) => {
                        ...
                        let alias = match alias {
                            Some(x) => x,
                            None => keystore.get_alias(&key.aiy_address)?,
                        };
                    ...
                    Err(_) => {
                        ...
                        let aiy_address = keystore
                            ...
                            .await?;
                        let skp = keystore.export(&aiy_address)?;
                        let mut key = Key::from(skp);
            ...
            KeyToolCommand::Export { key_identity } => {
                ...
                let mut key = Key::from(skp);
                key.alias = keystore.get_alias(&key.aiy_address).ok();
                let key = ExportedKey {
                ...
                let output = match read_keypair_from_file(&file) {
                    Ok(keypair) => {
                        ...
                        let network_worker_keypair = match &keypair {
                            AiyKeyPair::Ed25519(kp) => kp.encode_base64(),
                            AiyKeyPair::Secp256k1(kp) => kp.encode_base64(),
                            AiyKeyPair::Secp256r1(kp) => kp.encode_base64(),
                        };
                        KeypairData {
            ...
            KeyToolCommand::MultiSigAddress {
                ...
                let address: AiyAddress = (&multisig_pk).into();
                ...
                for (pk, w) in pks.into_iter().zip(weights.into_iter()) {
                    output.multisig.push(MultiSigOutput {
                        address: Into::<AiyAddress>::into(&pk),
                        public_base64_key: pk.encode_base64(),
                        weight: w,
                    });
                }
                    ...
                    Err(_) => match read_authority_keypair_from_file(&file) {
                        Ok(keypair) => {
                            ...
                            CommandOutput::Show(Key {
                                ...
                                aiy_address: (keypair.public()).into(),
                                public_base64_key,
                                key_scheme: SignatureScheme::BLS12381.to_string(),
                                flag: SignatureScheme::BLS12381.flag(),
                                peer_id: None,
            ...
            KeyToolCommand::Sign {
                ...
                let intent = intent.unwrap_or_else(Intent::aiy_transaction);
                ...
                CommandOutput::Sign(SignData {
                    ...
                    aiy_signature: aiy_signature.encode_base64(),
                })
            }
            ...
            KeyToolCommand::SignKMS {
                ...
                let pk_owner = PublicKey::decode_base64(&base64pk)
                    .map_err(|e| anyhow!("Invalid base64 key: {:?}", e))?;
                let address_owner = AiyAddress::from(&pk_owner);
            ...
            KeyToolCommand::Unpack { keypair } => {
                let keypair = AiyKeyPair::decode_base64(&keypair)
                    .map_err(|_| anyhow!("Invalid Base64 encode keypair"))?;

                let key = Key::from(&keypair);
                let path_str = format!("{}.key", key.aiy_address).to_lowercase();
            ...
            KeyToolCommand::ZkLoginSignAndExecuteTx {
                ...
                let skp = if fixed {
                    ...
                    AiyKeyPair::Ed25519(Ed25519KeyPair::generate(&mut rand::thread_rng()))
                };
                println!("Ephemeral keypair: {:?}", skp.encode());
                let pk = skp.public();
                let ephemeral_key_identifier: AiyAddress = (&skp.public()).into();
...
impl From<&AiyKeyPair> for Key {
    fn from(skp: &AiyKeyPair) -> Self {
        Key::from(skp.public())
    }
}
...
impl From<PublicKey> for Key {
    fn from(pk: PublicKey) -> Self {
        Key {
            ...
            aiy_address: AiyAddress::from(&pk),
            public_base64_key: pk.encode_base64(),
...
fn convert_private_key_to_bech32(value: String) -> Result<ConvertOutput, anyhow::Error> {
    let skp = match AiyKeyPair::decode(&value) {
        Ok(s) => s,
        Err(_) => match Hex::decode(&value) {
            Ok(decoded) => {
                ...
                AiyKeyPair::Ed25519(Ed25519KeyPair::from_bytes(&decoded)?)
            }
            Err(_) => match AiyKeyPair::decode_base64(&value) {
                Ok(skp) => skp,
                Err(_) => match Ed25519KeyPair::decode_base64(&value) {
                    Ok(kp) => AiyKeyPair::Ed25519(kp),
