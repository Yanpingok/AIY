...
use aiy_config::verifier_signing_config::VerifierSigningConfig;
use aiy_move::manage_package::resolve_lock_file_path;
use aiy_protocol_config::{Chain, ProtocolConfig, ProtocolVersion};
use aiy_source_validation::{BytecodeSourceVerifier, ValidationMode};

use shared_crypto::intent::Intent;
use aiy_json::AiyJsonValue;
use aiy_json_rpc_types::{
    Coin, DevInspectArgs, DevInspectResults, DryRunTransactionBlockResponse, DynamicFieldInfo,
    DynamicFieldPage, AiyCoinMetadata, AiyData, AiyExecutionStatus, AiyObjectData,
    AiyObjectDataOptions, AiyObjectResponse, AiyObjectResponseQuery, AiyParsedData,
    AiyProtocolConfigValue, AiyRawData, AiyTransactionBlockEffects, AiyTransactionBlockEffectsAPI,
    AiyTransactionBlockResponse, AiyTransactionBlockResponseOptions,
};
use aiy_keys::key_identity::KeyIdentity;
...
use aiy_package_management::{
    system_package_versions::{latest_system_packages, system_packages_for_protocol},
    LockCommand, PublishedAtError,
};
use aiy_sdk::{
    apis::ReadApi,
    aiy_client_config::{AiyClientConfig, AiyEnv},
    wallet_context::WalletContext,
    AiyClient, AIY_COIN_TYPE, AIY_DEVNET_URL, AIY_LOCAL_NETWORK_URL, AIY_LOCAL_NETWORK_URL_0,
    AIY_TESTNET_URL,
};
use aiy_types::{
    base_types::{FullObjectID, ObjectID, ObjectRef, ObjectType, SequenceNumber, AiyAddress},
...
use aiy_keys::key_derive;
use aiy_types::digests::ChainIdentifier;
...
pub enum AiyClientCommands {
    ...
    Balance {
        ...
        /// Show balance for the specified coin (e.g., 0x2::aiy::AIY).
    ...
    Call {
        ...
        #[clap(
            long,
            value_parser = parse_aiy_type_tag,
            num_args(1..),
        )]
    ...
    /// List all Aiy environments
    Envs,

    /// Execute a Signed Transaction. This is useful when the user prefers to sign elsewhere and use this command to execute.
    ExecuteSignedTx {
        /// BCS serialized transaction data bytes without its type tag, as base64 encoded string. This is the output of aiy client command using --serialize-unsigned-transaction.
        #[clap(long)]
        tx_bytes: String,
    ...
    ExecuteCombinedSignedTx {
        /// BCS serialized sender signed data, as base64 encoded string. This is the output of aiy client command using --serialize-signed-transaction.
        #[clap(long)]
    ...
    Objects {
        ...
        /// objects owned by `aiy client active-address`.
        #[clap(name = "owner_address")]
        address: Option<KeyIdentity>,
    },
    ...
    /// Pay all residual AIY coins to the recipient with input coins, after deducting the gas cost.
    ...
    /// Pay AIY coins to recipients following specified amounts, with input coins.
    /// Length of recipients must be the same as that of amounts.
    /// The input coins also include the coin for gas payment, so no extra gas coin is required.
    PayAiy {
        /// The input coins to be used for pay recipients, including the gas coin.
    ...
    /// Replay a given transaction to view transaction effects (deprecated; use `aiy replay` instead)
    #[clap(name = "replay-transaction")]
    ReplayTransaction {},

    /// Replay transactions listed in a file (deprecated; use `aiy replay` instead)
    #[clap(name = "replay-batch")]
    ReplayBatch {},

    /// Replay all transactions in a range of checkpoints (deprecated; use `aiy replay` instead)
...
pub struct TxProcessingArgs {
    ...
    /// be used to execute transaction with `aiy client execute-signed-tx --tx-bytes <TX_BYTES>`.
    #[arg(long)]
    ...
    /// `aiy client execute-combined-signed-tx --signed-tx-bytes <SIGNED_TX_BYTES>`.
    #[arg(long)]
...
impl AiyClientCommands {
    pub async fn execute(
        self,
        context: &mut WalletContext,
    ) -> Result<AiyClientCommandResult, anyhow::Error> {
        let ret = match self {
            AiyClientCommands::ReplayTransaction {} => {
                eprintln!("This command is deprecated. Use `aiy replay` instead.");
                AiyClientCommandResult::NoOutput
            }
            AiyClientCommands::ReplayBatch {} => {
                eprintln!("This command is deprecated. Use `aiy replay` instead.");
                AiyClientCommandResult::NoOutput
            }
            AiyClientCommands::ReplayCheckpoints {} => {
                eprintln!("This command is deprecated. Use `aiy replay` instead.");
                AiyClientCommandResult::NoOutput
            }
            AiyClientCommands::Addresses { sort_by_alias } => {
                let active_address = context.active_address()?;
                let mut addresses: Vec<(String, AiyAddress)> = context
                    .config
                    .keystore
                ...
                AiyClientCommandResult::Addresses(output)
            }
            AiyClientCommands::Balance {
                address,
                coin_type,
                with_coins,
            } => {
                ...
                let aiy_type_tag = canonicalize_type(AIY_COIN_TYPE)?;

                // show AIY first
                let ordered_coins_aiy_first = coins_by_type
                    .remove(&aiy_type_tag)
                    .into_iter()
                    .chain(coins_by_type.into_values())
                    .collect();

                AiyClientCommandResult::Balance(ordered_coins_aiy_first, with_coins)
            }

            AiyClientCommands::DynamicFieldQuery { id, cursor, limit } => {
                let client = context.get_client().await?;
                let df_read = client
                    .read_api()
                    .get_dynamic_fields(id, cursor, Some(limit))
                    .await?;
                AiyClientCommandResult::DynamicFieldQuery(df_read)
            }

            AiyClientCommands::Upgrade {
                package_path,
                upgrade_capability,
                build_config,
                ...
                let package_path =
                    package_path
                        .canonicalize()
                        .map_err(|e| AiyError::ModulePublishFailure {
                ...
                let previous_id = if let Some(ref chain_id) = chain_id {
                    aiy_package_management::set_package_id(
                        &package_path,
                ...
                if let (Some(chain_id), Some(previous_id)) = (chain_id.clone(), previous_id) {
                    let _ = aiy_package_management::set_package_id(
                        &package_path,
                        build_config.install_dir.clone(),
                        &chain_id,
                        previous_id,
                ...
                if let AiyClientCommandResult::TransactionBlock(ref response) = result {
                    if let Err(e) = aiy_package_management::update_lock_file(
                        context,
                        LockCommand::Upgrade,
            ...
            AiyClientCommands::Publish {
                ...
                if build_config.test_mode {
                    return Err(AiyError::ModulePublishFailure {
                        error:
                            ...
                            compiling the package normally with `aiy move build`."
                                .to_string(),
                    }
                    .into());
                }
            ...
            AiyClientCommands::VerifyBytecodeMeter {
                protocol_version,
                module_paths,
                package_path,
                build_config,
                ...
                let mut verifier = aiy_execution::verifier(
                    &protocol_config,
                    signing_limits,
                ...
                AiyClientCommandResult::VerifyBytecodeMeter {
                    success: !exceeded,
            ...
            AiyClientCommands::Object { id, bcs } => {
                // Fetch the object ref
                let client = context.get_client().await?;
                if !bcs {
                    let object_read = client
                        .read_api()
                        .get_object_with_options(id, AiyObjectDataOptions::full_content())
                        .await?;
                    AiyClientCommandResult::Object(object_read)
                } else {
                    let raw_object_read = client
                        .read_api()
                        .get_object_with_options(id, AiyObjectDataOptions::bcs_lossless())
                        .await?;
                    AiyClientCommandResult::RawObject(raw_object_read)
                }
            }

            AiyClientCommands::TransactionBlock { digest } => {
                let client = context.get_client().await?;
                let tx_read = client
                    .read_api()
                    ...
                    .await?;
                AiyClientCommandResult::TransactionBlock(tx_read)
            }

            AiyClientCommands::Call {
                package,
                module,
                function,
                type_args,
                ...
                // without failing AiyJSON's checks.
                let args = args
                    .into_iter()
                    .map(|value| AiyJsonValue::new(convert_number_to_string(value.to_json_value())))
                    .collect::<Result<_, _>>()?;
            ...
            AiyClientCommands::TransferAiy {
                to,
                aiy_coin_object_id: object_id,
                amount,
                gas_data,
                processing,
            } => {
                ...
                let tx_kind = client
                    .transaction_builder()
                    .transfer_aiy_tx_kind(to, amount);
            ...
            AiyClientCommands::Pay {
                ...
                let recipients = recipients
                    ...
                    .collect::<Result<Vec<AiyAddress>, anyhow::Error>>()
                    .map_err(|e| anyhow!("{e}"))?;
                ...
                let tx_kind = client
                    ...
                    "Gas coin is in input coins of Pay transaction, use PayAiy transaction instead!"
                );
            ...
            AiyClientCommands::PayAiy {
                ...
                processing,
                    ...
                    !input_coins.is_empty(),
                    "PayAiy transaction requires a non-empty list of input coins"
                );
                ensure!(
                    !recipients.is_empty(),
                    "PayAiy transaction requires a non-empty list of recipient addresses"
                );
                ensure!(
                    recipients.len() == amounts.len(),
                    format!(
                ...
                let tx_kind = client
                    .transaction_builder()
                    .pay_aiy_tx_kind(recipients, amounts)?;
            ...
            AiyClientCommands::PayAllAiy {
                ...
                processing,
                    ...
                    !input_coins.is_empty(),
                    "PayAllAiy transaction requires a non-empty list of input coins"
                );
                let recipient = context.get_identity_address(Some(recipient))?;
                let signer = context.get_object_owner(&input_coins[0]).await?;
                let client = context.get_client().await?;

                let tx_kind = client.transaction_builder().pay_all_aiy_tx_kind(recipient);
                let gas_payment = client
                    .transaction_builder()
                    .input_refs(&input_coins)
                    .await?;
            ...
            AiyClientCommands::Objects { address } => {
                let address = context.get_identity_address(address)?;
                let client = context.get_client().await?;
                let mut objects: Vec<AiyObjectResponse> = Vec::new();
                let mut cursor = None;
                loop {
                    let response = client
                        .read_api()
                        .get_owned_objects(
                            ...
                            Some(AiyObjectResponseQuery::new_with_options(
                                AiyObjectDataOptions::full_content(),
                            )),
                            cursor,
                            None,
                        )
                ...
                AiyClientCommandResult::Objects(objects)
            }

            AiyClientCommands::NewAddress {
                key_scheme,
                alias,
                derivation_path,
                word_length,
                ...
                AiyClientCommandResult::NewAddress(NewAddressOutput {
                    alias,
                    address,
                    key_scheme: scheme,
            ...
            AiyClientCommands::RemoveAddress { alias_or_address } => {
                let identity = KeyIdentity::from_str(&alias_or_address)
                    .map_err(|e| anyhow!("Invalid address or alias: {}", e))?;
                let address: AiyAddress = context.config.keystore.get_by_identity(&identity)?;

                context.config.keystore.remove(address).await?;

                AiyClientCommandResult::RemoveAddress(RemoveAddressOutput { alias_or_address })
            }

            AiyClientCommands::Gas { address } => {
                let address = context.get_identity_address(address)?;
                let coins = context
                    .gas_objects(address)
                    .await?
                    ...
                    .collect();
                AiyClientCommandResult::Gas(coins)
            }
            AiyClientCommands::Faucet { address, url } => {
                let address = context.get_identity_address(address)?;
                let url = if let Some(url) = url {
                    ...
                    if let Ok(env) = active_env {
                        let network = match env.rpc.as_str() {
                            AIY_DEVNET_URL => "https://faucet.devnet.sui.io/v2/gas",
                            AIY_TESTNET_URL => {
                                bail!("For testnet tokens, please use the Web UI: https://faucet.sui.io/?address={address}");
                            }
                            AIY_LOCAL_NETWORK_URL | AIY_LOCAL_NETWORK_URL_0 => "http://127.0.0.1:9123/v2/gas",
            ...
            AiyClientCommands::ChainIdentifier => {
                let ci = context
                    .get_client()
                    .await?
                    .read_api()
                    ...
                    .await?;
                AiyClientCommandResult::ChainIdentifier(ci)
            }
            AiyClientCommands::SplitCoin {
                coin_id,
                amounts,
                count,
                payment,
            ...
            AiyClientCommands::SerializedTx {
                tx_bytes,
                processing,
            } => {
            ...
            AiyClientCommands::SerializedTxKind {
                tx_bytes,
                payment,
                gas_data,
                processing,
            ...
            AiyClientCommands::Switch { address, env } => {
                let mut addr = None;
                ...
                context.config.save()?;
                AiyClientCommandResult::Switch(SwitchResponse { address: addr, env })
            }
            AiyClientCommands::ActiveAddress => {
                AiyClientCommandResult::ActiveAddress(context.active_address().ok())
            }

            AiyClientCommands::ExecuteSignedTx {
                tx_bytes,
                signatures,
            } => {
                let data = bcs::from_bytes(
                    ...
                    .to_vec()
                    .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse tx bytes, check if it matches the output of aiy client commands with --serialize-unsigned-transaction"))?;
                ...
                AiyClientCommandResult::TransactionBlock(response)
            }
            AiyClientCommands::ExecuteCombinedSignedTx { signed_tx_bytes } => {
                let data: SenderSignedData = bcs::from_bytes(
                    &Base64::try_from(signed_tx_bytes)
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                        .to_vec()
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse SenderSignedData bytes, check if it matches the output of aiy client commands with --serialize-signed-transaction"))?;
                let transaction = Envelope::<SenderSignedData, EmptySignInfo>::new(data);
                let response = context.execute_transaction_may_fail(transaction).await?;
                AiyClientCommandResult::TransactionBlock(response)
            }
            AiyClientCommands::NewEnv {
                alias,
                rpc,
                ws,
                basic_auth,
                ...
                context.config.save()?;
                AiyClientCommandResult::NewEnv(env)
            }
            AiyClientCommands::ActiveEnv => AiyClientCommandResult::ActiveEnv(
                context.get_active_env().ok().map(|e| e.alias.clone()),
            ),
            AiyClientCommands::Envs => AiyClientCommandResult::Envs(
                context.config.envs.clone(),
                context.get_active_env().ok().map(|e| e.alias.clone()),
            ),
            AiyClientCommands::VerifySource {
                package_path,
                mut build_config,
                verify_deps,
                skip_source,
                ...
                AiyClientCommandResult::VerifySource
            }
            AiyClientCommands::PartyTransfer {
                to,
                object_id,
                payment,
                gas_data,
    ...
    pub fn switch_env(config: &mut AiyClientConfig, env: &str) -> Result<(), anyhow::Error> {
        let env = Some(env.into());
        ensure!(config.get_env(&env).is_some(), "Environment config not found for [{env:?}], add new environment config using the `aiy client new-env` command.");
...
pub(crate) async fn upgrade_package(
 ...
 - update the `Move.lock` address using the `aiy manage-package` command to be the same as the `Move.toml`; OR
 - check that your `aiy active-env` {env_alias} corresponds to the chain on which the package is published (i.e., devnet, testnet, mainnet); OR
 - contact the maintainer if this package is a dependency and request resolving the conflict."
    ...
    let resp = read_api
        .get_object_with_options(
            upgrade_capability,
            AiyObjectDataOptions::default().with_bcs().with_owner(),
        )
        .await?;
...
pub(crate) async fn compile_package(
    ...
    if let Some(Some(AiyProtocolConfigValue::U32(min_version))) = protocol_config
        .attributes
    ...
    // Check that the package's Move version is compatible with the chain's
    if let Some(Some(AiyProtocolConfigValue::U32(max_version))) =
    ...
    compiled_package
        ...
        .map_err(|e| AiyError::ModuleBuildFailure {
...
impl Display for AiyClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let mut writer = String::new();
        match self {
            AiyClientCommandResult::Addresses(addresses) => {
            ...
            AiyClientCommandResult::Balance(coins, with_coins) => {
                if coins.is_empty() {
            ...
            AiyClientCommandResult::DynamicFieldQuery(df_refs) => {
            ...
            AiyClientCommandResult::Gas(gas_coins) => {
                ...
                let mut builder = TableBuilder::default();
                builder.set_header(vec!["gasCoinId", "mistBalance (MIST)", "aiyBalance (AIY)"]);
                for coin in &gas_coins {
                    builder.push_record(vec![
                        coin.gas_coin_id.to_string(),
                        coin.mist_balance.to_string(),
                        coin.aiy_balance.to_string(),
                    ]);
                }
                let mut table = builder.build();
            ...
            AiyClientCommandResult::NewAddress(new_address) => {
                let mut builder = TableBuilder::default();
            ...
            AiyClientCommandResult::RemoveAddress(remove_address) => {
                let mut builder = TableBuilder::default();
            ...
            AiyClientCommandResult::Object(object_read) => match object_read.object() {
                Ok(obj) => {
            ...
            AiyClientCommandResult::Objects(object_refs) => {
            ...
            AiyClientCommandResult::RawObject(raw_object_read) => {
                let raw_object = match raw_object_read.object() {
                    Ok(v) => match &v.bcs {
                        Some(AiyRawData::MoveObject(o)) => {
                            format!("{:?}\nNumber of bytes: {}", o.bcs_bytes, o.bcs_bytes.len())
                        }
                        Some(AiyRawData::Package(p)) => {
            ...
            AiyClientCommandResult::ComputeTransactionDigest(tx_data) => {
                writeln!(writer, "{}", tx_data.digest())?;
            }
            AiyClientCommandResult::SerializedUnsignedTransaction(tx_data) => {
                writeln!(
                    writer,
                    "{}",
            ...
            AiyClientCommandResult::SerializedSignedTransaction(sender_signed_tx) => {
                writeln!(
                    writer,
                    "{}",
            ...
            AiyClientCommandResult::SyncClientState => {
                writeln!(writer, "Client state sync complete.")?;
            }
            AiyClientCommandResult::ChainIdentifier(ci) => {
                writeln!(writer, "{}", ci)?;
            }
            AiyClientCommandResult::Switch(response) => {
                write!(writer, "{}", response)?;
            }
            AiyClientCommandResult::ActiveAddress(response) => {
                match response {
                    Some(r) => write!(writer, "{}", r)?,
                    None => write!(writer, "None")?,
                };
            }
            AiyClientCommandResult::ActiveEnv(env) => {
                write!(writer, "{}", env.as_deref().unwrap_or("None"))?;
            }
            AiyClientCommandResult::NewEnv(env) => {
                writeln!(writer, "Added new Aiy env [{}] to config.", env.alias)?;
            }
            AiyClientCommandResult::Envs(envs, active) => {
                let mut builder = TableBuilder::default();
            ...
            AiyClientCommandResult::NoOutput => {}
            AiyClientCommandResult::DryRun(response) => {
                writeln!(f, "{}", Pretty(response))?;
            }
            AiyClientCommandResult::DevInspect(response) => {
                writeln!(f, "{}", Pretty(response))?;
            }
        }
...
impl Debug for AiyClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
...
impl AiyClientCommandResult {
    pub fn objects_response(&self) -> Option<Vec<AiyObjectResponse>> {
        use AiyClientCommandResult::*;
        match self {
    ...
    pub fn tx_block_response(&self) -> Option<&AiyTransactionBlockResponse> {
        use AiyClientCommandResult::*;
        match self {
    ...
    pub async fn prerender_clever_errors(mut self, context: &mut WalletContext) -> Self {
        match &mut self {
            AiyClientCommandResult::DryRun(DryRunTransactionBlockResponse { effects, .. })
            | AiyClientCommandResult::TransactionBlock(AiyTransactionBlockResponse {
            ...
            AiyClientCommandResult::ActiveAddress(_)
            | AiyClientCommandResult::ActiveEnv(_)
            | AiyClientCommandResult::Addresses(_)
            | AiyClientCommandResult::Balance(_, _)
            | AiyClientCommandResult::ComputeTransactionDigest(_)
            | AiyClientCommandResult::ChainIdentifier(_)
            | AiyClientCommandResult::DynamicFieldQuery(_)
            | AiyClientCommandResult::DevInspect(_)
            | AiyClientCommandResult::Envs(_, _)
            | AiyClientCommandResult::Gas(_)
            | AiyClientCommandResult::NewAddress(_)
            | AiyClientCommandResult::NewEnv(_)
            | AiyClientCommandResult::NoOutput
            | AiyClientCommandResult::Object(_)
            | AiyClientCommandResult::Objects(_)
            | AiyClientCommandResult::RemoveAddress(_)
            | AiyClientCommandResult::RawObject(_)
            | AiyClientCommandResult::SerializedSignedTransaction(_)
            | AiyClientCommandResult::SerializedUnsignedTransaction(_)
            | AiyClientCommandResult::Switch(_)
            | AiyClientCommandResult::SyncClientState
            | AiyClientCommandResult::VerifyBytecodeMeter { .. }
            | AiyClientCommandResult::VerifySource => (),
        }
...
pub struct AddressesOutput {
    pub active_address: AiyAddress,
    pub addresses: Vec<(String, AiyAddress)>,
}
...
pub struct ObjectOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub content: Option<AiyParsedData>,
}

impl From<&AiyObjectData> for ObjectOutput {
    fn from(obj: &AiyObjectData) -> Self {
...
impl From<&GasCoin> for GasCoinOutput {
    fn from(gas_coin: &GasCoin) -> Self {
        Self {
            ...
            aiy_balance: format_balance(gas_coin.value() as u128, 9, 2, None),
        }
    }
}
...
impl ObjectsOutput {
    fn from(obj: AiyObjectResponse) -> Result<Self, anyhow::Error> {
        let obj = obj.into_object()?;
        // this replicates the object type display as in the aiy explorer
        let object_type = match obj.type_ {
            Some(aiy_types::base_types::ObjectType::Struct(x)) => {
            ...
            Some(aiy_types::base_types::ObjectType::Package) => "Package".to_string(),
            None => "unknown".to_string(),
        };
        Ok(Self {
            object_id: obj.object_id,
    ...
    fn from_vec(objs: Vec<AiyObjectResponse>) -> Result<Vec<Self>, anyhow::Error> {
        objs.into_iter()
...
#[derive(Serialize)]
#[serde(untagged)]
pub enum AiyClientCommandResult {
    ActiveAddress(Option<AiyAddress>),
    ActiveEnv(Option<String>),
    Addresses(AddressesOutput),
    Balance(Vec<(Option<AiyCoinMetadata>, Vec<Coin>)>, bool),
    ChainIdentifier(String),
    ComputeTransactionDigest(TransactionData),
    DynamicFieldQuery(DynamicFieldPage),
    DryRun(DryRunTransactionBlockResponse),
    DevInspect(DevInspectResults),
    Envs(Vec<AiyEnv>, Option<String>),
    Gas(Vec<GasCoin>),
    NewAddress(NewAddressOutput),
    NewEnv(AiyEnv),
    NoOutput,
    Object(AiyObjectResponse),
    Objects(Vec<AiyObjectResponse>),
    RawObject(AiyObjectResponse),
    ...
    SyncClientState,
    TransactionBlock(AiyTransactionBlockResponse),
    VerifyBytecodeMeter {
        success: bool,
    ...
    match resp.status() {
        StatusCode::ACCEPTED | StatusCode::CREATED | StatusCode::OK => {
            ...
            if let Some(err) = faucet_resp.error {
                ...
                println!("Request successful. It can take up to 1 minute to get the coin. Run aiy client gas to check your gas coins.");
            }
        }
...
fn pretty_print_balance(
    coins_by_type: &Vec<(Option<AiyCoinMetadata>, Vec<Coin>)>,
...
pub async fn execute_dry_run(
    ...
    let resp = AiyClientCommandResult::DryRun(response)
...
pub async fn estimate_gas_budget(
    ...
    if let Ok(AiyClientCommandResult::DryRun(dry_run)) = dry_run {
...
pub async fn max_gas_budget(client: &AiyClient) -> Result<u64, anyhow::Error> {
    let cfg = client.read_api().get_protocol_config(None).await?;
    Ok(match cfg.attributes.get("max_tx_gas") {
        Some(Some(aiy_json_rpc_types::AiyProtocolConfigValue::U64(y))) => *y,
        _ => bail!(
...
pub(crate) async fn dry_run_or_execute_or_serialize(
    ...
    if serialize_unsigned_transaction {
        Ok(AiyClientCommandResult::SerializedUnsignedTransaction(
            tx_data,
        ))
    } else if tx_digest {
        Ok(AiyClientCommandResult::ComputeTransactionDigest(tx_data))
    } else {
        let mut signatures = vec![context
            .config
            .keystore
            .sign_secure(&signer, &tx_data, Intent::aiy_transaction())
            .await?
            .into()];
            ...
            if gas_sponsor != signer {
                signatures.push(
                    context
                        .config
                        .keystore
                        .sign_secure(&gas_sponsor, &tx_data, Intent::aiy_transaction())
                        .await?
                        .into(),
                );
            }
        ...
        if serialize_signed_transaction {
            Ok(AiyClientCommandResult::SerializedSignedTransaction(
                sender_signed_data,
            ))
        } else {
            ...
            let effects = response.effects.as_ref().ok_or_else(|| {
                anyhow!("Effects from AiyTransactionBlockResult should not be empty")
            })?;
            if let AiyExecutionStatus::Failure { error } = effects.status() {
                return Err(anyhow!(
                    "Error executing transaction '{}': {error}",
                    response.digest
                ));
            }
            Ok(AiyClientCommandResult::TransactionBlock(response))
        }
    }
}
...
async fn execute_dev_inspect(
    ...
    gas_sponsor: Option<AiyAddress>,
    skip_checks: Option<bool>,
) -> Result<AiyClientCommandResult, anyhow::Error> {
    let client = context.get_client().await?;
    let gas_budget = gas_budget.map(aiy_serde::BigInt::from);
    ...
    let dev_inspect_result = client
        ...
        .dev_inspect_transaction_block(
            signer,
            tx_kind,
            Some(aiy_serde::BigInt::from(gas_price)),
            None,
            Some(dev_inspect_args),
        )
        .await?;
    Ok(AiyClientCommandResult::DevInspect(dev_inspect_result))
}

pub(crate) async fn prerender_clever_errors(
    effects: &mut AiyTransactionBlockEffects,
    read_api: &ReadApi,
) {
    let AiyTransactionBlockEffects::V1(effects) = effects;
    if let AiyExecutionStatus::Failure { error } = &mut effects.status {
...
fn to_package(o: AiyObjectResponse) -> anyhow::Result<MovePackage> {
    let id = o.object_id()?;
    let Some(AiyRawData::Package(p)) = o.into_object()?.bcs else {
...
async fn fetch_move_packages(
    ...
    let objects = read_api
        .multi_get_object_with_options(package_ids, AiyObjectDataOptions::bcs_lossless())
        .await?;
