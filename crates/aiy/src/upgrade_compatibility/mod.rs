...
use aiy_types::{base_types::ObjectID, execution_config_utils::to_binary_config};
...
pub(crate) async fn check_compatibility(
    ...
    let existing_obj_read = read_api
        .get_object_with_options(package_id, AiyObjectDataOptions::new().with_bcs())
        .await
    ...
    let existing_package = match existing_obj {
        ...
        AiyRawData::MoveObject(_) => Err(anyhow!("Object found when package expected")),
    }?;
