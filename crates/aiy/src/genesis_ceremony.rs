...
use aiy_config::{genesis::UnsignedGenesis, AIY_GENESIS_FILENAME};
use aiy_genesis_builder::Builder;
use aiy_types::multiaddr::Multiaddr;
use aiy_types::{
    base_types::AiyAddress,
    committee::ProtocolVersion,
    crypto::{
        generate_proof_of_possession, AuthorityKeyPair, KeypairTraits, NetworkKeyPair, AiyKeyPair,
    },
...
pub fn run(cmd: Ceremony) -> Result<()> {
    ...
    match cmd.command {
        ...
        CeremonyCommand::AddValidator {
            ...
            let account_keypair: AiyKeyPair = read_keypair_from_file(account_key_file)?;
            ...
            builder = builder.add_validator(
                aiy_genesis_builder::validator_info::ValidatorInfo {
                    name,
                    protocol_key: keypair.public().into(),
                    worker_key: worker_keypair.public().clone(),
                    account_address: AiyAddress::from(&account_keypair.public()),
                    network_key: network_keypair.public().clone(),
                    gas_price: aiy_config::node::DEFAULT_VALIDATOR_GAS_PRICE,
        ...
        CeremonyCommand::Finalize => {
            ...
            genesis.save(dir.join(AIY_GENESIS_FILENAME))?;

            println!("Successfully built {AIY_GENESIS_FILENAME}");
            println!(
                "{AIY_GENESIS_FILENAME} blake2b-256: {}",
...
fn check_protocol_version(builder: &Builder, protocol_version: ProtocolVersion) -> Result<()> {
    ...
    // (e.g. using a `aiy` binary built at the wrong commit).
...
mod test {
    ...
    use aiy_genesis_builder::validator_info::ValidatorInfo;
    use aiy_keys::keypair_file::{write_authority_keypair_to_file, write_keypair_to_file};
    use aiy_macros::nondeterministic;
    use aiy_types::crypto::{get_key_pair_from_rng, AccountKeyPair, AuthorityKeyPair, AiyKeyPair};
    ...
    fn ceremony() -> Result<()> {
        ...
        let validators = (0..10)
            .map(|i| {
                ...
                write_keypair_to_file(&AiyKeyPair::Ed25519(worker_keypair), &worker_key_file)
                    .unwrap();

                let network_key_file = dir.path().join(format!("{}-1.key", info.name));
                write_keypair_to_file(&AiyKeyPair::Ed25519(network_keypair), &network_key_file)
                    .unwrap();

                let account_key_file = dir.path().join(format!("{}-2.key", info.name));
                write_keypair_to_file(&AiyKeyPair::Ed25519(account_keypair), &account_key_file)
                    .unwrap();
