...
impl<'a, I: Iterator<Item = &'a str>> ProgramParser<'a, I> {
    ...
    fn parse_mvr_address(
        ...
        match prefix {
            ...
            Some(sp!(_, prefix)) => {
                ...
                match self.peek() {
                    sp!(sp, L(T::Ident, s)) => {
                        if s != "aiy" {
                            error!(sp => help: { "Expected 'aiy' extension" },
                    ...
                    sp!(sp, l) => {
                        error!(
                            sp,
                            "Expected a valid MVR domain extension ('.aiy') but got {}", l
                        )
                    }
                }
            }
        ...
        self.expect(T::ForwardSlash).map_err(|e| {
            err!(e.span => help: {
                "MVR packages must contain a slash and be in the format '(@<domain>|<domain>.aiy)/<package>[/<version>]'"
...
mod tests {
    ...
    fn test_parse_types() {
        let inputs = vec![
            ...
            "aiy::object::ID",
            "0x2::object::UID",
            "3::staking_pool::StakedAiy",
            // Generic types
            "0x2::coin::Coin<2::aiy::AIY>",
            "aiy::table::Table<aiy::object::ID, vector<0x1::option::Option<u32>>>",
        ];
        let mut parsed = Vec::new();
        for input in inputs {
            let mut x = shlex::split(input).unwrap();
    ...
    fn test_parse_commands() {
        let inputs = vec![
            ...
            // Move Call
            "--move-call 0x3::aiy_system::request_add_stake system coins.0 validator",
