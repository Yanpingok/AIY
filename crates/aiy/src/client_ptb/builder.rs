...
use aiy_json::{is_receiving_argument, primitive_type};
use aiy_json_rpc_types::{AiyObjectData, AiyObjectDataOptions, AiyRawData};
...
use aiy_types::{
    ...
    Identifier, TypeTag, AIY_FRAMEWORK_PACKAGE_ID,
};
...
impl<'a> PTBBuilder<'a> {
    ...
    fn declare_possible_address_binding(&mut self, ident: String, possible_addr: &Spanned<PTBArg>) {
        match possible_addr.value {
            ...
            // If we encounter a dotted string e.g., "foo.0" or "aiy.io" or something like that
    ...
    /// Fetch the `AiyObjectData` for an object ID -- this is used for object resolution.
    async fn get_object(&self, object_id: ObjectID, obj_loc: Span) -> PTBResult<AiyObjectData> {
        let res = self
            .reader
            .get_object_with_options(
                object_id,
                AiyObjectDataOptions::new().with_type().with_owner(),
            )
            .await
...
pub fn is_mvr_name(name: &str) -> bool {
    name.contains('/') && (name.contains("@") || name.contains(".aiy"))
}
...
/// ) or the known package names (e.g., aiy, deepbook, etc) by calling `resolve_address`.
