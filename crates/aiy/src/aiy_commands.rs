...
use crate::client_commands::{
    implicit_deps_for_protocol_version, pkg_tree_shake, AiyClientCommands, USER_AGENT,
};
use crate::fire_drill::{run_fire_drill, FireDrill};
use crate::genesis_ceremony::{run, Ceremony};
use crate::keytool::KeyToolCommand;
use crate::trace_analysis_commands::AnalyzeTraceCommand;
use crate::validator_commands::AiyValidatorCommand;
...
use std::sync::Arc;
use std::{fs, io};
use aiy_bridge::config::BridgeCommitteeConfig;
use aiy_bridge::metrics::BridgeMetrics;
use aiy_bridge::aiy_client::AiyBridgeClient;
use aiy_bridge::aiy_transaction_builder::build_committee_register_transaction;
use aiy_config::node::Genesis;
use aiy_config::p2p::SeedPeer;
use aiy_config::{
    genesis_blob_exists, aiy_config_dir, Config, PersistedConfig, FULL_NODE_DB_PATH,
    AIY_CLIENT_CONFIG, AIY_FULLNODE_CONFIG, AIY_NETWORK_CONFIG,
};
use aiy_config::{
    AIY_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME, AIY_GENESIS_FILENAME, AIY_KEYSTORE_FILENAME,
};
use aiy_faucet::{create_wallet_context, start_faucet, AppState, FaucetConfig, LocalFaucet};
use aiy_indexer::test_utils::{
    start_indexer_jsonrpc_for_testing, start_indexer_writer_for_testing,
};
use aiy_json_rpc_types::{AiyObjectDataOptions, AiyRawData};
use aiy_move::summary::PackageSummaryMetadata;
use aiy_sdk::apis::ReadApi;
use aiy_sdk::AiyClient;
use aiy_types::move_package::MovePackage;
...
use serde_json::json;
use aiy_keys::key_derive::generate_new_key;
use aiy_keys::keypair_file::read_key;
use aiy_keys::keystore::{AccountKeystore, FileBasedKeystore, Keystore};
use aiy_move::manage_package::resolve_lock_file_path;
use aiy_move::{self, execute_move_command};
use aiy_move_build::{
    check_conflicting_addresses, check_invalid_dependencies, check_unpublished_dependencies,
    implicit_deps, BuildConfig as AiyBuildConfig, AiyPackageHooks,
};
use aiy_package_management::system_package_versions::latest_system_packages;
use aiy_protocol_config::Chain;
use aiy_replay_2 as SR2;
use aiy_sdk::aiy_client_config::{AiyClientConfig, AiyEnv};
use aiy_sdk::wallet_context::WalletContext;
use aiy_swarm::memory::Swarm;
use aiy_swarm_config::genesis_config::GenesisConfig;
use aiy_swarm_config::network_config::NetworkConfig;
use aiy_swarm_config::network_config_builder::ConfigBuilder;
use aiy_swarm_config::node_config_builder::FullnodeConfigBuilder;
use aiy_types::base_types::{ObjectID, AiyAddress};
use aiy_types::crypto::{SignatureScheme, AiyKeyPair, ToFromBytes};
...
const DEFAULT_FAUCET_MIST_AMOUNT: u64 = 200_000_000_000; // 200 AIY
...
pub struct IndexerArgs {
    ...
    /// DB name for the Indexer Postgres DB. Default DB name is aiy_indexer.
    #[clap(long, default_value = "aiy_indexer")]
    pg_db_name: String,
...
impl IndexerArgs {
    pub fn for_testing() -> Self {
        Self {
            ...
            pg_db_name: "aiy_indexer".to_string(),
...
pub struct AiyEnvConfig {
    /// Sets the file storing the state of our user accounts (an empty one will be created if missing)
    #[clap(long = "client.config")]
    config: Option<PathBuf>,
    /// The Aiy environment to use. This must be present in the current config file.
...
pub enum AiyCommand {
    ...
    ///
    /// By default, aiy start will start a local network from the genesis blob that exists in
    /// the Aiy config default dir or in the config_dir that was passed. If the default directory
    ...
    /// follows:
    /// - AIY_PROTOCOL_CONFIG_OVERRIDE_ENABLE=1
    /// - Then, to configure an override, use the prefix `AIY_PROTOCOL_CONFIG_OVERRIDE_`
    ///   along with the parameter name. For example, to increase the interval between
    ///   checkpoint creation to >1/s, you might set:
    ///   AIY_PROTOCOL_CONFIG_OVERRIDE_min_checkpoint_interval_ms=1000
    ///
    ...
    Start {
        ...
        /// aiy genesis -f --with-faucet generates a genesis config that can be used to start this
        /// proces. Use with caution as the `-f` flag will overwrite the existing config directory.
        /// We can use any config dir that is generated by the `aiy genesis`.
        ...
        /// run this command.
        ///
        /// To run with persisted state, do not pass this flag and use the `aiy genesis` command
    ...
    /// Bootstrap and initialize a new aiy network
    #[clap(name = "genesis")]
    Genesis {
        ...
        #[clap(
            ...
            num_args(1..),
            value_delimiter = ',',
            help = "A list of ip addresses to generate a genesis aiytable for benchmarks"
        )]
        benchmark_ips: Option<Vec<String>>,
        #[clap(
            long,
            help = "Creates an extra faucet configuration for aiy persisted runs."
        )]
        with_faucet: bool,
    ...
    /// Client for interacting with the Aiy network.
    #[clap(name = "client")]
    Client {
        #[clap(flatten)]
        config: AiyEnvConfig,
        #[clap(subcommand)]
        cmd: Option<AiyClientCommands>,
        /// Return command outputs in json format.
        #[clap(long, global = true)]
        json: bool,
    ...
    Validator {
        ...
        config: Option<PathBuf>,
        #[clap(subcommand)]
        cmd: Option<AiyValidatorCommand>,
        /// Return command outputs in json format.
        #[clap(long, global = true)]
        json: bool,
        #[clap(short = 'y', long = "yes")]
    ...
    /// Invoke Aiy's move-analyzer via CLI
    ...
    pub async fn execute(self) -> Result<(), anyhow::Error> {
        ...
        match self {
            AiyCommand::Network {
                ...
                let config_path = config.unwrap_or(aiy_config_dir()?.join(AIY_NETWORK_CONFIG));
                let config: NetworkConfig = PersistedConfig::read(&config_path).map_err(|err| {
                    err.context(format!(
                        "Cannot open Aiy network config file at {:?}",
                        config_path
                    ))
                })?;
            ...
            AiyCommand::GenesisCeremony(cmd) => run(cmd),
            AiyCommand::KeyTool {
                keystore_path,
                json,
                cmd,
                ...
                let keystore_path =
                    keystore_path.unwrap_or(aiy_config_dir()?.join(AIY_KEYSTORE_FILENAME));
                let mut keystore =
            ...
            AiyCommand::Client {
                ...
                let config_path = config
                    .config
                    .unwrap_or(aiy_config_dir()?.join(AIY_CLIENT_CONFIG));
                ...
                if let Some(cmd) = cmd {
                    ...
                    // Print help
                    let mut app: Command = AiyCommand::command();
                    app.build();
                    app.find_subcommand_mut("client").unwrap().print_help()?;
                }
                Ok(())
            }
            AiyCommand::Validator {
                config,
                cmd,
                json,
                accept_defaults,
            } => {
                let config_path = config.unwrap_or(aiy_config_dir()?.join(AIY_CLIENT_CONFIG));
                prompt_if_no_config(&config_path, accept_defaults).await?;
                let mut context = WalletContext::new(&config_path)?;
                if let Some(cmd) = cmd {
                    if let Ok(client) = context.get_client().await {
            ...
            AiyCommand::Move {
                ...
                match cmd {
                    aiy_move::Command::Summary(mut s) if s.package_id.is_some() => {
                        let (_, client) = get_chain_id_and_client(
                            client_config,
                            "aiy move summary --package-id <object_id>",
                        )
                        .await?;
                        let Some(client) = client else {
                            bail!("`aiy move summary --package-id <object_id>` requires a configured network");
                        };
                        ...
                        if !s.summary.bytecode {
                            eprintln!("{}",
                                "[warning] `aiy move summary --package-id <object_id>` only supports bytecode summaries. \
                        ...
                        execute_move_command(
                            Some(path),
                            build_config,
                            aiy_move::Command::Summary(s),
                            Some(aiy_move::CommandMeta::Summary(package_metadata)),
                        )?;
                        return Ok(());
                    }
                    aiy_move::Command::Build(build) if build.dump_bytecode_as_base64 => {
                        // `aiy move build` does not ordinarily require a network connection.
                        ...
                        let (chain_id, client) = if build.ignore_chain {
                            ...
                            get_chain_id_and_client(
                                client_config,
                                "aiy move build --dump-bytecode-as-base64",
                            )
                            .await?
                        };
                ...
                if client_config.env.is_some() && matches!(cmd, aiy_move::Command::Build(_)) {
                    let (chain_id, _) =
                        get_chain_id_and_client(client_config, "aiy move build").await?;

                    let aiy_move::Command::Build(build_config) = &mut cmd else {
            ...
            AiyCommand::BridgeInitialize {
                network_config,
                client_config,
                bridge_committee_config_path,
            } => {
                // Load the config of the Aiy authority.
                let network_config_path = network_config
                    .clone()
                    .unwrap_or(aiy_config_dir()?.join(AIY_NETWORK_CONFIG));
                ...
                let config_path =
                    client_config.unwrap_or(aiy_config_dir()?.join(AIY_CLIENT_CONFIG));
                let mut context = WalletContext::new(&config_path)?;
                if let Ok(client) = context.get_client().await {
                    if let Err(e) = client.check_api_version() {
                        eprintln!("{}", format!("[warning] {e}").yellow().bold());
                ...
                let aiy_bridge_client = AiyBridgeClient::new(rpc_url, bridge_metrics).await?;
                let bridge_arg = aiy_bridge_client
                ...
                {
                    let account_kp = node_config.account_key_pair.keypair();
                    let aiy_address = AiyAddress::from(&account_kp.public());
            ...
            AiyCommand::FireDrill { fire_drill } => run_fire_drill(fire_drill).await,
            AiyCommand::Analyzer => {
                analyzer::run(implicit_deps(latest_system_packages()));
                Ok(())
            }
            AiyCommand::AnalyzeTrace {
                path,
                output_dir,
                command,
            } => command.execute(path, output_dir).await,
            AiyCommand::ReplayTransaction {
                config,
                replay_config,
            } => {
                let config_path = config
...
async fn start(
    ...
    if epoch_duration_ms.is_some() && genesis_blob_exists(config.clone()) && !force_regenesis {
        bail!(
            ...
            there is no genesis configuration in the default Aiy configuration folder or the given \
    ...
    let config_dir = if force_regenesis {
        ...
        // overriding the network.yaml found in the aiy config directry. Otherwise it is treated as
        // the aiy config directory for backwards compatibility with `aiy-test-validator`.
        let (network_config_path, aiy_config_path) = match config {
            Some(config)
                if config.is_file()
                    && config
                        .extension()
                ...
                if committee_size.is_some() {
                    eprintln!(
                        ...
                        "[warning] The committee-size arg wil be ignored as a network \
                            ...
                            with the desired committee size. See `aiy genesis --help` for more \
                            information."
                            .yellow()
                            .bold()
                    );
                }
                (config, aiy_config_dir()?)
            }
            ...
            Some(config) => {
                ...
                (config.join(AIY_NETWORK_CONFIG), config)
            }

            None => {
                let aiy_config = aiy_config_dir()?;
                let network_config = aiy_config.join(AIY_NETWORK_CONFIG);
                ...
                if !network_config.exists() {
                    ...
                    .map_err(|_| {
                        anyhow!(
                            "Cannot run genesis with non-empty Aiy config directory: {}.\n\n\
                                ...
                                genesis, use aiy genesis --help to see how to generate a new \
                                genesis.",
                            aiy_config.display(),
                        )
                    })?;
    ...
    let mut fullnode_url = aiy_config::node::default_json_rpc_address();
    ...
    if let Some(input) = with_faucet {
        ...
        if force_regenesis {
            ...
            let keystore_path = config_dir.join(AIY_KEYSTORE_FILENAME);
            let mut keystore =
                Keystore::from(FileBasedKeystore::load_or_create(&keystore_path).unwrap());
            let address: AiyAddress = kp.public().into();
            keystore
                .import(None, AiyKeyPair::Ed25519(kp))
                .await
                .unwrap();
            AiyClientConfig {
                keystore,
            ...
            .persisted(config_dir.join(AIY_CLIENT_CONFIG).as_path())
            .save()
            .unwrap();
        }
...
async fn genesis(
    ...
    let aiy_config_dir = &match working_dir {
        // if a directory is specified, it must exist (it
        // will not be created)
        Some(v) => v,
        // create default Aiy config dir if not specified
        // on the command line and if it does not exist
        // yet
        None => {
            let config_path = aiy_config_dir()?;
            fs::create_dir_all(&config_path)?;
            config_path
        }
    };

    // if Aiy config dir is not empty then either clean it
    // up (if --force/-f option was specified or report an
    // error
    let dir = aiy_config_dir.read_dir().map_err(|err| {
        anyhow!(err).context(format!("Cannot open Aiy config dir {:?}", aiy_config_dir))
    })?;
    let files = dir.collect::<Result<Vec<_>, _>>()?;

    let client_path = aiy_config_dir.join(AIY_CLIENT_CONFIG);
    let keystore_path = aiy_config_dir.join(AIY_KEYSTORE_FILENAME);
    ...
    if write_config.is_none() && !files.is_empty() {
        if force {
            ...
            let is_compatible = FileBasedKeystore::load_or_create(&keystore_path).is_ok()
                && PersistedConfig::<AiyClientConfig>::read(&client_path).is_ok();
            ...
            if is_compatible {
                ...
                fs::remove_dir_all(aiy_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot remove Aiy config dir {:?}", aiy_config_dir))
                })?;
                fs::create_dir(aiy_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot create Aiy config dir {:?}", aiy_config_dir))
                })?;
            }
        } else if files.len() != 2 || !client_path.exists() || !keystore_path.exists() {
            bail!("Cannot run genesis with non-empty Aiy config directory {}, please use the --force/-f option to remove the existing configuration", aiy_config_dir.to_str().unwrap());
        }
    }

    let network_path = aiy_config_dir.join(AIY_NETWORK_CONFIG);
    let genesis_path = aiy_config_dir.join(AIY_GENESIS_FILENAME);
    ...
    let mut genesis_conf = match from_config {
        ...
        None => {
            if let Some(ips) = benchmark_ips {
                ...
                let path = aiy_config_dir.join(AIY_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME);
    ...
    let builder = ConfigBuilder::new(aiy_config_dir);
    ...
    for key in &network_config.account_keys {
        keystore
            .import(None, AiyKeyPair::Ed25519(key.copy()))
            .await?;
    }
    ...
    for validator in &mut network_config.validator_configs {
        validator.genesis = aiy_config::node::Genesis::new_from_file(&genesis_path);
    }
    ...
    let fullnode_config = FullnodeConfigBuilder::new()
        ...
        .with_rpc_addr(aiy_config::node::default_json_rpc_address())
        .build(&mut OsRng, &network_config);

    fullnode_config.save(aiy_config_dir.join(AIY_FULLNODE_CONFIG))?;
    let mut ssfn_nodes = vec![];
    if let Some(ssfn_info) = ssfn_info {
        for (i, ssfn) in ssfn_info.into_iter().enumerate() {
            let path =
                aiy_config_dir.join(aiy_config::ssfn_config_file(ssfn.p2p_address.clone(), i));
            ...
            let ssfn_config = FullnodeConfigBuilder::new()
                ...
                .with_db_path(PathBuf::from("/opt/aiy/db/authorities_db/full_node_db"))
                ...
                .with_genesis(Genesis::new_from_file("/opt/aiy/config/genesis.blob"))
                .build(&mut OsRng, &network_config);
        ...
        for (i, mut validator) in network_config
            ...
            .enumerate()
        {
            let path = aiy_config_dir.join(aiy_config::validator_config_file(
    ...
    let mut client_config = if client_path.exists() {
        ...
        AiyClientConfig::new(keystore.into())
    };
    ...
    client_config.add_env(AiyEnv::devnet());
...
async fn prompt_if_no_config(
    ...
    if !wallet_conf_path.exists() {
        let env = match std::env::var_os("AIY_CONFIG_WITH_RPC_URL") {
            Some(v) => Some(AiyEnv {
            ...
            None => {
                if accept_defaults {
                    ...
                    print!(
                        "Config file [{:?}] doesn't exist, do you want to connect to a Aiy Full node server [y/N]?",
                        wallet_conf_path
                    );
                }
                if accept_defaults
...
async fn get_chain_id_and_client(
    client_config: AiyEnvConfig,
    command_err_string: &str,
) -> anyhow::Result<(Option<String>, Option<AiyClient>)> {
...
async fn resolve_package(reader: &ReadApi, package_id: ObjectID) -> anyhow::Result<MovePackage> {
    let object = reader
        .get_object_with_options(package_id, AiyObjectDataOptions::bcs_lossless())
        .await?
        .into_object()?;

    let Some(AiyRawData::Package(package)) = object.bcs else {
