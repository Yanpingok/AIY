...
use aiy_types::crypto::{PublicKey, AiyKeyPair};
...
pub async fn perform_zk_login_test_tx(
    ...
    let skp1 = AiyKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([1; 32])));
    ...
    let sender = if test_multisig {
        ...
        AiyAddress::from(&multisig_pk)
    } else {
        println!("Use single zklogin address as sender");
        AiyAddress::try_from_unpadded(&zk_login_inputs)?
    };
    println!("Sender: {:?}", sender);

    // Request some coin from faucet and build a test transaction.
    let aiy = AiyClientBuilder::default().build(fullnode_url).await?;
    ...
    if response.data.len() != 2 {
        panic!("Faucet did not work correctly and the provided Aiy address has no coins")
    }
    ...
    let txb_res = aiy
        ...
        .transfer_object(
            ...
            Some(gas_coin),
            5000000,
            AiyAddress::ZERO, // as a demo, send to a dummy address
        )
        .await?;
    println!(
