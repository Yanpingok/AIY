...
//!
//! Example usage:
//! aiy fire-drill metadata-rotation \
//! --aiy-node-config-path validator.yaml \
...
use aiy_config::node::{AuthorityKeyPairWithPath, KeyPairWithPath};
use aiy_config::{local_ip_utils, Config, NodeConfig, PersistedConfig};
use aiy_json_rpc_types::{AiyExecutionStatus, AiyTransactionBlockResponseOptions};
use aiy_keys::keypair_file::read_keypair_from_file;
use aiy_sdk::{rpc_types::AiyTransactionBlockEffectsAPI, AiyClient, AiyClientBuilder};
use aiy_types::base_types::{ObjectRef, AiyAddress};
use aiy_types::crypto::{generate_proof_of_possession, get_key_pair, AiyKeyPair};
use aiy_types::multiaddr::{Multiaddr, Protocol};
use aiy_types::transaction::{
    CallArg, Transaction, TransactionData, TEST_ONLY_GAS_UNIT_FOR_GENERIC,
};
use aiy_types::{committee::EpochId, crypto::get_authority_key_pair, AIY_SYSTEM_PACKAGE_ID};
use tracing::info;
...
pub struct MetadataRotation {
    /// Path to aiy node config.
    #[clap(long = "aiy-node-config-path")]
    aiy_node_config_path: PathBuf,
...
async fn run_metadata_rotation(metadata_rotation: MetadataRotation) -> anyhow::Result<()> {
    let MetadataRotation {
        aiy_node_config_path,
        account_key_path,
        fullnode_rpc_url,
    } = metadata_rotation;
    let account_key = read_keypair_from_file(&account_key_path)?;
    let config: NodeConfig = PersistedConfig::read(&aiy_node_config_path).map_err(|err| {
        err.context(format!(
            "Cannot open Aiy Node Config file at {:?}",
            aiy_node_config_path
        ))
    })?;

    let aiy_client = AiyClientBuilder::default().build(fullnode_rpc_url).await?;
    let aiy_address = AiyAddress::from(&account_key.public());
    let starting_epoch = current_epoch(&aiy_client).await?;
    info!("Running Metadata Rotation fire drill for validator address {aiy_address} in epoch {starting_epoch}.");

    // Prepare new metadata for next epoch
    let new_config_path =
        update_next_epoch_metadata(&aiy_node_config_path, &config, &aiy_client, &account_key)
            .await?;

    let current_epoch = current_epoch(&aiy_client).await?;
    if current_epoch > starting_epoch {
        bail!("Epoch already advanced to {current_epoch}");
    }
    let target_epoch = starting_epoch + 1;
    wait_for_next_epoch(&aiy_client, target_epoch).await?;
    info!("Just advanced to epoch {target_epoch}");

    // Replace new config
    std::fs::rename(new_config_path, aiy_node_config_path)?;
    info!("Updated Aiy Node config.");
...
pub async fn get_gas_obj_ref(
    ...
    let coins = aiy_client
        .coin_read_api()
        .get_coins(aiy_address, Some("0x2::aiy::AIY".into()), None, None)
        .await?
        .data;
    let gas_obj = coins.iter().find(|c| c.balance >= minimal_gas_balance);
    if gas_obj.is_none() {
        bail!("Validator doesn't have enough Aiy coins to cover transaction fees.");
    }
    Ok(gas_obj.unwrap().object_ref())
}

async fn update_next_epoch_metadata(
    aiy_node_config_path: &Path,
    config: &NodeConfig,
    aiy_client: &AiyClient,
    account_key: &AiyKeyPair,
) -> anyhow::Result<PathBuf> {
    // Save backup config just in case
    let mut backup_config_path = aiy_node_config_path.to_path_buf();
    ...
    let pop = generate_proof_of_possession(&new_protocol_key_pair, aiy_address);
    ...
    new_config.network_key_pair = KeyPairWithPath::new(AiyKeyPair::Ed25519(new_network_key_pair));

    // worker key
    let new_worker_key_pair: Ed25519KeyPair = get_key_pair().1;
    let new_worker_key_pair_copy = new_worker_key_pair.copy();
    new_config.worker_key_pair = KeyPairWithPath::new(AiyKeyPair::Ed25519(new_worker_key_pair));

    let validators = aiy_client
        .governance_api()
    ...
    let self_validator = validators
        .iter()
        .find(|v| v.aiy_address == aiy_address)
        .unwrap();
    ...
    // Save new config
    let mut new_config_path = aiy_node_config_path.to_path_buf();
    new_config_path.pop();
    new_config_path.push(
        String::from(aiy_node_config_path.file_name().unwrap().to_str().unwrap()) + ".next_epoch",
    );
...
async fn update_metadata_on_chain(
    ...
    let aiy_address = AiyAddress::from(&account_key.public());
    let gas_obj_ref = get_gas_obj_ref(aiy_address, aiy_client, 10000 * 100).await?;
    let rgp = aiy_client
        .governance_api()
    ...
    .unwrap();
    execute_tx(account_key, aiy_client, tx_data, function).await?;
...
async fn execute_tx(
    ...
    let resp = aiy_client
        ...
        .execute_transaction_block(
            tx,
            AiyTransactionBlockResponseOptions::full_content(),
            Some(aiy_types::quorum_driver_types::ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await
        .unwrap();
    if *resp.effects.unwrap().status() != AiyExecutionStatus::Success {
...
async fn wait_for_next_epoch(aiy_client: &AiyClient, target_epoch: EpochId) -> anyhow::Result<()> {
    loop {
        let epoch_id = current_epoch(aiy_client).await?;
...
async fn current_epoch(aiy_client: &AiyClient) -> anyhow::Result<EpochId> {
    Ok(aiy_client.read_api().get_committee_info(None).await?.epoch)
}
