...
use aiy_config::AIY_CLIENT_CONFIG;
...
/// of [path], with the `aiy` binary on the path.
///
...
async fn test_shell_snapshot(path: &Path) -> datatest_stable::Result<()> {
    ...
    let aiy_package_dir_src = get_aiy_package_dir();
    ...
    shell
        .env(
            "PATH",
            format!("{}:{}", get_aiy_bin_path(), std::env::var("PATH")?),
        )
    ...
    if let Some(ref cluster) = cluster {
        shell.env("CONFIG", cluster.swarm.dir().join(AIY_CLIENT_CONFIG));
    }
...
/// return the path to the `aiy` binary that is currently under test
fn get_aiy_bin_path() -> String {
    get_cargo_bin("aiy")
        .parent()
        .unwrap()
        .to_str()
...
/// Return the package dir for the Aiy framework packages which may be used in some shell tests.
fn get_aiy_package_dir() -> PathBuf {
    let mut path = PathBuf::from(std::env!("CARGO_MANIFEST_DIR"));
    path.push("../aiy-framework/packages");
    path
}
