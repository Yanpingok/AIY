...
pub struct BridgeOrchestratorTables {
    ...
    pub(crate) aiy_syncer_cursors: DBMap<Identifier, EventID>,
...
impl BridgeOrchestratorTables {
    ...
    pub(crate) fn update_aiy_event_cursor(
        &self,
        module: Identifier,
        cursor: EventID,
    ) -> BridgeResult<()> {
        let mut batch = self.aiy_syncer_cursors.batch();

        batch
            .insert_batch(&self.aiy_syncer_cursors, [(module, cursor)])
            .map_err(|e| {
                BridgeError::StorageError(format!(
                    "Coudln't insert into aiy_syncer_cursors: {:?}",
                    e
                ))
            })?;
        batch
    ...
    pub fn get_aiy_event_cursors(
        &self,
        identifiers: &[Identifier],
    ) -> BridgeResult<Vec<Option<EventID>>> {
        self.aiy_syncer_cursors.multi_get(identifiers).map_err(|e| {
            BridgeError::StorageError(format!("Couldn't get aiy_syncer_cursors: {:?}", e))
        })
    }
...
mod tests {
    ...
    async fn test_bridge_storage_basic() {
        ...
        // update aiy event cursor
        let aiy_module = Identifier::from_str("test").unwrap();
        let aiy_cursor = EventID {
            tx_digest: TransactionDigest::random(),
            event_seq: 1,
        };
        assert!(store.get_aiy_event_cursors(&[aiy_module.clone()]).unwrap()[0].is_none());
        store
            .update_aiy_event_cursor(aiy_module.clone(), aiy_cursor)
            .unwrap();
        assert_eq!(
            store.get_aiy_event_cursors(&[aiy_module.clone()]).unwrap()[0].unwrap(),
            aiy_cursor
        );
    }
}
