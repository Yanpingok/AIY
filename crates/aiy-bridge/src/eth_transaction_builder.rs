...
use crate::abi::{
    eth_bridge_committee, eth_committee_upgradeable_contract, eth_aiy_bridge, EthBridgeCommittee,
...
pub async fn build_eth_transaction(
    ...
    match action.data() {
        ...
        BridgeAction::AddTokensOnAiyAction(_) => {
            unreachable!();
        }
...
pub async fn build_emergency_op_approve_transaction(
    ...
    let contract = EthAiyBridge::new(contract_address, signer.into());

    let message: eth_aiy_bridge::Message = action.clone().try_into()?;
    let signatures = sigs
        .signatures
        .values()
