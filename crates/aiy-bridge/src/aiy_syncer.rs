...
//! The AiySyncer module is responsible for synchronizing Events emitted
//! on Aiy blockchain from concerned modules of bridge package 0x9.
...
use aiy_types::{event::EventID, Identifier};
use tokio::{
    sync::Notify,
    task::JoinHandle,
    time::{self, Duration},
...
const AIY_EVENTS_CHANNEL_SIZE: usize = 1000;

/// Map from contract address to their start cursor (exclusive)
pub type AiyTargetModules = HashMap<Identifier, Option<EventID>>;

pub struct AiySyncer<C> {
    ...
    C: AiyClientInner + 'static,
{
    ...
    pub async fn run(
        ...
        let (events_tx, events_rx) = mysten_metrics::metered_channel::channel(
            AIY_EVENTS_CHANNEL_SIZE,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["aiy_events_queue"]),
        );
        ...
        for (module, cursor) in self.cursors {
            ...
            let aiy_client_clone = self.aiy_client.clone();
    ...
    async fn run_event_listening_task(
        ...
        events_sender: mysten_metrics::metered_channel::Sender<(Identifier, Vec<AiyEvent>)>,
        aiy_client: Arc<AiyClient<C>>,
        query_interval: Duration,
        metrics: Arc<BridgeMetrics>,
    ) {
        tracing::info!(?module, ?cursor, "Starting aiy events listening task");
        ...
        let last_synced_aiy_checkpoints_metric = metrics
            .last_synced_aiy_checkpoints
            .with_label_values(&[&module.to_string()]);
        spawn_logged_monitored_task!(async move {
            loop {
                notify_clone.notified().await;
                let Ok(Ok(latest_checkpoint_sequence_number)) = retry_with_max_elapsed_time!(
                    aiy_client_clone.get_latest_checkpoint_sequence_number(),
                    Duration::from_secs(120)
                ) else {
                    tracing::error!("Failed to query latest checkpoint sequence number from aiy client after retry");
                    continue;
                };
                last_synced_aiy_checkpoints_metric.set(latest_checkpoint_sequence_number as i64);
            }
        });
        ...
        loop {
            ...
            let Ok(Ok(events)) = retry_with_max_elapsed_time!(
                aiy_client.query_events_by_module(BRIDGE_PACKAGE_ID, module.clone(), cursor),
                Duration::from_secs(120)
            ) else {
                tracing::error!("Failed to query events from aiy client after retry");
                continue;
            };
            ...
            if len != 0 {
                ...
                events_sender
                    ...
                    .await
                    .expect("All Aiy event channel receivers are closed");
                if let Some(next) = events.next_cursor {
                    cursor = Some(next);
                }
                tracing::info!(?module, ?cursor, "Observed {len} new Aiy events");
            }
        }
    }
}
...
mod tests {
    ...
    use crate::{aiy_client::AiyClient, aiy_mock_client::AiyMockClient};
    use prometheus::Registry;
    use aiy_json_rpc_types::EventPage;
    use aiy_types::{digests::TransactionDigest, event::EventID, Identifier};
    use tokio::time::timeout;

    #[tokio::test]
    async fn test_aiy_syncer_basic() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        let metrics = Arc::new(BridgeMetrics::new(&registry));
        let mock = AiyMockClient::default();
        let client = Arc::new(AiyClient::new_for_testing(mock.clone()));
        ...
        let (_handles, mut events_rx) = AiySyncer::new(client, target_modules, metrics.clone())
            .run(interval)
            .await
            .unwrap();
        ...
        let mut event_1: AiyEvent = AiyEvent::random_for_testing();
        let package_id = BRIDGE_PACKAGE_ID;
        event_1.type_.address = package_id.into();
        event_1.type_.module = module_foo.clone();
        let module_foo_events_1: aiy_json_rpc_types::Page<AiyEvent, EventID> = EventPage {
        ...
        let mut event_2: AiyEvent = AiyEvent::random_for_testing();
    ...
    async fn assert_no_more_events(
        interval: Duration,
        events_rx: &mut mysten_metrics::metered_channel::Receiver<(Identifier, Vec<AiyEvent>)>,
    ) {
