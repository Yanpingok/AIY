...
//! This file contains the definition of the AiyBridgeEvent enum, of
...
use crate::types::AiyToEthBridgeAction;
...
use aiy_types::bridge::MoveTypeBridgeMessageKey;
use aiy_types::bridge::MoveTypeCommitteeMember;
use aiy_types::bridge::MoveTypeCommitteeMemberRegistration;
use aiy_types::collection_types::VecMap;
...
impl TryFrom<MoveTokenRegistrationEvent> for TokenRegistrationEvent {
    ...
    fn try_from(event: MoveTokenRegistrationEvent) -> BridgeResult<Self> {
        let type_name = parse_aiy_type_tag(&format!("0x{}", event.type_name)).map_err(|e| {
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone, Hash)]
pub struct EmittedAiyToEthTokenBridgeV1 {
    pub nonce: u64,
    pub aiy_chain_id: BridgeChainId,
    pub eth_chain_id: BridgeChainId,
    pub aiy_address: AiyAddress,
    pub eth_address: EthAddress,
    pub token_id: u8,
    // The amount of tokens deposited with decimal points on Aiy side
    pub amount_aiy_adjusted: u64,
}
...
impl TryFrom<MoveTokenDepositedEvent> for EmittedAiyToEthTokenBridgeV1 {
    type Error = BridgeError;

    fn try_from(event: MoveTokenDepositedEvent) -> BridgeResult<Self> {
        if event.amount_aiy_adjusted == 0 {
            return Err(BridgeError::ZeroValueBridgeTransfer(format!(
                "Failed to convert MoveTokenDepositedEvent to EmittedAiyToEthTokenBridgeV1. Manual intervention is required. 0 value transfer should not be allowed in Move: {:?}",
                event,
            )));
        }

        let token_id = event.token_type;
        let aiy_chain_id = BridgeChainId::try_from(event.source_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEvent to EmittedAiyToEthTokenBridgeV1. Failed to convert source chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        let eth_chain_id = BridgeChainId::try_from(event.target_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEvent to EmittedAiyToEthTokenBridgeV1. Failed to convert target chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        if !aiy_chain_id.is_aiy_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEvent to EmittedAiyToEthTokenBridgeV1. Invalid source chain {}",
                event.source_chain
            )));
        }
        if eth_chain_id.is_aiy_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEvent to EmittedAiyToEthTokenBridgeV1. Invalid target chain {}",
                event.target_chain
            )));
        }

        let aiy_address = AiyAddress::from_bytes(event.sender_address)
            .map_err(|e| BridgeError::Generic(format!("Failed to convert MoveTokenDepositedEvent to EmittedAiyToEthTokenBridgeV1. Failed to convert sender_address to AiyAddress: {:?}", e)))?;
        ...
        Ok(Self {
            ...
            eth_address,
            token_id,
            amount_aiy_adjusted: event.amount_aiy_adjusted,
        })
    }
}

crate::declare_events!(
    AiyToEthTokenBridgeV1(EmittedAiyToEthTokenBridgeV1) => ("bridge::TokenDepositedEvent", MoveTokenDepositedEvent),
...
macro_rules! declare_events {
    ($($variant:ident($type:path) => ($event_tag:expr, $event_struct:path)),* $(,)?) => {
        ...
        // Try to convert a AiyEvent into AiyBridgeEvent
        impl AiyBridgeEvent {
            pub fn try_from_aiy_event(event: &AiyEvent) -> BridgeResult<Option<AiyBridgeEvent>> {
                ...
                $(
                    if &event.type_ == $variant.get().unwrap() {
                        ...
                        return Ok(Some(AiyBridgeEvent::$variant(event_struct.try_into()?)));
                    }
                )*
                Ok(None)
            }
...
impl AiyBridgeEvent {
    pub fn try_into_bridge_action(
        ...
        match self {
            AiyBridgeEvent::AiyToEthTokenBridgeV1(event) => {
                Some(BridgeAction::AiyToEthBridgeAction(AiyToEthBridgeAction {
                    aiy_tx_digest,
                    aiy_tx_event_index,
                    aiy_bridge_event: event.clone(),
                }))
            }
            AiyBridgeEvent::TokenTransferApproved(_event) => None,
            AiyBridgeEvent::TokenTransferClaimed(_event) => None,
            AiyBridgeEvent::TokenTransferAlreadyApproved(_event) => None,
            AiyBridgeEvent::TokenTransferAlreadyClaimed(_event) => None,
            AiyBridgeEvent::TokenTransferLimitExceed(_event) => None,
            AiyBridgeEvent::EmergencyOpEvent(_event) => None,
            AiyBridgeEvent::CommitteeMemberRegistration(_event) => None,
            AiyBridgeEvent::CommitteeUpdateEvent(_event) => None,
            AiyBridgeEvent::CommitteeMemberUrlUpdateEvent(_event) => None,
            AiyBridgeEvent::BlocklistValidatorEvent(_event) => None,
            AiyBridgeEvent::TokenRegistrationEvent(_event) => None,
            AiyBridgeEvent::NewTokenEvent(_event) => None,
            AiyBridgeEvent::UpdateTokenPriceEvent(_event) => None,
            AiyBridgeEvent::UpdateRouteLimitEvent(_event) => None,
        }
    }
}
...
pub mod tests {
    ...
    use aiy_types::base_types::AiyAddress;
    use aiy_types::bridge::BridgeChainId;
    use aiy_types::bridge::TOKEN_ID_AIY;
    use aiy_types::crypto::get_key_pair;
    use aiy_types::digests::TransactionDigest;
    use aiy_types::event::EventID;
    use aiy_types::Identifier;

    /// Returns a test AiyEvent and corresponding BridgeAction
    pub fn get_test_aiy_event_and_action(identifier: Identifier) -> (AiyEvent, BridgeAction) {
        init_all_struct_tags(); // Ensure all tags are initialized
        let sanitized_event = EmittedAiyToEthTokenBridgeV1 {
            nonce: 1,
        ...
        let emitted_event = MoveTokenDepositedEvent {
            ...
            source_chain: sanitized_event.aiy_chain_id as u8,
            sender_address: sanitized_event.aiy_address.to_vec(),
            ...
            amount_aiy_adjusted: sanitized_event.amount_aiy_adjusted,
        };

        let tx_digest = TransactionDigest::random();
        let event_idx = 10u16;
        let bridge_action = BridgeAction::AiyToEthBridgeAction(AiyToEthBridgeAction {
            aiy_tx_digest: tx_digest,
            aiy_tx_event_index: event_idx,
            aiy_bridge_event: sanitized_event.clone(),
        });
        let event = AiyEvent {
            type_: AiyToEthTokenBridgeV1.get().unwrap().clone(),
            ...
            sender: AiyAddress::random_for_testing_only(),
    ...
    async fn test_bridge_events_when_init() {
        ...
        for event in events.iter() {
            match AiyBridgeEvent::try_from_aiy_event(event).unwrap().unwrap() {
                AiyBridgeEvent::CommitteeMemberRegistration(_event) => mask |= 0x1,
                AiyBridgeEvent::CommitteeUpdateEvent(_event) => mask |= 0x2,
                AiyBridgeEvent::TokenRegistrationEvent(_event) => mask |= 0x4,
                AiyBridgeEvent::NewTokenEvent(_event) => mask |= 0x8,
    ...
    #[test]
    fn test_0_aiy_amount_conversion_for_aiy_event() {
        let emitted_event = MoveTokenDepositedEvent {
            seq_num: 1,
            source_chain: BridgeChainId::AiyTestnet as u8,
            sender_address: AiyAddress::random_for_testing_only().to_vec(),
            target_chain: BridgeChainId::EthSepolia as u8,
            target_address: EthAddress::random().as_bytes().to_vec(),
            token_type: TOKEN_ID_AIY,
            amount_aiy_adjusted: 0,
        };
        match EmittedAiyToEthTokenBridgeV1::try_from(emitted_event).unwrap_err() {
