...
//! `BridgeMonitor` receives all `AiyBridgeEvent` and `EthBridgeEvent`
//! and handles them accordingly.

use crate::abi::{
    EthBridgeCommitteeEvents, EthBridgeConfigEvents, EthBridgeEvent, EthBridgeLimiterEvents,
    EthCommitteeUpgradeableContractEvents, EthAiyBridgeEvents,
};
use crate::client::bridge_authority_aggregator::BridgeAuthorityAggregator;
use crate::crypto::BridgeAuthorityPublicKeyBytes;
use crate::events::{BlocklistValidatorEvent, CommitteeMemberUrlUpdateEvent};
use crate::events::{EmergencyOpEvent, AiyBridgeEvent};
use crate::metrics::BridgeMetrics;
use crate::retry_with_max_elapsed_time;
use crate::aiy_client::{AiyClient, AiyClientInner};
...
pub struct BridgeMonitor<C> {
    ...
    aiy_monitor_rx: mysten_metrics::metered_channel::Receiver<AiyBridgeEvent>,
    ...
    C: AiyClientInner + 'static,
{
    ...
    pub async fn run(self) {
        ...
        let mut latest_token_config = (*aiy_token_type_tags.load().clone()).clone();

        loop {
            tokio::select! {
                aiy_event = aiy_monitor_rx.recv() => {
                    if let Some(aiy_event) = aiy_event {
                        Self::handle_aiy_events(
                            aiy_event,
                            &aiy_client,
                            &bridge_auth_agg,
                            &bridge_paused_watch_tx,
                        ...
                        panic!("BridgeMonitor aiy events channel was closed unexpectedly");
                    }
                }
    ...
    async fn handle_aiy_events(
        event: AiyBridgeEvent,
        aiy_client: &Arc<AiyClient<C>>,
        bridge_auth_agg: &Arc<ArcSwap<BridgeAuthorityAggregator>>,
        bridge_paused_watch_tx: &tokio::sync::watch::Sender<IsBridgePaused>,
        aiy_token_type_tags: &Arc<ArcSwap<HashMap<u8, TypeTag>>>,
        ...
        info!("Received AiyBridgeEvent: {:?}", event);
        macro_rules! bump_aiy_counter {
            ($action:expr) => {
                bridge_metrics
                    .observed_governance_actions
                    .with_label_values(&[$action, "aiy"])
                    .inc();
            };
        }

        match event {
            AiyBridgeEvent::AiyToEthTokenBridgeV1(_) => (),
            AiyBridgeEvent::TokenTransferApproved(_) => (),
            AiyBridgeEvent::TokenTransferClaimed(_) => (),
            AiyBridgeEvent::TokenTransferAlreadyApproved(_) => (),
            AiyBridgeEvent::TokenTransferAlreadyClaimed(_) => (),
            AiyBridgeEvent::TokenTransferLimitExceed(_) => {
                // TODO do we want to do anything here?
            }

            AiyBridgeEvent::EmergencyOpEvent(event) => {
                bump_aiy_counter!(if event.frozen {
                    "bridge_paused"
                } else {
                    "bridge_unpaused"
                });
            ...
            AiyBridgeEvent::CommitteeMemberRegistration(_) => {
                bump_aiy_counter!("committee_member_registered");
            }
            AiyBridgeEvent::CommitteeUpdateEvent(_) => {
                bump_aiy_counter!("committee_updated");
            }

            AiyBridgeEvent::CommitteeMemberUrlUpdateEvent(event) => {
                bump_aiy_counter!("committee_member_url_updated");
            ...
            AiyBridgeEvent::BlocklistValidatorEvent(event) => {
                bump_aiy_counter!(if event.blocklisted {
            ...
            AiyBridgeEvent::TokenRegistrationEvent(_) => {
                bump_aiy_counter!("new_token_registered");
            }

            AiyBridgeEvent::NewTokenEvent(event) => {
                bump_aiy_counter!("new_token_added");
                    ...
                    latest_token_config.entry(event.token_id)
                {
                    ...
                    aiy_token_type_tags.store(Arc::new(latest_token_config.clone()));
                } else {
                    // invariant
            ...
            AiyBridgeEvent::UpdateTokenPriceEvent(_event) => {
                bump_aiy_counter!("token_price_updated");
            }

            AiyBridgeEvent::UpdateRouteLimitEvent(_event) => {
                bump_aiy_counter!("limit_updated");
            }
        }
    }
    ...
    fn handle_eth_events(event: EthBridgeEvent, bridge_metrics: &Arc<BridgeMetrics>) {
        ...
        match event {
            ...
            EthBridgeEvent::EthAiyBridgeEvents(event) => match event {
                EthAiyBridgeEvents::TokensClaimedFilter(_) => (),
                EthAiyBridgeEvents::TokensDepositedFilter(_) => (),
                EthAiyBridgeEvents::PausedFilter(_) => bump_eth_counter!("bridge_paused"),
                EthAiyBridgeEvents::UnpausedFilter(_) => bump_eth_counter!("bridge_unpaused"),
                EthAiyBridgeEvents::UpgradedFilter(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthAiyBridgeEvents::InitializedFilter(_) => {
                    bump_eth_counter!("bridge_contract_initialized")
                }
                EthAiyBridgeEvents::ContractUpgradedFilter(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthAiyBridgeEvents::EmergencyOperationFilter(e) => {
                    if e.paused {
...
async fn get_latest_bridge_committee_with_url_update_event<C: AiyClientInner>(
    aiy_client: Arc<AiyClient<C>>,
    ...
    loop {
        let Ok(Ok(committee)) = retry_with_max_elapsed_time!(
            aiy_client.get_bridge_committee(),
...
async fn get_latest_bridge_committee_with_blocklist_event<C: AiyClientInner>(
    aiy_client: Arc<AiyClient<C>>,
...
async fn get_latest_bridge_pause_status_with_emergency_event<C: AiyClientInner>(
    aiy_client: Arc<AiyClient<C>>,
    ...
    loop {
        let Ok(Ok(summary)) =
            retry_with_max_elapsed_time!(aiy_client.get_bridge_summary(), Duration::from_secs(600))
        else {
...
mod tests {
    ...
    use aiy_types::bridge::BridgeCommitteeSummary;
    use aiy_types::bridge::MoveTypeCommitteeMember;
    use aiy_types::crypto::get_key_pair;

    use crate::{aiy_mock_client::AiyMockClient, types::BridgeCommittee};
    ...
    async fn test_get_latest_bridge_committee_with_url_update_event() {
        ...
        aiy_client_mock.set_bridge_committee(summary.clone());
        ...
        aiy_client_mock.set_bridge_committee(old_summary.clone());
        let timer = std::time::Instant::now();
        // update the url to "http://new.url" in 1 second
        let aiy_client_mock_clone = aiy_client_mock.clone();
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            aiy_client_mock_clone.set_bridge_committee(summary.clone());
        });
        let committee = get_latest_bridge_committee_with_url_update_event(
            aiy_client.clone(),
            event.clone(),
        ...
        aiy_client_mock.set_bridge_committee(newer_summary.clone());
        let timer = std::time::Instant::now();
        let committee = get_latest_bridge_committee_with_url_update_event(
            aiy_client.clone(),
            event.clone(),
    ...
    async fn test_get_bridge_pause_status_with_emergency_event() {
        ...
        aiy_client_mock.set_is_bridge_paused(BRIDGE_PAUSED);
        let timer = std::time::Instant::now();
        assert!(
        ...
        aiy_client_mock.set_is_bridge_paused(BRIDGE_UNPAUSED);
        let timer = std::time::Instant::now();
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                aiy_client.clone(),
        ...
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            aiy_client_mock_clone.set_is_bridge_paused(BRIDGE_UNPAUSED);
        });
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                aiy_client.clone(),
    ...
    async fn test_update_bridge_authority_aggregation_with_url_change_event() {
        ...
        let aiy_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        ...
        aiy_client_mock.set_bridge_committee(new_committee_summary.clone());
        aiy_monitor_tx
            .send(AiyBridgeEvent::CommitteeMemberUrlUpdateEvent(
    ...
    async fn test_update_bridge_authority_aggregation_with_blocklist_event() {
        ...
        aiy_client_mock.set_bridge_committee(new_committee_summary.clone());
        aiy_monitor_tx
            .send(AiyBridgeEvent::BlocklistValidatorEvent(
    ...
    async fn test_update_bridge_pause_status_with_emergency_event() {
        ...
        aiy_client_mock.set_is_bridge_paused(event.frozen);
        aiy_monitor_tx
            .send(AiyBridgeEvent::EmergencyOpEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
    ...
    #[tokio::test]
    async fn test_update_aiy_token_type_tags() {
        let (
            aiy_monitor_tx,
            aiy_monitor_rx,
            _eth_monitor_tx,
        ...
        let aiy_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        let aiy_token_type_tags_clone = aiy_token_type_tags.clone();
        let _handle = tokio::task::spawn(
            BridgeMonitor::new(
                aiy_client.clone(),
                aiy_monitor_rx,
        ...
        aiy_monitor_tx
            .send(AiyBridgeEvent::NewTokenEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
        // Now expect new token type tags to appear in aiy_token_type_tags
        aiy_token_type_tags
            .load()
            .clone()
            .get(&event.token_id)
    ...
    fn setup() -> (
        mysten_metrics::metered_channel::Sender<AiyBridgeEvent>,
        mysten_metrics::metered_channel::Receiver<AiyBridgeEvent>,
        mysten_metrics::metered_channel::Sender<EthBridgeEvent>,
        mysten_metrics::metered_channel::Receiver<EthBridgeEvent>,
        AiyMockClient,
        Arc<AiyClient<AiyMockClient>>,
        ...
        let aiy_client_mock = AiyMockClient::default();
        let aiy_client = Arc::new(AiyClient::new_for_testing(aiy_client_mock.clone()));
        let (aiy_monitor_tx, aiy_monitor_rx) = mysten_metrics::metered_channel::channel(
            10000,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["aiy_monitor_queue"]),
        );
