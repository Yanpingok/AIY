...
//! 1. monitors Aiy and Ethereum events with the help of `AiySyncer` and `EthSyncer`
...
pub struct BridgeOrchestrator<C> {
    _aiy_client: Arc<AiyClient<C>>,
    aiy_events_rx: mysten_metrics::metered_channel::Receiver<(Identifier, Vec<AiyEvent>)>,
    eth_events_rx: mysten_metrics::metered_channel::Receiver<(EthAddress, u64, Vec<EthLog>)>,
    store: Arc<BridgeOrchestratorTables>,
    aiy_monitor_tx: mysten_metrics::metered_channel::Sender<AiyBridgeEvent>,
    ...
    C: AiyClientInner + 'static,
{
    ...
    pub async fn run(
        ...
        task_handles.push(spawn_logged_monitored_task!(Self::run_aiy_watcher(
            store_clone,
    ...
    async fn run_aiy_watcher(
        store: Arc<BridgeOrchestratorTables>,
        executor_tx: mysten_metrics::metered_channel::Sender<BridgeActionExecutionWrapper>,
        mut aiy_events_rx: mysten_metrics::metered_channel::Receiver<(Identifier, Vec<AiyEvent>)>,
        monitor_tx: mysten_metrics::metered_channel::Sender<AiyBridgeEvent>,
        metrics: Arc<BridgeMetrics>,
    ) {
        info!("Starting aiy watcher task");
        while let Some((identifier, events)) = aiy_events_rx.recv().await {
            if events.is_empty() {
                continue;
            }
            info!("Received {} Aiy events: {:?}", events.len(), events);
            metrics
                .aiy_watcher_received_events
                .inc_by(events.len() as u64);
            let bridge_events = events
                .iter()
                .filter_map(|aiy_event| {
                    match AiyBridgeEvent::try_from_aiy_event(aiy_event) {
                        ...
                        Err(BridgeError::ZeroValueBridgeTransfer(_)) => {
                            error!("Zero value bridge transfer: {:?}", aiy_event);
                            None
                        }
                        Err(e) => {
                            panic!(
                                "Aiy Event could not be deserialzed to AiyBridgeEvent: {:?}",
                                e
                            );
                        }
                    }
            ...
            let mut actions = vec![];
            for (aiy_event, opt_bridge_event) in events.iter().zip(bridge_events) {
                if opt_bridge_event.is_none() {
                    // TODO: we probably should not miss any events, log for now.
                    metrics.aiy_watcher_unrecognized_events.inc();
                    error!("Aiy event not recognized: {:?}", aiy_event);
                    continue;
                }
                // Unwrap safe: checked above
                let bridge_event: AiyBridgeEvent = opt_bridge_event.unwrap();
                info!("Observed Aiy bridge event: {:?}", bridge_event);
                ...
                if let Some(action) = bridge_event
                    .try_into_bridge_action(aiy_event.id.tx_digest, aiy_event.id.event_seq as u16)
                {
            ...
            if !actions.is_empty() {
                info!("Received {} actions from Aiy: {:?}", actions.len(), actions);
                metrics
                    .aiy_watcher_received_actions
            ...
            store
                .update_aiy_event_cursor(identifier, cursor)
                .expect("Store operation should not fail");
        }
        panic!("Aiy event channel was closed unexpectedly");
    }
...
mod tests {
    use crate::{
        test_utils::{get_test_eth_to_aiy_bridge_action, get_test_log_and_action},
    ...
    use crate::test_utils::get_test_aiy_to_eth_bridge_action;
    use crate::{events::tests::get_test_aiy_event_and_action, aiy_mock_client::AiyMockClient};

    #[tokio::test]
    async fn test_aiy_watcher_task() {
        // Note: this test may fail because of the following reasons:
        // the AiyEvent's struct tag does not match the ones in events.rs
        ...
        let identifier = Identifier::from_str("test_aiy_watcher_task").unwrap();
        let (aiy_event, bridge_action) = get_test_aiy_event_and_action(identifier.clone());
        aiy_events_tx
            .send((identifier.clone(), vec![aiy_event.clone()]))
            .await
            .unwrap();
        ...
        loop {
            ...
            assert_eq!(
                store.get_aiy_event_cursors(&[identifier]).unwrap()[0].unwrap(),
                aiy_event.id,
            );
            break;
        }
    ...
    async fn test_resume_actions_in_pending_logs() {
        ...
        let action1 = get_test_aiy_to_eth_bridge_action(
            None,
            Some(0),
            Some(99),
            Some(10000),
        ...
        let action2 = get_test_eth_to_aiy_bridge_action(None, None, None, None);
        store
    ...
    fn setup() -> (
        mysten_metrics::metered_channel::Sender<(Identifier, Vec<AiyEvent>)>,
        mysten_metrics::metered_channel::Receiver<(Identifier, Vec<AiyEvent>)>,
        ...
        let (aiy_events_tx, aiy_events_rx) = mysten_metrics::metered_channel::channel(
            100,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["unit_test_aiy_events_queue"]),
        );
