...
use crate::abi::{
    EthBridgeCommittee, EthBridgeConfig, EthBridgeLimiter, EthBridgeVault, EthAiyBridge,
};
use crate::config::{
    default_ed25519_key_pair, BridgeNodeConfig, EthConfig, MetricsConfig, AiyConfig, WatchdogConfig,
};
use crate::crypto::BridgeAuthorityKeyPair;
use crate::crypto::BridgeAuthorityPublicKeyBytes;
use crate::server::APPLICATION_JSON;
use crate::types::BridgeCommittee;
use crate::types::{AddTokensOnAiyAction, BridgeAction};
use anyhow::anyhow;
...
use aiy_types::bridge::{BRIDGE_MODULE_NAME, BRIDGE_REGISTER_FOREIGN_TOKEN_FUNCTION_NAME};
...
use aiy_types::transaction::{ObjectArg, TransactionData};
...
pub struct EthBridgeContracts<P> {
    pub bridge: EthAiyBridge<Provider<P>>,
...
pub fn generate_bridge_authority_key_and_write_to_file(
    ...
    let aiy_address = AiyAddress::from(&kp.public);
    println!(
        "Corresponding Aiy address by this ecdsa key: {:?}",
        aiy_address
    );
...
pub fn generate_bridge_client_key_and_write_to_file(
    ...
    let aiy_address = AiyAddress::from(&kp.public());
    println!("Corresponding Aiy address by this key: {:?}", aiy_address);
...
/// Given the address of AiyBridge Proxy, return the addresses of the committee, limiter, vault, and config.
pub async fn get_eth_contract_addresses<P: ethers::providers::JsonRpcClient + 'static>(
    ...
    let aiy_bridge = EthAiyBridge::new(bridge_proxy_address, provider.clone());
    let committee_address: EthAddress = aiy_bridge.committee().call().await?;
    ...
    let limiter_address: EthAddress = aiy_bridge.limiter().call().await?;
    let vault_address: EthAddress = aiy_bridge.vault().call().await?;
...
/// Given the address of AiyBridge Proxy, return the contracts of the committee, limiter, vault, and config.
...
pub fn examine_key(path: &PathBuf, is_validator_key: bool) -> Result<(), anyhow::Error> {
    ...
    let aiy_address = AiyAddress::from(&key.public());
    ...
    println!("Corresponding Aiy address: {:?}", aiy_address);
...
pub fn generate_bridge_node_config_and_write_to_file(
    ...
    let mut config = BridgeNodeConfig {
        ...
        aiy: AiyConfig {
            aiy_rpc_url: "your_aiy_rpc_url".to_string(),
            aiy_bridge_chain_id: BridgeChainId::AiyTestnet as u8,
    ...
    if run_client {
        config.aiy.bridge_client_key_path = Some(PathBuf::from("/path/to/your/bridge_client_key"));
...
pub async fn publish_and_register_coins_return_add_coins_on_aiy_action(
    ...
    let aiy_client = wallet_context.get_client().await.unwrap();
    let quorum_driver_api = Arc::new(aiy_client.quorum_driver_api().clone());
    let rgp = aiy_client
        .governance_api()
    ...
    for (response, sender) in publish_coin_responses.into_iter().zip(senders.clone()) {
        ...
        for object_change in &object_changes {
            if let o @ aiy_json_rpc_types::ObjectChange::Created { object_type, .. } = object_change
            {
...
/// If a validator is not in the Aiy committee, we will use its base URL as the name.
pub async fn get_committee_voting_power_by_name(
    bridge_committee: &Arc<BridgeCommittee>,
    system_state: &AiySystemStateSummary,
) -> BTreeMap<String, StakeUnit> {
    let mut aiy_committee: BTreeMap<_, _> = system_state
        .active_validators
        .iter()
        .map(|v| (v.aiy_address, v.name.clone()))
        .collect();
    bridge_committee
        .members()
        .iter()
