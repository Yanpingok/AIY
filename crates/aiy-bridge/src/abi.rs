...
use crate::types::{
    ...
    BridgeActionType, EmergencyAction, EthLog, EthToAiyBridgeAction, EvmContractUpgradeAction,
    LimitUpdateAction, AiyToEthBridgeAction,
};
...
#[rustfmt::skip]
gen_eth_events!(
    EthAiyBridge, EthAiyBridgeEvents, "abi/aiy_bridge.json",
...
impl EthBridgeEvent {
    pub fn try_into_bridge_action(
        ...
        Ok(match self {
            EthBridgeEvent::EthAiyBridgeEvents(event) => {
                match event {
                    EthAiyBridgeEvents::TokensDepositedFilter(event) => {
                        let bridge_event = match EthToAiyTokenBridgeV1::try_from(&event) {
                            Ok(bridge_event) => {
                                if bridge_event.aiy_adjusted_amount == 0 {
                            ...
                            Err(e) => {
                                return Err(BridgeError::Generic(format!("Manual intervention is required. Failed to convert TokensDepositedFilter log to EthToAiyTokenBridgeV1. This indicates incorrect parameters or a bug in the code: {:?}. Err: {:?}", event, e)));
                            }
                        };

                        Some(BridgeAction::EthToAiyBridgeAction(EthToAiyBridgeAction {
                            eth_tx_hash,
                            eth_event_index,
                            eth_bridge_event: bridge_event,
                        }))
                    }
                    EthAiyBridgeEvents::TokensClaimedFilter(_event) => None,
                    EthAiyBridgeEvents::PausedFilter(_event) => None,
                    EthAiyBridgeEvents::UnpausedFilter(_event) => None,
                    EthAiyBridgeEvents::UpgradedFilter(_event) => None,
                    EthAiyBridgeEvents::InitializedFilter(_event) => None,
                    EthAiyBridgeEvents::ContractUpgradedFilter(_event) => None,
                    EthAiyBridgeEvents::EmergencyOperationFilter(_event) => None,
                }
            }
...
pub struct EthToAiyTokenBridgeV1 {
    pub nonce: u64,
    ...
    pub token_id: u8,
    pub aiy_adjusted_amount: u64,
}

impl TryFrom<&TokensDepositedFilter> for EthToAiyTokenBridgeV1 {
    type Error = BridgeError;
    fn try_from(event: &TokensDepositedFilter) -> BridgeResult<Self> {
        Ok(Self {
            nonce: event.nonce,
            aiy_chain_id: BridgeChainId::try_from(event.destination_chain_id)?,
            eth_chain_id: BridgeChainId::try_from(event.source_chain_id)?,
            aiy_address: AiyAddress::from_bytes(event.recipient_address.as_ref())?,
            eth_address: event.sender_address,
            token_id: event.token_id,
            aiy_adjusted_amount: event.aiy_adjusted_amount,
        })
    }
}
...
impl TryFrom<AiyToEthBridgeAction> for eth_aiy_bridge::Message {
    type Error = BridgeError;

    fn try_from(action: AiyToEthBridgeAction) -> BridgeResult<Self> {
        Ok(eth_aiy_bridge::Message {
            message_type: BridgeActionType::TokenTransfer as u8,
            version: TOKEN_TRANSFER_MESSAGE_VERSION,
            nonce: action.aiy_bridge_event.nonce,
            chain_id: action.aiy_bridge_event.aiy_chain_id as u8,
            payload: action
                .as_payload_bytes()
...
impl From<ParsedTokenTransferMessage> for eth_aiy_bridge::Message {
    fn from(parsed_message: ParsedTokenTransferMessage) -> Self {
        eth_aiy_bridge::Message {
            message_type: BridgeActionType::TokenTransfer as u8,
...
impl TryFrom<EmergencyAction> for eth_aiy_bridge::Message {
    type Error = BridgeError;
...
mod tests {
    ...
    use hex_literal::hex;
    use std::str::FromStr;
    use aiy_types::{bridge::TOKEN_ID_ETH, crypto::ToFromBytes};
    ...
    fn test_eth_message_conversion_emergency_action_regression() -> anyhow::Result<()> {
        ...
        let message: eth_aiy_bridge::Message = action.try_into().unwrap();
        assert_eq!(
            message,
            eth_aiy_bridge::Message {
                message_type: BridgeActionType::EmergencyButton as u8,
    ...
    fn test_eth_message_conversion_update_limit_action_regression() -> anyhow::Result<()> {
        ...
        let action = LimitUpdateAction {
            nonce: 2,
            chain_id: BridgeChainId::EthSepolia,
            sending_chain_id: BridgeChainId::AiyTestnet,
    ...
    #[test]
    fn test_token_deposit_eth_log_to_aiy_bridge_event_regression() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        ...
        assert_eq!(
            event,
            EthBridgeEvent::EthAiyBridgeEvents(EthAiyBridgeEvents::TokensDepositedFilter(
                TokensDepositedFilter {
                    source_chain_id: 12,
                    nonce: 0,
                    destination_chain_id: 2,
                    token_id: 2,
                    aiy_adjusted_amount: 4200000000,
    ...
    #[test]
    fn test_0_aiy_amount_conversion_for_eth_event() {
        let e = EthBridgeEvent::EthAiyBridgeEvents(EthAiyBridgeEvents::TokensDepositedFilter(
            TokensDepositedFilter {
                source_chain_id: BridgeChainId::EthSepolia as u8,
                nonce: 0,
                destination_chain_id: BridgeChainId::AiyTestnet as u8,
                token_id: 2,
                aiy_adjusted_amount: 1,
                sender_address: EthAddress::random(),
                recipient_address: ethers::types::Bytes::from(
                    AiyAddress::random_for_testing_only().to_vec(),
                ),
            },
        ));
        assert!(e
