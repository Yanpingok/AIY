...
use crate::aiy_bridge_watchdog::eth_bridge_status::EthBridgeStatus;
use crate::aiy_bridge_watchdog::eth_vault_balance::{EthereumVaultBalance, VaultAsset};
use crate::aiy_bridge_watchdog::metrics::WatchdogMetrics;
use crate::aiy_bridge_watchdog::aiy_bridge_status::AiyBridgeStatus;
use crate::aiy_bridge_watchdog::total_supplies::TotalSupplies;
use crate::aiy_bridge_watchdog::{BridgeWatchDog, Observable};
...
pub async fn run_bridge_node(
    ...
    let aiy_chain_identifier = server_config
        .aiy_client
        .get_chain_identifier()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to get aiy chain identifier: {:?}", e))?;
    ...
    let aiy_client = server_config.aiy_client.clone();
    ...
    if let Some(client_config) = client_config {
        let committee_keys_to_names =
            Arc::new(get_validator_names_by_pub_keys(&committee, &aiy_system).await);
    ...
    let committee_name_mapping = get_committee_voting_power_by_name(&committee, &aiy_system).await;
...
async fn start_watchdog(
    ...
    let aiy_bridge_status = AiyBridgeStatus::new(
        aiy_client.clone(),
        watchdog_metrics.aiy_bridge_paused.clone(),
    );
    ...
    if let Some(watchdog_config) = watchdog_config {
        if !watchdog_config.total_supplies.is_empty() {
            let total_supplies = TotalSupplies::new(
                Arc::new(aiy_client.aiy_client().clone()),
...
async fn start_client_components(
    ...
    let aiy_modules_to_watch = get_aiy_modules_to_watch(
        &store,
        client_config.aiy_bridge_module_last_processed_event_id_override,
    );
    ...
    let aiy_client = client_config.aiy_client.clone();
    ...
    let (task_handles, aiy_events_rx) = AiySyncer::new(
        client_config.aiy_client,
        aiy_modules_to_watch,
        metrics.clone(),
    )
    .run(Duration::from_secs(2))
    .await
    .expect("Failed to start aiy syncer");
    all_handles.extend(task_handles);
    ...
    let is_bridge_paused = aiy_client.is_bridge_paused().await.unwrap();
    ...
    let aiy_token_type_tags = Arc::new(ArcSwap::from(Arc::new(aiy_token_type_tags)));
    let bridge_action_executor = BridgeActionExecutor::new(
        aiy_client.clone(),
        bridge_auth_agg.clone(),
        store.clone(),
        client_config.key,
        client_config.aiy_address,
...
fn get_aiy_modules_to_watch(
    store: &std::sync::Arc<BridgeOrchestratorTables>,
    aiy_bridge_module_last_processed_event_id_override: Option<EventID>,
) -> HashMap<Identifier, Option<EventID>> {
    let aiy_bridge_modules = vec![
        BRIDGE_MODULE_NAME.to_owned(),
        BRIDGE_COMMITTEE_MODULE_NAME.to_owned(),
        BRIDGE_TREASURY_MODULE_NAME.to_owned(),
        BRIDGE_LIMITER_MODULE_NAME.to_owned(),
    ];
    if let Some(cursor) = aiy_bridge_module_last_processed_event_id_override {
        info!("Overriding cursor for aiy bridge modules to {:?}", cursor);
    ...
    let aiy_bridge_module_stored_cursor = store
        .get_aiy_event_cursors(&aiy_bridge_modules)
        .expect("Failed to get eth aiy event cursors from storage");
    let mut aiy_modules_to_watch = HashMap::new();
    for (module_identifier, cursor) in aiy_bridge_modules
        .iter()
        .zip(aiy_bridge_module_stored_cursor)
    {
        if cursor.is_none() {
            info!(
                "No cursor found for aiy bridge module {} in storage or config override, query start from the beginning.",
                module_identifier
            );
        }
        aiy_modules_to_watch.insert(module_identifier.clone(), cursor);
    }
    aiy_modules_to_watch
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_aiy_modules_to_watch() {
        telemetry_subscribers::init_for_testing();
        let temp_dir = tempfile::tempdir().unwrap();
        ...
        let aiy_modules_to_watch = get_aiy_modules_to_watch(&store, None);
        assert_eq!(
            aiy_modules_to_watch,
            vec![
        ...
        let aiy_modules_to_watch = get_aiy_modules_to_watch(&store, Some(override_cursor));
        assert_eq!(
            aiy_modules_to_watch,
            vec![
        ...
        store
            .update_aiy_event_cursor(bridge_module.clone(), stored_cursor)
            .unwrap();
        let aiy_modules_to_watch = get_aiy_modules_to_watch(&store, None);
        assert_eq!(
            aiy_modules_to_watch,
        ...
        store
            .update_aiy_event_cursor(committee_module.clone(), stored_cursor)
            .unwrap();
        let aiy_modules_to_watch = get_aiy_modules_to_watch(&store, Some(override_cursor));
        assert_eq!(
            aiy_modules_to_watch,
    ...
    async fn test_starting_bridge_node() {
        ...
        let config = BridgeNodeConfig {
            ...
            aiy: AiyConfig {
                aiy_rpc_url: bridge_test_cluster.aiy_rpc_url(),
            ...
            eth: EthConfig {
                ...
                eth_bridge_proxy_address: bridge_test_cluster.aiy_bridge_address(),
    ...
    async fn test_starting_bridge_node_with_client() {
        ...
        let client_aiy_address = AiyAddress::from(kp.public());
        let sender_address = bridge_test_cluster.aiy_user_address();
        // send some gas to this address
        bridge_test_cluster
            .test_cluster
            .inner
            .transfer_aiy_must_exceed(sender_address, client_aiy_address, 1000000000)
            .await;
        ...
        let config = BridgeNodeConfig {
            ...
            aiy: AiyConfig {
                ...
                bridge_client_key_path: None,
                bridge_client_gas_object: None,
                aiy_bridge_module_last_processed_event_id_override: Some(EventID {
                    tx_digest: TransactionDigest::random(),
                    event_seq: 0,
                }),
            },
