...
use aiy_types::bridge::{
    BRIDGE_CREATE_ADD_TOKEN_ON_AIY_MESSAGE_FUNCTION_NAME,
...
use aiy_types::{
    base_types::{ObjectRef, AiyAddress},
    programmable_transaction_builder::ProgrammableTransactionBuilder,
    transaction::{ObjectArg, TransactionData},
    TypeTag,
};
use aiy_types::{Identifier, BRIDGE_PACKAGE_ID};
...
pub fn build_aiy_transaction(
    client_address: AiyAddress,
    ...
    // TODO: Check chain id?
    match action.data() {
        BridgeAction::EthToAiyBridgeAction(_) => build_token_bridge_approve_transaction(
            client_address,
            gas_object_ref,
            action,
            true,
        ...
        BridgeAction::AiyToEthBridgeAction(_) => build_token_bridge_approve_transaction(
            client_address,
            gas_object_ref,
            action,
            false,
        ...
        BridgeAction::EvmContractUpgradeAction(_) => {
            // It does not need a Aiy tranaction to execute EVM contract upgrade
            unreachable!()
        }
        BridgeAction::AddTokensOnAiyAction(_) => build_add_tokens_on_aiy_transaction(
            client_address,
            gas_object_ref,
            action,
            bridge_object_arg,
        ...
        BridgeAction::AddTokensOnEvmAction(_) => {
            // It does not need a Aiy tranaction to add tokens on EVM
            unreachable!()
        }
    }
}
...
fn build_token_bridge_approve_transaction(
    ...
    let (source_chain, seq_num, sender, target_chain, target, token_type, amount) =
        match bridge_action {
            BridgeAction::AiyToEthBridgeAction(a) => {
                let bridge_event = a.aiy_bridge_event;
                (
                    bridge_event.aiy_chain_id,
                    bridge_event.nonce,
                    bridge_event.aiy_address.to_vec(),
                    bridge_event.eth_chain_id,
                    bridge_event.eth_address.to_fixed_bytes().to_vec(),
                    bridge_event.token_id,
                    bridge_event.amount_aiy_adjusted,
                )
            }
            BridgeAction::EthToAiyBridgeAction(a) => {
                ...
                (
                    ...
                    bridge_event.aiy_address.to_vec(),
                    bridge_event.token_id,
                    bridge_event.aiy_adjusted_amount,
                )
            }
            _ => unreachable!(),
        };
    ...
    builder.programmable_move_call(
        BRIDGE_PACKAGE_ID,
        aiy_types::bridge::BRIDGE_MODULE_NAME.to_owned(),
...
pub fn build_add_tokens_on_aiy_transaction(
    client_address: AiyAddress,
    gas_object_ref: &ObjectRef,
    action: VerifiedCertifiedBridgeAction,
    bridge_object_arg: ObjectArg,
    ...
    let (source_chain, seq_num, native, token_ids, token_type_names, token_prices) =
        match bridge_action {
            BridgeAction::AddTokensOnAiyAction(a) => (
                a.chain_id,
                a.nonce,
                a.native,
                a.token_ids,
    ...
    let message_arg = builder.programmable_move_call(
        ...
        BRIDGE_CREATE_ADD_TOKEN_ON_AIY_MESSAGE_FUNCTION_NAME.into(),
        vec![],
        vec![
            source_chain,
            seq_num,
...
mod tests {
    ...
    use std::sync::Arc;
    use aiy_types::bridge::{BridgeChainId, TOKEN_ID_BTC, TOKEN_ID_USDC};
    use aiy_types::crypto::get_key_pair;
    use aiy_types::crypto::ToFromBytes;

    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_aiy_transaction_for_token_transfer() {
        ...
        let aiy_client = AiyClient::new(&test_cluster.inner.fullnode_handle.rpc_url, metrics)
            .await
            .unwrap();
        let bridge_authority_keys = test_cluster.authority_keys_clone();

        // Note: We don't call `aiy_client.get_bridge_committee` here because it will err if the committee
        ...
        // 1. Test Eth -> Aiy Transfer approval
        ...
        // 2. Test Aiy -> Eth Transfer approval
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_aiy_transaction_for_emergency_op() {
        telemetry_subscribers::init_for_testing();
        let num_valdiator = 2;
        let mut bridge_keys = vec![];
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let summary = aiy_client.get_bridge_summary().await.unwrap();
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_aiy_transaction_for_committee_blocklist() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let committee = aiy_client.get_bridge_summary().await.unwrap().committee;
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_aiy_transaction_for_limit_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        .await;
        let new_transfer_limit = aiy_client
            .get_bridge_summary()
            .await
            .unwrap()
            .limiter
        ...
        for limit in new_transfer_limit {
            if limit.0 == BridgeChainId::EthCustom && limit.1 == BridgeChainId::AiyCustom {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_aiy_transaction_for_price_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let notional_values = aiy_client.get_notional_values().await.unwrap();
        ...
        .await;
        let new_notional_values = aiy_client.get_notional_values().await.unwrap();
