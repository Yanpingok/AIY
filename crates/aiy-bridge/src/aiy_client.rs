...
use aiy_json_rpc_types::DevInspectResults;
use aiy_json_rpc_types::{EventFilter, Page, AiyEvent};
use aiy_json_rpc_types::{
    EventPage, AiyObjectDataOptions, AiyTransactionBlockResponse,
...
use aiy_types::bridge::BridgeTreasurySummary;
use aiy_types::bridge::MoveTypeCommitteeMember;
use aiy_types::bridge::MoveTypeParsedTokenTransferMessage;
use aiy_types::gas_coin::GasCoin;
use aiy_types::object::Owner;
use aiy_types::parse_aiy_type_tag;
use aiy_types::transaction::Argument;
use aiy_types::transaction::CallArg;
...
use aiy_types::BRIDGE_PACKAGE_ID;
...
use crate::events::AiyBridgeEvent;
...
pub struct AiyClient<P> {
    inner: P,
    bridge_metrics: Arc<BridgeMetrics>,
}

pub type AiyBridgeClient = AiyClient<AiySdkClient>;

impl AiyBridgeClient {
    pub async fn new(rpc_url: &str, bridge_metrics: Arc<BridgeMetrics>) -> anyhow::Result<Self> {
        let inner = AiyClientBuilder::default()
            .build(rpc_url)
            .await
            .map_err(|e| {
                anyhow!("Can't establish connection with Aiy Rpc {rpc_url}. Error: {e}")
            })?;
        let self_ = Self {
            inner,
            bridge_metrics,
    ...
    pub fn aiy_client(&self) -> &AiySdkClient {
        &self.inner
    }
}
    ...
    P: AiyClientInner,
{
    ...
    async fn describe(&self) -> anyhow::Result<()> {
        ...
        tracing::info!(
            "AiyClient is connected to chain {chain_id}, current block number: {block_number}"
        );
        Ok(())
    }
    ...
    ) -> BridgeResult<Page<AiyEvent, EventID>> {
    ...
    /// Returns BridgeAction from a Aiy Transaction with transaction hash
    ...
    pub async fn get_bridge_action_by_tx_digest_and_event_idx_maybe(
        ...
        if event.type_.address.as_ref() != BRIDGE_PACKAGE_ID.as_ref() {
            return Err(BridgeError::BridgeEventInUnrecognizedAiyPackage);
        }
        let bridge_event = AiyBridgeEvent::try_from_aiy_event(event)?
            .ok_or(BridgeError::NoBridgeEventsInTxPosition)?;
    ...
    pub async fn get_token_id_map(&self) -> BridgeResult<HashMap<u8, TypeTag>> {
        self.get_bridge_summary()
            ...
            .into_iter()
            .map(|(id, name)| {
                parse_aiy_type_tag(&format!("0x{name}"))
                    .map(|name| (id, name))
                    .map_err(|e| {
        ...
        for (_, member) in move_type_bridge_committee.members {
            ...
            let base_url = from_utf8(&http_rest_url).unwrap_or_else(|_e| {
                warn!(
                    ...
                    aiy_address, bridge_pubkey_bytes, http_rest_url
                );
                ""
            });
    ...
    pub async fn execute_transaction_block_with_effects(
        &self,
        tx: aiy_types::transaction::Transaction,
    ) -> BridgeResult<AiyTransactionBlockResponse> {
...
/// Use a trait to abstract over the AiySDKClient and AiyMockClient for testing.
#[async_trait]
pub trait AiyClientInner: Send + Sync {
    ...
    async fn get_events_by_tx_digest(
        &self,
        tx_digest: TransactionDigest,
    ) -> Result<Vec<AiyEvent>, Self::Error>;
    ...
    async fn execute_transaction_block_with_effects(
        &self,
        tx: Transaction,
    ) -> Result<AiyTransactionBlockResponse, BridgeError>;
...
#[async_trait]
impl AiyClientInner for AiySdkClient {
    type Error = aiy_sdk::error::Error;
    ...
    async fn execute_transaction_block_with_effects(
        ...
        match self.quorum_driver_api().execute_transaction_block(
            tx,
            AiyTransactionBlockResponseOptions::new().with_effects().with_events(),
            Some(aiy_types::quorum_driver_types::ExecuteTransactionRequestType::WaitForEffectsCert),
        ).await {
            Ok(response) => Ok(response),
            Err(e) => return Err(BridgeError::AiyTxFailureGeneric(e.to_string())),
        }
    }
        ...
        loop {
            match self
                ...
                .get_object_with_options(
                    gas_object_id,
                    AiyObjectDataOptions::default().with_owner().with_content(),
                )
                .await
                .map(|resp| resp.data)
            {
    ...
    T: DeserializeOwned,
{
    ...
    let resp = aiy_client
        .read_api()
        .dev_inspect_transaction_block(AiyAddress::ZERO, kind, None, None, None)
        .await?;
...
mod tests {
    ...
    use crate::{
        events::{EmittedAiyToEthTokenBridgeV1, MoveTokenDepositedEvent},
        aiy_mock_client::AiyMockClient,
        test_utils::{
            approve_action_with_validator_secrets, bridge_token, get_test_eth_to_aiy_bridge_action,
            get_test_aiy_to_eth_bridge_action,
        },
        types::AiyToEthBridgeAction,
    };
    ...
    use aiy_types::bridge::{BridgeChainId, TOKEN_ID_AIY, TOKEN_ID_USDC};
    use aiy_types::crypto::get_key_pair;

    use super::*;
    use crate::events::{init_all_struct_tags, AiyToEthTokenBridgeV1};
    ...
    async fn get_bridge_action_by_tx_digest_and_event_idx_maybe() {
        ...
        let mock_client = AiyMockClient::default();
        let aiy_client = AiyClient::new_for_testing(mock_client.clone());
        let tx_digest = TransactionDigest::random();

        // Ensure all struct tags are inited
        init_all_struct_tags();

        let sanitized_event_1 = EmittedAiyToEthTokenBridgeV1 {
            nonce: 1,
            aiy_chain_id: BridgeChainId::AiyTestnet,
            aiy_address: AiyAddress::random_for_testing_only(),
            eth_chain_id: BridgeChainId::EthSepolia,
            eth_address: EthAddress::random(),
            token_id: TOKEN_ID_AIY,
            amount_aiy_adjusted: 100,
        };
        let emitted_event_1 = MoveTokenDepositedEvent {
            seq_num: sanitized_event_1.nonce,
            source_chain: sanitized_event_1.aiy_chain_id as u8,
            sender_address: sanitized_event_1.aiy_address.to_vec(),
            ...
            amount_aiy_adjusted: sanitized_event_1.amount_aiy_adjusted,
        };

        let mut aiy_event_1 = AiyEvent::random_for_testing();
        aiy_event_1.type_ = AiyToEthTokenBridgeV1.get().unwrap().clone();
        aiy_event_1.bcs = BcsEvent::new(bcs::to_bytes(&emitted_event_1).unwrap());
        ...
        let mut aiy_event_2 = AiyEvent::random_for_testing();
        aiy_event_2.type_ = AiyToEthTokenBridgeV1.get().unwrap().clone();
        aiy_event_2.type_.module = Identifier::from_str("unrecognized_module").unwrap();
        aiy_event_2.bcs = BcsEvent::new(bcs::to_bytes(&event_2).unwrap());

        // Event 3 is defined in non-bridge package
        let mut aiy_event_3 = aiy_event_1.clone();
        aiy_event_3.type_.address = AccountAddress::random();
        ...
        let expected_action_1 = BridgeAction::AiyToEthBridgeAction(AiyToEthBridgeAction {
            aiy_tx_digest: tx_digest,
            aiy_tx_event_index: 0,
            aiy_bridge_event: sanitized_event_1.clone(),
        });
        assert_eq!(
            aiy_client
        ...
        let expected_action_2 = BridgeAction::AiyToEthBridgeAction(AiyToEthBridgeAction {
            aiy_tx_digest: tx_digest,
            aiy_tx_event_index: 2,
            aiy_bridge_event: sanitized_event_1.clone(),
        });
        assert_eq!(
            aiy_client
        ...
        assert!(matches!(
            aiy_client
                ...
                .await
                .unwrap_err(),
            BridgeError::BridgeEventInUnrecognizedAiyPackage
        ),);
        assert!(matches!(
            aiy_client
        ...
        aiy_event_2.type_ = AiyToEthTokenBridgeV1.get().unwrap().clone();
        mock_client.add_events_by_tx_digest(tx_digest, vec![aiy_event_2]);
        aiy_client
            .get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 2)
            .await
            .unwrap_err();
    ...
    async fn test_get_action_onchain_status_for_aiy_to_eth_transfer() {
        telemetry_subscribers::init_for_testing();
        ...
        let aiy_client =
            AiyClient::new(&test_cluster.inner.fullnode_handle.rpc_url, bridge_metrics)
                .await
                .unwrap();
        ...
        let bridge_object_arg = aiy_client
            ...
            .await;
        let id_token_map = aiy_client.get_token_id_map().await.unwrap();

        // 1. Create a Eth -> Aiy Transfer (recipient is sender address), approve with validator secrets and assert its status to be Claimed
        let action = get_test_eth_to_aiy_bridge_action(None, Some(usdc_amount), Some(sender), None);
        ...
        // 2. Create a Aiy -> Eth Transfer, approve with validator secrets and assert its status to be Approved
        ...
        let action =
            get_test_aiy_to_eth_bridge_action(None, None, Some(100), None, None, None, None);
        let status = aiy_client
            .inner
            .get_token_transfer_action_onchain_status(
                bridge_object_arg,
