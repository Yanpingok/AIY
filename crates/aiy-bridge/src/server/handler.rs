...
pub trait BridgeRequestHandlerTrait {
    ...
    /// from Ethereum to Aiy. The inputs are a transaction hash on Ethereum
    ...
    /// Handles a request to sign a BridgeAction that bridges assets
    /// from Aiy to Ethereum. The inputs are a transaction digest on Aiy
    /// that emitted the bridge event and the Event index in that transaction
    async fn handle_aiy_tx_digest(
        &self,
...
#[async_trait::async_trait]
impl<C> ActionVerifier<(TransactionDigest, u16)> for AiyActionVerifier<C>
where
    C: AiyClientInner + Send + Sync + 'static,
{
    fn name(&self) -> &'static str {
        "AiyActionVerifier"
    }
    ...
    async fn verify(&self, key: (TransactionDigest, u16)) -> BridgeResult<BridgeAction> {
        ...
        self.aiy_client
            ...
            .await
            .tap_ok(|action| info!("Aiy action found: {:?}", action))
    }
}
...
pub struct BridgeRequestHandler {
    aiy_signer_tx: mysten_metrics::metered_channel::Sender<(
...
impl BridgeRequestHandler {
    pub fn new<
        SC: AiyClientInner + Send + Sync + 'static,
        EP: JsonRpcClient + Send + Sync + 'static,
    >(
        signer: BridgeAuthorityKeyPair,
        aiy_client: Arc<AiyClient<SC>>,
        eth_client: Arc<EthClient<EP>>,
        approved_governance_actions: Vec<BridgeAction>,
        metrics: Arc<BridgeMetrics>,
    ) -> Self {
        let (aiy_signer_tx, aiy_rx) = mysten_metrics::metered_channel::channel(
            1000,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["server_aiy_action_signing_queue"]),
        );
...
impl BridgeRequestHandlerTrait for BridgeRequestHandler {
    ...
    async fn handle_aiy_tx_digest(
        ...
        self.aiy_signer_tx
            ...
            .await
            .unwrap_or_else(|_| panic!("Server aiy signing channel is closed"));
        let signed_action = rx
            .await
            .unwrap_or_else(|_| panic!("Server signing task's oneshot channel is dropped"))?;
        Ok(Json(signed_action))
...
mod tests {
    ...
    use crate::{
        ...
        events::{init_all_struct_tags, MoveTokenDepositedEvent, AiyToEthTokenBridgeV1},
        aiy_mock_client::AiyMockClient,
        test_utils::{
            get_test_log_and_action, get_test_aiy_to_eth_bridge_action, mock_last_finalized_block,
        },
        types::{EmergencyAction, EmergencyActionType, LimitUpdateAction},
    };
    use ethers::types::{Address as EthAddress, TransactionReceipt};
    use aiy_json_rpc_types::{BcsEvent, AiyEvent};
    use aiy_types::bridge::{BridgeChainId, TOKEN_ID_USDC};
    use aiy_types::{base_types::AiyAddress, crypto::get_key_pair};

    #[tokio::test]
    async fn test_aiy_signer_with_cache() {
        let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        let signer = Arc::new(kp);
        let aiy_client_mock = AiyMockClient::default();
        let aiy_verifier = AiyActionVerifier {
            aiy_client: Arc::new(AiyClient::new_for_testing(aiy_client_mock.clone())),
        };
        let metrics = Arc::new(BridgeMetrics::new_for_testing());
        let mut aiy_signer_with_cache = SignerWithCache::new(signer.clone(), aiy_verifier, metrics);

        // Test `get_cache_entry` creates a new entry if not exist
        let aiy_tx_digest = TransactionDigest::random();
        let aiy_event_idx = 42;
        assert!(aiy_signer_with_cache
            .get_testing_only((aiy_tx_digest, aiy_event_idx))
            .await
            .is_none());
        let entry = aiy_signer_with_cache
            .get_cache_entry((aiy_tx_digest, aiy_event_idx))
            .await;
        let entry_ = aiy_signer_with_cache
            .get_testing_only((aiy_tx_digest, aiy_event_idx))
            .await;
        ...
        aiy_client_mock.add_events_by_tx_digest_error(aiy_tx_digest);
        aiy_signer_with_cache
            .sign((aiy_tx_digest, aiy_event_idx))
            .await
            .unwrap_err();
        let entry_ = aiy_signer_with_cache
            .get_testing_only((aiy_tx_digest, aiy_event_idx))
        ...
        aiy_client_mock.add_events_by_tx_digest(aiy_tx_digest, vec![]);
        assert!(matches!(
            aiy_signer_with_cache
                .sign((aiy_tx_digest, aiy_event_idx))
                .await,
        ...
        // TODO: test BridgeEventInUnrecognizedAiyPackage, AiyBridgeEvent::try_from_aiy_event
        // and BridgeEventNotActionable to be cached

        // Test `sign` caches Ok result
        let emitted_event_1 = MoveTokenDepositedEvent {
            seq_num: 1,
            source_chain: BridgeChainId::AiyCustom as u8,
        ...
        let aiy_tx_digest = aiy_event_1.id.tx_digest;

        let mut aiy_event_2 = AiyEvent::random_for_testing();
        aiy_event_2.type_ = AiyToEthTokenBridgeV1.get().unwrap().clone();
        aiy_event_2.bcs = BcsEvent::new(bcs::to_bytes(&emitted_event_1).unwrap());
        let aiy_event_idx_2 = 1;
        aiy_client_mock.add_events_by_tx_digest(aiy_tx_digest, vec![aiy_event_2.clone()]);

        aiy_client_mock.add_events_by_tx_digest(
            aiy_tx_digest,
            vec![aiy_event_1.clone(), aiy_event_2.clone()],
        );
        let signed_1 = aiy_signer_with_cache
            .sign((aiy_tx_digest, aiy_event_idx))
            .await
            .unwrap();
        let signed_2 = aiy_signer_with_cache
            .sign((aiy_tx_digest, aiy_event_idx_2))
            .await
            .unwrap();
    ...
    async fn test_signer_with_governace_verifier() {
        ...
        let action_2 = BridgeAction::LimitUpdateAction(LimitUpdateAction {
            chain_id: BridgeChainId::EthCustom,
            sending_chain_id: BridgeChainId::AiyCustom,
            nonce: 1,
            new_usd_limit: 10000,
        });
        ...
        let action_4 = get_test_aiy_to_eth_bridge_action(None, None, None, None, None, None, None);
        assert!(matches!(
