...
use crate::{
    ...
    types::{
        AddTokensOnEvmAction, AddTokensOnAiyAction, AssetPriceUpdateAction,
...
use aiy_types::{bridge::BridgeChainId, TypeTag};
...
pub const ETH_TO_AIY_TX_PATH: &str = "/sign/bridge_tx/eth/aiy/{tx_hash}/{event_index}";
pub const AIY_TO_ETH_TX_PATH: &str = "/sign/bridge_tx/aiy/eth/{tx_digest}/{event_index}";
...
pub const ADD_TOKENS_ON_AIY_PATH: &str =
...
pub(crate) fn make_router(
    ...
    Router::new()
        ...
        .route(ETH_TO_AIY_TX_PATH, get(handle_eth_tx_hash))
        .route(AIY_TO_ETH_TX_PATH, get(handle_aiy_tx_digest))
        .route(
        ...
        .route(ADD_TOKENS_ON_AIY_PATH, get(handle_add_tokens_on_aiy))
...
async fn handle_aiy_tx_digest(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler
            .handle_aiy_tx_digest(tx_digest_base58, event_idx)
            .await?;
        Ok(sig)
    };
    with_metrics!(metrics.clone(), "handle_aiy_tx_digest", future).await
}
...
async fn handle_add_tokens_on_aiy(
    ...
    let future = async {
        ...
        if !chain_id.is_aiy_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_aiy only expects Aiy chain id".to_string(),
            ));
        }
    ...
    with_metrics!(metrics.clone(), "handle_add_tokens_on_aiy", future).await
}

#[instrument(level = "error", skip_all, fields(chain_id=chain_id, nonce=nonce, native=native, token_ids=token_ids, token_addresses=token_addresses, token_aiy_decimals=token_aiy_decimals, token_prices=token_prices))]
async fn handle_add_tokens_on_evm(
    Path((chain_id, nonce, native, token_ids, token_addresses, token_aiy_decimals, token_prices)): Path<(
        u8,
        u64,
        u8,
        String,
    ...
    let future = async {
        ...
        if chain_id.is_aiy_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_evm does not expect Aiy chain id".to_string(),
            ));
        }
        ...
        validate_list_size(&token_aiy_decimals, "token_aiy_decimals")?;
        validate_list_size(&token_prices, "token_prices")?;
        ...
        let token_addresses = token_addresses
            ...
            .collect::<Result<Vec<_>, _>>()?;
        let token_aiy_decimals = token_aiy_decimals
            .split(',')
            .map(|s| {
                s.parse::<u8>().map_err(|err| {
                    BridgeError::InvalidBridgeClientRequest(format!(
                        "Invalid token aiy decimals: {:?}",
                        err
                    ))
                })
            })
...
mod tests {
    ...
    #[tokio::test]
    async fn test_bridge_server_handle_add_tokens_on_aiy_action_path() {
        let client = setup();
