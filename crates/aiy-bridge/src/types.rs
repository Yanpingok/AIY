...
use aiy_types::bridge::{
    ...
    APPROVAL_THRESHOLD_ADD_TOKENS_ON_AIY, BRIDGE_COMMITTEE_MAXIMAL_VOTING_POWER,
...
use aiy_types::digests::{Digest, TransactionDigest};
use aiy_types::message_envelope::{Envelope, Message, VerifiedEnvelope};
...
pub struct AiyToEthBridgeAction {
    // Digest of the transaction where the event was emitted
    pub aiy_tx_digest: TransactionDigest,
    // The index of the event in the transaction
    pub aiy_tx_event_index: u16,
    pub aiy_bridge_event: EmittedAiyToEthTokenBridgeV1,
}

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct EthToAiyBridgeAction {
    // Digest of the transaction where the event was emitted
    pub eth_tx_hash: EthTransactionHash,
    // The index of the event in the transaction
    pub eth_event_index: u16,
    pub eth_bridge_event: EthToAiyTokenBridgeV1,
}
...
pub struct LimitUpdateAction {
    ...
    // for the limit update. For example, if chain_id is EthMainnet and sending_chain_id is AiyMainnet,
    // it means we want to update the limit for the AiyMainnet to EthMainnet route.
    pub chain_id: BridgeChainId,
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct AddTokensOnAiyAction {
    pub nonce: u64,
    pub chain_id: BridgeChainId,
    pub native: bool,
...
pub enum BridgeAction {
    /// Aiy to Eth bridge action
    AiyToEthBridgeAction(AiyToEthBridgeAction),
    /// Eth to aiy bridge action
    EthToAiyBridgeAction(EthToAiyBridgeAction),
    ...
    AddTokensOnAiyAction(AddTokensOnAiyAction),
...
impl BridgeAction {
    ...
    pub fn chain_id(&self) -> BridgeChainId {
        match self {
            BridgeAction::AiyToEthBridgeAction(a) => a.aiy_bridge_event.aiy_chain_id,
            BridgeAction::EthToAiyBridgeAction(a) => a.eth_bridge_event.eth_chain_id,
            ...
            BridgeAction::AddTokensOnAiyAction(a) => a.chain_id,
    ...
    pub fn is_governace_action(&self) -> bool {
        match self.action_type() {
            ...
            BridgeActionType::AddTokensOnAiy => true,
    ...
    pub fn action_type(&self) -> BridgeActionType {
        match self {
            BridgeAction::AiyToEthBridgeAction(_) => BridgeActionType::TokenTransfer,
            BridgeAction::EthToAiyBridgeAction(_) => BridgeActionType::TokenTransfer,
            ...
            BridgeAction::AddTokensOnAiyAction(_) => BridgeActionType::AddTokensOnAiy,
    ...
    pub fn seq_number(&self) -> u64 {
        match self {
            BridgeAction::AiyToEthBridgeAction(a) => a.aiy_bridge_event.nonce,
            BridgeAction::EthToAiyBridgeAction(a) => a.eth_bridge_event.nonce,
            ...
            BridgeAction::AddTokensOnAiyAction(a) => a.nonce,
    ...
    pub fn approval_threshold(&self) -> u64 {
        match self {
            BridgeAction::AiyToEthBridgeAction(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            BridgeAction::EthToAiyBridgeAction(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            ...
            BridgeAction::AddTokensOnAiyAction(_) => APPROVAL_THRESHOLD_ADD_TOKENS_ON_AIY,
...
pub fn is_route_valid(one: BridgeChainId, other: BridgeChainId) -> bool {
    if one.is_aiy_chain() && other.is_aiy_chain() {
        return false;
    }
    if !one.is_aiy_chain() && !other.is_aiy_chain() {
        return false;
    ...
    if one == BridgeChainId::EthMainnet {
        return other == BridgeChainId::AiyMainnet;
    }
    if one == BridgeChainId::AiyMainnet {
        return other == BridgeChainId::EthMainnet;
    }
    if other == BridgeChainId::EthMainnet {
        return one == BridgeChainId::AiyMainnet;
    }
    if other == BridgeChainId::AiyMainnet {
...
mod tests {
    ...
    use crate::test_utils::get_test_eth_to_aiy_bridge_action;
    ...
    fn test_bridge_action_approval_threshold_regression_test() -> anyhow::Result<()> {
        let action = get_test_aiy_to_eth_bridge_action(None, None, None, None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);

        let action = get_test_eth_to_aiy_bridge_action(None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);
