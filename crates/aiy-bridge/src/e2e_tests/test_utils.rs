...
use crate::abi::{EthBridgeCommittee, EthBridgeEvent, EthERC20, EthAiyBridge, EthAiyBridgeEvents};
...
use crate::aiy_transaction_builder::build_committee_register_transaction;
...
use crate::types::{BridgeAction, BridgeActionStatus, AiyToEthBridgeAction};
...
use std::str::FromStr;
use std::sync::Arc;
use aiy_json_rpc_api::BridgeReadApiClient;
use aiy_json_rpc_types::AiyEvent;
use aiy_json_rpc_types::AiyExecutionStatus;
use aiy_json_rpc_types::AiyTransactionBlockEffectsAPI;
use aiy_json_rpc_types::AiyTransactionBlockResponse;
...
use aiy_types::base_types::{ObjectID, ObjectRef};
use aiy_types::bridge::get_bridge_obj_initial_shared_version;
use aiy_types::bridge::BridgeChainId;
use aiy_types::bridge::BridgeSummary;
use aiy_types::bridge::BridgeTrait;
use aiy_types::bridge::{get_bridge, BRIDGE_MODULE_NAME};
use aiy_types::bridge::{TOKEN_ID_BTC, TOKEN_ID_ETH, TOKEN_ID_USDC, TOKEN_ID_USDT};
use aiy_types::committee::{ProtocolVersion, TOTAL_VOTING_POWER};
...
use aiy_types::transaction::{ObjectArg, Transaction, TransactionData};
use aiy_types::{BRIDGE_PACKAGE_ID, AIY_BRIDGE_OBJECT_ID};
use tokio::join;
...
use crate::config::{BridgeNodeConfig, EthConfig, AiyConfig};
use crate::node::run_bridge_node;
use crate::aiy_client::AiyBridgeClient;
use anyhow::anyhow;
use ethers::prelude::*;
...
const AIY_BRIDGE_NAME: &str = "AiyBridge";
...
impl BridgeTestClusterBuilder {
    pub fn new() -> Self {
        BridgeTestClusterBuilder {
            ...
            aiy_chain_id: BridgeChainId::AiyCustom,
        }
    }
    ...
    pub fn with_aiy_chain_id(mut self, chain_id: BridgeChainId) -> Self {
        self.aiy_chain_id = chain_id;
        self
    }
    ...
    pub async fn build(self) -> BridgeTestCluster {
        ...
        let bridge_client =
            AiyBridgeClient::new(&test_cluster.inner.fullnode_handle.rpc_url, metrics)
                .await
                .unwrap();
        info!(
            "Bridge committee: {:?}",
        ...
        BridgeTestCluster {
            ...
            aiy_chain_id: self.aiy_chain_id,
...
impl BridgeTestCluster {
    ...
    pub fn bridge_client(&self) -> &AiyBridgeClient {
        &self.bridge_client
    }

    pub fn aiy_client(&self) -> &AiyClient {
        &self.test_cluster.inner.fullnode_handle.aiy_client
    }

    pub fn aiy_user_address(&self) -> AiyAddress {
        self.test_cluster.inner.get_address_0()
    }

    pub fn aiy_chain_id(&self) -> BridgeChainId {
        self.aiy_chain_id
    }
    ...
    pub fn aiy_bridge_address(&self) -> String {
        self.eth_environment.contracts().aiy_bridge_addrress_hex()
    }
    ...
    pub fn aiy_rpc_url(&self) -> String {
    ...
    pub async fn new_bridge_transactions(
        &mut self,
        assert_success: bool,
    ) -> Vec<AiyTransactionBlockResponse> {
        let resps = self
            .aiy_client()
            .read_api()
...
impl DeployedSolContracts {
    ...
    pub fn aiy_bridge_addrress_hex(&self) -> String {
        Self::eth_adress_to_hex(self.aiy_bridge)
    }
}
...
pub(crate) async fn deploy_sol_contract(
    ...
    let deploy_config = SolDeployConfig {
        ...
        aiy_decimals: vec![],     // this is set up in the deploy script
    ...
    let contracts = DeployedSolContracts {
        aiy_bridge: deployed_contracts.remove(AIY_BRIDGE_NAME).unwrap(),
...
impl EthBridgeEnvironment {
    ...
    pub async fn get_supported_token(&self, token_id: u8) -> (EthAddress, u8, u64) {
        ...
        let token_aiy_decimal = config.token_aiy_decimal_of(token_id).call().await.unwrap();
        let token_price = config.token_price_of(token_id).call().await.unwrap();
        (token_address, token_aiy_decimal, token_price)
    }
}
...
pub(crate) async fn start_bridge_cluster(
        ...
        .enumerate()
    {
        ...
        let config = BridgeNodeConfig {
            ...
            aiy: AiyConfig {
                aiy_rpc_url: test_cluster.inner.fullnode_handle.rpc_url.clone(),
                aiy_bridge_chain_id: BridgeChainId::AiyCustom as u8,
                bridge_client_key_path: None,
                bridge_client_gas_object: None,
                aiy_bridge_module_last_processed_event_id_override: None,
            },
...
pub async fn get_signatures(
    aiy_bridge_client: &AiyBridgeClient,
    nonce: u64,
    aiy_chain_id: u8,
) -> Vec<Bytes> {
    let sigs = aiy_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(aiy_chain_id, nonce)
        .await
        .unwrap();
...
impl TestClusterWrapperBuilder {
    ...
    pub async fn build(self) -> TestClusterWrapper {
        ...
        let gas_objects_for_authority_keys = self
            ...
            .iter()
            .map(|k| {
                let address = AiyAddress::from(k.public());
        ...
        for (node, kp) in reordered_nodes.zip(self.bridge_authority_keys.iter()) {
            let validator_address = node.config().aiy_address();
            ...
            tasks.push(async move {
                api_clone
                    .execute_transaction_block(
                        tx,
                        AiyTransactionBlockResponseOptions::new().with_effects(),
                        None,
                    )
                    .await
            });
        ...
        if self.deploy_tokens {
            ...
            let action = publish_and_register_coins_return_add_coins_on_aiy_action(
                test_cluster.wallet(),
                bridge_arg,
                vec![
        ...
        async fn await_committee_register_tasks(
            ...
            tasks: Vec<
                impl Future<Output = Result<AiyTransactionBlockResponse, aiy_sdk::error::Error>>,
            >,
        ) {
            ...
            for response in responses {
                if response.unwrap().effects.unwrap().status() != &AiyExecutionStatus::Success {
                    has_failure = true;
                }
            }
            if has_failure {
...
pub async fn initiate_bridge_eth_to_aiy(
    ...
    let aiy_chain_id = bridge_test_cluster.aiy_chain_id();
    let eth_chain_id = bridge_test_cluster.eth_chain_id();
    let token_id = TOKEN_ID_ETH;

    let aiy_amount = (U256::from(amount) * U256::exp10(8)).as_u64(); // DP for Ether on Aiy
    ...
    let eth_bridge_event = tx_receipt
        ...
        .unwrap();
    let EthBridgeEvent::EthAiyBridgeEvents(EthAiyBridgeEvents::TokensDepositedFilter(
        eth_bridge_event,
    )) = eth_bridge_event
    else {
        unreachable!();
    ...
    assert_eq!(eth_bridge_event.destination_chain_id, aiy_chain_id as u8);
    assert_eq!(eth_bridge_event.token_id, token_id);
    assert_eq!(eth_bridge_event.aiy_adjusted_amount, aiy_amount);
    assert_eq!(eth_bridge_event.sender_address, eth_address);
    assert_eq!(eth_bridge_event.recipient_address, aiy_address.to_vec());
    info!(
    ...
    .await
    .tap_ok(|_| {
        info!("Eth to Aiy bridge transfer claimed");
    })
}

pub async fn initiate_bridge_aiy_to_eth(
    bridge_test_cluster: &BridgeTestCluster,
    eth_address: EthAddress,
    token: ObjectRef,
    nonce: u64,
    aiy_amount: u64,
) -> Result<AiyToEthBridgeAction, anyhow::Error> {
    let bridge_object_arg = bridge_test_cluster
        .bridge_client()
        .get_mutable_bridge_object_arg_must_succeed()
        .await;
    let aiy_client = bridge_test_cluster.aiy_client();
    ...
    let resp = match deposit_eth_to_aiy_package(
        aiy_client,
        aiy_address,
    ...
    .await
    {
        Ok(resp) => {
            if !resp.status_ok().unwrap() {
                return Err(anyhow!("Aiy TX error"));
            } else {
                resp
            }
        }
    ...
    let aiy_events = resp.events.unwrap().data;
    let bridge_event = aiy_events
        .iter()
        .filter_map(|e| {
            let aiy_bridge_event = AiyBridgeEvent::try_from_aiy_event(e).unwrap()?;
            aiy_bridge_event.try_into_bridge_action(e.id.tx_digest, e.id.event_seq as u16)
        })
        .find_map(|e| {
            if let BridgeAction::AiyToEthBridgeAction(a) = e {
                Some(a)
            } else {
                None
            }
    ...
    assert_eq!(bridge_event.aiy_bridge_event.nonce, nonce);
    assert_eq!(
        bridge_event.aiy_bridge_event.aiy_chain_id,
        bridge_test_cluster.aiy_chain_id()
    );
    assert_eq!(
        bridge_event.aiy_bridge_event.eth_chain_id,
        bridge_test_cluster.eth_chain_id()
    );
    assert_eq!(bridge_event.aiy_bridge_event.aiy_address, aiy_address);
    assert_eq!(bridge_event.aiy_bridge_event.eth_address, eth_address);
    assert_eq!(bridge_event.aiy_bridge_event.token_id, TOKEN_ID_ETH);
    assert_eq!(
        bridge_event.aiy_bridge_event.amount_aiy_adjusted,
        aiy_amount
    );
    ...
    .await
    .unwrap();
    info!("Aiy to Eth bridge transfer approved.");
...
async fn deposit_eth_to_aiy_package(
    ...
    aiy_token_type_tags: &HashMap<u8, TypeTag>,
) -> Result<AiyTransactionBlockResponse, anyhow::Error> {
    ...
    builder.programmable_move_call(
        ...
        vec![aiy_token_type_tags.get(&TOKEN_ID_ETH).unwrap().clone()],
...
pub async fn initiate_bridge_erc20_to_aiy(
    bridge_test_cluster: &BridgeTestCluster,
    amount_u64: u64,
    ...
    let allowance_call = contract.approve(bridge_test_cluster.contracts().aiy_bridge, amount);
    ...
    let aiy_recipient_address = bridge_test_cluster.aiy_user_address();
    let aiy_chain_id = bridge_test_cluster.aiy_chain_id();
    let eth_chain_id = bridge_test_cluster.eth_chain_id();
    ...
    let deposit_call = contract.bridge_erc20(
        token_id,
        amount,
        aiy_recipient_address.to_vec().into(),
        aiy_chain_id as u8,
    );
    ...
    .tap_ok(|_| {
        info!(
            nonce,
            token_id, amount_u64, "Eth to Aiy bridge transfer claimed"
        );
    })
}
...
pub(crate) async fn deposit_native_eth_to_sol_contract(
    ...
    aiy_recipient_address: AiyAddress,
    aiy_chain_id: BridgeChainId,
    amount: u64,
) -> ContractCall<EthSigner, ()> {
    let contract = EthAiyBridge::new(contract_address, signer.clone().into());
    let aiy_recipient_address = aiy_recipient_address.to_vec().into();
    let amount = U256::from(amount) * U256::exp10(18); // 1 ETH
    contract
        .bridge_eth(aiy_recipient_address, aiy_chain_id as u8)
        .value(amount)
}
