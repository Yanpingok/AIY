...
use crate::types::AddTokensOnAiyAction;
...
use crate::types::EthToAiyBridgeAction;
...
use aiy_types::base_types::AIY_ADDRESS_LENGTH;
...
pub const ADD_TOKENS_ON_AIY_MESSAGE_VERSION: u8 = 1;
pub const ADD_TOKENS_ON_EVM_MESSAGE_VERSION: u8 = 1;

pub const BRIDGE_MESSAGE_PREFIX: &[u8] = b"AIY_BRIDGE_MESSAGE";
...
impl BridgeMessageEncoding for AiyToEthBridgeAction {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        let e = &self.aiy_bridge_event;
        // Add message type
        ...
        // Add source chain id
        bytes.push(e.aiy_chain_id as u8);
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        bytes.push(AIY_ADDRESS_LENGTH as u8);
        // Add source address
        bytes.extend_from_slice(&e.aiy_address.to_vec());
        // Add dest chain id
        ...
        // Add token amount
        bytes.extend_from_slice(&e.amount_aiy_adjusted.to_be_bytes());

        Ok(bytes)
    }
}

impl BridgeMessageEncoding for EthToAiyBridgeAction {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        // Add token amount
        bytes.extend_from_slice(&e.aiy_adjusted_amount.to_be_bytes());
...
impl BridgeMessageEncoding for AddTokensOnAiyAction {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        // Add message type
        bytes.push(BridgeActionType::AddTokensOnAiy as u8);
        // Add message version
        bytes.push(ADD_TOKENS_ON_AIY_MESSAGE_VERSION);
        // Add nonce
        bytes.extend_from_slice(&self.nonce.to_be_bytes());
        // Add chain id
        bytes.push(self.chain_id as u8);
...
impl BridgeMessageEncoding for AddTokensOnEvmAction {
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        // Add token aiy decimals
        bytes.push(u8::try_from(self.token_aiy_decimals.len())?);
        for token_aiy_decimal in &self.token_aiy_decimals {
            bytes.push(*token_aiy_decimal);
        }
...
mod tests {
    ...
    use crate::events::EmittedAiyToEthTokenBridgeV1;
    ...
    use std::str::FromStr;
    use aiy_types::base_types::{AiyAddress, TransactionDigest};
    use aiy_types::bridge::BridgeChainId;
    use aiy_types::bridge::TOKEN_ID_BTC;
    use aiy_types::bridge::TOKEN_ID_USDC;
    ...
    fn test_bridge_message_encoding() -> anyhow::Result<()> {
        ...
        let aiy_chain_id = BridgeChainId::AiyTestnet;
        let aiy_tx_event_index = 1u16;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let aiy_address = AiyAddress::random_for_testing_only();
        let eth_address = EthAddress::random();
        let token_id = TOKEN_ID_USDC;
        let amount_aiy_adjusted = 1_000_000;

        let aiy_bridge_event = EmittedAiyToEthTokenBridgeV1 {
            nonce,
            aiy_chain_id,
            eth_chain_id,
            aiy_address,
        ...
        let encoded_bytes = BridgeAction::AiyToEthBridgeAction(AiyToEthBridgeAction {
            aiy_tx_digest,
            aiy_tx_event_index,
            aiy_bridge_event,
        })
        ...
        let source_chain_id_bytes = vec![aiy_chain_id as u8]; // len: 1

        let aiy_address_length_bytes = vec![AIY_ADDRESS_LENGTH as u8]; // len: 1
        let aiy_address_bytes = aiy_address.to_vec(); // len: 32
        ...
        let token_amount_bytes = amount_aiy_adjusted.to_be_bytes().to_vec(); // len: 8
        ...
        combined_bytes.extend_from_slice(&aiy_address_length_bytes);
        combined_bytes.extend_from_slice(&aiy_address_bytes);
    ...
    #[test]
    fn test_bridge_message_encoding_regression_emitted_aiy_to_eth_token_bridge_v1(
    ) -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        ...
        let aiy_chain_id = BridgeChainId::AiyTestnet;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let aiy_address = AiyAddress::from_str(
    ...
    #[test]
    fn test_bridge_message_encoding_regression_eth_to_aiy_token_bridge_v1() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        ...
        let eth_address =
            EthAddress::from_str("0x00000000000000000000000000000000000000c8").unwrap();
        let token_id = TOKEN_ID_USDC;
        let aiy_adjusted_amount = 12345;

        let eth_bridge_event = EthToAiyTokenBridgeV1 {
            nonce,
            aiy_chain_id,
            eth_chain_id,
            aiy_address,
        ...
        let encoded_bytes = BridgeAction::EthToAiyBridgeAction(EthToAiyBridgeAction {
            eth_tx_hash,
            eth_event_index,
            eth_bridge_event,
        })
    ...
    #[test]
    fn test_bridge_message_encoding_regression_add_coins_on_aiy() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();

        let action = BridgeAction::AddTokensOnAiyAction(AddTokensOnAiyAction {
            nonce: 0,
            chain_id: BridgeChainId::AiyCustom,
            native: false,
