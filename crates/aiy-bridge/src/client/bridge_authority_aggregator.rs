...
use aiy_authority_aggregation::{quorum_map_then_reduce_with_timeout_and_prefs, SigRequestPrefs};
...
impl GetSigsState {
    ...
    fn handle_verified_signed_action(
        ...
        if self.total_ok_stake >= self.validity_threshold {
            ...
            let certified_action: aiy_types::message_envelope::Envelope<
                BridgeAction,
...
async fn request_sign_bridge_action_into_certification(
    ...
    let preference = match action {
        BridgeAction::AiyToEthBridgeAction(_) => Some(SigRequestPrefs {
            ordering_pref: BTreeSet::new(),
            prefetch_timeout,
        }),
        BridgeAction::EthToAiyBridgeAction(_) => None,
        _ => {
            if action.chain_id().is_aiy_chain() {
                None
            } else {
...
mod tests {
    ...
    use aiy_types::committee::VALIDITY_THRESHOLD;
