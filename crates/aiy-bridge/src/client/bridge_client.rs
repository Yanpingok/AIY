...
impl BridgeClient {
    ...
    fn bridge_action_to_path(event: &BridgeAction) -> String {
        match event {
            BridgeAction::AiyToEthBridgeAction(e) => format!(
                "sign/bridge_tx/aiy/eth/{}/{}",
                e.aiy_tx_digest, e.aiy_tx_event_index
            ),
            BridgeAction::EthToAiyBridgeAction(e) => format!(
                "sign/bridge_tx/eth/aiy/{}/{}",
            ...
            BridgeAction::AddTokensOnAiyAction(a) => {
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_aiy/{chain_id}/{nonce}/{native}/{token_ids}/{token_type_names}/{token_prices}"
                )
            }
            BridgeAction::AddTokensOnEvmAction(a) => {
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_evm/{chain_id}/{nonce}/{native}/{token_ids}/{token_addresses}/{token_aiy_decimals}/{token_prices}"
                )
            }
        }
    }
...
mod tests {
    ...
    use aiy_types::bridge::{BridgeChainId, TOKEN_ID_BTC, TOKEN_ID_USDT};
    ...
    async fn test_bridge_client() {
        ...
        let action =
            get_test_aiy_to_eth_bridge_action(None, Some(1), Some(1), Some(100), None, None, None);
    ...
    async fn test_bridge_client_request_sign_action() {
        ...
        mock_handler.add_aiy_event_response(tx_digest, event_idx, Ok(signed_event.clone()), None);
        ...
        let action2 = get_test_aiy_to_eth_bridge_action(
            Some(tx_digest),
            Some(event_idx),
            Some(2),
            Some(200),
        ...
        mock_handler.add_aiy_event_response(tx_digest, event_idx, Ok(wrong_signed_action), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_aiy_event_response(tx_digest, event_idx, Ok(signed_event), None);
        ...
        mock_handler.add_aiy_event_response(tx_digest, event_idx, Ok(signed_event2), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_aiy_event_response(tx_digest, event_idx, Ok(signed_event3), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
    ...
    fn test_bridge_action_path_regression_tests() {
        ...
        let aiy_tx_event_index = 5;
        let action = BridgeAction::AiyToEthBridgeAction(crate::types::AiyToEthBridgeAction {
            aiy_tx_digest,
            aiy_tx_event_index,
        ...
        assert_eq!(
            ...
            format!(
                "sign/bridge_tx/aiy/eth/{}/{}",
                aiy_tx_digest, aiy_tx_event_index
            )
        );

        let eth_tx_hash = TxHash::random();
        let eth_event_index = 6;
        let action = BridgeAction::EthToAiyBridgeAction(crate::types::EthToAiyBridgeAction {
            eth_tx_hash,
            eth_event_index,
        ...
        let action = BridgeAction::AddTokensOnAiyAction(crate::types::AddTokensOnAiyAction {
            nonce: 3,
            chain_id: BridgeChainId::AiyCustom,
            native: false,
