...
//! A mock implementation of Aiy JSON-RPC client.
...
use aiy_json_rpc_types::{EventFilter, EventPage, AiyEvent};
...
pub struct AiyMockClient {
    ...
    events_by_tx_digest:
        Arc<Mutex<HashMap<TransactionDigest, Result<Vec<AiyEvent>, aiy_sdk::error::Error>>>>,
    transaction_responses:
        Arc<Mutex<HashMap<TransactionDigest, BridgeResult<AiyTransactionBlockResponse>>>>,
    wildcard_transaction_response: Arc<Mutex<Option<BridgeResult<AiyTransactionBlockResponse>>>>,
...
impl AiyMockClient {
    ...
    pub fn add_events_by_tx_digest(&self, tx_digest: TransactionDigest, events: Vec<AiyEvent>) {
    ...
    pub fn add_events_by_tx_digest_error(&self, tx_digest: TransactionDigest) {
        self.events_by_tx_digest.lock().unwrap().insert(
            tx_digest,
            Err(aiy_sdk::error::Error::DataError("".to_string())),
        );
    }
    ...
    pub fn add_transaction_response(
        ...
        response: BridgeResult<AiyTransactionBlockResponse>,
    ) {
...
#[async_trait]
impl AiyClientInner for AiyMockClient {
    ...
    async fn get_events_by_tx_digest(
            ...
            .unwrap_or_else(|| panic!("No preset events found for tx_digest: {:?}", tx_digest))
        {
            ...
            // aiy_sdk::error::Error is not Clone
            Err(_) => Err(aiy_sdk::error::Error::DataError("".to_string())),
        }
    }
