...
use aiy_json_rpc_types::{
    AiyExecutionStatus, AiyTransactionBlockEffectsAPI, AiyTransactionBlockResponse,
};
use aiy_types::transaction::ObjectArg;
use aiy_types::TypeTag;
use aiy_types::{
    base_types::{ObjectID, ObjectRef, AiyAddress},
    crypto::{Signature, AiyKeyPair},
...
use crate::{
    ...
    aiy_client::{AiyClient, AiyClientInner},
    aiy_transaction_builder::build_aiy_transaction,
...
pub struct BridgeActionExecutor<C> {
    ...
    aiy_token_type_tags: Arc<ArcSwap<HashMap<u8, TypeTag>>>,
    ...
    C: AiyClientInner + 'static,
{
    pub async fn new(
        ...
        let bridge_object_arg = aiy_client
    ...
    fn run_inner(
        ...
        let client_clone = self.aiy_client.clone();
        let mut tasks = vec![];
    ...
    async fn should_proceed_signing(aiy_client: &Arc<AiyClient<C>>) -> bool {
        let Ok(Ok(is_paused)) =
            retry_with_max_elapsed_time!(aiy_client.is_bridge_paused(), Duration::from_secs(600))
        else {
    ...
    async fn handle_signing_task(
        ...
        let should_proceed = Self::should_proceed_signing(aiy_client).await;
        if !should_proceed {
        ...
        let aiy_client_clone = aiy_client.clone();
    ...
    async fn handle_already_processed_token_transfer_action_maybe(
        ...
        match status {
            ...
            // Although theoretically a legit AiyToEthBridgeAction should not have
            // status `NotFound`
    ...
    async fn request_signatures(
        ...
        match &action {
            BridgeAction::AiyToEthBridgeAction(_) | BridgeAction::EthToAiyBridgeAction(_) => (),
    ...
    // Before calling this function, `key` and `aiy_address` need to be
    // verified to match.
    ...
    async fn handle_execution_task(
        ...
        >,
        bridge_object_arg: &ObjectArg,
        aiy_token_type_tags: &ArcSwap<HashMap<u8, TypeTag>>,
        metrics: &Arc<BridgeMetrics>,
    ) {
        metrics
        ...
        let (gas_coin, gas_object_ref) =
            Self::get_gas_data_assert_ownership(*aiy_address, gas_object_id, aiy_client).await;
        ...
        if Self::handle_already_processed_token_transfer_action_maybe(
            aiy_client, action, store, metrics,
        )
        .await
        {
        ...
        info!("Building Aiy transaction");
        let rgp = aiy_client.get_reference_gas_price_until_success().await;
        let tx_data = match build_aiy_transaction(
            *aiy_address,
            &gas_object_ref,
            ceriticate_clone,
            *bridge_object_arg,
            aiy_token_type_tags.load().as_ref(),
            rgp,
        ) {
            Ok(tx_data) => tx_data,
            Err(err) => {
                metrics.err_build_aiy_transaction.inc();
                error!(
        ...
        let sig = Signature::new_secure(
            &IntentMessage::new(Intent::aiy_transaction(), &tx_data),
            aiy_key,
        );
        ...
        info!(?tx_digest, ?gas_object_ref, "Sending transaction to Aiy");
        match aiy_client
            ...
            .await
        {
            ...
            Err(err) => {
                error!(
                    ?action_key,
                    ?tx_digest,
                    "Aiy transaction failed at signing: {err:?}"
                );
                metrics.err_aiy_transaction_submission.inc();
                let metrics_clone = metrics.clone();
                ...
                spawn_logged_monitored_task!(async move {
                    ...
                    if attempt_times >= MAX_EXECUTION_ATTEMPTS {
                        metrics_clone
                            .err_aiy_transaction_submission_too_many_failures
                            .inc();
    ...
    async fn handle_execution_effects(
        ...
        match status {
            AiyExecutionStatus::Success => {
                ...
                info!(?tx_digest, "Aiy transaction executed successfully");
                // track successful approval and claim events
                relevant_events.iter().for_each(|e| {
                    if e.type_ == *TokenTransferClaimed.get().unwrap() {
                        match action {
                            BridgeAction::EthToAiyBridgeAction(_) => {
                                metrics.eth_aiy_token_transfer_claimed.inc();
                            }
                            BridgeAction::AiyToEthBridgeAction(_) => {
                                metrics.aiy_eth_token_transfer_claimed.inc();
                            }
                        ...
                        match action {
                            BridgeAction::EthToAiyBridgeAction(_) => {
                                metrics.eth_aiy_token_transfer_approved.inc();
                            }
                            BridgeAction::AiyToEthBridgeAction(_) => {
                                metrics.aiy_eth_token_transfer_approved.inc();
                            }
            ...
            AiyExecutionStatus::Failure { error } => {
                ...
                metrics.err_aiy_transaction_execution.inc();
                error!(?tx_digest, "Manual intervention is needed. Aiy transaction executed and failed with error: {error:?}");
            }
        }
    }
    ...
    async fn get_gas_data_assert_ownership(
        aiy_address: AiyAddress,
        gas_object_id: ObjectID,
        aiy_client: &AiyClient<C>,
    ) -> (GasCoin, ObjectRef) {
        let (gas_coin, gas_obj_ref, owner) = aiy_client
        ...
        assert_eq!(
            owner,
            Owner::AddressOwner(aiy_address),
...
mod tests {
    ...
    use std::str::FromStr;
    use aiy_json_rpc_types::AiyTransactionBlockEffects;
    use aiy_json_rpc_types::AiyTransactionBlockEvents;
    use aiy_json_rpc_types::{AiyEvent, AiyTransactionBlockResponse};
    use aiy_types::crypto::get_key_pair;
    use aiy_types::gas_coin::GasCoin;
    use aiy_types::TypeTag;
    use aiy_types::{base_types::random_object_ref, transaction::TransactionData};
    ...
    use crate::{
        ...
        aiy_mock_client::AiyMockClient,
        test_utils::{
            get_test_authorities_and_run_mock_bridge_server, get_test_eth_to_aiy_bridge_action,
            get_test_aiy_to_eth_bridge_action, sign_action_with_key,
        },
    ...
    async fn test_onchain_execution_loop() {
        ...
        let id_token_map = (*aiy_token_type_tags.load().clone()).clone();
        let tx_data = build_aiy_transaction(
            aiy_address,
            &gas_object_ref,
            action_certificate,
        ...
        let tx_digest = get_tx_digest(tx_data, &dummy_aiy_key);

        let gas_coin = GasCoin::new_for_testing(1_000_000_000_000); // dummy gas coin
        aiy_client_mock.add_gas_object_info(
            gas_coin.clone(),
            gas_object_ref,
            Owner::AddressOwner(aiy_address),
        );
        ...
        let mut event = AiyEvent::random_for_testing();
    ...
    async fn test_signature_aggregation_loop() {
        ...
        let id_token_map = (*aiy_token_type_tags.load().clone()).clone();
        let (action_certificate, aiy_tx_digest, aiy_tx_event_index) =
        ...
        loop {
            let requested_times =
                mock0.get_aiy_token_events_requested(aiy_tx_digest, aiy_tx_event_index);
    ...
    async fn test_skip_request_signature_if_already_processed_on_chain() {
        let (
            ...
            _gas_object_ref,
            _aiy_address,
            _aiy_token_type_tags,
            _bridge_pause_tx,
        ) = setup().await;

        let aiy_tx_digest = TransactionDigest::random();
        let aiy_tx_event_index = 0;
        let action = get_test_aiy_to_eth_bridge_action(
            Some(aiy_tx_digest),
            Some(aiy_tx_event_index),
            None,
            None,
            None,
            None,
        ...
        aiy_client_mock.set_action_onchain_status(&action, BridgeActionStatus::Approved);
    ...
    async fn test_skip_tx_submission_if_already_processed_on_chain() {
        ...
        aiy_client_mock.set_action_onchain_status(&action, BridgeActionStatus::Pending);
    ...
    async fn test_skip_tx_submission_if_bridge_is_paused() {
        ...
        let id_token_map: HashMap<u8, TypeTag> = (*aiy_token_type_tags.load().clone()).clone();
        let (action_certificate, _, _) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            None,
    ...
    async fn test_action_executor_handle_new_token() {
        ...
        let mut id_token_map: HashMap<u8, TypeTag> = (*aiy_token_type_tags.load().clone()).clone();
        let (action_certificate, _, _) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            Some(new_token_id),
            false, // we need an eth -> aiy action that entails the new token type tag in transaction building
        );
        ...
        aiy_token_type_tags.store(Arc::new(id_token_map));
    ...
    fn mock_bridge_authority_sigs(
        ...
        aiy_tx_digest: TransactionDigest,
        aiy_tx_event_index: u16,
    ...
    fn get_bridge_authority_approved_action(
        ...
        let aiy_tx_digest = TransactionDigest::random();
        let aiy_tx_event_index = 1;
        let action = if aiy_to_eth {
            get_test_aiy_to_eth_bridge_action(
                Some(aiy_tx_digest),
                Some(aiy_tx_event_index),
                None,
                None,
            ...
            get_test_eth_to_aiy_bridge_action(None, None, None, token_id)
        };

        let sigs =
            mock_bridge_authority_sigs(mocks, &action, secrets, aiy_tx_digest, aiy_tx_event_index);
        let certified_action = CertifiedBridgeAction::new_from_data_and_sig(
            action,
            BridgeCommitteeValiditySignInfo { signatures: sigs },
        );
    ...
    fn get_tx_digest(tx_data: TransactionData, dummy_aiy_key: &AiyKeyPair) -> TransactionDigest {
        let sig = Signature::new_secure(
            &IntentMessage::new(Intent::aiy_transaction(), &tx_data),
            dummy_aiy_key,
        );
    ...
    fn mock_transaction_response(
        aiy_client_mock: &AiyMockClient,
        tx_digest: TransactionDigest,
        status: AiyExecutionStatus,
        events: Option<Vec<AiyEvent>>,
        wildcard: bool,
    ) {
        let mut response = AiyTransactionBlockResponse::new(tx_digest);
        let effects = AiyTransactionBlockEffects::new_for_testing(tx_digest, status);
        if let Some(events) = events {
            response.events = Some(AiyTransactionBlockEvents { data: events });
        }
        response.effects = Some(effects);
        if wildcard {
            aiy_client_mock.set_wildcard_transaction_response(Ok(response));
        } else {
            aiy_client_mock.add_transaction_response(tx_digest, Ok(response));
        }
    }
    ...
    fn mock_transaction_error(
        ...
        if wildcard {
            aiy_client_mock.set_wildcard_transaction_response(Err(error));
        } else {
            aiy_client_mock.add_transaction_response(tx_digest, Err(error));
        }
    }
    ...
    async fn setup() -> (
        ...
        let (aiy_address, kp): (_, fastcrypto::secp256k1::Secp256k1KeyPair) = get_key_pair();
        let aiy_key = AiyKeyPair::from(kp);
        let gas_object_ref = random_object_ref();
        let temp_dir = tempfile::tempdir().unwrap();
        let store = BridgeOrchestratorTables::new(temp_dir.path());
        let aiy_client_mock = AiyMockClient::default();
        let tx_subscription = aiy_client_mock.subscribe_to_requested_transactions();
        let aiy_client = Arc::new(AiyClient::new_for_testing(aiy_client_mock.clone()));
        ...
        let dummy_aiy_key = AiyKeyPair::from(dummy_kp);
        ...
        let aiy_token_type_tags = aiy_client.get_token_id_map().await.unwrap();
        let aiy_token_type_tags = Arc::new(ArcSwap::new(Arc::new(aiy_token_type_tags)));
