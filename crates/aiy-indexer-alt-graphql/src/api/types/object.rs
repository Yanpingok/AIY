...
use aiy_indexer_alt_schema::{objects::StoredObjVersion, schema::obj_versions};
use aiy_pg_db::sql;
use aiy_types::{
    base_types::{
        SequenceNumber, AiyAddress as NativeAiyAddress, TransactionDigest, VersionDigest,
    },
...
#[graphql(
    ...
    field(
        ...
        desc = "The AIY returned to the sponsor or sender of the transaction that modifies or deletes this object."
    ),
    field(
...
/// An Object on Aiy is either a typed value (a Move Object) or a Package (modules containing functions and types).
///
/// Every object on Aiy is identified by a unique address, and has a version number that increases with every modification. Objects also hold metadata detailing their current owner (who can sign for access to the object and whether that access can modify and/or delete the object), and the digest of the last transaction that modified the object.
#[Object]
impl Object {
    /// The Object's ID.
    pub(crate) async fn address(&self, ctx: &Context<'_>) -> Result<AiyAddress, RpcError> {
    ...
    /// Fetch the total balance for coins with marker type `coinType` (e.g. `0x2::aiy::AIY`), owned by this address.
    ///
    ...
    /// The domain explicitly configured as the default AiyNS name for this address.
    pub(crate) async fn default_aiyns_name(
        &self,
        ctx: &Context<'_>,
    ) -> Result<Option<String>, RpcError> {
        self.super_.default_aiyns_name(ctx).await
    }
    ...
    /// Fetch the total balances keyed by coin types (e.g. `0x2::aiy::AIY`) owned by this address.
    ///
    ...
    /// The AIY returned to the sponsor or sender of the transaction that modifies or deletes this object.
...
impl Object {
    ...
    /// checkpoint.
    pub(crate) fn with_address(scope: Scope, address: NativeAiyAddress) -> Self {
        Self {
    ...
    fn from_stored_version(
        ...
        let address = Address::with_address(
            scope,
            NativeAiyAddress::from_bytes(stored.object_id)
                .context("Failed to deserialize AiyAddress")?,
        );
