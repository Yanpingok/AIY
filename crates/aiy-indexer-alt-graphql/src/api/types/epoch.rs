...
use aiy_indexer_alt_reader::cp_sequence_numbers::CpSequenceNumberKey;
use aiy_indexer_alt_reader::{
    epochs::{CheckpointBoundedEpochStartKey, EpochEndKey, EpochStartKey},
    pg_reader::PgReader,
};
use aiy_indexer_alt_schema::cp_sequence_numbers::StoredCpSequenceNumbers;
use aiy_indexer_alt_schema::epochs::{StoredEpochEnd, StoredEpochStart};
use aiy_types::messages_checkpoint::CheckpointCommitment;
...
/// Activity on Aiy is partitioned in time, into epochs.
///
...
impl Epoch {
    ...
    async fn validator_set(&self, ctx: &Context<'_>) -> Result<Option<ValidatorSet>, RpcError> {
        ...
        let validator_set_v1 = match system_state {
            AiySystemState::V1(inner) => inner.validators,
            AiySystemState::V2(inner) => inner.validators,
            #[cfg(msim)]
            AiySystemState::SimTestV1(_)
            | AiySystemState::SimTestShallowV2(_)
            | AiySystemState::SimTestDeepV2(_) => return Ok(None),
        };
    ...
    /// AIY set aside to account for objects stored on-chain, at the start of the epoch.
    ...
    async fn storage_fund(&self, ctx: &Context<'_>) -> Result<Option<StorageFund>, RpcError> {
        ...
        let storage_fund = match system_state {
            AiySystemState::V1(inner) => inner.storage_fund.into(),
            AiySystemState::V2(inner) => inner.storage_fund.into(),
            #[cfg(msim)]
            AiySystemState::SimTestV1(_)
            | AiySystemState::SimTestShallowV2(_)
            | AiySystemState::SimTestDeepV2(_) => return Ok(None),
    ...
    /// The value of the `version` field of `0x5`, the `0x3::aiy::AiySystemState` object.
    ...
    async fn system_parameters(
        ...
        let system_parameters = match system_state {
            AiySystemState::V1(inner) => from_system_parameters_v1(inner.parameters),
            AiySystemState::V2(inner) => from_system_parameters_v2(inner.parameters),
            #[cfg(msim)]
            AiySystemState::SimTestV1(_)
            | AiySystemState::SimTestShallowV2(_)
            | AiySystemState::SimTestDeepV2(_) => return Ok(None),
    ...
    async fn system_stake_subsidy(
        ...
        let stake_subsidy = match system_state {
            AiySystemState::V1(inner) => from_stake_subsidy_v1(inner.stake_subsidy),
            AiySystemState::V2(inner) => from_stake_subsidy_v1(inner.stake_subsidy),
            #[cfg(msim)]
            AiySystemState::SimTestV1(_)
            | AiySystemState::SimTestShallowV2(_)
            | AiySystemState::SimTestDeepV2(_) => return Ok(None),
...
impl Epoch {
    ...
    async fn system_state(&self, ctx: &Context<'_>) -> Result<Option<AiySystemState>, RpcError> {
        let Some(start) = self.start(ctx).await? else {
            return Ok(None);
        };

        let system_state = bcs::from_bytes::<AiySystemState>(&start.system_state)
