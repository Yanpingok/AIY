...
use aiy_indexer_alt_schema::transactions::BalanceChange as NativeBalanceChange;
    ...
    async fn execution_error(&self) -> Result<Option<ExecutionError>, RpcError> {
        ...
        let programmable_tx = content
            ...
            .and_then(|tx_data| match tx_data.into_kind() {
                aiy_types::transaction::TransactionKind::ProgrammableTransaction(tx) => Some(tx),
                _ => None,
            });
...
impl TransactionEffects {
    ...
    pub(crate) fn from_execution_response(
        scope: Scope,
        response: aiy_rpc::proto::aiy::rpc::v2beta2::ExecuteTransactionResponse,
        transaction_data: TransactionData,
        signatures: Vec<GenericSignature>,
    ) -> Result<Self, RpcError> {
    ...
    pub(crate) fn from_simulation_response(
        scope: Scope,
        response: aiy_rpc::proto::aiy::rpc::v2beta2::SimulateTransactionResponse,
        transaction_data: TransactionData,
    ) -> Result<Self, RpcError> {
        let executed_transaction = response
            .transaction
