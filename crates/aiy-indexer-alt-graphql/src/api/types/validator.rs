...
use aiy_types::aiy_system_state::aiy_system_state_inner_v1::ValidatorV1;

use crate::{
    api::{
        scalars::{
            base64::Base64, big_int::BigInt, aiy_address::AiyAddress, type_filter::TypeInput,
            uint53::UInt53,
        },
        types::{
            address::Address,
...
impl Validator {
    ...
    /// The exchange rate is used to determine the amount of AIY tokens that each past AIY staker can withdraw in the future.
    ...
    async fn staking_pool_aiy_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(self.native.staking_pool.aiy_balance))
    }
    ...
    async fn pending_total_aiy_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.native.staking_pool.pending_total_aiy_withdraw,
        ))
    }
    ...
    /// The total number of AIY tokens in this pool plus the pending stake amount for this epoch.
...
impl Validator {
    pub(crate) fn from_validator_v1(scope: Scope, native: ValidatorV1, at_risk: u64) -> Self {
        Self {
            super_: Address::with_address(scope, native.metadata.aiy_address),
            native,
            at_risk,
        }
    }
