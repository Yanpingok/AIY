...
use aiy_types::{base_types::AiyAddress, transaction::GasData};
...
impl GasInput {
    ...
    async fn gas_sponsor(&self) -> Option<Address> {
        (self.native.owner != AiyAddress::ZERO)
    ...
    /// The maximum AIY that can be expended by executing this transaction
    ...
    async fn gas_payment(
        ...
        if self.native.owner == AiyAddress::ZERO {
