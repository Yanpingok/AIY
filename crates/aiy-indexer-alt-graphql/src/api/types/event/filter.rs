...
use crate::{
    api::{
        scalars::{
            module_filter::ModuleFilter, aiy_address::AiyAddress, type_filter::TypeFilter,
            uint53::UInt53,
        },
...
pub(crate) struct EventFilter {
    ...
    ///
    /// Generic types can be queried by either the generic type name, e.g. `0x2::coin::Coin`, or by the full type name, such as `0x2::coin::Coin<0x2::aiy::AIY>`.
...
impl EventFilter {
    ...
    pub(crate) fn matches(&self, event: &NativeEvent) -> bool {
        if let Some(sender) = &self.sender {
            if sender != &AiyAddress::from(event.sender) {
                return false;
            }
        }

        if let Some(module) = &self.module {
            if module.package() != AiyAddress::from(event.package_id) {
                return false;
            }
        ...
        if let Some(type_) = &self.type_ {
            if type_.package() != AiyAddress::from(event.type_.address) {
                return false;
            }
