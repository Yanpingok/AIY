...
use aiy_indexer_alt_reader::{displays::DisplayKey, pg_reader::PgReader};
use aiy_types::{display::DisplayVersionUpdatedEvent, proto_value::ProtoVisitorBuilder, TypeTag};
use tokio::join;
...
impl MoveValue {
    ...
    async fn display(&self, ctx: &Context<'_>) -> Result<Option<Display>, RpcError> {
        ...
        for (field, value) in
            aiy_display::v1::Format::parse(limits.max_display_field_depth, &event.fields)
