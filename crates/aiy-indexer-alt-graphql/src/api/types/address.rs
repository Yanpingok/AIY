...
use aiy_types::{base_types::AiyAddress as NativeAiyAddress, dynamic_field::DynamicFieldType};

use crate::{
    api::{
        scalars::{owner_kind::OwnerKind, aiy_address::AiyAddress, type_filter::TypeInput},
...
#[graphql(
    ...
    field(
        ...
        desc = "Fetch the total balance for coins with marker type `coinType` (e.g. `0x2::aiy::AIY`), owned by this address.\n\nIf the address does not own any coins of that type, a balance of zero is returned.",
    ),
    field(
        name = "balances",
...
impl Address {
    /// Construct an address that is represented by just its identifier (`AiyAddress`).
