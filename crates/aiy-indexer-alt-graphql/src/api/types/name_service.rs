...
use aiy_name_service::{Domain as NativeDomain, NameRecord, NameServiceConfig};
...
/// Attempt to translate the given AiyNS `domain` to its address, as long as the mapping exists,
...
/// Attempt to translate the given `address` to its default AiyNS name, as long as the reverse
...
async fn reverse_record(
    ...
    scope: Scope,
    address: AiyAddress,
) -> Result<Option<Field<AiyAddress, Domain>>, RpcError> {
    let config: &NameServiceConfig = ctx.data()?;
