...
use crate::{
    api::scalars::{owner_kind::OwnerKind, aiy_address::AiyAddress, type_filter::TypeFilter},
    intersect,
};
...
mod tests {
    ...
    fn test_type_filter_intersection() {
        ...
        let aiy = ObjectFilter {
            owner_kind: None,
            owner: None,
            type_: Some("0x2::coin::Coin<0x2::aiy::AIY>".parse().unwrap()),
        };
        ...
        // intersection order.
        assert_eq!(coin.clone().intersect(aiy.clone()).unwrap(), aiy);
        assert_eq!(aiy.clone().intersect(coin.clone()).unwrap(), aiy);
    ...
    fn test_combined_intersection() {
        ...
        assert_eq!(a_coin.clone().intersect(a_aiy.clone()).unwrap(), a_aiy);
