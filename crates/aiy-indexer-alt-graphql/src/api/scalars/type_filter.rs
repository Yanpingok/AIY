...
use aiy_types::{
    parse_aiy_address, parse_aiy_module_id, parse_aiy_struct_tag, parse_aiy_type_tag, TypeTag,
};

use crate::api::scalars::{impl_string_input, aiy_address::AiyAddress};
...
impl TypeFilter {
    ...
    pub(crate) fn package(&self) -> AiyAddress {
        match self {
...
impl FromStr for TypeFilter {
    ...
    fn from_str(s: &str) -> Result<Self, TypeFilterError> {
        ...
        } else if let Ok(module) = parse_aiy_module_id(s) {
...
mod tests {
    ...
    fn test_parse_type_filter() {
        ...
        let filter = TypeFilter::from_str("0x2::coin::Coin<0x2::aiy::AIY>").unwrap();
        assert!(matches!(filter, TypeFilter::Type(ref t) if !t.type_params.is_empty()));
    }
    ...
    fn test_type_filter_intersect() {
        ...
        let type_with_params = TypeFilter::from_str("0x2::coin::Coin<0x2::aiy::AIY>").unwrap();
