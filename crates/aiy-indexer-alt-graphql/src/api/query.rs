...
use aiy_indexer_alt_reader::fullnode_client::{Error::GrpcExecutionError, FullnodeClient};
...
use super::{
    scalars::{
        digest::Digest, domain::Domain, aiy_address::AiyAddress, type_filter::TypeInput,
        uint53::UInt53,
    },
    types::{
        address::Address,
...
impl Query {
    ...
    /// Look-up an account by its AiyNS name, assuming it has a valid, unexpired name registration.
    async fn aiyns_name(
        &self,
        ctx: &Context<'_>,
        address: Domain,
    ...
    ///
    /// Accepts a JSON transaction matching the [Aiy gRPC API schema](https://docs.sui.io/references/fullnode-protocol#sui-rpc-v2-Transaction).
