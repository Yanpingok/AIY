...
diesel setup                                                                       \
    ...
    --migration-dir ../aiy-indexer-alt-schema/migrations
```
...
cargo nextest run                \
    -p aiy-indexer-alt           \
    -p aiy-indexer-alt-framework \
    -p aiy-indexer-alt-e2e-tests
```

The first package is the indexer's own unit tests, the second is the indexing
framework's unit tests, and the third is an end-to-end test aiyte that includes
...
```sh
cargo run --bin aiy-indexer-alt -- generate-config > indexer_alt_config.toml
```
...
```sh
cargo run --bin aiy-indexer-alt -- indexer               \
  --database-url {url}                                   \
