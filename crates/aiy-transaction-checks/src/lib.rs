...
mod checked {
    ...
    use aiy_types::error::{AiyResult, UserInputError, UserInputResult};
    use aiy_types::executable_transaction::VerifiedExecutableTransaction;
    use aiy_types::metrics::BytecodeVerifierMetrics;
    ...
    ) -> AiyResult<(AiyGasStatus, CheckedInputObjects)> {
    ...
    pub fn check_dev_inspect_input(
        ...
        let mut used_objects: HashSet<AiyAddress> = HashSet::new();
    ...
    fn check_gas(
        ...
        if tx_kind.is_system_tx() {
            Ok(AiyGasStatus::new_unmetered())
        } else {
            let gas_status =
                AiyGasStatus::new(gas_budget, gas_price, reference_gas_price, protocol_config)?;
    ...
    pub fn check_non_system_packages_to_be_published(
        ...
        let mut verifier = aiy_execution::verifier(protocol_config, signing_limits, metrics);
