...
use aiy_types::{
    ...
    MOVE_STDLIB_PACKAGE_ID, AIY_FRAMEWORK_PACKAGE_ID, AIY_SYSTEM_PACKAGE_ID,
};
use aiy_types::{BRIDGE_PACKAGE_ID, DEEPBOOK_PACKAGE_ID};
use tracing::error;
...
pub struct SystemPackageMetadata {
    ...
    pub name: String,
    /// The path within the repo to the source (e.g. "crates/aiy-framework/packages/move-stdlib")
    pub path: String,
        ...
        static PACKAGES: LazyLock<Vec<SystemPackageMetadata>> = LazyLock::new(|| {
            vec![
                $(SystemPackageMetadata::new(
                    $name,
                    concat!("crates/aiy-framework/packages/", $path),
                    $id,
