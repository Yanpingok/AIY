...
fun test_bridge_and_claim() {
    ...
    //
    // move from eth and transfer to aiy account
    let transfer_id1 = env.bridge_to_aiy<ETH>(
        source_chain,
        eth_address,
        aiy_address,
    ...
    let transfer_id = env.bridge_to_aiy<ETH>(
        source_chain,
        eth_address,
        aiy_address,
        amount,
    );
    let token = env.claim_token<ETH>(aiy_address, source_chain, transfer_id);
    env.send_token<ETH>(
        aiy_address,
        source_chain,
        eth_address,
