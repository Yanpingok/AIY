...
module bridge::bridge_env {
    ...
    use aiy::address;
    use aiy::clock::Clock;
    use aiy::coin::{Self, Coin, CoinMetadata, TreasuryCap};
    use aiy::ecdsa_k1::{KeyPair, secp256k1_keypair_from_seed, secp256k1_sign};
    use aiy::event;
    use aiy::package::UpgradeCap;
    use aiy::test_scenario::{Self, Scenario};
    use aiy::test_utils::destroy;
    use aiy_system::governance_test_utils::{
        advance_epoch_with_reward_amounts,
        create_aiy_system_state_for_testing,
        create_validator_for_testing
    };
    use aiy_system::aiy_system::{validator_voting_powers_for_testing, AiySystemState};
    ...
    public fun create_env(chain_id: u8): BridgeEnv {
        ...
        let mut clock = aiy::clock::create_for_testing(ctx);
    ...
    public fun setup_validators(env: &mut BridgeEnv, validators_info: vector<ValidatorInfo>) {
        ...
        create_aiy_system_state_for_testing(validators, 0, 0, ctx);
    ...
    public fun register_committee(env: &mut BridgeEnv) {
        ...
        let mut system_state = test_scenario::take_shared<AiySystemState>(
            scenario,
        );
    ...
    fun add_default_tokens(env: &mut BridgeEnv, sender: address) {
        ...
        let add_token_message = create_add_tokens_on_aiy_message(
            env.chain_id,
            bridge.get_seq_num_for(message_types::add_tokens_on_aiy()),
            false,
    ...
    const AIY_MESSAGE_PREFIX: vector<u8> = b"AIY_BRIDGE_MESSAGE";

    fun sign_message(env: &BridgeEnv, message: BridgeMessage): vector<vector<u8>> {
        let mut message_bytes = AIY_MESSAGE_PREFIX;
    ...
    public fun bridge_to_aiy<Token>(
        env: &mut BridgeEnv,
        source_chain: u8,
        source_address: vector<u8>,
        target_address: address,
...
module bridge::test_token {
    ...
    use std::type_name;
    use aiy::address;
    use aiy::coin::{CoinMetadata, TreasuryCap, create_currency};
    use aiy::hex;
    use aiy::package::{UpgradeCap, test_publish};
    use aiy::test_utils::create_one_time_witness;
