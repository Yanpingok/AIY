...
use aiy::address;
use aiy::clock::Clock;
use aiy::coin::{Coin, TreasuryCap, CoinMetadata};
use aiy::event;
use aiy::linked_table::{Self, LinkedTable};
use aiy::package::UpgradeCap;
use aiy::vec_map::{Self, VecMap};
use aiy::versioned::{Self, Versioned};
...
const EInvariantAiyInitializedTokenTransferShouldNotBeClaimed: u64 = 10;
...
// Record bridge message approvals in Aiy, called by the bridge client
...
public fun approve_token_transfer(
    ...
    // retrieve pending message if source chain is Aiy, the initial message
    // must exist on chain
    if (message.source_chain() == inner.chain_id) {
        ...
        assert!(!record.claimed, EInvariantAiyInitializedTokenTransferShouldNotBeClaimed);
...
public fun execute_system_message(
    ...
    } else if (message_type == message_types::add_tokens_on_aiy()) {
        let payload = message.extract_add_tokens_on_aiy();
        inner.execute_add_tokens_on_aiy(payload);
    } else {
...
fun execute_add_tokens_on_aiy(inner: &mut BridgeInner, payload: AddTokenOnAiy) {
...
#[test_only]
public fun test_execute_add_tokens_on_aiy(bridge: &mut Bridge, payload: AddTokenOnAiy) {
    let inner = load_inner_mut(bridge);
    inner.execute_add_tokens_on_aiy(payload);
}
