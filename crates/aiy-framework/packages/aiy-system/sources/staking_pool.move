...
#[allow(unused_const)]
module aiy_system::staking_pool;
...
const MIN_STAKING_THRESHOLD: u64 = 1_000_000_000; // 1 AIY

const EInsufficientPoolTokenBalance: u64 = 0;
const EWrongPool: u64 = 1;
const EWithdrawAmountCannotBeZero: u64 = 2;
const EInsufficientAiyTokenBalance: u64 = 3;
...
const EIncompatibleStakedAiy: u64 = 12;
...
const EDelegationOfZeroAiy: u64 = 17;
const EStakedAiyBelowThreshold: u64 = 18;
const ECannotMintFungibleStakedAiyYet: u64 = 19;
...
public struct StakingPool has key, store {
    ...
    pending_total_aiy_withdraw: u64,
...
public struct StakedAiy has key, store {
    id: UID,
    ...
    principal: Balance<AIY>,
}

/// An alternative to `StakedAiy` that holds the pool token amount instead of the AIY balance.
...
public struct FungibleStakedAiy has key, store {
    id: UID,
    /// ID of the staking pool we are staking with.
    pool_id: ID,
...
public struct FungibleStakedAiyData has key, store {
    id: UID,
...
// === dynamic field keys ===
public struct FungibleStakedAiyDataKey has copy, drop, store {}
...
public struct UnderflowAiyBalance has copy, drop, store {}
...
public(package) fun request_add_stake(
    ...
    let aiy_amount = stake.value();
    assert!(!pool.is_inactive(), EDelegationToInactivePool);
    assert!(aiy_amount > 0, EDelegationOfZeroAiy);

    pool.pending_stake = pool.pending_stake + aiy_amount;
    StakedAiy {
        id: object::new(ctx),
...
public(package) fun request_withdraw_stake(
    ...
    if (staked_aiy.stake_activation_epoch > ctx.epoch() && !pool.is_preactive()) {
        let principal = staked_aiy.into_balance();
    ...
    let total_aiy_withdraw_amount = principal_withdraw_amount + rewards_withdraw.value();
...
public(package) fun redeem_fungible_staked_aiy(
    pool: &mut StakingPool,
    fungible_staked_aiy: FungibleStakedAiy,
    ctx: &TxContext,
): Balance<AIY> {
    let FungibleStakedAiy { id, pool_id, value } = fungible_staked_aiy;
    assert!(pool_id == object::id(pool), EWrongPool);

    id.delete();

    let latest_exchange_rate = pool.pool_token_exchange_rate_at_epoch(ctx.epoch());
    let fungible_staked_aiy_data: &mut FungibleStakedAiyData =
        &mut pool.extra_fields[FungibleStakedAiyDataKey {}];

    let (
        principal_amount,
        rewards_amount,
    ) = latest_exchange_rate.calculate_fungible_staked_aiy_withdraw_amount(
        value,
    ...
    let mut aiy_out = fungible_staked_aiy_data.principal.split(principal_amount);
    aiy_out.join(pool.rewards_pool.split(rewards_amount));
...
fun calculate_fungible_staked_aiy_withdraw_amount(
    latest_exchange_rate: PoolTokenExchangeRate,
    fungible_staked_aiy_value: u64,
    ...
    // 1. if the entire FungibleStakedAiyData supply is redeemed, how much aiy should we receive?
    let total_aiy_amount = latest_exchange_rate.get_aiy_amount(
        fungible_staked_aiy_data_total_supply,
    );

    // min with total_aiy_amount to prevent underflow
    let fungible_staked_aiy_data_principal_amount = fungible_staked_aiy_data_principal_amount.min(
        total_aiy_amount,
    );

    // 2. how much do we need to withdraw from the rewards pool?
    let total_rewards = total_aiy_amount - fungible_staked_aiy_data_principal_amount;

    // 3. proportionally withdraw from both wrt the fungible_staked_aiy_value.
    let principal_withdraw_amount = mul_div!(
        fungible_staked_aiy_value,
    ...
    let expected_aiy_amount = latest_exchange_rate.get_aiy_amount(fungible_staked_aiy_value);
    assert!(
...
public(package) fun convert_to_fungible_staked_aiy(
    pool: &mut StakingPool,
    staked_aiy: StakedAiy,
    ctx: &mut TxContext,
): FungibleStakedAiy {
    let StakedAiy { id, pool_id, stake_activation_epoch, principal } = staked_aiy;

    assert!(pool_id == object::id(pool), EWrongPool);
    assert!(ctx.epoch() >= stake_activation_epoch, ECannotMintFungibleStakedAiyYet);
    ...
    let key = FungibleStakedAiyDataKey {};
    ...
    if (!pool.extra_fields.contains(key)) {
        ...
        let fungible_staked_aiy_data: &mut FungibleStakedAiyData = &mut pool.extra_fields[key];
        fungible_staked_aiy_data.total_supply =
            fungible_staked_aiy_data.total_supply + pool_token_amount;
...
public(package) fun withdraw_from_principal(
    ...
    assert!(staked_aiy.pool_id == object::id(pool), EWrongPool);

    let exchange_rate_at_staking_epoch = pool.pool_token_exchange_rate_at_epoch(staked_aiy.stake_activation_epoch);
    let principal_withdraw = staked_aiy.into_balance();
...
/// Allows calling `.into_balance()` on `StakedAiy` to invoke `unwrap_staked_aiy`
use fun unwrap_staked_aiy as StakedAiy.into_balance;

fun unwrap_staked_aiy(staked_aiy: StakedAiy): Balance<AIY> {
    let StakedAiy { id, principal, .. } = staked_aiy;
    id.delete();
    principal
}
...
public(package) fun deposit_rewards(pool: &mut StakingPool, rewards: Balance<AIY>) {
    pool.aiy_balance = pool.aiy_balance + rewards.value();
...
fun process_pending_stake_withdraw(pool: &mut StakingPool) {
    pool.aiy_balance = if (pool.aiy_balance >= pool.pending_total_aiy_withdraw) {
        pool.aiy_balance - pool.pending_total_aiy_withdraw
    } else {
        // the diff will be applied in the `process_pending_stake` function.
        let diff = pool.pending_total_aiy_withdraw - pool.aiy_balance;
        pool.extra_fields.add(UnderflowAiyBalance {}, diff);
        0
    };
...
public(package) fun process_pending_stake(pool: &mut StakingPool) {
    ...
    // This key is only present if the `aiy_balance` underflowed, hence, the current value of `aiy_balance`
    // is `0`. Pool token balance will be recalculated automatically for `0` value.
    let aiy_diff = {
        let key = UnderflowAiyBalance {};
        if (pool.extra_fields.contains(key)) pool.extra_fields.remove(key) else 0
    };

    pool.aiy_balance = pool.aiy_balance + pool.pending_stake - aiy_diff;
    pool.pool_token_balance = latest_exchange_rate.get_token_amount(pool.aiy_balance);
...
fun withdraw_rewards(
    ...
    let total_aiy_withdraw_amount = exchange_rate.get_aiy_amount(pool_token_withdraw_amount);
    let mut reward_withdraw_amount = if (total_aiy_withdraw_amount >= principal_withdraw_amount) {
...
public fun aiy_balance(pool: &StakingPool): u64 { pool.aiy_balance }

public fun pool_id(staked_aiy: &StakedAiy): ID { staked_aiy.pool_id }

public use fun fungible_staked_aiy_pool_id as FungibleStakedAiy.pool_id;

public fun fungible_staked_aiy_pool_id(fungible_staked_aiy: &FungibleStakedAiy): ID {
    fungible_staked_aiy.pool_id
}

/// Allows calling `.amount()` on `StakedAiy` to invoke `staked_aiy_amount`
public use fun staked_aiy_amount as StakedAiy.amount;

/// Returns the principal amount of `StakedAiy`.
public fun staked_aiy_amount(staked_aiy: &StakedAiy): u64 { staked_aiy.principal.value() }

public use fun stake_activation_epoch as StakedAiy.activation_epoch;

/// Returns the activation epoch of `StakedAiy`.
public fun stake_activation_epoch(staked_aiy: &StakedAiy): u64 {
    staked_aiy.stake_activation_epoch
}
...
public use fun fungible_staked_aiy_value as FungibleStakedAiy.value;

public fun fungible_staked_aiy_value(fungible_staked_aiy: &FungibleStakedAiy): u64 {
    fungible_staked_aiy.value
}

public use fun split_fungible_staked_aiy as FungibleStakedAiy.split;

public fun split_fungible_staked_aiy(
    fungible_staked_aiy: &mut FungibleStakedAiy,
    split_amount: u64,
    ctx: &mut TxContext,
): FungibleStakedAiy {
    assert!(split_amount <= fungible_staked_aiy.value, EInsufficientPoolTokenBalance);

    fungible_staked_aiy.value = fungible_staked_aiy.value - split_amount;

    FungibleStakedAiy {
        id: object::new(ctx),
        pool_id: fungible_staked_aiy.pool_id,
        value: split_amount,
    }
}

public use fun join_fungible_staked_aiy as FungibleStakedAiy.join;

public fun join_fungible_staked_aiy(self: &mut FungibleStakedAiy, other: FungibleStakedAiy) {
    let FungibleStakedAiy { id, pool_id, value } = other;
...
/// Split StakedAiy `self` to two parts, one with principal `split_amount`,
/// and the remaining principal is left in `self`.
/// All the other parameters of the StakedAiy like `stake_activation_epoch` or `pool_id` remain the same.
public fun split(self: &mut StakedAiy, split_amount: u64, ctx: &mut TxContext): StakedAiy {
    let original_amount = self.principal.value();
    assert!(split_amount <= original_amount, EInsufficientAiyTokenBalance);
    let remaining_amount = original_amount - split_amount;
    // Both resulting parts should have at least MIN_STAKING_THRESHOLD.
    assert!(remaining_amount >= MIN_STAKING_THRESHOLD, EStakedAiyBelowThreshold);
    assert!(split_amount >= MIN_STAKING_THRESHOLD, EStakedAiyBelowThreshold);
    StakedAiy {
        id: object::new(ctx),
...
/// Allows calling `.split_to_sender()` on `StakedAiy` to invoke `split_staked_aiy`
public use fun split_staked_aiy as StakedAiy.split_to_sender;

#[allow(lint(public_entry))]
/// Split the given StakedAiy to the two parts, one with principal `split_amount`,
/// transfer the newly split part to the sender address.
public entry fun split_staked_aiy(stake: &mut StakedAiy, split_amount: u64, ctx: &mut TxContext) {
    transfer::transfer(stake.split(split_amount, ctx), ctx.sender());
}

/// Allows calling `.join()` on `StakedAiy` to invoke `join_staked_aiy`
public use fun join_staked_aiy as StakedAiy.join;

#[allow(lint(public_entry))]
/// Consume the staked aiy `other` and add its value to `self`.
/// Aborts if some of the staking parameters are incompatible (pool id, stake activation epoch, etc.)
public entry fun join_staked_aiy(self: &mut StakedAiy, other: StakedAiy) {
    assert!(is_equal_staking_metadata(self, &other), EIncompatibleStakedAiy);
    let StakedAiy { id, principal, .. } = other;
...
public fun is_equal_staking_metadata(self: &StakedAiy, other: &StakedAiy): bool {
...
public fun pending_stake_withdraw_amount(staking_pool: &StakingPool): u64 {
    staking_pool.pending_total_aiy_withdraw
}
...
public fun aiy_amount(exchange_rate: &PoolTokenExchangeRate): u64 {
    exchange_rate.aiy_amount
}
...
fun get_aiy_amount(exchange_rate: &PoolTokenExchangeRate, token_amount: u64): u64 {
    // When either amount is 0, that means we have no stakes with this pool.
    // The other amount might be non-zero when there's dust left in the pool.
    if (exchange_rate.aiy_amount == 0 || exchange_rate.pool_token_amount == 0) {
        return token_amount
    };

    mul_div!(exchange_rate.aiy_amount, token_amount, exchange_rate.pool_token_amount)
}

fun get_token_amount(exchange_rate: &PoolTokenExchangeRate, aiy_amount: u64): u64 {
    // When either amount is 0, that means we have no stakes with this pool.
    // The other amount might be non-zero when there's dust left in the pool.
    if (exchange_rate.aiy_amount == 0 || exchange_rate.pool_token_amount == 0) {
        return aiy_amount
    ...
    mul_div!(exchange_rate.pool_token_amount, aiy_amount, exchange_rate.aiy_amount)
}

fun initial_exchange_rate(): PoolTokenExchangeRate {
    PoolTokenExchangeRate { aiy_amount: 0, pool_token_amount: 0 }
}
...
fun check_balance_invariants(pool: &StakingPool, epoch: u64) {
    ...
    let expected = exchange_rate.get_token_amount(pool.aiy_balance);
...
// Given the `staked_aiy` receipt calculate the current rewards (in terms of AIY) for it.
public(package) fun calculate_rewards(
    pool: &StakingPool,
    staked_aiy: &StakedAiy,
    current_epoch: u64,
): u64 {
    let staked_amount = staked_aiy.amount();
    ...
    let total_aiy_withdraw_amount = new_epoch_exchange_rate.get_aiy_amount(
        pool_token_withdraw_amount,
    );

    let mut reward_withdraw_amount = if (total_aiy_withdraw_amount >= staked_amount) {
...
#[test_only]
public(package) fun fungible_staked_aiy_data(pool: &StakingPool): &FungibleStakedAiyData {
    bag::borrow(&pool.extra_fields, FungibleStakedAiyDataKey {})
}

#[test_only]
public use fun fungible_staked_aiy_data_total_supply as FungibleStakedAiyData.total_supply;

#[test_only]
public(package) fun fungible_staked_aiy_data_total_supply(
    fungible_staked_aiy_data: &FungibleStakedAiyData,
): u64 {
    fungible_staked_aiy_data.total_supply
}

#[test_only]
public use fun fungible_staked_aiy_data_principal_value as FungibleStakedAiyData.principal_value;

#[test_only]
public(package) fun fungible_staked_aiy_data_principal_value(
    fungible_staked_aiy_data: &FungibleStakedAiyData,
): u64 {
    fungible_staked_aiy_data.principal.value()
}
...
#[test_only]
public(package) fun create_fungible_staked_aiy_for_testing(
    self: &StakingPool,
    value: u64,
    ctx: &mut TxContext,
): FungibleStakedAiy {
...
#[test_only]
public(package) fun increase_pending_total_aiy_withdraw_for_testing(
    pool: &mut StakingPool,
    delta: u64,
) {
    pool.pending_total_aiy_withdraw = pool.pending_total_aiy_withdraw + delta
}
...
#[random_test]
fun test_calculate_fungible_staked_aiy_withdraw_amount(
    mut total_aiy_amount: u64,
    // these are all in basis points
    mut pool_token_frac: u16,
    mut fungible_staked_aiy_data_total_supply_frac: u16,
    mut fungible_staked_aiy_data_principal_frac: u16,
    mut fungible_staked_aiy_value_bps: u16,
) {
    total_aiy_amount = total_aiy_amount.max(1);

    pool_token_frac = pool_token_frac % 10_000;
    fungible_staked_aiy_data_total_supply_frac =
        fungible_staked_aiy_data_total_supply_frac % 10_000;
    fungible_staked_aiy_data_principal_frac = fungible_staked_aiy_data_principal_frac % 10_000;
    fungible_staked_aiy_value_bps = fungible_staked_aiy_value_bps % 10_000;

    let total_pool_token_amount = mul_div!(total_aiy_amount, pool_token_frac as u64, 10_000).max(1);

    let exchange_rate = PoolTokenExchangeRate {
        aiy_amount: total_aiy_amount,
        pool_token_amount: total_pool_token_amount,
    };

    let fungible_staked_aiy_data_total_supply = mul_div!(
        total_pool_token_amount,
        fungible_staked_aiy_data_total_supply_frac as u64,
        10_000,
    ).max(1);
    let fungible_staked_aiy_value = mul_div!(
        fungible_staked_aiy_data_total_supply,
        fungible_staked_aiy_value_bps as u64,
        10_000,
    );

    let max_principal = exchange_rate.get_aiy_amount(fungible_staked_aiy_data_total_supply);
    let fungible_staked_aiy_data_principal_amount = mul_div!(
        max_principal,
        fungible_staked_aiy_data_principal_frac as u64,
        10_000,
    ).max(1);

    let (principal_amount, rewards_amount) = calculate_fungible_staked_aiy_withdraw_amount(
        exchange_rate,
        fungible_staked_aiy_value,
        fungible_staked_aiy_data_principal_amount,
        fungible_staked_aiy_data_total_supply,
    ...
    let expected_out = exchange_rate.get_aiy_amount(fungible_staked_aiy_value);
