...
/// `AiySystemState` is a thin wrapper around `AiySystemStateInner` that provides a versioned interface.
/// The `AiySystemState` object has a fixed ID 0x5, and the `AiySystemStateInner` object is stored as a dynamic field.
/// There are a few different ways to upgrade the `AiySystemStateInner` type:
///
/// The simplest and one that doesn't involve a real upgrade is to just add dynamic fields to the `extra_fields` field
/// of `AiySystemStateInner` or any of its sub type. This is useful when we are in a rush, or making a small change,
/// or still experimenting a new field.
///
/// To properly upgrade the `AiySystemStateInner` type, we need to ship a new framework that does the following:
/// 1. Define a new `AiySystemStateInner`type (e.g. `AiySystemStateInnerV2`).
/// 2. Define a data migration function that migrates the old `AiySystemStateInner` to the new one (i.e. AiySystemStateInnerV2).
/// 3. Replace all uses of `AiySystemStateInner` with `AiySystemStateInnerV2` in both aiy_system.move and aiy_system_state_inner.move,
///    with the exception of the `aiy_system_state_inner::create` function, which should always return the genesis type.
...
/// 1. Define a new `AiySystemStateInner` struct type that matches the new Move type, and implement the AiySystemStateTrait.
/// 2. Update the `AiySystemState` struct to include the new version as a new enum variant.
/// 3. Update the `get_aiy_system_state` function to handle the new version.
/// To test that the upgrade will be successful, we need to modify `aiy_system_state_production_upgrade_test` test in
...
use aiy::aiy::AIY;
use aiy::table::Table;
use aiy::vec_map::VecMap;
use aiy_system::stake_subsidy::StakeSubsidy;
use aiy_system::staking_pool::{StakedAiy, FungibleStakedAiy, PoolTokenExchangeRate};
use aiy_system::aiy_system_state_inner::{
    Self,
    SystemParameters,
    AiySystemStateInner,
    AiySystemStateInnerV2
};
use aiy_system::validator::Validator;
use aiy_system::validator_cap::UnverifiedValidatorOperationCap;

#[test_only]
use aiy::balance;
#[test_only]
use aiy_system::validator_set::ValidatorSet;
#[test_only]
use aiy::vec_set::VecSet;
...
/// Note: `proof_of_possession` MUST be a valid signature using aiy_address and protocol_pubkey_bytes.
...
public entry fun request_add_validator(wrapper: &mut AiySystemState, ctx: &mut TxContext) {
...
/// At the end of the epoch, the `validator` object will be returned to the aiy_address
/// of the validator.
public entry fun request_remove_validator(wrapper: &mut AiySystemState, ctx: &mut TxContext) {
...
public entry fun request_add_stake(
    ...
    let staked_aiy = request_add_stake_non_entry(wrapper, stake, validator_address, ctx);
    transfer::public_transfer(staked_aiy, ctx.sender());
}

/// The non-entry version of `request_add_stake`, which returns the staked AIY instead of transferring it to the sender.
...
public entry fun request_withdraw_stake(
    ...
    let withdrawn_stake = wrapper.request_withdraw_stake_non_entry(staked_aiy, ctx);
    transfer::public_transfer(withdrawn_stake.into_coin(ctx), ctx.sender());
}

/// Convert StakedAiy into a FungibleStakedAiy object.
public fun convert_to_fungible_staked_aiy(
    wrapper: &mut AiySystemState,
    staked_aiy: StakedAiy,
    ctx: &mut TxContext,
): FungibleStakedAiy {
    wrapper.load_system_state_mut().convert_to_fungible_staked_aiy(staked_aiy, ctx)
}

/// Convert FungibleStakedAiy into a StakedAiy object.
public fun redeem_fungible_staked_aiy(
    wrapper: &mut AiySystemState,
    fungible_staked_aiy: FungibleStakedAiy,
    ctx: &TxContext,
): Balance<AIY> {
    wrapper.load_system_state_mut().redeem_fungible_staked_aiy(fungible_staked_aiy, ctx)
}

/// Non-entry version of `request_withdraw_stake` that returns the withdrawn AIY instead of transferring it to the sender.
public fun request_withdraw_stake_non_entry(
    wrapper: &mut AiySystemState,
    staked_aiy: StakedAiy,
    ctx: &mut TxContext,
): Balance<AIY> {
    wrapper.load_system_state_mut().request_withdraw_stake(staked_aiy, ctx)
}
...
public entry fun rotate_operation_cap(self: &mut AiySystemState, ctx: &mut TxContext) {
...
public fun validator_address_by_pool_id(wrapper: &mut AiySystemState, pool_id: &ID): address {
...
public fun active_validator_addresses_ref(wrapper: &AiySystemState): vector<address> {
...
public fun active_validator_voting_powers(wrapper: &AiySystemState): VecMap<address, u64> {
...
public(package) fun calculate_rewards(
    ...
    system_state
        ...
        .calculate_rewards(staked_aiy, ctx.epoch())
}
...
fun load_system_state(self: &mut AiySystemState): &AiySystemStateInnerV2 {
...
fun load_system_state_ref(self: &AiySystemState): &AiySystemStateInnerV2 {
    let inner: &AiySystemStateInnerV2 = dynamic_field::borrow(
        &self.id,
        self.version,
    );
...
fun load_inner_maybe_upgrade(self: &mut AiySystemState): &mut AiySystemStateInnerV2 {
    if (self.version == 1) {
        let v1: AiySystemStateInner = dynamic_field::remove(&mut self.id, self.version);
        let v2 = v1.v1_to_v2();
        self.version = 2;
        dynamic_field::add(&mut self.id, self.version, v2);
    };

    let inner: &mut AiySystemStateInnerV2 = dynamic_field::borrow_mut(
        &mut self.id,
        self.version,
    );
    assert!(inner.system_state_version() == self.version, EWrongInnerVersion);
...
fun validator_voting_powers(wrapper: &mut AiySystemState): VecMap<address, u64> {
...
#[test_only]
public fun validator_voting_powers_for_testing(wrapper: &mut AiySystemState): VecMap<address, u64> {
...
public fun epoch(wrapper: &mut AiySystemState): u64 {
    wrapper.load_system_state_mut().epoch()
}

#[test_only]
/// Returns unix timestamp of the start of current epoch
public fun epoch_start_timestamp_ms(wrapper: &mut AiySystemState): u64 {
...
public fun validator_stake_amount(wrapper: &mut AiySystemState, validator_addr: address): u64 {
...
/// Aborts if `validator_addr` is not an active validator.
public fun validator_staking_pool_id(wrapper: &mut AiySystemState, validator_addr: address): ID {
    wrapper.load_system_state_mut().validator_staking_pool_id(validator_addr)
}

#[test_only]
/// Returns reference to the staking pool mappings that map pool ids to active validator addresses
public fun validator_staking_pool_mappings(wrapper: &mut AiySystemState): &Table<ID, address> {
    wrapper.load_system_state_mut().validator_staking_pool_mappings()
}

#[test_only]
/// Returns all the validators who are currently reporting `addr`
public fun get_reporters_of(wrapper: &mut AiySystemState, addr: address): VecSet<address> {
    wrapper.load_system_state_mut().get_reporters_of(addr)
}

#[test_only]
/// Return the current validator set
public fun validators(wrapper: &mut AiySystemState): &ValidatorSet {
    wrapper.load_system_state_mut().validators()
}

#[test_only]
/// Return a mutable reference to the validator set
public fun validators_mut(wrapper: &mut AiySystemState): &mut ValidatorSet {
...
#[test_only]
public fun set_epoch_for_testing(wrapper: &mut AiySystemState, epoch_num: u64) {
    wrapper.load_system_state_mut().set_epoch_for_testing(epoch_num)
}

#[test_only]
public fun request_add_validator_for_testing(wrapper: &mut AiySystemState, ctx: &TxContext) {
    wrapper.load_system_state_mut().request_add_validator(ctx)
}

#[test_only]
public fun get_storage_fund_total_balance(wrapper: &mut AiySystemState): u64 {
    wrapper.load_system_state_mut().get_storage_fund_total_balance()
}

#[test_only]
public fun get_storage_fund_object_rebates(wrapper: &mut AiySystemState): u64 {
    wrapper.load_system_state_mut().get_storage_fund_object_rebates()
}

#[test_only]
public fun get_stake_subsidy_distribution_counter(wrapper: &mut AiySystemState): u64 {
    wrapper.load_system_state_mut().get_stake_subsidy_distribution_counter()
}

#[test_only]
public fun set_stake_subsidy_distribution_counter(wrapper: &mut AiySystemState, counter: u64) {
    wrapper.load_system_state_mut().set_stake_subsidy_distribution_counter(counter)
}

#[test_only]
public fun inner_mut_for_testing(wrapper: &mut AiySystemState): &mut AiySystemStateInnerV2 {
