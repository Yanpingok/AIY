...
use aiy_system::validator_cap::{Self, ValidatorOperationCap};

public use fun aiy_system::validator_wrapper::create_v1 as Validator.wrap_v1;
...
public struct ConvertingToFungibleStakedAiyEvent has copy, drop {
    pool_id: ID,
    stake_activation_epoch: u64,
    staked_aiy_principal_amount: u64,
    fungible_staked_aiy_amount: u64,
}

/// Event emitted when a fungible staked AIY is redeemed.
public struct RedeemingFungibleStakedAiyEvent has copy, drop {
    pool_id: ID,
    fungible_staked_aiy_amount: u64,
    aiy_amount: u64,
}
...
public(package) fun request_add_stake(
    ...
    let staked_aiy = self.staking_pool.request_add_stake(stake, stake_epoch, ctx);
    ...
    event::emit(StakingRequestEvent {
        ...
        validator_address: self.metadata.aiy_address,
        staker_address,
        epoch: ctx.epoch(),
        amount: stake_amount,
    });
...
public(package) fun convert_to_fungible_staked_aiy(
    ...
    let stake_activation_epoch = staked_aiy.activation_epoch();
    let staked_aiy_principal_amount = staked_aiy.amount();
    let fungible_staked_aiy = self.staking_pool.convert_to_fungible_staked_aiy(staked_aiy, ctx);

    event::emit(ConvertingToFungibleStakedAiyEvent {
        pool_id: self.staking_pool_id(),
...
public(package) fun redeem_fungible_staked_aiy(
    ...
    let fungible_staked_aiy_amount = fungible_staked_aiy.value();
    let aiy = self.staking_pool.redeem_fungible_staked_aiy(fungible_staked_aiy, ctx);

    self.next_epoch_stake = self.next_epoch_stake - aiy.value();

    event::emit(RedeemingFungibleStakedAiyEvent {
        pool_id: self.staking_pool_id(),
...
public(package) fun request_add_stake_at_genesis(
    ...
    // 0 = genesis epoch
    let staked_aiy = self.staking_pool.request_add_stake(stake, 0, ctx);
...
public(package) fun request_withdraw_stake(
    ...
    let principal_amount = staked_aiy.amount();
    let stake_activation_epoch = staked_aiy.activation_epoch();
    let withdrawn_stake = self.staking_pool.request_withdraw_stake(staked_aiy, ctx);
...
public(package) fun request_set_gas_price(
    ...
    assert!(validator_address == self.metadata.aiy_address, EInvalidCap);
...
public(package) fun deposit_stake_rewards(self: &mut Validator, reward: Balance<AIY>) {
...
public fun aiy_address(self: &Validator): address {
    self.metadata.aiy_address
}
...
public fun is_duplicate(self: &Validator, other: &Validator): bool {
    ...
    self.aiy_address == other.aiy_address
...
public(package) fun new_unverified_validator_operation_cap_and_transfer(
    ...
    assert!(sender == self.metadata.aiy_address, ENewCapNotCreatedByValidatorItself);
...
fun new_from_metadata(
    ...
    let aiy_address = metadata.aiy_address;
...
// Note: `proof_of_possession` MUST be a valid signature using aiy_address and
...
public(package) fun new_for_testing(
    ...
    primary_address: vector<u8>,
    worker_address: vector<u8>,
    initial_stake_option: Option<Balance<AIY>>,
    gas_price: u64,
    commission_rate: u64,
    ...
    initial_stake_option.do!(|balance| {
        request_add_stake_at_genesis(
            &mut validator,
            balance,
            aiy_address, // give the stake to the validator
            ctx,
        );
    });
