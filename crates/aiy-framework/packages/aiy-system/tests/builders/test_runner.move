...
/// provides a set of convenience methods for testing the Aiy System.
module aiy_system::test_runner;

use aiy::balance::{Self, Balance};
use aiy::coin::{Self, Coin};
use aiy::aiy::AIY;
use aiy::test_scenario::{Self, Scenario};
use aiy_system::stake_subsidy;
...
use aiy_system::validator_builder::{Self, ValidatorBuilder};
...
public fun build(builder: TestRunnerBuilder): TestRunner {
    ...
    let system_parameters = aiy_system_state_inner::create_system_parameters(
    ...
    let stake_subsidy = stake_subsidy::create(
        balance::create_for_testing<AIY>(aiy_supply_amount.destroy_or!(1000) * MIST_PER_AIY), // aiy_supply
    ...
    let genesis_validator_addresses = validators.map_ref!(|v| v.aiy_address());

    // create aiy system state
    aiy_system::create(
        object::new(scenario.ctx()), // it doesn't matter what ID aiy system state has in tests
        validators,
        balance::create_for_testing<AIY>(storage_fund_amount.destroy_or!(0) * MIST_PER_AIY), // storage_fund
...
public fun aiy_supply_amount(
    mut builder: TestRunnerBuilder,
    aiy_supply_amount: u64,
): TestRunnerBuilder {
    builder.aiy_supply_amount = option::some(aiy_supply_amount);
    builder
}
...
/// Mint a AIY balance for testing.
public fun mint(amount: u64): Balance<AIY> {
    balance::create_for_testing(amount * MIST_PER_AIY)
}

/// Destroy an object.
public fun destroy<T>(v: T) {
    aiy::test_utils::destroy(v);
}
...
public macro fun system_tx(
    ...
    $f: |&mut AiySystemState, &mut TxContext|,
): &mut TestRunner {
    let sender = sender($runner);
    let scenario = scenario_mut($runner);
    scenario.next_tx(sender);
    let mut system_state = scenario.take_shared<AiySystemState>();
    ...
    runner.system_tx!(|system, ctx| {
        ...
        storage_rebate_balance =
            system.advance_epoch_for_testing(
                ...
                storage_charge.destroy_or!(0) * MIST_PER_AIY,
                computation_charge.destroy_or!(0) * MIST_PER_AIY,
...
public fun stake_with(runner: &mut TestRunner, validator: address, amount: u64) {
    ...
    runner.system_tx!(|system, ctx| {
        system.request_add_stake(
            coin::mint_for_testing(amount * MIST_PER_AIY, ctx),
            validator,
            ctx,
        );
    });
...
public fun unstake(runner: &mut TestRunner, staked_aiy_idx: u64) {
    let sender = runner.sender;
    runner.set_sender(sender);
    let stake_aiy_ids = runner.scenario.ids_for_sender<StakedAiy>();
    let staked_aiy = runner.scenario.take_from_sender_by_id(stake_aiy_ids[staked_aiy_idx]);
    runner.system_tx!(|system, ctx| {
        system.request_withdraw_stake(staked_aiy, ctx);
    });
}
...
public fun add_validator_candidate(runner: &mut TestRunner, validator: Validator) {
    let sender = runner.sender;
    runner.scenario.next_tx(validator.aiy_address());
    runner.system_tx!(|system, ctx| {
...
/// Get the sum of the balances of all the AIY coins in the sender's account.
public fun aiy_balance(runner: &mut TestRunner): u64 {
    let sender = runner.sender;
    let scenario = runner.scenario_mut();
    scenario.next_tx(sender);
    scenario.ids_for_sender<Coin<AIY>>().fold!(0, |mut sum, coin_id| {
        let coin = scenario.take_from_sender_by_id<Coin<AIY>>(coin_id);
        sum = sum + coin.value();
...
/// Get the sum of the StakedAiy objects' principal and the rewards for these objects.
public fun staking_rewards_balance(runner: &mut TestRunner): u64 {
    let sender = runner.sender;
    let scenario = runner.scenario_mut();
    let mut system = scenario.take_shared<AiySystemState>();

    scenario.next_tx(sender);
    let total_balance = scenario.ids_for_sender<StakedAiy>().fold!(0, |mut sum, staked_aiy_id| {
        let staked_aiy = scenario.take_from_sender_by_id<StakedAiy>(staked_aiy_id);
        let rewards = system.calculate_rewards(&staked_aiy, scenario.ctx());

        sum = sum + rewards + staked_aiy.amount();
        scenario.return_to_sender(staked_aiy);
        sum
    });
