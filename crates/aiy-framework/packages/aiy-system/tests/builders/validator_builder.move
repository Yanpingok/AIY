...
module aiy_system::validator_builder;

use aiy::bag;
use aiy::balance;
use aiy::aiy::AIY;
use aiy::url;
use aiy_system::validator::{Self, Validator, ValidatorMetadata};
...
public fun preset(): ValidatorBuilder {
    ValidatorBuilder {
        aiy_address: option::some(VALID_ADDRESS),
...
public fun build(builder: ValidatorBuilder, ctx: &mut TxContext): Validator {
    ...
    validator::new_for_testing(
        aiy_address.destroy_or!(ctx.fresh_object_address()),
        ...
        initial_stake.map!(|amount| balance::create_for_testing<AIY>(amount * 1_000_000_000)),
...
public fun build_metadata(builder: ValidatorBuilder, ctx: &mut TxContext): ValidatorMetadata {
    ...
    validator::new_metadata(
        aiy_address.destroy_or!(VALID_ADDRESS),
...
public fun aiy_address(mut builder: ValidatorBuilder, aiy_address: address): ValidatorBuilder {
    builder.aiy_address = option::some(aiy_address);
    builder
}
...
public fun valid_aiy_address(): address { VALID_ADDRESS }
