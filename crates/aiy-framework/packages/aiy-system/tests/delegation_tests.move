...
#[test_only]
module aiy_system::delegation_tests;

use std::unit_test::assert_eq;
use aiy::table::Table;
use aiy_system::staking_pool::{Self, StakedAiy, PoolTokenExchangeRate};
use aiy_system::test_runner;
use aiy_system::validator_builder;
use aiy_system::validator_set;
...
#[test]
// Scenario:
// 1. Stake 60 AIY to VALIDATOR_ADDR_1
// 2. Split the stake into 20 and 40
// 3. Join the 20 and 40 back together
// 4. Check that the stake is 60 again
fun split_join_staked_aiy() {
    let validator = validator_builder::new().initial_stake(100).aiy_address(VALIDATOR_ADDR_1);
    ...
    runner.owned_tx!<StakedAiy>(|mut stake| {
        stake.split_to_sender(20 * MIST_PER_AIY, runner.ctx());
        runner.keep(stake);
    });

    runner.scenario_fn!(|scenario| {
        let ids = scenario.ids_for_sender<StakedAiy>();
        assert_eq!(ids.length(), 2);

        let mut stake_1 = scenario.take_from_sender_by_id<StakedAiy>(ids[0]);
        let stake_2 = scenario.take_from_sender_by_id<StakedAiy>(ids[1]);

        assert_eq!(stake_1.amount(), 20 * MIST_PER_AIY);
        assert_eq!(stake_2.amount(), 40 * MIST_PER_AIY);

        stake_1.join(stake_2);
        runner.keep(stake_1);
    });

    runner.owned_tx!<StakedAiy>(|stake| {
        assert_eq!(stake.amount(), 60 * MIST_PER_AIY);
        runner.keep(stake);
    });
...
#[test, expected_failure(abort_code = staking_pool::EIncompatibleStakedAiy)]
fun join_different_epochs() {
    ...
    runner.scenario_fn!(|scenario| {
        let staked_aiy_ids = scenario.ids_for_sender<StakedAiy>();
        let mut part1 = scenario.take_from_sender_by_id<StakedAiy>(staked_aiy_ids[0]);
        let part2 = scenario.take_from_sender_by_id<StakedAiy>(staked_aiy_ids[1]);
...
#[test, expected_failure(abort_code = staking_pool::EStakedAiyBelowThreshold)]
fun split_below_threshold() {
    let validator = validator_builder::new().initial_stake(100).aiy_address(VALIDATOR_ADDR_1);
    let mut runner = test_runner::new().validators(vector[validator]).build();

    // Stake 2 AIY to the validator.
    runner.set_sender(STAKER_ADDR_1).stake_with(VALIDATOR_ADDR_1, 2);

    runner.owned_tx!<StakedAiy>(|mut stake| {
        stake.split_to_sender(1 * MIST_PER_AIY + 1, runner.ctx());
    });
...
fun add_remove_stake_flow() {
    let mut runner = test_runner::new()
        .validators(vector[
            ...
            validator_builder::new().initial_stake(100).aiy_address(VALIDATOR_ADDR_2),
        ])
        .build();

    // Stake 60 AIY to the validator.
    runner.set_sender(STAKER_ADDR_1).stake_with(VALIDATOR_ADDR_1, 60);

    // Check that the stake is NOT yet added to the validator.
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 100 * MIST_PER_AIY);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 100 * MIST_PER_AIY);
    });
    ...
    runner.owned_tx!<StakedAiy>(|stake| {
        runner.system_tx!(|system, ctx| {
            assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 160 * MIST_PER_AIY);
            assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 100 * MIST_PER_AIY);
...
// Scenario:
// 1. Stake 100 AIY to VALIDATOR_ADDR_1
// 2. Advance epoch
...
fun remove_stake_post_active_flow(should_distribute_rewards: bool) {
    let mut runner = test_runner::new()
        ...
        .aiy_supply_amount(300)
        .storage_fund_amount(100)
        .build();

    // Stake 100 AIY to the validator.
    ...
    let reward_amt = if (should_distribute_rewards) 15 * MIST_PER_AIY else 0;
    let validator_reward_amt = if (should_distribute_rewards) 10 * MIST_PER_AIY else 0;

    runner.set_sender(STAKER_ADDR_1);
    runner.owned_tx!<StakedAiy>(|stake| {
        assert_eq!(stake.amount(), 100 * MIST_PER_AIY);
        runner.system_tx!(|system, ctx| {
            assert!(!system.validators().is_active_validator_by_aiy_address(VALIDATOR_ADDR_1));
            system.request_withdraw_stake(stake, ctx);
        });
    });
    ...
    runner.set_sender(STAKER_ADDR_1);
    assert_eq!(runner.aiy_balance(), 100 * MIST_PER_AIY + reward_amt);
    ...
    assert_eq!(runner.aiy_balance(), 100 * MIST_PER_AIY + reward_amt + validator_reward_amt);
...
fun earns_rewards_at_last_epoch() {
    ...
    let reward_amt = 15 * MIST_PER_AIY;
    let validator_reward_amt = 10 * MIST_PER_AIY;
...
// 2. Stake 100 AIY to the validator candidate
...
fun add_preactive_remove_preactive() {
    ...
    let validator = validator_builder::preset().aiy_address(NEW_VALIDATOR_ADDR).build(runner.ctx());
    ...
    assert_eq!(runner.aiy_balance(), 100 * MIST_PER_AIY);
...
fun add_preactive_remove_pending_failure() {
    ...
    // Stake 100 AIY to the validator candidate.
    runner.set_sender(STAKER_ADDR_1).stake_with(NEW_VALIDATOR_ADDR, 100);
...
fun add_preactive_remove_active() {
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 250 * MIST_PER_AIY);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 250 * MIST_PER_AIY);
        assert_eq!(
    ...
    // Stake 50 AIY to the validator candidate from staker 2.
    runner.set_sender(STAKER_ADDR_2).stake_with(NEW_VALIDATOR_ADDR, 50);

    // Stake 100 AIY to the validator candidate from staker 3.
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 250 * MIST_PER_AIY);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 250 * MIST_PER_AIY);
        assert_eq!(system.validator_stake_amount(NEW_VALIDATOR_ADDR), 250 * MIST_PER_AIY);
        assert_eq!(
            system.inner_mut_for_testing().get_storage_fund_total_balance(),
            100 * MIST_PER_AIY,
        );
    ...
    // staker 1 and 3 unstake from the validator and earns about 2/5 * (85 - 10) * 1/3 = 10 AIY each.
    ...
    // So they will both get slightly more than 110 AIY in total balance.
    runner.set_sender(STAKER_ADDR_1).unstake(0);
    assert_eq!(runner.aiy_balance(), 110002000000);
    ...
    // staker 2 earns about 5 AIY from the previous epoch and 24-ish from this one
    // so in total she has about 50 + 5 + 24 = 79 AIY.
    runner.set_sender(STAKER_ADDR_2).unstake(0);
    assert_eq!(runner.aiy_balance(), 78862939078);
...
fun add_preactive_remove_post_active() {
    ...
    // staker 1 earns a bit greater than 30 AIY here. A bit greater because the new validator's voting power
    ...
    runner.set_sender(STAKER_ADDR_1).unstake(0);
    assert_eq!(runner.aiy_balance(), 130006000000);
...
fun add_remove_stake_preactive_candidate() {
    ...
    // Stake 100 AIY to the validator candidate from each of the two stakers.
    runner.set_sender(STAKER_ADDR_1).stake_with(NEW_VALIDATOR_ADDR, 100);
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(validator.total_stake(), 200 * MIST_PER_AIY);
...
// Scenario:
// 1. Add a validator candidate
// 2. Delegate 100 AIY to the validator candidate
...
fun add_preactive_candidate_drop_out() {
    ...
    let validator_address = validator.aiy_address();
    runner.add_validator_candidate(validator);
...
fun remove_inactive_stake_from_inactive_candidate() {
    ...
    assert_eq!(runner.set_sender(validator_address).aiy_balance(), 100 * MIST_PER_AIY);
...
#[test]
/// Scenario:
/// 1. Stake 100 AIY to the validator.
/// 2. Advance epoch.
...
fun staking_pool_exchange_rate_getter() {
    ...
    // Each staking pool (2 pools) gets 10 AIY of rewards.
    ...
    runner.system_tx!(|system, _| {
        ...
        rates.assert_exchange_rate_eq(1, 200, 200); // 200 AIY of self + delegate stake at epoch 1
        rates.assert_exchange_rate_eq(2, 210, 200); // 10 AIY of rewards at epoch 2
    });
...
fun assert_exchange_rate_eq(
    ...
    assert_eq!(rate.aiy_amount(), aiy_amount * MIST_PER_AIY);
    assert_eq!(rate.pool_token_amount(), pool_token_amount * MIST_PER_AIY);
}
