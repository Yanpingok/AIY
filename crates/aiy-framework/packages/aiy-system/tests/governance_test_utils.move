...
#[test_only]
#[deprecated(note = b"Use aiy_system::test_runner instead")]
module aiy_system::governance_test_utils;
...
use aiy_system::staking_pool::{StakedAiy, StakingPool};
...
public fun create_validator_for_testing(
    ...
    let validator = validator::new_for_testing(
        ...
        b"/ip4/127.0.0.1/udp/80",
        b"/ip4/127.0.0.1/udp/80",
        option::some(balance::create_for_testing<AIY>(init_stake_amount_in_aiy * MIST_PER_AIY)),
        1,
        0,
        true,
        ctx,
...
public fun create_aiy_system_state_for_testing(
    ...
    let stake_subsidy = stake_subsidy::create(
        balance::create_for_testing<AIY>(aiy_supply_amount * MIST_PER_AIY), // aiy_supply
    ...
    aiy_system::create(
        object::new(ctx), // it doesn't matter what ID aiy system state has in tests
        validators,
        balance::create_for_testing<AIY>(storage_fund_amount * MIST_PER_AIY), // storage_fund
        1, // protocol version
...
public fun set_up_aiy_system_state(mut addrs: vector<address>) {
    ...
    create_aiy_system_state_for_testing(validators, 1000, 0, ctx);
    scenario.end();
}
...
public fun advance_epoch_with_reward_amounts(
    ...
    let storage_rebate = advance_epoch_with_reward_amounts_return_rebate(
        storage_charge * MIST_PER_AIY,
        computation_charge * MIST_PER_AIY,
        0,
        0,
        scenario,
    );
...
public fun unstake(staker: address, staked_aiy_idx: u64, scenario: &mut Scenario) {
    scenario.next_tx(staker);
    let stake_aiy_ids = scenario.ids_for_sender<StakedAiy>();
    let staked_aiy = scenario.take_from_sender_by_id(stake_aiy_ids[staked_aiy_idx]);
    let mut system_state = scenario.take_shared<AiySystemState>();

    let ctx = scenario.ctx();
    system_state.request_withdraw_stake(staked_aiy, ctx);
    test_scenario::return_shared(system_state);
}
...
public fun add_validator_full_flow(
    ...
    system_state.request_add_stake(
        coin::mint_for_testing<AIY>(init_stake_amount * MIST_PER_AIY, ctx),
        validator,
        ctx,
    );
...
/// Return the rewards for the validator at `addr` in terms of AIY.
public fun stake_plus_current_rewards_for_validator(
    addr: address,
    system_state: &mut AiySystemState,
    scenario: &mut Scenario,
): u64 {
...
public fun stake_plus_current_rewards(
    ...
    let mut stake_ids = scenario.ids_for_sender<StakedAiy>();
    let current_epoch = scenario.ctx().epoch();

    while (!stake_ids.is_empty()) {
        let staked_aiy_id = stake_ids.pop_back();
        let staked_aiy = scenario.take_from_sender_by_id<StakedAiy>(staked_aiy_id);
        sum =
            sum + staked_aiy.amount() + staking_pool.calculate_rewards(&staked_aiy, current_epoch);
        scenario.return_to_sender(staked_aiy);
    };
    sum
}

public fun total_aiy_balance(addr: address, scenario: &mut Scenario): u64 {
    let mut sum = 0;
    scenario.next_tx(addr);
    let coin_ids = scenario.ids_for_sender<Coin<AIY>>();
    let mut i = 0;
    while (i < coin_ids.length()) {
        let coin = scenario.take_from_sender_by_id<Coin<AIY>>(coin_ids[i]);
        sum = sum + coin.value();
