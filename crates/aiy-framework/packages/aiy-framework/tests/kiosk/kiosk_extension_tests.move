...
#[test_only]
module aiy::kiosk_marketplace_ext {
    use aiy::coin::Coin;
    use aiy::kiosk::{Self, KioskOwnerCap, Kiosk, PurchaseCap};
    use aiy::kiosk_extension as ext;
    use aiy::aiy::AIY;
    use aiy::transfer_policy::{Self as policy, TransferPolicy, TransferRequest};

    /// Trying to access an owner-only action.
    const ENotOwner: u64 = 0;
    /// Trying to purchase an item with an incorrect amount of AIY.
    ...
    /// Collection bidding: the Kiosk Owner offers a bid (in AIY) for an item of type `T`.
    ///
    /// There can be only one bid per type.
    public fun bid<Market, T: key + store>(kiosk: &mut Kiosk, cap: &KioskOwnerCap, bid: Coin<AIY>) {
    ...
    public fun accept_bid<Market, T: key + store>(
        ...
        let bid: Coin<AIY> = ext::storage_mut(Ext<Market> {}, destination).remove(Bid<T> {});
...
#[test_only]
module aiy::kiosk_extensions_tests {
    use aiy::kiosk_extension as ext;
    use aiy::kiosk_test_utils as test;
    ...
    #[test, expected_failure(abort_code = aiy::kiosk_extension::EExtensionNotAllowed)]
    ...
    #[test, expected_failure(abort_code = aiy::kiosk_extension::EExtensionNotInstalled)]
