...
module aiy::accumulator_settlement;

use aiy::accumulator::{AccumulatorRoot, accumulator_key, U128, create_u128, destroy_u128};
use aiy::bcs;
use aiy::hash;
...
use fun aiy::accumulator_metadata::remove_accumulator_metadata as AccumulatorRoot.remove_metadata;
use fun aiy::accumulator_metadata::create_accumulator_metadata as AccumulatorRoot.create_metadata;
...
fun settlement_prologue(
    ...
    _idx: u64,
    // Total input aiy received from user transactions
    input_aiy: u64,
    // Total output aiy withdrawn by user transactions
    output_aiy: u64,
    ctx: &TxContext,
) {
    assert!(ctx.sender() == @0x0, ENotSystemAddress);
    record_settlement_aiy_conservation(input_aiy, output_aiy);
}
...
native fun record_settlement_aiy_conservation(input_aiy: u64, output_aiy: u64);
