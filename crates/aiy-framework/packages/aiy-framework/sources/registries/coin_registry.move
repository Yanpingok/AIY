...
module aiy::coin_registry;

use std::ascii;
use std::string::String;
use std::type_name::TypeName;
use aiy::bag::{Self, Bag};
use aiy::balance::{Supply, Balance};
use aiy::coin::{Self, TreasuryCap, DenyCapV2, CoinMetadata, RegulatedCoinMetadata, Coin};
...
public fun new_currency_with_otw<T: drop>(
    ...
    assert!(aiy::types::is_one_time_witness(&otw));
    assert!(is_ascii_printable!(&symbol), EInvalidSymbol);
...
public fun finalize<T>(builder: CurrencyInitializer<T>, ctx: &mut TxContext): MetadataCap<T> {
    ...
    if (is_otw) transfer::transfer(currency, object::aiy_coin_registry_address())
...
fun create(ctx: &TxContext) {
    ...
    transfer::share_object(CoinRegistry {
        id: object::aiy_coin_registry_object_id(),
    });
}
