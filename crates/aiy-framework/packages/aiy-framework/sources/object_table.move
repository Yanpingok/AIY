...
/// Similar to `aiy::table`, an `ObjectTable<K, V>` is a map-like collection. But unlike
/// `aiy::table`, the values bound to these dynamic fields _must_ be objects themselves. This allows
/// for the objects to still exist within in storage, which may be important for external tools.
/// The difference is otherwise not observable from within Move.
module aiy::object_table;
...
/// Aborts with `aiy::dynamic_field::EFieldAlreadyExists` if the table already has an entry with
/// that key `k: K`.
...
/// Aborts with `aiy::dynamic_field::EFieldDoesNotExist` if the table does not have an entry with
/// that key `k: K`.
