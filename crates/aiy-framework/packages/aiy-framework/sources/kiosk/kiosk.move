...
/// `list_with_purchase_cap` therefore performing a trade (isaiyng a
...
entry fun default(ctx: &mut TxContext) {
    ...
    aiy::transfer::transfer(cap, ctx.sender());
    aiy::transfer::share_object(kiosk);
}
...
public fun close_and_withdraw(self: Kiosk, cap: KioskOwnerCap, ctx: &mut TxContext): Coin<AIY> {
...
public fun profits_mut(self: &mut Kiosk, cap: &KioskOwnerCap): &mut Balance<AIY> {
    assert!(self.has_access(cap), ENotOwner);
    &mut self.profits
}
