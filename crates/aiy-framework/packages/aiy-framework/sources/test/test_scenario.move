...
/// Utility for mocking a multi-transaction Aiy execution in a single Move procedure.
...
public fun create_system_objects(scenario: &mut Scenario) {
    ...
    aiy::clock::create_for_testing(scenario.ctx()).share_for_testing();
    aiy::random::create_for_testing(scenario.ctx());
    aiy::deny_list::create_for_testing(scenario.ctx());
...
public fun return_to_address<T: key>(account: address, t: T) {
    ...
    aiy::transfer::transfer_impl(t, account)
}
...
public fun return_immutable<T: key>(t: T) {
    ...
    aiy::transfer::freeze_object_impl(t)
}
...
public fun return_shared<T: key>(t: T) {
    ...
    aiy::transfer::share_object_impl(t)
}
...
public fun most_recent_receiving_ticket<T: key>(owner: &ID): aiy::transfer::Receiving<T> {
...
public fun receiving_ticket_by_id<T: key>(object_id: ID): aiy::transfer::Receiving<T> {
    let version = allocate_receiving_ticket_for_object<T>(object_id);
    aiy::transfer::make_receiver(object_id, version)
}
...
/// test scenario.
public fun return_receiving_ticket<T: key>(ticket: aiy::transfer::Receiving<T>) {
    let id = aiy::transfer::receiving_id(&ticket);
