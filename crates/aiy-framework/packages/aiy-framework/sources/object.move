...
const AIY_SYSTEM_STATE_OBJECT_ID: address = @0x5;

/// The hardcoded ID for the singleton Clock Object.
const AIY_CLOCK_OBJECT_ID: address = @0x6;

/// The hardcoded ID for the singleton AuthenticatorState Object.
const AIY_AUTHENTICATOR_STATE_ID: address = @0x7;

/// The hardcoded ID for the singleton Random Object.
const AIY_RANDOM_ID: address = @0x8;

/// The hardcoded ID for the singleton DenyList.
const AIY_DENY_LIST_OBJECT_ID: address = @0x403;

/// The hardcoded ID for the singleton AccumulatorRoot Object.
const AIY_ACCUMULATOR_ROOT_OBJECT_ID: address = @0xacc;

/// The hardcoded ID for the Bridge Object.
const AIY_BRIDGE_ID: address = @0x9;

/// The hardcoded ID for the Coin Registry Object.
const AIY_COIN_REGISTRY_OBJECT_ID: address = @0xc;
...
/// Create the `UID` for the singleton `AiySystemState` object.
/// This should only be called once from `aiy_system`.
fun aiy_system_state(ctx: &TxContext): UID {
    assert!(ctx.sender() == @0x0, ENotSystemAddress);
    UID {
        id: ID { bytes: AIY_SYSTEM_STATE_OBJECT_ID },
    }
}
...
public(package) fun clock(): UID {
    UID {
        id: ID { bytes: AIY_CLOCK_OBJECT_ID },
    }
}
...
public(package) fun authenticator_state(): UID {
    UID {
        id: ID { bytes: AIY_AUTHENTICATOR_STATE_ID },
    }
}
...
public(package) fun randomness_state(): UID {
    UID {
        id: ID { bytes: AIY_RANDOM_ID },
    }
}

/// Create the `UID` for the singleton `DenyList` object.
/// This should only be called once from `deny_list`.
public(package) fun aiy_deny_list_object_id(): UID {
    UID {
        id: ID { bytes: AIY_DENY_LIST_OBJECT_ID },
    }
}

public(package) fun aiy_accumulator_root_object_id(): UID {
    UID {
        id: ID { bytes: AIY_ACCUMULATOR_ROOT_OBJECT_ID },
    }
}

public(package) fun aiy_accumulator_root_address(): address {
    AIY_ACCUMULATOR_ROOT_OBJECT_ID
}

/// Create the `UID` for the singleton `CoinRegistry` object.
/// This should only be called once from `coin_registry`.
public(package) fun aiy_coin_registry_object_id(): UID {
    UID {
        id: ID { bytes: AIY_COIN_REGISTRY_OBJECT_ID },
    }
}

public(package) fun aiy_coin_registry_address(): address {
    AIY_COIN_REGISTRY_OBJECT_ID
}
...
/// Create a new object. Returns the `UID` that must be stored in a Aiy object.
