...
fn build_system_packages() {
    ...
    let aiy_system_path = packages_path.join("aiy-system");
    let aiy_framework_path = packages_path.join("aiy-framework");
...
fn check_diff(checked_in: &Path, built: &Path) {
    for path in [COMPILED_PACKAGES_DIR, DOCS_DIR, PUBLISHED_API_FILE] {
        ...
        if !output.status.success() {
            let header =
                "Generated and checked-in aiy-framework packages and/or docs do not match.\n\
...
fn build_packages(
    bridge_path: &Path,
    deepbook_path: &Path,
    aiy_system_path: &Path,
    aiy_framework_path: &Path,
    stdlib_path: &Path,
    out_dir: &Path,
) {
...
fn build_packages_with_move_config(
    ...
    let aiy_system = system_pkg.get_aiy_system_modules();
    let aiy_framework = framework_pkg.get_aiy_framework_modules();
    ...
    let aiy_system_members =
        serialize_modules_to_file(aiy_system, &compiled_packages_dir.join(system_dir)).unwrap();
    let aiy_framework_members =
        serialize_modules_to_file(aiy_framework, &compiled_packages_dir.join(framework_dir))
            .unwrap();
    ...
    let published_api = [
        aiy_system_members.join("\n"),
        aiy_framework_members.join("\n"),
