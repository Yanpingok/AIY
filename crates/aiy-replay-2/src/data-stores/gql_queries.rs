...
//! GQL Queries
//! Interface to the rpc for the gql schema defined in `crates\aiy-indexer-alt-graphql/schema.graphql`.
...
pub(crate) mod txn_query {
    ...
    pub(crate) async fn query(
        digest: String,
        data_store: &DataStore,
    ) -> Result<Option<(TransactionData, aiy_types::effects::TransactionEffects, u64)>, anyhow::Error>
    {
        ...
        let effects: aiy_types::effects::TransactionEffects = bcs::from_bytes(
            &CryptoBase64::decode(
                &effect_frag
                    .effects_bcs
                    .ok_or_else(|| {
...
pub(crate) mod object_query {
    ...
    #[derive(cynic::Scalar, Debug, Clone)]
    #[cynic(graphql_type = "AiyAddress")]
    pub(crate) struct AiyAddress(pub String);
    ...
    impl From<replay_interface::ObjectKey> for ObjectKey {
        fn from(key: replay_interface::ObjectKey) -> Self {
            ObjectKey {
                address: AiyAddress(key.object_id.to_string()),
