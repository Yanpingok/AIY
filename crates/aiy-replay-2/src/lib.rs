...
use aiy_types::{effects::TransactionEffects, supported_protocol_versions::Chain};
...
/// Load replay configuration from ~/.aiy/aiy_config/replay.toml file.
...
pub fn load_config_file() -> anyhow::Result<ReplayConfigStable> {
    let config_dir = match aiy_config_dir() {
        Ok(dir) => dir,
        Err(e) => {
...
pub fn print_effects_or_fork<W: Write>(
    ...
    if manager.member(Artifact::ForkedTransactionEffects).exists() {
        ...
        writeln!(
            w,
            "{}",
            AiyTransactionBlockEffects::try_from(tx_effects.clone())
