...
use aiy_core::authority::test_authority_builder::TestAuthorityBuilder;
use aiy_core::authority::AuthorityState;
use aiy_framework::DEFAULT_FRAMEWORK_PATH;
use aiy_graphql_rpc::test_infra::cluster::{RetentionConfig, SnapshotLagConfig};
use aiy_json_rpc_api::QUERY_MAX_RESULT_LIMIT;
use aiy_json_rpc_types::{
    DevInspectResults, DryRunTransactionBlockResponse, AiyExecutionStatus,
    AiyTransactionBlockEffects, AiyTransactionBlockEffectsAPI, AiyTransactionBlockEvents,
};
use aiy_protocol_config::{Chain, ProtocolConfig, ProtocolVersion};
use aiy_storage::{
    key_value_store::TransactionKeyValueStore, key_value_store_metrics::KeyValueStoreMetrics,
};
use aiy_swarm_config::genesis_config::AccountConfig;
use aiy_swarm_config::network_config_builder::KeyPairWrapper;
use aiy_types::base_types::{SequenceNumber, VersionNumber};
...
use aiy_types::digests::{ConsensusCommitDigest, TransactionDigest};
use aiy_types::effects::{TransactionEffects, TransactionEffectsAPI, TransactionEvents};
use aiy_types::execution_status::ExecutionFailureStatus;
...
use aiy_types::storage::{ObjectStore, RpcStateReader};
use aiy_types::transaction::Command;
use aiy_types::transaction::ProgrammableTransaction;
use aiy_types::utils::to_sender_signed_transaction_with_multi_signers;
use aiy_types::AIY_SYSTEM_ADDRESS;
use aiy_types::{
    base_types::{ObjectID, ObjectRef, AiyAddress, AIY_ADDRESS_LENGTH},
    crypto::{get_key_pair_from_rng, AccountKeyPair},
    event::Event,
    object::{self, Object},
    transaction::{Transaction, TransactionData, VerifiedTransaction},
    MOVE_STDLIB_ADDRESS, AIY_CLOCK_OBJECT_ID, AIY_FRAMEWORK_ADDRESS, AIY_SYSTEM_STATE_OBJECT_ID,
};
use aiy_types::{execution_status::ExecutionStatus, transaction::TransactionKind};
use aiy_types::{gas::GasCostSummary, object::GAS_VALUE_FOR_TESTING};
use aiy_types::{
    move_package::MovePackage,
    transaction::{Argument, CallArg, TransactionDataAPI, TransactionExpiration},
};
use aiy_types::{
    programmable_transaction_builder::ProgrammableTransactionBuilder, AIY_FRAMEWORK_PACKAGE_ID,
};
use aiy_types::{utils::to_sender_signed_transaction, AIY_SYSTEM_PACKAGE_ID};
use aiy_types::{BRIDGE_ADDRESS, MOVE_STDLIB_PACKAGE_ID};
use aiy_types::{DEEPBOOK_ADDRESS, AIY_DENY_LIST_OBJECT_ID};
use aiy_types::{DEEPBOOK_PACKAGE_ID, AIY_RANDOMNESS_STATE_OBJECT_ID};
...
const WELL_KNOWN_OBJECTS: &[ObjectID] = &[
    ...
    AIY_RANDOMNESS_STATE_OBJECT_ID,
];
...
pub struct OffChainConfig {
    ...
    /// URL for the Aiy REST API. To be passed to the offchain indexer if it uses the REST API.
    pub rest_api_url: Option<String>,
}

pub struct AiyTestAdapter {
...
impl AdapterInitConfig {
    fn from_args(init_cmd: InitCommand, aiy_args: AiyInitArgs) -> Self {
...
#[async_trait]
impl MoveTestAdapter<'_> for AiyTestAdapter {
    type ExtraPublishArgs = AiyPublishArgs;
    type ExtraRunArgs = AiyRunArgs;
    type ExtraInitArgs = AiyInitArgs;
    type ExtraValueArgs = AiyExtraValueArgs;
    type Subcommand = AiySubcommand<Self::ExtraValueArgs, Self::ExtraRunArgs>;
    ...
    fn render_command_input(
        ...
        match &task.command {
            TaskCommand::Subcommand(AiySubcommand::ProgrammableTransaction(..)) => {
    ...
    async fn init(
        ...
        assert!(
            ...
            "Must populate 'pre_compiled_deps' with Aiy framework"
        );
        ...
        let AdapterInitConfig {
            ...
            Some((init_cmd, aiy_args)) => AdapterInitConfig::from_args(init_cmd, aiy_args),
    ...
    async fn publish_modules(
        ...
        // we are assuming that all packages depend on Move Stdlib and Aiy Framework, so these
        // don't have to be provided explicitly as parameters
        dependencies.extend([MOVE_STDLIB_PACKAGE_ID, AIY_FRAMEWORK_PACKAGE_ID]);
    ...
    async fn call_function(
        ...
        self.next_task();
        let AiyRunArgs { summarize, .. } = extra;
    ...
    async fn handle_subcommand(
        ...
        match command {
            AiySubcommand::RunGraphql(RunGraphqlCommand {
                show_usage,
                show_headers,
            ...
            AiySubcommand::RunJsonRpc(RunJsonRpcCommand {
                show_headers,
                cursors,
            }) => {
            ...
            AiySubcommand::CreateCheckpoint(CreateCheckpointCommand { count }) => {
            ...
            AiySubcommand::AdvanceEpoch(cmd) => {
            ...
            AiySubcommand::AdvanceClock(advance_clock_command) => {
            ...
            AiySubcommand::SetRandomState(SetRandomStateCommand {
                randomness_round,
                random_bytes,
            ...
            AiySubcommand::AuthenticatorStateUpdate(AuthenticatorStateUpdateCommand {
                round,
                jwk_iss,
            ...
            AiySubcommand::ViewObject(ViewObjectCommand { id: fake_id }) => {
            ...
            AiySubcommand::TransferObject(TransferObjectCommand {
                id: fake_id,
                recipient,
                sender,
                gas_budget,
                ...
                let mut builder = ProgrammableTransactionBuilder::new();
                let obj_arg = AiyValue::Object(fake_id, None).into_argument(&mut builder, self)?;
                ...
                let transaction = self.sign_txn(sender, |sender, gas| {
                    ...
                    builder.command(aiy_types::transaction::Command::TransferObjects(
                        vec![obj_arg],
                        rec_arg,
                    ));
                    let pt = builder.finish();
            ...
            AiySubcommand::ConsensusCommitPrologue(ConsensusCommitPrologueCommand {
                timestamp_ms,
            }) => {
            ...
            AiySubcommand::ProgrammableTransaction(ProgrammableTransactionCommand {
                sender,
                sponsor,
                gas_budget,
                gas_price,
            ...
            AiySubcommand::UpgradePackage(UpgradePackageCommand {
                package,
                upgrade_capability,
                dependencies,
                sender,
            ...
            AiySubcommand::StagePackage(StagePackageCommand {
                syntax,
                dependencies,
            }) => {
                let syntax = syntax.unwrap_or_else(|| self.default_syntax());
            ...
            AiySubcommand::SetAddress(SetAddressCommand { address, input }) => {
                ...
                let (value, package) = match input {
                    AiyValue::Object(fake_id, version) => {
                        let id = match self.fake_to_real_object_id(fake_id) {
                            Some(id) => id,
                    ...
                    AiyValue::MoveValue(v) => {
                        let bytes = v.simple_serialize().unwrap();
                        let value: AccountAddress = bcs::from_bytes(&bytes)?;
                        (value, None)
                    }
                    AiyValue::Digest(_) => bail!("digest is not supported as an input"),
                    AiyValue::ObjVec(_) => bail!("obj vec is not supported as an input"),
                    AiyValue::Receiving(_, _) => bail!("receiving is not supported as an input"),
                    AiyValue::ImmShared(_, _) => {
...
impl AiyTestAdapter {
    ...
    async fn upgrade_package(
        ...
        // Argument::Input(0)
        AiyValue::Object(upgrade_capability, None).into_argument(&mut builder, self)?;
    ...
    fn tx_summary_from_effects(
        &mut self,
        effects: AiyTransactionBlockEffects,
        events: AiyTransactionBlockEvents,
    ) -> anyhow::Result<TxnSummary> {
    ...
    fn stabilize_str(&self, input: impl AsRef<str>) -> String {
        fn candidate_is_hex(s: &str) -> bool {
            const HEX_STR_LENGTH: usize = AIY_ADDRESS_LENGTH * 2;
            let n = s.len();
...
impl<'a> GetModule for &'a AiyTestAdapter {
...
static NAMED_ADDRESSES: Lazy<BTreeMap<String, NumericalAddress>> = Lazy::new(|| {
    ...
    // TODO fix Aiy framework constants
    map.insert(
        "aiy".to_string(),
        NumericalAddress::new(
            AIY_FRAMEWORK_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
        "aiy_system".to_string(),
        NumericalAddress::new(
            AIY_SYSTEM_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
...
pub static PRE_COMPILED: Lazy<PreCompiledProgramInfo> = Lazy::new(|| {
    ...
    let aiy_files: &Path = Path::new(DEFAULT_FRAMEWORK_PATH);
    let aiy_system_sources = {
        let mut buf = aiy_files.to_path_buf();
        buf.extend(["packages", "aiy-system", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let aiy_sources = {
        let mut buf = aiy_files.to_path_buf();
        buf.extend(["packages", "aiy-framework", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let aiy_deps = {
        let mut buf = aiy_files.to_path_buf();
    ...
    let pre_compiled_program = move_compiler::construct_pre_compiled_lib(
        vec![PackagePaths {
            name: Some(("aiy-framework".into(), config)),
            paths: vec![
                aiy_system_sources,
                aiy_sources,
                aiy_deps,
    ...
    match pre_compiled_program {
        Err((files, diags)) => {
            eprintln!("!!!Aiy framework failed to compile!!!");
...
async fn init_val_fullnode_executor(
    ...
    // For each named Aiy account without an address value, create an account with an address
    // and a gas object
...
async fn init_sim_executor(
    ...
    // For each named Aiy account without an address value, create a key pair
    for n in account_names {
    ...
    let addr_keys: Vec<_> = (0..num_custom_validator_accounts)
        ...
        .map(|_| {
            let (addr, account_key_pair): (AiyAddress, AccountKeyPair) =
...
impl ObjectStore for AiyTestAdapter {
...
impl ReadStore for AiyTestAdapter {
    ...
    fn get_committee(
        &self,
        epoch: aiy_types::committee::EpochId,
    ...
    fn get_lowest_available_checkpoint(
        &self,
    ) -> aiy_types::storage::error::Result<CheckpointSequenceNumber> {
        self.executor.get_lowest_available_checkpoint()
    }

    fn get_checkpoint_by_digest(
        &self,
        digest: &aiy_types::messages_checkpoint::CheckpointDigest,
