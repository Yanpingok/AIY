...
use aiy_swarm_config::network_config_builder::{ConfigBuilder, KeyPairWrapper};
use aiy_types::storage::{ReadStore, RpcStateReader};
use aiy_types::{
    base_types::{ObjectID, SequenceNumber, AiyAddress, VersionNumber},
...
pub struct PersistedStoreInner {
    // Checkpoint data
    checkpoints: DBMap<CheckpointSequenceNumber, aiy_types::messages_checkpoint::TrustedCheckpoint>,
    ...
    // Transaction data
    transactions: DBMap<TransactionDigest, aiy_types::transaction::TrustedTransaction>,
...
impl SimulatorStore for PersistedStore {
    ...
    fn get_system_state(&self) -> aiy_types::aiy_system_state::AiySystemState {
        aiy_types::aiy_system_state::get_aiy_system_state(self).expect("system state must exist")
    }

    fn get_clock(&self) -> aiy_types::clock::Clock {
        SimulatorStore::get_object(self, &aiy_types::AIY_CLOCK_OBJECT_ID)
    ...
    fn backing_store(&self) -> &dyn aiy_types::storage::BackingStore {
        self
    }
}
...
impl BackingPackageStore for PersistedStore {
    fn get_package_object(
        &self,
        package_id: &ObjectID,
    ) -> aiy_types::error::AiyResult<Option<PackageObject>> {
...
impl ChildObjectResolver for PersistedStore {
    ...
    ) -> aiy_types::error::AiyResult<Option<Object>> {
...
impl ParentSync for PersistedStore {
    fn get_latest_parent_entry_ref_deprecated(
        &self,
        _object_id: ObjectID,
    ) -> Option<aiy_types::base_types::ObjectRef> {
