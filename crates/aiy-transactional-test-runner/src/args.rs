...
use crate::test_adapter::{FakeID, AiyTestAdapter};
...
use aiy_graphql_rpc::test_infra::cluster::SnapshotLagConfig;
use aiy_types::base_types::{SequenceNumber, AiyAddress};
use aiy_types::move_package::UpgradePolicy;
use aiy_types::object::{Object, Owner};
use aiy_types::programmable_transaction_builder::ProgrammableTransactionBuilder;
use aiy_types::transaction::{Argument, CallArg, ObjectArg};

pub const AIY_ARGS_LONG: &str = "aiy-args";

#[derive(Clone, Debug, clap::Parser)]
pub struct AiyRunArgs {
    #[clap(long = "sender")]
...
pub struct AiyInitArgs {
    ...
    /// URL for the Aiy REST API. To be passed to the offchain indexer and reader.
    #[clap(long)]
...
pub struct ProgrammableTransactionCommand {
    ...
    #[clap(
        long = "inputs",
        value_parser = ParsedValue::<AiyExtraValueArgs>::parse,
        num_args(1..),
        action = clap::ArgAction::Append,
    )]
    pub inputs: Vec<ParsedValue<AiyExtraValueArgs>>,
}
...
pub struct SetAddressCommand {
    pub address: String,
    #[clap(value_parser = ParsedValue::<AiyExtraValueArgs>::parse)]
    pub input: ParsedValue<AiyExtraValueArgs>,
}
...
#[derive(Debug)]
pub enum AiySubcommand<ExtraValueArgs: ParsableValue, ExtraRunArgs: Parser> {
...
impl<ExtraValueArgs: ParsableValue, ExtraRunArgs: Parser> clap::FromArgMatches
    for AiySubcommand<ExtraValueArgs, ExtraRunArgs>
{
    fn from_arg_matches(matches: &clap::ArgMatches) -> Result<Self, clap::Error> {
        Ok(match matches.subcommand() {
            Some(("view-object", matches)) => {
                AiySubcommand::ViewObject(ViewObjectCommand::from_arg_matches(matches)?)
            }
            Some(("transfer-object", matches)) => {
                AiySubcommand::TransferObject(TransferObjectCommand::from_arg_matches(matches)?)
            }
            Some(("consensus-commit-prologue", matches)) => AiySubcommand::ConsensusCommitPrologue(
                ConsensusCommitPrologueCommand::from_arg_matches(matches)?,
            ),
            Some(("programmable", matches)) => AiySubcommand::ProgrammableTransaction(
                ProgrammableTransactionCommand::from_arg_matches(matches)?,
            ),
            Some(("upgrade", matches)) => {
                AiySubcommand::UpgradePackage(UpgradePackageCommand::from_arg_matches(matches)?)
            }
            Some(("stage-package", matches)) => {
                AiySubcommand::StagePackage(StagePackageCommand::from_arg_matches(matches)?)
            }
            Some(("set-address", matches)) => {
                AiySubcommand::SetAddress(SetAddressCommand::from_arg_matches(matches)?)
            }
            Some(("create-checkpoint", matches)) => {
                AiySubcommand::CreateCheckpoint(CreateCheckpointCommand::from_arg_matches(matches)?)
            }
            Some(("advance-epoch", matches)) => {
                AiySubcommand::AdvanceEpoch(AdvanceEpochCommand::from_arg_matches(matches)?)
            }
            Some(("advance-clock", matches)) => {
                AiySubcommand::AdvanceClock(AdvanceClockCommand::from_arg_matches(matches)?)
            }
            Some(("set-random-state", matches)) => {
                AiySubcommand::SetRandomState(SetRandomStateCommand::from_arg_matches(matches)?)
            }
            Some(("authenticator-state-update", matches)) => {
                AiySubcommand::AuthenticatorStateUpdate(
                    AuthenticatorStateUpdateCommand::from_arg_matches(matches)?,
                )
            }
            Some(("view-checkpoint", _)) => AiySubcommand::ViewCheckpoint,
            Some(("run-graphql", matches)) => {
                AiySubcommand::RunGraphql(RunGraphqlCommand::from_arg_matches(matches)?)
            }
            Some(("run-jsonrpc", matches)) => {
                AiySubcommand::RunJsonRpc(RunJsonRpcCommand::from_arg_matches(matches)?)
            }
            Some(("bench", matches)) => AiySubcommand::Bench(
    ...
    for AiySubcommand<ExtraValueArgs, ExtraRunArgs>
{
    fn command() -> clap::Command {
        clap::Command::new("aiy_sub_command")
...
impl AiyExtraValueArgs {
    fn parse_object_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(AiyExtraValueArgs::Object(fake_id, version))
    }
    ...
    fn parse_receiving_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(AiyExtraValueArgs::Receiving(fake_id, version))
    }
    ...
    fn parse_read_shared_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(AiyExtraValueArgs::ImmShared(fake_id, version))
    }
    ...
    fn parse_digest_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(AiyExtraValueArgs::Digest(package.to_owned()))
    }
    ...
    fn parse_receiving_or_object_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        let fake_id = if let Some(ValueToken::Comma) = parser.peek_tok() {
            ...
            u256_bytes.reverse();
            let address: AiyAddress = AiyAddress::from_bytes(&u256_bytes).unwrap();
...
impl AiyValue {
    fn assert_move_value(self) -> MoveValue {
        match self {
            AiyValue::MoveValue(v) => v,
            AiyValue::Object(_, _) => panic!("unexpected nested Aiy object in args"),
            AiyValue::ObjVec(_) => panic!("unexpected nested Aiy object vector in args"),
            AiyValue::Digest(_) => panic!("unexpected nested Aiy package digest in args"),
            AiyValue::Receiving(_, _) => panic!("unexpected nested Aiy receiving object in args"),
            AiyValue::ImmShared(_, _) => panic!("unexpected nested Aiy shared object in args"),
        }
    }

    fn assert_object(self) -> (FakeID, Option<SequenceNumber>) {
        match self {
            AiyValue::MoveValue(_) => panic!("unexpected nested non-object value in args"),
            AiyValue::Object(id, version) => (id, version),
            AiyValue::ObjVec(_) => panic!("unexpected nested Aiy object vector in args"),
            AiyValue::Digest(_) => panic!("unexpected nested Aiy package digest in args"),
            AiyValue::Receiving(_, _) => panic!("unexpected nested Aiy receiving object in args"),
            AiyValue::ImmShared(_, _) => panic!("unexpected nested Aiy shared object in args"),
    ...
    fn resolve_object(
        ...
        let obj_res = if let Some(v) = version {
            aiy_types::storage::ObjectStore::get_object_by_key(&*test_adapter.executor, &id, v)
        } else {
            aiy_types::storage::ObjectStore::get_object(&*test_adapter.executor, &id)
        };
    ...
    pub(crate) fn into_call_arg(self, test_adapter: &AiyTestAdapter) -> anyhow::Result<CallArg> {
        Ok(match self {
            AiyValue::Object(fake_id, version) => {
                CallArg::Object(Self::object_arg(fake_id, version, test_adapter)?)
            }
            AiyValue::MoveValue(v) => CallArg::Pure(v.simple_serialize().unwrap()),
            AiyValue::Receiving(fake_id, version) => {
                CallArg::Object(Self::receiving_arg(fake_id, version, test_adapter)?)
            }
            AiyValue::ImmShared(fake_id, version) => {
    ...
    pub(crate) fn into_argument(
        ...
        match self {
            AiyValue::ObjVec(vec) => builder.make_obj_vec(
                vec.iter()
...
impl ParsableValue for AiyExtraValueArgs {
    type ConcreteValue = AiyValue;
    ...
    fn move_value_into_concrete(v: MoveValue) -> anyhow::Result<Self::ConcreteValue> {
        Ok(AiyValue::MoveValue(v))
    }

    fn concrete_vector(elems: Vec<Self::ConcreteValue>) -> anyhow::Result<Self::ConcreteValue> {
        if !elems.is_empty() && matches!(elems[0], AiyValue::Object(_, _)) {
            Ok(AiyValue::ObjVec(
                elems.into_iter().map(AiyValue::assert_object).collect(),
            ))
        } else {
            Ok(AiyValue::MoveValue(MoveValue::Vector(
                elems.into_iter().map(AiyValue::assert_move_value).collect(),
            )))
        }
    }

    fn concrete_struct(values: Vec<Self::ConcreteValue>) -> anyhow::Result<Self::ConcreteValue> {
        Ok(AiyValue::MoveValue(MoveValue::Struct(MoveStruct(
    ...
    fn into_concrete_value(
        ...
        match self {
            AiyExtraValueArgs::Object(id, version) => Ok(AiyValue::Object(id, version)),
            AiyExtraValueArgs::Digest(pkg) => Ok(AiyValue::Digest(pkg)),
            AiyExtraValueArgs::Receiving(id, version) => Ok(AiyValue::Receiving(id, version)),
            AiyExtraValueArgs::ImmShared(id, version) => Ok(AiyValue::ImmShared(id, version)),
        }
    }
}
