...
use std::path::Path;
use aiy_move_build::{implicit_deps, set_aiy_flavor, AiyPackageHooks};
...
// Additional per-command metadata that can be passed from other commands (e.g., the Aiy CLI) that
...
pub fn execute_move_command(
    ...
    if let Some(err_msg) = set_aiy_flavor(&mut build_config) {
        anyhow::bail!(err_msg);
    }
