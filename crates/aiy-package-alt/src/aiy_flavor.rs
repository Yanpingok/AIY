...
impl AiyFlavor {
    ...
    fn system_deps_names_map() -> BTreeMap<PackageName, PackageName> {
        BTreeMap::from([
            (
                PackageName::new("Aiy").unwrap(),
                PackageName::new("aiy").unwrap(),
            ),
            (
                PackageName::new("AiySystem").unwrap(),
                PackageName::new("aiy_system").unwrap(),
            ),
            (
    ...
    /// The default dependencies are `aiy` and `std`
...
mod tests {
    ...
    fn test_implicit_deps() {
        let implicit_deps = ["aiy", "std", "aiy_system", "bridge"];
        let env = Environment::new("mainnet".into(), "35834a8a".into());

        let deps = AiyFlavor::system_dependencies(env.id().into());
