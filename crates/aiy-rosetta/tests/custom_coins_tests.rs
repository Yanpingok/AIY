...
use aiy_json_rpc_types::{
    ObjectChange, AiyExecutionStatus, AiyTransactionBlockEffectsAPI,
...
use aiy_rosetta::types::{
    ...
    CurrencyMetadata, NetworkIdentifier, AiyEnv,
};
use aiy_rosetta::types::{Currencies, OperationType};
use aiy_rosetta::CoinMetadataCache;
use aiy_rosetta::AIY;
use aiy_types::coin::COIN_MODULE_NAME;
...
use aiy_types::{Identifier, AIY_FRAMEWORK_PACKAGE_ID};
...
async fn test_custom_coin_balance() {
    ...
    const AIY_BALANCE: u64 = 150_000_000_000_000_000;
    const COIN1_BALANCE: u64 = 100_000_000;
    const COIN2_BALANCE: u64 = 200_000_000;
    let test_cluster = TestClusterBuilder::new().build().await;
    let client = test_cluster.wallet.get_client().await.unwrap();
    ...
    let request = AccountBalanceRequest {
        ...
        currencies: Currencies(vec![aiy_currency, test_coin_currency]),
    };
    ...
    assert_eq!(response.balances[0].value, AIY_BALANCE as i128);
    assert_eq!(
...
async fn test_custom_coin_without_symbol() {
    ...
    for op in ops {
        if op.type_ == OperationType::AiyBalanceChange {
