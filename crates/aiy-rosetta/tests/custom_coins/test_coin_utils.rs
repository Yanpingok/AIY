...
use aiy_json_rpc_types::{
    ObjectChange, AiyObjectDataFilter, AiyObjectDataOptions, AiyObjectResponseQuery, AiyRawData,
...
use aiy_types::{Identifier, TypeTag, AIY_FRAMEWORK_PACKAGE_ID};
...
pub async fn select_gas(
    ...
    if let Some(gas) = input_gas {
        ...
        let object = read_api
            .get_object_with_options(gas, AiyObjectDataOptions::new())
            .await?
    ...
    let gas_objs = read_api
        .get_owned_objects(
            ...
            Some(AiyObjectResponseQuery {
                filter: Some(AiyObjectDataFilter::StructType(GasCoin::type_())),
                options: Some(AiyObjectDataOptions::new().with_bcs()),
            }),
            None,
            None,
        )
    ...
    for obj in gas_objs {
        let AiyRawData::MoveObject(raw_obj) = &obj
            .data
            .as_ref()
...
pub async fn mint(
    ...
    keystore: &Keystore,
    init_ret: InitRet,
    balances_to: Vec<(u64, AiyAddress)>,
