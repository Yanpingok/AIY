...
use aiy_rosetta::operations::Operations;
use aiy_rosetta::types::{
    AccountBalanceRequest, AccountBalanceResponse, AccountIdentifier, Currency, NetworkIdentifier,
    SubAccount, SubAccountType, AiyEnv,
};
use aiy_rosetta::types::{Currencies, OperationType, TransactionIdentifierResponse};
use aiy_rosetta::CoinMetadataCache;
use aiy_sdk::rpc_types::{AiyExecutionStatus, AiyTransactionBlockEffectsAPI};
...
async fn test_get_staked_aiy() {
    ...
    let network_identifier = NetworkIdentifier {
        blockchain: "aiy".to_string(),
        network: AiyEnv::LocalNet,
    };
...
#[tokio::test]
async fn test_pay_aiy_multiple_times() {
    let test_cluster = TestClusterBuilder::new()
...
async fn test_transfer_single_gas_coin() {
    ...
    let input_objects = pt
        ...
        .collect::<Vec<_>>();
    let gas = vec![get_random_aiy(&client, sender, input_objects).await];
    ...
    let signature = keystore
        .sign_secure(&sender, &data, Intent::aiy_transaction())
        .await
        .unwrap();
    ...
    operations.into_iter().for_each(|op| {
        ...
        if op.type_ == OperationType::PayAiy {
...
/// The below AiyTransactionBlockResponse is created by using the below contract:
/// ```move
/// module vault::vault;
///
...
/// }
///
/// public fun from_coin(coin: Coin<AIY>, ctx: &mut TxContext): Vault {
///     Vault {
///         id: object::new(ctx),
...
/// }
///
/// public fun to_coin(self: Vault, ctx: &mut TxContext): Coin<AIY> {
///     let Vault { id, balance } = self;
///     id.delete();
///     balance.into_coin(ctx)
/// }
///
/// public fun amount_to_coin(self: &mut Vault, amount: u64, ctx: &mut TxContext): Coin<AIY> {
...
async fn test_balance_from_obj_paid_eq_gas() {
    ...
    let client = test_cluster.wallet.get_client().await.unwrap();
    let response: AiyTransactionBlockResponse = serde_json::from_value(json!({
    ...
    let mut balance_changes: HashMap<AiyAddress, i128> = HashMap::new();
    operations.into_iter().for_each(|op| {
    ...
    assert_eq!(
        *balance_changes
            .get(&AiyAddress::from_str(RECIPIENT).unwrap())
            .unwrap(),
        AMOUNT
    );
    assert_eq!(
        *balance_changes
            .get(&AiyAddress::from_str(SENDER).unwrap())
            .unwrap_or(&0),
        0
    );
}
