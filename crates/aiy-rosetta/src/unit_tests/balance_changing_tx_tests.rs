...
use std::str::FromStr;
use aiy_json_rpc_types::{Coin, AiyTransactionBlockResponseOptions};
use aiy_json_rpc_types::{
    ObjectChange, AiyObjectDataOptions, AiyObjectRef, AiyObjectResponseQuery,
};
use aiy_keys::keystore::AccountKeystore;
use aiy_keys::keystore::Keystore;
use aiy_move_build::BuildConfig;
use aiy_sdk::rpc_types::{
    OwnedObjectRef, AiyData, AiyExecutionStatus, AiyTransactionBlockEffectsAPI,
    AiyTransactionBlockResponse,
};
use aiy_sdk::AiyClient;
use aiy_types::base_types::{FullObjectRef, ObjectID, ObjectRef, AiyAddress};
use aiy_types::gas_coin::{GasCoin, GAS};
...
async fn test_transfer_aiy() {
    ...
    let pt = {
        ...
        builder.transfer_aiy(recipient, Some(50000));
        builder.finish()
    };
    test_transaction(
        &client,
...
#[tokio::test]
async fn test_transfer_aiy_whole_coin() {
    let network = TestClusterBuilder::new().build().await;
    let client = network.wallet.get_client().await.unwrap();
    let keystore = &network.wallet.config.keystore;
    let rgp = network.get_reference_gas_price().await;

    // Test transfer aiy whole coin
    let addresses = network.get_addresses();
    let sender = get_random_address(&addresses, vec![]);
    let recipient = get_random_address(&addresses, vec![sender]);
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_aiy(recipient, None);
        builder.finish()
    };
    test_transaction(
        &client,
...
async fn test_transfer_object() {
    ...
    let sender = get_random_address(&addresses, vec![]);
    let recipient = get_random_address(&addresses, vec![sender]);
    let object_ref = get_random_aiy(&client, sender, vec![]).await.object_ref();
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder
...
async fn test_split_coin() {
    ...
    let coin = get_random_aiy(&client, sender, vec![]).await.object_ref();
    let tx = client
        .transaction_builder()
...
async fn test_merge_coin() {
    ...
    let sender = get_random_address(&network.get_addresses(), vec![]);
    let coin = get_random_aiy(&client, sender, vec![]).await.object_ref();
    let coin2 = get_random_aiy(&client, sender, vec![coin.0])
        .await
        .object_ref();
    let tx = client
        .transaction_builder()
...
async fn test_pay_multiple_coin_multiple_recipient() {
    ...
    let coin1 = get_random_aiy(&client, sender, vec![]).await.object_ref();
    let coin2 = get_random_aiy(&client, sender, vec![coin1.0])
        .await
        .object_ref();
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
...
#[tokio::test]
async fn test_pay_aiy_multiple_coin_same_recipient() {
    let network = TestClusterBuilder::new().build().await;
    let client = network.wallet.get_client().await.unwrap();
    let keystore = &network.wallet.config.keystore;
    let rgp = network.get_reference_gas_price().await;
...
async fn test_pay_aiy() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder
            .pay_aiy(vec![recipient1, recipient2], vec![1000000, 2000000])
            .unwrap();
        builder.finish()
    };
    test_transaction(
...
#[tokio::test]
async fn test_stake_aiy_with_none_amount() {
    let network = TestClusterBuilder::new().build().await;
    let client = network.wallet.get_client().await.unwrap();
    let keystore = &network.wallet.config.keystore;
    let rgp = network.get_reference_gas_price().await;
...
async fn test_delegation_parsing() -> Result<(), anyhow::Error> {
    ...
    let client = network.wallet.get_client().await.unwrap();
    let sender = get_random_address(&network.get_addresses(), vec![]);
    let coin = get_random_aiy(&client, sender, vec![]).await;
    ...
    let ops: Operations = serde_json::from_value(json!(
        [{
            ...
            "amount" : { "value": "-100000" , "currency": { "symbol": "AIY", "decimals": 9}},
...
// Record current Aiy balance of an address then execute the transaction,
// and compare the balance change reported by the event against the actual balance change.
async fn test_transaction(
    client: &AiyClient,
    keystore: &Keystore,
    addr_to_check: Vec<AiyAddress>,
    sender: AiyAddress,
    ...
    let gas = if !gas.is_empty() {
        ...
        let input_objects = tx
            ...
            .collect::<Vec<_>>();
        vec![get_random_aiy(client, sender, input_objects)
            .await
            .object_ref()]
    };
    ...
    let signature = keystore
        .sign_secure(&data.sender(), &data, Intent::aiy_transaction())
        .await
        .unwrap();
...
fn extract_balance_changes_from_ops(ops: Operations) -> HashMap<AiyAddress, i128> {
    ops.into_iter()
        .fold(HashMap::<AiyAddress, i128>::new(), |mut changes, op| {
            if let Some(OperationStatus::Success) = op.status {
                match op.type_ {
                    OperationType::AiyBalanceChange
                    | OperationType::Gas
                    | OperationType::PayAiy
                    ...
                    | OperationType::Stake => {
                        if let (Some(addr), Some(amount)) = (op.account, op.amount) {
                            ...
                            if amount.currency.metadata.coin_type
                                == aiy_types::TypeTag::from(GAS::type_()).to_canonical_string(true)
                            {
...
async fn get_random_aiy(client: &AiyClient, sender: AiyAddress, except: Vec<ObjectID>) -> Coin {
    let coins = client
        .coin_read_api()
...
fn get_random_address(addresses: &[AiyAddress], except: Vec<AiyAddress>) -> AiyAddress {
    *addresses
        .iter()
...
async fn get_balance(client: &AiyClient, address: AiyAddress) -> u64 {
    let coins = client
        .read_api()
        .get_owned_objects(
            address,
