...
use aiy_types::governance::ADD_STAKE_FUN_NAME;
use aiy_types::aiy_system_state::AIY_SYSTEM_MODULE_NAME;
use aiy_types::transaction::{Argument, CallArg, Command, ObjectArg, ProgrammableTransaction};
    ...
    async fn try_fetch_needed_objects(
        ...
        if amount.is_none() {
            ...
            let budget = match budget {
                Some(budget) => budget,
                None => {
                    let pt = stake_pt(validator, total_aiy_balance as u64, true, &extra_gas_coins)?;
...
pub fn stake_pt(
    ...
    let (validator, system_state) = if !stake_all {
        ...
        let state = builder.input(CallArg::AIY_SYSTEM_MUT)?;
        (validator, state)
    } else {
        let state = builder.input(CallArg::AIY_SYSTEM_MUT)?;
        let validator = builder.input(CallArg::Pure(bcs::to_bytes(&validator)?))?;
