...
use aiy_types::transaction::{Argument, Command, ObjectArg, ProgrammableTransaction};
...
#[async_trait]
impl TryConstructTransaction for PayAiy {
    async fn try_fetch_needed_objects(
        ...
        if let Some(budget) = budget {
            ...
            let total_aiy_balance = all_coins.iter().map(|c| c.balance).sum::<u64>() as i128;
        ...
        let total_amount = amounts.iter().sum::<u64>();
        let pay_aiy_pt = |extra_gas_coins: &[ObjectRef]| {
            pay_aiy_pt(recipients.clone(), amounts.clone(), extra_gas_coins)
        };
        collect_coins_until_budget_met(client, sender, pay_aiy_pt, total_amount, gas_price).await
    }
}

/// Creates the `ProgrammableTransaction` for a pay-aiy operation.
/// In case pay-aiy needs more than 255 gas-coins to be smashed, it tries to merge the surplus
