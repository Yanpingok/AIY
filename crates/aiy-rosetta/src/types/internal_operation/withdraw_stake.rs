...
use aiy_types::transaction::{CallArg, Command, ObjectArg, ProgrammableTransaction};
...
impl TryConstructTransaction for WithdrawStake {
    async fn try_fetch_needed_objects(
        ...
        let responses = client
            .read_api()
            .multi_get_object_with_options(stake_ids, AiyObjectDataOptions::default())
            .await?;
        let stake_refs = responses
            .into_iter()
            .map(|stake| stake.into_object().map(|o| o.object_ref()))
            .collect::<Result<Vec<_>, _>>()
            .map_err(AiyError::from)?;
        ...
        let total_aiy_balance = gas_coins_iter.clone().map(|c| c.balance).sum::<u64>() as i128;
...
pub fn withdraw_stake_pt(
    ...
    for stake_id in stake_objs {
        ...
        let (system_state, id) = if !withdraw_all {
            let system_state = builder.input(CallArg::AIY_SYSTEM_MUT)?;
