...
use aiy_types::base_types::{ObjectID, ObjectRef, SequenceNumber, AiyAddress};
use aiy_types::digests::ObjectDigest;
use aiy_types::transaction::{ProgrammableTransaction, TransactionData};

use crate::errors::Error;
use crate::types::ConstructionMetadata;
use pay_coin::pay_coin_pt;
pub use pay_coin::PayCoin;
use pay_aiy::pay_aiy_pt;
pub use pay_aiy::PayAiy;
use stake::stake_pt;
pub use stake::Stake;
...
pub struct TransactionObjectData {
    ...
    /// Refers to the sum of the `Coin<AIY>` balance of the coins participating in the transaction;
    /// either as gas or as objects.
    pub total_aiy_balance: i128,
    pub budget: u64,
}
...
impl InternalOperation {
    pub fn sender(&self) -> AiyAddress {
        match self {
            InternalOperation::PayAiy(PayAiy { sender, .. })
    ...
    pub fn try_into_data(self, metadata: ConstructionMetadata) -> Result<TransactionData, Error> {
        let pt = match self {
            Self::PayAiy(PayAiy {
                ...
                amounts,
                ..
            }) => pay_aiy_pt(recipients, amounts, &metadata.extra_gas_coins)?,
    ...
    loop {
        ...
        if gathered < amount + budget {
            return Err(Error::InvalidInput(format!(
                "Address {sender} does not have amount: {amount} + budget: {budget} balance. AIY balance: {gathered}."
            )));
        }
