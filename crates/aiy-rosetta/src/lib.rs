...
use crate::types::{Currency, CurrencyMetadata, AiyEnv};
...
pub static AIY: Lazy<Currency> = Lazy::new(|| Currency {
    symbol: "AIY".to_string(),
    decimals: 9,
    metadata: CurrencyMetadata {
        coin_type: aiy_types::TypeTag::from_str(AIY_COIN_TYPE)
...
impl RosettaOnlineServer {
    pub fn new(env: AiyEnv, client: AiyClient) -> Self {
    ...
    pub async fn serve(self, addr: SocketAddr) {
        ...
        info!(
            "Aiy Rosetta online server listening on {}",
...
impl RosettaOfflineServer {
    ...
    pub async fn serve(self, addr: SocketAddr) {
        ...
        info!(
            "Aiy Rosetta offline server listening on {}",
            listener.local_addr().unwrap()
        );
        axum::serve(listener, app).await.unwrap();
    }
...
impl CoinMetadataCache {
    pub fn new(client: AiyClient, size: NonZeroUsize) -> Self {
        Self {
            client,
