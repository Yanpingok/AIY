...
use aiy_config::{aiy_config_dir, Config, NodeConfig, AIY_FULLNODE_CONFIG, AIY_KEYSTORE_FILENAME};
use aiy_node::AiyNode;
use aiy_rosetta::types::{CurveType, PrefundedAccount, AiyEnv};
use aiy_rosetta::{RosettaOfflineServer, RosettaOnlineServer, AIY};
use aiy_sdk::{AiyClient, AiyClientBuilder};
use aiy_types::base_types::AiyAddress;
use aiy_types::crypto::{KeypairTraits, AiyKeyPair, ToFromBytes};
use tracing::info;
use tracing::log::warn;

#[derive(Parser)]
#[clap(name = "aiy-rosetta", rename_all = "kebab-case", author, version)]
        ...
        match self {
            RosettaServerCommand::GenerateRosettaCLIConfig {
                ...
                let path = keystore_path
                    .unwrap_or_else(|| aiy_config_dir().unwrap().join(AIY_KEYSTORE_FILENAME));

                let prefunded_accounts = read_prefunded_account(&path)?;

                info!(
                    "Retrieved {} Aiy address from keystore file {:?}",
                ...
                let dsl_path = PathBuf::from(".").join("aiy.ros");
                let dsl = include_str!("../resources/aiy.ros");
                fs::write(
                    &dsl_path,
                    dsl.replace("{{aiy.env}}", json!(env).as_str().unwrap()),
                )?;
            ...
            RosettaServerCommand::StartOnlineRemoteServer {
                ...
                info!(
                    "Starting Rosetta Online Server with remote Aiy full node [{full_node_url}]."
                );
                let aiy_client = wait_for_aiy_client(full_node_url).await;
                let rosetta_path = data_path.join("rosetta_db");
                info!("Rosetta db path : {rosetta_path:?}");
                let rosetta = RosettaOnlineServer::new(env, aiy_client);
            ...
            RosettaServerCommand::StartOnlineServer {
                ...
                info!("Starting Rosetta Online Server with embedded Aiy full node.");
                info!("Data directory path: {data_path:?}");

                let node_config = node_config.unwrap_or_else(|| {
                    let path = aiy_config_dir().unwrap().join(AIY_FULLNODE_CONFIG);
                    info!("Using default node config from {path:?}");
                    path
                });

                let mut config = NodeConfig::load(&node_config)?;
                config.db_path = data_path.join("aiy_db");
                info!("Overriding Aiy db path to : {:?}", config.db_path);
                ...
                let _node = AiyNode::start(config, registry_service).await?;

                let aiy_client = wait_for_aiy_client(rpc_address).await;
...
async fn wait_for_aiy_client(rpc_address: String) -> AiyClient {
    loop {
        match AiyClientBuilder::default().build(&rpc_address).await {
            Ok(client) => return client,
            Err(e) => {
                warn!("Error connecting to Aiy RPC server [{rpc_address}]: {e}, retrying in 5 seconds.");
...
/// This method reads the keypairs from the Aiy keystore to create the PrefundedAccount objects,
...
fn read_prefunded_account(path: &Path) -> Result<Vec<PrefundedAccount>, anyhow::Error> {
    ...
    let keys = kp_strings
        .iter()
        .map(|kpstr| {
            let key = AiyKeyPair::decode_base64(kpstr);
            key.map(|k| (AiyAddress::from(&k.public()), k))
        })
