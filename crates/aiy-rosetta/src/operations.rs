...
use aiy_json_rpc_types::AiyProgrammableMoveCall;
use aiy_json_rpc_types::AiyProgrammableTransactionBlock;
use aiy_json_rpc_types::{BalanceChange, AiyArgument};
use aiy_json_rpc_types::{AiyCallArg, AiyCommand};
use aiy_sdk::rpc_types::{
    AiyTransactionBlockData, AiyTransactionBlockDataAPI, AiyTransactionBlockEffectsAPI,
    AiyTransactionBlockKind, AiyTransactionBlockResponse,
};
use aiy_types::base_types::{ObjectID, SequenceNumber, AiyAddress};
use aiy_types::gas_coin::GasCoin;
use aiy_types::governance::{ADD_STAKE_FUN_NAME, WITHDRAW_STAKE_FUN_NAME};
use aiy_types::object::Owner;
use aiy_types::aiy_system_state::AIY_SYSTEM_MODULE_NAME;
use aiy_types::transaction::TransactionData;
use aiy_types::{AIY_SYSTEM_ADDRESS, AIY_SYSTEM_PACKAGE_ID};

use crate::types::internal_operation::{PayCoin, PayAiy, Stake, WithdrawStake};
use crate::types::{
...
use crate::{CoinMetadataCache, Error, AIY};
...
impl Operations {
    ...
    pub fn into_internal(self) -> Result<InternalOperation, Error> {
        ...
        match type_ {
            OperationType::PayAiy => self.pay_aiy_ops_to_internal(),
    ...
    fn pay_aiy_ops_to_internal(self) -> Result<InternalOperation, Error> {
        ...
        Ok(InternalOperation::PayAiy(PayAiy {
            sender,
            recipients,
            amounts,
        }))
    ...
    fn from_transaction(
        tx: AiyTransactionBlockKind,
        sender: AiyAddress,
        status: Option<OperationStatus>,
    ) -> Result<Vec<Operation>, Error> {
        Ok(match tx {
            AiyTransactionBlockKind::ProgrammableTransaction(pt)
    ...
    fn parse_programmable_transaction(
        sender: AiyAddress,
        status: Option<OperationStatus>,
        pt: AiyProgrammableTransactionBlock,
    ) -> Result<Vec<Operation>, Error> {
        #[derive(Debug)]
        enum KnownValue {
            GasCoin(u64),
            ...
            let amounts = amounts
                ...
                .map(|amount| {
                    let value: u64 = match *amount {
                        ...
                        AiyArgument::GasCoin
                        | AiyArgument::Result(_)
                        | AiyArgument::NestedResult(_, _) => return None,
                    };
        ...
        fn transfer_object(
            aggregated_recipients: &mut HashMap<AiyAddress, u64>,
            inputs: &[AiyCallArg],
            known_results: &[Vec<KnownValue>],
            objs: &[AiyArgument],
            recipient: AiyArgument,
        ) -> Option<Vec<KnownValue>> {
            let addr = match recipient {
                AiyArgument::Input(i) => inputs.get(i as usize)?.pure()?.to_aiy_address().ok()?,
                AiyArgument::GasCoin | AiyArgument::Result(_) | AiyArgument::NestedResult(_, _) => {
                    return None
                }
            };
            for obj in objs {
                let value = match *obj {
                    ...
                    AiyArgument::GasCoin | AiyArgument::Input(_) => return None,
                };
        ...
        fn stake_call(
            inputs: &[AiyCallArg],
            known_results: &[Vec<KnownValue>],
            call: &AiyProgrammableMoveCall,
        ) -> Result<Option<(Option<u64>, AiyAddress)>, Error> {
            let AiyProgrammableMoveCall { arguments, .. } = call;
            let (amount, validator) = match &arguments[..] {
                [_, coin, validator] => {
                    ...
                    let (some_amount, validator) = match validator {
                        ...
                        AiyArgument::Input(i) => (*i==1, inputs.get(*i as usize).and_then(|input| input.pure()).map(|v|v.to_aiy_address()).transpose()),
                        _=> return Ok(None),
                    };
        ...
        let AiyProgrammableTransactionBlock { inputs, commands } = &pt;
        let mut known_results: Vec<Vec<KnownValue>> = vec![];
        let mut aggregated_recipients: HashMap<AiyAddress, u64> = HashMap::new();
        ...
        for command in commands {
            let result = match command {
                AiyCommand::SplitCoins(coin, amounts) => {
                    split_coins(inputs, &known_results, *coin, amounts)
                }
                AiyCommand::TransferObjects(objs, addr) => transfer_object(
                ...
                AiyCommand::MoveCall(m) if Self::is_stake_call(m) => {
                ...
                AiyCommand::MoveCall(m) if Self::is_unstake_call(m) => {
                    let stake_id = unstake_call(inputs, m)?;
                    stake_ids.push(stake_id);
                    Some(vec![])
                }
                AiyCommand::MergeCoins(_merge_into, _merges) => {
                ...
                aggregated_recipients
                    ...
                    .map(|(recipient, amount)| {
                        currency = inputs.iter().last().and_then(|arg| {
                            if let AiyCallArg::Pure(value) = arg {
                                let bytes = value
                                    .value()
                                    .to_json_value()
                                    .as_array()?
                        ...
                        match currency {
                            ...
                            None => Operation::pay_aiy(status, recipient, amount.into()),
                        }
                    }),
            );
            match currency {
                ...
                _ => operations.push(Operation::pay_aiy(status, sender, -(total_paid as i128))),
            }
    ...
    fn is_stake_call(tx: &AiyProgrammableMoveCall) -> bool {
        tx.package == AIY_SYSTEM_PACKAGE_ID
            && tx.module == AIY_SYSTEM_MODULE_NAME.as_str()
            && tx.function == ADD_STAKE_FUN_NAME.as_str()
    }

    fn is_unstake_call(tx: &AiyProgrammableMoveCall) -> bool {
        tx.package == AIY_SYSTEM_PACKAGE_ID
            && tx.module == AIY_SYSTEM_MODULE_NAME.as_str()
    ...
    fn process_balance_change(
        gas_owner: AiyAddress,
        gas_used: i128,
        balance_changes: &[(BalanceChange, Currency)],
        status: Option<OperationStatus>,
        balances: HashMap<(AiyAddress, Currency), i128>,
        ...
        *balances.entry((gas_owner, AIY.clone())).or_default() -= gas_used;
    ...
    fn is_gascoin_transfer(tx: &AiyTransactionBlockKind) -> bool {
        if let AiyTransactionBlockKind::ProgrammableTransaction(pt) = tx {
            let AiyProgrammableTransactionBlock {
                inputs: _,
                commands,
            } = &pt;
            return commands.iter().any(|command| match command {
                AiyCommand::TransferObjects(objs, _) => {
                    objs.iter().any(|&obj| obj == AiyArgument::GasCoin)
                }
                _ => false,
            });
        }
    ...
    fn add_missing_gas_owner(operations: &mut Vec<Operation>, gas_owner: AiyAddress) {
        if !operations.iter().any(|operation| {
            if let Some(amount) = &operation.amount {
                if let Some(account) = &operation.account {
                    if account.address == gas_owner && amount.currency == *AIY {
                        return true;
                    }
                }
            }
    ...
    fn validate_operations(
        ...
        let balances: HashMap<(AiyAddress, Currency), i128> = HashMap::new();
    ...
    fn process_gascoin_transfer(
        ...
        data: AiyTransactionBlockData,
        new_gas_owner: AiyAddress,
        gas_used: i128,
        initial_balance_changes: &[(BalanceChange, Currency)],
        ...
        if Self::is_gascoin_transfer(tx) && prev_gas_owner != new_gas_owner {
            ...
            for operation in &mut operations {
                match operation.type_ {
                    ...
                    OperationType::AiyBalanceChange => {
                        ...
                        if account.address == prev_gas_owner && amount.currency == *AIY {
                            amount.value -= gas_used;
                        } else if account.address == new_gas_owner && amount.currency == *AIY {
                            amount.value += gas_used;
                        }
                    }
                    _ => {
...
impl Operations {
    pub async fn try_from_response(
        ...
        let staking_balance = if principal_amounts != 0 {
            *accounted_balances.entry((sender, AIY.clone())).or_default() -= principal_amounts;
            *accounted_balances.entry((sender, AIY.clone())).or_default() -= reward_amounts;
            vec![
        ...
        // and convert BalanceChange to PayAiy when GasCoin is transferred
        ...
        // This is a workaround for the payCoin cases that are mistakenly considered to be payAiy operations
        // In this case we remove any irrelevant, AIY specific operation entries that sum up to 0 balance changes per address
        ...
        let mutually_cancelling_balances: HashMap<_, _> = ops
            ...
            .fold(
                HashMap::new(),
                |mut balances: HashMap<(AiyAddress, Currency), i128>, op| {
                    if let (Some(acc), Some(amount), Some(OperationStatus::Success)) =
                        (&op.account, &op.amount, &op.status)
                    {
...
fn is_unstake_event(tag: &StructTag) -> bool {
    tag.address == AIY_SYSTEM_ADDRESS
...
impl TryFrom<TransactionData> for Operations {
    ...
    fn try_from(data: TransactionData) -> Result<Self, Self::Error> {
        ...
        Ok(Operations::try_from_data(
            AiyTransactionBlockData::try_from_with_module_cache(data, &&mut NoOpsModuleResolver)?,
            None,
        )?)
    }
}
...
pub enum OperationMetadata {
    GenericTransaction(AiyTransactionBlockKind),
    Stake { validator: AiyAddress },
...
impl Operation {
    ...
    pub fn genesis(index: u64, sender: AiyAddress, coin: GasCoin) -> Self {
        Operation {
    ...
    fn pay_aiy(status: Option<OperationStatus>, address: AiyAddress, amount: i128) -> Self {
        Operation {
            operation_identifier: Default::default(),
            type_: OperationType::PayAiy,
            status,
    ...
    fn balance_change(
        ...
        Self {
            operation_identifier: Default::default(),
            type_: OperationType::AiyBalanceChange,
            status,
    ...
    fn gas(addr: AiyAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_reward(status: Option<OperationStatus>, addr: AiyAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_principle(status: Option<OperationStatus>, addr: AiyAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
