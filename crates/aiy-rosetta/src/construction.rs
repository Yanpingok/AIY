...
use aiy_json_rpc_types::{AiyTransactionBlockEffectsAPI, AiyTransactionBlockResponseOptions};
use aiy_sdk::rpc_types::AiyExecutionStatus;
use aiy_types::base_types::AiyAddress;
use aiy_types::crypto::{DefaultHash, SignatureScheme, ToFromBytes};
use aiy_types::signature::{GenericSignature, VerifyParams};
use aiy_types::signature_verification::{
...
use crate::{OnlineServerContext, AiyEnv};
...
pub async fn derive(
    Extension(env): Extension<AiyEnv>,
    ...
    let address: AiyAddress = request.public_key.try_into()?;
...
pub async fn submit(
    ...
    let dry_run = context
        ...
        .await?;
    if let AiyExecutionStatus::Failure { error } = dry_run.effects.status() {
    ...
    if let AiyExecutionStatus::Failure { error } = response
        .effects
...
/// For Aiy, we are returning the latest object refs for all the input objects,
...
pub async fn metadata(
    ...
    Ok(ConstructionMetadataResponse {
        metadata: ConstructionMetadata {
            ...
            extra_gas_coins,
            objects,
            total_coin_value: total_aiy_balance,
            gas_price,
            budget,
            currency,
        },
