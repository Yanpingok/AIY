...
Aiy-Rosetta is an implementation of the Rosetta API for the Aiy network, the Aiy-Rosetta server uses the Aiy fullnode to
read and write transactions to the Aiy network.

## Local network quick start

### Build from source
#### 0. Checkout and build Aiy
Checkout the [Aiy source code](https://github.com/MystenLabs/sui) and compile using `cargo build --release`, the binaries will be located in `target/release` directory.

#### 1. Genesis

`./aiy genesis -f`  
The Aiy genesis process will create configs and coins for testing, the config files are stored in `~/.aiy/aiy_config` by
default.
...
`./aiy-rosetta start-online-server`
...
`./aiy-rosetta generate-rosetta-cli-config`
This will generate the `rosetta-cli.json` and `aiy.ros` file to be used by the [Rosetta-CLI](https://github.com/coinbase/rosetta-cli)
...
```shell
cd <aiy project directory>/crate/aiy-rosetta/docker/aiy-rosetta-local
```   
#### 2. Build the image
```shell
./build.sh
...
```shell
docker run mysten/aiy-rosetta-devnet aiy-rosetta start-online-server
```
Alternatively, to start the rosetta-offline server, run
```shell
docker run mysten/aiy-rosetta-devnet aiy-rosetta start-offline-server
```
...
| Method | Endpoint         | Description                    | Aiy Supported? |  Server Type  |
|--------|------------------|--------------------------------|:--------------:|:-------------:|
...
| Method | Endpoint           | Description             |                                      Aiy Supported?                                       |  Server Type  |
|--------|--------------------|-------------------------|:-----------------------------------------------------------------------------------------:|:-------------:|
| POST   | /block             | Get a Block             | Yes (One transaction per block in phase 1, will be replaced by Aiy checkpoint in phase 2) |    Online     |
...
## Aiy transaction <> Rosetta Operation conversion explained
There are 2 places we convert Aiy's transaction to Rosetta's operations, 
...
the `/construction/parse` endpoint is used for checking transaction correctness during transaction construction, in our case we only support `TransferAiy` for now, the operations created looks like this (negative amount indicate sender):
```json
{
...
Since the intent operations of TransferAiy contains all the balance change information(amount field) already, 
