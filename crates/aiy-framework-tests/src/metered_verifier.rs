...
use aiy_adapter::adapter::run_metered_move_bytecode_verifier;
use aiy_config::verifier_signing_config::VerifierSigningConfig;
use aiy_framework::BuiltInFramework;
use aiy_move_build::{CompiledPackage, AiyPackageHooks};
use aiy_protocol_config::ProtocolConfig;
use aiy_types::{
    error::{AiyError, AiyResult},
    metrics::BytecodeVerifierMetrics,
};
use aiy_verifier::meter::AiyVerifierMeter;

fn build(path: &Path) -> AiyResult<CompiledPackage> {
    let mut config = aiy_move_build::BuildConfig::new_for_testing();
...
fn test_metered_move_bytecode_verifier() {
    ...
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../aiy-framework/packages/aiy-framework");
    ...
    let mut meter = AiyVerifierMeter::new(meter_config.clone());
    ...
    let mut meter = AiyVerifierMeter::new(meter_config);
    let timer_start = Instant::now();
    let r = run_metered_move_bytecode_verifier(
        &compiled_modules,
        &verifier_config,
