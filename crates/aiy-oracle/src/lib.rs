...
use aiy_json_rpc_types::{
    AiyObjectDataOptions, AiyTransactionBlockEffects, AiyTransactionBlockEffectsAPI,
    AiyTransactionBlockResponseOptions,
};
use aiy_sdk::apis::ReadApi;
use aiy_sdk::rpc_types::AiyObjectResponse;
...
use aiy_types::quorum_driver_types::NON_RECOVERABLE_ERROR_MSG;
use aiy_types::transaction::{Argument, Transaction};
use aiy_types::transaction::{Command, ObjectArg};
...
use aiy_types::base_types::{random_object_ref, ObjectID, ObjectRef};
...
impl DataProviderRunner {
    pub async fn new(
        ...
        client: Arc<AiyClient>,
        signer_address: AiyAddress,
        metrics: Arc<OracleMetrics>,
    ) -> Self {
...
async fn get_gas_obj_ref(
    read_api: &ReadApi,
    gas_obj_id: ObjectID,
    owner_address: AiyAddress,
) -> ObjectRef {
    loop {
        match read_api
            .get_object_with_options(gas_obj_id, AiyObjectDataOptions::default().with_owner())
            .await
            .map(|resp| resp.data)
        {
            Ok(Some(gas_obj)) => {
...
impl OnChainDataUploader {
    ...
    async fn upload(
    ...
    ) -> anyhow::Result<AiyTransactionBlockEffects> {
        ...
        for data_point in &data_points {
            ...
            builder.command(Command::move_call(
                ...
                vec![
                    parse_aiy_type_tag(&format!("{package_id}::decimal_value::DecimalValue"))
                        .unwrap(),
                ],
                arguments,
            ));
    ...
    async fn execute(&mut self, tx: Transaction) -> anyhow::Result<AiyTransactionBlockResponse> {
        ...
        loop {
            match self
                ...
                Err(aiy_sdk::error::Error::RpcError(err)) => {
                    // jsonrpsee translate every AiyError into jsonrpsee::core::Error, so we need to further distinguish 
        ...
        loop {
            ...
            for (feed_name, object_id) in &self.read_configs {
                match self
                    .client
                    .read_api()
                    .get_object_with_options(*object_id, AiyObjectDataOptions::default())
                    .await
                {
                    Ok(AiyObjectResponse {
                        data: Some(_data), ..
                    }) => {
