...
use aiy_json_rpc_types::{ObjectChange, AiyExecutionStatus};
use aiy_keys::keystore::{AccountKeystore, FileBasedKeystore, Keystore};
use aiy_move_build::BuildConfig;
use aiy_sdk::rpc_types::AiyTransactionBlockResponseOptions;
use aiy_sdk::types::base_types::{ObjectID, AiyAddress};
...
use aiy_sdk::types::transaction::{CallArg, ObjectArg, Transaction, TransactionData};
use aiy_sdk::types::Identifier;
use aiy_sdk::{AiyClient, AiyClientBuilder};
use aiy_types::base_types::{ObjectRef, SequenceNumber};
use aiy_types::{parse_aiy_type_tag, TypeTag};

// Integration tests for AIY Oracle, these test can be run manually on local or remote testnet.
#[ignore]
#[tokio::test]
...
async fn test_publish_complex_value() {
    ...
    for i in 1..200 {
        ...
        builder.programmable_move_call(
            ...
            module.clone(),
            submit_data.clone(),
            vec![parse_aiy_type_tag(&format!("{package}::decimal_value::DecimalValue")).unwrap()],
...
async fn test_consume_oracle_data() {
    ...
    for _ in 0..3 {
        ...
        let ticker = builder
            .input(CallArg::Pure(bcs::to_bytes("AIYUSD".as_bytes()).unwrap()))
            .unwrap();
        let identifier = builder
            .input(CallArg::Pure(
                bcs::to_bytes("identifier".as_bytes()).unwrap(),
...
async fn get_gas(client: &AiyClient, sender: AiyAddress) -> (ObjectRef, u64) {
    let gas = client
        .coin_read_api()
...
async fn init_test_client() -> (AiyClient, Keystore, AiyAddress) {
    let client = AiyClientBuilder::default()
    ...
    let keystore = Keystore::File(
        FileBasedKeystore::load_or_create(
            &dirs::home_dir()
                .unwrap()
                .join(".aiy/aiy_config/aiy.keystore"),
        )
        .unwrap(),
    );
    let sender: AiyAddress = keystore.addresses()[0];
    let gas = client
        .coin_read_api()
        .get_coins(sender, None, None, Some(1))
        .await
