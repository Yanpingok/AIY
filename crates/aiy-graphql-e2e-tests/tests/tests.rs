...
use aiy_graphql_rpc::test_infra::cluster::{serve_executor, ExecutorCluster};
use aiy_transactional_test_runner::{
    args::AiyInitArgs,
    create_adapter,
    offchain_state::{OffchainStateReader, TestResponse},
    run_tasks_with_adapter,
    test_adapter::{AiyTestAdapter, PRE_COMPILED},
};
...
async fn run_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !cfg!(msim) {
        ...
        // In another crate like `aiy-mvr-graphql-e2e-tests`, this would be the place to translate
