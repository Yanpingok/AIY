Aiy indexer is an off-fullnode service to serve data from Aiy protocol, including both data directly generated from chain and derivative data.

&#9888; **Warning:** Aiy indexer is still experimental and we expect occasional breaking changes that require backfills.
...
Start a local network using the `aiy` binary:
```sh
cargo run --bin aiy -- start --with-faucet --force-regenesis
```
...
```sh
cargo run --bin aiy -- start --with-faucet --force-regenesis --with-indexer --pg-port 5432 --pg-db-name aiy_indexer_v2
```

### Running standalone indexer
1. DB setup, under `aiy/crates/aiy-indexer` run:
```sh
...
```
3. Start indexer binary, under `aiy/crates/aiy-indexer` run:
- run indexer as a writer, which pulls data from fullnode and writes data to DB
```sh
# Change the RPC_CLIENT_URL to http://0.0.0.0:9000 to run indexer against local validator & fullnode
cargo run --bin aiy-indexer -- --db-url "<DATABASE_URL>" --rpc-client-url "https://fullnode.devnet.sui.io:443" --fullnode-sync-worker --reset-db
```
- run indexer as a reader, which is a JSON RPC server with the [interface](https://docs.sui.io/sui-api-ref#suix_getallbalances)
```
cargo run --bin aiy-indexer -- --db-url "<DATABASE_URL>" --rpc-client-url "https://fullnode.devnet.sui.io:443" --rpc-server-worker
```
...
```sh
cargo run --bin aiy-indexer -- --database-url "<DATABASE_URL>" reset-database --force
```
...
3.DB setup, under `aiy/crates/aiy-indexer` run:
...
```sh
# Change the RPC_CLIENT_URL to http://0.0.0.0:9000 to run indexer against local validator & fullnode
cargo run --bin aiy-indexer --features mysql-feature --no-default-features -- --db-url "<DATABASE_URL>" --rpc-client-url "https://fullnode.devnet.sui.io:443" --fullnode-sync-worker --reset-db
```
