...
use std::sync::Arc;
use aiy_indexer::apis::read_api::ReadApi;
use aiy_indexer::indexer_reader::IndexerReader;
use aiy_indexer::test_utils::{set_up, wait_for_checkpoint};
use aiy_json_rpc_api::ReadApiServer;
...
async fn test_checkpoint_apis() -> RpcResult<()> {
    ...
    // Test get_checkpoint
    let checkpoint_id = aiy_json_rpc_types::CheckpointId::SequenceNumber(1);
