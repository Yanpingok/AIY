...
use aiy_indexer::handlers::TransactionObjectChangesToCommit;
use aiy_indexer::models::{
    checkpoints::StoredCheckpoint, objects::StoredObject, objects::StoredObjectSnapshot,
    transactions::StoredTransaction,
};
use aiy_indexer::schema::{checkpoints, objects, objects_snapshot, transactions};
use aiy_indexer::store::indexer_store::IndexerStore;
use aiy_indexer::test_utils::{
    set_up, set_up_with_start_and_end_checkpoints, wait_for_checkpoint, wait_for_objects_snapshot,
};
use aiy_indexer::types::EventIndex;
use aiy_indexer::types::IndexedDeletedObject;
use aiy_indexer::types::IndexedObject;
use aiy_indexer::types::TxIndex;
...
pub async fn test_transaction_table() -> Result<(), IndexerError> {
    ...
    let transfer_recipient = AiyAddress::random_for_testing_only();
...
pub async fn test_object_type() -> Result<(), IndexerError> {
    ...
    assert_eq!(
        ...
        Some(AIY_FRAMEWORK_PACKAGE_ID.to_vec())
    );
