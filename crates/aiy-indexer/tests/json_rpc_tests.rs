...
use aiy_json_rpc_api::{CoinReadApiClient, IndexerApiClient, ReadApiClient};
use aiy_json_rpc_types::{
    CoinPage, EventFilter, AiyObjectDataOptions, AiyObjectResponse, AiyObjectResponseQuery,
};
use aiy_swarm_config::genesis_config::DEFAULT_GAS_AMOUNT;
use aiy_test_transaction_builder::publish_package;
use aiy_types::{event::EventID, transaction::CallArg};
...
async fn test_get_owned_objects() -> Result<(), anyhow::Error> {
    ...
    let data_option = AiyObjectDataOptions::new().with_owner();
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    // We should get all the 5 coins for AIY with the right balance.
    let result: CoinPage = http_client
