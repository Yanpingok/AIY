...
use aiy_data_ingestion_core::Worker;
use aiy_types::dynamic_field::DynamicFieldType;
use aiy_types::effects::{ObjectChange, TransactionEffectsAPI};
use aiy_types::event::SystemEpochInfoEvent;
...
use aiy_types::aiy_system_state::{get_aiy_system_state, AiySystemStateTrait};
...
impl CheckpointHandler {
    ...
    async fn index_epoch(
        ...
        if *checkpoint_summary.sequence_number() == 0 {
            ...
            let system_state_summary =
                get_aiy_system_state(&checkpoint_object_store)?.into_aiy_system_state_summary();
        ...
        if epoch_event_opt.is_none() {
            ...
            assert!(
                ...
                "Aiy is not in safe mode but no SystemEpochInfoEvent found at end of epoch {}",
                checkpoint_summary.epoch
            );
        }
    ...
    async fn index_checkpoint(
        ...
        let (checkpoint, db_transactions, db_events, db_tx_indices, db_event_indices, db_displays) = {
            ...
            (
                IndexedCheckpoint::from_aiy_checkpoint(
                    checkpoint_summary,
                    checkpoint_contents,
        ...
        checkpoint_data
            ...
            .flat_map(|data| {
                ...
                data.transactions
                    ...
                    .flat_map(|tx| &tx.output_objects)
                    .filter_map(|o| {
                        if let aiy_types::object::Data::Package(p) = &o.data {
                            Some(IndexedPackage {
                                package_id: o.id(),
