...
use aiy_json_rpc::coin_api::{parse_to_struct_tag, parse_to_type_tag};
use aiy_json_rpc::error::AiyRpcInputError;
use aiy_json_rpc::AiyRpcModule;
use aiy_json_rpc_api::{cap_page_limit, CoinReadApiServer};
use aiy_json_rpc_types::{Balance, CoinPage, Page, AiyCoinMetadata};
...
impl CoinReadApiServer for CoinReadApi {
    async fn get_coins(
        ...
        let cursor = match cursor {
            Some(c) => c
                .parse()
                .map_err(|e| AiyRpcInputError::GenericInvalid(format!("invalid cursor: {e}")))?,
...
impl AiyRpcModule for CoinReadApi {
    ...
    fn rpc_doc_module() -> Module {
        aiy_json_rpc_api::CoinReadApiOpenRpc::module_doc()
    }
}
