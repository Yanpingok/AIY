...
use aiy_json_rpc::{governance_api::ValidatorExchangeRates, AiyRpcModule};
use aiy_json_rpc_api::GovernanceReadApiServer;
use aiy_json_rpc_types::{
    DelegatedStake, EpochInfo, StakeStatus, AiyCommittee, AiyObjectDataFilter, ValidatorApys,
};
use aiy_open_rpc::Module;
use aiy_types::{
    base_types::{MoveObjectType, ObjectID, AiyAddress},
    committee::EpochId,
    governance::StakedAiy,
    aiy_serde::BigInt,
    aiy_system_state::{aiy_system_state_summary::AiySystemStateSummary, PoolTokenExchangeRate},
};
...
impl GovernanceReadApi {
    ...
    async fn get_latest_aiy_system_state(&self) -> Result<AiySystemStateSummary, IndexerError> {
        self.inner.get_latest_aiy_system_state().await
    }
    ...
    async fn get_stakes_by_ids(
        ...
        for stored_object in self.inner.multi_get_objects(ids).await? {
            let object = aiy_types::object::Object::try_from(stored_object)?;
            let stake_object = StakedAiy::try_from(&object)?;
    ...
    async fn get_staked_by_owner(
        ...
        for stored_object in self
            ...
            .get_owned_objects(
                owner,
                Some(AiyObjectDataFilter::StructType(
                    MoveObjectType::staked_aiy().into(),
                )),
                None,
    ...
    pub async fn get_delegated_stakes(
        ...
        let system_state_summary = self.get_latest_aiy_system_state().await?;
        ...
        for (pool_id, stakes) in pools {
            ...
            for stake in stakes {
                ...
                delegations.push(aiy_json_rpc_types::Stake {
...
pub async fn exchange_rates(
    ...
    for df in state
        ...
        .await?
    {
        let pool_id: aiy_types::id::ID = bcs::from_bytes(&df.bcs_name).map_err(|e| {
            aiy_types::error::AiyError::ObjectDeserializationError {
                error: e.to_string(),
            }
        })?;
    ...
    for (address, pool_id, exchange_rates_id, exchange_rates_size, active) in tables {
            ...
            .await?
        {
            let dynamic_field = df
                ...
                .ok_or_else(|| aiy_types::error::AiyError::ObjectDeserializationError {
...
impl GovernanceReadApiServer for GovernanceReadApi {
    async fn get_stakes_by_ids(
        ...
        self.get_stakes_by_ids(staked_aiy_ids)
            .await
            .map_err(Into::into)
    }
    ...
    async fn get_latest_aiy_system_state(&self) -> RpcResult<AiySystemStateSummary> {
        self.get_latest_aiy_system_state().await.map_err(Into::into)
    }
...
impl AiyRpcModule for GovernanceReadApi {
    ...
    fn rpc_doc_module() -> Module {
        aiy_json_rpc_api::GovernanceReadApiOpenRpc::module_doc()
    }
}
