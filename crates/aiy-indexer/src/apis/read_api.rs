...
use aiy_json_rpc_api::{ReadApiServer, QUERY_MAX_RESULT_LIMIT};
use aiy_json_rpc_types::ZkLoginIntentScope;
use aiy_json_rpc_types::ZkLoginVerifyResult;
use aiy_json_rpc_types::{
    Checkpoint, CheckpointId, CheckpointPage, ProtocolConfigResponse, AiyEvent,
    AiyGetPastObjectRequest, AiyObjectDataOptions, AiyObjectResponse, AiyPastObjectResponse,
...
use aiy_types::base_types::{ObjectID, SequenceNumber};
...
impl ReadApiServer for ReadApi {
    ...
    async fn multi_get_objects(
        ...
        if object_ids.len() > *QUERY_MAX_RESULT_LIMIT {
            return Err(
                AiyRpcInputError::SizeLimitExceeded(QUERY_MAX_RESULT_LIMIT.to_string()).into(),
            );
        }
    ...
    async fn try_get_past_object(
        ...
        _options: Option<AiyObjectDataOptions>,
    ...
    async fn try_multi_get_past_objects(
        &self,
        _past_objects: Vec<AiyGetPastObjectRequest>,
        _options: Option<AiyObjectDataOptions>,
    ...
    async fn get_checkpoints(
        ...
        let limit = aiy_json_rpc_api::validate_limit(
            limit,
            aiy_json_rpc_api::QUERY_MAX_RESULT_LIMIT_CHECKPOINTS,
        )
...
impl AiyRpcModule for ReadApi {
    ...
    fn rpc_doc_module() -> Module {
        aiy_json_rpc_api::ReadApiOpenRpc::module_doc()
    }
}
...
async fn object_read_to_object_response(
    ...
    match object_read {
        ...
        ObjectRead::Deleted((object_id, version, digest)) => Ok(AiyObjectResponse::new_with_error(
