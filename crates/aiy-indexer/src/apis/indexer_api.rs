...
use aiy_json_rpc_api::{cap_page_limit, IndexerApiServer};
use aiy_json_rpc_types::{
    DynamicFieldPage, EventFilter, EventPage, ObjectsPage, Page, AiyObjectResponse,
    AiyObjectResponseQuery, AiyTransactionBlockResponseQuery, TransactionBlocksPage,
    TransactionFilter,
};
use aiy_name_service::{Domain, NameRecord, NameServiceConfig, NameServiceError};
use aiy_open_rpc::Module;
use aiy_types::base_types::{ObjectID, AiyAddress};
use aiy_types::digests::TransactionDigest;
use aiy_types::dynamic_field::{DynamicFieldName, Field};
...
impl IndexerApi {
    ...
    async fn get_owned_objects_internal(
        ...
        let AiyObjectResponseQuery { filter, options } = query.unwrap_or_default();
...
impl IndexerApiServer for IndexerApi {
    ...
    async fn get_dynamic_field_object(
        ...
        let id = aiy_types::dynamic_field::derive_dynamic_field_id(
            parent_object_id,
            &name.type_,
            &name_bcs_value,
        )
        ...
        let options = aiy_json_rpc_types::AiyObjectDataOptions::full_content();
        match self.inner.get_object_read(id).await? {
            aiy_types::object::ObjectRead::NotExists(_)
            | aiy_types::object::ObjectRead::Deleted(_) => {}
            aiy_types::object::ObjectRead::Exists(object_ref, o, layout) => {
                return Ok(AiyObjectResponse::new_with_data(
        ...
        let dynamic_object_field_struct =
            aiy_types::dynamic_field::DynamicFieldInfo::dynamic_object_field_wrapper(name.type_);
        let dynamic_object_field_type = TypeTag::Struct(Box::new(dynamic_object_field_struct));
        let dynamic_object_field_id = aiy_types::dynamic_field::derive_dynamic_field_id(
            parent_object_id,
        ...
        Ok(AiyObjectResponse::new_with_error(
            aiy_types::error::AiyObjectResponseError::DynamicFieldNotFound { parent_object_id },
        ))
    }
    ...
    async fn resolve_name_service_address(&self, name: String) -> RpcResult<Option<AiyAddress>> {
        ...
        let domains: Vec<_> = self
            ...
            .await?
            .into_iter()
            .map(|o| aiy_types::object::Object::try_from(o).ok())
            .collect();
    ...
    async fn resolve_name_service_names(
        ...
        let domain = field_reverse_record_object
            .to_rust::<Field<AiyAddress, Domain>>()
            .ok_or_else(|| {
...
impl AiyRpcModule for IndexerApi {
    fn rpc(self) -> RpcModule<Self> {
        self.into_rpc()
    }

    fn rpc_doc_module() -> Module {
        aiy_json_rpc_api::IndexerApiOpenRpc::module_doc()
    }
}
