...
use aiy_json_rpc_types::{
    ObjectChange, AiyTransactionBlockResponse, AiyTransactionBlockResponseOptions,
};
...
pub fn owner_to_owner_info(owner: &Owner) -> (OwnerType, Option<AiyAddress>) {
    match owner {
...
impl IndexedObject {
    pub fn random() -> Self {
        let mut rng = rand::thread_rng();
        let random_address = AiyAddress::random_for_testing_only();
        IndexedObject {
...
pub struct IndexedTransaction {
    ...
    pub balance_change: Vec<aiy_json_rpc_types::BalanceChange>,
    pub events: Vec<aiy_types::event::Event>,
...
pub struct TxIndex {
    ...
    pub payers: Vec<AiyAddress>,
    pub sender: AiyAddress,
    pub recipients: Vec<AiyAddress>,
    ...
    pub fn random() -> Self {
        ...
        TxIndex {
            ...
            payers: (0..rng.gen_range(0..100))
                .map(|_| AiyAddress::random_for_testing_only())
                .collect(),
            sender: AiyAddress::random_for_testing_only(),
...
// AiyTransactionBlockResponseWithOptions is only used on the reading path
pub struct AiyTransactionBlockResponseWithOptions {
    pub response: AiyTransactionBlockResponse,
    pub options: AiyTransactionBlockResponseOptions,
}

impl From<AiyTransactionBlockResponseWithOptions> for AiyTransactionBlockResponse {
    fn from(value: AiyTransactionBlockResponseWithOptions) -> Self {
        let AiyTransactionBlockResponseWithOptions { response, options } = value;
