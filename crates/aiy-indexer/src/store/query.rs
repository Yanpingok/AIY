...
use aiy_json_rpc_types::AiyObjectDataFilter;
...
impl DBFilter<ObjectID> for AiyObjectDataFilter {
...
fn to_latest_objects_clauses(filter: &AiyObjectDataFilter) -> Option<String> {
    match filter {
        AiyObjectDataFilter::AddressOwner(a) => Some(format!(
...
fn to_clauses(filter: &AiyObjectDataFilter) -> Option<String> {
    match filter {
        AiyObjectDataFilter::MatchAll(sub_filters) => {
        ...
        AiyObjectDataFilter::MatchAny(sub_filters) => {
            let sub_filters = sub_filters.iter().flat_map(to_clauses).collect::<Vec<_>>();
            if sub_filters.is_empty() {
        ...
        AiyObjectDataFilter::MatchNone(sub_filters) => {
            let sub_filters = sub_filters.iter().flat_map(to_clauses).collect::<Vec<_>>();
            if sub_filters.is_empty() {
                None
            } else {
        ...
        AiyObjectDataFilter::Package(p) => Some(format!("o.object_type LIKE '{}::%'", p.to_hex_literal())),
        AiyObjectDataFilter::MoveModule { package, module } => Some(format!(
            "o.object_type LIKE '{}::{}::%'",
        ...
        AiyObjectDataFilter::ObjectId(id) => {
            Some(format!("o.object_id = '{id}'"))
        }
        AiyObjectDataFilter::ObjectIds(ids) => {
            if ids.is_empty() {
                None
            } else {
                let ids = ids
        ...
        AiyObjectDataFilter::Version(v) => Some(format!("o.version = {v}")),
    }
}

fn to_outer_clauses(filter: &AiyObjectDataFilter) -> Option<String> {
    match filter {
        AiyObjectDataFilter::MatchNone(sub_filters) => {
        ...
        AiyObjectDataFilter::AddressOwner(a) => Some(format!("t1.owner_address = '{a}'")),
        _ => None,
    }
}
...
mod test {
    ...
    fn test_address_filter() {
        let address = AiyAddress::from_str(
            "0x92dd4d9b0150c251661d821583ef078024ae9e9ee11063e216500861eec7f381",
        )
        .unwrap();
        let filter = AiyObjectDataFilter::AddressOwner(address);
...
LIMIT 100;";
    ...
    fn test_move_module_filter() {
        let filter = AiyObjectDataFilter::MoveModule {
...
LIMIT 100;";
    ...
    fn test_empty_all_filter() {
        let filter = AiyObjectDataFilter::MatchAll(vec![]);
        let expected_sql = "SELECT t1.*
FROM (SELECT DISTINCT ON (o.object_id) *
      FROM objects_history o
      WHERE o.checkpoint <= $1
...
LIMIT 100;";
    ...
    fn test_empty_any_filter() {
        let filter = AiyObjectDataFilter::MatchAny(vec![]);
        let expected_sql = "SELECT t1.*
FROM (SELECT DISTINCT ON (o.object_id) *
      FROM objects_history o
      WHERE o.checkpoint <= $1
...
LIMIT 100;";
    ...
    fn test_all_filter() {
        let filter = AiyObjectDataFilter::MatchAll(vec![
            AiyObjectDataFilter::ObjectId(
                ObjectID::from_str(
                    "0xef9fb75a7b3d4cb5551ef0b08c83528b94d5f5cd8be28b1d08a87dbbf3731738",
                )
                .unwrap(),
            ),
            AiyObjectDataFilter::StructType(parse_aiy_struct_tag("0x2::test::Test").unwrap()),
        ]);
