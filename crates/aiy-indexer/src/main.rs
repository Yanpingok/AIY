...
use clap::Parser;
use aiy_indexer::backfill::backfill_runner::BackfillRunner;
use aiy_indexer::config::Command;
use aiy_indexer::database::ConnectionPool;
use aiy_indexer::db::setup_postgres::clear_database;
use aiy_indexer::db::{
    check_db_migration_consistency, check_prunable_tables_valid, reset_database, run_migrations,
};
use aiy_indexer::indexer::Indexer;
use aiy_indexer::metrics::{
...
async fn main() -> anyhow::Result<()> {
    let opts = aiy_indexer::config::IndexerConfig::parse();

    // NOTE: this is to print out tracing like info, warn & error.
    let _guard = telemetry_subscribers::TelemetryConfig::new()
        .with_env()
        .init();
    warn!("WARNING: Aiy indexer is still experimental and we expect occasional breaking changes that require backfills.");
