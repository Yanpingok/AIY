...
use aiy_json_rpc_types::{
    BalanceChange, ObjectChange, AiyEvent, AiyTransactionBlock, AiyTransactionBlockEffects,
...
impl StoredTransaction {
    ...
    pub async fn try_into_aiy_transaction_block_response(
        self,
        options: AiyTransactionBlockResponseOptions,
        package_resolver: Arc<Resolver<impl PackageStore>>,
    ) -> IndexerResult<AiyTransactionBlockResponse> {
        ...
        let transaction = if options.show_input {
            ...
            let tx_block = AiyTransactionBlock::try_from_with_package_resolver(
                sender_signed_data,
                &package_resolver,
            )
            .await?;
        ...
        let effects = if options.show_effects {
            let effects = self.try_into_aiy_transaction_effects()?;
            Some(effects)
        } else {
            None
        };
        ...
        let events = if options.show_events {
            ...
            tx_events_to_aiy_tx_events(tx_events, package_resolver, tx_digest, timestamp).await?
        } else {
            None
        };
        ...
        Ok(AiyTransactionBlockResponse {
            digest: tx_digest,
            transaction,
            raw_transaction,
            effects,
    ...
    pub fn try_into_aiy_transaction_effects(&self) -> IndexerResult<AiyTransactionBlockEffects> {
        ...
        let effects = AiyTransactionBlockEffects::try_from(effects)?;
        Ok(effects)
    }
}
...
pub async fn tx_events_to_aiy_tx_events(
    tx_events: TransactionEvents,
    package_resolver: Arc<Resolver<impl PackageStore>>,
    tx_digest: TransactionDigest,
    timestamp: u64,
) -> Result<Option<AiyTransactionBlockEvents>, IndexerError> {
    ...
    for tx_event in tx_events.data.clone() {
        ...
        aiy_event_futures.push(tokio::task::spawn(async move {
    ...
    let event_move_type_layouts = futures::future::join_all(aiy_event_futures)
        .await
        .into_iter()
        ...
        .map_err(|e| {
            IndexerError::ResolveMoveStructError(format!(
                "Failed to convert to aiy event with Error: {e}",
            ))
        })?;
    ...
    let aiy_events = tx_events
        ...
        .collect::<Result<Vec<_>, _>>()?;
    let aiy_tx_events = AiyTransactionBlockEvents { data: aiy_events };
    Ok(Some(aiy_tx_events))
}
