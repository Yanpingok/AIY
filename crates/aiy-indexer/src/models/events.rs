...
use aiy_json_rpc_types::{type_and_fields_from_move_event_data, BcsEvent, AiyEvent};
...
impl StoredEvent {
    pub async fn try_into_aiy_event(
        self,
        package_resolver: Arc<Resolver<impl PackageStore>>,
    ) -> Result<AiyEvent, IndexerError> {
        ...
        // Note: AiyEvent only has one sender today, so we always use the first one.
        let sender = {
        ...
        let sender = match sender {
            Some(ref s) => AiyAddress::from_bytes(s).map_err(|_e| {
                IndexerError::PersistentStorageDataCorruptionError(format!(
        ...
        let type_ = parse_aiy_struct_tag(&self.event_type)?;
