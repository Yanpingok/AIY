...
use aiy_json_rpc_types::{Balance, Coin as AiyCoin, AiyCoinMetadata, AiyMoveValue};
use aiy_json_rpc_types::{
    CheckpointId, EpochInfo, EventFilter, AiyEvent, AiyObjectDataFilter,
    AiyTransactionBlockResponse, TransactionFilter,
};
use aiy_package_resolver::Package;
use aiy_package_resolver::PackageStore;
use aiy_package_resolver::{PackageStoreWithLruCache, Resolver};
use aiy_types::effects::TransactionEvents;
use aiy_types::{balance::Supply, coin::TreasuryCap, dynamic_field::DynamicFieldName};
use aiy_types::{
    base_types::{ObjectID, AiyAddress, VersionNumber},
    committee::EpochId,
    digests::TransactionDigest,
    dynamic_field::DynamicFieldInfo,
    object::{Object, ObjectRead},
    aiy_system_state::{aiy_system_state_summary::AiySystemStateSummary, AiySystemStateTrait},
};
use aiy_types::{coin::CoinMetadata, event::EventID};
...
use crate::{
    ...
    models::{
        ...
        transactions::{tx_events_to_aiy_tx_events, StoredTransaction},
...
impl IndexerReader {
    ...
    pub async fn get_latest_aiy_system_state(&self) -> Result<AiySystemStateSummary, IndexerError> {
        ...
        let system_state = tokio::task::spawn_blocking(move || {
            aiy_types::aiy_system_state::get_aiy_system_state(&object_store)
        })
        .await
        .unwrap()?
    ...
    pub async fn get_validator_from_table(
        &self,
        table_id: ObjectID,
        pool_id: aiy_types::id::ID,
    ) -> Result<
        aiy_types::aiy_system_state::aiy_system_state_summary::AiyValidatorSummary,
        IndexerError,
    > {
        let object_store = ConnectionAsObjectStore::from_pool(&self.pool)
            .await
        ...
        let validator = tokio::task::spawn_blocking(move || {
            aiy_types::aiy_system_state::get_validator_from_table(&object_store, table_id, &pool_id)
        })
        .await
        .unwrap()?;
        Ok(validator)
    ...
    /// `get_latest_aiy_system_state` for current epoch instead.
    pub async fn get_epoch_aiy_system_state(
        &self,
        epoch: Option<EpochId>,
    ) -> Result<AiySystemStateSummary, IndexerError> {
        let stored_epoch = self.get_epoch_info_from_db(epoch).await?;
        let stored_epoch = match stored_epoch {
            Some(stored_epoch) => stored_epoch,
    ...
    pub async fn get_checkpoint(
        &self,
        checkpoint_id: CheckpointId,
    ) -> Result<Option<aiy_json_rpc_types::Checkpoint>, IndexerError> {
        let stored_checkpoint = match self.get_checkpoint_from_db(checkpoint_id).await? {
            Some(stored_checkpoint) => stored_checkpoint,
            None => return Ok(None),
        };

        let checkpoint = aiy_json_rpc_types::Checkpoint::try_from(stored_checkpoint)?;
        Ok(Some(checkpoint))
    }

    pub async fn get_latest_checkpoint(
        &self,
    ) -> Result<aiy_json_rpc_types::Checkpoint, IndexerError> {
        let stored_checkpoint = self.get_latest_checkpoint_from_db().await?;

        aiy_json_rpc_types::Checkpoint::try_from(stored_checkpoint)
    }
    ...
    pub async fn get_checkpoints(
        ...
        limit: usize,
        descending_order: bool,
    ) -> Result<Vec<aiy_json_rpc_types::Checkpoint>, IndexerError> {
        self.get_checkpoints_from_db(cursor, limit, descending_order)
            .await?
            .into_iter()
            .map(aiy_json_rpc_types::Checkpoint::try_from)
            .collect()
    }
    ...
    async fn stored_transaction_to_transaction_block(
        ...
        options: aiy_json_rpc_types::AiyTransactionBlockResponseOptions,
    ) -> IndexerResult<Vec<AiyTransactionBlockResponse>> {
        ...
        for stored_tx in stored_txes {
            ...
            tx_block_responses_futures.push(tokio::task::spawn(
                stored_tx
                    .try_into_aiy_transaction_block_response(options_clone, package_resolver_clone),
            ));
        }
    ...
    pub async fn get_owned_objects(
        ...
        if let Some(filter) = filter {
            match filter {
                AiyObjectDataFilter::StructType(struct_tag) => {
                ...
                AiyObjectDataFilter::MatchAny(filters) => {
                    let mut condition = "(".to_string();
                    for (i, filter) in filters.iter().enumerate() {
                        if let AiyObjectDataFilter::StructType(struct_tag) = filter {
                            let object_type =
                ...
                AiyObjectDataFilter::MatchNone(filters) => {
    ...
    async fn multi_get_transaction_block_response_in_blocking_task_impl(
        ...
        digests: &[TransactionDigest],
        options: aiy_json_rpc_types::AiyTransactionBlockResponseOptions,
    ) -> Result<Vec<aiy_json_rpc_types::AiyTransactionBlockResponse>, IndexerError> {
    ...
    pub async fn get_transaction_events(
    ...
    ) -> Result<Vec<aiy_json_rpc_types::AiyEvent>, IndexerError> {
        use diesel_async::RunQueryDsl;
        ...
        let aiy_tx_events = tx_events_to_aiy_tx_events(
            tx_events,
        ...
        .await?;
        Ok(aiy_tx_events.map_or(vec![], |ste| ste.data))
    }
    ...
    async fn query_events_by_tx_digest(
        ...
        let mut aiy_event_futures = vec![];
        for stored_event in stored_events {
            aiy_event_futures.push(tokio::task::spawn(
                stored_event.try_into_aiy_event(self.package_resolver.clone()),
            ));
        }

        let aiy_events = futures::future::join_all(aiy_event_futures)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| error!("Failed to join aiy event futures: {}", e))?
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| error!("Failed to collect aiy event futures: {}", e))?;
        Ok(aiy_events)
    }
    ...
    pub async fn bcs_name_from_dynamic_field_name(
        ...
        let aiy_json_value = aiy_json::AiyJsonValue::new(name.value.clone())?;
        let name_bcs_value = aiy_json_value.to_bcs_bytes(&move_type_layout)?;
        Ok(name_bcs_value)
    }
    ...
    pub async fn get_owned_coins(
        ...
        cursor: ObjectID,
        limit: usize,
    ) -> Result<Vec<AiyCoin>, IndexerError> {
        use diesel_async::RunQueryDsl;
...
impl aiy_types::storage::ObjectStore for ConnectionAsObjectStore {
    fn get_object(&self, object_id: &ObjectID) -> Option<aiy_types::object::Object> {
    ...
    fn get_object_by_key(
        &self,
        object_id: &ObjectID,
        version: aiy_types::base_types::VersionNumber,
    ) -> Option<aiy_types::object::Object> {
