...
use aiy_json_rpc_types::AiyMoveStruct;
    ...
    async fn process_object(
            ...
            .and_then(|tag| object.data.try_as_move().map(|mo| (tag, mo.contents())))
        {
            ...
            .await
            {
                ...
                Err(err)
                    if err
                        .downcast_ref::<aiy_types::object::bounded_visitor::Error>()
                        .filter(|e| {
                            matches!(e, aiy_types::object::bounded_visitor::Error::OutOfBudget)
                        })
                        .is_some() =>
                {
                    self.metrics
        ...
        let (struct_tag, aiy_move_struct) = if let Some(move_struct) = move_struct {
            match move_struct.into() {
                AiyMoveStruct::WithTypes { type_, fields } => {
                    (Some(type_), Some(AiyMoveStruct::WithFields(fields)))
                }
        ...
        let entry = ObjectEntry {
            ...
            object_json: aiy_move_struct.map(|x| x.to_json_value().to_string()),
        };
        Ok(Some(entry))
    }
}
