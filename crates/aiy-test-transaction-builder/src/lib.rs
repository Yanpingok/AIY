...
use aiy_genesis_builder::validator_info::GenesisValidatorMetadata;
use aiy_move_build::{BuildConfig, CompiledPackage};
use aiy_sdk::rpc_types::{
    AiyObjectDataOptions, AiyTransactionBlockEffectsAPI, AiyTransactionBlockResponse,
};
use aiy_sdk::wallet_context::WalletContext;
use aiy_types::base_types::{FullObjectRef, ObjectID, ObjectRef, SequenceNumber, AiyAddress};
use aiy_types::crypto::{get_key_pair, AccountKeyPair, Signature, Signer};
use aiy_types::digests::TransactionDigest;
use aiy_types::multisig::{BitmapUnit, MultiSig, MultiSigPublicKey};
...
use aiy_types::{TypeTag, AIY_SYSTEM_PACKAGE_ID};
...
impl TestTransactionBuilder {
    pub fn new(sender: AiyAddress, gas_object: ObjectRef, gas_price: u64) -> Self {
        Self {
    ...
    pub fn call_staking(self, stake_coin: ObjectRef, validator: AiyAddress) -> Self {
        self.move_call(
            AIY_SYSTEM_PACKAGE_ID,
            AIY_SYSTEM_MODULE_NAME.as_str(),
            "request_add_stake",
            vec![
    ...
    pub fn transfer(mut self, object: FullObjectRef, recipient: AiyAddress) -> Self {
        self.test_data = TestTransactionData::Transfer(TransferData { object, recipient });
        self
    }

    pub fn transfer_aiy(mut self, amount: Option<u64>, recipient: AiyAddress) -> Self {
        self.test_data = TestTransactionData::TransferAiy(TransferAiyData { amount, recipient });
        self
    }
    ...
    pub fn build(self) -> TransactionData {
        match self.test_data {
            ...
            TestTransactionData::TransferAiy(data) => TransactionData::new_transfer_aiy(
                data.recipient,
                self.sender,
                data.amount,
                self.gas_object,
    ...
    pub fn build_and_sign_multisig(
        ...
        let data = self.build();
        let intent_msg = IntentMessage::new(Intent::aiy_transaction(), data.clone());
    ...
    pub fn build_and_sign_multisig_legacy(
        ...
        let data = self.build();
        let intent = Intent::aiy_transaction();
...
pub async fn make_transfer_aiy_transaction(
    context: &WalletContext,
    recipient: Option<AiyAddress>,
    amount: Option<u64>,
) -> Transaction {
    ...
    context
        .sign_transaction(
            &TestTransactionBuilder::new(sender, gas_object, gas_price)
                .transfer_aiy(amount, recipient.unwrap_or(sender))
                .build(),
        )
        .await
}
...
pub async fn create_nft(
    context: &WalletContext,
    package_id: ObjectID,
) -> (AiyAddress, ObjectID, TransactionDigest) {
    let (sender, gas_object) = context.get_one_gas_object().await.unwrap().unwrap();
    let rgp = context.get_reference_gas_price().await.unwrap();
