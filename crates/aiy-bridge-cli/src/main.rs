...
use aiy_bridge::client::bridge_authority_aggregator::BridgeAuthorityAggregator;
use aiy_bridge::crypto::{BridgeAuthorityPublicKey, BridgeAuthorityPublicKeyBytes};
use aiy_bridge::eth_transaction_builder::build_eth_transaction;
use aiy_bridge::metrics::BridgeMetrics;
use aiy_bridge::aiy_client::AiyClient;
use aiy_bridge::aiy_transaction_builder::build_aiy_transaction;
use aiy_bridge::types::BridgeActionType;
use aiy_bridge::utils::{
...
use aiy_bridge::utils::{get_eth_contracts, EthBridgeContracts};
use aiy_bridge_cli::{
...
use aiy_sdk::AiyClient as AiySdkClient;
use aiy_sdk::AiyClientBuilder;
use aiy_types::base_types::AiyAddress;
use aiy_types::bridge::BridgeChainId;
use aiy_types::bridge::{MoveTypeCommitteeMember, MoveTypeCommitteeMemberRegistration};
use aiy_types::committee::TOTAL_VOTING_POWER;
...
async fn main() -> anyhow::Result<()> {
    ...
    match args.command {
        ...
        BridgeCommand::Governance {
            ...
            let aiy_bridge_client =
                AiyClient::<AiySdkClient>::new(&config.aiy_rpc_url, metrics.clone()).await?;

            let (aiy_key, aiy_address, gas_object_ref) = config
                .get_aiy_account_info()
                .await
                .expect("Failed to get aiy account info");
            let bridge_summary = aiy_bridge_client
                .get_bridge_summary()
                .await
            ...
            // Handle Aiy Side
            if chain_id.is_aiy_chain() {
                let aiy_chain_id = BridgeChainId::try_from(bridge_summary.chain_id).unwrap();
                assert_eq!(
                    aiy_chain_id, chain_id,
                    "Chain ID mismatch, expected: {:?}, got from url: {:?}",
                    chain_id, aiy_chain_id
                );
                // Create BridgeAction
                let aiy_action = make_action(aiy_chain_id, &cmd);
                println!("Action to execute on Aiy: {:?}", aiy_action);
                ...
                let bridge_arg = aiy_bridge_client
                    ...
                    .await;
                let rgp = aiy_bridge_client
                    .get_reference_gas_price_until_success()
                    .await;
                let id_token_map = aiy_bridge_client.get_token_id_map().await.unwrap();
                ...
                .expect("Failed to build aiy transaction");
                let aiy_sig = Signature::new_secure(
                    &IntentMessage::new(Intent::aiy_transaction(), tx.clone()),
                    &aiy_key,
                );
                let tx = Transaction::from_data(tx, vec![aiy_sig]);
                ...
                if resp.status_ok().unwrap() {
                    println!("Aiy Transaction succeeded: {:?}", resp.digest);
                } else {
                    println!(
                        "Aiy Transaction failed: {:?}. Effects: {:?}",
        ...
        BridgeCommand::ViewBridgeRegistration { aiy_rpc_url } => {
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let aiy_bridge_client = AiyClient::<AiySdkClient>::new(&aiy_rpc_url, metrics).await?;
            let bridge_summary = aiy_bridge_client
                .get_bridge_summary()
                .await
                .map_err(|e| anyhow::anyhow!("Failed to get bridge summary: {:?}", e))?;
            let move_type_bridge_committee = bridge_summary.committee;
            let aiy_client = AiyClientBuilder::default().build(aiy_rpc_url).await?;
            let stakes = aiy_client
                .governance_api()
                ...
                .map(|summary| {
                    let protocol_key =
                        AuthorityPublicKeyBytes::from_bytes(&summary.protocol_pubkey_bytes)
                            .unwrap();
                    (summary.aiy_address, (protocol_key, summary.name))
                })
                .collect::<HashMap<_, _>>();
            let mut authorities = vec![];
            let mut output_wrapper = Output::<OutputAiyBridgeRegistration>::default();
            for (_, member) in move_type_bridge_committee.member_registration {
                ...
                let Ok(pubkey) = BridgeAuthorityPublicKey::from_bytes(&bridge_pubkey_bytes) else {
                    output_wrapper.add_error(format!(
                        ...
                        aiy_address, bridge_pubkey_bytes
                    ));
                    continue;
                };
                ...
                let Ok(url) = from_utf8(&http_rest_url) else {
                    output_wrapper.add_error(format!(
                        ...
                        aiy_address, http_rest_url
                    ));
                    continue;
                };
                let url = url.to_string();

                let (protocol_key, name) = names.get(&aiy_address).unwrap();
                let stake = stakes.get(protocol_key).unwrap();
                authorities.push((name, aiy_address, pubkey, eth_address, url, stake));
            }
            let total_stake = authorities
                .iter()
                .map(|(_, _, _, _, _, stake)| **stake)
                .sum::<u64>();
            let mut output = OutputAiyBridgeRegistration {
                total_registered_stake: total_stake as f32 / TOTAL_VOTING_POWER as f32 * 100.0,
                ..Default::default()
            };
            for (name, aiy_address, pubkey, eth_address, url, stake) in authorities {
        ...
        BridgeCommand::ViewAiyBridge {
            ...
            let names = aiy_client
                ...
                .active_validators
                .into_iter()
                .map(|summary| (summary.aiy_address, summary.name))
                .collect::<HashMap<_, _>>();
            let mut authorities = vec![];
            ...
            let client = reqwest::Client::builder()
                ...
                .build()
                .unwrap();
            let mut output_wrapper = Output::<OutputAiyBridge>::default();
            for (_, member) in move_type_bridge_committee.members {
                ...
                let name = names.get(&aiy_address).cloned().unwrap_or(url.clone());
            ...
            let total_stake = authorities
                ...
                .sum::<u64>();
            let mut output = OutputAiyBridge {
            ...
            for ((name, aiy_address, pubkey, eth_address, url, stake, blocklisted), ping_resp) in
        ...
        BridgeCommand::Client { config_path, cmd } => {
            ...
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let aiy_bridge_client =
                AiyClient::<AiySdkClient>::new(&config.aiy_rpc_url, metrics).await?;
            cmd.handle(&config, aiy_bridge_client).await?;
            return Ok(());
        }
    }
...
#[derive(serde::Serialize, Default)]
struct OutputAiyBridgeRegistration {
