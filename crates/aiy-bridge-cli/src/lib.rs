...
use std::str::FromStr;
use std::sync::Arc;
use aiy_bridge::abi::EthBridgeCommittee;
use aiy_bridge::abi::{eth_aiy_bridge, EthAiyBridge};
use aiy_bridge::crypto::BridgeAuthorityPublicKeyBytes;
use aiy_bridge::error::BridgeResult;
use aiy_bridge::aiy_client::AiyBridgeClient;
use aiy_bridge::types::BridgeAction;
use aiy_bridge::types::{
    AddTokensOnEvmAction, AddTokensOnAiyAction, AssetPriceUpdateAction, BlocklistCommitteeAction,
    BlocklistType, EmergencyAction, EmergencyActionType, EvmContractUpgradeAction,
    LimitUpdateAction,
};
use aiy_bridge::utils::{get_eth_signer_client, EthSigner};
...
use aiy_types::bridge::{BridgeChainId, BRIDGE_MODULE_NAME};
use aiy_types::crypto::{Signature, AiyKeyPair};
...
use aiy_types::{TypeTag, BRIDGE_PACKAGE_ID};
use tracing::info;
...
pub enum BridgeCommand {
    ...
    ViewBridgeRegistration {
        #[clap(long = "aiy-rpc-url")]
        aiy_rpc_url: String,
    },
    /// View current status of Aiy bridge
    #[clap(name = "view-aiy-bridge")]
    ViewAiyBridge {
        #[clap(long = "aiy-rpc-url")]
        aiy_rpc_url: String,
...
pub enum GovernanceClientCommands {
    ...
    AddTokensOnEvm {
        ...
        #[clap(name = "token-prices", use_value_delimiter = true, long)]
        token_prices: Vec<u64>,
        #[clap(name = "token-aiy-decimals", use_value_delimiter = true, long)]
        token_aiy_decimals: Vec<u8>,
    },
...
pub fn make_action(chain_id: BridgeChainId, cmd: &GovernanceClientCommands) -> BridgeAction {
    match cmd {
        ...
        GovernanceClientCommands::AddTokensOnAiy {
            nonce,
            token_ids,
            token_type_names,
            token_prices,
            ...
            BridgeAction::AddTokensOnAiyAction(AddTokensOnAiyAction {
                nonce: *nonce,
                chain_id,
        ...
        GovernanceClientCommands::AddTokensOnEvm {
            ...
            assert_eq!(token_ids.len(), token_prices.len());
            assert_eq!(token_ids.len(), token_aiy_decimals.len());
            BridgeAction::AddTokensOnEvmAction(AddTokensOnEvmAction {
                ...
                token_prices: token_prices.clone(),
                token_aiy_decimals: token_aiy_decimals.clone(),
            })
        }
...
pub fn select_contract_address(
    ...
    match cmd {
        ...
        GovernanceClientCommands::AddTokensOnAiy { .. } => unreachable!(),
...
pub struct BridgeCliConfig {
    ...
    /// Proxy address for AiyBridge deployed on Eth
    ...
    /// At leaset one of `aiy_key_path` or `eth_key_path` must be provided.
    /// If only one is provided, it will be used for both Aiy and Eth.
    pub aiy_key_path: Option<PathBuf>,
    /// See `aiy_key_path`. Must be Secp256k1 key.
...
impl LoadedBridgeCliConfig {
    pub async fn load(cli_config: BridgeCliConfig) -> anyhow::Result<Self> {
        if cli_config.eth_key_path.is_none() && cli_config.aiy_key_path.is_none() {
            return Err(anyhow!(
                "At least one of `aiy_key_path` or `eth_key_path` must be provided"
            ));
        }
        let aiy_key = if let Some(aiy_key_path) = &cli_config.aiy_key_path {
            Some(read_key(aiy_key_path, false)?)
        } else {
            None
        };
        ...
        let (eth_key, aiy_key) = {
            if eth_key.is_none() {
                let aiy_key = aiy_key.unwrap();
                if !matches!(aiy_key, AiyKeyPair::Secp256k1(_)) {
                    return Err(anyhow!("Eth key must be an ECDSA key"));
                }
                (aiy_key.copy(), aiy_key)
            } else if aiy_key.is_none() {
                let eth_key = eth_key.unwrap();
                (eth_key.copy(), eth_key)
            } else {
                (eth_key.unwrap(), aiy_key.unwrap())
            }
        };
        ...
        let aiy_bridge = EthAiyBridge::new(cli_config.eth_bridge_proxy_address, provider.clone());
        let eth_bridge_committee_proxy_address: EthAddress = aiy_bridge.committee().call().await?;
        let eth_bridge_limiter_proxy_address: EthAddress = aiy_bridge.limiter().call().await?;
        let eth_committee =
        ...
        let aiy_address = AiyAddress::from(&aiy_key.public());
        println!("Using Aiy address: {:?}", aiy_address);
        println!("Using Eth address: {:?}", eth_address);
        println!("Using Eth chain: {:?}", eth_chain_id);

        Ok(Self {
            aiy_rpc_url: cli_config.aiy_rpc_url,
...
impl LoadedBridgeCliConfig {
    ...
    pub async fn get_aiy_account_info(
        self: &LoadedBridgeCliConfig,
    ) -> anyhow::Result<(AiyKeyPair, AiyAddress, ObjectRef)> {
        let pubkey = self.aiy_key.public();
        let aiy_client_address = AiyAddress::from(&pubkey);
        let aiy_sdk_client = AiyClientBuilder::default()
            .build(self.aiy_rpc_url.clone())
            .await?;
        let gases = aiy_sdk_client
            .coin_read_api()
            .get_coins(aiy_client_address, None, None, None)
            .await?
            .data;
        // TODO: is 5 Aiy a good number?
        let gas = gases
            .into_iter()
        ...
        Ok((self.aiy_key.copy(), aiy_client_address, gas.object_ref()))
    }
}
#[derive(Parser)]
#[clap(rename_all = "kebab-case")]
    ...
    pub async fn handle(
        ...
        match self {
            BridgeClientCommands::DepositNativeEtherOnEth {
                ...
                let eth_tx = eth_aiy_bridge
                    .bridge_eth(aiy_recipient_address.to_vec().into(), target_chain)
                    .value(amount);
                let pending_tx = eth_tx.send().await.unwrap();
                let tx_receipt = pending_tx.await.unwrap().unwrap();
                info!(
                    "Deposited {ether_amount} Ethers to {:?} (target chain {target_chain}). Receipt: {:?}", aiy_recipient_address, tx_receipt,
                );
                Ok(())
            }
            BridgeClientCommands::ClaimOnEth { seq_num, dry_run } => {
                claim_on_eth(seq_num, config, aiy_bridge_client, dry_run)
                    .await
                    .map_err(|e| anyhow!("{:?}", e))
            }
            BridgeClientCommands::DepositOnAiy {
                coin_object_id,
                coin_type,
                target_chain,
                recipient_address,
...
async fn deposit_on_aiy(
    ...
    let aiy_client = aiy_bridge_client.aiy_client();
    let bridge_object_arg = aiy_bridge_client
    ...
    let rgp = aiy_client
        ...
        .await
        .unwrap();
    let sender = AiyAddress::from(&config.aiy_key.public());
    let gas_obj_ref = aiy_client
        .coin_read_api()
        ...
        .object_ref();
    let coin_obj_ref = aiy_client
        .read_api()
        .get_object_with_options(coin_object_id, AiyObjectDataOptions::default())
        .await?
        .data
        .unwrap()
        .object_ref();
    ...
    let sig = Signature::new_secure(
        &IntentMessage::new(Intent::aiy_transaction(), tx_data.clone()),
        &config.aiy_key,
    );
    let signed_tx = Transaction::from_data(tx_data, vec![sig]);
    let tx_digest = *signed_tx.digest();
    info!(?tx_digest, "Sending deposit transction to Aiy.");
    let resp = aiy_bridge_client
...
async fn claim_on_eth(
    ...
    let aiy_chain_id = aiy_bridge_client.get_bridge_summary().await?.chain_id;
    let parsed_message = aiy_bridge_client
        .get_parsed_token_transfer_message(aiy_chain_id, seq_num)
        .await?;
    if parsed_message.is_none() {
        println!("No record found for seq_num: {seq_num}, chain id: {aiy_chain_id}");
        return Ok(());
    }
    let parsed_message = parsed_message.unwrap();
    let sigs = aiy_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(aiy_chain_id, seq_num)
        .await;
    if sigs.is_none() {
        println!("No signatures found for seq_num: {seq_num}, chain id: {aiy_chain_id}");
        return Ok(());
    }
    let signatures = sigs
        .unwrap()
    ...
    let message = eth_aiy_bridge::Message::from(parsed_message);
    let tx = eth_aiy_bridge.transfer_bridged_tokens_with_signatures(signatures, message);
    if dry_run {
        let tx = tx.tx;
        let resp = config.eth_signer.estimate_gas(&tx, None).await;
        println!(
            "Aiy to Eth bridge transfer claim dry run result: {:?}",
            resp
        );
    } else {
        let eth_claim_tx_receipt = tx.send().await.unwrap().await.unwrap().unwrap();
        println!(
            "Aiy to Eth bridge transfer claimed: {:?}",
...
mod tests {
    ...
    async fn test_encode_call_data() {
        let abi_json =
            std::fs::read_to_string("../aiy-bridge/abi/tests/mock_aiy_bridge_v2.json").unwrap();
