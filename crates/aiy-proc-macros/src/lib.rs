    ...
    input.block = syn::parse2(quote! {
        {
            ...
            std::thread::spawn(|| {
                ...
                ::aiy_simulator::telemetry_subscribers::init_for_testing();
                ::aiy_simulator::aiy_types::execution_params::get_denied_certificates_for_sim_test();
                ::aiy_simulator::aiy_framework::BuiltInFramework::all_package_ids();
                ::aiy_simulator::aiy_types::gas::AiyGasStatus::new_unmetered();
                ...
                let mut cache = ::aiy_simulator::lru::LruCache::new(1.try_into().unwrap());
                cache.put(1, 1);

                {
                    ...
                    use aiy_simulator::aiy_move_build::{BuildConfig, AiyPackageHooks};
                    use aiy_simulator::tempfile::TempDir;
                    use aiy_simulator::move_package::package_hooks::register_package_hooks;

                    register_package_hooks(Box::new(AiyPackageHooks {}));
                ...
                use ::aiy_simulator::anemo_tower::callback::CallbackLayer;
                use ::aiy_simulator::anemo_tower::trace::DefaultMakeSpan;
                use ::aiy_simulator::anemo_tower::trace::DefaultOnFailure;
                use ::aiy_simulator::anemo_tower::trace::TraceLayer;
                use ::aiy_simulator::fastcrypto::traits::KeyPair;
                use ::aiy_simulator::mysten_network::metrics::MetricsMakeCallbackHandler;
                use ::aiy_simulator::mysten_network::metrics::NetworkMetrics;
                use ::aiy_simulator::rand_crate::rngs::{StdRng, OsRng};
                use ::aiy_simulator::rand::SeedableRng;
                use ::aiy_simulator::tower::ServiceBuilder;
                ...
                let rt = ::aiy_simulator::runtime::Runtime::new();
                rt.block_on(async move {
                    use ::aiy_simulator::anemo::{Network, Request};

                    let make_network = |port: u16| {
                        let registry = prometheus::Registry::new();
                        let inbound_network_metrics =
                            NetworkMetrics::new("aiy", "inbound", &registry);
                        let outbound_network_metrics =
                            NetworkMetrics::new("aiy", "outbound", &registry);
                        ...
                        let service = ServiceBuilder::new()
                            ...
                            .service(::aiy_simulator::anemo::Router::new());
                        ...
                        Network::bind(format!("127.0.0.1:{}", port))
                            ...
                            .private_key(
                                ::aiy_simulator::fastcrypto::ed25519::Ed25519KeyPair::generate(&mut StdRng::from_rng(OsRng).unwrap())
                                    .private()
                                    .0
                                    .to_bytes(),
                            )
...
/// The aiy_test macro will invoke either `#[msim::test]` or `#[tokio::test]`,
...
#[proc_macro_attribute]
pub fn aiy_test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let header = if cfg!(msim) {
        quote! {
            #[::aiy_simulator::sim_test(crate = "aiy_simulator", #(#args)* )]
        }
    } else {
        quote! {
            #[::tokio::test(#(#args)*)]
    ...
    let result = quote! {
        #header
        #[::aiy_macros::init_static_initializers]
        #input
    };
    ...
    let result = if cfg!(msim) {
        ...
        quote! {
            ...
            #sig {
                ...
                ::aiy_simulator::task::shutdown_all_nodes();
                ...
                assert_eq!(
                    aiy_simulator::NodeLeakDetector::get_current_node_count(),
                    0,
                    "AiyNode leak detected"
                );
        ...
        quote! {
            ...
            #ignore
            #sig {
                if std::env::var("AIY_SKIP_SIMTESTS").is_ok() {
                    println!("not running test {} in `cargo test`: AIY_SKIP_SIMTESTS is set", stringify!(#fn_name));
...
pub fn enum_variant_order_derive(input: TokenStream) -> TokenStream {
    ...
    if let Data::Enum(DataEnum { variants, .. }) = ast.data {
        ...
        let deriv = quote! {
            impl aiy_enum_compat_util::EnumOrderMap for #name {
