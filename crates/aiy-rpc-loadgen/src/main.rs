...
use aiy_types::crypto::{EncodeDecodeBase64, AiyKeyPair};
use tracing::info;
...
#[derive(Parser)]
#[clap(
    name = "Aiy RPC Load Generator",
    version = "0.1",
    about = "A load test application for Aiy RPC"
)]
struct Opts {
    ...
    #[clap(long, default_value = "~/.aiy/aiy_config/logs")]
    logs_directory: String,

    #[clap(long, default_value = "~/.aiy/loadgen/data")]
...
fn get_keypair() -> Result<SignerInfo> {
    ...
    // Load keystore from ~/.aiy/aiy_config/aiy.keystore
    let keystore_path = get_aiy_config_directory().join("aiy.keystore");
    ...
    let keypair: &AiyKeyPair = keystore.export(&active_address)?;
    println!("using address {active_address} for signing");
    Ok(SignerInfo::new(keypair.encode_base64()))
}

fn get_aiy_config_directory() -> PathBuf {
    match dirs::home_dir() {
        Some(v) => v.join(".aiy").join("aiy_config"),
...
fn get_log_file_path(dir_path: String) -> String {
    ...
    let log_filename = format!("aiy-rpc-loadgen.{}.log", timestamp);
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (command, common, need_keystore) = match opts.command {
        ...
        ClapCommand::PayAiy { common } => (Command::new_pay_aiy(), common, true),
