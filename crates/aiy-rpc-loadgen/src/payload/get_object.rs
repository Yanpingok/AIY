...
use aiy_json_rpc_types::{AiyObjectDataOptions, AiyObjectResponse};
...
pub(crate) async fn get_object(
    client: &AiyClient,
    object_id: ObjectID,
) -> Result<AiyObjectResponse> {
    let result = client
        .read_api()
        .get_object_with_options(object_id, AiyObjectDataOptions::full_content())
        .await
        .unwrap();
    Ok(result)
}
