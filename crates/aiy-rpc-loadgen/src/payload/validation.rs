...
use aiy_json_rpc_types::{
    AiyObjectDataOptions, AiyObjectResponse, AiyTransactionBlockEffectsAPI,
    AiyTransactionBlockResponse, AiyTransactionBlockResponseOptions,
};
use aiy_sdk::AiyClient;
use aiy_types::base_types::{ObjectID, TransactionDigest};
use tracing::error;
...
pub(crate) async fn check_transactions(
    ...
    cross_validate: bool,
    verify_objects: bool,
) -> Vec<Vec<AiyTransactionBlockResponse>> {
    let transactions: Vec<Vec<AiyTransactionBlockResponse>> =
        join_all(clients.iter().map(|client| async move {
            client
                .read_api()
                .multi_get_transactions_with_options(
                    digests.to_vec(),
                    AiyTransactionBlockResponseOptions::full_content(), // todo(Will) support options for this
                )
                .await
        }))
        .await
...
pub(crate) fn get_all_object_ids(response: &AiyTransactionBlockResponse) -> Vec<ObjectID> {
...
pub(crate) async fn multi_get_object(
    clients: &[AiyClient],
    object_ids: &[ObjectID],
) -> Vec<Vec<AiyObjectResponse>> {
    let objects: Vec<Vec<AiyObjectResponse>> = join_all(clients.iter().map(|client| async move {
        ...
        client
            ...
            .multi_get_object_with_options(
                object_ids.to_vec(),
                AiyObjectDataOptions::full_content(), // todo(Will) support options for this
            )
            .await
    }))
    .await
