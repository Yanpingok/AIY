...
use aiy_json_rpc_types::{
    Page, AiyTransactionBlockResponse, AiyTransactionBlockResponseOptions,
...
impl<'a> ProcessPayload<'a, &'a QueryTransactionBlocks> for RpcCommandProcessor {
    async fn process(
        ...
        let queries: Vec<AiyTransactionBlockResponseQuery> = filters
            .into_iter()
            .map(|filter| AiyTransactionBlockResponseQuery {
                filter,
...
) -> Result<Page<AiyTransactionBlockResponse, TransactionDigest>> {
