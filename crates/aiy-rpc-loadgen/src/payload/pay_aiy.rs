...
use crate::payload::{PayAiy, ProcessPayload, RpcCommandProcessor, SignerInfo};
...
#[async_trait]
impl<'a> ProcessPayload<'a, &'a PayAiy> for RpcCommandProcessor {
    async fn process(
        &'a self,
        _op: &'a PayAiy,
        ...
        let keypair =
            AiyKeyPair::decode_base64(&encoded_keypair).expect("Decoding keypair should not fail");

        debug!(
            "Transfer Aiy {} time to {recipient} with {amount} MIST with {gas_payments:?}",
            gas_payments.len()
        );
        ...
        join_all(gas_payments.iter().map(|gas| async {
            let tx = TransactionData::new_transfer_aiy(
                recipient,
                sender,
                Some(amount),
