...
use aiy_rpc::proto::aiy::rpc::v2beta2::signature_verification_service_client::SignatureVerificationServiceClient;
...
async fn test_verify_signature_zklogin() -> Result<(), anyhow::Error> {
    ...
    let (kp, pk_zklogin, inputs) = &aiy_types::utils::load_test_vectors(
        "../aiy-types/src/unit_tests/zklogin_test_vectors.json",
    )[1];
    ...
    // address checks fail
    let wrong_address = AiyAddress::random_for_testing_only();
    let response = client
        .verify_signature({
            let mut request = VerifySignatureRequest::default();
            request.message = Some(message.clone());
