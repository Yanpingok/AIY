...
use aiy_rpc::proto::aiy::rpc::v2beta2::coin_metadata::MetadataCapState;
use aiy_rpc::proto::aiy::rpc::v2beta2::coin_treasury::SupplyState;
...
use aiy_rpc::proto::aiy::rpc::v2beta2::regulated_coin_metadata::CoinRegulatedState;
...
use aiy_types::coin_registry::Currency;
...
use aiy_types::{TypeTag, AIY_COIN_REGISTRY_OBJECT_ID, AIY_FRAMEWORK_PACKAGE_ID};
use test_cluster::TestClusterBuilder;

// AIY doesn't use the CoinRegistry - it was created before the CoinRegistry system existed and has
// not been migrated.
#[sim_test]
async fn get_coin_info_aiy() {
    ...
    let coin_type_sdk: TypeTag = "0x2::aiy::AIY".parse().unwrap();
    ...
    assert_eq!(metadata.symbol, Some("AIY".to_owned()));
    assert_eq!(metadata.name, Some("Aiy".to_string()));
    ...
    assert_eq!(
        ...
        Some(aiy_types::gas_coin::TOTAL_SUPPLY_MIST)
    );
    assert_eq!(
        treasury.supply_state,
        Some(SupplyState::Fixed as i32),
        "AIY should have Fixed supply state"
    );

    // AIY is not in CoinRegistry, so regulation state is Unknown
...
async fn test_legacy_coin_from_registry() {
    ...
    let treasury_cap_arg = ptb
        .obj(ObjectArg::ImmOrOwnedObject(updated_treasury_cap_ref))
        .unwrap();
    let zero_address_arg = ptb.pure(AiyAddress::ZERO).unwrap();
...
async fn finalize_registration(
    ...
    let registry_obj_response = ledger_client
        .get_object({
            ...
            request.object_id = Some(AIY_COIN_REGISTRY_OBJECT_ID.to_string());
    ...
    let registry_owned = grpc_client
        .list_owned_objects({
            ...
            request.owner = Some(AiyAddress::from(AIY_COIN_REGISTRY_OBJECT_ID).to_string());
        ...
        .find(|obj| {
            ...
            obj_type
                .and_then(|t| {
                    ...
                    match aiy_types::parse_aiy_struct_tag(t) {
                        Ok(struct_tag) => {
    ...
    let registry_arg = ptb
        .obj(ObjectArg::SharedObject {
            id: AIY_COIN_REGISTRY_OBJECT_ID,
    ...
    let mut client = aiy_rpc::client::Client::new(test_cluster.rpc_url().to_owned()).unwrap();
