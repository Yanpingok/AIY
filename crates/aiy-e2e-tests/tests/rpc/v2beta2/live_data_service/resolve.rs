...
use aiy_rpc::proto::aiy::rpc::v2beta2::live_data_service_client::LiveDataServiceClient;
use aiy_rpc::proto::aiy::rpc::v2beta2::Argument;
use aiy_rpc::proto::aiy::rpc::v2beta2::Command;
use aiy_rpc::proto::aiy::rpc::v2beta2::GasPayment;
use aiy_rpc::proto::aiy::rpc::v2beta2::Input;
use aiy_rpc::proto::aiy::rpc::v2beta2::MoveCall;
use aiy_rpc::proto::aiy::rpc::v2beta2::ObjectReference;
use aiy_rpc::proto::aiy::rpc::v2beta2::ProgrammableTransaction;
use aiy_rpc::proto::aiy::rpc::v2beta2::SimulateTransactionRequest;
use aiy_rpc::proto::aiy::rpc::v2beta2::Transaction;
use aiy_rpc::proto::aiy::rpc::v2beta2::TransactionKind;
use aiy_rpc::proto::aiy::rpc::v2beta2::TransferObjects;
use aiy_rpc_api::Client;
...
use aiy_types::transaction::{
    Command as AiyCommand, ObjectArg, TransactionData, TransactionDataAPI,
};
use test_cluster::TestClusterBuilder;

fn proto_to_response(
    proto: aiy_rpc::proto::aiy::rpc::v2beta2::SimulateTransactionResponse,
) -> (
    aiy_types::transaction::TransactionData,
    aiy_types::effects::TransactionEffects,
...
async fn resolve_transaction_transfer_with_sponsor() {
    ...
    let sender_sig = test_cluster
        ...
        .config
        .keystore
        .sign_secure(&sender, &transaction, Intent::aiy_transaction())
        .await
        .unwrap();
    let sponsor_sig = test_cluster
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &transaction, Intent::aiy_transaction())
        .await
        .unwrap();

    let signed_transaction =
        aiy_types::transaction::Transaction::from_data(transaction, vec![sender_sig, sponsor_sig]);
    let effects = client
        .execute_transaction(&signed_transaction)
        .await
        .unwrap()
...
async fn resolve_transaction_mutable_shared_object() {
    ...
    let validator_address = test_cluster.swarm.config().validator_configs()[0].aiy_address();
    ...
    unresolved_transaction.kind = Some(TransactionKind::from({
        ...
        ptb.commands = vec![Command::from({
            ...
            message.module = Some("aiy_system".to_owned());
...
async fn resolve_transaction_insufficient_gas() {
    ...
    unresolved_transaction.sender = Some(AiyAddress::random_for_testing_only().to_string()); // random account with no
...
async fn resolve_transaction_insufficient_gas_with_payment_objects() {
    ...
    let split_result = builder.command(AiyCommand::SplitCoins(coin_arg, vec![amt_arg]));

    let split_coin = match split_result {
        aiy_types::transaction::Argument::Result(idx) => {
            aiy_types::transaction::Argument::NestedResult(idx, 0)
        }
    ...
    let mut client = aiy_rpc::client::Client::new(test_cluster.rpc_url()).unwrap();

    let mut transaction = aiy_rpc::proto::aiy::rpc::v2beta2::Transaction::default();
    transaction.bcs = Some(
        aiy_rpc::proto::aiy::rpc::v2beta2::Bcs::serialize(signed_tx.transaction_data()).unwrap(),
    );
    ...
    let signatures = signed_tx
        ...
        .iter()
        .map(|s| {
            let mut message = aiy_rpc::proto::aiy::rpc::v2beta2::UserSignature::default();
            message.bcs = Some({
                let mut message = aiy_rpc::proto::aiy::rpc::v2beta2::Bcs::default();
    ...
    let mut request = aiy_rpc::proto::aiy::rpc::v2beta2::ExecuteTransactionRequest::default();
    ...
    let effects: aiy_types::effects::TransactionEffects =
