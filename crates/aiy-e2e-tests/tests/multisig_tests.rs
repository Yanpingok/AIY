...
use aiy_core::authority_client::AuthorityAPI;
use aiy_macros::sim_test;
use aiy_protocol_config::ProtocolConfig;
use aiy_test_transaction_builder::TestTransactionBuilder;
use aiy_types::crypto::{SignatureScheme, ToFromBytes};
use aiy_types::error::UserInputError;
use aiy_types::multisig_legacy::MultiSigLegacy;
use aiy_types::passkey_authenticator::{to_signing_message, PasskeyAuthenticator};
use aiy_types::{
    base_types::AiyAddress,
    crypto::{
        get_key_pair, CompressedSignature, PublicKey, Signature, AiyKeyPair,
...
use url::Url;
async fn do_upgraded_multisig_test() -> AiyResult {
...
async fn create_credential_and_sign_test_tx_with_passkey_multisig(
    ...
    // Compute aiy address as sender, fund gas and make a test transaction.
    let sender = match sender {
        Some(s) => s,
        None => AiyAddress::from(&multisig_pk),
    };
...
async fn construct_simple_zklogin_multisig_tx(
    ...
    let multisig_addr = AiyAddress::from(&multisig_pk);
    let gas = test_cluster
...
async fn test_upgraded_multisig_feature_allow() {
    ...
    // feature gate.
    assert!(matches!(res.unwrap_err(), AiyError::UserInputError { .. }));
}
...
async fn test_multisig_e2e() {
    ...
    let kp3: AiyKeyPair = AiyKeyPair::Secp256r1(get_key_pair().1);
    let pk3 = kp3.public();
    ...
    .unwrap();
    let wrong_sender = AiyAddress::from(&wrong_multisig_pk);
    let gas = test_cluster
...
async fn test_multisig_with_zklogin_scenerios() {
    ...
    let (eph_kp, _eph_pk, zklogin_inputs) =
        &load_test_vectors("../aiy-types/src/unit_tests/zklogin_test_vectors.json")[1];
    let (eph_kp_1, _, _) =
        &load_test_vectors("../aiy-types/src/unit_tests/zklogin_test_vectors.json")[2];
    let zklogin_pk = PublicKey::ZkLogin(
        ZkLoginPublicIdentifier::new(zklogin_inputs.get_iss(), zklogin_inputs.get_address_seed())
            .unwrap(),
    );
    ...
    let wrong_multisig_addr = AiyAddress::from(
    ...
    let sender = AiyAddress::try_from(&multisig).unwrap();
    let tx_data = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_aiy(None, AiyAddress::ZERO)
        .build();
    ...
    let bad_multisig_addr = AiyAddress::from(&bad_multisig_pk);
    let gas = test_cluster
    ...
    let bad_multisig_addr_2 = AiyAddress::from(&bad_multisig_pk_2);
    let gas = test_cluster
    ...
    let bad_multisig_addr_3 = AiyAddress::from(&small_multisig_pk);
    let gas = test_cluster
    ...
    let bad_multisig_addr_4 = AiyAddress::from(&multisig_pk_with_dup);
    let gas = test_cluster
    ...
    let bad_multisig_addr_11 = AiyAddress::from(&multisig_pk_11);
    let gas = test_cluster
    ...
    let bad_multisig_addr = AiyAddress::from(&multisig_pk_12);
    let gas = test_cluster
        .fund_address_and_return_gas(rgp, Some(20000000000), bad_multisig_addr)
        .await;
    let tx_data = TestTransactionBuilder::new(bad_multisig_addr, gas, rgp)
    ...
    let bad_multisig_addr = AiyAddress::from(&bad_multisig_empty_pk);
    let gas = test_cluster
        .fund_address_and_return_gas(rgp, Some(20000000000), bad_multisig_addr)
        .await;
    let tx_data = TestTransactionBuilder::new(bad_multisig_addr, gas, rgp)
...
async fn test_random_zklogin_in_multisig() {
    ...
    let test_vectors =
        &load_test_vectors("../aiy-types/src/unit_tests/zklogin_test_vectors.json")[1..11];
    let test_cluster = TestClusterBuilder::new()
        .with_epoch_duration_ms(15000)
        .with_default_jwks()
        .build()
...
async fn test_multisig_legacy_works() {
    ...
    let transfer_from_multisig = TestTransactionBuilder::new(multisig_addr, gas, rgp)
        .transfer_aiy(Some(1000000), AiyAddress::ZERO)
