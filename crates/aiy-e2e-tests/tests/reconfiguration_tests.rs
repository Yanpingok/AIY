...
use aiy_core::consensus_adapter::position_submit_certificate;
use aiy_json_rpc_types::ObjectChange;
...
use aiy_test_transaction_builder::{make_transfer_aiy_transaction, TestTransactionBuilder};
...
async fn test_transaction_expiration() {
    ...
    *expired_data.expiration_mut_for_testing() = TransactionExpiration::Epoch(0);
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains(&AiyError::TransactionExpired.to_string()));
...
async fn test_expired_locks() {
    ...
    let transfer_aiy = |amount| {
        TestTransactionBuilder::new(sender, gas_object, gas_price)
            .transfer_aiy(Some(amount), receiver)
            .build()
    };

    let t1 = test_cluster.wallet.sign_transaction(&transfer_aiy(1)).await;
    // attempt to equivocate
    let t2 = test_cluster.wallet.sign_transaction(&transfer_aiy(2)).await;
...
async fn test_create_advance_epoch_tx_race() {
    use std::sync::Arc;
    use aiy_macros::{register_fail_point, register_fail_point_async};
    use tokio::sync::broadcast;
    use tracing::info;

    telemetry_subscribers::init_for_testing();
    aiy_protocol_config::ProtocolConfig::poison_get_for_min_version();
    ...
    let register_wait = |failpoint, node_id, tx: Arc<broadcast::Sender<()>>| {
        let node = aiy_simulator::task::NodeId(node_id);
        register_fail_point_async(failpoint, move || {
            let cur_node = aiy_simulator::current_simnode_id();
            let tx = tx.clone();
            async move {
                if cur_node == node {
...
async fn test_validator_resign_effects() {
    ...
    // in the new epoch.
    let test_cluster = TestClusterBuilder::new().build().await;
    let tx = make_transfer_aiy_transaction(&test_cluster.wallet, None, None).await;
...
async fn test_validator_candidate_pool_read() {
    ...
    let address: AiyAddress = (&new_validator.account_key_pair.public()).into();
    let test_cluster = TestClusterBuilder::new()
        .with_validator_candidates([address])
        .build()
        .await;
    add_validator_candidate(&test_cluster, &new_validator).await;
    test_cluster.fullnode_handle.aiy_node.with(|node| {
        let system_state = node
            .state()
            .get_aiy_system_state_object_for_testing()
            .unwrap();
        let system_state_summary = system_state.clone().into_aiy_system_state_summary();
        ...
        .unwrap();
        assert_eq!(validator.aiy_address, address);
    });
}
...
async fn test_inactive_validator_pool_read() {
    ...
    let address = validator.with(|node| node.get_config().aiy_address());
    let staking_pool_id = test_cluster.fullnode_handle.aiy_node.with(|node| {
        node.state()
            .get_aiy_system_state_object_for_testing()
            .unwrap()
...
const VALIDATOR_STARTING_STAKE: u64 = 1_000_000_000_000_000; // 1M AIY
...
async fn test_reconfig_with_committee_change_basic() {
    ...
    let total_stake = test_cluster.fullnode_handle.aiy_node.with(|node| {
        node.state()
            .get_aiy_system_state_object_for_testing()
            .unwrap()
            .into_aiy_system_state_summary()
...
async fn test_protocol_upgrade_to_sip_39_enabled_version() {
    let _guard =
        aiy_protocol_config::ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
...
async fn test_epoch_flag_upgrade() {
    ...
    use aiy_macros::register_fail_point_arg;
    ...
    register_fail_point_arg("initial_epoch_flags", move || {
        ...
        let current_node = aiy_simulator::current_simnode_id();
...
async fn safe_mode_reconfig_test() {
    use aiy_test_transaction_builder::make_staking_transaction;
    use aiy_types::aiy_system_state::advance_epoch_result_injection;
...
async fn add_validator_candidate(
    ...
    let cur_validator_candidate_count = test_cluster.fullnode_handle.aiy_node.with(|node| {
        node.state()
            .get_aiy_system_state_object_for_testing()
            .unwrap()
            .into_aiy_system_state_summary()
    ...
    test_cluster.fullnode_handle.aiy_node.with(|node| {
        let system_state = node
            ...
            .get_aiy_system_state_object_for_testing()
            .unwrap();
        let system_state_summary = system_state.into_aiy_system_state_summary();
        assert_eq!(
...
async fn execute_remove_validator_tx(test_cluster: &TestCluster, handle: &AiyNodeHandle) {
    let address = handle.with(|node| node.get_config().aiy_address());
    let gas = test_cluster
        .wallet
        .get_one_gas_object_owned_by_address(address)
        .await
...
async fn execute_add_stake_transaction(
    ...
    let system_arg = ptb.obj(ObjectArg::AIY_SYSTEM_MUT).unwrap();
    ...
    response
        ...
        .filter(|change| match change {
            ObjectChange::Created { object_type, .. } => {
                object_type.name == ident_str!("StakedAiy").into()
            }
            _ => false,
        })
        .collect::<Vec<_>>()
...
async fn execute_add_validator_transactions(
    ...
    let pending_active_count = test_cluster.fullnode_handle.aiy_node.with(|node| {
        let system_state = node
            .state()
            .get_aiy_system_state_object_for_testing()
            .unwrap();
    ...
    test_cluster.fullnode_handle.aiy_node.with(|node| {
        ...
        assert_eq!(
            pending_active_validators[pending_active_validators.len() - 1].aiy_address,
            address
        );
    });
}
