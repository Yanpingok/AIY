...
module aiy_system::aiy_system {
    ...
    fun load_inner_maybe_upgrade(self: &mut AiySystemState): &mut AiySystemStateInner {
        let version = self.version;
        // TODO: This is where we check the version and perform upgrade if necessary.

        let inner: &mut AiySystemStateInner = dynamic_field::borrow_mut(&mut self.id, version);
