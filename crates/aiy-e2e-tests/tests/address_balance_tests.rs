...
async fn get_sender_and_gas(context: &mut WalletContext) -> (AiyAddress, ObjectRef) {
    let sender = context
        .config
        .keystore
        .addresses()
...
fn verify_accumulator_exists(
    ...
    let aiy_coin_type = Balance::type_tag(GAS::type_tag());

    assert!(
        AccumulatorValue::exists(child_object_resolver, None, owner, &aiy_coin_type).unwrap(),
        "Accumulator value should have been created"
    );

    let accumulator_object =
        AccumulatorValue::load_object(child_object_resolver, None, owner, &aiy_coin_type)
    ...
    let accumulator_value =
        AccumulatorValue::load(child_object_resolver, None, owner, &aiy_coin_type)
            .expect("read cannot fail")
            .expect("accumulator should exist");
    ...
    assert!(
        owner
            .metadata_exists(child_object_resolver, None, &aiy_coin_type)
            .unwrap(),
        "Metadata object should have been created"
    );

    let _metadata = owner
        .load_metadata(child_object_resolver, None, &aiy_coin_type)
        .unwrap();
}
...
async fn test_deposit_and_withdraw() {
    ...
    test_cluster.fullnode_handle.aiy_node.with(|node| {
        ...
        assert!(
            !AccumulatorValue::exists(child_object_resolver, None, sender, &aiy_coin_type).unwrap(),
...
fn withdraw_from_balance_tx_with_reservation(
    ...
    let withdraw_arg = aiy_types::transaction::FundsWithdrawalArg::balance_from_sender(
        reservation_amount,
        aiy_types::type_input::TypeInput::from(aiy_types::gas_coin::GAS::type_tag()),
    );
