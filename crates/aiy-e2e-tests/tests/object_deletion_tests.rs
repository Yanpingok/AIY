...
mod sim_only_tests {
    ...
    use aiy_test_transaction_builder::publish_package;
    use aiy_types::messages_checkpoint::CheckpointSequenceNumber;
    use aiy_types::{base_types::ObjectID, digests::TransactionDigest};
    ...
    async fn object_pruning_test() {
        let test_cluster = TestClusterBuilder::new()
            ...
            .build()
            .await;
        let fullnode = &test_cluster.fullnode_handle.aiy_node;
    ...
    async fn publish_package_and_create_parent_object(
        ...
        let package_id = publish_package(
            ...
            PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("../aiy-surfer/tests/move_building_blocks"),
        )
        .await
        .0;
    ...
    async fn wait_until_checkpoint_pruned(node: &AiyNode, checkpoint: CheckpointSequenceNumber) {
        loop {
            if node
                .state()
