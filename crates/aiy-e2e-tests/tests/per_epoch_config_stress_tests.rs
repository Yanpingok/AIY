...
use aiy_types::base_types::{EpochId, ObjectID, ObjectRef, AiyAddress};
...
const DENY_ADDRESS: AiyAddress = AiyAddress::ZERO;
...
struct TestEnv {
    ...
    regulated_coin_owner: AiyAddress,
...
async fn create_test_env() -> TestEnv {
    ...
    let deny_list_object_init_version = test_cluster
        .get_object_from_fullnode_store(&AIY_DENY_LIST_OBJECT_ID)
        .await
        .unwrap()
        .version();
