...
use aiy_macros::*;
use aiy_swarm_config::genesis_config::{AccountConfig, DEFAULT_GAS_AMOUNT};
...
use aiy_types::{
    base_types::{ObjectDigest, ObjectID, ObjectRef, AiyAddress},
...
const MAX_DELEGATION_AMOUNT: u64 = 1_000_000_000_000_000; // 1M AIY
const MIN_DELEGATION_AMOUNT: u64 = 500_000_000_000_000; // 0.5M AIY
...
struct StressTestRunner {
    ...
    pub accounts: Vec<AiyAddress>,
    pub active_validators: BTreeSet<AiyAddress>,
    pub preactive_validators: BTreeMap<AiyAddress, u64>,
    pub removed_validators: BTreeSet<AiyAddress>,
    pub delegation_requests_this_epoch: BTreeMap<ObjectID, AiyAddress>,
    pub delegation_withdraws_this_epoch: u64,
    pub delegations: BTreeMap<ObjectID, (AiyAddress, ObjectID, ObjectDigest, SequenceNumber)>,
    pub reports: BTreeMap<AiyAddress, BTreeSet<AiyAddress>>,
    pub rng: StdRng,
}
...
impl StressTestRunner {
    ...
    pub fn pick_random_sender(&mut self) -> AiyAddress {
        self.accounts[self.rng.gen_range(0..self.accounts.len())]
    }

    pub fn system_state(&self) -> AiySystemStateSummary {
        self.state()
    ...
    pub fn pick_random_active_validator(&mut self) -> AiyValidatorSummary {
    ...
    pub async fn run(&self, sender: AiyAddress, pt: ProgrammableTransaction) -> TransactionEffects {
    ...
    pub fn display_effects(&self, effects: &TransactionEffects) {
        ...
        for (obj_ref, _) in effects.created() {
            ...
            println!(">> {struct_tag} TOTAL_AIY: {total_aiy}");
        }
    ...
    pub fn state(&self) -> Arc<AuthorityState> {
        self.test_cluster.fullnode_handle.aiy_node.state()
    }
...
mod add_stake {
    ...
    impl GenStateChange for RequestAddStakeGen {
        ...
        fn create(&self, runner: &mut StressTestRunner) -> Self::StateChange {
            ...
            let staked_with = runner.pick_random_active_validator().aiy_address;
    ...
    impl StatePredicate for RequestAddStake {
        async fn run(&mut self, runner: &mut StressTestRunner) -> Result<TransactionEffects> {
            let pt = {
                ...
                builder.obj(ObjectArg::AIY_SYSTEM_MUT).unwrap();
                builder.pure(self.staked_with).unwrap();
                let coin = StressTestRunner::split_off(&mut builder, self.stake_amount);
                move_call! {
                    builder,
                    (AIY_SYSTEM_PACKAGE_ID)::aiy_system::request_add_stake(Argument::Input(0), coin, Argument::Input(1))
                };
                builder.finish()
            };
        ...
        async fn pre_epoch_post_condition(
            ...
            // Assert that a `StakedAiy` object matching the amount delegated is created.
            // Assert that this staked aiy
            let object = runner
                .get_created_object_of_type_name(effects, "StakedAiy")
                .await
                .unwrap();
            ...
            let staked_aiy: StakedAiy =
                bcs::from_bytes(object.data.try_as_move().unwrap().contents()).unwrap();

            assert_eq!(staked_aiy.principal(), self.stake_amount);
    ...
    impl StatePredicate for RequestWithdrawStake {
        async fn run(&mut self, runner: &mut StressTestRunner) -> Result<TransactionEffects> {
            let pt = {
                ...
                move_call! {
                    builder,
                    (AIY_SYSTEM_PACKAGE_ID)::aiy_system::request_withdraw_stake(Argument::Input(0), staked_aiy)
                };
                builder.finish()
            };
...
async fn fuzz_dynamic_committee() {
    ...
    let mut initial_committee = runner
        ...
        .active_validators
        .iter()
        .map(|v| (v.aiy_address, v.voting_power))
        .collect::<Vec<_>>();
    ...
    let total_stake = active_validators
        .iter()
        .fold(0, |acc, v| acc + v.staking_pool_aiy_balance);

    // Use the formula for voting_power from Aiy System to check if the voting power is correctly
    // set. See `crates/aiy-framework/packages/aiy-system/sources/voting_power.move`.
    ...
    active_validators.iter().for_each(|v| {
        ...
        let calculated_power =
            ((v.staking_pool_aiy_balance as u128 * 10_000) / total_stake as u128).min(1_000) as u64;
