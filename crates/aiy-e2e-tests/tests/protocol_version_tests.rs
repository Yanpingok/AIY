...
mod sim_only_tests {
    ...
    use aiy_core::authority::framework_injection;
    use aiy_framework::BuiltInFramework;
    use aiy_json_rpc_api::WriteApiClient;
    use aiy_json_rpc_types::{AiyTransactionBlockEffects, AiyTransactionBlockEffectsAPI};
    use aiy_macros::*;
    use aiy_move_build::{BuildConfig, CompiledPackage};
    use aiy_protocol_config::Chain;
    use aiy_types::base_types::ConciseableName;
    use aiy_types::base_types::{FullObjectID, FullObjectRef, ObjectID, ObjectRef};
    use aiy_types::effects::{TransactionEffects, TransactionEffectsAPI};
    use aiy_types::id::ID;
    use aiy_types::object::Owner;
    use aiy_types::aiy_system_state::{
        epoch_start_aiy_system_state::EpochStartSystemStateTrait, get_validator_from_table,
        AiySystemState, AiySystemStateTrait, AIY_SYSTEM_STATE_SIM_TEST_DEEP_V2,
        AIY_SYSTEM_STATE_SIM_TEST_SHALLOW_V2, AIY_SYSTEM_STATE_SIM_TEST_V1,
    };
    use aiy_types::supported_protocol_versions::SupportedProtocolVersions;
    ...
    use aiy_types::{
        base_types::{SequenceNumber, AiyAddress},
        digests::TransactionDigest,
        object::Object,
        programmable_transaction_builder::ProgrammableTransactionBuilder,
        transaction::TransactionKind,
        MOVE_STDLIB_PACKAGE_ID, AIY_BRIDGE_OBJECT_ID, AIY_FRAMEWORK_PACKAGE_ID,
        AIY_SYSTEM_PACKAGE_ID,
    };
    use aiy_types::{
        AIY_AUTHENTICATOR_STATE_OBJECT_ID, AIY_CLOCK_OBJECT_ID, AIY_RANDOMNESS_STATE_OBJECT_ID,
    ...
    async fn test_framework_add_struct_ability() {
        ...
        transfer_obj(&cluster, AiyAddress::ZERO, to_transfer0).await;
        wrap_obj(&cluster, to_wrap1.as_object_ref()).await;
        transfer_obj(&cluster, AiyAddress::ZERO, to_transfer1).await;
    }
    ...
    async fn test_new_framework_package() {
        ...
        let aiy_extra = ObjectID::from_single_byte(0x42);
        framework_injection::set_override(aiy_extra, fixture_modules("extra_package"));
        ...
        // initializer
        let effects = get_framework_upgrade_effects(&cluster, &aiy_extra).await;
            ...
            .find_map(|(obj, owner)| {
                if let Owner::Shared { .. } = owner {
                    let is_framework_obj = [
                        AIY_SYSTEM_STATE_OBJECT_ID,
                        AIY_CLOCK_OBJECT_ID,
                        AIY_AUTHENTICATOR_STATE_OBJECT_ID,
                        AIY_RANDOMNESS_STATE_OBJECT_ID,
                        AIY_BRIDGE_OBJECT_ID,
                    ]
                    .contains(&obj.0);
    ...
    async fn run_framework_upgrade(from: &str, to: &str) -> TestCluster {
        ...
        override_aiy_system_modules(to);
        TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([aiy_system_package_object(from)])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    async fn get_framework_upgrade_versions(
        ...
        let effects = get_framework_upgrade_effects(cluster, &AIY_SYSTEM_PACKAGE_ID).await;

        let modified_at = effects
            .modified_at_versions()
            .iter()
            .find_map(|(id, v)| (id == &AIY_SYSTEM_PACKAGE_ID).then_some(*v));

        let mutated_to = effects
            .mutated()
            .iter()
            .find_map(|((id, v, _), _)| (id == &AIY_SYSTEM_PACKAGE_ID).then_some(*v));
    ...
    async fn get_framework_upgrade_effects(
        ...
        let node_handle = &cluster.fullnode_handle.aiy_node;
    ...
    async fn test_framework_compatible_upgrade_no_protocol_version() {
        ...
        override_aiy_system_modules("compatible");
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([aiy_system_package_object("base")])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, START,
            ))
            .build()
    ...
    async fn test_framework_upgrade_conflicting_versions() {
        ...
        override_aiy_system_modules_cb(Box::new(move |name| {
            if name == first_name {
                info!("node {:?} using compatible packages", name.concise());
                Some(aiy_system_modules("base"))
            } else {
                Some(aiy_system_modules("compatible"))
            }
        }));
    ...
    async fn test_safe_mode_recovery() {
        ...
        override_aiy_system_modules("mock_aiy_systems/base");
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            // Overrides with a aiy system package that would abort during epoch change txn
            .with_objects([aiy_system_package_object("mock_aiy_systems/safe_mode")])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    #[sim_test]
    async fn aiy_system_mock_smoke_test() {
        let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
            config.set_disable_bridge_for_testing();
            config
        });
        ...
        let test_cluster = TestClusterBuilder::new()
            ...
            .with_objects([aiy_system_package_object("mock_aiy_systems/base")])
            .build()
            .await;
    ...
    #[sim_test]
    async fn aiy_system_state_shallow_upgrade_test() {
        let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
            config.set_disable_bridge_for_testing();
            config
        });

        override_aiy_system_modules("mock_aiy_systems/shallow_upgrade");
        ...
        assert!(matches!(system_state, AiySystemState::SimTestV1(_)));
        ...
        assert!(matches!(system_state, AiySystemState::SimTestShallowV2(_)));
    }

    #[sim_test]
    async fn aiy_system_state_deep_upgrade_test() {
        let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
            config.set_disable_bridge_for_testing();
            config
        });

        override_aiy_system_modules("mock_aiy_systems/deep_upgrade");
        ...
        if let AiySystemState::SimTestV1(inner) = system_state {
        ...
        if let AiySystemState::SimTestDeepV2(inner) = system_state {
            // Make sure we have 1 inactive validator for latter testing.
            assert_eq!(inner.validators.inactive_validators.size, 1);
            get_validator_from_table(
                test_cluster
    ...
    #[sim_test]
    async fn aiy_system_state_production_upgrade_test() {
        // Use this test to test a real aiy system state upgrade. To make this test work,
        // put the new aiy system in a new path and point to it in the override.
        ...
        override_aiy_system_modules("../../../aiy-framework/packages/aiy-system");
        // Wait for the upgrade to finish. After the upgrade, the new framework will be installed,
        // but the system state object hasn't been upgraded yet.
        let system_state = test_cluster.wait_for_epoch(Some(1)).await;
        assert_eq!(system_state.protocol_version(), FINISH);
        ...
        // at epoch boundary.
        let system_state = test_cluster.wait_for_epoch(Some(2)).await;
        if let AiySystemState::V2(inner) = system_state {
            assert_eq!(inner.parameters.min_validator_count, 4);
        } else {
            unreachable!("Unexpected aiy system state version");
        }
    }
    ...
    fn override_aiy_system_modules(path: &str) {
        framework_injection::set_override(AIY_SYSTEM_PACKAGE_ID, aiy_system_modules(path));
    }

    fn override_aiy_system_modules_cb(f: framework_injection::PackageUpgradeCallback) {
        framework_injection::set_override_cb(AIY_SYSTEM_PACKAGE_ID, f)
    }

    /// Get compiled modules for Aiy System, built from fixture `fixture` in the
    /// `framework_upgrades` directory.
    fn aiy_system_modules(fixture: &str) -> Vec<CompiledModule> {
        fixture_package(fixture)
            .get_aiy_system_modules()
            .cloned()
            .collect()
    }

    /// Like `aiy_system_modules`, but package the modules in an `Object`.
    fn aiy_system_package_object(fixture: &str) -> Object {
        Object::new_package(
            &aiy_system_modules(fixture),
            ...
            &[
                ...
                BuiltInFramework::get_package_by_id(&AIY_FRAMEWORK_PACKAGE_ID)
