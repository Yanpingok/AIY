...
use aiy_types::{AIY_COIN_REGISTRY_OBJECT_ID, AIY_FRAMEWORK_PACKAGE_ID};
...
async fn test_create_coin_registry_object() {
    ...
    let framework = aiy_framework_snapshot::load_bytecode_snapshot(95)
        .unwrap()
        .into_iter()
    ...
    let package = framework
        .iter()
        .find(|f| f.id() == AIY_FRAMEWORK_PACKAGE_ID)
        .unwrap()
        .data
        .try_as_package()
        .unwrap();
    ...
    for h in &handles {
        h.with(|node| {
            assert!(node
                ...
                .get_latest_object_ref_or_tombstone(AIY_COIN_REGISTRY_OBJECT_ID)
                .is_none());
        });
    }
