...
use std::future;
use aiy_sdk::{AiyClientBuilder, AIY_COIN_TYPE};
...
//     let client = AiyClientBuilder::default().build(rpc_url).await?;
//     let txs = client
//         .read_api()
//         .get_transactions_stream(AiyTransactionBlockResponseQuery::default(), None, true)
//         .collect::<Vec<_>>()
//         .await;
...
//     AiyClientCommands::CreateExampleNFT {
//         name: None,
//         description: None,
//         url: None,
//         gas: None,
...
async fn test_coins_stream() -> Result<(), anyhow::Error> {
    ...
    let coins = client
        .coin_read_api()
        .get_coins_stream(address, Some(AIY_COIN_TYPE.to_string()))
        .collect::<Vec<_>>()
        .await;
    ...
    let page = client
        .coin_read_api()
        .get_coins(address, Some(AIY_COIN_TYPE.to_string()), None, None)
        .await?;
