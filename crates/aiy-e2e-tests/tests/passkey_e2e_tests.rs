...
async fn execute_tx(tx: Transaction, test_cluster: &TestCluster) -> AiyResult {
    test_cluster
...
async fn test_passkey_fails_wrong_author() {
    ...
    let response =
        create_credential_and_sign_test_tx(&test_cluster, Some(AiyAddress::ZERO), false, false)
            .await;
    let sig = GenericSignature::PasskeyAuthenticator(
        PasskeyAuthenticator::new_for_testing(
            response.authenticator_data,
    ...
    let res = execute_tx(tx, &test_cluster).await;
    let err = res.unwrap_err();
    assert!(matches!(err, AiyError::SignerSignatureAbsent { .. }));
}
