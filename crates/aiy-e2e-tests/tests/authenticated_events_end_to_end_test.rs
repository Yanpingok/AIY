...
use aiy_json_rpc_types::{AiyTransactionBlockEffectsAPI, AiyTransactionBlockResponse};
...
async fn setup_test_cluster_with_auth_events() -> TestCluster {
    let _guard: aiy_protocol_config::OverrideGuard =
...
async fn publish_auth_event_package(test_cluster: &TestCluster) -> ObjectID {
    ...
    let (package_id, _, _) =
        aiy_test_transaction_builder::publish_package(&test_cluster.wallet, path).await;
    package_id
}
...
async fn try_emit_authenticated_event(
    ...
    let tx_data = TransactionData::new(
        aiy_types::transaction::TransactionKind::ProgrammableTransaction(ptb.finish()),
        sender,
        {
...
async fn load_event_stream_head_by_object_id(
    state: &aiy_core::authority::AuthorityState,
    object_id: ObjectID,
...
async fn authenticated_events_single_event_test() {
    ...
    assert_eq!(
        event.address,
        AiyAddress::from(AccountAddress::from(package_id))
    );

    let state = test_cluster.fullnode_handle.aiy_node.state();
