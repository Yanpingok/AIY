...
use aiy_name_service::{Domain, NameRecord, SubDomainRegistration};
use aiy_types::{
...
pub struct AiynsIndexer {
    registry_table_id: AiyAddress,
    subdomain_wrapper_type: StructTag,
    name_record_type: StructTag,
}

impl std::default::Default for AiynsIndexer {
    fn default() -> Self {
        Self::new(
...
impl AiynsIndexer {
    ...
    pub fn new(registry_address: String, wrapper_type: String, record_type: String) -> Self {
        let registry_table_id = AiyAddress::from_str(&registry_address).unwrap();
    ...
    pub fn process_checkpoint(&self, data: &CheckpointData) -> (Vec<VerifiedDomain>, Vec<String>) {
        let mut checkpoint = AiynsIndexerCheckpoint::new(data.checkpoint_summary.sequence_number);
...
pub struct AiynsIndexerCheckpoint {
...
impl AiynsIndexerCheckpoint {
    ...
    pub fn parse_record_changes(&mut self, config: &AiynsIndexer, objects: &[Object]) {
...
/// Allows us to format a AiyNS specific query for updating the DB entries
