...
use aiyns_indexer::{
    get_connection_pool,
    indexer::{format_update_field_query, format_update_subdomain_wrapper_query, AiynsIndexer},
    models::VerifiedDomain,
    schema::domains,
    PgConnectionPool,
};

struct AiynsIndexerWorker {
    pg_pool: PgConnectionPool,
    indexer: AiynsIndexer,
}

impl AiynsIndexerWorker {
...
#[async_trait]
impl Worker for AiynsIndexerWorker {
    type Result = ();
...
async fn main() -> Result<()> {
    ...
    let indexer_setup =
        ...
        {
            AiynsIndexer::new(registry_id, subdomain_wrapper_type, name_record_type)
        } else {
            AiynsIndexer::default()
        };
    ...
    let worker_pool = WorkerPool::new(
        ...
        "aiyns_indexing".to_string(), /* task name used as a key in the progress store */
        100,                          /* concurrency */
    );
