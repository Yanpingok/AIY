...
use aiy_config::genesis::{GenesisCeremonyParameters, TokenAllocation};
use aiy_config::node::{DEFAULT_COMMISSION_RATE, DEFAULT_VALIDATOR_GAS_PRICE};
use aiy_config::{local_ip_utils, Config};
use aiy_genesis_builder::validator_info::{GenesisValidatorInfo, ValidatorInfo};
use aiy_types::base_types::AiyAddress;
use aiy_types::crypto::{
    generate_proof_of_possession, get_key_pair_from_rng, AccountKeyPair, AuthorityKeyPair,
    AuthorityPublicKeyBytes, NetworkKeyPair, NetworkPublicKey, PublicKey, AiyKeyPair,
};
...
pub struct ValidatorGenesisConfig {
    ...
    #[serde(default = "default_aiy_key_pair")]
    pub account_key_pair: AiyKeyPair,
    #[serde(default = "default_ed25519_key_pair")]
...
impl ValidatorGenesisConfig {
    pub fn to_validator_info(&self, name: String) -> GenesisValidatorInfo {
        ...
        let info = ValidatorInfo {
            ...
            worker_key,
            network_key,
            account_address: AiyAddress::from(&account_key),
...
fn default_aiy_key_pair() -> AiyKeyPair {
    AiyKeyPair::Ed25519(get_key_pair_from_rng(&mut rand::rngs::OsRng).1)
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct AccountConfig {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub address: Option<AiyAddress>,
...
impl GenesisConfig {
    ...
    pub fn for_local_testing_with_addresses(addresses: Vec<AiyAddress>) -> Self {
    ...
    pub fn benchmark_gas_keys(n: usize) -> Vec<AiyKeyPair> {
        let mut rng = StdRng::seed_from_u64(Self::BENCHMARKS_RNG_SEED);
        (0..n)
            .map(|_| AiyKeyPair::Ed25519(NetworkKeyPair::generate(&mut rng)))
            .collect()
    }
