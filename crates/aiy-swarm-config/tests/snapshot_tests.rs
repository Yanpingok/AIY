...
// This file contains tests that detect changes in Aiy configs.
...
// 1. Run `cargo insta test --review` under `./aiy-config`.
...
use aiy_config::genesis::{GenesisCeremonyParameters, TokenDistributionScheduleBuilder};
...
fn populated_genesis_snapshot_matches() {
    ...
    let genesis = Builder::new()
        ...
        .build();
    assert_yaml_snapshot!(genesis.aiy_system_wrapper_object());
