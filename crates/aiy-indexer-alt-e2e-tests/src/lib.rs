...
use aiy_indexer_alt::{config::IndexerConfig, setup_indexer, BootstrapGenesis};
...
use aiy_indexer_alt_consistent_store::{
...
use aiy_indexer_alt_framework::{ingestion::ClientArgs, postgres::schema::watermarks, IndexerArgs};
...
impl FullCluster {
    ...
    pub fn funded_account(
        &mut self,
        amount: u64,
    ) -> anyhow::Result<(AiyAddress, AccountKeyPair, ObjectRef)> {
