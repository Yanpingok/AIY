...
use aiy_indexer_alt_e2e_tests::{
    local_ingestion_client_args, write_checkpoint, OffchainCluster, OffchainClusterConfig,
};
use aiy_indexer_alt_schema::{checkpoints::StoredGenesis, epochs::StoredEpochStart};
use aiy_types::{
    balance::Balance,
    digests::Digest,
    messages_checkpoint::{CheckpointCommitment, ECMHLiveObjectSetDigest},
    aiy_system_state::{
        mock, aiy_system_state_inner_v1::AiySystemStateInnerV1,
        aiy_system_state_inner_v2::AiySystemStateInnerV2, AiySystemState,
    },
...
async fn safe_mode_system_state_v1() -> Result<(), anyhow::Error> {
    let aiy_system_state = AiySystemState::V1(AiySystemStateInnerV1 {
        safe_mode: ENABLED,
        ...
        ..mock::aiy_system_state_inner_v1()
    });
    let SafeModeEpoch {
        safe_mode:
            SafeMode {
        ...
        SAFE_MODE_QUERY,
        aiy_system_state.clone(),
        AdvanceEpochConfig {
            output_objects: mock::system_state_output_objects(aiy_system_state),
            ..Default::default()
        },
    )
    .await?;
...
async fn safe_mode_system_state_v2() -> Result<(), anyhow::Error> {
    let aiy_system_state = AiySystemState::V2(AiySystemStateInnerV2 {
        safe_mode: ENABLED,
        safe_mode_computation_rewards: Balance::new(COMPUTATION_COST),
        safe_mode_storage_rewards: Balance::new(STORAGE_COST),
        safe_mode_storage_rebates: STORAGE_REBATE,
        safe_mode_non_refundable_storage_fee: NON_REFUNDABLE_STORAGE_FEE,
        ..mock::aiy_system_state_inner_v2()
    });
    let SafeModeEpoch {
        safe_mode:
            SafeMode {
...
async fn live_object_set_digest() -> Result<(), anyhow::Error> {
    let aiy_system_state = AiySystemState::V2(mock::aiy_system_state_inner_v2());
...
async fn test_graphql<T: DeserializeOwned>(
    query: &str,
    genesis_aiy_system_state: AiySystemState,
        ...
        OffchainClusterConfig {
            ...
            bootstrap_genesis: Some(BootstrapGenesis {
                ...
                stored_epoch_start: StoredEpochStart {
                    ...
                    system_state: bcs::to_bytes(&genesis_aiy_system_state)?,
                },
            }),
            ..Default::default()
        },
