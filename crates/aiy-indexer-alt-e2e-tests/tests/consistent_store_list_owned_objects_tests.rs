...
use aiy_types::{
    base_types::{FullObjectRef, ObjectRef, AiyAddress},
    crypto::{get_account_key_pair, Signature, Signer},
    effects::{TransactionEffects, TransactionEffectsAPI},
    gas_coin::GasCoin,
    programmable_transaction_builder::ProgrammableTransactionBuilder,
    transaction::{Argument, Command, Transaction, TransactionData},
    TypeTag, AIY_FRAMEWORK_PACKAGE_ID,
};
...
fn create_bag(cluster: &mut FullCluster, owner: AiyAddress, ty: TypeTag, size: u64) -> ObjectRef {
    let (sender, kp, gas) = cluster
...
/// owned by `owner` with `size` many elements. The purpose of this is to create an object that
/// isn't a coin. `ty` can be any numeric Move type.
fn create_table(cluster: &mut FullCluster, owner: AiyAddress, ty: TypeTag, size: u64) -> ObjectRef {
    let (sender, kp, gas) = cluster
        .funded_account(DEFAULT_GAS_BUDGET)
        .expect("Failed to fund account");
