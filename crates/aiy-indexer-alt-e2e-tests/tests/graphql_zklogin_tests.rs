...
use aiy_indexer_alt_e2e_tests::{OffchainCluster, OffchainClusterConfig};
use aiy_indexer_alt_framework::ingestion::ClientArgs;
use aiy_indexer_alt_graphql::config::{RpcConfig as GraphQlConfig, ZkLoginConfig, ZkLoginEnv};
use aiy_swarm_config::genesis_config::AccountConfig;
use aiy_test_transaction_builder::TestTransactionBuilder;
use aiy_types::{
    base_types::AiyAddress, crypto::Signature, signature::GenericSignature,
...
const QUERY: &str = r#"
query ($bytes: Base64!, $signature: Base64!, $scope: ZkLoginIntentScope!, $author: AiyAddress!) {
...
async fn test_verify_transaction() {
    ...
    let message = IntentMessage::new(Intent::aiy_transaction(), tx.clone());
...
async fn test_verify_invalid_transaction() {
    ...
    let message = IntentMessage::new(Intent::aiy_transaction(), tx);
    let signature = GenericSignature::ZkLoginAuthenticator(ZkLoginAuthenticator::new(
        inputs.clone(),
        2,
        Signature::new_secure(&message, kp),
...
async fn test_verify_invalid_signature() {
    ...
    let (_, pk, _) = &load_test_vectors("../aiy-types/src/unit_tests/zklogin_test_vectors.json")[1];

    let addr: AiyAddress = pk.into();
    let personal = b"Hello, World!".to_vec();
...
async fn test_verify_not_zklogin_signature() {
    ...
    let keypair = AiyKeyPair::Ed25519(Ed25519KeyPair::generate(
        &mut rand::rngs::StdRng::from_seed([1; 32]),
    ));
    let addr = AiyAddress::from(&keypair.public());
