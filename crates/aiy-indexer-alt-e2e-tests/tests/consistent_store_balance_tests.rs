...
use aiy_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::{
    consistent_service_client::ConsistentServiceClient, BatchGetBalancesRequest, GetBalanceRequest,
    ListBalancesRequest,
};
use aiy_indexer_alt_e2e_tests::{find, FullCluster};
...
async fn test_aggregation() {
    ...
    // Create multiple AIY coins for address A with different amounts
    create_coin(&mut cluster, a, 1);
    create_coin(&mut cluster, a, 2);
    create_coin(&mut cluster, a, 3);

    // Create AIY coins for address B
...
async fn test_multiple_coin_types() {
    ...
    let aiy_balance = DEFAULT_GAS_BUDGET as i64 - fx.gas_cost_summary().net_gas_usage();
    ...
    // P's balances should include AIY (left over from the gas coin) and MY_COIN (1000 + 200 + 30)
    // = 1230 from minting during publish and init.
    let mut balances = vec![
        (gas_type.clone(), aiy_balance as u64),
    ...
    assert_eq!(
        ...
        vec![
            ...
            (p.to_string(), gas_type.clone(), aiy_balance as u64),
        ]
    );
}
...
async fn test_transfers() {
    ...
    builder.transfer_aiy(b, Some(1000));
...
async fn test_edge_cases() {
    ...
    request
        .metadata_mut()
        .insert("x-aiy-checkpoint", "10".parse().unwrap());
...
/// `amount` to `owner`.
fn create_coin(cluster: &mut FullCluster, owner: AiyAddress, amount: u64) -> ObjectRef {
    let (sender, kp, gas) = cluster
        .funded_account(DEFAULT_GAS_BUDGET + amount)
        .expect("Failed to fund account");

    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_aiy(owner, Some(amount));
...
async fn list_balances(
    ...
    if let Some(checkpoint) = checkpoint {
        request
            .metadata_mut()
            .insert("x-aiy-checkpoint", checkpoint.to_string().parse().unwrap());
    }
...
async fn batch_get_balances(
    cluster: &FullCluster,
    requests: Vec<(AiyAddress, String)>,
    checkpoint: Option<u64>,
