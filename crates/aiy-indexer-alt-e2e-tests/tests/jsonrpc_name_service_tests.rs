...
use aiy_indexer_alt_jsonrpc::config::{NameServiceConfig, RpcConfig as JsonRpcConfig};
...
/// Tests happy path for AiyNS resolution.
...
/// Tests AiyNS resolution response when the domain has expired.
...
async fn test_resolve_domain_expiry() {
    ...
    assert_invalid_params!(c.resolve_address("foo.aiy").await.unwrap());
...
async fn test_resolve_subdomain_parent_expiry() {
    ...
    assert_invalid_params!(c.resolve_address("bar.foo.aiy").await.unwrap());
    assert_no_reverse!(c.resolve_name(target).await.unwrap());
...
impl AiyNSCluster {
    /// Sets up a full cluster with a mock AiyNS package and registries. RPC is configured to read
    ...
    async fn new() -> Self {
        ...
        // (7) Configure the RPC to read from the mock AiyNS package. Everything else is configured
    ...
    /// Send a JSON-RPC request to the cluster to resolve the given AiyNS name.
    ...
    /// Send a JSON-RPC request to the cluster to resolve the AiyNS name for a given address.
    async fn resolve_name(&self, addr: AiyAddress) -> anyhow::Result<Value> {
        let query = json!({
            "jsonrpc": "2.0",
            "id": 1,
            "method": "aiyx_resolveNameServiceNames",
            "params": [addr],
        });
