...
use aiy_indexer_alt_jsonrpc::config::{ObjectsConfig, RpcConfig as JsonRpcConfig};
use aiy_json_rpc_types::Page;
use aiy_types::{
...
/// Deserialized successful JSON-RPC response for `aiyx_getOwnedObjects`.
#[derive(Deserialize)]
struct Response {
...
/// `amount` to `owner`.
fn create_coin(cluster: &mut FullCluster, owner: AiyAddress, amount: u64) -> ObjectID {
...
/// isn't a coin.
fn create_bag(cluster: &mut FullCluster, owner: AiyAddress, size: u64) -> ObjectID {
    let (sender, kp, gas) = cluster
...
/// Make a call to `aiyx_getOwnedObjects` to the RPC server running on `cluster`.
