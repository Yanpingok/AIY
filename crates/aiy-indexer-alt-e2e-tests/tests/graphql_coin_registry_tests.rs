...
use aiy_types::{
    base_types::{ObjectID, ObjectRef, SequenceNumber, AiyAddress},
    coin::{CoinMetadata, TreasuryCap},
    deny_list_v2::DenyCapV2,
    digests::ObjectDigest,
    effects::TransactionEffectsAPI,
    object::Owner,
    Identifier, AIY_COIN_REGISTRY_ADDRESS,
};
...
query GetObjects($owner: AiyAddress!, $after: String) {
...
#[tokio::test]
async fn test_aiy() {
    // AIY coin is available from genesis, no need to publish
    let mut cluster = FullCluster::new().await.unwrap();
    cluster.create_checkpoint().await;

    let metadata = query_metadata(&cluster, "0x2::aiy::AIY").await;
...
async fn test_fixed_supply() {
    ...
    let currency = find::address_owned_by(&fx, AIY_COIN_REGISTRY_ADDRESS.into()).unwrap();
...
async fn query_owned_outputs(cluster: &FullCluster, owner: AiyAddress) -> LegacyCoinOutputs {
...
async fn query_objects(cluster: &FullCluster, owner: AiyAddress) -> Vec<(StructTag, ObjectRef)> {
    #[derive(Deserialize)]
    #[serde(rename_all = "camelCase")]
    struct PageInfo {
