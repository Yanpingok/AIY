...
use aiy_indexer_alt_jsonrpc::{
    args::SystemPackageTaskArgs, config::RpcConfig, start_rpc, NodeArgs, RpcArgs,
};
use aiy_indexer_alt_reader::bigtable_reader::BigtableArgs;
use aiy_macros::sim_test;
use aiy_pg_db::{temp::get_available_port, DbArgs};
use aiy_swarm_config::genesis_config::AccountConfig;
use aiy_test_transaction_builder::{make_publish_transaction, make_staking_transaction};
use aiy_types::{base_types::AiyAddress, transaction::TransactionDataAPI};
...
impl FnDelegationTestCluster {
    ...
    async fn transfer_transaction(&self) -> anyhow::Result<(String, String, Vec<String>)> {
        ...
        let tx = self
            ...
            .await
            .transfer_aiy(Some(1_000), recipient)
            .build();
    ...
    async fn privileged_transaction(&self) -> anyhow::Result<(String, String, Vec<String>)> {
        let tx: aiy_types::transaction::TransactionData = self
            .onchain_cluster
            .test_transaction_builder()
            .await
    ...
    async fn get_validator_address(&self) -> AiyAddress {
...
async fn test_execution() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "aiy_executeTransactionBlock".to_string(),
            json!({
                "tx_bytes": tx_bytes,
                "signatures": sigs,
                "options": {
...
async fn test_dry_run() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "aiy_dryRunTransactionBlock".to_string(),
            json!({
                "tx_bytes": tx_bytes,
            }),
        )
...
async fn test_get_all_balances() {
    ...
    assert_eq!(response["result"][0]["coinType"], "0x2::aiy::AIY");
    test_cluster.stopped().await;
}
...
async fn test_get_stakes_and_by_ids() {
    ...
    assert!(get_stakes_response["result"][0]["stakes"][0]["stakedAiyId"].is_string());
    let stake_id = get_stakes_response["result"][0]["stakes"][0]["stakedAiyId"]
        .as_str()
        .unwrap();
    ...
    let get_stakes_by_ids_response = test_cluster
        .execute_jsonrpc(
            "aiyx_getStakesByIds".to_string(),
            json!({ "staked_aiy_ids": [stake_id] }),
        )
        .await
        .unwrap();
...
async fn test_get_stakes_invalid_params() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "aiyx_getStakesByIds".to_string(),
            json!({ "staked_aiy_ids": ["invalid_stake_id"] }),
        )
        .await
        .unwrap();
...
async fn test_get_validators_apy() {
    ...
    let response = test_cluster
        .execute_jsonrpc("aiyx_getValidatorsApy".to_string(), json!({}))
        .await
        .unwrap();
...
async fn test_get_balance() {
    ...
    let coin_type = aiy_types::parse_aiy_struct_tag(&format!("{}::my_coin::MY_COIN", package_id))
        .unwrap()
        .to_string();
    let response = test_cluster
        .execute_jsonrpc(
            "aiyx_getBalance".to_string(),
    ...
    assert_eq!(response["result"]["coinType"], "0x2::aiy::AIY");
