...
use aiy_types::crypto::{
    enum_dispatch, EncodeDecodeBase64, PublicKey, Signature, SignatureScheme, AiyKeyPair,
};
...
pub trait AccountKeystore: Send + Sync {
    ...
    /// Import a keypair into the keystore from a `AiyKeyPair` and optional alias.
    async fn import(
        &mut self,
        alias: Option<String>,
        keypair: AiyKeyPair,
    ...
    async fn remove(&mut self, address: AiyAddress) -> Result<(), anyhow::Error>;
    /// Return an array of `PublicKey`, consisting of every public key in the keystore.
    fn entries(&self) -> Vec<PublicKey>;
    /// Return `AiyKeyPair` for the given address.
    fn export(&self, address: &AiyAddress) -> Result<&AiyKeyPair, anyhow::Error>;
    ...
    /// Return an array of `AiyAddress`, consisting of every address in the keystore.
    fn addresses(&self) -> Vec<AiyAddress> {
        self.entries().iter().map(|k| k.into()).collect()
    }
    /// Return an array of (&AiyAddress, &Alias), consisting of every address and its corresponding alias in the keystore.
    ...
    fn get_by_identity(&self, key_identity: &KeyIdentity) -> Result<AiyAddress, anyhow::Error> {
        match key_identity {
...
pub struct FileBasedKeystore {
    keys: BTreeMap<AiyAddress, AiyKeyPair>,
    aliases: BTreeMap<AiyAddress, Alias>,
    path: Option<PathBuf>,
}
...
impl AccountKeystore for FileBasedKeystore {
    ...
    async fn import(
        ...
        let address: AiyAddress = (&keypair.public()).into();
...
impl FileBasedKeystore {
        ...
        let aliases = if aliases_path.exists() {
            ...
            aliases
                ...
                .map(|alias| {
                    ...
                    key.map(|k| (Into::<AiyAddress>::into(&k), alias))
                })
                .collect::<Result<BTreeMap<_, _>, _>>()
                .map_err(|e| {
                    anyhow!(
    ...
    /// To see Bech32 format encoding, use `aiy keytool export $AIY_ADDRESS` where
    /// $AIY_ADDRESS can be found with `aiy keytool list`. Or use `aiy keytool convert $BASE64_STR`
    ...
    pub fn key_pairs(&self) -> Vec<&AiyKeyPair> {
...
impl InMemKeystore {
    pub fn new_insecure_for_tests(initial_key_number: usize) -> Self {
        ...
        let keys = (0..initial_key_number)
            ...
            .map(|(ad, k)| (ad, AiyKeyPair::Ed25519(k)))
            .collect::<BTreeMap<AiyAddress, AiyKeyPair>>();
...
mod tests {
    ...
    fn validate_alias_test() {
        ...
        assert!(validate_alias("abc_123.aiy").is_ok());
        // Not allowed
