...
use aiy_types::{
    ...
    crypto::{SignatureScheme, AiyKeyPair},
    error::AiyError,
};
...
) -> Result<(AiyAddress, AiyKeyPair), AiyError> {
    ...
    match key_scheme {
        SignatureScheme::ED25519 => {
            ...
            let sk = Ed25519PrivateKey::from_bytes(&derived)
                .map_err(|e| AiyError::SignatureKeyGenError(e.to_string()))?;
            let kp: Ed25519KeyPair = sk.into();
            Ok((kp.public().into(), AiyKeyPair::Ed25519(kp)))
        }
        SignatureScheme::Secp256k1 => {
            ...
            Ok((kp.public().into(), AiyKeyPair::Secp256k1(kp)))
        }
        SignatureScheme::Secp256r1 => {
            ...
            Ok((kp.public().into(), AiyKeyPair::Secp256r1(kp)))
        }
        SignatureScheme::BLS12381
        | SignatureScheme::MultiSig
        | SignatureScheme::ZkLoginAuthenticator
        | SignatureScheme::PasskeyAuthenticator => Err(AiyError::UnsupportedFeatureError {
...
) -> Result<DerivationPath, AiyError> {
    match key_scheme {
        SignatureScheme::ED25519 => {
            match path {
                Some(p) => {
                    ...
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                            ...
                            && address.is_hardened()
                        {
                            ...
                            Err(AiyError::SignatureKeyGenError("Invalid path".to_string()))
                        }
                    } else {
                        Err(AiyError::SignatureKeyGenError("Invalid path".to_string()))
                    }
                ...
                .parse()
                .map_err(|_| AiyError::SignatureKeyGenError("Cannot parse path".to_string()))?),
            }
        }
        SignatureScheme::Secp256k1 => {
            match path {
...
) -> Result<(AiyAddress, AiyKeyPair, SignatureScheme, String), anyhow::Error> {
