...
use aiy_types::crypto::{PublicKey, Signature, AiyKeyPair, AiySignature, AiySignatureInner};
...
pub struct External {
    ...
    pub aliases: BTreeMap<AiyAddress, Alias>,
    // Holds a map of addresses to [`StoredKey`]
    pub keys: BTreeMap<AiyAddress, StoredKey>,
...
pub struct StoredKey {
    ...
    /// External signer binary (e.g., "aiy-ledger-signer")
...
impl External {
    ...
    pub fn load_or_create(path: &PathBuf) -> Result<Self, Error> {
        ...
        let aliases: BTreeMap<AiyAddress, Alias> = if aliases_store_directory.exists() {
        ...
        let keys: BTreeMap<AiyAddress, StoredKey> = if path.exists() {
    ...
    /// Add a Key ID from the given an external signer to the Aiy CLI index
    ...
    pub fn is_indexed(&self, key: &StoredKey) -> bool {
        self.keys.contains_key(&AiyAddress::from(&key.public_key))
    }
...
impl AccountKeystore for External {
    async fn generate(
        ...
        let address: AiyAddress = (&public_key).into();
    ...
    async fn import(&mut self, _alias: Option<String>, _keypair: AiyKeyPair) -> Result<(), Error> {
        Err(anyhow!("Import not supported for external keys."))
    }

    async fn remove(&mut self, address: AiyAddress) -> Result<(), Error> {
    ...
    /// Export the key pair for the given address as a `AiyKeyPair`.
    fn export(&self, _address: &AiyAddress) -> Result<&AiyKeyPair, Error> {
    ...
    async fn sign_hashed(
        ...
        let address = AiyAddress::from(&public_key);
        match &result.signature {
            Signature::Ed25519AiySignature(s) => {
                ...
                let signature_address = AiyAddress::from(&pk);
            ...
            Signature::Secp256k1AiySignature(s) => {
                let (sig, pk) = s.get_verification_inputs().map_err(|e| {
                    signature::Error::from_source(anyhow!(
                        "Failed to get verification inputs: {}",
                        e
            ...
            Signature::Secp256r1AiySignature(s) => {
                let (sig, pk) = s.get_verification_inputs().map_err(|e| {
                    signature::Error::from_source(anyhow!(
                        "Failed to get verification inputs: {}",
                        e
    ...
    fn addresses_with_alias(&self) -> Vec<(&AiyAddress, &Alias)> {
    ...
    fn get_alias(&self, address: &AiyAddress) -> Result<String, anyhow::Error> {
...
mod tests {
    ...
    use std::str::FromStr;
    use aiy_types::base_types::AiyAddress;
    use aiy_types::crypto::SignatureScheme::{Secp256k1, ED25519};
    use aiy_types::crypto::{PublicKey, Signature, AiyKeyPair};
    ...
    async fn test_serialize() {
        ...
        external.keys.insert(
            AiyAddress::from_str(ADDRESS).unwrap(),
            StoredKey {
    ...
    async fn test_exec() {
        ...
        assert!(external
            .exec("aiy-key-tool", "test_method", args)
            .await
            .is_ok());
    }
    ...
    async fn test_create_key_success() {
        ...
        assert_eq!(address, AiyAddress::from_str(ADDRESS).unwrap());
    ...
    async fn test_add_existing_key() {
        ...
        let key = keys.get(&AiyAddress::from_str(ADDRESS).expect("Invalid address format"));
        assert!(key.is_some());
    }
    ...
    async fn test_add_keypair_not_supported() {
        ...
        let result = external
            .import(None, AiyKeyPair::Ed25519(ed25519_keypair))
            .await;
    ...
    async fn test_remove_key() {
        ...
        let address = AiyAddress::from_str(ADDRESS).unwrap();
        external.keys.insert(
            address,
            StoredKey {
                public_key: PublicKey::decode_base64(PUBLIC_KEY).unwrap(),
    ...
    fn test_get_key_fails() {
        ...
        let result = external.export(&AiyAddress::from_str(ADDRESS).unwrap());
        assert!(result.is_err())
    }

    #[tokio::test]
    async fn test_sign_hashed() {
        let skp = AiyKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([0; 32])));
