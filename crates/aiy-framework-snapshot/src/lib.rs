...
use aiy_framework::{SystemPackage, SystemPackageMetadata};
use aiy_protocol_config::ProtocolVersion;
use aiy_types::base_types::ObjectID;
use aiy_types::{
    BRIDGE_PACKAGE_ID, DEEPBOOK_PACKAGE_ID, MOVE_STDLIB_PACKAGE_ID, AIY_FRAMEWORK_PACKAGE_ID,
...
// [aiy_framework::{SystemPackageMetadata, SystemPackage}] types,
// and also to the [aiy::framework_versions::{FrameworkVersion, FrameworkPackage}] types.
// They are sort of a stepping stone from one to the other - the [aiy_framework] types contain
...
//
// These types serve as a kind of stepping stone; they are constructed from the [aiy_framework]
// types and serialized in the manifest, and then the build script for the [aiy] crate reads them
// from the manifest file and encodes them in the `aiy` binary. A little information is dropped in
// each of these steps.
...
pub struct SnapshotPackage {
    ...
    pub name: String,
    /// Path to the package in the monorepo (e.g. "crates/aiy-framework/packages/move-stdlib")
    pub path: String,
