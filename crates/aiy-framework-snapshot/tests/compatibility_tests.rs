...
mod compatibility_tests {
    ...
    use std::path::Path;
    use aiy_framework::{compare_system_package, BuiltInFramework};
    use aiy_framework_snapshot::{load_bytecode_snapshot, load_bytecode_snapshot_manifest};
    use aiy_move_build::published_at_property;
    ...
    async fn test_framework_compatibility() {
            ...
            .take(SNAPSHOT_BACKTEST_WINDOW)
        {
            ...
            for cur_package in BuiltInFramework::iter_system_packages() {
                ...
                {
                    panic!(
                        "The current Aiy framework {:?} is not compatible with version {:?}",
