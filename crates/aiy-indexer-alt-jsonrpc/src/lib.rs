...
use aiy_indexer_alt_reader::pg_reader::db::DbArgs;
use aiy_indexer_alt_reader::system_package_task::{SystemPackageTask, SystemPackageTaskArgs};
use aiy_open_rpc::Project;
...
impl RpcService {
    ...
    pub fn new(
        ...
        let schema = Project::new(
            ...
            "Aiy JSON-RPC",
            "A JSON-RPC API for interacting with the Aiy blockchain.",
            "Mysten Labs",
...
mod tests {
    ...
    async fn test_rpc_discovery() {
        ...
        assert_eq!(resp["result"]["info"]["title"], "Aiy JSON-RPC");
        assert_eq!(
            resp["result"]["methods"],
            json!([
                {
