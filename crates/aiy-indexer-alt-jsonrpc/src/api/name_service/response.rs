...
use aiy_indexer_alt_schema::schema::watermarks;
use aiy_name_service::{Domain, NameRecord, NameServiceError};
use aiy_types::{base_types::AiyAddress, dynamic_field::Field};
use tokio::join;
...
/// Attempt to translate the given AiyNS `name` to its address, as long as the mapping exists,
/// and it hasn't expired.
pub(super) async fn resolved_address(
    ctx: &Context,
    name: &str,
) -> Result<Option<AiyAddress>, RpcError<Error>> {
    use Error as E;
...
/// Attempt to translate the given `address` to its AiyNS name, as long as the reverse mapping
...
pub(super) async fn resolved_name(
    ...
    let reverse_record: Field<AiyAddress, Domain> = bcs::from_bytes(
