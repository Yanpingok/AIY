...
use aiy_json::{MoveTypeLayout, AiyJsonValue};
use aiy_json_rpc_types::{
    BcsName, DynamicFieldInfo as DynamicFieldInfoResponse, AiyMoveValue, AiyObjectDataOptions,
    AiyObjectResponse,
};
use aiy_types::{
    base_types::ObjectID,
...
pub(super) async fn dynamic_field_object(
...
) -> Result<AiyObjectResponse, RpcError<Error>> {
    let layout = resolve_type(ctx, &name.type_).await?;
    let bytes = AiyJsonValue::new(name.value)
    ...
    let options = AiyObjectDataOptions::full_content();
...
/// `aiy::dynamic_field::Field<K, V>`, and extract the name and value from it.
