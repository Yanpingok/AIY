...
use aiy_indexer_alt_reader::{
    kv_loader::TransactionContents, objects::VersionedObjectKey,
    tx_balance_changes::TxBalanceChangeKey,
};
use aiy_indexer_alt_schema::transactions::{BalanceChange, StoredTxBalanceChange};
use aiy_json_rpc_types::{
    BalanceChange as AiyBalanceChange, ObjectChange as AiyObjectChange, AiyEvent,
    AiyTransactionBlock, AiyTransactionBlockData, AiyTransactionBlockEffects,
    AiyTransactionBlockEvents, AiyTransactionBlockResponse, AiyTransactionBlockResponseOptions,
};
use aiy_types::{
...
pub(super) async fn transaction(
...
) -> Result<AiyTransactionBlockResponse, RpcError<Error>> {
    ...
    let mut response = AiyTransactionBlockResponse::new(digest);
...
) -> Result<AiyTransactionBlock, RpcError<Error>> {
    let data: TransactionData = tx.data()?;
    let tx_signatures: Vec<GenericSignature> = tx.signatures()?;

    Ok(AiyTransactionBlock {
        data: AiyTransactionBlockData::try_from_with_package_resolver(data, ctx.package_resolver())
            .await
...
fn effects(tx: &TransactionContents) -> Result<AiyTransactionBlockEffects, RpcError<Error>> {
...
async fn events(
    ...
    digest: TransactionDigest,
    tx: &TransactionContents,
) -> Result<AiyTransactionBlockEvents, RpcError<Error>> {
    let events: Vec<Event> = tx.events()?;
    let mut aiy_events = Vec::with_capacity(events.len());
    ...
    for (ix, event) in events.into_iter().enumerate() {
        ...
        let aiy_event =
            AiyEvent::try_from(event, digest, ix as u64, Some(tx.timestamp_ms()), layout)
                .with_context(|| format!("Failed to convert Event {ix} into response"))?;

        aiy_events.push(aiy_event)
    }

    Ok(AiyTransactionBlockEvents { data: aiy_events })
}

/// Extract the transaction's balance changes from their stored form.
fn balance_changes(
    balance_changes: StoredTxBalanceChange,
) -> Result<Vec<AiyBalanceChange>, RpcError<Error>> {
    ...
    } in balance_changes
    {
        ...
        response.push(AiyBalanceChange {
            owner,
            coin_type,
            amount,
        });
...
async fn object_changes(
    ...
    digest: TransactionDigest,
    tx: &TransactionContents,
) -> Result<Vec<AiyObjectChange>, RpcError<Error>> {
    ...
    for change in native_changes {
        ...
        changes.push(match (id_operation, input, output) {
            ...
            (ID::Created, _, Some((o, d))) if o.is_package() => AiyObjectChange::Published {
            ...
            (ID::Created, _, Some((o, d))) => AiyObjectChange::Created {
            ...
            (ID::None, Some((i, _)), Some((o, od))) => AiyObjectChange::Mutated {
                sender: tx_data.sender(),
                owner: o.owner().clone(),
                object_type: o
                    .struct_tag()
            ...
            (ID::None, Some((i, _)), None) => AiyObjectChange::Wrapped {
                sender: tx_data.sender(),
                object_type: i
                    .struct_tag()
                    .with_context(|| format!("No type for object {object_id}"))?,
            ...
            (ID::Deleted, Some((i, _)), None) => AiyObjectChange::Deleted {
                sender: tx_data.sender(),
                object_type: i
                    .struct_tag()
                    .with_context(|| format!("No type for object {object_id}"))?,
