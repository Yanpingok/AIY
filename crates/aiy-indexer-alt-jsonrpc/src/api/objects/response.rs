...
use aiy_indexer_alt_reader::displays::DisplayKey;
use aiy_json_rpc_types::{
    DisplayFieldsResponse, AiyData, AiyObjectData, AiyObjectDataOptions, AiyObjectResponse,
    AiyParsedData, AiyPastObjectResponse, AiyRawData,
};
use aiy_types::{
...
) -> Result<AiyObjectResponse, RpcError> {
    let Some(object) = load_live(ctx, object_id)
        .await
        .context("Failed to load latest object")?
    else {
        return Ok(AiyObjectResponse::new_with_error(
...
pub(super) async fn past_object(
    ...
    options: &AiyObjectDataOptions,
) -> Result<AiyPastObjectResponse, RpcError> {
    let Some(object) = ctx
        .kv_loader()
        ...
        return Ok(AiyPastObjectResponse::VersionNotFound(object_id, version));
    };
...
pub(crate) async fn object_data_with_options(
    ...
    let content: OptionFuture<_> = options
        .show_content
        .then(|| object_data::<AiyParsedData>(ctx, &object))
        .into();

    let bcs: OptionFuture<_> = options
        .show_bcs
        .then(|| object_data::<AiyRawData>(ctx, &object))
        .into();
...
async fn object_data<D: AiyData>(ctx: &Context, object: &Object) -> Result<D, RpcError> {
...
async fn display(ctx: &Context, object: &Object) -> DisplayFieldsResponse {
    ...
    DisplayFieldsResponse {
        ...
        error: (!field_errors.is_empty()).then_some(AiyObjectResponseError::DisplayError {
            error: field_errors,
        }),
    }
}
