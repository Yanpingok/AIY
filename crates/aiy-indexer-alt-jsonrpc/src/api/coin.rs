...
use aiy_indexer_alt_reader::coin_metadata::CoinMetadataKey;
use aiy_indexer_alt_schema::objects::StoredCoinOwnerKind;
use aiy_indexer_alt_schema::schema::coin_balance_buckets;
use aiy_json_rpc_types::{Balance, Coin, Page as PageResponse, AiyCoinMetadata};
use aiy_open_rpc::Module;
use aiy_open_rpc_macros::open_rpc;
use aiy_sql_macro::sql;
...
#[open_rpc(namespace = "aiyx", tag = "Delegation Coin API")]
...
trait DelegationCoinsApi {
    ...
    /// If no coin type is specified, AIY coin balance is returned.
...
impl CoinsApiServer for Coins {
    async fn get_coins(
        ...
        let coin_type_tag = if let Some(coin_type) = coin_type {
            aiy_types::parse_aiy_type_tag(&coin_type)
...
async fn coin_registry_response(
    ctx: &Context,
    coin_type: &str,
) -> Result<Option<AiyCoinMetadata>, RpcError<Error>> {
