...
use aiy_node::{AiyNode, AiyNodeHandle};
...
pub(crate) struct Container {
    ...
    node_watch: watch::Receiver<Weak<AiyNode>>,
}
...
impl Drop for Container {
    fn drop(&mut self) {
        if let Some(handle) = self.handle.take() {
            ...
            aiy_simulator::runtime::Handle::try_current().map(|h| h.delete_node(handle.node_id));
        }
    }
}
...
impl Container {
        ...
        let node = builder
            ...
            .init(move || {
                ...
                async move {
                    ...
                    let server = AiyNode::start(config, registry_service).await.unwrap();
    ...
    /// Get a AiyNodeHandle to the node owned by the container.
    pub fn get_node_handle(&self) -> Option<AiyNodeHandle> {
        Some(AiyNodeHandle::new(self.node_watch.borrow().upgrade()?))
    }
