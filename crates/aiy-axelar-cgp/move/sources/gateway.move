...
///
/// - When call approvals is sent to Aiy, it targets an object and not a module;
/// - To support cross-chain messaging, a Channel object has to be created;
/// - Channel can be either owned or shared but not frozen;
/// - Module developer on the Aiy side will have to implement a system to support messaging;
...
module axelar::gateway {
    ...
    use axelar::utils::to_aiy_signed;
    ...
    /// For when approval chainId is not AIY.
    ...
    /// Emitted when a new message is sent from the AIY network.
    ...
    entry fun process_commands(
        ...
        let mut allow_operatorship_transfer = validate_proof(validators, to_aiy_signed(*&data), proof);
    ...
    fun test_execute() {
        use aiy::test_scenario::{Self as ts, ctx};
