...
use crate::{
    ...
    types::query::{Query, AiyGraphQLSchema},
};
...
use aiy_graphql_rpc_headers::LIMITS_HEADER;
use aiy_indexer::db::check_db_migration_consistency;
...
impl ServerBuilder {
    ...
    pub async fn from_config(
        ...
        let aiy_sdk_client = if let Some(url) = &config.tx_exec_full_node.node_rpc_url {
            Some(
                AiyClientBuilder::default()
                    ...
                    .build(url)
                    .await
                    .map_err(|e| Error::Internal(format!("Failed to create AiyClient: {}", e)))?,
            )
        } else {
...
async fn graphql_handler(
    ...
    schema: Extension<AiyGraphQLSchema>,
...
pub mod tests {
    ...
    use std::sync::Arc;
    use std::time::Duration;
    use aiy_pg_db::temp::get_available_port;
    use aiy_sdk::AiyClient;
    use aiy_types::digests::get_mainnet_chain_identifier;
    ...
    async fn test_timeout() {
        ...
        cluster
            ...
            .await;
        // timeout test includes mutation timeout, which requires a [AiyClient] to be able to run
        ...
        async fn test_timeout(
            ...
            let schema = prep_schema(db_url, Some(cfg))
                .await
                .context_data(Some(aiy_client.clone()))
                .extension(Timeout)
        ...
        let aiy_client = wallet.get_client().await.unwrap();

        test_timeout(delay, timeout, query, &aiy_client, db_url.clone())
            .await
            .into_result()
            .expect("Should complete successfully");

        // Should timeout
        let errs: Vec<_> = test_timeout(delay, delay, query, &aiy_client, db_url.clone())
            .await
            .into_result()
            .unwrap_err()
            .into_iter()
