...
use crate::{
    error::Error,
    types::{address::Address, aiy_address::AiyAddress, validator::Validator},
};
use std::{collections::BTreeMap, time::Duration};
use aiy_indexer::db::ConnectionPoolConfig;
use aiy_indexer::{apis::GovernanceReadApi, indexer_reader::IndexerReader};
use aiy_json_rpc_types::Stake as RpcStakedAiy;
use aiy_types::{
    governance::StakedAiy as NativeStakedAiy,
    aiy_system_state::aiy_system_state_summary::AiySystemStateSummary as NativeAiySystemStateSummary,
};
...
impl PgManager {
    ...
    /// returns the latest aiy system state.
    pub(crate) async fn fetch_aiy_system_state(
        &self,
        epoch_id: Option<u64>,
    ) -> Result<NativeAiySystemStateSummary, Error> {
        let latest_aiy_system_state = self.inner.get_latest_aiy_system_state().await?;

        if let Some(epoch_id) = epoch_id {
            if epoch_id == latest_aiy_system_state.epoch {
                Ok(latest_aiy_system_state)
            } else {
                Ok(self
                    .inner
                    .get_epoch_aiy_system_state(Some(epoch_id))
                    .await?)
            }
        } else {
            Ok(latest_aiy_system_state)
    ...
    /// Make a request to the RPC for its representations of the staked aiy we parsed out of the
    /// object.  Used to implement fields that are implemented in JSON-RPC but not GraphQL (yet).
    pub(crate) async fn fetch_rpc_staked_aiy(
        &self,
        stake: NativeStakedAiy,
    ) -> Result<RpcStakedAiy, Error> {
...
/// `AiyValidatorSummary` was queried for. Each `Validator` will inherit this checkpoint, so that
/// when viewing the `Validator`'s state, it will be as if it was read at the same checkpoint.
pub(crate) fn convert_to_validators(
    system_state_at_requested_epoch: NativeAiySystemStateSummary,
    ...
    system_state_at_requested_epoch
        ...
        .map(move |validator_summary| {
            let at_risk = at_risk.get(&validator_summary.aiy_address).copied();
            let report_records = reports.get(&validator_summary.aiy_address).map(|addrs| {
                addrs
                    .iter()
                    .cloned()
                    .map(|a| Address {
                        address: AiyAddress::from(a),
