...
use aiy_name_service::{validate_label, Domain};
use aiy_types::{
...
pub(crate) struct AppInfo {
    ...
    pub(crate) package_address: Option<AiyAddress>,
...
impl Name {
    ...
    pub(crate) fn type_(package_address: AiyAddress) -> StructTag {
        StructTag {
    ...
    pub(crate) fn to_dynamic_field_id(
        ...
        aiy_types::dynamic_field::derive_dynamic_field_id(
            config.registry_id,
            &aiy_types::TypeTag::Struct(Box::new(domain_type_tag)),
            &self.to_bytes(),
        )
    }
}
