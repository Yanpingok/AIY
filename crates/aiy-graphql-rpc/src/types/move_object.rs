...
use super::stake::StakedAiyDowncastError;
use super::aiy_address::AiyAddress;
use super::aiyns_registration::{DomainFormat, AiynsRegistration, AiynsRegistrationDowncastError};
...
use crate::types::stake::StakedAiy;
use async_graphql::connection::Connection;
use async_graphql::*;
use aiy_name_service::NameServiceConfig;
use aiy_types::object::{Data, MoveObject as NativeMoveObject};
...
#[graphql(
    ...
    field(
        ...
        desc = "Determines whether a transaction can transfer this object, using the \
                TransferObjects transaction command or `aiy::transfer::public_transfer`, both of \
...
pub(crate) enum IMoveObject {
    ...
    StakedAiy(StakedAiy),
    AiynsRegistration(AiynsRegistration),
}
...
impl MoveObject {
    ...
    /// Attempts to convert the Move object into a `0x3::staking_pool::StakedAiy`.
    async fn as_staked_aiy(&self) -> Result<Option<StakedAiy>> {
        match StakedAiy::try_from(self) {
            Ok(coin) => Ok(Some(coin)),
            Err(StakedAiyDowncastError::NotAStakedAiy) => Ok(None),
            Err(StakedAiyDowncastError::Bcs(e)) => Err(Error::Internal(format!(
                "Failed to deserialize StakedAiy: {e}"
            )))
            .extend(),
        }
    }
    ...
    /// Attempts to convert the Move object into a `AiynsRegistration` object.
    async fn as_aiyns_registration(&self, ctx: &Context<'_>) -> Result<Option<AiynsRegistration>> {
        let cfg: &NameServiceConfig = ctx.data_unchecked();
        let tag = AiynsRegistration::type_(cfg.package_address.into());

        match AiynsRegistration::try_from(self, &tag) {
            Ok(registration) => Ok(Some(registration)),
            Err(AiynsRegistrationDowncastError::NotAAiynsRegistration) => Ok(None),
            Err(AiynsRegistrationDowncastError::Bcs(e)) => Err(Error::Internal(format!(
                "Failed to deserialize AiynsRegistration: {e}",
            )))
            .extend(),
        }
    }
