...
use aiy_types::dynamic_field::visitor::{Field, FieldVisitor};
use aiy_types::dynamic_field::{derive_dynamic_field_id, DynamicFieldInfo, DynamicFieldType};
...
use super::{
    base64::Base64, move_object::MoveObject, move_value::MoveValue, aiy_address::AiyAddress,
};
...
/// 2) Dynamic Object Fields values must be Aiy objects (have the `key` and `store`
...
impl DynamicField {
    ...
    async fn value(&self, ctx: &Context<'_>) -> Result<Option<DynamicFieldValue>> {
        ...
        if kind == DynamicFieldType::DynamicObject {
            let df_object_id: AiyAddress = bcs::from_bytes(value_bytes)
...
fn apply_filter(query: RawQuery, parent: AiyAddress, parent_version: Option<u64>) -> RawQuery {
    let query = filter!(
        query,
        format!(
