...
use aiy_indexer::apis::GovernanceReadApi;
use aiy_types::committee::EpochId;
use aiy_types::aiy_system_state::PoolTokenExchangeRate;

use aiy_types::base_types::AiyAddress as NativeAiyAddress;
...
use async_graphql::*;
use aiy_indexer::apis::governance_api::exchange_rates;
use aiy_types::aiy_system_state::aiy_system_state_summary::AiyValidatorSummary as NativeAiyValidatorSummary;
#[derive(Clone, Debug)]
pub(crate) struct Validator {
    pub validator_summary: NativeAiyValidatorSummary,
...
impl Loader<u64> for Db {
    type Value = (
        ...
        BTreeMap<NativeAiyAddress, Vec<(EpochId, PoolTokenExchangeRate)>>,
    );
    type Error = Error;
    ...
    async fn load(
        ...
        Error,
    > {
        let latest_aiy_system_state = self
            .inner
            .get_latest_aiy_system_state()
            .await
            .map_err(|_| Error::Internal("Failed to fetch latest Aiy system state".to_string()))?;
        let governance_api = GovernanceReadApi::new(self.inner.clone());
        let exchange_rates = exchange_rates(&governance_api, &latest_aiy_system_state)
            .await
        ...
        let epoch_to_filter_out = if let Some(epoch) = keys.first() {
            if epoch == &latest_aiy_system_state.epoch {
                *epoch - 1
            } else {
                *epoch
            }
        } else {
            latest_aiy_system_state.epoch - 1
        };
        ...
        let requested_epoch = match keys.first() {
            Some(x) => *x,
            None => latest_aiy_system_state.epoch,
        };

        let mut r = HashMap::new();
        r.insert(
            requested_epoch,
            (latest_aiy_system_state.stake_subsidy_start_epoch, results),
        );
...
impl Validator {
    ...
    async fn address(&self) -> Address {
        Address {
            address: AiyAddress::from(self.validator_summary.aiy_address),
    ...
    async fn staking_pool_id(&self) -> AiyAddress {
    ...
    /// the amount of AIY tokens that each past AIY staker can withdraw in the future.
    #[graphql(
    ...
    /// of AIY tokens that each past AIY staker can withdraw in the future.
    ...
    /// The total number of AIY tokens in this pool.
    async fn staking_pool_aiy_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.staking_pool_aiy_balance,
        ))
    }
    ...
    async fn pending_total_aiy_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.pending_total_aiy_withdraw,
        ))
    }
    ...
    /// The total number of AIY tokens in this pool plus
    ...
    async fn apy(&self, ctx: &Context<'_>) -> Result<Option<u64>, Error> {
        ...
        let rates = exchange_rates
            .get(&self.validator_summary.aiy_address)
            .ok_or_else(|| {
                Error::Internal(format!(
                    "Failed to get the exchange rate for this validator address {} for requested epoch {}",
                    self.validator_summary.aiy_address, self.requested_for_epoch
                ))
            })?;
...
impl Validator {
    pub fn operation_cap_id(&self) -> AiyAddress {
        AiyAddress::from_array(**self.validator_summary.operation_cap_id)
    }
}
