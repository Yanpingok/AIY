...
use aiy_indexer::models::objects::StoredHistoryObject;
use aiy_name_service::{Domain as NativeDomain, NameRecord, NameServiceConfig, NameServiceError};
use aiy_types::{base_types::AiyAddress as NativeAiyAddress, dynamic_field::Field, id::UID};

const MOD_REGISTRATION: &IdentStr = ident_str!("aiyns_registration");
const TYP_REGISTRATION: &IdentStr = ident_str!("AiynsRegistration");
...
/// Wrap AiyNS Domain type to expose as a string scalar in GraphQL.
#[derive(Debug)]
pub(crate) struct Domain(NativeDomain);

#[derive(Enum, Copy, Clone, Eq, PartialEq)]
#[graphql(remote = "aiy_name_service::DomainFormat")]
...
pub(crate) struct NativeAiynsRegistration {
    pub id: UID,
...
#[derive(Clone)]
pub(crate) struct AiynsRegistration {
    /// Representation of this AiynsRegistration as a generic Move object.
    pub super_: MoveObject,

    /// The deserialized representation of the Move object's contents.
    pub native: NativeAiynsRegistration,
}
...
pub(crate) enum AiynsRegistrationDowncastError {
    NotAAiynsRegistration,
    Bcs(bcs::Error),
}

#[Object]
impl AiynsRegistration {
    pub(crate) async fn address(&self) -> AiyAddress {
    ...
    ///
    ///`type` is a filter on the coin's type parameter, defaulting to `0x2::aiy::AIY`.
    ...
    /// The `0x3::staking_pool::StakedAiy` objects owned by this object.
    pub(crate) async fn staked_aiys(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
        after: Option<object::Cursor>,
        last: Option<u64>,
        before: Option<object::Cursor>,
    ) -> Result<Connection<String, StakedAiy>> {
        OwnerImpl::from(&self.super_.super_)
            .staked_aiys(ctx, first, after, last, before)
            .await
    }

    /// The domain explicitly configured as the default domain pointing to this object.
    pub(crate) async fn default_aiyns_name(
        &self,
        ctx: &Context<'_>,
        format: Option<DomainFormat>,
    ) -> Result<Option<String>> {
        OwnerImpl::from(&self.super_.super_)
            .default_aiyns_name(ctx, format)
            .await
    }

    /// The AiynsRegistration NFTs owned by this object. These grant the owner the capability to
    /// manage the associated domain.
    pub(crate) async fn aiyns_registrations(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
        after: Option<object::Cursor>,
        last: Option<u64>,
        before: Option<object::Cursor>,
    ) -> Result<Connection<String, AiynsRegistration>> {
        OwnerImpl::from(&self.super_.super_)
            .aiyns_registrations(ctx, first, after, last, before)
            .await
    }
    ...
    /// The amount of AIY we would rebate if this object gets deleted or mutated. This number is
    ...
    /// transaction command or `aiy::transfer::public_transfer`, both of which require the object to
    ...
    /// Domain name of the AiynsRegistration object
...
impl NameService {
    /// Lookup the AiyNS NameRecord for the given `domain` name. `config` specifies where to find
    ...
    /// Lookup the AiyNS Domain for the given `address`. `config` specifies where to find the domain
    ...
    pub(crate) async fn reverse_resolve_to_name(
        ...
        let field: Field<NativeAiyAddress, NativeDomain> = object
            .native
            .to_rust()
            .ok_or_else(|| Error::Internal("Malformed Aiyns Domain".to_string()))?;
    ...
    async fn query_domain_expiration(
        ...
        // parent's `NameRecord`.
        let mut object_ids = vec![AiyAddress::from(config.record_field_id(&domain.0))];
        if domain.0.is_subdomain() {
            object_ids.push(AiyAddress::from(config.record_field_id(&domain.0.parent())));
        }
...
impl AiynsRegistration {
    /// Query the database for a `page` of AiyNS registrations. The page uses the same cursor type
    ...
    pub(crate) async fn paginate(
        ...
        owner: AiyAddress,
        checkpoint_viewed_at: u64,
    ) -> Result<Connection<String, AiynsRegistration>, Error> {
        let type_ = AiynsRegistration::type_(config.package_address.into());
        ...
        Object::paginate_subtype(db, page, filter, checkpoint_viewed_at, |object| {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a AiynsRegistration, but it's not a Move Object.",
                ))
            })?;

            AiynsRegistration::try_from(&move_object, &type_).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a AiynsRegistration, but it is not."
                ))
            })
        })
        .await
    ...
    /// Return the type representing a `AiynsRegistration` on chain. This can change from chain to
    /// chain (mainnet, testnet, devnet etc).
    pub(crate) fn type_(package: AiyAddress) -> StructTag {
        StructTag {
    ...
    // Because the type of the AiynsRegistration object is not constant,
    // we need to take it in as a param.
    pub(crate) fn try_from(
        move_object: &MoveObject,
        tag: &StructTag,
    ) -> Result<Self, AiynsRegistrationDowncastError> {
        if !move_object.native.is_type(tag) {
            return Err(AiynsRegistrationDowncastError::NotAAiynsRegistration);
        }

        Ok(Self {
            super_: move_object.clone(),
            native: bcs::from_bytes(move_object.native.contents())
                .map_err(AiynsRegistrationDowncastError::Bcs)?,
        })
    }
}
