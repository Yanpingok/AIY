...
use crate::types::aiy_address::addr;
...
use aiy_indexer::models::objects::StoredFullHistoryObject;
use aiy_indexer::schema::packages;
use aiy_package_resolver::{error::Error as PackageCacheError, Package as ParsedMovePackage};
use aiy_types::is_system_package;
use aiy_types::{move_package::MovePackage as NativeMovePackage, object::Data};
...
impl MovePackage {
    ...
    /// The `0x3::staking_pool::StakedAiy` objects owned by this package.
    ///
    /// Note that objects owned by a package are inaccessible, because packages are immutable and
    /// cannot be owned by an address.
    ...
    /// The AiynsRegistration NFTs owned by this package. These grant the owner the capability to
...
impl MovePackage {
    ...
    pub(crate) fn try_from_serialized(
        ...
        let object = Object::new_serialized(
            AiyAddress::from_bytes(&history_object.object_id)
...
impl Loader<PackageVersionKey> for Db {
    type Value = AiyAddress;
    type Error = Error;

    async fn load(
        &self,
        keys: &[PackageVersionKey],
    ) -> Result<HashMap<PackageVersionKey, AiyAddress>, Error> {
        use packages::dsl;
...
impl Loader<LatestKey> for Db {
    ...
    async fn load(&self, keys: &[LatestKey]) -> Result<HashMap<LatestKey, AiyAddress>, Error> {
        use packages::dsl;
        let other = diesel::alias!(packages as other);
