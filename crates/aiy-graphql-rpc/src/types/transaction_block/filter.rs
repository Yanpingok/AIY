...
use crate::types::{digest::Digest, aiy_address::AiyAddress, type_filter::FqNameFilter};
...
pub(crate) struct TransactionBlockFilter {
    ...
    pub affected_address: Option<AiyAddress>,
    ...
    #[cfg(feature = "staging")]
    pub affected_object: Option<AiyAddress>,

    /// Limit to transactions that were sent by the given address.
    pub sent_address: Option<AiyAddress>,
    ...
    pub input_object: Option<AiyAddress>,
    ...
    /// This filter will be removed with 1.36.0 (2024-10-14), or at least one release after
    /// `affectedObject` is introduced, whichever is later.
    pub changed_object: Option<AiyAddress>,
...
impl TransactionBlockFilter {
    ...
    pub(crate) fn explicit_sender(&self) -> Option<AiyAddress> {
            ...
            || matches!(
                ...
                (Some(kind), Some(signer))
                    if (kind == TransactionBlockKindInput::SystemTx)
                        != (signer == AiyAddress::from(NativeAiyAddress::ZERO))
            )
    }
}
