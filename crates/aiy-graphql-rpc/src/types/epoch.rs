...
use aiy_indexer::models::epoch::QueryableEpochInfo;
use aiy_indexer::schema::epochs;
use aiy_types::messages_checkpoint::CheckpointCommitment as EpochCommitment;
...
/// Operation of the Aiy network is temporally partitioned into non-overlapping epochs,
...
impl Epoch {
    ...
    async fn validator_set(&self, ctx: &Context<'_>) -> Result<Option<ValidatorSet>> {
        let system_state = ctx
            ...
            .fetch_aiy_system_state(Some(self.stored.epoch as u64))
            .await?;
