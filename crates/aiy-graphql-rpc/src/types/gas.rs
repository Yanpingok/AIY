...
use super::{address::Address, big_int::BigInt, object::Object, aiy_address::AiyAddress};
...
impl GasInput {
    ...
    pub(crate) fn from(s: &GasData, checkpoint_viewed_at: u64) -> Self {
        let payment_obj_keys = match s.owner {
            NativeAiyAddress::ZERO => vec![], // system transactions do not have payment objects
            _ => s
                .payment
                .iter()
                .map(|o| ObjectKey {
