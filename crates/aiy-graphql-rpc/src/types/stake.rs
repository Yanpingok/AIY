...
use super::aiyns_registration::{DomainFormat, AiynsRegistration};
use super::transaction_block::{self, TransactionBlock, TransactionBlockFilter};
use super::type_filter::ExactTypeFilter;
use super::uint53::UInt53;
use super::{
    big_int::BigInt, epoch::Epoch, move_object::MoveObject, object, aiy_address::AiyAddress,
};
use async_graphql::connection::Connection;
use async_graphql::*;
use move_core_types::language_storage::StructTag;
use aiy_json_rpc_types::{Stake as RpcStakedAiy, StakeStatus as RpcStakeStatus};
use aiy_types::base_types::MoveObjectType;
use aiy_types::governance::StakedAiy as NativeStakedAiy;
...
pub(crate) enum StakedAiyDowncastError {
    NotAStakedAiy,
    Bcs(bcs::Error),
}

#[derive(Clone)]
pub(crate) struct StakedAiy {
    /// Representation of this StakedAiy as a generic Move Object.
    pub super_: MoveObject,

    /// Deserialized representation of the Move Object's contents as a
    /// `0x3::staking_pool::StakedAiy`.
    pub native: NativeStakedAiy,
}

/// Represents a `0x3::staking_pool::StakedAiy` Move object on-chain.
#[Object]
impl StakedAiy {
    ...
    async fn pool_id(&self) -> Option<AiyAddress> {
        Some(self.native.pool_id().into())
    }

    /// The AIY that was initially staked.
...
impl StakedAiy {
    /// Query the database for a `page` of Staked AIY. The page uses the same cursor type as is used
    ...
    ) -> Result<Connection<String, StakedAiy>, Error> {
        let type_: StructTag = MoveObjectType::staked_aiy().into();
        ...
        Object::paginate_subtype(db, page, filter, checkpoint_viewed_at, |object| {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a StakedAiy, but it's not a Move Object.",
                ))
            })?;

            StakedAiy::try_from(&move_object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a StakedAiy, but it is not."
                ))
            })
        })
        .await
    ...
    /// The JSON-RPC representation of a StakedAiy so that we can "cheat" to implement fields that
    /// are not yet implemented directly for GraphQL.
    ///
    /// TODO: Make this obsolete
    async fn rpc_stake(&self, ctx: &Context<'_>) -> Result<RpcStakedAiy, Error> {
        ctx.data_unchecked::<PgManager>()
            .fetch_rpc_staked_aiy(self.native.clone())
            .await
    }
}

impl TryFrom<&MoveObject> for StakedAiy {
    type Error = StakedAiyDowncastError;

    fn try_from(move_object: &MoveObject) -> Result<Self, Self::Error> {
        if !move_object.native.is_staked_aiy() {
            return Err(StakedAiyDowncastError::NotAStakedAiy);
        }

        Ok(Self {
            super_: move_object.clone(),
            native: bcs::from_bytes(move_object.native.contents())
                .map_err(StakedAiyDowncastError::Bcs)?,
        })
    }
}
