...
use super::stake::StakedAiy;
use super::aiy_address::addr;
...
use super::{owner::Owner, aiy_address::AiyAddress, transaction_block::TransactionBlock};
...
use aiy_indexer::models::obj_indices::StoredObjectVersion;
use aiy_indexer::models::objects::{StoredFullHistoryObject, StoredHistoryObject};
use aiy_indexer::schema::{full_objects_history, objects_version};
use aiy_indexer::types::ObjectStatus as NativeObjectStatus;
use aiy_indexer::types::OwnerType;
use aiy_types::object::{Object as NativeObject, Owner as NativeOwner};
...
pub(crate) struct ObjectFilter {
    ...
    /// the full type name, such as `0x2::coin::Coin<0x2::aiy::AIY>`.
    pub type_: Option<TypeFilter>,

    /// Filter for live objects by their current owners.
    pub owner: Option<AiyAddress>,

    /// Filter for live objects by their IDs.
    pub object_ids: Option<Vec<AiyAddress>>,
}

#[derive(InputObject, Debug, Clone, Eq, PartialEq)]
pub(crate) struct ObjectKey {
    pub object_id: AiyAddress,
    pub version: UInt53,
}
...
/// An object in Aiy is a package (set of Move bytecode modules) or object (typed data structure
...
impl Object {
    ...
    pub(crate) async fn default_aiyns_name(
        ...
        OwnerImpl::from(self).default_aiyns_name(ctx, format).await
    }
    ...
    pub(crate) async fn owner(&self) -> Option<ObjectOwner> {
        ...
        match &native.owner {
            O::AddressOwner(address) => {
                let address = AiyAddress::from(*address);
...
mod tests {
    ...
    fn test_owner_filter_intersection() {
        let f0 = ObjectFilter {
            owner: Some(AiyAddress::from_str("0x1").unwrap()),
            ..Default::default()
        };

        let f1 = ObjectFilter {
            owner: Some(AiyAddress::from_str("0x2").unwrap()),
            ..Default::default()
        };
    ...
    fn test_object_filter_intersection() {
        let i1 = AiyAddress::from_str("0x1").unwrap();
        let i2 = AiyAddress::from_str("0x2").unwrap();
        let i3 = AiyAddress::from_str("0x3").unwrap();
