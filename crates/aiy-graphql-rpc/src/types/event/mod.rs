...
use aiy_indexer::models::{events::StoredEvent, transactions::StoredTransaction};
use aiy_indexer::schema::{checkpoints, events};
use aiy_types::base_types::ObjectID;
use aiy_types::Identifier;
use aiy_types::{
    base_types::AiyAddress as NativeAiyAddress, event::Event as NativeEvent, parse_aiy_struct_tag,
};
...
/// A Aiy node emits one of the following events:
/// Move event
/// Publish event
...
impl Event {
    ...
    async fn sender(&self) -> Result<Option<Address>> {
        if self.native.sender == NativeAiyAddress::ZERO {
            return Ok(None);
        }
...
impl Event {
    ...
    fn try_from_stored_event(
        ...
        let sender = NativeAiyAddress::from_bytes(sender_bytes)
            .map_err(|e| Error::Internal(e.to_string()))?;
        let package_id =
            ObjectID::from_bytes(&stored.package).map_err(|e| Error::Internal(e.to_string()))?;
        let type_ =
            parse_aiy_struct_tag(&stored.event_type).map_err(|e| Error::Internal(e.to_string()))?;
