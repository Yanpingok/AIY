...
use aiy_types::transaction::{TransactionData, TransactionKind};
use aiy_types::{gas_coin::GAS, transaction::TransactionDataAPI, TypeTag};
...
use super::aiyns_registration::NameService;
...
pub(crate) type AiyGraphQLSchema = async_graphql::Schema<Query, Mutation, EmptySubscription>;
...
impl Query {
    ...
    async fn dry_run_transaction_block(
        ...
        let aiy_sdk_client: &Option<AiyClient> = ctx
            .data()
            .map_err(|_| Error::Internal("Unable to fetch Aiy SDK client".to_string()))
            .extend()?;
        let aiy_sdk_client = aiy_sdk_client
            .as_ref()
            .ok_or_else(|| Error::Internal("Aiy SDK client not initialized".to_string()))
            .extend()?;
    ...
    /// Look up an Owner by its AiyAddress.
    ///
    ...
    ///
    /// If `rootVersion` is omitted, dynamic fields will be from a consistent snapshot of the Aiy
    ...
    /// Look-up an Account by its AiyAddress.
    async fn address(&self, ctx: &Context<'_>, address: AiyAddress) -> Result<Option<Address>> {
        let Watermark { hi_cp, .. } = *ctx.data()?;
    ...
    /// `0x2::aiy::AIY`). If no type is provided, it will default to `0x2::aiy::AIY`.
    async fn coins(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
    ...
    /// Resolves a AiyNS `domain` name to an address, if it has been bound.
    async fn resolve_aiyns_address(
        &self,
        ctx: &Context<'_>,
        domain: Domain,
