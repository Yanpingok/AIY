...
impl<M: MetricsCallbackProvider> ServerBuilder<M> {
    ...
    pub async fn bind(self, addr: &Multiaddr, tls_config: Option<ServerConfig>) -> Result<Server> {
        ...
        let mut builder = aiy_http::Builder::new().config(http_config);
...
/// TLS server name to use for the public Aiy validator interface.
pub const AIY_TLS_SERVER_NAME: &str = "aiy";

pub struct Server {
    server: aiy_http::ServerHandle,
...
impl Server {
    ...
    pub fn handle(&self) -> &aiy_http::ServerHandle {
        &self.server
    }
}
...
mod test {
    ...
    async fn test_metrics_layer_successful() {
        ...
        let server = super::ServerBuilder::from_config(&config, metrics.clone())
            .bind(
                &address,
                Some(aiy_tls::create_rustls_server_config(
