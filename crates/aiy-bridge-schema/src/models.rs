...
use aiy_field_count::FieldCount;
use aiy_indexer_builder::{Task, LIVE_TASK_TARGET_CHECKPOINT};

use crate::schema::{
    governance_actions, progress_store, aiy_error_transactions, aiy_progress_store, token_transfer,
    token_transfer_data,
};
...
#[derive(Queryable, Selectable, Insertable, Identifiable, Debug)]
#[diesel(table_name = aiy_progress_store, primary_key(txn_digest))]
pub struct AiyProgressStore {
...
#[diesel(table_name = aiy_error_transactions, primary_key(txn_digest))]
pub struct AiyErrorTransactions {
    pub txn_digest: Vec<u8>,
    pub sender_address: Vec<u8>,
    pub timestamp_ms: i64,
