...
use aiy_types::base_types::{AuthorityName, TransactionDigest};
use aiy_types::transaction::VerifiedSignedTransaction;
use tokio::select;
...
mod tests {
    ...
    use aiy_types::base_types::{AuthorityName, ObjectID, AiyAddress, TransactionDigest};
    use aiy_types::committee::{CommitteeTrait, StakeUnit};
    use aiy_types::crypto::{get_account_key_pair, AccountKeyPair};
    use aiy_types::effects::{TransactionEffectsAPI, TransactionEvents};
    ...
    impl AuthorityAPI for MockAuthorityClient {
        ...
        async fn handle_transaction(
            ...
            if self.inject_fault.load(Relaxed) {
                return Err(AiyError::TimeoutError);
            }
