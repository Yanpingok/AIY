...
use aiy_types::{
    ...
    aiy_system_state::AiySystemState,
};
...
impl AuthorityAPI for LocalAuthorityClient {
    async fn submit_transaction(
    ...
    ) -> Result<SubmitTxResponse, AiyError> {
        if self.fault_config.fail_before_submit_transaction {
            return Err(AiyError::from("Mock error before submit_transaction"));
        }
        ...
        let deserialized_transaction = bcs::from_bytes::<Transaction>(&raw_request.transactions[0])
            .map_err(|e| AiyError::TransactionDeserializationError {
                error: e.to_string(),
            })?;
        ...
        if self.fault_config.fail_after_vote_transaction {
            return Err(AiyError::GenericAuthorityError {
    ...
    async fn wait_for_effects(
        ...
        _client_addr: Option<SocketAddr>,
    ) -> Result<WaitForEffectsResponse, AiyError> {
        unimplemented!()
    }
    ...
    async fn handle_transaction(
        ...
        _client_addr: Option<SocketAddr>,
    ) -> Result<HandleTransactionResponse, AiyError> {
        if self.fault_config.fail_before_handle_transaction {
            return Err(AiyError::from("Mock error before handle_transaction"));
        }
        let state = self.state.clone();
        let epoch_store = self.state.load_epoch_store_one_call_per_task();
        let transaction = epoch_store
    ...
    async fn handle_certificate_v2(
        ...
        _client_addr: Option<SocketAddr>,
    ) -> Result<HandleCertificateResponseV2, AiyError> {
        let state = self.state.clone();
    ...
    async fn handle_certificate_v3(
        ...
        _client_addr: Option<SocketAddr>,
    ) -> Result<HandleCertificateResponseV3, AiyError> {
        let state = self.state.clone();
        let fault_config = self.fault_config;
        spawn_monitored_task!(Self::handle_certificate(state, request, fault_config))
            .await
    ...
    async fn handle_soft_bundle_certificates_v3(
        ...
        _client_addr: Option<SocketAddr>,
    ) -> Result<HandleSoftBundleCertificatesResponseV3, AiyError> {
        unimplemented!()
    }
    ...
    ) -> Result<ObjectInfoResponse, AiyError> {
        let state = self.state.clone();
    ...
    ) -> Result<TransactionInfoResponse, AiyError> {
        let state = self.state.clone();
    ...
    ) -> Result<CheckpointResponse, AiyError> {
        let state = self.state.clone();
    ...
    ) -> Result<CheckpointResponseV2, AiyError> {
        let state = self.state.clone();
    ...
    ) -> Result<AiySystemState, AiyError> {
        self.state.get_aiy_system_state_object_for_testing()
    }

    async fn validator_health(
        &self,
        _request: ValidatorHealthRequest,
    ) -> Result<ValidatorHealthResponse, AiyError> {
...
pub struct MockAuthorityApi {
    delay: Duration,
    count: Arc<Mutex<u32>>,
    handle_object_info_request_result: Option<AiyResult<ObjectInfoResponse>>,
}
...
impl MockAuthorityApi {
    ...
    pub fn set_handle_object_info_request(&mut self, result: AiyResult<ObjectInfoResponse>) {
...
impl AuthorityAPI for MockAuthorityApi {
    /// Submit a new transaction to a Aiy or Primary account.
    async fn submit_transaction(
        &self,
    ...
    /// Initiate a new transaction to a Aiy or Primary account.
    async fn handle_transaction(
        &self,
    ...
    async fn handle_transaction_info_request(
        ...
        Err(AiyError::TransactionNotFound {
...
pub struct HandleTransactionTestAuthorityClient {
    pub tx_info_resp_to_return: AiyResult<HandleTransactionResponse>,
    pub cert_resp_to_return: AiyResult<HandleCertificateResponseV2>,
...
impl HandleTransactionTestAuthorityClient {
    pub fn new() -> Self {
        Self {
            tx_info_resp_to_return: Err(AiyError::Unknown("".to_string())),
            cert_resp_to_return: Err(AiyError::Unknown("".to_string())),
    ...
    pub fn set_tx_info_response_error(&mut self, error: AiyError) {
        self.tx_info_resp_to_return = Err(error);
    }

    pub fn reset_tx_info_response(&mut self) {
        self.tx_info_resp_to_return = Err(AiyError::Unknown("".to_string()));
    }
    ...
    pub fn set_cert_resp_to_return_error(&mut self, error: AiyError) {
        self.cert_resp_to_return = Err(error);
    }

    pub fn reset_cert_response(&mut self) {
        self.cert_resp_to_return = Err(AiyError::Unknown("".to_string()));
    }
