...
use aiy_types::accumulator_root::{
...
use aiy_types::balance::{BALANCE_MODULE_NAME, BALANCE_STRUCT_NAME};
...
use aiy_types::transaction::{Argument, CallArg, ObjectArg, TransactionKind};
use aiy_types::{
    TypeTag, AIY_ACCUMULATOR_ROOT_OBJECT_ID, AIY_FRAMEWORK_ADDRESS, AIY_FRAMEWORK_PACKAGE_ID,
};
...
impl ClassifiedType {
    fn classify(ty: &TypeTag) -> Self {
        ...
        if struct_tag.address == AIY_FRAMEWORK_ADDRESS
        ...
        match (ty, merge, split) {
            ...
            (_, MergedValue::EventDigest(digest, event_count), MergedValue::EventDigest(_, _)) => {
                ...
                builder.programmable_move_call(
                    AIY_FRAMEWORK_PACKAGE_ID,
                    ACCUMULATOR_SETTLEMENT_MODULE.into(),
                    aiy_types::accumulator_root::ACCUMULATOR_ROOT_SETTLEMENT_SETTLE_EVENTS_FUNC
                        .into(),
                    vec![],
                    args,
                );
...
impl AccumulatorSettlementTxBuilder {
    ...
    pub fn build_tx(
        ...
        let total_input_aiy_arg = builder.pure(self.total_input_aiy).unwrap();
        let total_output_aiy_arg = builder.pure(self.total_output_aiy).unwrap();
        tracing::debug!("total_input_aiy: {}", self.total_input_aiy);
        tracing::debug!("total_output_aiy: {}", self.total_output_aiy);
