...
use aiy_network::tonic::metadata::KeyAndValueRef;
use aiy_network::tonic::transport::Channel;
...
pub struct NetworkAuthorityClient {
    client: AiyResult<ValidatorClient<Channel>>,
}
...
impl NetworkAuthorityClient {
    ...
    pub fn connect_lazy(address: &Multiaddr, tls_target: NetworkPublicKey) -> Self {
        ...
        let client: AiyResult<_> = mysten_network::client::connect_lazy(address, tls_config)
    ...
    fn new_lazy(client: AiyResult<Channel>) -> Self {
        Self {
            client: client.map(ValidatorClient::new),
        }
    }

    pub(crate) fn client(&self) -> AiyResult<ValidatorClient<Channel>> {
        self.client.clone()
    }
}
...
impl AuthorityAPI for NetworkAuthorityClient {
    /// Submits a transaction to the Aiy network for certification and execution.
    async fn submit_transaction(
        &self,
        request: SubmitTxRequest,
        client_addr: Option<SocketAddr>,
...
pub fn make_network_authority_clients_with_network_config(
    ...
    for (name, (_state, network_metadata)) in committee.validators() {
        ...
        let tls_config = network_metadata
            ...
            .as_ref()
            .map(|key| {
                aiy_tls::create_rustls_client_config(
                    key.clone(),
                    aiy_tls::AIY_VALIDATOR_SERVER_NAME.to_string(),
                    None,
                )
            })
            .ok_or(AiyError::from("network public key is not available"));
