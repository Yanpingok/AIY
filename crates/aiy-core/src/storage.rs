...
use aiy_types::base_types::TransactionDigest;
...
use aiy_types::messages_checkpoint::EndOfEpochData;
use aiy_types::messages_checkpoint::FullCheckpointContents;
use aiy_types::messages_checkpoint::VerifiedCheckpoint;
use aiy_types::messages_checkpoint::VerifiedCheckpointContents;
use aiy_types::object::Object;
use aiy_types::storage::error::Error as StorageError;
use aiy_types::storage::error::Result;
use aiy_types::storage::BalanceInfo;
use aiy_types::storage::BalanceIterator;
use aiy_types::storage::CoinInfo;
use aiy_types::storage::DynamicFieldKey;
use aiy_types::storage::ObjectStore;
use aiy_types::storage::OwnedObjectInfo;
use aiy_types::storage::RpcIndexes;
use aiy_types::storage::RpcStateReader;
use aiy_types::storage::TransactionInfo;
use aiy_types::storage::WriteStore;
use aiy_types::storage::{ObjectKey, ReadStore};
...
impl ReadStore for RocksDbStore {
            ...
            .and_then(|contents| {
                ...
                for tx in contents.iter() {
                    if let (Some(t), Some(e)) = (
                        ...
                        transactions.push(aiy_types::base_types::ExecutionData::new(
                            (*t).clone().into_inner(),
                            e,
                        ))
                    } else {
    ...
    fn get_latest_checkpoint(&self) -> aiy_types::storage::error::Result<VerifiedCheckpoint> {
        self.checkpoint_store
            ...
            .expect("db error")
            .ok_or_else(|| {
                aiy_types::storage::error::Error::missing("unable to get latest checkpoint")
            })
    }
...
impl ObjectStore for RocksDbStore {
    fn get_object(&self, object_id: &aiy_types::base_types::ObjectID) -> Option<Object> {
        self.cache_traits.object_store.get_object(object_id)
    }

    fn get_object_by_key(
        &self,
        object_id: &aiy_types::base_types::ObjectID,
...
impl WriteStore for RocksDbStore {
    ...
    ) -> Result<(), aiy_types::storage::error::Error> {
    ...
    fn update_highest_synced_checkpoint(
        ...
        self.checkpoint_store
            ...
            .map_err(aiy_types::storage::error::Error::custom)?;
...
impl RestReadStore {
    ...
    fn index(&self) -> aiy_types::storage::error::Result<&RpcIndexStore> {
        self.state
            .rpc_index
            .as_deref()
            .ok_or_else(|| aiy_types::storage::error::Error::custom("rest index store is disabled"))
    }
}
...
impl RpcStateReader for RestReadStore {
    ...
    fn get_chain_identifier(&self) -> Result<aiy_types::digests::ChainIdentifier> {
...
impl RpcIndexes for RpcIndexStore {
    fn get_epoch_info(&self, epoch: EpochId) -> Result<Option<aiy_types::storage::EpochInfo>> {
    ...
    fn get_transaction_info(
        &self,
        digest: &TransactionDigest,
    ) -> aiy_types::storage::error::Result<Option<TransactionInfo>> {
    ...
    ) -> aiy_types::storage::error::Result<
    ...
    fn get_coin_info(
        &self,
        coin_type: &StructTag,
    ) -> aiy_types::storage::error::Result<Option<CoinInfo>> {
    ...
    fn get_balance(
        ...
        owner: &AiyAddress,
        coin_type: &StructTag,
    ) -> aiy_types::storage::error::Result<Option<BalanceInfo>> {
    ...
    fn balance_iter(
        &self,
        owner: &AiyAddress,
        cursor: Option<(AiyAddress, StructTag)>,
    ) -> aiy_types::storage::error::Result<BalanceIterator<'_>> {
        let cursor_key =
