...
pub struct AiyTxValidator {
    ...
    metrics: Arc<AiyTxValidatorMetrics>,
}
...
impl AiyTxValidator {
    pub fn new(
        ...
        info!(
            "AiyTxValidator constructed for epoch {}",
            epoch_store.epoch()
        );
        Self {
            authority_state,
    ...
    fn validate_transactions(&self, txs: &[ConsensusTransactionKind]) -> Result<(), AiyError> {
        ...
        for tx in txs.iter() {
            match tx {
                ...
                ConsensusTransactionKind::CheckpointSignatureV2(signature) => {
                    ...
                    {
                        return Err(AiyError::UnexpectedMessage(
                ...
                ConsensusTransactionKind::RandomnessDkgMessage(_, bytes) => {
                    if bytes.len() > dkg_v1::DKG_MESSAGES_MAX_SIZE {
                        ...
                        return Err(AiyError::InvalidDkgMessageSize);
                    }
                }
                ...
                ConsensusTransactionKind::ExecutionTimeObservation(obs) => {
                    ...
                    if obs.estimates.len()
                        > epoch_store
                            ...
                            .unwrap()
                    {
                        return Err(AiyError::UnexpectedMessage(format!(
...
impl TransactionVerifier for AiyTxValidator {
...
mod tests {
    ...
    use aiy_config::transaction_deny_config::TransactionDenyConfigBuilder;
    use aiy_macros::sim_test;
    use aiy_protocol_config::{Chain, ProtocolConfig, ProtocolVersion};
    use aiy_types::crypto::deterministic_random_account_key;
    ...
    use crate::{
        ...
        consensus_validator::{AiyTxValidator, AiyTxValidatorMetrics},
    };
    ...
    async fn accept_valid_transaction() {
        ...
        let network_config =
            aiy_swarm_config::network_config_builder::ConfigBuilder::new_with_temp_dir()
        ...
        let metrics = AiyTxValidatorMetrics::new(&Default::default());
        let validator = AiyTxValidator::new(
            state.clone(),
        ...
        let bogus_transaction_bytes: Vec<_> = certificates
            ...
            .map(|mut cert| {
                ...
                cert.tx_signatures_mut_for_testing()[0] =
                    GenericSignature::Signature(aiy_types::crypto::Signature::Ed25519AiySignature(
                        Ed25519AiySignature::default(),
                    ));
                bcs::to_bytes(&ConsensusTransaction::new_certificate_message(&name1, cert)).unwrap()
            })
            .collect();
    ...
    async fn test_verify_and_vote_batch() {
        ...
        let validator = AiyTxValidator::new(
            ...
            Arc::new(NoopConsensusOverloadChecker {}),
            Arc::new(CheckpointServiceNoop {}),
            AiyTxValidatorMetrics::new(&Default::default()),
        );
    ...
    async fn reject_checkpoint_signature_v2_when_flag_disabled() {
        ...
        let network_config =
            aiy_swarm_config::network_config_builder::ConfigBuilder::new_with_temp_dir().build();
