...
use aiy_types::{
    base_types::{FullObjectID, ObjectID, ObjectRef, SequenceNumber, AiyAddress},
...
async fn test_object_lock_conflict() {
    ...
    assert!(matches!(
        mutate_cert_res.err(),
        Some(AiyError::ObjectLockConflict { .. })
    ));
}
