...
use aiy_types::move_package::UpgradeCap;
...
make_vec_tests_for_type!(
    ...
    ObjectID,
    TypeTag::Struct(Box::new(aiy_types::id::ID::type_())),
    ObjectID::ZERO
);
...
async fn test_object_no_id_error() {
    ...
    matches!(res.err(), Some(AiyError::ExecutionError(err_str)) if
                 err_str.contains("AiyMoveVerificationError")
