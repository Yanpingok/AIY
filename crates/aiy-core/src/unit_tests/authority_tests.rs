...
use aiy_json_rpc_types::{
    AiyArgument, AiyExecutionResult, AiyExecutionStatus, AiyTransactionBlockEffectsAPI,
    AiyTransactionBlockEffectsV1, AiyTypeTag,
};
...
use aiy_types::dynamic_field::DynamicFieldType;
use aiy_types::effects::TransactionEffects;
use aiy_types::epoch_data::EpochData;
use aiy_types::error::UserInputError;
use aiy_types::execution::SharedInput;
use aiy_types::execution_status::{ExecutionFailureStatus, ExecutionStatus};
...
use aiy_types::object::Data;
use aiy_types::programmable_transaction_builder::ProgrammableTransactionBuilder;
use aiy_types::randomness_state::get_randomness_state_obj_initial_shared_version;
use aiy_types::aiy_system_state::AiySystemStateWrapper;
...
use aiy_types::{
    ...
    MOVE_STDLIB_PACKAGE_ID, AIY_AUTHENTICATOR_STATE_OBJECT_ID, AIY_CLOCK_OBJECT_ID,
    AIY_FRAMEWORK_PACKAGE_ID, AIY_RANDOMNESS_STATE_OBJECT_ID, AIY_SYSTEM_STATE_OBJECT_ID,
};
use aiy_types::{digests::Digest, AIY_CLOCK_OBJECT_SHARED_VERSION};
...
async fn test_dry_run_no_gas_big_transfer() {
    ...
    builder.transfer_aiy(recipient, Some(amount));
    ...
    let (dry_run_res, _, _, _) = fullnode
        ...
        .await
        .unwrap();
    assert_eq!(*dry_run_res.effects.status(), AiyExecutionStatus::Success);
}
...
async fn test_dev_inspect_gas_coin_argument() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_aiy(vec![recipient], vec![amount]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    ...
    assert_eq!(arg, &AiyArgument::GasCoin);
    check_coin_value(
        arg_value,
        arg_type,
...
fn check_coin_value(actual_value: &[u8], actual_type: &AiyTypeTag, expected_value: u64) {
...
async fn test_dry_run_dev_inspect_dynamic_field_too_new() {
    ...
    match effects {
        AiyTransactionBlockEffects::V1(AiyTransactionBlockEffectsV1 { abort_error, .. }) => {
...
async fn test_transfer_package() {
    ...
    let package_object_ref = authority_state
        .get_aiy_system_package_object_ref()
        .await
        .unwrap();
...
async fn test_objected_owned_gas() {
    ...
    authority_state
        ...
        .await;
    let rgp = authority_state.reference_gas_price_for_testing().unwrap();
    let data = TransactionData::new_transfer_aiy(
        recipient,
        sender,
        None,
...
async fn test_conflicting_transactions() {
    ...
    for _ in 0..100 {
        ...
        assert!(matches!(err, AiyError::ObjectLockConflict { .. }));
...
#[tokio::test]
async fn test_handle_transfer_aiy_with_amount_insufficient_gas() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let object_id = ObjectID::random();
    let authority_state = init_state_with_ids(vec![(sender, object_id)]).await;
...
async fn test_invalid_authenticator_state_parameter() {
    ...
    let tx_data = TransactionData::new_move_call(
        ...
        vec![CallArg::Object(ObjectArg::SharedObject {
            id: AIY_AUTHENTICATOR_STATE_OBJECT_ID,
    ...
    assert_eq!(
        ...
        UserInputError::InaccessibleSystemObject {
            object_id: AIY_AUTHENTICATOR_STATE_OBJECT_ID
        }
    );
}
...
async fn test_invalid_randomness_parameter() {
    ...
    let random_mut = CallArg::Object(ObjectArg::SharedObject {
        id: AIY_RANDOMNESS_STATE_OBJECT_ID,
    ...
    assert_eq!(
        UserInputError::try_from(e).unwrap(),
        UserInputError::ImmutableParameterExpectedError {
            object_id: AIY_RANDOMNESS_STATE_OBJECT_ID
        }
    );
}
...
#[tokio::test]
async fn test_genesis_aiy_system_state_object() {
    // This test verifies that we can read the genesis AiySystemState object.
    ...
    let _aiy_system_state =
        bcs::from_bytes::<AiySystemStateWrapper>(move_object.contents()).unwrap();
    assert!(move_object.type_().is(&AiySystemStateWrapper::type_()));
    let aiy_system_state = authority_state
        .get_aiy_system_state_object_for_testing()
        .unwrap();
    assert_eq!(
        &aiy_system_state
...
#[tokio::test]
async fn test_transfer_aiy_no_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    let init_balance = aiy_types::gas::get_gas_balance(&gas_object).unwrap();
    ...
    let gas_ref = gas_object.compute_object_reference();
    let tx_data = TransactionData::new_transfer_aiy(
        recipient,
        sender,
        None,
        gas_ref,
    ...
    let new_balance =
        aiy_types::gas::get_gas_balance(&authority_state.get_object(&gas_object_id).await.unwrap())
            .unwrap();
    assert_eq!(
...
#[tokio::test]
async fn test_transfer_aiy_with_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    ...
    let new_gas = authority_state
        ...
        .await
        .unwrap();
    assert_eq!(aiy_types::gas::get_gas_balance(&new_gas).unwrap(), 500);
    assert!(gas_ref.1 < effects.gas_object().0 .1);
...
#[tokio::test]
async fn test_clear_cache_reverts_transfer_aiy() {
...
async fn create_and_retrieve_df_info(function: &IdentStr) -> (AiyAddress, Vec<DynamicFieldInfo>) {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let gas_object_id = ObjectID::random();
    let (authority_state, object_basics) =
        init_state_with_ids_and_object_basics(vec![(sender, gas_object_id)]).await;
...
pub async fn init_state_with_ids_and_object_basics<
    I: IntoIterator<Item = (AiyAddress, ObjectID)>,
>(
    objects: I,
) -> (Arc<AuthorityState>, ObjectRef) {
    let state = TestAuthorityBuilder::new().build().await;
    ...
    let get_assigned_version = |txn_key: &TransactionKey| -> SequenceNumber {
        assigned_versions
            ...
            .filter_map(|((id, initial_shared_version), seq)| {
                if id == &AIY_CLOCK_OBJECT_ID
                    && initial_shared_version == &AIY_CLOCK_OBJECT_SHARED_VERSION
                {
                    Some(*seq)
                } else {
                    None
...
async fn test_consensus_message_processed() {
    ...
    let network_config = aiy_swarm_config::network_config_builder::ConfigBuilder::new(&dir)
...
async fn test_gas_smashing() {
    ...
    fn make_gas_coins(owner: AiyAddress, gas_amount: u64, coin_num: u64) -> Vec<Object> {
        let mut objects = vec![];
    ...
    async fn run_and_check(
        ...
        let balance =
            aiy_types::gas::get_gas_balance(&state.get_object(&gas_coin_ids[0]).await.unwrap())
                .unwrap();
...
async fn test_publish_missing_dependency() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.publish_immutable(modules, vec![AIY_FRAMEWORK_PACKAGE_ID]);
    let kind = TransactionKind::programmable(builder.finish());
...
async fn test_publish_not_a_package_dependency() {
    ...
    deps.push(AIY_SYSTEM_STATE_OBJECT_ID);
    builder.publish_immutable(modules, deps);
    let kind = TransactionKind::programmable(builder.finish());
    ...
    assert_eq!(
        AiyError::UserInputError {
            error: UserInputError::MoveObjectAsPackage {
                object_id: AIY_SYSTEM_STATE_OBJECT_ID
            }
        },
        failure,
    )
...
pub fn create_gas_objects(num: u32, owner: AiyAddress) -> Vec<Object> {
    let mut objects = vec![];
    for _ in 0..num {
        let gas_object_id = ObjectID::random();
