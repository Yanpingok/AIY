...
use aiy_types::base_types::{random_object_ref, ObjectRef, AiyAddress};
...
use aiy_types::message_envelope::Message as _;
...
async fn test_submit_transaction_wrong_epoch() {
    ...
    .transfer_aiy(None, test_context.sender)
    .build();
...
async fn test_submit_batched_transactions() {
    ...
    for result in raw_response.results {
        match result.inner {
            Some(aiy_types::messages_grpc::RawValidatorSubmitStatus::Submitted(_)) => {
...
async fn test_submit_batched_transactions_with_already_executed() {
    ...
    match &raw_response.results[0].inner {
        Some(aiy_types::messages_grpc::RawValidatorSubmitStatus::Executed(_)) => {
