...
use aiy_types::{MOVE_STDLIB_ADDRESS, AIY_FRAMEWORK_ADDRESS};
...
fn test_to_json_value() {
    ...
    let aiy_move_struct: AiyMoveStruct =
        BoundedVisitor::deserialize_struct(&event_bytes, &TestEvent::layout())
            .unwrap()
            .into();
    let json_value = aiy_move_struct.to_json_value();
    assert_eq!(
        Some(&json!("1000000")),
...
// TODO: Do we need this in the aiy-types lib?
