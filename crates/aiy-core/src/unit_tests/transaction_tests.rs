...
use std::ops::Deref;
use aiy_types::crypto::{PublicKey, AiySignature, ToFromBytes, ZkLoginPublicIdentifier};
use aiy_types::messages_grpc::HandleSoftBundleCertificatesRequestV3;
use aiy_types::utils::get_one_zklogin_inputs;
use aiy_types::{
    authenticator_state::ActiveJwk,
    base_types::{dbg_addr, FullObjectRef},
    crypto::{get_key_pair, AccountKeyPair, Signature, AiyKeyPair},
    error::{AiyError, UserInputError},
...
use aiy_types::digests::ConsensusCommitDigest;
...
async fn test_handle_transfer_transaction_bad_signature() {
    do_transaction_test(
        ...
        |err| {
            assert_matches!(err, AiyError::SignerSignatureAbsent { .. });
        },
    )
    .await;
}
...
pub fn init_move_call_transaction(
    ...
    let mut data = TransactionData::new_move_call(
        ...
        AIY_SYSTEM_MODULE_NAME.into(),
        ident_str!("request_add_validator").to_owned(),
        vec![],
        gas_object_ref,
        vec![CallArg::AIY_SYSTEM_MUT, call_arg],
        gas_budget,
        gas_price,
    )
    .unwrap();
...
async fn do_transaction_test_impl(
    ...
    senders: &[(AiyAddress, AccountKeyPair)],
    input_object: Object,
    pre_sign_mutations: impl Fn(&mut TransactionData),
    post_sign_mutations: impl Fn(&mut Transaction),
...
async fn test_zklogin_transfer_with_large_address_seed() {
    ...
    let sender = AiyAddress::generate(StdRng::from_seed([3; 32]));
    let recipient = dbg_addr(2);
...
async fn zklogin_test_caching_scenarios() {
    ...
    let zklogin_json_string =
        &get_one_zklogin_inputs("../aiy-types/src/unit_tests/zklogin_test_vectors.json");
    let bad_zklogin_inputs = ZkLoginInputs::from_json(zklogin_json_string, "111").unwrap();
    let sender = AiyAddress::try_from_unpadded(&bad_zklogin_inputs).unwrap();
...
async fn setup_zklogin_network(
    ...
    aiy_types::message_envelope::Envelope<SenderSignedData, aiy_types::crypto::EmptySignInfo>,
    ...
    let sender = AiyAddress::try_from_unpadded(zklogin).unwrap();
    ...
    let sender_2 = AiyAddress::from(&multisig_pk);
...
async fn zk_multisig_test() {
    ...
    .unwrap();
    let victim_addr = AiyAddress::from(&multisig_pk);
    ...
    for (kp, _pk_zklogin, inputs) in test_vectors {
        let intent_message = IntentMessage::new(Intent::aiy_transaction(), data.clone());
...
async fn test_very_large_certificate() {
    ...
    let quorum_signature = aiy_types::crypto::AuthorityQuorumSignInfo {
        epoch: 0,
        signature: aiy_types::crypto::AggregateAuthoritySignature::aggregate(&sigs)
    ...
    let cert = aiy_types::message_envelope::Envelope::new_from_data_and_sig(
...
async fn test_handle_soft_bundle_certificates_errors() {
    ...
    println!("Case 0: submit an empty soft bundle.");
    {
        ...
        assert_matches!(
            response.unwrap_err(),
            AiyError::NoCertificateProvidedError { .. }
        );
    }
...
fn sender_signed_data_serialized_intent() {
    ...
    assert_eq!(txn.intent_message().intent, Intent::aiy_transaction());
