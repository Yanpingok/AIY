...
use aiy_types::base_types::{dbg_addr, ObjectID, ObjectRef, AiyAddress};
use aiy_types::crypto::{get_account_key_pair, AccountKeyPair};
use aiy_types::deny_list_v1::{CoinDenyCap, RegulatedCoinMetadata};
use aiy_types::deny_list_v2::{
...
use aiy_types::{AIY_DENY_LIST_OBJECT_ID, AIY_FRAMEWORK_PACKAGE_ID};
...
async fn test_regulated_coin_v2_types() {
    ...
    let deny_list_object_init_version = env.get_latest_object_ref(&AIY_DENY_LIST_OBJECT_ID).await.1;
