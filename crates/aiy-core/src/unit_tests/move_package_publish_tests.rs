...
use aiy_move_build::{check_unpublished_dependencies, gather_published_ids, BuildConfig};
use aiy_types::{
...
async fn test_generate_lock_file() {
    ...
    let expected = expect![[r##"
        ...
        [[move.package]]
        id = "MoveStdlib"
        source = { local = "../../../../../aiy-framework/packages/move-stdlib" }

        [[move.package]]
        id = "Aiy"
        source = { local = "../../../../../aiy-framework/packages/aiy-framework" }
...
async fn test_custom_property_check_unpublished_dependencies() {
    ...
    let AiyError::ModulePublishFailure { error } = check_unpublished_dependencies(
