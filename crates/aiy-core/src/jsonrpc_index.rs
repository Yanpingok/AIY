...
//! IndexStore supports creation of various ancillary indexes of state in AiyDataStore.
...
use aiy_json_rpc_types::{AiyObjectDataFilter, TransactionFilter};
use aiy_storage::mutex_table::MutexTable;
use aiy_storage::sharded_lru::ShardedLruCache;
use aiy_types::base_types::{
    ObjectDigest, ObjectID, SequenceNumber, AiyAddress, TransactionDigest, TxSequenceNumber,
};
use aiy_types::base_types::{ObjectInfo, ObjectRef};
use aiy_types::digests::TransactionEventsDigest;
use aiy_types::dynamic_field::{self, DynamicFieldInfo};
use aiy_types::effects::TransactionEvents;
use aiy_types::error::{AiyError, AiyResult, UserInputError};
use aiy_types::inner_temporary_store::TxCoins;
use aiy_types::object::{Object, Owner};
use aiy_types::parse_aiy_struct_tag;
use aiy_types::storage::error::Error as StorageError;
...
type OwnerIndexKey = (AiyAddress, ObjectID);
...
impl CoinIndexKey2 {
    ...
    pub fn new(owner: AiyAddress, coin_type: String, balance: u64, object_id: ObjectID) -> Self {
        Self {
            owner,
            coin_type,
...
pub struct IndexStoreCaches {
    per_coin_type_balance: ShardedLruCache<(AiyAddress, TypeTag), AiyResult<TotalBalance>>,
    all_balances: ShardedLruCache<AiyAddress, AiyResult<Arc<HashMap<TypeTag, TotalBalance>>>>,
    pub locks: MutexTable<AiyAddress>,
}

#[derive(Default)]
pub struct IndexStoreCacheUpdates {
    _locks: Vec<OwnedMutexGuard<()>>,
    per_coin_type_balance_changes: Vec<((AiyAddress, TypeTag), AiyResult<TotalBalance>)>,
    all_balance_changes: Vec<(AiyAddress, AiyResult<Arc<AllBalance>>)>,
}
...
pub struct IndexStoreTables {
    ...
    /// Index from aiy address to transactions initiated by that address.
    transactions_from_addr: DBMap<(AiyAddress, TxSequenceNumber), TransactionDigest>,

    /// Index from aiy address to transactions that were sent to that address.
    transactions_to_addr: DBMap<(AiyAddress, TxSequenceNumber), TransactionDigest>,
    ...
    /// composite key of the AiyAddress of their owner and the object ID of the object.
    ...
    event_by_sender: DBMap<(AiyAddress, EventId), EventIndex>,
...
impl IndexStore {
        ...
        let table_options = DBMapTableConfigMap::new(BTreeMap::from([
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    |(_, id): (AiyAddress, TxSequenceNumber)| id,
                ),
            ),
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    db_options.clone(),
                    pruner_watermark.clone(),
                    |(_, sequence_number): (AiyAddress, TxSequenceNumber)| sequence_number,
                ),
            ),
            (
            ...
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    db_options.clone(),
                    pruner_watermark.clone(),
                    |(_, event_id): (AiyAddress, EventId)| event_id.0,
                ),
            ),
            (
    ...
    pub fn index_coin(
    ...
    ) -> AiyResult<IndexStoreCacheUpdates> {
        ...
        let mut addresses: HashSet<AiyAddress> = HashSet::new();
        addresses.extend(
        ...
        let mut balance_changes: HashMap<AiyAddress, HashMap<TypeTag, TotalBalance>> =
            HashMap::new();
        // Index coin info
            ...
            .flat_map(|(address, balance_map)| {
                balance_map.iter().map(|(type_tag, balance)| {
                    (
                        ...
                        Ok::<TotalBalance, AiyError>(*balance),
                    )
                })
            })
            .collect();
        let all_balance_changes: Vec<_> = balance_changes
            ...
            .map(|(address, balance_map)| {
                (
                    address,
                    Ok::<Arc<HashMap<TypeTag, TotalBalance>>, AiyError>(Arc::new(balance_map)),
                )
            })
            .collect();
    ...
    pub fn get_transactions(
        ...
        let cursor = if let Some(cursor) = cursor {
            Some(
                self.get_transaction_seq(&cursor)?
                    .ok_or(AiyError::TransactionNotFound { digest: cursor })?,
            )
        } else {
            None
        };
        match filter {
            ...
            Some(_) => Err(AiyError::UserInputError {
    ...
    pub fn get_transaction_seq(
        &self,
        digest: &TransactionDigest,
    ) -> AiyResult<Option<TxSequenceNumber>> {
    ...
    pub fn all_events(
        ...
        limit: usize,
        descending: bool,
    ) -> AiyResult<Vec<(TransactionEventsDigest, TransactionDigest, usize, u64)>> {
        Ok(if descending {
            self.tables
                .event_order
    ...
    pub fn events_by_transaction(
        ...
        let seq = self
            ...
            .ok_or(AiyError::TransactionNotFound { digest: *digest })?;
        Ok(if descending {
            self.tables
                .event_order
    ...
    pub fn prune(&self, cut_time_ms: u64) -> AiyResult<TxSequenceNumber> {
        match self
            .tables
            .event_by_time
            .reversed_safe_iter_with_bounds(
    ...
    pub fn insert_genesis_objects(&self, object_index_changes: ObjectIndexChanges) -> AiyResult {
    ...
    pub fn checkpoint_db(&self, path: &Path) -> AiyResult {
    ...
    #[instrument(skip(self))]
    pub fn get_balance(&self, owner: AiyAddress, coin_type: TypeTag) -> AiyResult<TotalBalance> {
        ...
        if force_disable_cache {
            Self::get_balance_from_db(metrics_cloned, coin_index_cloned, owner, cloned_coin_type)
                .map_err(|e| {
                AiyError::ExecutionError(format!("Failed to read balance frm DB: {:?}", e))
            })?;
        }
    ...
    pub fn get_all_balance(
        &self,
        owner: AiyAddress,
    ) -> AiyResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        let force_disable_cache = read_size_from_env(ENV_VAR_DISABLE_INDEX_CACHE).unwrap_or(0) > 0;
        let metrics_cloned = self.metrics.clone();
        let coin_index_cloned = self.tables.coin_index_2.clone();
        if force_disable_cache {
            Self::get_all_balances_from_db(metrics_cloned, coin_index_cloned, owner).map_err(
                |e| {
                    AiyError::ExecutionError(format!("Failed to read all balance from DB: {:?}", e))
                },
            )?;
        }
    ...
    /// Read balance for a `AiyAddress` and `CoinType` from the backend database
    #[instrument(skip_all)]
    ...
    /// Read all balances for a `AiyAddress` from the backend database
    #[instrument(skip_all)]
    pub fn get_all_balances_from_db(
        ...
        for (coin_type, coins) in &coins {
            ...
            let coin_type =
                TypeTag::Struct(Box::new(parse_aiy_struct_tag(&coin_type).map_err(|e| {
                    AiyError::ExecutionError(format!(
    ...
    fn invalidate_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = (AiyAddress, TypeTag)>,
    ) -> AiyResult {
    ...
    fn invalidate_all_balance_cache(
        &self,
        addresses: impl IntoIterator<Item = AiyAddress>,
    ) -> AiyResult {
    ...
    fn update_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = ((AiyAddress, TypeTag), AiyResult<TotalBalance>)>,
    ) -> AiyResult {
        self.caches
            .per_coin_type_balance
    ...
    fn merge_balance(
        old_balance: &AiyResult<TotalBalance>,
        balance_delta: &AiyResult<TotalBalance>,
    ) -> AiyResult<TotalBalance> {
    ...
    fn update_all_balance_cache(
        &self,
        keys: impl IntoIterator<Item = (AiyAddress, AiyResult<Arc<HashMap<TypeTag, TotalBalance>>>)>,
    ) -> AiyResult {
        self.caches
            .all_balances
    ...
    fn merge_all_balance(
        old_balance: &AiyResult<Arc<HashMap<TypeTag, TotalBalance>>>,
        balance_delta: &AiyResult<Arc<HashMap<TypeTag, TotalBalance>>>,
    ) -> AiyResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        if let Ok(old_balance) = old_balance {
            if let Ok(balance_delta) = balance_delta {
...
mod tests {
    ...
    use aiy_types::base_types::{ObjectInfo, ObjectType, AiyAddress};
    use aiy_types::digests::TransactionDigest;
    use aiy_types::effects::TransactionEvents;
    use aiy_types::gas_coin::GAS;
    use aiy_types::object;
    ...
    async fn test_index_cache() -> anyhow::Result<()> {
        ...
        let address: AiyAddress = AccountAddress::random().into();
