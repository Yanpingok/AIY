...
use aiy_types::{
    base_types::{random_object_ref, FullObjectRef, AiyAddress},
...
impl Scenario {
    ...
    fn new_outputs() -> TransactionOutputs {
        ...
        let (sender, keypair): (AiyAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
        let (receiver, _): (AiyAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
    ...
    pub async fn commit(&mut self, tx: TransactionDigest) -> AiyResult {
