...
use aiy_types::effects::{AccumulatorOperation, AccumulatorValue, TransactionEvents};
use aiy_types::event::Event;
use aiy_types::execution::ExecutionTimeObservationKey;
use aiy_types::execution_status::{
    CommandArgumentError, ExecutionFailureStatus, ExecutionStatus, PackageUpgradeError,
    TypeArgumentError,
};
use aiy_types::full_checkpoint_content::{CheckpointData, CheckpointTransaction};
use aiy_types::messages_checkpoint::{CertifiedCheckpointSummary, CheckpointCommitment};
use aiy_types::messages_consensus::ConsensusDeterminedVersionAssignments;
use aiy_types::messages_grpc::ObjectInfoRequestKind;
use aiy_types::move_package::TypeOrigin;
...
use aiy_types::type_input::{StructInput, TypeInput};
use aiy_types::{
...
use aiy_types::{
    ...
    crypto::{
        ...
        AuthorityPublicKeyBytes, AuthoritySignature, KeypairTraits, Signature, AiyKeyPair,
    },
...
fn get_registry() -> Result<Registry> {
    ...
    let kp1: AiyKeyPair =
        AiyKeyPair::Ed25519(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp2: AiyKeyPair =
        AiyKeyPair::Secp256k1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp3: AiyKeyPair =
        AiyKeyPair::Secp256r1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    ...
    // ObjectID and AiyAddress are the same length
    ...
    let struct_tag = StructTag::from_str("0x2::coin::Coin<0x2::aiy::AIY>").unwrap();
    ...
    tracer
        .trace_type::<base_types::AiyAddress>(&samples)
        .unwrap();
...
#[derive(Debug, Parser)]
#[clap(
    name = "Aiy format generator",
    about = "Trace serde (de)serialization to generate format descriptions for Aiy types"
)]
struct Options {
...
const FILE_PATH: &str = "aiy-core/tests/staged/aiy.yaml";
