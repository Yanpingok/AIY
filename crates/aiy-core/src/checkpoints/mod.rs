...
use aiy_network::default_mysten_network_config;
...
use aiy_types::messages_checkpoint::{CheckpointArtifacts, CheckpointCommitment};
use aiy_types::aiy_system_state::epoch_start_aiy_system_state::EpochStartSystemStateTrait;
use aiy_types::AIY_ACCUMULATOR_ROOT_OBJECT_ID;
...
use aiy_types::base_types::{AuthorityName, EpochId, TransactionDigest};
use aiy_types::committee::StakeUnit;
use aiy_types::crypto::AuthorityStrongQuorumSignInfo;
use aiy_types::digests::{CheckpointContentsDigest, CheckpointDigest, TransactionEffectsDigest};
...
use aiy_types::messages_checkpoint::{CheckpointRequestV2, SignedCheckpointSummary};
use aiy_types::messages_consensus::ConsensusTransactionKey;
use aiy_types::signature::GenericSignature;
use aiy_types::aiy_system_state::{AiySystemState, AiySystemStateTrait};
...
impl CheckpointStore {
    ...
    fn prune_local_summaries(&self) -> AiyResult {
    ...
    pub fn reset_db_for_execution_since_genesis(&self) -> AiyResult {
...
impl<AiyError: std::error::Error + Send + Sync + 'static> From<AiyError>
    for CheckpointBuilderError
{
    fn from(e: AiyError) -> Self {
        Self::Retry(e.into())
    }
}
...
impl CheckpointBuilder {
    ...
    async fn construct_and_execute_settlement_transactions(
        ...
        for fx in settlement_effects.iter() {
            ...
            if let Some(version) = fx
                .mutated()
                .iter()
                .find_map(|(oref, _)| (oref.0 == AIY_ACCUMULATOR_ROOT_OBJECT_ID).then_some(oref.1))
            {
                assert!(
    ...
    ) -> AiyResult<(Vec<TransactionEffects>, HashSet<TransactionDigest>)> {
    ...
    ) -> AiyResult<Option<(TransactionDigest, TransactionEffects)>> {
    ...
    ) -> CheckpointBuilderResult<AiySystemState> {
    ...
    ) -> AiyResult<Vec<TransactionEffects>> {
...
impl CheckpointAggregator {
    ...
    async fn run_and_notify(&mut self) -> AiyResult {
    ...
    fn run_inner(&mut self) -> AiyResult<Vec<CertifiedCheckpointSummary>> {
    ...
    fn next_checkpoint_to_certify(&self) -> AiyResult<CheckpointSequenceNumber> {
        Ok(self
            .store
            .tables
            .certified_checkpoints
...
impl CheckpointSignatureAggregator {
    ...
    pub fn try_aggregate(
        ...
        match self.signatures_by_digest.insert(their_digest, envelope) {
            ...
            InsertResult::Failed {
                error:
                    AiyError::StakeAggregatorRepeatedSigner {
...
async fn diagnose_split_brain(
    ...
    let committee = epoch_store
        .epoch_start_state()
        .get_aiy_committee_with_network_metadata();
...
pub trait CheckpointServiceNotify {
    ...
    fn notify_checkpoint(&self) -> AiyResult;
}
...
mod tests {
    ...
    use aiy_protocol_config::{Chain, ProtocolConfig};
    use aiy_types::accumulator_event::AccumulatorEvent;
    use aiy_types::authenticator_state::ActiveJwk;
    use aiy_types::base_types::{SequenceNumber, TransactionEffectsDigest};
    use aiy_types::crypto::Signature;
    use aiy_types::effects::{TransactionEffects, TransactionEvents};
    use aiy_types::messages_checkpoint::SignedCheckpointSummary;
    use aiy_types::transaction::VerifiedTransaction;
    use tokio::sync::mpsc;

    #[tokio::test]
    async fn test_clear_locally_computed_checkpoints_from_deletes_inclusive_range() {
        let store = CheckpointStore::new_for_tests();
        let protocol = aiy_protocol_config::ProtocolConfig::get_for_max_version_UNSAFE();
        for seq in 70u64..=80u64 {
            let contents =
                aiy_types::messages_checkpoint::CheckpointContents::new_with_digests_only_for_tests(
                    [aiy_types::base_types::ExecutionDigests::new(
                        aiy_types::digests::TransactionDigest::random(),
                        aiy_types::digests::TransactionEffectsDigest::ZERO,
                    )],
                );
            let summary = aiy_types::messages_checkpoint::CheckpointSummary::new(
                &protocol,
                0,
                seq,
                0,
                &contents,
                None,
                aiy_types::gas::GasCostSummary::default(),
                None,
                0,
                Vec::new(),
                Vec::new(),
    ...
    pub async fn checkpoint_builder_test() {
        ...
        for digest in all_digests {
            let signature = Signature::Ed25519AiySignature(Default::default()).into();
            state
