...
use aiy_types::full_checkpoint_content::{
...
pub(crate) fn store_checkpoint_locally(
    ...
    Blob::encode(&checkpoint_data, BlobEncoding::Bcs)
        .map_err(|_| AiyError::TransactionSerializationError {
            error: "failed to serialize full checkpoint content".to_string(),
        }) // Map the first error
        .and_then(|blob| {
            std::fs::write(path.join(file_name), blob.to_bytes()).map_err(|_| {
                AiyError::FileIOError("failed to save full checkpoint content locally".to_string())
            })
        })?;
...
pub(crate) fn load_checkpoint(
    ...
    let object_set = {
        let refs = transactions
            .iter()
            .flat_map(|tx| {
                aiy_types::storage::get_transaction_object_set(
                    &tx.transaction,
                    &tx.effects,
        ...
        for (idx, object) in objects.into_iter().enumerate() {
            object_set.insert(object.ok_or_else(|| {
                aiy_types::storage::error::Error::custom(format!(
