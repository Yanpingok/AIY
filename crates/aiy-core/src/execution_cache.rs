...
use crate::authority::authority_store::{ExecutionLockWriteGuard, AiyLockResult};
...
use aiy_types::base_types::{FullObjectID, VerifiedExecutionData};
use aiy_types::digests::{TransactionDigest, TransactionEffectsDigest};
...
use aiy_types::transaction::{VerifiedSignedTransaction, VerifiedTransaction};
use aiy_types::{
...
/// Should only be used for aiy-tool or tests. Nodes must use build_execution_cache which
...
pub trait ObjectCacheRead: Send + Sync {
    fn get_package_object(&self, id: &ObjectID) -> AiyResult<Option<PackageObject>>;
        ...
        for (object_opt, object_ref) in objects.into_iter().zip(object_refs) {
            match object_opt {
                None => {
                    ...
                    return Err(AiyError::UserInputError { error });
                }
                Some(object) => {
                    result.push(object);
                }
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_store: &AuthorityPerEpochStore) -> AiyLockResult;

    // This method is considered "private" - only used by multi_get_objects_with_more_accurate_error_return
    fn _get_live_objref(&self, object_id: ObjectID) -> AiyResult<ObjectRef>;
    ...
    fn check_owned_objects_are_live(&self, owned_object_refs: &[ObjectRef]) -> AiyResult;

    fn get_aiy_system_state_object_unsafe(&self) -> AiyResult<AiySystemState>;

    fn get_bridge_object_unsafe(&self) -> AiyResult<Bridge>;
...
pub trait TransactionCacheRead: Send + Sync {
    ...
    fn get_transactions_and_serialized_sizes(
        &self,
        digests: &[TransactionDigest],
    ) -> AiyResult<Vec<Option<(VerifiedTransaction, usize)>>> {
...
pub trait ExecutionCacheReconfigAPI: Send + Sync {
    ...
    fn expensive_check_aiy_conservation(
        &self,
        old_epoch_store: &AuthorityPerEpochStore,
    ) -> AiyResult;

    fn checkpoint_db(&self, path: &Path) -> AiyResult;
    ...
    ($implementor: ident) => {
        ...
        impl BackingPackageStore for $implementor {
            fn get_package_object(
                &self,
                package_id: &ObjectID,
            ) -> AiyResult<Option<PackageObject>> {
    ...
    ($implementor: ident) => {
        ...
        impl ExecutionCacheReconfigAPI for $implementor {
            ...
            fn expensive_check_aiy_conservation(
                ...
                self.store
                    .expensive_check_aiy_conservation(self, old_epoch_store)
            }

            fn checkpoint_db(&self, path: &std::path::Path) -> AiyResult {
