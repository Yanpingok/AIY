...
use aiy_network::{
    ...
    tonic,
    validator::server::AIY_TLS_SERVER_NAME,
};
use aiy_types::message_envelope::Message;
use aiy_types::messages_consensus::ConsensusPosition;
use aiy_types::messages_consensus::{ConsensusTransaction, ConsensusTransactionKind};
...
use aiy_types::traffic_control::{ClientIdSource, Weight};
use aiy_types::{
...
use aiy_types::{effects::TransactionEvents, messages_grpc::SubmitTxType};
use aiy_types::{error::*, transaction::*};
use aiy_types::{
    fp_ensure,
    messages_checkpoint::{
...
use aiy_config::local_ip_utils::new_local_tcp_address_for_testing;
use aiy_types::messages_grpc::PingType;
...
pub struct AuthorityServerHandle {
    server_handle: aiy_network::validator::server::Server,
}
...
impl AuthorityServer {
    ...
    pub async fn spawn_with_bind_address_for_test(
        ...
        let server = aiy_network::validator::server::ServerBuilder::from_config(
            &config,
...
impl ValidatorService {
    ...
    // `Self::handle_submit_transaction()` and `AiyTxValidator::vote_transaction()` as well.
    async fn handle_transaction(
        ...
        let info = state
            ...
            .await
            .tap_err(|e| {
                if let AiyError::ValidatorHaltedAtEpochEnd = e {
    ...
    async fn handle_submit_transaction(
        ...
        if is_ping_request {
            fp_ensure!(
                ...
                AiyError::InvalidRequest(format!(
        ...
        for (idx, tx_bytes) in request.transactions.iter().enumerate() {
            let transaction = match bcs::from_bytes::<Transaction>(tx_bytes) {
                ...
                Err(e) => {
                    ...
                    return Err(AiyError::TransactionDeserializationError {
    ...
    ) -> Result<RawSubmitTxResponse, AiyError> {
        let mut raw_results = Vec::new();
        for (i, result) in results.into_iter().enumerate() {
            let result = result.ok_or_else(|| AiyError::GenericAuthorityError {
    ...
    async fn handle_certificates(
        ...
        fp_ensure!(
            ...
            AiyError::FullNodeCantHandleCertificate.into()
        );
    ...
    async fn handle_submit_to_consensus_for_position(
...

        {
            ...
            if !reconfiguration_lock.should_accept_user_certs() {
                ...
                return Err(AiyError::ValidatorHaltedAtEpochEnd.into());
            }
        ...
        Ok(rx_consensus_positions.await.map_err(|e| {
            AiyError::FailedToSubmitToConsensus(format!("Failed to get consensus position: {e}"))
        })?)
    }
    ...
    ) -> AiyResult<(Option<TransactionEvents>, Vec<Object>, Vec<Object>)> {
        let events = if include_events && effects.events_digest().is_some() {
...
impl ValidatorService {
    ...
    async fn wait_for_effects_response(
        ...
        epoch_store: &Arc<AuthorityPerEpochStore>,
    ) -> AiyResult<WaitForEffectsResponse> {
        if request.ping_type.is_some() {
            ...
            .await
            .map_err(|_| AiyError::TimeoutError)?;
        }

        let Some(tx_digest) = request.transaction_digest else {
            return Err(AiyError::InvalidRequest(
    ...
    async fn handle_soft_bundle_certificates_v3_impl(
        ...
        let certificates =
            NonEmpty::from_vec(request.certificates).ok_or(AiyError::NoCertificateProvidedError)?;
        let mut total_size_bytes = 0;
        for certificate in &certificates {
    ...
    ) -> WrappedServiceResponse<AiySystemState> {
        let response = self
            .state
    ...
    async fn validator_health_impl(
        &self,
        _request: tonic::Request<aiy_types::messages_grpc::RawValidatorHealthRequest>,
    ) -> WrappedServiceResponse<aiy_types::messages_grpc::RawValidatorHealthResponse> {
        ...
        let typed_response = aiy_types::messages_grpc::ValidatorHealthResponse {
        ...
        let raw_response = typed_response
            .try_into()
            .map_err(|e: aiy_types::error::AiyError| {
    ...
    async fn handle_traffic_req(&self, client: Option<IpAddr>) -> Result<(), tonic::Status> {
        if let Some(traffic_controller) = &self.traffic_controller {
            if !traffic_controller.check(&client, &None).await {
                // Entity in blocklist
                Err(tonic::Status::from_error(AiyError::TooManyRequests.into()))
            } else {
                Ok(())
            }
        } else {
    ...
    fn handle_traffic_resp<T>(
        ...
        let (error, spam_weight, unwrapped_response) = match wrapped_response {
            ...
            Err(status) => (
                Some(AiyError::from(status.clone())),
                Weight::zero(),
                Err(status.clone()),
            ),
        };
...
fn normalize(err: AiyError) -> Weight {
    match err {
        AiyError::UserInputError {
        ...
        | AiyError::WrongEpoch { .. } => Weight::one(),
        _ => Weight::zero(),
    }
}
...
impl Validator for ValidatorService {
    ...
    ) -> Result<tonic::Response<AiySystemState>, tonic::Status> {
        handle_with_decoration!(self, get_system_state_object_impl, request)
    }

    async fn validator_health(
        &self,
        request: tonic::Request<aiy_types::messages_grpc::RawValidatorHealthRequest>,
    ) -> Result<tonic::Response<aiy_types::messages_grpc::RawValidatorHealthResponse>, tonic::Status>
    {
