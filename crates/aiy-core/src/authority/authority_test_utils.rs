...
use aiy_types::base_types::FullObjectRef;
use aiy_types::crypto::{AccountKeyPair, AuthorityKeyPair};
...
pub async fn certify_transaction(
    authority: &AuthorityState,
    transaction: Transaction,
) -> Result<VerifiedCertificate, AiyError> {
...
pub async fn init_state_validator_with_fullnode() -> (Arc<AuthorityState>, Arc<AuthorityState>) {
    use aiy_types::crypto::get_authority_key_pair;
...
pub async fn init_state_with_ids<I: IntoIterator<Item = (AiyAddress, ObjectID)>>(
    objects: I,
) -> Arc<AuthorityState> {
...
pub async fn init_state_with_ids_and_versions<
    I: IntoIterator<Item = (AiyAddress, ObjectID, SequenceNumber)>,
>(
    objects: I,
) -> Arc<AuthorityState> {
    let state = TestAuthorityBuilder::new().build().await;
...
pub async fn init_state_with_objects<I: IntoIterator<Item = Object>>(
    ...
    let network_config = aiy_swarm_config::network_config_builder::ConfigBuilder::new(&dir).build();
...
pub async fn certify_shared_obj_transaction_no_execution(
    authority: &AuthorityState,
    transaction: Transaction,
) -> Result<(VerifiedCertificate, AssignedVersions), AiyError> {
    let epoch_store = authority.load_epoch_store_one_call_per_task();
    let transaction = epoch_store.verify_transaction(transaction).unwrap();
    let response = authority
        .handle_transaction(&epoch_store, transaction.clone())
...
) -> Result<Vec<TransactionEffects>, AiyError> {
...
) -> Result<(TransactionEffects, Option<ExecutionError>), AiyError> {
