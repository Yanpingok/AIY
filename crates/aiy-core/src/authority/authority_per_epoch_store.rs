...
use aiy_config::node::ExpensiveSafetyCheckConfig;
use aiy_execution::{self, Executor};
use aiy_macros::fail_point;
use aiy_macros::fail_point_arg;
use aiy_protocol_config::{Chain, PerObjectCongestionControlMode, ProtocolConfig, ProtocolVersion};
use aiy_storage::mutex_table::{MutexGuard, MutexTable};
use aiy_types::authenticator_state::{get_authenticator_state, ActiveJwk};
use aiy_types::base_types::{
    AuthorityName, ConsensusObjectSequenceKey, EpochId, FullObjectID, ObjectID, SequenceNumber,
    TransactionDigest,
};
use aiy_types::base_types::{ConciseableName, ObjectRef};
...
use aiy_types::digests::{ChainIdentifier, TransactionEffectsDigest};
use aiy_types::dynamic_field::get_dynamic_field_from_store;
use aiy_types::effects::{TransactionEffects, TransactionEffectsAPI};
use aiy_types::error::{AiyError, AiyResult};
use aiy_types::executable_transaction::{
    TrustedExecutableTransaction, VerifiedExecutableTransaction,
};
use aiy_types::execution::{ExecutionTimeObservationKey, ExecutionTiming};
use aiy_types::global_state_hash::GlobalStateHash;
use aiy_types::message_envelope::TrustedEnvelope;
...
use aiy_types::storage::{BackingPackageStore, InputKey, ObjectStore};
use aiy_types::aiy_system_state::epoch_start_aiy_system_state::{
    EpochStartSystemState, EpochStartSystemStateTrait,
};
use aiy_types::aiy_system_state::{self, AiySystemState};
...
pub enum ConsensusCertificateResult {
    ...
    AiyTransaction(VerifiedExecutableTransaction),
...
impl AuthorityEpochTables {
    ...
    fn load_reconfig_state(&self) -> AiyResult<ReconfigState> {
        let state = self
            .reconfig_state
    ...
    pub fn get_all_pending_consensus_transactions(&self) -> AiyResult<Vec<ConsensusTransaction>> {
        Ok(self
    ...
    pub fn remove_executed_tx_subtle(&self, digest: &TransactionDigest) -> AiyResult {
        self.executed_transactions_to_checkpoint.remove(digest)?;
        Ok(())
    }

    pub fn get_last_consensus_index(&self) -> AiyResult<Option<ExecutionIndices>> {
        Ok(self
    ...
    pub fn get_last_consensus_stats(&self) -> AiyResult<Option<ExecutionIndicesWithStats>> {
        Ok(self.last_consensus_stats.get(&LAST_CONSENSUS_STATS_ADDR)?)
    }

    pub fn get_locked_transaction(&self, obj_ref: &ObjectRef) -> AiyResult<Option<LockDetails>> {
        Ok(self
    ...
    ) -> AiyResult<Vec<Option<LockDetails>>> {
        Ok(self
            .owned_object_locked_transactions
    ...
    fn get_all_deferred_transactions(
        &self,
    ) -> AiyResult<BTreeMap<DeferralKey, Vec<VerifiedSequencedConsensusTransaction>>> {
        Ok(self
    ...
    fn get_all_deferred_transactions_v2(
        &self,
    ) -> AiyResult<BTreeMap<DeferralKey, Vec<VerifiedExecutableTransaction>>> {
        Ok(self
...
impl AuthorityPerEpochStore {
    ...
    pub fn tables(&self) -> AiyResult<Arc<AuthorityEpochTables>> {
        match self.tables.load_full() {
            Some(tables) => Ok(tables),
            None => Err(AiyError::EpochEnded(self.epoch())),
        }
    }
    ...
    ) -> AiyResult<Option<GlobalStateHash>> {
        Ok(self
            .tables()?
    ...
    pub fn get_highest_running_root_state_hash(
        &self,
    ) -> AiyResult<Option<(CheckpointSequenceNumber, GlobalStateHash)>> {
        Ok(self
            .tables()?
            .running_root_state_hash
    ...
    > {
        ...
        // Load stored execution time observations from the AiySystemState object.
        let system_state =
            aiy_system_state::get_aiy_system_state(object_store).expect("System state must exist");
        let system_state = match system_state {
            AiySystemState::V2(system_state) => system_state,
            AiySystemState::V1(_) => {
                if committee.epoch() > 1 {
                    error!("`PerObjectCongestionControlMode::ExecutionTimeEstimate` cannot load execution time observations to AiySystemState because it has an old version. This should not happen outside tests.");
                }
                return itertools::Either::Left(std::iter::empty());
            }
            #[cfg(msim)]
            AiySystemState::SimTestV1(_)
            | AiySystemState::SimTestShallowV2(_)
            | AiySystemState::SimTestDeepV2(_) => {
                return itertools::Either::Left(std::iter::empty());
            }
        };
    ...
    pub fn acquire_tx_guard(&self, cert: &VerifiedExecutableTransaction) -> AiyResult<CertTxGuard> {
    ...
    pub fn store_reconfig_state(&self, new_state: &ReconfigState) -> AiyResult {
        self.tables()?
            .reconfig_state
    ...
    pub fn insert_signed_transaction(&self, transaction: VerifiedSignedTransaction) -> AiyResult {
        Ok(self
            .tables()?
            .signed_transactions
    ...
    pub fn tx_key_to_digest(&self, key: &TransactionKey) -> AiyResult<Option<TransactionDigest>> {
        let tables = self.tables()?;
    ...
    pub fn get_effects_signature(
        &self,
        tx_digest: &TransactionDigest,
    ) -> AiyResult<Option<AuthoritySignInfo>> {
        let tables = self.tables()?;
    ...
    pub fn get_signed_effects_digest(
        &self,
        tx_digest: &TransactionDigest,
    ) -> AiyResult<Option<TransactionEffectsDigest>> {
        let tables = self.tables()?;
    ...
    pub fn get_transaction_cert_sig(
        &self,
        tx_digest: &TransactionDigest,
    ) -> AiyResult<Option<AuthorityStrongQuorumSignInfo>> {
    ...
    pub fn get_last_consensus_stats(&self) -> AiyResult<ExecutionIndicesWithStats> {
        assert!(
    ...
    ) -> AiyResult<Vec<(CheckpointSequenceNumber, GlobalStateHash)>> {
        self.tables()?
            .state_hash_by_checkpoint
    ...
    pub fn handle_finalized_checkpoint(
        ...
        let tables = match self.tables() {
            ...
            Err(AiyError::EpochEnded(_)) => return Ok(()),
            Err(e) => return Err(e),
        };
    ...
    ) -> AiyResult<HashMap<ConsensusObjectSequenceKey, SequenceNumber>> {
    ...
    ) -> AiyResult<AssignedTxAndVersions> {
    ...
    pub(crate) fn load_deferred_transactions_for_randomness(
        &self,
        output: &mut ConsensusCommitOutput,
    ) -> AiyResult<Vec<(DeferralKey, Vec<VerifiedSequencedConsensusTransaction>)>> {
    ...
    pub(crate) fn load_deferred_transactions_for_randomness_v2(
        &self,
        output: &mut ConsensusCommitOutput,
    ) -> AiyResult<Vec<(DeferralKey, Vec<VerifiedExecutableTransaction>)>> {
        let (min, max) = DeferralKey::full_range_for_randomness();
    ...
    pub fn clear_override_protocol_upgrade_buffer_stake(&self) -> AiyResult {
        warn!(
            epoch = ?self.epoch(),
    ...
    pub fn set_override_protocol_upgrade_buffer_stake(&self, new_stake_bps: u64) -> AiyResult {
        warn!(
            ?new_stake_bps,
            epoch = ?self.epoch(),
    ...
    pub fn record_capabilities(&self, capabilities: &AuthorityCapabilitiesV1) -> AiyResult {
    ...
    /// Record most recently advertised capabilities of all authorities
    pub fn record_capabilities_v2(&self, capabilities: &AuthorityCapabilitiesV2) -> AiyResult {
    ...
    pub fn get_capabilities_v1(&self) -> AiyResult<Vec<AuthorityCapabilitiesV1>> {
    ...
    pub fn get_capabilities_v2(&self) -> AiyResult<Vec<AuthorityCapabilitiesV2>> {
        assert!(self.protocol_config.authority_capabilities_v2());
        Ok(self
            .tables()?
            .authority_capabilities_v2
    ...
    pub(crate) fn get_new_jwks(&self, round: u64) -> AiyResult<Vec<ActiveJwk>> {
    ...
    pub(crate) fn get_randomness_last_round_timestamp(&self) -> AiyResult<Option<TimestampMs>> {
        if let Some(ts) = self
            .consensus_quarantine
            .read()
    ...
    #[instrument(level = "trace", skip_all)]
    pub fn verify_transaction(&self, tx: Transaction) -> AiyResult<VerifiedTransaction> {
    ...
    pub(crate) fn verify_consensus_transaction(
        ...
        // Signatures are verified as part of the consensus payload verification in AiyTxValidator
    ...
    fn db_batch(&self) -> AiyResult<DBBatch> {
    ...
    ) -> AiyResult<(Vec<Schedulable>, AssignedTxAndVersions)> {
    ...
    pub(crate) fn add_consensus_commit_prologue_transaction(
    ...
    ) -> AiyResult<Option<TransactionKey>> {
        {
        ...
        let consensus_commit_prologue_root = match self.process_consensus_system_transaction(&transaction) {
            ConsensusCertificateResult::AiyTransaction(processed_tx) => {
    ...
    async fn process_consensus_transactions<C: CheckpointServiceNotify>(
            ...
            .chain(randomness_transactions.iter().map(Either::Right))
        {
            ...
            match self
                ...
                .await?
            {
                ConsensusCertificateResult::AiyTransaction(cert) => {
    ...
    async fn process_consensus_transaction<C: CheckpointServiceNotify>(
        ...
        execution_time_estimator: Option<&ExecutionTimeEstimator>,
        authority_metrics: &Arc<AuthorityMetrics>,
    ) -> AiyResult<ConsensusCertificateResult> {
        ...
        match &transaction {
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                // Safe because signatures are verified when consensus called into AiyTxValidator::validate_batch.
            ...
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                // We usually call notify_checkpoint_signature in AiyTxValidator, but that step can
    ...
    fn process_consensus_system_transaction(
        ...
        ConsensusCertificateResult::AiyTransaction(system_transaction.clone())
    }
    ...
    fn process_consensus_user_transaction(
        ...
        Ok(ConsensusCertificateResult::AiyTransaction(transaction))
    }
    ...
    ) -> AiyResult<Vec<(CheckpointHeight, PendingCheckpoint)>> {
        Ok(self
            .consensus_quarantine
            .read()
    ...
    pub fn pending_checkpoint_exists(&self, index: &CheckpointHeight) -> AiyResult<bool> {
        Ok(self
            .consensus_quarantine
            .read()
    ...
    pub fn last_built_checkpoint_builder_summary(
        &self,
    ) -> AiyResult<Option<BuilderCheckpointSummary>> {
    ...
    pub fn last_built_checkpoint_summary(
        &self,
    ) -> AiyResult<Option<(CheckpointSequenceNumber, CheckpointSummary)>> {
    ...
    pub fn get_built_checkpoint_summary(
        &self,
        sequence: CheckpointSequenceNumber,
    ) -> AiyResult<Option<CheckpointSummary>> {
        if let Some(BuilderCheckpointSummary { summary, .. }) =
    ...
    pub fn get_last_checkpoint_signature_index(&self) -> AiyResult<u64> {
        Ok(self
            .tables()?
    ...
    pub(crate) fn set_rejection_vote_reason(&self, position: ConsensusPosition, reason: &AiyError) {
