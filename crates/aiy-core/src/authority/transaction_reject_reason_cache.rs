...
/// A cache that maintains rejection reasons (AiyError) when validators cast reject votes for transactions
...
pub(crate) struct TransactionRejectReasonCache {
    cache: RwLock<BTreeMap<ConsensusPosition, AiyError>>,
...
impl TransactionRejectReasonCache {
    ...
    /// uniquely identifies the transaction and the `reason` (AiyError) that caused the transaction to be rejected during voting
    /// should be provided.
    pub fn set_rejection_vote_reason(&self, position: ConsensusPosition, reason: &AiyError) {
    ...
    pub fn get_rejection_vote_reason(&self, position: ConsensusPosition) -> Option<AiyError> {
        debug_assert_eq!(position.epoch, self.epoch, "Epoch mismatch");
...
mod test {
    ...
    async fn test_set_rejection_vote_reason_and_get_reason() {
        ...
        {
            let reason = AiyError::ValidatorHaltedAtEpochEnd;
        ...
        {
            let reason = AiyError::InvalidTransactionDigest;
            cache.set_rejection_vote_reason(position, &reason);
            assert_eq!(cache.get_rejection_vote_reason(position), Some(reason));
        }
        ...
        for round in 0..TOTAL_ROUNDS {
            for transaction_index in 0..5 {
                cache.set_rejection_vote_reason(
                    ...
                    &AiyError::InvalidTransactionDigest,
                );
            }
        }
            ...
            for transaction_index in 0..5 {
                ...
                if round <= 2 {
                    ...
                    assert_eq!(
                        ...
                        Some(AiyError::InvalidTransactionDigest)
                    );
                }
            }
        }
