...
use aiy_types::base_types::ConsensusObjectSequenceKey;
...
use aiy_types::transaction::{SharedInputObject, TransactionDataAPI, TransactionKey};
use aiy_types::AIY_ACCUMULATOR_ROOT_OBJECT_ID;
use aiy_types::{base_types::SequenceNumber, error::AiyResult, AIY_RANDOMNESS_STATE_OBJECT_ID};
use tracing::trace;
        ...
        T: AsTx,
    {
        match self {
            ...
            Schedulable::AccumulatorSettlement(_, _) => {
                Either::Right(std::iter::once(SharedInputObject {
                    id: AIY_ACCUMULATOR_ROOT_OBJECT_ID,
                    initial_shared_version: epoch_store
                        .epoch_start_config()
...
impl SharedObjVerManager {
    ...
    ) -> AiyResult<ConsensusSharedObjVerAssignment>
    where
        T: AsTx + 'a,
    {
            ...
            assignable.as_tx().and_then(|tx| {
                if tx.transaction_data().has_funds_withdrawals() {
                    ...
                    let accumulator_version = *shared_input_next_versions
                        .get(&(AIY_ACCUMULATOR_ROOT_OBJECT_ID, accumulator_initial_version))
            ...
            } in shared_input_objects.iter()
            {
                let assigned_version = match cancellation_info {
                    ...
                    Some(CancelConsensusCertificateReason::DkgFailed) => {
                        if id == &AIY_RANDOMNESS_STATE_OBJECT_ID {
...
mod tests {
    ...
    use aiy_types::crypto::{get_account_key_pair, RandomnessRound};
    use aiy_types::digests::ObjectDigest;
    use aiy_types::effects::TestEffectsBuilder;
    ...
    use aiy_types::transaction::{ObjectArg, SenderSignedData, VerifiedTransaction};

    use aiy_types::gas_coin::GAS;
    use aiy_types::transaction::FundsWithdrawalArg;
    use aiy_types::type_input::TypeInput;
    use aiy_types::{AIY_ACCUMULATOR_ROOT_OBJECT_ID, AIY_RANDOMNESS_STATE_OBJECT_ID};
    ...
    async fn test_assign_versions_from_consensus_with_randomness() {
        ...
        assert_eq!(
            epoch_store
                .get_next_object_version(&AIY_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version)
                .unwrap(),
            randomness_obj_version
        );
        ...
        assert_eq!(
            ...
            HashMap::from([(
                (AIY_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version),
    ...
    async fn test_assign_versions_from_consensus_with_withdraws_simple() {
        ...
        let acc_version = ctx
            .authority
            .get_object(&AIY_ACCUMULATOR_ROOT_OBJECT_ID)
            .await
            .unwrap()
            .version();
            ...
            ConsensusSharedObjVerAssignment {
                assigned_versions: AssignedTxAndVersions::new(vec![
                    ...
                    (
                        ...
                        AssignedVersions {
                            shared_object_versions: vec![(
                                (AIY_ACCUMULATOR_ROOT_OBJECT_ID, acc_version),
                                acc_version
                            )],
