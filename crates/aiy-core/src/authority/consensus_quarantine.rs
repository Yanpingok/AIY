...
use aiy_types::base_types::{AuthorityName, SequenceNumber};
...
impl ConsensusOutputCache {
    pub(crate) fn new(
        ...
        assert!(
            ...
            "This version of aiy-node can only run after data quarantining has been enabled. Please run version 1.45.0 or later to the end of the current epoch and retry"
        );
...
impl ConsensusOutputQuarantine {
    ...
    pub(super) fn commit(&mut self, epoch_store: &AuthorityPerEpochStore) -> AiyResult {
...
impl ConsensusOutputQuarantine {
    ...
    ) -> AiyResult<Vec<Option<SequenceNumber>>> {
    ...
    ) -> AiyResult<impl IntoIterator<Item = (ObjectID, u64)>> {
