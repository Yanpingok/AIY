...
use aiy_types::aiy_system_state::get_aiy_system_state;
use aiy_types::{base_types::SequenceNumber, fp_bail, fp_ensure};
...
use aiy_types::effects::{TransactionEffects, TransactionEvents};
use aiy_types::gas_coin::TOTAL_SUPPLY_MIST;

const NUM_SHARDS: usize = 4096;

struct AuthorityStoreMetrics {
    aiy_conservation_check_latency: IntGauge,
    aiy_conservation_live_object_count: IntGauge,
    aiy_conservation_live_object_size: IntGauge,
    aiy_conservation_imbalance: IntGauge,
    aiy_conservation_storage_fund: IntGauge,
    aiy_conservation_storage_fund_imbalance: IntGauge,
...
impl AuthorityStoreMetrics {
    pub fn new(registry: &Registry) -> Self {
        Self {
            aiy_conservation_check_latency: register_int_gauge_with_registry!(
                "aiy_conservation_check_latency",
                "Number of seconds took to scan all live objects in the store for AIY conservation check",
                registry,
            ).unwrap(),
            aiy_conservation_live_object_count: register_int_gauge_with_registry!(
                "aiy_conservation_live_object_count",
                "Number of live objects in the store",
                registry,
            ).unwrap(),
            aiy_conservation_live_object_size: register_int_gauge_with_registry!(
                "aiy_conservation_live_object_size",
                "Size in bytes of live objects in the store",
                registry,
            ).unwrap(),
            aiy_conservation_imbalance: register_int_gauge_with_registry!(
                "aiy_conservation_imbalance",
                "Total amount of AIY in the network - 10B * 10^9. This delta shows the amount of imbalance",
                registry,
            ).unwrap(),
            aiy_conservation_storage_fund: register_int_gauge_with_registry!(
                "aiy_conservation_storage_fund",
                "Storage Fund pool balance (only includes the storage fund proper that represents object storage)",
                registry,
            ).unwrap(),
            aiy_conservation_storage_fund_imbalance: register_int_gauge_with_registry!(
                "aiy_conservation_storage_fund_imbalance",
...
/// S is a template on Authority signature state. This allows AiyDataStore to be used on either
/// authorities or non-authorities. Specifically, when storing transactions and effects,
/// S allows AiyDataStore to either store the authority signed version or unsigned version.
pub struct AuthorityStore {
    ...
    /// Whether to enable expensive AIY conservation check at epoch boundaries.
    enable_epoch_aiy_conservation_check: bool,
...
impl AuthorityStore {
    ...
    pub async fn open(
        ...
        let enable_epoch_aiy_conservation_check = config
            .expensive_safety_check_config
            .enable_epoch_aiy_conservation_check();
        ...
        let epoch_start_configuration = if perpetual_tables.database_is_empty()? {
            ...
            let epoch_start_configuration = EpochStartConfiguration::new(
                genesis.aiy_system_object().into_epoch_start_state(),
    ...
    pub fn get_recovery_epoch_at_restart(&self) -> AiyResult<EpochId> {
    ...
    ) -> AiyResult<Option<TransactionEffects>> {
    ...
    pub fn effects_exists(&self, effects_digest: &TransactionEffectsDigest) -> AiyResult<bool> {
        self.perpetual_tables
            .effects
    ...
    pub fn is_tx_already_executed(&self, digest: &TransactionDigest) -> AiyResult<bool> {
        Ok(self
            .perpetual_tables
            .executed_effects
            .contains_key(digest)?)
    ...
    pub fn get_latest_marker(
        ...
        epoch_id: EpochId,
    ) -> AiyResult<Option<(SequenceNumber, MarkerValue)>> {
    ...
    pub async fn notify_read_root_state_hash(
        &self,
        epoch: EpochId,
    ) -> AiyResult<(CheckpointSequenceNumber, GlobalStateHash)> {
    ...
    pub fn deprecated_get_transaction_checkpoint(
        &self,
        digest: &TransactionDigest,
    ) -> AiyResult<Option<(EpochId, CheckpointSequenceNumber)>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .get(digest)?)
    ...
    pub fn deprecated_multi_get_transaction_checkpoint(
        &self,
        digests: &[TransactionDigest],
    ) -> AiyResult<Vec<Option<(EpochId, CheckpointSequenceNumber)>>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .multi_get(digests)?
    ...
    pub fn database_is_empty(&self) -> AiyResult<bool> {
    ...
    pub fn multi_object_exists_by_key(&self, object_keys: &[ObjectKey]) -> AiyResult<Vec<bool>> {
        Ok(self
            .perpetual_tables
            .objects
    ...
    fn get_object_ref_prior_to_key(
        ...
        object_id: &ObjectID,
        version: VersionNumber,
    ) -> Result<Option<ObjectRef>, AiyError> {
    ...
    ) -> Result<Vec<Option<Object>>, AiyError> {
        let wrappers = self
            .perpetual_tables
            .objects
    ...
    /// Get many objects
    pub fn get_objects(&self, objects: &[ObjectID]) -> Result<Vec<Option<Object>>, AiyError> {
    ...
    pub(crate) fn insert_genesis_object(&self, object: Object) -> AiyResult {
    ...
    fn insert_object_direct(&self, object_ref: ObjectRef, object: &Object) -> AiyResult {
    ...
    pub(crate) fn bulk_insert_genesis_objects(&self, objects: &[Object]) -> AiyResult<()> {
    ...
    pub fn bulk_insert_live_objects(
        ...
        if *expected_sha3_digest != sha3_digest {
            ...
            return Err(AiyError::from("Sha does not match"));
        }
        batch.write()?;
        Ok(())
    }
    ...
    pub fn get_epoch_start_configuration(&self) -> AiyResult<Option<EpochStartConfiguration>> {
    ...
    pub(crate) fn persist_transaction(&self, tx: &VerifiedExecutableTransaction) -> AiyResult {
        ...
        for (live_marker, lock, obj_ref) in izip!(
            ...
            if let Some(previous_tx_digest) = &lock {
                if previous_tx_digest == &tx_digest {
                    ...
                    return Err(AiyError::ObjectLockConflict {
                        obj_ref: *obj_ref,
    ...
    pub fn check_owned_objects_are_live(&self, objects: &[ObjectRef]) -> AiyResult {
        let locks = self
            .perpetual_tables
            .live_owned_object_markers
            .multi_get(objects)?;
    ...
    /// Returns AiyError::ObjectLockAlreadyInitialized if the lock already exists and is locked to a transaction
    ...
    pub fn initialize_live_object_markers(
        ...
        if !is_force_reset {
            ...
            if !existing_live_object_markers.is_empty() {
                ...
                return Err(AiyError::ObjectLockAlreadyInitialized {
    ...
    pub fn get_latest_object_or_tombstone(
        &self,
        object_id: ObjectID,
    ) -> Result<Option<(ObjectKey, ObjectOrTombstone)>, AiyError> {
    ...
    /// If the intent is for testing, you can use AuthorityState:: get_aiy_system_state_object_for_testing.
    pub fn get_aiy_system_state_object_unsafe(&self) -> AiyResult<AiySystemState> {
        get_aiy_system_state(self.perpetual_tables.as_ref())
    }

    pub fn expensive_check_aiy_conservation<T>(
        self: &Arc<Self>,
        type_layout_store: T,
    ...
    {
        if !self.enable_epoch_aiy_conservation_check {
            return Ok(());
        }

        let executor = old_epoch_store.executor();
        info!("Starting AIY conservation check. This may take a while..");
        let cur_time = Instant::now();
        let mut pending_objects = vec![];
        let mut count = 0;
        let mut size = 0;
        let (mut total_aiy, mut total_storage_rebate) = thread::scope(|s| {
                        ...
                        if count % 1_000_000 == 0 {
                            ...
                            pending_tasks.push(s.spawn(move || {
                                ...
                                for object in task_objects {
                                    ...
                                    // get_total_aiy includes storage rebate, however all storage rebate is
                                    // also stored in the storage fund, so we need to subtract it here.
                                    total_aiy +=
                                        object.get_total_aiy(layout_resolver.as_mut()).unwrap()
                                            - object.storage_rebate;
                                }
                                if count % 50_000_000 == 0 {
                                    info!("Processed {} objects", count);
                                }
                                (total_aiy, total_storage_rebate)
                            }));
                        }
                    }
        ...
        for object in pending_objects {
            total_storage_rebate += object.storage_rebate;
            total_aiy +=
                object.get_total_aiy(layout_resolver.as_mut()).unwrap() - object.storage_rebate;
        }
        info!(
        ...
        let system_state = self
            ...
            .expect("Reading aiy system state object cannot fail")
            .into_aiy_system_state_summary();
        let storage_fund_balance = system_state.storage_fund_total_object_storage_rebates;
        info!(
            "Total AIY amount in the network: {}, storage fund balance: {}, total storage rebate: {} at beginning of epoch {}",
            total_aiy, storage_fund_balance, total_storage_rebate, system_state.epoch
        );
        ...
        self.metrics
            .aiy_conservation_imbalance
            .set((total_aiy as i128 - TOTAL_SUPPLY_MIST as i128) as i64);
        ...
        if let Some(expected_aiy) = self
            .perpetual_tables
            .expected_network_aiy_amount
            .get(&())
            .expect("DB read cannot fail")
        {
            fp_ensure!(
                total_aiy == expected_aiy,
                AiyError::from(
                    format!(
                        "Inconsistent state detected at epoch {}: total aiy: {}, expecting {}",
                        system_state.epoch, total_aiy, expected_aiy
                    )
                    .as_str()
                )
            );
...
impl GlobalStateHashStore for AuthorityStore {
    ...
    fn get_root_state_hash_for_highest_epoch(
        &self,
    ) -> AiyResult<Option<(EpochId, (CheckpointSequenceNumber, GlobalStateHash))>> {
        Ok(self
            .perpetual_tables
            .root_state_hash_by_epoch
...
pub type AiyLockResult = AiyResult<ObjectLockStatus>;
