...
use aiy_protocol_config::{ExecutionTimeEstimateParams, PerObjectCongestionControlMode};
use aiy_types::{
    base_types::ObjectID,
    committee::Committee,
    error::AiyError,
...
impl ExecutionTimeObserver {
    pub fn spawn(
        ...
        let mut observer = Self {
            ...
            next_generation_number: SystemTime::now()
                ...
                .expect("Aiy did not exist prior to 1970")
                .as_micros()
                .try_into()
                .expect("This build of aiy is not supported in the year 500,000"),
        };
    ...
    fn share_observations(&mut self, to_share: Vec<(ExecutionTimeObservationKey, Duration)>) {
        ...
        if let Err(e) = self.consensus_adapter.submit_best_effort(
            ...
            if !matches!(e, AiyError::EpochEnded(_)) {
                epoch_store
                    .metrics
                    .epoch_execution_time_observations_dropped
...
// Key used to save StoredExecutionTimeObservations in the Aiy system state object's
// `extra_fields` Bag.
...
mod tests {
    ...
    use aiy_types::transaction::{Argument, CallArg, ObjectArg, ProgrammableMoveCall};
    use {
        rand::{Rng, SeedableRng},
        aiy_protocol_config::ProtocolVersion,
        aiy_types::supported_protocol_versions::Chain,
    };
