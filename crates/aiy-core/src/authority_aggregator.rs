...
use aiy_authority_aggregation::ReduceOutput;
...
use aiy_types::crypto::{AuthorityPublicKeyBytes, AuthoritySignInfo};
use aiy_types::error::UserInputError;
use aiy_types::message_envelope::Message;
use aiy_types::object::Object;
use aiy_types::quorum_driver_types::{GroupedErrors, QuorumDriverResponse};
...
use aiy_types::committee::{CommitteeWithNetworkMetadata, StakeUnit};
...
use aiy_types::messages_safe_client::PlainTransactionInfoResponse;
...
pub fn group_errors(errors: Vec<(AiyError, Vec<AuthorityName>, StakeUnit)>) -> GroupedErrors {
...
struct ProcessTransactionState {
    ...
    errors: Vec<(AiyError, Vec<AuthorityName>, StakeUnit)>,
    // This is exclusively non-retryable stake.
    non_retryable_stake: StakeUnit,
    // This includes both object and package not found aiy errors.
...
impl ProcessTransactionState {
    pub fn record_conflicting_transaction_if_any(
        ...
        if let AiyError::ObjectLockConflict {
            obj_ref,
    ...
    pub fn check_if_error_indicates_tx_finalized_with_different_user_sig(
        ...
        // and return Aiy::FailedToVerifyTxCertWithExecutedEffects.
        ...
        let invalid_sig_stake: StakeUnit = self
            ...
            .filter_map(|(e, _, stake)| {
                if matches!(e, AiyError::FailedToVerifyTxCertWithExecutedEffects { .. }) {
                    Some(stake)
                } else {
                    None
                }
...
struct ProcessCertificateState {
    ...
    non_retryable_stake: StakeUnit,
    non_retryable_errors: Vec<(AiyError, Vec<AuthorityName>, StakeUnit)>,
    retryable_errors: Vec<(AiyError, Vec<AuthorityName>, StakeUnit)>,
    // As long as none of the exit criteria are met we consider the state retryable
    // 1) >= 2f+1 signatures
    // 2) >= f+1 non-retryable errors
    retryable: bool,
...
impl AuthorityAggregator<NetworkAuthorityClient> {
    ...
    pub fn new_from_epoch_start_state(
        ...
        let committee = epoch_start_state.get_aiy_committee_with_network_metadata();
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn get_latest_object_version_for_testing(
        ...
        let result = quorum_map_then_reduce_with_timeout(
            ...
            .await.map_err(|_state| AiyError::from(UserInputError::ObjectNotFound {
                object_id,
                version: None,
            }))?;
        Ok(result.0)
    ...
    pub async fn get_latest_system_state_object_for_testing(
        &self,
    ) -> anyhow::Result<AiySystemState> {
        #[derive(Debug, Default)]
        struct State {
            latest_system_state: Option<AiySystemState>,
            total_weight: StakeUnit,
        }
        let initial_state = State::default();
        let result = quorum_map_then_reduce_with_timeout(
    ...
    fn record_rpc_error_maybe(metrics: Arc<AuthAggMetrics>, display_name: &str, error: &AiyError) {
        if let AiyError::RpcError(_message, code) = error {
            metrics
                .total_rpc_err
    ...
    fn handle_process_transaction_response(
        ...
        response: AiyResult<PlainTransactionInfoResponse>,
        name: AuthorityName,
        weight: StakeUnit,
    ) -> AiyResult<Option<ProcessTransactionResult>> {
        match response {
    ...
    pub async fn process_certificate(
        ...
        .map_err(|state| {
            ...
            for (aiy_err, _, _) in state.retryable_errors.iter().chain(state.non_retryable_errors.iter()) {
                self
                    .metrics
                    .total_aggregated_err
                    .with_label_values(&[
    ...
    fn handle_process_certificate_response(
        ...
        response: AiyResult<HandleCertificateResponseV3>,
        name: AuthorityName,
    ) -> AiyResult<Option<QuorumDriverResponse>> {
        match response {
        ...
        if let Some(genesis) = self.get_genesis() {
            ...
            state
                ...
                .map(|v| {
                    ...
                    let name = metadata.aiy_pubkey_bytes();
