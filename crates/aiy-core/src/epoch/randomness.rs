...
use aiy_macros::fail_point_if;
use aiy_network::randomness;
use aiy_types::base_types::AuthorityName;
use aiy_types::committee::{Committee, EpochId, StakeUnit};
use aiy_types::crypto::{AuthorityKeyPair, RandomnessRound};
...
impl RandomnessManager {
    ...
    pub async fn start_dkg(&mut self) -> AiyResult {
    ...
    pub(crate) fn reserve_next_randomness(
        ...
        output: &mut ConsensusCommitOutput,
    ) -> AiyResult<Option<RandomnessRound>> {
        let epoch_store = self.epoch_store()?;
    ...
    fn epoch_store(&self) -> AiyResult<Arc<AuthorityPerEpochStore>> {
        self.epoch_store
            .upgrade()
            .ok_or(AiyError::EpochEnded(self.epoch))
    }
...
impl RandomnessReporter {
    ...
    /// the round.
    pub fn notify_randomness_in_checkpoint(&self, round: RandomnessRound) -> AiyResult {
    ...
    async fn test_dkg(version: u64) {
        ...
        for validator in network_config.validator_configs.iter() {
            ...
            let randomness_manager = RandomnessManager::try_new(
                ...
                aiy_network::randomness::Handle::new_stub(),
