...
use aiy_macros::{register_fail_point, sim_test};
use aiy_types::base_types::AiyAddress;
use aiy_types::base_types::TransactionDigest;
use aiy_types::crypto::{deterministic_random_account_key, get_key_pair, AccountKeyPair};
use aiy_types::effects::TransactionEffectsAPI;
use aiy_types::object::{generate_test_gas_objects, Object};
...
async fn test_quorum_driver_object_locked() -> Result<(), anyhow::Error> {
    ...
    let (sender, keypair): (AiyAddress, AccountKeyPair) = deterministic_random_account_key();
