...
use aiy_types::messages_grpc::{HandleCertificateRequestV3, TxType};
...
use aiy_types::transaction::{CertifiedTransaction, Transaction};
...
impl<A: Clone> QuorumDriver<A> {
    ...
    async fn enqueue_task(&self, task: QuorumDriverTask) -> AiyResult<()> {
        self.task_sender
            .send(task.clone())
            .await
            ...
            .map_err(|e| AiyError::QuorumDriverCommunicationError {
                error: e.to_string(),
            })
    }
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn submit_transaction(
        &self,
        request: ExecuteTransactionRequestV3,
    ) -> AiyResult<Registration<TransactionDigest, QuorumDriverResult>> {
