...
use aiy_macros::{fail_point, fail_point_arg, fail_point_if};
...
use aiy_types::{
    ...
    aiy_system_state::epoch_start_aiy_system_state::EpochStartSystemStateTrait,
...
impl<C: CheckpointServiceNotify + Send + Sync> ConsensusHandler<C> {
    ...
    async fn handle_consensus_commit_v2(&mut self, consensus_commit: impl ConsensusCommitAPI) {
        ...
        fail_point_if!("correlated-crash-after-consensus-commit-boundary", || {
            ...
            if aiy_simulator::random::deterministic_probability_once(&key, 0.01) {
    ...
    async fn handle_consensus_commit(&mut self, consensus_commit: impl ConsensusCommitAPI) {
        ...
        //   setting `AIY_USE_NEW_COMMIT_HANDLER` in the environment. This is so that we can test the
...
mod tests {
    ...
    use aiy_types::{
        ...
        base_types::{random_object_ref, AuthorityName, FullObjectRef, ObjectID, AiyAddress},
        committee::Committee,
    ...
    async fn test_checkpoint_signature_dedup() {
        ...
        use aiy_types::messages_consensus::ConsensusTransactionKey as CK;
    ...
    async fn test_verify_consensus_transaction_filters_mismatched_authorities() {
        ...
        let (_, wrong_keypair) = aiy_types::crypto::get_authority_key_pair();
