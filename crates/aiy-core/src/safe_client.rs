...
use aiy_types::effects::{SignedTransactionEffects, TransactionEffectsAPI, TransactionEvents};
...
use aiy_types::{base_types::*, committee::*, fp_ensure};
use aiy_types::{
...
impl<C: Clone> SafeClient<C> {
    ...
    fn get_committee(&self, epoch_id: &EpochId) -> AiyResult<Arc<Committee>> {
        self.committee_store
            .get_committee(epoch_id)?
            .ok_or(AiyError::MissingCommitteeAtEpoch(*epoch_id))
    }
    ...
    ) -> AiyResult<SignedTransactionEffects> {
    ...
    ) -> AiyResult<PlainTransactionInfoResponse> {
        fp_ensure!(
                ...
                match cert_opt {
                    Some(cert) => {
                        ...
                        ct.verify_committee_sigs_only(&committee).map_err(|e| {
                            AiyError::FailedToVerifyTxCertWithExecutedEffects {
    ...
    ) -> AiyResult<VerifiedObjectInfoResponse> {
    ...
    C: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    /// Initiate a new transfer to a Aiy or Primary account.
    pub async fn handle_transaction(
        &self,
        transaction: Transaction,
        client_addr: Option<SocketAddr>,
    ) -> Result<PlainTransactionInfoResponse, AiyError> {
    ...
    ) -> AiyResult<HandleCertificateResponseV2> {
        let signed_effects =
        ...
        match (events, events_digest) {
            ...
            (Some(events), None) => {
                if !events.data.is_empty() {
                    Err(AiyError::ByzantineAuthoritySuspicion {
                        authority: self.address,
    ...
    fn verify_objects<I>(&self, objects: &Option<Vec<Object>>, expected_refs: I) -> AiyResult
    where
        I: IntoIterator<Item = (ObjectID, ObjectRef)>,
    {
        if let Some(objects) = objects {
            ...
            for object in objects {
                ...
                {
                    return Err(AiyError::ByzantineAuthoritySuspicion {
                        authority: self.address,
    ...
    ) -> AiyResult<HandleCertificateResponseV3> {
    ...
    pub async fn handle_object_info_request(
        &self,
        request: ObjectInfoRequest,
    ) -> Result<VerifiedObjectInfoResponse, AiyError> {
    ...
    fn verify_checkpoint_sequence(
        ...
        if let (Some(e), Some(o)) = (expected_seq, observed_seq) {
            fp_ensure!(
                e == o,
                AiyError::from("Expected checkpoint number doesn't match with returned")
            );
        }
        Ok(())
    }
    ...
    fn verify_contents_exist<T, O>(
        ...
        match (request_content, checkpoint, contents) {
            ...
            (true, Some(_), None) | (false, _, Some(_)) | (_, None, Some(_)) => Err(
                AiyError::from("Checkpoint contents inconsistent with request"),
            ),
            _ => Ok(()),
        }
    }
    ...
    #[instrument(level = "trace", skip_all, fields(authority = ?self.address.concise()))]
    pub async fn handle_system_state_object(&self) -> Result<AiySystemState, AiyError> {
        self.authority_client
