...
use aiy_types::committee::{Committee, CommitteeTrait, StakeUnit};
use aiy_types::crypto::{AuthorityQuorumSignInfo, AuthoritySignInfo, AuthoritySignInfoTrait};
use aiy_types::error::{AiyError, AiyResult};
use aiy_types::message_envelope::{Envelope, Message};
use tracing::warn;
...
impl<S: Clone + Eq, const STRENGTH: bool> StakeAggregator<S, STRENGTH> {
    ...
    pub fn insert_generic(
        ...
        match self.data.entry(authority) {
            Entry::Occupied(oc) => {
                return InsertResult::Failed {
                    error: AiyError::StakeAggregatorRepeatedSigner {
                        signer: authority,
        ...
        if votes > 0 {
            ...
            InsertResult::Failed {
                error: AiyError::InvalidAuthenticator,
            }
        }
    }
