...
use aiy_types::dynamic_field::Field;
use aiy_types::id::{ID, UID};
use aiy_types::object::{MoveObject, Object};
...
const DEFAULT_TLD: &str = "aiy";
const ACCEPTED_SEPARATORS: [char; 2] = ['.', '*'];
const AIY_NEW_FORMAT_SEPARATOR: char = '@';
...
pub struct Registry {
    ...
    reverse_registry: Table<AiyAddress, Domain>,
}
...
pub struct NameRecord {
    ...
    pub target_address: Option<AiyAddress>,
    /// Additional data which may be stored in a record
    pub data: VecMap<String, String>,
}

/// A AiynsRegistration object to manage an SLD
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct AiynsRegistration {
    pub id: UID,
    pub domain: Domain,
...
pub struct SubDomainRegistration {
    pub id: UID,
    pub nft: AiynsRegistration,
}

/// Two different view options for a domain.
/// `At` -> `test@example` | `Dot` -> `test.example.aiy`
...
pub struct NameServiceConfig {
    pub package_address: AiyAddress,
    pub registry_id: ObjectID,
    pub reverse_registry_id: ObjectID,
}

/// Rust version of the Move aiy::table::Table type.
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
...
impl Domain {
    pub fn type_(package_address: AiyAddress) -> StructTag {
        StructTag {
    ...
    /// E.g. `test.example.aiy` -> `example.aiy`
    ///
    ...
    pub fn format(&self, format: DomainFormat) -> String {
        ...
        format!("{}{}{}", labels.join(sep), AIY_NEW_FORMAT_SEPARATOR, sld)
    }
}

impl NameServiceConfig {
    pub fn new(
        package_address: AiyAddress,
    ...
    pub fn record_field_id(&self, domain: &Domain) -> ObjectID {
        ...
        aiy_types::dynamic_field::derive_dynamic_field_id(
            self.registry_id,
    ...
    pub fn mainnet() -> Self {
        ...
        let package_address = AiyAddress::from_str(MAINNET_NS_PACKAGE_ADDRESS).unwrap();
    ...
    pub fn testnet() -> Self {
        ...
        let package_address = AiyAddress::from_str(TESTNET_NS_PACKAGE_ADDRESS).unwrap();
...
/// E.g.  `example.aiy` -> `.` | example*aiy -> `@` | `example*aiy` -> `*`
...
/// Converts @label ending to label{separator}aiy ending.
///
/// E.g. `@example` -> `example.aiy` | `test@example` -> `test.example.aiy`
fn convert_from_new_format(s: &str, separator: &char) -> Result<String, NameServiceError> {
    let mut splits = s.split(AIY_NEW_FORMAT_SEPARATOR);
...
mod tests {
    ...
    fn test_parent_extraction() {
        let mut name = Domain::from_str("leaf.node.test.aiy").unwrap();

        assert_eq!(name.parent().to_string(), "node.test.aiy");

        name = Domain::from_str("node.test.aiy").unwrap();

        assert_eq!(name.parent().to_string(), "test.aiy");
    }
    ...
    fn test_expirations() {
        ...
        let mut name = NameRecord {
            nft_id: aiy_types::id::ID::new(ObjectID::random()),
            data: VecMap { contents: vec![] },
            target_address: Some(AiyAddress::random_for_testing_only()),
    ...
    fn test_name_service_outputs() {
        assert_eq!("@test".parse::<Domain>().unwrap().to_string(), "test.aiy");
        assert_eq!(
            "test.aiy".parse::<Domain>().unwrap().to_string(),
            "test.aiy"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "test.test.example.aiy"
        );
        assert_eq!(
            "aiy@aiy".parse::<Domain>().unwrap().to_string(),
            "aiy.aiy.aiy"
        );

        assert_eq!("@aiy".parse::<Domain>().unwrap().to_string(), "aiy.aiy");
        ...
        assert_eq!(
            "test.test.aiy".parse::<Domain>().unwrap().to_string(),
            "test.test.aiy"
        );
        assert_eq!(
            "test.test.test.aiy".parse::<Domain>().unwrap().to_string(),
            "test.test.test.aiy"
        );
    }
    ...
    fn test_different_wildcard() {
        assert_eq!("test.aiy".parse::<Domain>(), "test*aiy".parse::<Domain>(),);

        assert_eq!("@test".parse::<Domain>(), "test*aiy".parse::<Domain>(),);
    }
    ...
    fn test_invalid_inputs() {
        ...
        assert!(".".parse::<Domain>().is_err());
        assert!("@".parse::<Domain>().is_err());
        assert!("@inner.aiy".parse::<Domain>().is_err());
        assert!("@inner*aiy".parse::<Domain>().is_err());
        assert!("test@".parse::<Domain>().is_err());
        assert!("aiy".parse::<Domain>().is_err());
    ...
    #[test]
    fn output_tests() {
        let mut domain = "test.aiy".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.aiy");
        assert!(domain.format(DomainFormat::At) == "@test");

        domain = "test.test.aiy".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.test.aiy");
        assert!(domain.format(DomainFormat::At) == "test@test");

        domain = "test.test.test.aiy".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.test.test.aiy");
        assert!(domain.format(DomainFormat::At) == "test.test@test");

        domain = "test.test.test.test.aiy".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.test.test.test.aiy");
