...
impl MoveObject {
    ...
    pub fn get_layout(&self, resolver: &impl GetModule) -> Result<MoveStructLayout, AiyError> {
    ...
    pub fn to_move_struct(&self, layout: &MoveStructLayout) -> Result<MoveStruct, AiyError> {
    ...
    /// Get the total amount of AIY embedded in `self`. Intended for testing purposes
    pub fn get_total_aiy(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, AiyError> {
        if self.type_.is_gas_coin() {
            let balance = self.get_coin_value_unsafe();
            Ok(balance)
        } else if self.type_.coin_type_maybe().is_some() {
            // It's a coin, but its not AIY
            Ok(0)
        } else if self.type_.is_aiy_balance_accumulator_field() {
            ...
            // anywhere, which is 10B for AIY.
            assert!(
                v.value <= u64::MAX as u128,
                "AIY balance cannot exceed u64::MAX"
            );
            Ok(v.value as u64)
        } else {
...
pub enum Owner {
    ...
    /// The object ID is converted to AiyAddress as AiyAddress is universal.
    ObjectOwner(AiyAddress),
...
impl Owner {
    ...
    pub fn get_address_owner_address(&self) -> AiyResult<AiyAddress> {
        match self {
            Self::AddressOwner(address) => Ok(*address),
            Self::Shared { .. }
            | Self::Immutable
            | Self::ObjectOwner(_)
            | Self::ConsensusAddressOwner { .. } => Err(AiyError::UnexpectedOwnerType),
        }
    }
    ...
    // type is AiyAddress.
    pub fn get_owner_address(&self) -> AiyResult<AiyAddress> {
        match self {
            Self::AddressOwner(address)
            | Self::ObjectOwner(address)
            | Self::ConsensusAddressOwner { owner: address, .. } => Ok(*address),
            Self::Shared { .. } | Self::Immutable => Err(AiyError::UnexpectedOwnerType),
        }
    }
...
impl PartialEq<ObjectID> for Owner {
    fn eq(&self, other: &ObjectID) -> bool {
        let other_id: AiyAddress = (*other).into();
        match self {
...
fn is_object_debug_verbose() -> bool {
    static AIY_OBJECT_DEBUG_VERBOSE: Lazy<bool> =
        Lazy::new(|| std::env::var("AIY_OBJECT_DEBUG_VERBOSE").is_ok());
    *AIY_OBJECT_DEBUG_VERBOSE
}
...
impl ObjectInner {
    ...
    pub fn get_single_owner(&self) -> Option<AiyAddress> {
    ...
    pub fn transfer(&mut self, new_owner: AiyAddress) {
    ...
    pub fn get_layout(
        &self,
        resolver: &impl GetModule,
    ) -> Result<Option<MoveStructLayout>, AiyError> {
        match &self.data {
    ...
    pub fn get_move_template_type(&self) -> AiyResult<TypeTag> {
        let move_struct = self.data.struct_tag().ok_or_else(|| AiyError::TypeError {
...
impl Object {
    /// Get the total amount of AIY embedded in `self`, including both Move objects and the storage rebate
    pub fn get_total_aiy(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, AiyError> {
        Ok(self.storage_rebate
            + match &self.data {
                Data::Move(m) => m.get_total_aiy(layout_resolver)?,
                Data::Package(_) => 0,
            })
    }
    ...
    pub fn with_id_owner_gas_for_testing(id: ObjectID, owner: AiyAddress, gas: u64) -> Self {
        let data = Data::Move(MoveObject {
            type_: GasCoin::type_().into(),
            has_public_transfer: true,
            version: OBJECT_START_VERSION,
    ...
    pub fn with_id_owner_for_testing(id: ObjectID, owner: AiyAddress) -> Self {
    ...
    pub fn with_owner_for_testing(owner: AiyAddress) -> Self {
    ...
    pub fn new_gas_with_balance_and_owner_for_testing(value: u64, owner: AiyAddress) -> Self {
...
mod tests {
    ...
    use crate::{
        base_types::{ObjectID, AiyAddress, TransactionDigest},
        gas_coin::GasCoin,
    };
    ...
    fn test_object_digest_and_serialized_format() {
        ...
        let o = Object::new_move(
            g,
            Owner::AddressOwner(AiyAddress::ZERO),
