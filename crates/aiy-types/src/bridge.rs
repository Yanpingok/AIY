...
use crate::error::AiyResult;
use crate::object::Owner;
use crate::storage::ObjectStore;
use crate::aiy_serde::BigInt;
use crate::aiy_serde::Readable;
use crate::versioned::Versioned;
use crate::AIY_BRIDGE_OBJECT_ID;
use crate::{
...
pub const BRIDGE_CREATE_ADD_TOKEN_ON_AIY_MESSAGE_FUNCTION_NAME: &IdentStr =
    ident_str!("create_add_tokens_on_aiy_message");
...
impl BridgeChainId {
    pub fn is_aiy_chain(&self) -> bool {
        matches!(
            self,
            BridgeChainId::AiyMainnet | BridgeChainId::AiyTestnet | BridgeChainId::AiyCustom
        )
    }
}
...
) -> AiyResult<Option<SequenceNumber>> {
    Ok(object_store
        .get_object(&AIY_BRIDGE_OBJECT_ID)
...
/// Rust version of the Move aiy::bridge::Bridge type
...
pub trait BridgeTrait {
    ...
    fn frozen(&self) -> bool;
    fn try_into_bridge_summary(self) -> AiyResult<BridgeSummary>;
}
...
pub fn get_bridge_wrapper(object_store: &dyn ObjectStore) -> Result<BridgeWrapper, AiyError> {
    let wrapper = object_store
        .get_object(&AIY_BRIDGE_OBJECT_ID)
        // Don't panic here on None because object_store is a generic store.
        .ok_or_else(|| AiyError::AiyBridgeReadError("BridgeWrapper object not found".to_owned()))?;
    let move_object = wrapper.data.try_as_move().ok_or_else(|| {
        AiyError::AiyBridgeReadError("BridgeWrapper object must be a Move object".to_owned())
    })?;
    let result = bcs::from_bytes::<BridgeWrapper>(move_object.contents())
        .map_err(|err| AiyError::AiyBridgeReadError(err.to_string()))?;
    Ok(result)
}

pub fn get_bridge(object_store: &dyn ObjectStore) -> Result<Bridge, AiyError> {
    ...
    match version {
        1 => {
            let result: BridgeInnerV1 = get_dynamic_field_from_store(object_store, id, &version)
                .map_err(|err| {
                    AiyError::AiyBridgeReadError(format!(
        ...
        _ => Err(AiyError::AiyBridgeReadError(format!(
            "Unsupported AiyBridge version: {}",
            version
        ))),
    }
}
...
pub struct MoveTypeBridgeCommittee {
    ...
    pub member_registrations: VecMap<AiyAddress, MoveTypeCommitteeMemberRegistration>,
...
pub struct BridgeCommitteeSummary {
    ...
    pub member_registration: Vec<(AiyAddress, MoveTypeCommitteeMemberRegistration)>,
    pub last_committee_update_epoch: u64,
}
...
pub fn is_bridge_committee_initiated(object_store: &dyn ObjectStore) -> AiyResult<bool> {
    match get_bridge(object_store) {
        Ok(bridge) => Ok(!bridge.committee().members.contents.is_empty()),
        Err(AiyError::AiyBridgeReadError(..)) => Ok(false),
        Err(other) => Err(other),
    }
}
