...
/// The value should be the same in Aiy committee.
...
impl ConsensusPosition {
    pub fn into_raw(self) -> Result<Bytes, AiyError> {
        bcs::to_bytes(&self)
            .map_err(|e| AiyError::GrpcMessageSerializeError {
...
impl TryFrom<&[u8]> for ConsensusPosition {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        bcs::from_bytes(bytes).map_err(|e| AiyError::GrpcMessageDeserializeError {
            type_info: "ConsensusPosition".to_string(),
            error: e.to_string(),
        })
    }
...
pub struct ConsensusTransaction {
    /// Encodes an u64 unique tracking id to allow us trace a message between Aiy and consensus.
...
impl AuthorityCapabilitiesV1 {
    pub fn new(
        ...
        let generation = SystemTime::now()
            ...
            .as_millis()
            .try_into()
            .expect("This build of aiy is not supported in the year 500,000,000");
        Self {
            authority,
            generation,
            supported_protocol_versions,
