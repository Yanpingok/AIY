...
impl TryFrom<Vec<u8>> for Digest {
    ...
    fn try_from(bytes: Vec<u8>) -> Result<Self, AiyError> {
        let bytes: [u8; 32] =
            <[u8; 32]>::try_from(&bytes[..]).map_err(|_| AiyError::InvalidDigestLength {
                expected: 32,
                actual: bytes.len(),
            })?;
...
pub const AIY_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME: &str =
    "AIY_PROTOCOL_CONFIG_CHAIN_OVERRIDE";

static AIY_PROTOCOL_CONFIG_CHAIN_OVERRIDE: Lazy<Option<Chain>> = Lazy::new(|| {
    if let Ok(s) = env::var(AIY_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME) {
        info!("AIY_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {:?}", s);
        match s.as_str() {
            "mainnet" => Some(Chain::Mainnet),
            "testnet" => Some(Chain::Testnet),
            "" => None,
            _ => panic!("unrecognized AIY_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {s:?}"),
        }
    } else {
        None
    }
...
impl ChainIdentifier {
    ...
    pub fn chain(&self) -> Chain {
        ...
        if let Some(override_chain) = *AIY_PROTOCOL_CONFIG_CHAIN_OVERRIDE {
...
impl TryFrom<&[u8]> for TransactionDigest {
    type Error = crate::error::AiyError;

    fn try_from(bytes: &[u8]) -> Result<Self, crate::error::AiyError> {
        let arr: [u8; 32] = bytes
            .try_into()
            .map_err(|_| crate::error::AiyError::InvalidTransactionDigest)?;
        Ok(Self::new(arr))
    }
}
...
impl CheckpointArtifactsDigest {
    ...
    pub fn from_artifact_digests(digests: Vec<Digest>) -> AiyResult<Self> {
        let bytes =
            bcs::to_bytes(&digests).map_err(|e| AiyError::from(format!("BCS error: {}", e)))?;
...
#[cfg(test)]
mod test {
    use crate::digests::{ChainIdentifier, AIY_PROTOCOL_CONFIG_CHAIN_OVERRIDE};

    fn has_env_override() -> bool {
        AIY_PROTOCOL_CONFIG_CHAIN_OVERRIDE.is_some()
    }
    ...
    fn test_chain_id_mainnet() {
        ...
        assert_eq!(
            ...
            aiy_protocol_config::Chain::Mainnet
        );
    }
    ...
    fn test_chain_id_testnet() {
        ...
        assert_eq!(
            chain_id.unwrap().chain(),
            aiy_protocol_config::Chain::Testnet
        );
    }
