...
use crate::{
    ...
    crypto::DefaultHash,
    error::{ExecutionError, ExecutionErrorKind, AiyError, AiyResult},
    id::{ID, UID},
...
/// Rust representation of upgrade policy constants in `aiy::package`.
#[repr(u8)]
...
/// Rust representation of `aiy::package::UpgradeCap`.
...
/// Rust representation of `aiy::package::UpgradeTicket`.
#[derive(Debug, Serialize, Deserialize)]
...
/// Rust representation of `aiy::package::UpgradeReceipt`.
#[derive(Debug, Serialize, Deserialize)]
...
impl MovePackage {
    ...
    pub fn deserialize_module_by_str(
        ...
        let bytes =
            self.serialized_module_map()
                .get(module)
                .ok_or_else(|| AiyError::ModuleNotFound {
    ...
    pub fn normalize<S: Hash + Eq + Clone + ToString, Pool: normalized::StringPool<String = S>>(
        ...
        binary_config: &BinaryConfig,
        include_code: bool,
    ) -> AiyResult<BTreeMap<String, normalized::Module<S>>> {
