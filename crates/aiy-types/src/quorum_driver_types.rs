...
use crate::error::{ErrorCategory, AiyError};
...
pub enum QuorumDriverError {
    ...
    QuorumDriverInternalError(AiyError),
    #[error("Invalid user signature: {0}.")]
    InvalidUserSignature(AiyError),
    #[error(
...
pub type GroupedErrors = Vec<(AiyError, StakeUnit, Vec<ConciseAuthorityPublicKeyBytes>)>;
