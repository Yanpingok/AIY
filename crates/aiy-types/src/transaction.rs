...
use super::{base_types::*, error::*, AIY_BRIDGE_OBJECT_ID};
...
use crate::crypto::{
    ...
    AuthorityStrongQuorumSignInfo, DefaultHash, Ed25519AiySignature, EmptySignInfo,
    RandomnessRound, Signature, Signer, AiySignatureInner, ToFromBytes,
};
...
use crate::{
    AIY_AUTHENTICATOR_STATE_OBJECT_ID, AIY_CLOCK_OBJECT_ID, AIY_CLOCK_OBJECT_SHARED_VERSION,
    AIY_FRAMEWORK_PACKAGE_ID, AIY_RANDOMNESS_STATE_OBJECT_ID, AIY_SYSTEM_STATE_OBJECT_ID,
    AIY_SYSTEM_STATE_OBJECT_SHARED_VERSION,
};
...
use aiy_protocol_config::{PerObjectCongestionControlMode, ProtocolConfig};
use tap::Pipe;
use tracing::trace;
...
impl EndOfEpochTransactionKind {
        ...
        match self {
            Self::ChangeEpoch(_) => {
                vec![InputObjectKind::SharedMoveObject {
                    id: AIY_SYSTEM_STATE_OBJECT_ID,
                    initial_shared_version: AIY_SYSTEM_STATE_OBJECT_SHARED_VERSION,
                    mutable: true,
                }]
            }
    ...
    fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match self {
            Self::ChangeEpoch(_) => {
                Either::Left(vec![SharedInputObject::AIY_SYSTEM_OBJ].into_iter())
            }
            ...
            Self::BridgeCommitteeInit(bridge_version) => Either::Left(
                vec![
                    ...
                    SharedInputObject::AIY_SYSTEM_OBJ,
                ]
                .into_iter(),
            ),
            Self::StoreExecutionTimeObservations(_) => {
...
impl ObjectArg {
    pub const AIY_SYSTEM_MUT: Self = Self::SharedObject {
        id: AIY_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: AIY_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    };
...
impl ProgrammableTransaction {
    ...
    fn validity_check(&self, config: &ProtocolConfig) -> UserInputResult {
        ...
        if let Some(random_index) = inputs.iter().position(|obj| {
            matches!(obj, CallArg::Object(ObjectArg::SharedObject { id, .. }) if *id == AIY_RANDOMNESS_STATE_OBJECT_ID)
        }) {
            fp_ensure!(
...
impl SharedInputObject {
    pub const AIY_SYSTEM_OBJ: Self = Self {
        id: AIY_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: AIY_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    };
...
impl TransactionKind {
    ...
    pub fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match &self {
            Self::ChangeEpoch(_) => {
                Either::Left(Either::Left(iter::once(SharedInputObject::AIY_SYSTEM_OBJ)))
            }
...
impl TransactionData {
    ...
    pub fn new_with_gas_data(kind: TransactionKind, sender: AiyAddress, gas_data: GasData) -> Self {
        TransactionData::V1(TransactionDataV1 {
            kind,
            sender,
            gas_data,
    ...
    pub fn new_transfer_aiy(
        ...
        Self::new_transfer_aiy_allow_sponsor(
            recipient,
            sender,
            amount,
            gas_payment,
    ...
    pub fn new_transfer_aiy_allow_sponsor(
        recipient: AiyAddress,
        sender: AiyAddress,
        amount: Option<u64>,
        gas_payment: ObjectRef,
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_aiy(recipient, amount);
            builder.finish()
        };
    ...
    pub fn new_pay_aiy(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.pay_aiy(recipients, amounts)?;
            builder.finish()
        };
        Ok(Self::new_programmable(
    ...
    pub fn execution_parts(&self) -> (TransactionKind, AiyAddress, GasData) {
    ...
    pub fn uses_randomness(&self) -> bool {
        self.kind()
            ...
            .any(|obj| obj.id() == AIY_RANDOMNESS_STATE_OBJECT_ID)
    }
...
pub trait TransactionDataAPI {
    ...
    fn signers(&self) -> NonEmpty<AiyAddress>;

    fn gas_data(&self) -> &GasData;

    fn gas_owner(&self) -> AiyAddress;
    ...
    fn sender_mut_for_testing(&mut self) -> &mut AiyAddress;
...
impl Serialize for SenderSignedTransaction {
        ...
        S: serde::Serializer,
    {
        ...
        if self.intent_message().intent != Intent::aiy_transaction() {
...
impl<'de> Deserialize<'de> for SenderSignedTransaction {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if intent_message.intent != Intent::aiy_transaction() {
...
impl SenderSignedTransaction {
    ...
    ) -> AiyResult<BTreeMap<AiyAddress, &GenericSignature>> {
        ...
        for sig in &self.tx_signatures {
            if verify_legacy_zklogin_address {
                ...
                if let GenericSignature::ZkLoginAuthenticator(z) = sig {
                    mapping.insert(AiyAddress::try_from_padded(&z.inputs)?, sig);
                };
            }
...
impl SenderSignedData {
    pub fn new(tx_data: TransactionData, tx_signatures: Vec<GenericSignature>) -> Self {
        Self(SizeOneVec::new(SenderSignedTransaction {
            intent_message: IntentMessage::new(Intent::aiy_transaction(), tx_data),
            tx_signatures,
        }))
    }
    ...
    pub fn serialized_size(&self) -> AiyResult<usize> {
        bcs::serialized_size(self).map_err(|e| AiyError::TransactionSerializationError {
            error: e.to_string(),
        })
    }

    fn check_user_signature_protocol_compatibility(&self, config: &ProtocolConfig) -> AiyResult {
    ...
    pub fn validity_check(&self, context: &TxValidityCheckContext<'_>) -> Result<usize, AiyError> {
        ...
        if match &tx_data.expiration() {
            ...
            return Err(AiyError::TransactionExpired);
        }
...
impl<S> Envelope<SenderSignedData, S> {
    pub fn sender_address(&self) -> AiyAddress {
        self.data().intent_message().value.sender()
    }

    pub fn gas_owner(&self) -> AiyAddress {
...
impl Transaction {
    pub fn from_data_and_signer(
        ...
        let signatures = {
            let intent_msg = IntentMessage::new(Intent::aiy_transaction(), &data);
            signers
                .into_iter()
        ...
        system_transaction
            ...
            .pipe(|data| {
                SenderSignedData::new_from_sender_signature(
                    data,
                    Ed25519AiySignature::from_bytes(&[0; Ed25519AiySignature::LENGTH])
                        .unwrap()
                        .into(),
                )
            })
...
impl SignedTransaction {
    ...
    pub fn try_into_verified_for_testing(
        ...
        committee: &Committee,
        verify_params: &VerifyParams,
    ) -> AiyResult<VerifiedSignedTransaction> {
...
impl CertifiedTransaction {
    ...
    pub fn verify_committee_sigs_only(&self, committee: &Committee) -> AiyResult {
        self.auth_sig().verify_secure(
            self.data(),
            Intent::aiy_app(IntentScope::SenderSignedTransaction),
            committee,
...
// An InputObjects new-type that has been verified by aiy-transaction-checks, and can be
// safely passed to execution.
pub struct CheckedInputObjects(InputObjects);

// DO NOT CALL outside of aiy-transaction-checks, genesis, or replay.
//
// CheckedInputObjects should really be defined in aiy-transaction-checks so that we can
// make public construction impossible. But we can't do that because it would result in circular
// dependencies.
impl CheckedInputObjects {
    // Only called by aiy-transaction-checks.
