...
use crate::base_types::{AuthorityName, ConciseableName, AiyAddress};
use crate::committee::CommitteeTrait;
use crate::committee::{Committee, EpochId, StakeUnit};
use crate::error::{AiyError, AiyResult};
use crate::signature::GenericSignature;
use crate::aiy_serde::{Readable, AiyBitmap};
...
pub const AIY_PRIV_KEY_PREFIX: &str = "aiyprivkey";
...
pub fn generate_proof_of_possession(
    ...
    AuthoritySignature::new_secure(
        &IntentMessage::new(Intent::aiy_app(IntentScope::ProofOfPossession), msg),
        &DEFAULT_EPOCH_ID,
        keypair,
    )
}
...
pub fn verify_proof_of_possession(
    ...
    aiy_address: AiyAddress,
) -> Result<(), AiyError> {
    protocol_pubkey
        .validate()
        .map_err(|_| AiyError::InvalidSignature {
            error: "Fail to validate pubkey".to_string(),
        })?;
    let mut msg = protocol_pubkey.as_bytes().to_vec();
    msg.extend_from_slice(aiy_address.as_ref());
    pop.verify_secure(
        &IntentMessage::new(Intent::aiy_app(IntentScope::ProofOfPossession), msg),
        DEFAULT_EPOCH_ID,
...
// * accounts to interact with Aiy.
// * Currently we support eddsa and ecdsa on Aiy.
//
...
impl AiyKeyPair {
    pub fn public(&self) -> PublicKey {
        match self {
            AiyKeyPair::Ed25519(kp) => PublicKey::Ed25519(kp.public().into()),
            AiyKeyPair::Secp256k1(kp) => PublicKey::Secp256k1(kp.public().into()),
            AiyKeyPair::Secp256r1(kp) => PublicKey::Secp256r1(kp.public().into()),
        }
    }

    pub fn copy(&self) -> Self {
        match self {
            AiyKeyPair::Ed25519(kp) => kp.copy().into(),
            AiyKeyPair::Secp256k1(kp) => kp.copy().into(),
            AiyKeyPair::Secp256r1(kp) => kp.copy().into(),
        }
    }
}

impl Signer<Signature> for AiyKeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        match self {
            AiyKeyPair::Ed25519(kp) => kp.sign(msg),
            AiyKeyPair::Secp256k1(kp) => kp.sign(msg),
            AiyKeyPair::Secp256r1(kp) => kp.sign(msg),
        }
    }
}

impl EncodeDecodeBase64 for AiyKeyPair {
...
impl AiyKeyPair {
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        match self {
            AiyKeyPair::Ed25519(kp) => {
                bytes.extend_from_slice(kp.as_bytes());
            }
            AiyKeyPair::Secp256k1(kp) => {
                bytes.extend_from_slice(kp.as_bytes());
            }
            AiyKeyPair::Secp256r1(kp) => {
                bytes.extend_from_slice(kp.as_bytes());
            }
        }
        bytes
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, eyre::Report> {
        ...
        {
            Ok(x) => match x {
                SignatureScheme::ED25519 => Ok(AiyKeyPair::Ed25519(Ed25519KeyPair::from_bytes(
                    bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                )?)),
                SignatureScheme::Secp256k1 => {
                    Ok(AiyKeyPair::Secp256k1(Secp256k1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
                SignatureScheme::Secp256r1 => {
                    Ok(AiyKeyPair::Secp256r1(Secp256r1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
    ...
    pub fn to_bytes_no_flag(&self) -> Vec<u8> {
        match self {
            AiyKeyPair::Ed25519(kp) => kp.as_bytes().to_vec(),
            AiyKeyPair::Secp256k1(kp) => kp.as_bytes().to_vec(),
            AiyKeyPair::Secp256r1(kp) => kp.as_bytes().to_vec(),
        }
    }

    /// Encode a AiyKeyPair as `flag || privkey` in Bech32 starting with "aiyprivkey" to a string. Note that the pubkey is not encoded.
    pub fn encode(&self) -> Result<String, eyre::Report> {
        Bech32::encode(self.to_bytes(), AIY_PRIV_KEY_PREFIX).map_err(|e| eyre!(e))
    }

    /// Decode a AiyKeyPair from `flag || privkey` in Bech32 starting with "aiyprivkey" to AiyKeyPair. The public key is computed directly from the private key bytes.
    pub fn decode(value: &str) -> Result<Self, eyre::Report> {
        let bytes = Bech32::decode(value, AIY_PRIV_KEY_PREFIX)?;
        Self::from_bytes(&bytes)
    }
}

impl Serialize for AiyKeyPair {
...
impl<'de> Deserialize<'de> for AiyKeyPair {
        ...
        D: Deserializer<'de>,
    {
        ...
        AiyKeyPair::decode_base64(&s).map_err(|e| Error::custom(e.to_string()))
    }
}
...
impl ZkLoginPublicIdentifier {
    ...
    pub fn new(iss: &str, address_seed: &Bn254FrElement) -> AiyResult<Self> {
...
impl PublicKey {
    ...
    pub fn scheme(&self) -> SignatureScheme {
        match self {
            PublicKey::Ed25519(_) => Ed25519AiySignature::SCHEME,
            PublicKey::Secp256k1(_) => Secp256k1AiySignature::SCHEME,
            PublicKey::Secp256r1(_) => Secp256r1AiySignature::SCHEME,
            PublicKey::ZkLogin(_) => SignatureScheme::ZkLoginAuthenticator,
            PublicKey::Passkey(_) => SignatureScheme::PasskeyAuthenticator,
        }
    }

    pub fn from_zklogin_inputs(inputs: &ZkLoginInputs) -> AiyResult<Self> {
...
pub trait AiyAuthoritySignature {
    fn verify_secure<T>(
        &self,
...
impl AiyAuthoritySignature for AuthoritySignature {
...
pub fn get_key_pair<KP: KeypairTraits>() -> (AiyAddress, KP)
where
    <KP as KeypairTraits>::PubKey: AiyPublicKey,
{
...
pub fn random_committee_key_pairs_of_size(size: usize) -> Vec<AuthorityKeyPair> {
    ...
    (0..size)
        .map(|_| {
            ...
            // keys in ConfigBuilder::build (aiy-config/src/network_config_builder). This is because
...
pub fn deterministic_random_account_key() -> (AiyAddress, AccountKeyPair) {
    let mut rng = StdRng::from_seed([0; 32]);
    get_key_pair_from_rng(&mut rng)
}

pub fn get_account_key_pair() -> (AiyAddress, AccountKeyPair) {
    get_key_pair()
}

pub fn get_authority_key_pair() -> (AiyAddress, AuthorityKeyPair) {
    get_key_pair()
}

/// Generate a keypair from the specified RNG (useful for testing with seedable rngs).
pub fn get_key_pair_from_rng<KP: KeypairTraits, R>(csprng: &mut R) -> (AiyAddress, KP)
where
...
// TODO: C-GETTER
pub fn get_key_pair_from_bytes<KP: KeypairTraits>(bytes: &[u8]) -> AiyResult<(AiyAddress, KP)>
where
    <KP as KeypairTraits>::PubKey: AiyPublicKey,
{
    ...
    if bytes.len() != priv_length + pub_key_length {
        return Err(AiyError::KeyConversionError(format!(
    ...
    let sk = <KP as KeypairTraits>::PrivKey::from_bytes(
        bytes
            .get(..priv_length)
            .ok_or(AiyError::InvalidPrivateKey)?,
    )
    .map_err(|_| AiyError::InvalidPrivateKey)?;
    let kp: KP = sk.into();
...
pub enum Signature {
    Ed25519AiySignature,
    Secp256k1AiySignature,
    Secp256r1AiySignature,
}
...
impl AsRef<[u8]> for Signature {
    fn as_ref(&self) -> &[u8] {
        match self {
            Signature::Ed25519AiySignature(sig) => sig.as_ref(),
            Signature::Secp256k1AiySignature(sig) => sig.as_ref(),
            Signature::Secp256r1AiySignature(sig) => sig.as_ref(),
        }
    }
}
impl AsMut<[u8]> for Signature {
    fn as_mut(&mut self) -> &mut [u8] {
        match self {
            Signature::Ed25519AiySignature(sig) => sig.as_mut(),
            Signature::Secp256k1AiySignature(sig) => sig.as_mut(),
            Signature::Secp256r1AiySignature(sig) => sig.as_mut(),
        }
    }
}
...
impl ToFromBytes for Signature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        match bytes.first() {
            Some(x) => {
                if x == &Ed25519AiySignature::SCHEME.flag() {
                    Ok(<Ed25519AiySignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else if x == &Secp256k1AiySignature::SCHEME.flag() {
                    Ok(<Secp256k1AiySignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else if x == &Secp256r1AiySignature::SCHEME.flag() {
                    Ok(<Secp256r1AiySignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else {
                    Err(FastCryptoError::InvalidInput)
                }
            }
...
impl AiyPublicKey for BLS12381PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::BLS12381;
}

//
// Ed25519 Aiy Signature port
//
...
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Ed25519AiySignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
...
impl Default for Ed25519AiySignature {
    fn default() -> Self {
        Self([0; Ed25519PublicKey::LENGTH + Ed25519Signature::LENGTH + 1])
    }
}

impl AiySignatureInner for Ed25519AiySignature {
...
impl AiyPublicKey for Ed25519PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::ED25519;
}

impl ToFromBytes for Ed25519AiySignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
...
impl Signer<Signature> for Ed25519KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Ed25519AiySignature::new(self, msg).into()
    }
}

//
// Secp256k1 Aiy Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash, AsRef, AsMut)]
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Secp256k1AiySignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256k1PublicKey::LENGTH + Secp256k1Signature::LENGTH + 1],
);

impl AiySignatureInner for Secp256k1AiySignature {
...
impl AiyPublicKey for Secp256k1PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::Secp256k1;
}

impl ToFromBytes for Secp256k1AiySignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256k1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256k1AiySignature::new(self, msg).into()
    }
}

//
// Secp256r1 Aiy Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash, AsRef, AsMut)]
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Secp256r1AiySignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256r1PublicKey::LENGTH + Secp256r1Signature::LENGTH + 1],
);

impl AiySignatureInner for Secp256r1AiySignature {
...
impl AiyPublicKey for Secp256r1PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::Secp256r1;
}

impl ToFromBytes for Secp256r1AiySignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256r1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256r1AiySignature::new(self, msg).into()
    }
}
...
//
pub trait AiySignatureInner: Sized + ToFromBytes + PartialEq + Eq + Hash {
    type Sig: Authenticator<PubKey = Self::PubKey>;
    type PubKey: VerifyingKey<Sig = Self::Sig> + AiyPublicKey;
    ...
    fn get_verification_inputs(&self) -> AiyResult<(Self::Sig, Self::PubKey)> {
        let pk = Self::PubKey::from_bytes(self.public_key_bytes())
            .map_err(|_| AiyError::KeyConversionError("Invalid public key".to_string()))?;

        // deserialize the signature
        let signature = Self::Sig::from_bytes(self.signature_bytes()).map_err(|_| {
            AiyError::InvalidSignature {
    ...
    fn new(kp: &Self::KeyPair, message: &[u8]) -> Self {
        ...
        signature_bytes
            .extend_from_slice(&[<Self::PubKey as AiyPublicKey>::SIGNATURE_SCHEME.flag()]);
...
pub trait AiyPublicKey: VerifyingKey {
    const SIGNATURE_SCHEME: SignatureScheme;
}

#[enum_dispatch(Signature)]
pub trait AiySignature: Sized + ToFromBytes {
...
impl<S: AiySignatureInner + Sized> AiySignature for S {
    fn signature_bytes(&self) -> &[u8] {
        ...
        T: Serialize,
    {
        ...
        match scheme {
            ...
            _ => {
                let address = AiyAddress::from(pk);
                if author != address {
                    return Err(AiyError::IncorrectSigner {
                        error: format!(
...
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema)]
pub struct AiyAuthorityStrongQuorumSignInfo {
    pub epoch: EpochId,
...
impl From<&AuthorityStrongQuorumSignInfo> for AiyAuthorityStrongQuorumSignInfo {
...
impl TryFrom<&AiyAuthorityStrongQuorumSignInfo> for AuthorityStrongQuorumSignInfo {
    type Error = FastCryptoError;

    fn try_from(info: &AiyAuthorityStrongQuorumSignInfo) -> Result<Self, Self::Error> {
        Ok(Self {
            epoch: info.epoch,
    ...
    for AuthorityQuorumSignInfo<STRONG_THRESHOLD>
{
    ...
    fn add_to_verification_obligation<'a>(
        ...
        obligation
            .signatures
            .get_mut(message_index)
            .ok_or(AiyError::InvalidAuthenticator)?
        ...
        for authority_index in self.signers_map.iter() {
            ...
            let authority = committee
                ...
                .ok_or_else(|| AiyError::UnknownSigner {
                    signer: None,
        ...
        fp_ensure!(
            ...
            AiyError::CertificateRequiresQuorum
        );
...
impl<const STRONG_THRESHOLD: bool> AuthorityQuorumSignInfo<STRONG_THRESHOLD> {
    ...
    pub fn authorities<'a>(
        &'a self,
        committee: &'a Committee,
    ) -> impl Iterator<Item = AiyResult<&'a AuthorityName>> {
...
/// * We use `BCS` to generate canonical bytes aiytable for hashing and signing.
///
/// # Safety
...
impl<'a> VerificationObligation<'a> {
    ...
    pub fn verify_all(self) -> AiyResult<()> {
...
pub mod bcs_signable_test {
        ...
        T: super::bcs_signable::BcsSignable,
    {
        ...
        let idx = obligation.add_message(
            value,
            0,
            Intent::aiy_app(IntentScope::SenderSignedTransaction),
        );
        (obligation, idx)
    }
}
...
pub enum SignatureScheme {
    ...
    Secp256k1,
    Secp256r1,
    BLS12381, // This is currently not supported for user Aiy Address.
    MultiSig,
...
impl SignatureScheme {
    pub fn flag(&self) -> u8 {
        match self {
            ...
            SignatureScheme::BLS12381 => 0x04, // This is currently not supported for user Aiy Address.
            SignatureScheme::ZkLoginAuthenticator => 0x05,
            SignatureScheme::PasskeyAuthenticator => 0x06,
        }
    }

    pub fn from_flag(flag: &str) -> Result<SignatureScheme, AiyError> {
        let byte_int = flag
            .parse::<u8>()
            .map_err(|_| AiyError::KeyConversionError("Invalid key scheme".to_string()))?;
        Self::from_flag_byte(&byte_int)
    }

    pub fn from_flag_byte(byte_int: &u8) -> Result<SignatureScheme, AiyError> {
        match byte_int {
            ...
            _ => Err(AiyError::KeyConversionError(
