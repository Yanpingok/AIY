...
use crate::{
    ...
    MoveTypeTagTrait, MoveTypeTagTraitGeneric, AIY_ACCUMULATOR_ROOT_ADDRESS,
    AIY_ACCUMULATOR_ROOT_OBJECT_ID, AIY_FRAMEWORK_ADDRESS, AIY_FRAMEWORK_PACKAGE_ID,
};
use move_core_types::{
    ident_str,
    identifier::IdentStr,
...
impl MoveTypeTagTraitGeneric for AccumulatorKey {
    fn get_type_tag(type_params: &[TypeTag]) -> TypeTag {
        TypeTag::Struct(Box::new(StructTag {
            address: AIY_FRAMEWORK_PACKAGE_ID.into(),
...
impl AccumulatorValue {
    pub fn get_field_id(owner: AiyAddress, type_: &TypeTag) -> AiyResult<AccumulatorObjId> {
        if !Balance::is_balance_type(type_) {
            return Err(AiyError::TypeError {
    ...
    pub fn load(
        ...
        let Some(value) = DynamicFieldKey(AIY_ACCUMULATOR_ROOT_OBJECT_ID, key, key_type_tag)
            .into_id_with_bound(version_bound.unwrap_or(SequenceNumber::MAX))?
            .load_object(child_object_resolver)?
    ...
    pub fn load_object(
        ...
        Ok(
            DynamicFieldKey(AIY_ACCUMULATOR_ROOT_OBJECT_ID, key, key_type_tag)
                .into_id_with_bound(version_bound.unwrap_or(SequenceNumber::MAX))?
                .load_object(child_object_resolver)?
                .map(|o| o.as_object()),
        )
    ...
    pub fn create_for_testing(owner: AiyAddress, type_tag: TypeTag, balance: u64) -> Object {
        let key = AccumulatorKey { owner };
        let value = U128 {
        ...
        Object::new_move(
            move_object,
            Owner::ObjectOwner(AIY_ACCUMULATOR_ROOT_ADDRESS.into()),
...
pub(crate) fn is_balance_accumulator_field(s: &StructTag) -> bool {
    s.address == AIY_FRAMEWORK_ADDRESS
