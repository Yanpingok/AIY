...
/// The number of Mist per Aiy token
pub const MIST_PER_AIY: u64 = 1_000_000_000;

/// Total supply denominated in Aiy
pub const TOTAL_SUPPLY_AIY: u64 = 10_000_000_000;

// Note: cannot use checked arithmetic here since `const unwrap` is still unstable.
/// Total supply denominated in Mist
pub const TOTAL_SUPPLY_MIST: u64 = TOTAL_SUPPLY_AIY * MIST_PER_AIY;

pub const GAS_MODULE_NAME: &IdentStr = ident_str!("aiy");
pub const GAS_STRUCT_NAME: &IdentStr = ident_str!("AIY");
...
mod checked {
    ...
    /// Rust version of the Move aiy::coin::Coin<Aiy::aiy::AIY> type
    ...
    impl GasCoin {
        ...
        /// Return `true` if `s` is the type of a gas coin (i.e., 0x2::coin::Coin<0x2::aiy::AIY>)
        pub fn is_gas_coin(s: &StructTag) -> bool {
            Coin::is_coin(s) && s.type_params.len() == 1 && GAS::is_gas_type(&s.type_params[0])
        }

        /// Return `true` if `s` is the type of a gas balance (i.e., 0x2::balance::Balance<0x2::aiy::AIY>)
