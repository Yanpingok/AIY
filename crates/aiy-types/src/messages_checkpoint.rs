...
use crate::aiy_serde::AsProtocolVersion;
use crate::aiy_serde::BigInt;
use crate::aiy_serde::Readable;
use crate::transaction::{Transaction, TransactionData};
use crate::{base_types::AuthorityName, committee::Committee, error::AiyError};
use anyhow::Result;
...
impl CheckpointArtifact {
    pub fn digest(&self) -> AiyResult<Digest> {
        match self {
            Self::ObjectStates(object_states) => {
                ...
                .map_err(|e| AiyError::GenericAuthorityError {
...
impl CheckpointArtifacts {
    ...
    pub fn add_artifact(&mut self, artifact: CheckpointArtifact) -> AiyResult<()> {
        if self
            .artifacts
            .iter()
            .any(|existing| existing.artifact_type() == artifact.artifact_type())
        {
            return Err(AiyError::from(format!(
    ...
    pub fn object_states(&self) -> AiyResult<&BTreeMap<ObjectID, (SequenceNumber, ObjectDigest)>> {
        self.artifacts
            .iter()
            ...
            .ok_or(AiyError::GenericAuthorityError {
                error: "Object states not found in checkpoint artifacts".to_string(),
            })
    }

    pub fn digest(&self) -> AiyResult<CheckpointArtifactsDigest> {
...
impl CheckpointSummary {
    ...
    pub fn verify_epoch(&self, epoch: EpochId) -> AiyResult {
        fp_ensure!(
            self.epoch == epoch,
            AiyError::WrongEpoch {
    ...
    pub fn checkpoint_artifacts_digest(&self) -> AiyResult<&CheckpointArtifactsDigest> {
...
impl CertifiedCheckpointSummary {
    pub fn verify_authority_signatures(&self, committee: &Committee) -> AiyResult {
    ...
    pub fn try_into_verified(self, committee: &Committee) -> AiyResult<VerifiedCheckpoint> {
    ...
    pub fn verify_with_contents(
        ...
        if let Some(contents) = contents {
            ...
            fp_ensure!(
                ...
                AiyError::GenericAuthorityError{error:format!("Checkpoint contents digest mismatch: summary={:?}, received content digest {:?}, received {} transactions", self.data(), content_digest, contents.size())}
            );
        }
...
impl SignedCheckpointSummary {
    ...
    pub fn try_into_verified(
        self,
        committee: &Committee,
    ) -> AiyResult<VerifiedEnvelope<CheckpointSummary, AuthoritySignInfo>> {
        self.verify_authority_signatures(committee)?;
...
impl CheckpointSignatureMessage {
    pub fn verify(&self, committee: &Committee) -> AiyResult {
