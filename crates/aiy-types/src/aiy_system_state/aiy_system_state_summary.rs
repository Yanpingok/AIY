...
use super::{AiySystemState, AiySystemStateTrait};
use crate::base_types::{AuthorityName, ObjectID, AiyAddress};
...
use crate::aiy_system_state::get_validator_from_table;
...
/// This is the JSON-RPC type for the AIY system state object.
/// It flattens all fields to make them top-level fields such that it as minimum
/// dependencies to the internal data structures of the AIY system state type.

#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct AiySystemStateSummary {
    ...
    pub active_validators: Vec<AiyValidatorSummary>,
    ...
    /// ID of the object that maps from staking pool's ID to the aiy address of a validator.
    ...
    #[schemars(with = "Vec<(AiyAddress, BigInt<u64>)>")]
    #[serde_as(as = "Vec<(_, Readable<BigInt<u64>, _>)>")]
    pub at_risk_validators: Vec<(AiyAddress, u64)>,
    /// A map storing the records of validator reporting each other.
    pub validator_report_records: Vec<(AiyAddress, Vec<AiyAddress>)>,
}

impl AiySystemStateSummary {
    pub fn get_aiy_committee_for_benchmarking(&self) -> CommitteeWithNetworkMetadata {
        let validators = self
            .active_validators
            .iter()
            .map(|validator| {
...
/// This is the JSON-RPC type for the AIY validator. It flattens all inner structures
/// to top-level fields so that they are decoupled from the internal definitions.
#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct AiyValidatorSummary {
    // Metadata
    ...
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "Readable<BigInt<u64>, _>")]
    pub staking_pool_aiy_balance: u64,
...
impl Default for AiySystemStateSummary {
    fn default() -> Self {
        Self {
            epoch: 0,
            protocol_version: 1,
...
impl Default for AiyValidatorSummary {
    fn default() -> Self {
        Self {
            aiy_address: AiyAddress::default(),
...
/// Given the staking pool id of a validator, return the validator's `AiyValidatorSummary`,
/// works for validator candidates, active validators, as well as inactive validators.
pub fn get_validator_by_pool_id<S>(
    object_store: &S,
    system_state: &AiySystemState,
    system_state_summary: &AiySystemStateSummary,
    pool_id: ObjectID,
    ...
    S: ObjectStore + ?Sized,
{
    ...
    let candidate_address: AiyAddress = get_dynamic_field_from_store(
        &object_store,
