...
use crate::aiy_system_state::aiy_system_state_inner_v1::{
    StakeSubsidyV1, StorageFundV1, AiySystemStateInnerV1, SystemParametersV1, ValidatorSetV1,
};
use crate::aiy_system_state::aiy_system_state_inner_v2::{
    AiySystemStateInnerV2, SystemParametersV2,
};
use crate::aiy_system_state::{AiySystemState, AiySystemStateWrapper};
use crate::{MoveTypeTagTrait, AIY_SYSTEM_STATE_OBJECT_ID};
...
pub fn aiy_system_state_inner_v1() -> AiySystemStateInnerV1 {
    AiySystemStateInnerV1 {
        epoch: 0,
        protocol_version: 0,
        // must be 1
...
pub fn aiy_system_state_inner_v2() -> AiySystemStateInnerV2 {
...
pub fn system_state_output_objects(aiy_system_state: AiySystemState) -> Vec<Object> {
    let version = aiy_system_state.version();
    let system_state_wrapper_object = Object::new_move(
        unsafe {
            MoveObject::new_from_execution(
                ...
                true,
                0.into(),
                bcs::to_bytes(&AiySystemStateWrapper {
                    // must be AIY_SYSTEM_STATE_OBJECT_ID
                    id: UID::new(AIY_SYSTEM_STATE_OBJECT_ID),
                    version,
                })
                .unwrap(),
    ...
    let object_bytes = match aiy_system_state {
        AiySystemState::V1(inner) => serialize_dynamic_field(&field_id, &dynamic_field_key, inner),
        AiySystemState::V2(inner) => serialize_dynamic_field(&field_id, &dynamic_field_key, inner),
        #[cfg(msim)]
        AiySystemState::SimTestV1(_)
        | AiySystemState::SimTestShallowV2(_)
        | AiySystemState::SimTestDeepV2(_) => unimplemented!(),
    }
    .unwrap();
