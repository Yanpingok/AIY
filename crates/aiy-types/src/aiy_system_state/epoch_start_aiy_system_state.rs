...
use crate::base_types::{AuthorityName, EpochId, AiyAddress};
...
pub trait EpochStartSystemStateTrait {
    ...
    fn get_validator_addresses(&self) -> Vec<AiyAddress>;
    fn get_aiy_committee(&self) -> Committee;
    fn get_aiy_committee_with_network_metadata(&self) -> CommitteeWithNetworkMetadata;
...
/// This type captures the minimum amount of information from AiySystemState needed by a validator
/// to run the protocol. This allows us to decouple from the actual AiySystemState type, and hence
/// do not need to evolve it when we upgrade the AiySystemState type.
...
impl EpochStartSystemStateTrait for EpochStartSystemStateV1 {
    ...
    fn get_validator_addresses(&self) -> Vec<AiyAddress> {
        self.active_validators
            .iter()
            .map(|validator| validator.aiy_address)
            .collect()
    }
            ...
            .map(|validator| {
                (
                    ...
                    (
                        ...
                        NetworkMetadata {
                            network_address: validator.aiy_net_address.clone(),
    ...
    fn get_consensus_committee(&self) -> ConsensusCommittee {
        ...
        // in the Aiy committee returned from get_aiy_committee().
        authorities.sort_by(|a1, a2| a1.authority_key.cmp(&a2.authority_key));

        for ((i, mysticeti_authority), aiy_authority_name) in authorities
            .iter()
            .enumerate()
            .zip(self.get_aiy_committee().names())
        {
            if aiy_authority_name.0 != mysticeti_authority.authority_key.to_bytes() {
                error!(
                    "Mismatched authority order between Aiy and Mysticeti! Index {}, Mysticeti authority {:?}\nAiy authority name {}",
                    i, mysticeti_authority, aiy_authority_name
                );
            }
        }
...
mod test {
    ...
    #[test]
    fn test_aiy_and_mysticeti_committee_are_same() {
        // GIVEN
        let mut active_validators = vec![];

        for i in 0..10 {
            let (aiy_address, protocol_key): (AiyAddress, AuthorityKeyPair) = get_key_pair();
            ...
            active_validators.push(EpochStartValidatorInfoV1 {
                ...
                aiy_net_address: Multiaddr::empty(),
        ...
        // WHEN
        let aiy_committee = state.get_aiy_committee();
        let consensus_committee = state.get_consensus_committee();

        // THEN
        // assert the validators details
        assert_eq!(aiy_committee.num_members(), 10);
        assert_eq!(aiy_committee.num_members(), consensus_committee.size());
        assert_eq!(
            aiy_committee.validity_threshold(),
            consensus_committee.validity_threshold()
        );
        assert_eq!(
            aiy_committee.quorum_threshold(),
        ...
        for (authority_index, consensus_authority) in consensus_committee.authorities() {
            let aiy_authority_name = aiy_committee
                .authority_by_index(authority_index.value() as u32)
                .unwrap();

            assert_eq!(
                consensus_authority.authority_key.to_bytes(),
                aiy_authority_name.0,
                "Mysten & AIY committee member of same index correspond to different public key"
            );
            assert_eq!(
                consensus_authority.stake,
                aiy_committee.weight(aiy_authority_name),
                "Mysten & AIY committee member stake differs"
            );
        }
    }
}
