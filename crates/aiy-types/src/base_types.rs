...
use crate::crypto::{
    AuthorityPublicKeyBytes, DefaultHash, PublicKey, SignatureScheme, AiyPublicKey, AiySignature,
};
...
use crate::error::AiyError;
use crate::error::{ExecutionError, AiyResult};
use crate::gas_coin::GasCoin;
use crate::gas_coin::GAS;
use crate::governance::StakedAiy;
use crate::governance::STAKED_AIY_STRUCT_NAME;
use crate::governance::STAKING_POOL_MODULE_NAME;
use crate::id::RESOLVED_AIY_ID;
use crate::messages_checkpoint::CheckpointTimestamp;
use crate::multisig::MultiSigPublicKey;
use crate::object::{Object, Owner};
use crate::parse_aiy_struct_tag;
use crate::signature::GenericSignature;
use crate::aiy_serde::to_custom_deser_error;
use crate::aiy_serde::to_aiy_struct_tag_string;
use crate::aiy_serde::Readable;
...
use crate::AIY_CLOCK_OBJECT_ID;
use crate::AIY_FRAMEWORK_ADDRESS;
use crate::AIY_SYSTEM_ADDRESS;
use anyhow::anyhow;
...
pub enum MoveObjectType_ {
    ...
    Coin(TypeTag),
    /// A AIY balance accumulator field
    /// (i.e., `0x2::dynamic_field::Field<0x2::accumulator::Key<0x2::balance::Balance<0x2::aiy::AIY>>, 0x2::accumulator::U128>`)
    AiyBalanceAccumulatorField,
    /// A non-AIY balance accumulator field
...
impl MoveObjectType {
    ...
    pub fn staked_aiy() -> Self {
        Self(MoveObjectType_::StakedAiy)
    }

    pub fn address(&self) -> AccountAddress {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => AIY_FRAMEWORK_ADDRESS,
            MoveObjectType_::StakedAiy => AIY_SYSTEM_ADDRESS,
            MoveObjectType_::AiyBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_) => AIY_FRAMEWORK_ADDRESS,
    ...
    pub fn module(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedAiy => STAKING_POOL_MODULE_NAME,
            MoveObjectType_::AiyBalanceAccumulatorField
    ...
    pub fn name(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedAiy => STAKED_AIY_STRUCT_NAME,
            MoveObjectType_::AiyBalanceAccumulatorField
    ...
    pub fn coin_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            ...
            MoveObjectType_::StakedAiy => None,
            MoveObjectType_::AiyBalanceAccumulatorField => None,
    ...
    pub fn balance_accumulator_field_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            MoveObjectType_::AiyBalanceAccumulatorField => Some(GAS::type_tag()),
    ...
    pub fn is_aiy_balance_accumulator_field(&self) -> bool {
        matches!(self.0, MoveObjectType_::AiyBalanceAccumulatorField)
    }
    ...
    pub fn size_for_gas_metering(&self) -> usize {
        ...
        match &self.0 {
            ...
            MoveObjectType_::StakedAiy => 1,
            MoveObjectType_::Coin(inner) => bcs::serialized_size(inner).unwrap() + 1,
            MoveObjectType_::AiyBalanceAccumulatorField => 1,
            MoveObjectType_::BalanceAccumulatorField(inner) => {
    ...
    /// Return true if `self` is `0x2::coin::Coin<T>` for some T (note: T can be AIY)
    pub fn is_coin(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => true,
            MoveObjectType_::StakedAiy
            | MoveObjectType_::AiyBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_)
    ...
    /// Return true if `self` is 0x2::coin::Coin<0x2::aiy::AIY>
    ...
    pub fn is_staked_aiy(&self) -> bool {
        match &self.0 {
            MoveObjectType_::StakedAiy => true,
            MoveObjectType_::GasCoin
            | MoveObjectType_::Coin(_)
            | MoveObjectType_::AiyBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_)
    ...
    pub fn is_upgrade_cap(&self) -> bool {
        self.address() == AIY_FRAMEWORK_ADDRESS
    ...
    pub fn is_dynamic_field(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedAiy | MoveObjectType_::Coin(_) => {
                false
            }
            MoveObjectType_::AiyBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_) => {
    ...
    pub fn try_extract_field_name(&self, type_: &DynamicFieldType) -> AiyResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedAiy | MoveObjectType_::Coin(_) => {
    ...
    pub fn try_extract_field_value(&self) -> AiyResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedAiy | MoveObjectType_::Coin(_) => {
                Err(AiyError::ObjectDeserializationError {
    ...
    pub fn is(&self, s: &StructTag) -> bool {
        match &self.0 {
            ...
            MoveObjectType_::StakedAiy => StakedAiy::is_staked_aiy(s),
...
impl From<StructTag> for MoveObjectType {
    fn from(mut s: StructTag) -> Self {
        ...
        } else if StakedAiy::is_staked_aiy(&s) {
            MoveObjectType_::StakedAiy
...
impl From<MoveObjectType> for StructTag {
    fn from(t: MoveObjectType) -> Self {
        match t.0 {
            ...
            MoveObjectType_::StakedAiy => StakedAiy::type_(),
...
pub fn is_primitive_type_tag(t: &TypeTag) -> bool {
    ...
    match t {
        ...
        T::Struct(st) => {
            ...
            // is id or..
            if resolved_struct == RESOLVED_AIY_ID {
                return true;
            }
            // is utf8 string
...
impl FromStr for ObjectType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.to_lowercase() == PACKAGE {
            ...
            let tag = parse_aiy_struct_tag(s)?;
...
pub const AIY_ADDRESS_LENGTH: usize = ObjectID::LENGTH;
...
#[cfg_attr(feature = "fuzzing", derive(proptest_derive::Arbitrary))]
pub struct AiyAddress(
    #[schemars(with = "Hex")]
    #[serde_as(as = "Readable<Hex, _>")]
    [u8; AIY_ADDRESS_LENGTH],
);

impl AiyAddress {
    pub const ZERO: Self = Self([0u8; AIY_ADDRESS_LENGTH]);
    ...
    /// Deserialize into an `Option<AiyAddress>`.
    pub fn optional_address_from_hex<'de, D>(
        deserializer: D,
    ) -> Result<Option<AiyAddress>, D::Error>
    where
    ...
    /// Return the underlying byte array of a AiyAddress.
    pub fn to_inner(self) -> [u8; AIY_ADDRESS_LENGTH] {
        self.0
    }

    /// Parse a AiyAddress from a byte array or buffer.
    pub fn from_bytes<T: AsRef<[u8]>>(bytes: T) -> Result<Self, AiyError> {
        <[u8; AIY_ADDRESS_LENGTH]>::try_from(bytes.as_ref())
            .map_err(|_| AiyError::InvalidAddress)
            .map(AiyAddress)
    }
    ...
    pub fn try_from_padded(inputs: &ZkLoginInputs) -> AiyResult<Self> {
        Ok((&PublicKey::from_zklogin_inputs(inputs)?).into())
    }

    /// Define as iss_bytes_len || iss_bytes || unpadded_32_byte_address_seed.
    pub fn try_from_unpadded(inputs: &ZkLoginInputs) -> AiyResult<Self> {
        ...
        Ok(AiyAddress(hasher.finalize().digest))
    }
}

impl From<ObjectID> for AiyAddress {
    fn from(object_id: ObjectID) -> AiyAddress {
        Self(object_id.into_bytes())
    }
}

impl From<AccountAddress> for AiyAddress {
    fn from(address: AccountAddress) -> AiyAddress {
        Self(address.into_bytes())
    }
}

impl TryFrom<&[u8]> for AiyAddress {
    type Error = AiyError;

    /// Tries to convert the provided byte array into a AiyAddress.
    fn try_from(bytes: &[u8]) -> Result<Self, AiyError> {
...
impl TryFrom<Vec<u8>> for AiyAddress {
    ...
    /// Tries to convert the provided byte buffer into a AiyAddress.
    fn try_from(bytes: Vec<u8>) -> Result<Self, AiyError> {
        Self::from_bytes(bytes)
    }
}

impl AsRef<[u8]> for AiyAddress {
    fn as_ref(&self) -> &[u8] {
        &self.0[..]
    }
}
...
impl<T: AiyPublicKey> From<&T> for AiyAddress {
    fn from(pk: &T) -> Self {
        let mut hasher = DefaultHash::default();
        hasher.update([T::SIGNATURE_SCHEME.flag()]);
        hasher.update(pk);
        let g_arr = hasher.finalize();
        AiyAddress(g_arr.digest)
    }
}

impl From<&PublicKey> for AiyAddress {
...
impl From<&MultiSigPublicKey> for AiyAddress {
    /// Derive a AiyAddress from [struct MultiSigPublicKey]. A MultiSig address
    ...
    fn from(multisig_pk: &MultiSigPublicKey) -> Self {
        ...
        AiyAddress(hasher.finalize().digest)
    }
}

/// Aiy address for [struct ZkLoginAuthenticator] is defined as the black2b hash of
/// [zklogin_flag || iss_bytes_length || iss_bytes || unpadded_address_seed_in_bytes].
impl TryFrom<&ZkLoginAuthenticator> for AiyAddress {
    type Error = AiyError;
    fn try_from(authenticator: &ZkLoginAuthenticator) -> AiyResult<Self> {
        AiyAddress::try_from_unpadded(&authenticator.inputs)
    }
}

impl TryFrom<&GenericSignature> for AiyAddress {
    type Error = AiyError;
    /// Derive a AiyAddress from a serialized signature in Aiy [GenericSignature].
    fn try_from(sig: &GenericSignature) -> AiyResult<Self> {
        match sig {
            ...
            GenericSignature::ZkLoginAuthenticator(zklogin) => {
                AiyAddress::try_from_unpadded(&zklogin.inputs)
            }
            GenericSignature::PasskeyAuthenticator(s) => Ok(AiyAddress::from(&s.get_pk()?)),
        }
    }
}
...
impl fmt::Debug for AiyAddress {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        write!(f, "0x{}", Hex::encode(self.0))
    }
}

/// Generate a fake AiyAddress with repeated one byte.
pub fn dbg_addr(name: u8) -> AiyAddress {
    let addr = [name; AIY_ADDRESS_LENGTH];
    AiyAddress(addr)
}
...
impl TxContext {
    ...
    pub fn sponsor(&self) -> Option<AiyAddress> {
        self.sponsor.map(AiyAddress::from)
    }
...
impl ObjectID {
    ...
    pub fn is_clock(&self) -> bool {
        *self == AIY_CLOCK_OBJECT_ID
    }
}

impl From<AiyAddress> for ObjectID {
    fn from(address: AiyAddress) -> ObjectID {
...
impl fmt::Display for MoveObjectType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            to_aiy_struct_tag_string(&s).map_err(fmt::Error::custom)?
        )
    }
}
