...
pub const RESOLVED_COIN_STRUCT: (&AccountAddress, &IdentStr, &IdentStr) =
    (&AIY_FRAMEWORK_ADDRESS, COIN_MODULE_NAME, COIN_STRUCT_NAME);
...
// Rust version of the Move aiy::coin::Coin type
...
impl Coin {
    ...
    pub fn is_coin(other: &StructTag) -> bool {
        other.address == AIY_FRAMEWORK_ADDRESS
...
// Rust version of the Move aiy::coin::TreasuryCap type
...
impl TreasuryCap {
    ...
    pub fn from_bcs_bytes(content: &[u8]) -> Result<Self, AiyError> {
        bcs::from_bytes(content).map_err(|err| AiyError::ObjectDeserializationError {
...
// Rust version of the Move aiy::coin::CoinMetadata type
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema, Eq, PartialEq)]
...
// Rust version of the Move aiy::coin::RegulatedCoinMetadata type
