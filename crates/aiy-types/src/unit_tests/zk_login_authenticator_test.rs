...
use crate::{
    base_types::AiyAddress, signature::GenericSignature, zk_login_util::DEFAULT_JWK_BYTES,
};
...
fn test_serde_zk_login_signature() {
    ...
    let addr: AiyAddress = (&authenticator).try_into().unwrap();
...
fn test_serde_zk_public_identifier() {
    ...
    assert_eq!(
        AiyAddress::try_from_unpadded(&modified_inputs).unwrap(),
        AiyAddress::from(&pk1)
    );
    ...
    assert_eq!(
        AiyAddress::try_from_padded(&modified_inputs).unwrap(),
        AiyAddress::from(&pk2)
    );
