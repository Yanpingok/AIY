...
use crate::base_types::{MoveObjectType, SequenceNumber, AiyAddress};
use crate::dynamic_field::{DynamicFieldInfo, DynamicFieldKey};
use crate::gas_coin::GAS;
use crate::object::MoveObject;
use crate::{
    MoveTypeTagTrait, MoveTypeTagTraitGeneric, AIY_ACCUMULATOR_ROOT_OBJECT_ID,
    AIY_FRAMEWORK_ADDRESS,
};
use move_core_types::language_storage::{StructTag, TypeTag};

#[test]
fn test_aiy_balance_accumulator_field_recognition() {
    // Create a Field<Key<Balance<AIY>>, U128> type
    let aiy_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[aiy_balance]);
    ...
    assert!(move_type.is_aiy_balance_accumulator_field());
...
#[test]
fn test_non_aiy_balance_accumulator_field_recognition() {
    ...
    // Should be recognized as a balance accumulator field, but not AIY
    assert!(move_type.is_balance_accumulator_field());
    assert!(!move_type.is_aiy_balance_accumulator_field());
...
fn test_accumulator_storage_savings() {
    let owner = AiyAddress::random_for_testing_only();
    let balance = 1000u64;

    // Create the field struct tag for Field<Key<Balance<AIY>>, U128>
    let aiy_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[aiy_balance]);
    let u128_type = U128::get_type_tag();
    ...
    assert!(efficient_move_object
        .type_()
        .is_aiy_balance_accumulator_field());
}
