...
impl From<NitroAttestationVerifyError> for AiyError {
    fn from(err: NitroAttestationVerifyError) -> Self {
        AiyError::NitroAttestationFailedToVerify(err.to_string())
    }
}
...
) -> AiyResult<(Vec<u8>, Vec<u8>, AttestationDocument)> {
...
impl CoseSign1 {
    ...
    fn to_signed_message(&self) -> AiyResult<Vec<u8>> {
        ...
        ciborium::ser::into_writer(&value, &mut bytes).map_err(|_| {
            AiyError::NitroAttestationFailedToVerify("cannot parse message".to_string())
        })?;
        Ok(bytes)
    }
}
