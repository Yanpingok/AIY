...
use crate::{
    parse_aiy_struct_tag, parse_aiy_type_tag, DEEPBOOK_ADDRESS, AIY_CLOCK_ADDRESS,
    AIY_FRAMEWORK_ADDRESS, AIY_SYSTEM_ADDRESS, AIY_SYSTEM_STATE_ADDRESS,
};
...
pub struct AiyStructTag;

impl SerializeAs<StructTag> for AiyStructTag {
    fn serialize_as<S>(value: &StructTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let f = to_aiy_struct_tag_string(value).map_err(S::Error::custom)?;
        f.serialize(serializer)
    }
}

const AIY_ADDRESSES: [AccountAddress; 7] = [
    AccountAddress::ZERO,
    AccountAddress::ONE,
    AIY_FRAMEWORK_ADDRESS,
    AIY_SYSTEM_ADDRESS,
    DEEPBOOK_ADDRESS,
    AIY_SYSTEM_STATE_ADDRESS,
    AIY_CLOCK_ADDRESS,
];
/// Serialize StructTag as a string, retaining the leading zeros in the address.
pub fn to_aiy_struct_tag_string(value: &StructTag) -> Result<String, fmt::Error> {
    let mut f = String::new();
    // trim leading zeros if address is in AIY_ADDRESSES
    let address = if AIY_ADDRESSES.contains(&value.address) {
    ...
    if let Some(first_ty) = value.type_params.first() {
        write!(f, "<")?;
        write!(f, "{}", to_aiy_type_tag_string(first_ty)?)?;
        for ty in value.type_params.iter().skip(1) {
            write!(f, ", {}", to_aiy_type_tag_string(ty)?)?;
        }
        write!(f, ">")?;
    }
    Ok(f)
...
fn to_aiy_type_tag_string(value: &TypeTag) -> Result<String, fmt::Error> {
    match value {
        TypeTag::Vector(t) => Ok(format!("vector<{}>", to_aiy_type_tag_string(t)?)),
        TypeTag::Struct(s) => to_aiy_struct_tag_string(s),
        _ => Ok(value.to_string()),
    }
}

impl<'de> DeserializeAs<'de, StructTag> for AiyStructTag {
    fn deserialize_as<D>(deserializer: D) -> Result<StructTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_aiy_struct_tag(&s).map_err(D::Error::custom)
    }
}

pub struct AiyTypeTag;

impl SerializeAs<TypeTag> for AiyTypeTag {
    fn serialize_as<S>(value: &TypeTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s = to_aiy_type_tag_string(value).map_err(S::Error::custom)?;
        s.serialize(serializer)
    }
}

impl<'de> DeserializeAs<'de, TypeTag> for AiyTypeTag {
    fn deserialize_as<D>(deserializer: D) -> Result<TypeTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_aiy_type_tag(&s).map_err(D::Error::custom)
    }
}
...
pub(crate) struct AiyBitmap;

impl SerializeAs<roaring::RoaringBitmap> for AiyBitmap {
...
impl<'de> DeserializeAs<'de, roaring::RoaringBitmap> for AiyBitmap {
    fn deserialize_as<D>(deserializer: D) -> Result<roaring::RoaringBitmap, D::Error>
    where
        D: Deserializer<'de>,
    {
        let bytes: Vec<u8> = Bytes::deserialize_as(deserializer)?;
        deserialize_aiy_bitmap(&bytes).map_err(to_custom_deser_error::<'de, D, _>)
    }
}
...
fn deserialize_aiy_bitmap(bytes: &[u8]) -> std::io::Result<roaring::RoaringBitmap> {
...
mod test {
    ...
    #[test]
    fn test_aiy_bitmap_unique_deserialize() {
        ...
        let aiy_bitmap = deserialize_aiy_bitmap(&bytes[..]).unwrap();
        assert_eq!(aiy_bitmap.len(), 1);
        let bitmap_values: Vec<u32> = aiy_bitmap.iter().collect();
