...
use crate::gas_coin::MIST_PER_AIY;
use crate::id::{ID, UID};
...
///
/// 30 million AIY
pub const MIN_VALIDATOR_JOINING_STAKE_MIST: u64 = 30_000_000 * MIST_PER_AIY;
...
///
/// 20 million AIY
pub const VALIDATOR_LOW_STAKE_THRESHOLD_MIST: u64 = 20_000_000 * MIST_PER_AIY;
...
///
/// 15 million AIY
pub const VALIDATOR_VERY_LOW_STAKE_THRESHOLD_MIST: u64 = 15_000_000 * MIST_PER_AIY;
...
pub const STAKED_AIY_STRUCT_NAME: &IdentStr = ident_str!("StakedAiy");
...
impl StakedAiy {
    pub fn type_() -> StructTag {
        StructTag {
            ...
            name: STAKED_AIY_STRUCT_NAME.to_owned(),
            type_params: vec![],
        }
    }

    pub fn is_staked_aiy(s: &StructTag) -> bool {
        s.address == AIY_SYSTEM_ADDRESS
            && s.module.as_ident_str() == STAKING_POOL_MODULE_NAME
            && s.name.as_ident_str() == STAKED_AIY_STRUCT_NAME
...
impl TryFrom<&Object> for StakedAiy {
    type Error = AiyError;
    fn try_from(object: &Object) -> Result<Self, Self::Error> {
        match &object.data {
            Data::Move(o) => {
                if o.type_().is_staked_aiy() {
                    return bcs::from_bytes(o.contents()).map_err(|err| AiyError::TypeError {
                        error: format!("Unable to deserialize StakedAiy object: {:?}", err),
                    });
                }
            }
            Data::Package(_) => {}
        ...
        Err(AiyError::TypeError {
            error: format!("Object type is not a StakedAiy: {:?}", object),
        })
    }
}
