...
pub enum UserInputError {
    ...
    #[error("AIY payment transactions use first input coin for gas payment, but found a different gas object")]
...
pub enum AiyObjectResponseError {
    ...
    // TODO: also integrate AiyPastObjectResponse (VersionNotFound,  VersionTooHigh)
}

/// Custom error type for Aiy.
#[derive(
    Eq, PartialEq, Clone, Debug, Serialize, Deserialize, Error, Hash, AsRefStr, IntoStaticStr,
)]
pub enum AiyError {
    ...
    AiyObjectResponseError { error: AiyObjectResponseError },
    ...
    TooManyIncorrectAuthorities {
        errors: Vec<(AuthorityName, AiyError)>,
        action: String,
    },
    ...
    #[error("Failed to execute handle_consensus_transaction on Aiy: {0}")]
...
pub type AiyResult<T = ()> = Result<T, AiyError>;
pub type UserInputResult<T = ()> = Result<T, UserInputError>;

impl From<aiy_protocol_config::Error> for AiyError {
    fn from(error: aiy_protocol_config::Error) -> Self {
        AiyError::WrongMessageVersion { error: error.0 }
    }
}

impl From<ExecutionError> for AiyError {
    fn from(error: ExecutionError) -> Self {
        AiyError::ExecutionError(error.to_string())
    }
}

impl From<Status> for AiyError {
    fn from(status: Status) -> Self {
        if status.message() == "Too many requests" {
            return Self::TooManyRequests;
        }

        let result = bcs::from_bytes::<AiyError>(status.details());
        if let Ok(aiy_error) = result {
            aiy_error
        } else {
            Self::RpcError(
...
impl From<TypedStoreError> for AiyError {
    fn from(e: TypedStoreError) -> Self {
        Self::Storage(e.to_string())
    }
}

impl From<crate::storage::error::Error> for AiyError {
    fn from(e: crate::storage::error::Error) -> Self {
        Self::Storage(e.to_string())
    }
}

impl From<AiyError> for Status {
    fn from(error: AiyError) -> Self {
        let bytes = bcs::to_bytes(&error).unwrap();
        Status::with_details(tonic::Code::Internal, error.to_string(), bytes.into())
    }
}

impl From<ExecutionErrorKind> for AiyError {
    fn from(kind: ExecutionErrorKind) -> Self {
        ExecutionError::from_kind(kind).into()
    }
}

impl From<&str> for AiyError {
    fn from(error: &str) -> Self {
        AiyError::GenericAuthorityError {
            error: error.to_string(),
        }
    }
}

impl From<String> for AiyError {
    fn from(error: String) -> Self {
        AiyError::GenericAuthorityError { error }
    }
}

impl TryFrom<AiyError> for UserInputError {
    type Error = anyhow::Error;

    fn try_from(err: AiyError) -> Result<Self, Self::Error> {
        match err {
            AiyError::UserInputError { error } => Ok(error),
            other => anyhow::bail!("error {:?} is not UserInputError", other),
        }
    }
}

impl From<UserInputError> for AiyError {
    fn from(error: UserInputError) -> Self {
        AiyError::UserInputError { error }
    }
}

impl From<AiyObjectResponseError> for AiyError {
    fn from(error: AiyObjectResponseError) -> Self {
        AiyError::AiyObjectResponseError { error }
    }
}
...
impl AiyError {
    pub fn individual_error_indicates_epoch_change(&self) -> bool {
        matches!(
            self,
            AiyError::ValidatorHaltedAtEpochEnd | AiyError::MissingCommitteeAtEpoch(_)
        )
    }
    ...
    pub fn is_retryable(&self) -> (bool, bool) {
        let retryable = match self {
            // Network error
            AiyError::RpcError { .. } => true,

            // Reconfig error
            AiyError::ValidatorHaltedAtEpochEnd => true,
            AiyError::MissingCommitteeAtEpoch(..) => true,
            AiyError::WrongEpoch { .. } => true,
            AiyError::EpochEnded(..) => true,
            ...
            AiyError::PotentiallyTemporarilyInvalidSignature { .. } => true,

            // Overload errors
            AiyError::TooManyTransactionsPendingExecution { .. } => true,
            AiyError::TooManyTransactionsPendingOnObject { .. } => true,
            AiyError::TooOldTransactionPendingOnObject { .. } => true,
            AiyError::TooManyTransactionsPendingConsensus => true,
            AiyError::ValidatorOverloadedRetryAfter { .. } => true,

            // Non retryable error
            AiyError::ExecutionError(..) => false,
            AiyError::ByzantineAuthoritySuspicion { .. } => false,
            AiyError::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. } => false,
            AiyError::TxAlreadyFinalizedWithDifferentUserSigs => false,
            AiyError::FailedToVerifyTxCertWithExecutedEffects { .. } => false,
            AiyError::ObjectLockConflict { .. } => false,
            ...
            AiyError::TooManyRequests => false,
    ...
    pub fn is_overload(&self) -> bool {
        matches!(
            self,
            AiyError::TooManyTransactionsPendingExecution { .. }
                | AiyError::TooManyTransactionsPendingOnObject { .. }
                | AiyError::TooOldTransactionPendingOnObject { .. }
                | AiyError::TooManyTransactionsPendingConsensus
        )
    }

    pub fn is_retryable_overload(&self) -> bool {
        matches!(self, AiyError::ValidatorOverloadedRetryAfter { .. })
    }

    pub fn retry_after_secs(&self) -> u64 {
        match self {
            AiyError::ValidatorOverloadedRetryAfter { retry_after_secs } => *retry_after_secs,
            _ => 0,
        }
    }

    /// Categorizes AiyError into ErrorCategory.
    pub fn categorize(&self) -> ErrorCategory {
        match self {
            ...
            | AiyError::SignerSignatureAbsent { .. }
            | AiyError::SignerSignatureNumberMismatch { .. }
            | AiyError::IncorrectSigner { .. }
            | AiyError::UnknownSigner { .. }
            | AiyError::TransactionExpired => ErrorCategory::InvalidTransaction,

            AiyError::ObjectLockConflict { .. } => ErrorCategory::LockConflict,

            AiyError::Unknown { .. }
            | AiyError::GrpcMessageSerializeError { .. }
            | AiyError::GrpcMessageDeserializeError { .. }
            | AiyError::ByzantineAuthoritySuspicion { .. }
            | AiyError::InvalidTxKindInSoftBundle { .. }
            | AiyError::UnsupportedFeatureError { .. }
            | AiyError::InvalidRequest { .. } => ErrorCategory::Internal,

            AiyError::TooManyTransactionsPendingExecution { .. }
            | AiyError::TooManyTransactionsPendingOnObject { .. }
            | AiyError::TooOldTransactionPendingOnObject { .. }
            | AiyError::TooManyTransactionsPendingConsensus
            | AiyError::ValidatorOverloadedRetryAfter { .. } => ErrorCategory::ValidatorOverloaded,

            AiyError::TimeoutError { .. } => ErrorCategory::Unavailable,
...
impl Ord for AiyError {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        Ord::cmp(self.as_ref(), other.as_ref())
    }
}

impl PartialOrd for AiyError {
