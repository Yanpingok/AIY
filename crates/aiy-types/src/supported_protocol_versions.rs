...
pub use aiy_protocol_config::{Chain, ProtocolConfig, ProtocolVersion};

/// Models the set of protocol versions supported by a validator.
/// The `aiy-node` binary will always use the SYSTEM_DEFAULT constant, but for testing we need
/// to be able to inject arbitrary versions into AiyNode.
...
impl SupportedProtocolVersionsWithHashes {
    ...
    // Ideally this would be in aiy-protocol-config, but aiy-types depends on aiy-protocol-config,
