...
use crate::{id::UID, AIY_AUTHENTICATOR_STATE_OBJECT_ID, AIY_FRAMEWORK_ADDRESS};
...
pub const RESOLVED_AIY_AUTHENTICATOR_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
) -> AiyResult<Option<AuthenticatorStateInner>> {
    let outer = object_store.get_object(&AIY_AUTHENTICATOR_STATE_OBJECT_ID);
...
pub fn get_authenticator_state_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&AIY_AUTHENTICATOR_STATE_OBJECT_ID)
