...
use crate::base_types::{EpochId, AiyAddress};
...
use crate::{
    MoveTypeTagTrait, AIY_DENY_LIST_OBJECT_ID, AIY_FRAMEWORK_ADDRESS, AIY_FRAMEWORK_PACKAGE_ID,
};
...
fn check_new_regulated_coin_owners(
    new_regulated_coin_owners: BTreeMap<String, (Config, BTreeSet<AiyAddress>)>,
    cur_epoch: EpochId,
    object_store: &dyn ObjectStore,
...
pub fn get_per_type_coin_deny_list_v2(
    ...
    let config: Config =
        get_dynamic_field_from_store(object_store, AIY_DENY_LIST_OBJECT_ID, &config_key).ok()?;
    Some(config)
}
