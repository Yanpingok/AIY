...
use crate::{
    base_types::{FullObjectID, FullObjectRef, ObjectID, ObjectRef, AiyAddress},
...
impl ProgrammableTransactionBuilder {
    ...
    pub fn transfer_arg(&mut self, recipient: AiyAddress, arg: Argument) {
        self.transfer_args(recipient, vec![arg])
    }

    pub fn transfer_args(&mut self, recipient: AiyAddress, args: Vec<Argument>) {
    ...
    pub fn transfer_aiy(&mut self, recipient: AiyAddress, amount: Option<u64>) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    pub fn pay_all_aiy(&mut self, recipient: AiyAddress) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    pub fn split_coin(&mut self, recipient: AiyAddress, coin: ObjectRef, amounts: Vec<u64>) {
    ...
    fn pay_impl(
        ...
        let mut recipient_map: IndexMap<AiyAddress, Vec<usize>> = IndexMap::new();
