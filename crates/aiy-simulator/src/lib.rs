...
// Re-export things used by aiy-macros
...
pub use aiy_framework;
pub use aiy_move_build;
pub use aiy_types;
...
pub mod configs {
    ...
    /// Select from among a number of configs using the AIY_SIM_CONFIG env var.
    pub fn env_config(
        // Config to use when AIY_SIM_CONFIG is not set.
        default: SimConfig,
        // List of (&str, SimConfig) pairs - the SimConfig associated with the value
        // of the AIY_SIM_CONFIG var is chosen.
        ...
        if let Some(env) = std::env::var("AIY_SIM_CONFIG").ok() {
            if let Some(cfg) = env_configs.remove(env.as_str()) {
                info!("Using test config for AIY_SIM_CONFIG={}", env);
                cfg
            } else {
                panic!(
                    "No config found for AIY_SIM_CONFIG={}. Available configs are: {:?}",
                    env,
                    env_configs.keys()
                );
            }
...
pub fn has_mainnet_protocol_config_override() -> bool {
    use aiy_types::{digests::ChainIdentifier, supported_protocol_versions::Chain};
