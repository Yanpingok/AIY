...
- Provides deterministic, randomized execution of an entire Aiy network in a single process.
...
1. Procedural macros that replace `#[tokio::test]` and run test code inside a testing environment. These are `#[aiy_test]` and `#[sim_test]` and are documented below. The test harness created by these macros initializes the simulator runtime with a starting seed, generates the simulator configuration, and runs the test inside a newly created thread. The test must be run in its own thread in order to provide each test case with fresh thread local storage.
...
To address this, most such test code launches validators via the `spawn_test_authorities` function in the `test-utils` carate, which returns `Vec<AiyNodeHandle>` rather than `Vec<AiyNode>`.
`AiyNodeHandle` hides the `AiyNode` from the test code.
...
`AiyNodeHandle` runs the provided callbacks/futures inside the context of the appropriate simulator node, so that network requests, spawned tasks, etc continue running in the correct context.
...
Network latency and packet loss can be configured using [SimConfig](https://github.com/MystenLabs/mysten-sim/blob/main/msim/src/sim/config.rs#L8), which is re-exported from this crate as `aiy_simulator::config::SimConfig`.
...
There is a small but growing library of functions for building network configs in [aiy_simulator::configs](https://github.com/MystenLabs/sui/blob/main/crates/sui-simulator/src/lib.rs).
