...
use aiy_json_rpc_types::AiyGetPastObjectRequest;
use aiy_json_rpc_types::AiyObjectData;
use aiy_json_rpc_types::AiyObjectDataOptions;
use aiy_json_rpc_types::AiyObjectResponse;
use aiy_json_rpc_types::AiyPastObjectResponse;
...
use aiy_types::base_types::{ObjectID, SequenceNumber, VersionNumber};
...
use aiy_types::transaction::{EndOfEpochTransactionKind, TransactionKind};
...
pub(crate) trait DataFetcher {
    ...
    ) -> Result<AiyTransactionBlockResponse, ReplayEngineError>;
    ...
    async fn get_epoch_change_events(
        &self,
        reverse: bool,
    ) -> Result<Vec<AiyEvent>, ReplayEngineError>;
...
impl RemoteFetcher {
    pub fn new(rpc_client: AiyClient) -> Self {
        Self {
            rpc_client,
...
impl DataFetcher for RemoteFetcher {
    ...
    async fn multi_get_versioned(
        ...
        let options = AiyObjectDataOptions::bcs_lossless();

        let objs: Vec<_> = to_fetch
            .iter()
            .map(|(object_id, version)| AiyGetPastObjectRequest {
                object_id: *object_id,
                version: *version,
            })
            .collect();
    ...
    async fn get_child_object(
        ...
        let response = self
            ...
            .await
            .map_err(|q| ReplayEngineError::AiyRpcError { err: q.to_string() })?;
    ...
    async fn get_transaction(
        ...
        let tx_fetch_opts = AiyTransactionBlockResponseOptions::full_content();
    ...
    async fn get_epoch_change_events(
        ...
        let mut epoch_change_events: Vec<AiyEvent> = vec![];
...
fn convert_past_obj_response(resp: AiyPastObjectResponse) -> Result<Object, ReplayEngineError> {
    match resp {
        AiyPastObjectResponse::VersionFound(o) => obj_from_aiy_obj_data(&o),
        AiyPastObjectResponse::ObjectDeleted(r) => Err(ReplayEngineError::ObjectDeleted {
            id: r.object_id,
            version: r.version,
            digest: r.digest,
        }),
        AiyPastObjectResponse::ObjectNotExists(id) => Err(ReplayEngineError::ObjectNotExist { id }),
        AiyPastObjectResponse::VersionNotFound(id, version) => {
            Err(ReplayEngineError::ObjectVersionNotFound { id, version })
        }
        AiyPastObjectResponse::VersionTooHigh {
            object_id,
            asked_version,
            latest_version,
...
#[allow(clippy::result_large_err)]
fn obj_from_aiy_obj_response(o: &AiyObjectResponse) -> Result<Object, ReplayEngineError> {
    let o = o.object().map_err(ReplayEngineError::from)?.clone();
    obj_from_aiy_obj_data(&o)
}

#[allow(clippy::result_large_err)]
fn obj_from_aiy_obj_data(o: &AiyObjectData) -> Result<Object, ReplayEngineError> {
...
#[allow(clippy::result_large_err)]
pub fn extract_epoch_and_version(ev: AiyEvent) -> Result<(u64, u64), ReplayEngineError> {
