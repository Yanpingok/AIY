...
use aiy_sdk::error::Error as AiyRpcError;
use aiy_types::base_types::{ObjectID, ObjectRef, SequenceNumber, AiyAddress, VersionNumber};
use aiy_types::digests::{ObjectDigest, TransactionDigest};
use aiy_types::error::{AiyError, AiyObjectResponseError, AiyResult, UserInputError};
use aiy_types::object::Object;
use aiy_types::transaction::{InputObjectKind, SenderSignedData, TransactionKind};
use thiserror::Error;
...
pub(crate) const EPOCH_CHANGE_STRUCT_TAG: &str =
    "0x3::aiy_system_state_inner::SystemEpochInfoEvent";
...
pub struct OnChainTransactionInfo {
    ...
    #[serde(default)]
    pub gas_owner: Option<AiyAddress>,
    pub gas_budget: u64,
    pub gas_price: u64,
...
pub enum ReplayEngineError {
    #[error("AiyError: {:#?}", err)]
    AiyError { err: AiyError },

    #[error("AiyRpcError: {:#?}", err)]
    AiyRpcError { err: String },

    #[error("AiyObjectResponseError: {:#?}", err)]
    AiyObjectResponseError { err: AiyObjectResponseError },
    ...
    #[error("AiyRpcRequestTimeout")]
    AiyRpcRequestTimeout,
    ...
    EffectsForked {
        ...
        diff: String,
        on_chain: Box<AiyTransactionBlockEffects>,
        local: Box<AiyTransactionBlockEffects>,
    },
    ...
    UnexpectedEventFormat { event: AiyEvent },
...
impl From<AiyObjectResponseError> for ReplayEngineError {
    fn from(err: AiyObjectResponseError) -> Self {
        match err {
            AiyObjectResponseError::NotExists { object_id } => {
            ...
            _ => ReplayEngineError::AiyObjectResponseError { err },
        }
    }
}

impl From<ReplayEngineError> for AiyError {
    fn from(err: ReplayEngineError) -> Self {
        AiyError::Unknown(format!("{:#?}", err))
    }
}

impl From<AiyError> for ReplayEngineError {
    fn from(err: AiyError) -> Self {
        ReplayEngineError::AiyError { err }
    }
}
impl From<AiyRpcError> for ReplayEngineError {
    fn from(err: AiyRpcError) -> Self {
        match err {
            AiyRpcError::RpcError(JsonRpseeError::RequestTimeout) => {
                ReplayEngineError::AiyRpcRequestTimeout
            }
            _ => ReplayEngineError::AiyRpcError {
                err: format!("{:?}", err),
            },
        }
    }
...
pub enum ExecutionStoreEvent {
    BackingPackageGetPackageObject {
        package_id: ObjectID,
        result: AiyResult<Option<Object>>,
    },
    ...
    ResourceResolverGetResource {
        ...
        typ: StructTag,
        result: AiyResult<Option<Vec<u8>>>,
    },
    ...
    GetModuleGetModuleByModuleId {
        id: ModuleId,
        result: AiyResult<Option<CompiledModule>>,
    },
    ReceiveObject {
        owner: ObjectID,
        receive: ObjectID,
