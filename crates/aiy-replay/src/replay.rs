...
use aiy_core::authority::NodeStateDump;
use aiy_execution::Executor;
use aiy_framework::BuiltInFramework;
use aiy_json_rpc_types::{
    AiyExecutionStatus, AiyTransactionBlockEffects, AiyTransactionBlockEffectsAPI,
};
use aiy_protocol_config::{Chain, ProtocolConfig};
use aiy_sdk::{AiyClient, AiyClientBuilder};
use aiy_types::execution_params::{
...
use aiy_types::storage::{get_module, PackageObject};
use aiy_types::transaction::GasData;
use aiy_types::transaction::TransactionKind::ProgrammableTransaction;
use aiy_types::AIY_DENY_LIST_OBJECT_ID;
use aiy_types::{
...
pub struct ExecutionSandboxState {
    ...
    pub local_exec_effects: AiyTransactionBlockEffects,
...
impl ExecutionSandboxState {
    ...
    pub fn check_effects(&self) -> Result<(), ReplayEngineError> {
        let AiyTransactionBlockEffects::V1(mut local_effects) = self.local_exec_effects.clone();
        let AiyTransactionBlockEffects::V1(on_chain_effects) =
        ...
        let local_effects = AiyTransactionBlockEffects::V1(local_effects);
        let on_chain_effects = AiyTransactionBlockEffects::V1(on_chain_effects);
    ...
    pub fn diff_effects(
        on_chain_effects: &AiyTransactionBlockEffects,
        local_effects: &AiyTransactionBlockEffects,
    ) -> String {
...
impl LocalExec {
    ...
    pub async fn multi_download(
        ...
        while num_retries_for_timeout >= 0 {
            match self.fetcher.multi_get_versioned(objs).await {
                ...
                Err(ReplayEngineError::AiyRpcRequestTimeout) => {
                    warn!(
    ...
    pub fn download_latest_object(
        ...
        match resp {
            ...
            Err(err) => Err(ReplayEngineError::AiyRpcError {
                err: err.to_string(),
            }),
        }
    }
    ...
    pub async fn get_checkpoint_txs(
        ...
        self.fetcher
            ...
            .await
            .map_err(|e| ReplayEngineError::AiyRpcError { err: e.to_string() })
    }
    ...
    pub async fn execution_engine_execute_with_tx_info_impl(
        ...
        let effects =
            AiyTransactionBlockEffects::try_from(effects).map_err(ReplayEngineError::from)?;
    ...
    pub async fn certificate_execute_with_sandbox_state(
        ...
        let executor = aiy_execution::executor(&protocol_config, true).unwrap();
        let early_execution_error = get_early_execution_error(
            executable.digest(),
            &input_objects,
            &HashSet::new(),
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `aiy_core::authority::AuthorityState::try_execute_immediately`
    #[allow(clippy::result_large_err)]
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `aiy_adapter::execution_engine::execute_transaction_to_effects`
    #[allow(clippy::result_large_err)]
        ...
        match parse_effect_error_for_denied_coins(status) {
            Some(coin_type) => {
                ...
                if !config_id_and_version
                    .iter()
                    .any(|(id, _)| id == &AIY_DENY_LIST_OBJECT_ID)
                {
                    let deny_list_oid_version = self.download_latest_object(&AIY_DENY_LIST_OBJECT_ID)
                        .ok()
                        .flatten()
                        .expect("Unable to download the deny list object for a transaction that requires it")
                        .version();
                    config_id_and_version.push((AIY_DENY_LIST_OBJECT_ID, deny_list_oid_version));
                }
    ...
    async fn resolve_tx_components(
        ...
        let sender = match tx_info.clone().transaction.unwrap().data {
            aiy_json_rpc_types::AiyTransactionBlockData::V1(tx) => tx.sender,
        };
        let AiyTransactionBlockEffects::V1(effects) = tx_info.clone().effects.unwrap();
        ...
        let gas_data = match tx_info.clone().transaction.unwrap().data {
            aiy_json_rpc_types::AiyTransactionBlockData::V1(tx) => tx.gas_data,
        };
        ...
        Ok(OnChainTransactionInfo {
            ...
            effects: AiyTransactionBlockEffects::V1(effects),
            receiving_objs,
            config_objects,
    ...
    async fn resolve_tx_components_from_dump(
        ...
        let effects = AiyTransactionBlockEffects::try_from(effects).unwrap();
    ...
    fn read_child_object(
        ...
        fn inner(
            ...
            if child_object.version() > child_version_upper_bound {
                return Err(AiyError::Unknown(format!(
            ...
            if child_object.owner != Owner::ObjectOwner(parent.into()) {
                return Err(AiyError::InvalidChildObjectAccess {
                    object: *child,
                    given_parent: parent,
...
impl ModuleResolver for LocalExec {
    ...
    /// We do not download
    fn get_module(&self, module_id: &ModuleId) -> AiyResult<Option<Vec<u8>>> {
        fn inner(self_: &LocalExec, module_id: &ModuleId) -> AiyResult<Option<Vec<u8>>> {
...
impl GetModule for LocalExec {
    ...
    fn get_module_by_id(&self, id: &ModuleId) -> AiyResult<Option<Self::Item>> {
...
pub fn get_executor(
    ...
    let silent = true;
    aiy_execution::executor(&protocol_config, silent)
        .expect("Creating an executor should not fail here")
}

fn parse_effect_error_for_denied_coins(status: &AiyExecutionStatus) -> Option<String> {
    let AiyExecutionStatus::Failure { error } = status else {
        return None;
    };
