...
use aiy_indexer_alt_framework_store_traits::{
...
pub use aiy_field_count::FieldCount;
/// External users access the store trait through framework::store
pub use aiy_indexer_alt_framework_store_traits as store;
...
mod tests {
    ...
    use std::sync::Arc;
    use aiy_synthetic_ingestion::synthetic_ingestion;
    use tokio_util::sync::CancellationToken;
    ...
    impl Processor for MockHandler {
        ...
        fn process(
            &self,
            _checkpoint: &Arc<aiy_types::full_checkpoint_content::CheckpointData>,
