...
const AIY_DIR: &str = ".aiy";
pub const AIY_CONFIG_DIR: &str = "aiy_config";
pub const AIY_NETWORK_CONFIG: &str = "network.yaml";
pub const AIY_FULLNODE_CONFIG: &str = "fullnode.yaml";
pub const AIY_CLIENT_CONFIG: &str = "client.yaml";
pub const AIY_KEYSTORE_FILENAME: &str = "aiy.keystore";
pub const AIY_KEYSTORE_ALIASES_FILENAME: &str = "aiy.aliases";
pub const AIY_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME: &str = "benchmark.keystore";
pub const AIY_GENESIS_FILENAME: &str = "genesis.blob";
pub const AIY_DEV_NET_URL: &str = "https://fullnode.devnet.sui.io:443";

pub const AUTHORITIES_DB_NAME: &str = "authorities_db";
pub const CONSENSUS_DB_NAME: &str = "consensus_db";
pub const FULL_NODE_DB_PATH: &str = "full_node_db";

pub fn aiy_config_dir() -> Result<PathBuf, anyhow::Error> {
    match std::env::var_os("AIY_CONFIG_DIR") {
        Some(config_env) => Ok(config_env.into()),
        None => match dirs::home_dir() {
            Some(v) => Ok(v.join(AIY_DIR).join(AIY_CONFIG_DIR)),
...
pub fn genesis_blob_exists(config_dir: Option<PathBuf>) -> bool {
    if let Some(dir) = config_dir {
        dir.join(AIY_GENESIS_FILENAME).exists()
    } else if let Some(config_env) = std::env::var_os("AIY_CONFIG_DIR") {
        Path::new(&config_env).join(AIY_GENESIS_FILENAME).exists()
    } else if let Some(home) = dirs::home_dir() {
        let mut config = PathBuf::new();
        config.push(&home);
        config.extend([AIY_DIR, AIY_CONFIG_DIR, AIY_GENESIS_FILENAME]);
        config.exists()
    } else {
        false
    }
