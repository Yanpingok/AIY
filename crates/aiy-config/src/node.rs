...
use aiy_keys::keypair_file::{read_authority_keypair_from_file, read_keypair_from_file};
use aiy_types::base_types::{ObjectID, AiyAddress};
use aiy_types::committee::EpochId;
use aiy_types::crypto::AuthorityPublicKeyBytes;
use aiy_types::crypto::KeypairTraits;
use aiy_types::crypto::NetworkKeyPair;
use aiy_types::crypto::AiyKeyPair;
use aiy_types::messages_checkpoint::CheckpointSequenceNumber;
use aiy_types::supported_protocol_versions::{Chain, SupportedProtocolVersions};
use aiy_types::traffic_control::{PolicyConfig, RemoteFirewallConfig};

use aiy_types::crypto::{get_key_pair_from_rng, AccountKeyPair, AuthorityKeyPair};
use aiy_types::multiaddr::Multiaddr;
use tracing::info;
...
pub const DEFAULT_VALIDATOR_GAS_PRICE: u64 = aiy_types::transaction::DEFAULT_VALIDATOR_GAS_PRICE;
...
pub struct NodeConfig {
    ...
    /// In a `aiy-node` binary, this is set to SupportedProtocolVersions::SYSTEM_DEFAULT
    /// in aiy-node/src/main.rs. It is present in the config so that it can be changed by tests in
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name_service_package_address: Option<AiyAddress>,
...
impl ExecutionCacheConfig {
    pub fn max_cache_size(&self) -> u64 {
        std::env::var("AIY_MAX_CACHE_SIZE")
            .ok()
    ...
    pub fn package_cache_size(&self) -> u64 {
        std::env::var("AIY_PACKAGE_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn object_cache_size(&self) -> u64 {
        std::env::var("AIY_OBJECT_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn marker_cache_size(&self) -> u64 {
        std::env::var("AIY_MARKER_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn object_by_id_cache_size(&self) -> u64 {
        std::env::var("AIY_OBJECT_BY_ID_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn transaction_cache_size(&self) -> u64 {
        std::env::var("AIY_TRANSACTION_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn executed_effect_cache_size(&self) -> u64 {
        std::env::var("AIY_EXECUTED_EFFECT_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn effect_cache_size(&self) -> u64 {
        std::env::var("AIY_EFFECT_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn events_cache_size(&self) -> u64 {
        std::env::var("AIY_EVENTS_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn transaction_objects_cache_size(&self) -> u64 {
        std::env::var("AIY_TRANSACTION_OBJECTS_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn backpressure_threshold(&self) -> u64 {
        std::env::var("AIY_BACKPRESSURE_THRESHOLD")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn backpressure_threshold_for_rpc(&self) -> u64 {
        std::env::var("AIY_BACKPRESSURE_THRESHOLD_FOR_RPC")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn fastpath_transaction_outputs_cache_size(&self) -> u64 {
        std::env::var("AIY_FASTPATH_TRANSACTION_OUTPUTS_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
...
impl NodeConfig {
    ...
    pub fn aiy_address(&self) -> AiyAddress {
...
pub struct ExpensiveSafetyCheckConfig {
    ...
    /// rebate of all live objects, and that the total AIY in the network remains
    /// the same.
    #[serde(default)]
    enable_epoch_aiy_conservation_check: bool,

    /// If enabled, we will check that the total AIY in all input objects of a tx
    /// (both the Move part and the storage rebate) matches the total AIY in all
    /// output objects of the tx + gas fees
    #[serde(default)]
    enable_deep_per_tx_aiy_conservation_check: bool,

    /// Disable epoch AIY conservation check even when we are running in debug mode.
    #[serde(default)]
    force_disable_epoch_aiy_conservation_check: bool,
...
impl ExpensiveSafetyCheckConfig {
    pub fn new_enable_all() -> Self {
        Self {
            enable_epoch_aiy_conservation_check: true,
            enable_deep_per_tx_aiy_conservation_check: true,
            force_disable_epoch_aiy_conservation_check: false,
    ...
    pub fn new_disable_all() -> Self {
        Self {
            enable_epoch_aiy_conservation_check: false,
            enable_deep_per_tx_aiy_conservation_check: false,
            force_disable_epoch_aiy_conservation_check: true,
    ...
    pub fn force_disable_epoch_aiy_conservation_check(&mut self) {
        self.force_disable_epoch_aiy_conservation_check = true;
    }

    pub fn enable_epoch_aiy_conservation_check(&self) -> bool {
        (self.enable_epoch_aiy_conservation_check || cfg!(debug_assertions))
            && !self.force_disable_epoch_aiy_conservation_check
    }
    ...
    pub fn enable_deep_per_tx_aiy_conservation_check(&self) -> bool {
        self.enable_deep_per_tx_aiy_conservation_check || cfg!(debug_assertions)
    }
...
/// Wrapper struct for AiyKeyPair that can be deserialized from a file path. Used by network, worker, and account keypair.
...
pub struct KeyPairWithPath {
    ...
    #[serde(skip)]
    keypair: OnceCell<Arc<AiyKeyPair>>,
}
...
impl KeyPairWithPath {
    pub fn new(kp: AiyKeyPair) -> Self {
        let cell: OnceCell<Arc<AiyKeyPair>> = OnceCell::new();
    ...
    pub fn keypair(&self) -> &AiyKeyPair {
        self.keypair
...
mod tests {
    ...
    use aiy_keys::keypair_file::{write_authority_keypair_to_file, write_keypair_to_file};
    use aiy_types::crypto::{get_key_pair_from_rng, AuthorityKeyPair, NetworkKeyPair, AiyKeyPair};
    ...
    fn legacy_validator_config() {
        const FILE: &str = include_str!("../data/aiy-node-legacy.yaml");
    ...
    fn load_key_pairs_to_node_config() {
        ...
        write_keypair_to_file(
            &AiyKeyPair::Ed25519(worker_key_pair.copy()),
            PathBuf::from("worker.key"),
        )
        .unwrap();
        write_keypair_to_file(
            &AiyKeyPair::Ed25519(network_key_pair.copy()),
