...
use aiy_types::authenticator_state::{get_authenticator_state, AuthenticatorStateInner};
use aiy_types::base_types::{ObjectID, AiyAddress};
use aiy_types::clock::Clock;
use aiy_types::committee::CommitteeWithNetworkMetadata;
use aiy_types::crypto::DefaultHash;
use aiy_types::deny_list_v1::{get_coin_deny_list, PerTypeDenyList};
...
use aiy_types::aiy_system_state::{
    get_aiy_system_state, get_aiy_system_state_wrapper, AiySystemState, AiySystemStateTrait,
    AiySystemStateWrapper, AiyValidatorGenesis,
};
...
use aiy_types::{AIY_BRIDGE_OBJECT_ID, AIY_RANDOMNESS_STATE_OBJECT_ID};
use tracing::trace;
...
impl Genesis {
    ...
    pub fn validator_set_for_tooling(&self) -> Vec<AiyValidatorGenesis> {
        self.aiy_system_object()
    ...
    pub fn committee_with_network(&self) -> CommitteeWithNetworkMetadata {
        self.aiy_system_object().get_current_epoch_committee()
    }

    pub fn reference_gas_price(&self) -> u64 {
        self.aiy_system_object().reference_gas_price()
    }

    // TODO: No need to return AiyResult. Also consider return &.
    pub fn committee(&self) -> AiyResult<Committee> {
        Ok(self.committee_with_network().committee().clone())
    }

    pub fn aiy_system_wrapper_object(&self) -> AiySystemStateWrapper {
        get_aiy_system_state_wrapper(&self.objects())
            .expect("Aiy System State Wrapper object must always exist")
    }

    pub fn aiy_system_object(&self) -> AiySystemState {
        get_aiy_system_state(&self.objects()).expect("Aiy System State object must always exist")
    }
    ...
    pub fn clock(&self) -> Clock {
        let clock = self
            .objects()
            .iter()
            .find(|o| o.id() == aiy_types::AIY_CLOCK_OBJECT_ID)
...
impl UnsignedGenesis {
    ...
    pub fn has_randomness_state_object(&self) -> bool {
        self.objects()
            .get_object(&AIY_RANDOMNESS_STATE_OBJECT_ID)
            .is_some()
    }

    pub fn has_bridge_object(&self) -> bool {
        self.objects().get_object(&AIY_BRIDGE_OBJECT_ID).is_some()
    }
...
impl GenesisCeremonyParameters {
    ...
    fn default_initial_stake_subsidy_distribution_amount() -> u64 {
        // 1M Aiy
        1_000_000 * aiy_types::gas_coin::MIST_PER_AIY
    }
    ...
    pub fn to_genesis_chain_parameters(&self) -> GenesisChainParameters {
        GenesisChainParameters {
            ...
            max_validator_count: aiy_types::governance::MAX_VALIDATOR_COUNT,
            min_validator_joining_stake: aiy_types::governance::MIN_VALIDATOR_JOINING_STAKE_MIST,
            validator_low_stake_threshold:
                aiy_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_MIST,
            validator_very_low_stake_threshold:
                aiy_types::governance::VALIDATOR_VERY_LOW_STAKE_THRESHOLD_MIST,
            validator_low_stake_grace_period:
                aiy_types::governance::VALIDATOR_LOW_STAKE_GRACE_PERIOD,
        }
    }
}
...
impl TokenDistributionSchedule {
    ...
    pub fn check_all_stake_operations_are_for_valid_validators<
        I: IntoIterator<Item = AiyAddress>,
    >(
        &self,
        validators: I,
    ) {
        ...
        let mut validators: HashMap<AiyAddress, u64> =
    ...
    #[allow(deprecated)]
    pub fn new_for_validators_with_default_allocation<I: IntoIterator<Item = AiyAddress>>(
        validators: I,
    ) -> Self {
        let mut supply = TOTAL_SUPPLY_MIST;
        let default_allocation = aiy_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_MIST;
    ...
    ///
    /// All entries in a token distribution schedule must add up to 10B Aiy.
    pub fn from_csv<R: std::io::Read>(reader: R) -> Result<Self> {
        ...
        assert_eq!(
            ...
            "Token Distribution Schedule must add up to 10B Aiy",
        );
    ...
    pub fn to_csv<W: std::io::Write>(&self, writer: W) -> Result<()> {
        ...
        writer.serialize(TokenAllocation {
            recipient_address: AiyAddress::default(),
...
pub struct TokenAllocation {
    pub recipient_address: AiyAddress,
    pub amount_mist: u64,

    /// Indicates if this allocation should be staked at genesis and with which validator
    pub staked_with_validator: Option<AiyAddress>,
}
...
impl TokenDistributionScheduleBuilder {
    ...
    #[allow(deprecated)]
    pub fn default_allocation_for_validators<I: IntoIterator<Item = AiyAddress>>(
        &mut self,
        validators: I,
    ) {
        let default_allocation = aiy_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_MIST;
