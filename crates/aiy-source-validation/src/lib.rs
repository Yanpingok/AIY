...
use aiy_move_build::CompiledPackage;
use aiy_sdk::apis::ReadApi;
use aiy_sdk::error::Error as SdkError;
use aiy_sdk::rpc_types::{AiyObjectDataOptions, AiyRawData, AiyRawMovePackage};
    ...
    fn local(&self, package: &CompiledPackage) -> Result<LocalModules, Error> {
        ...
        if self.verify_deps() {
            ...
            let deps_compiled_units: Vec<_> = deps_compiled_units
                .into_iter()
                .filter(|pkg| aiy_package.dependency_ids.published.contains_key(&pkg.0))
                .collect();
            ...
            for (package, module) in aiy_package.bytecode_deps.iter() {
...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    async fn pkg_for_address(&self, addr: AccountAddress) -> Result<AiyRawMovePackage, Error> {
        // Move packages are specified with an AccountAddress, but are
        // fetched from a aiy network via aiy_getObject, which takes an object ID
        let obj_id = ObjectID::from(addr);

        // fetch the Aiy object at the address specified for the package in the local resolution table
        ...
        let obj_read = self
            .rpc_client
            .get_object_with_options(obj_id, AiyObjectDataOptions::new().with_bcs())
            .await
            .map_err(Error::DependencyObjectReadFailure)?;

        let obj = obj_read
            .into_object()
            .map_err(Error::AiyObjectRefFailure)?
            .bcs
            .ok_or_else(|| {
        ...
        match obj {
            AiyRawData::Package(pkg) => Ok(pkg),
            AiyRawData::MoveObject(move_obj) => {
