...
use aiy_types::base_types::{
    is_primitive_type_tag, move_ascii_str_layout, move_utf8_str_layout, ObjectID, AiyAddress,
...
use aiy_types::id::{self, ID, RESOLVED_AIY_ID};
use aiy_types::move_package::MovePackage;
use aiy_types::object::bounded_visitor::BoundedVisitor;
use aiy_types::transfer::RESOLVED_RECEIVING_STRUCT;
use aiy_types::MOVE_STDLIB_ADDRESS;
...
pub enum AiyJsonValueErrorKind {
...
#[derive(Debug)]
pub struct AiyJsonValueError {
    kind: AiyJsonValueErrorKind,
    val: JsonValue,
}

impl AiyJsonValueError {
    pub fn new(val: &JsonValue, kind: AiyJsonValueErrorKind) -> Self {
        Self {
            kind,
            val: val.clone(),
        }
...
impl std::error::Error for AiyJsonValueError {}

impl fmt::Display for AiyJsonValueError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err_str = match self.kind {
            AiyJsonValueErrorKind::ValueTypeNotAllowed => {
                format!("JSON value type {} not allowed.", self.val)
            }
            AiyJsonValueErrorKind::ArrayNotHomogeneous => {
...
pub struct AiyJsonValue(JsonValue);
impl AiyJsonValue {
    pub fn new(json_value: JsonValue) -> Result<AiyJsonValue, anyhow::Error> {
    ...
    pub fn from_object_id(id: ObjectID) -> AiyJsonValue {
    ...
    pub fn to_aiy_address(&self) -> anyhow::Result<AiyAddress> {
        json_value_to_aiy_address(&self.0)
    }
    ...
    pub fn to_move_value(
        ...
        Ok(match (val, ty) {
                ...
                if struct_layout.type_ == ID::type_() =>
            {
                ...
                let addr = AiyAddress::from_str(s)?;
            ...
            (v, MoveTypeLayout::Address) => {
                let addr = json_value_to_aiy_address(v)?;
                R::MoveValue::Address(addr.into())
            }
...
impl Debug for AiyJsonValue {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", self.0)
    }
}

fn json_value_to_aiy_address(value: &JsonValue) -> anyhow::Result<AiyAddress> {
    match value {
        JsonValue::String(s) => {
        ...
        JsonValue::Array(bytes) => {
            ...
            let bytes = value_to_byte_array(bytes)
                .ok_or_else(|| anyhow!("Invalid input: Cannot parse input into AiyAddress."))?;
            Ok(AiyAddress::try_from(bytes)?)
        }
...
fn move_value_to_json(move_value: &MoveValue) -> Option<JsonValue> {
    Some(match move_value {
        ...
        MoveValue::Signer(v) | MoveValue::Address(v) => json!(AiyAddress::from(*v).to_string()),
...
impl FromStr for AiyJsonValue {
    ...
    fn from_str(s: &str) -> Result<Self, anyhow::Error> {
        ...
        AiyJsonValue::new(serde_json::from_str(s).unwrap_or_else(|_| try_escape_array(s)))
    }
}
...
pub fn check_valid_homogeneous(val: &JsonValue) -> Result<(), AiyJsonValueError> {
...
fn check_valid_homogeneous_rec(curr_q: &mut VecDeque<&JsonValue>) -> Result<(), AiyJsonValueError> {
    if curr_q.is_empty() {
        // Nothing to do
        return Ok(());
    }
    ...
    while let Some(v) = curr_q.pop_front() {
        let curr = match v {
            ...
            // Not valid
            _ => {
                return Err(AiyJsonValueError::new(
                    v,
                    AiyJsonValueErrorKind::ValueTypeNotAllowed,
                ))
            }
...
pub fn primitive_type(
    ...
    Some(match param {
        ...
        SignatureToken::Datatype(struct_handle_idx) => {
            ...
            } else if resolved_struct == RESOLVED_AIY_ID {
...
fn resolve_object_vec_arg(idx: usize, arg: &AiyJsonValue) -> Result<Vec<ObjectID>, anyhow::Error> {
    // Every elem has to be a string convertible to a ObjectID
...
pub fn resolve_move_function_args(
    ...
    function: Identifier,
    type_args: &[TypeTag],
    combined_args_json: Vec<AiyJsonValue>,
...
macro_rules! call_args {
        ($($value:expr),*) => {
        Ok::<_, anyhow::Error>(vec![$(aiy_json::call_arg!($value)?,)*])
    };
    }
...
macro_rules! call_arg {
    ($value:expr) => {{
        ...
        trait AiyJsonArg {
            fn to_aiy_json(&self) -> anyhow::Result<AiyJsonValue>;
        }
        // TODO: anyway to condense this?
        impl AiyJsonArg for &str {
            fn to_aiy_json(&self) -> anyhow::Result<AiyJsonValue> {
                AiyJsonValue::from_str(self)
            }
        }
        impl AiyJsonArg for String {
            fn to_aiy_json(&self) -> anyhow::Result<AiyJsonValue> {
                AiyJsonValue::from_str(&self)
            }
        }
        impl AiyJsonArg for aiy_types::base_types::ObjectID {
            fn to_aiy_json(&self) -> anyhow::Result<AiyJsonValue> {
                AiyJsonValue::from_str(&self.to_string())
            }
        }
        impl AiyJsonArg for aiy_types::base_types::AiyAddress {
            fn to_aiy_json(&self) -> anyhow::Result<AiyJsonValue> {
                AiyJsonValue::from_str(&self.to_string())
            }
        }
        impl AiyJsonArg for u64 {
            fn to_aiy_json(&self) -> anyhow::Result<AiyJsonValue> {
                AiyJsonValue::from_bcs_bytes(
                    Some(&aiy_json::MoveTypeLayout::U64),
                    &bcs::to_bytes(self)?,
                )
            }
        }
        impl AiyJsonArg for Vec<u8> {
            fn to_aiy_json(&self) -> anyhow::Result<AiyJsonValue> {
                AiyJsonValue::from_bcs_bytes(None, &self)
            }
        }
        impl AiyJsonArg for &[u8] {
            fn to_aiy_json(&self) -> anyhow::Result<AiyJsonValue> {
                AiyJsonValue::from_bcs_bytes(None, self)
            }
        }
...
macro_rules! type_args {
    ($($value:expr), *) => {{
        use aiy_json_rpc_types::AiyTypeTag;
        use aiy_types::TypeTag;
        trait AiyJsonTypeArg {
            fn to_aiy_json(&self) -> anyhow::Result<AiyTypeTag>;
        }
        impl <T: core::fmt::Display> AiyJsonTypeArg for T {
            fn to_aiy_json(&self) -> anyhow::Result<AiyTypeTag> {
                Ok(aiy_types::parse_aiy_type_tag(&self.to_string())?.into())
            }
        }
        Ok::<_, anyhow::Error>(vec![$($value.to_aiy_json()?,)*])
    }};
    }
