...
use aiy_json_rpc_types::{
    RPCTransactionRequestParams, AiyTransactionBlockBuilderMode, AiyTypeTag, TransactionBlockBytes,
};
...
pub trait TransactionBuilder {
    ...
    async fn transfer_object(
        &self,
        /// the transaction signer's Aiy address
        signer: AiyAddress,
        ...
        /// the recipient's Aiy address
        recipient: AiyAddress,
    ) -> RpcResult<TransactionBlockBytes>;

    /// Create an unsigned transaction to send AIY coin object to a Aiy address. The AIY object is also used as the gas object.
    #[method(name = "transferAiy")]
    async fn transfer_aiy(
        &self,
        /// the transaction signer's Aiy address
        signer: AiyAddress,
        /// the Aiy coin object to be used in this transaction
    ...
    async fn pay(
        ...
        /// the transaction signer's Aiy address
        signer: AiyAddress,
        /// the Aiy coins to be used in this transaction
    ...
    /// Send AIY coins to a list of addresses, following a list of amounts.
    /// This is for AIY coin only and does not require a separate gas coin object.
    /// Specifically, what pay_aiy does are:
    /// 1. debit each input_coin to create new coin following the order of
    /// amounts and assign it to the corresponding recipient.
    /// 2. accumulate all residual AIY from input coins left and deposit all AIY to the first
    ...
    async fn pay_aiy(
        ...
        /// the transaction signer's Aiy address
        signer: AiyAddress,
        /// the Aiy coins to be used in this transaction, including the coin for gas payment.
        input_coins: Vec<ObjectID>,
        /// the recipients' addresses, the length of this vector must be the same as amounts.
        recipients: Vec<AiyAddress>,
        /// the amounts to be transferred to recipients, following the same order
    ...
    /// Send all AIY coins to one recipient.
    /// This is for AIY coin only and does not require a separate gas coin object.
    /// Specifically, what pay_all_aiy does are:
    /// 1. accumulate all AIY from input coins and deposit all AIY to the first input coin
    ...
    #[method(name = "payAllAiy")]
    async fn pay_all_aiy(
        &self,
        /// the transaction signer's Aiy address
        signer: AiyAddress,
        /// the Aiy coins to be used in this transaction, including the coin for gas payment.
    ...
    async fn move_call(
        ...
        type_arguments: Vec<AiyTypeTag>,
        /// the arguments to be passed into the Move function, in [AiyJson](https://docs.sui.io/build/sui-json) format
        arguments: Vec<AiyJsonValue>,
        /// gas object to be used in this transaction, node will pick one from the signer's possession if not provided
        gas: Option<ObjectID>,
        /// the gas budget, the transaction will fail if the gas cost exceed the budget
        gas_budget: BigInt<u64>,
        /// Whether this is a Normal transaction or a Dev Inspect Transaction. Default to be `AiyTransactionBlockBuilderMode::Commit` when it's None.
        execution_mode: Option<AiyTransactionBlockBuilderMode>,
    ) -> RpcResult<TransactionBlockBytes>;
    ...
    async fn batch_transaction(
        ...
        txn_builder_mode: Option<AiyTransactionBlockBuilderMode>,
    ) -> RpcResult<TransactionBlockBytes>;
    ...
    async fn request_add_stake(
        ...
        /// the transaction signer's Aiy address
        signer: AiyAddress,
        /// Coin<AIY> object to stake
        coins: Vec<ObjectID>,
        /// stake amount
        amount: Option<BigInt<u64>>,
        /// the validator's Aiy address
