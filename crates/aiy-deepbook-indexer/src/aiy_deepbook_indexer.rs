...
use aiy_indexer_builder::progress::ProgressSavingPolicy;
use aiy_types::base_types::ObjectID;
use aiy_types::transaction::{Command, TransactionDataAPI};
use tracing::info;

use aiy_indexer_builder::indexer_builder::{DataMapper, IndexerProgressStore, Persistent};
use aiy_indexer_builder::aiy_datasource::CheckpointTxnData;
use aiy_indexer_builder::{Task, Tasks, LIVE_TASK_TARGET_CHECKPOINT};
...
use crate::schema::{
    ...
    aiy_error_transactions, trade_params_update, votes,
};
use crate::types::{
    Balances, Flashloan, OrderFill, OrderUpdate, OrderUpdateStatus, PoolPrice, ProcessedTxnData,
    Proposals, Rebates, Stakes, AiyTxnError, TradeParamsUpdate, Votes,
};
            ...
            .transaction(|conn| {
                async move {
                    ...
                    if !error_transactions_batch.is_empty() {
                        tasks.push(
                            diesel::insert_into(aiy_error_transactions::table)
...
/// Data mapper impl
#[derive(Clone)]
pub struct AiyDeepBookDataMapper {
    pub metrics: DeepBookIndexerMetrics,
    pub package_id: ObjectID,
}

impl DataMapper<CheckpointTxnData, ProcessedTxnData> for AiyDeepBookDataMapper {
    fn map(
        &self,
        ...
        match &data.events {
            Some(events) => {
                let processed_aiy_events =
                    events
                        .data
                        .iter()
                        .enumerate()
                        .try_fold(vec![], |mut result, (i, ev)| {
                            if let Some(data) = process_aiy_event(
                                ev,
                                i,
                                &data,
                                checkpoint_num,
                ...
                if !processed_aiy_events.is_empty() {
                    info!(
                        "AIY: Extracted {} deepbook data entries for tx {}.",
                        processed_aiy_events.len(),
                        data.transaction.digest()
                    );
                }
                Ok(processed_aiy_events)
            }
            None => {
