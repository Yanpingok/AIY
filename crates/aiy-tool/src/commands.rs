...
use aiy_replay::{execute_replay_command, ReplayToolCommand};
use aiy_sdk::{rpc_types::AiyTransactionBlockResponseOptions, AiyClient, AiyClientBuilder};
use aiy_types::messages_consensus::ConsensusTransaction;
...
use aiy_core::authority_aggregator::AuthorityAggregatorBuilder;
use aiy_types::messages_checkpoint::{
    CheckpointRequest, CheckpointResponse, CheckpointSequenceNumber,
};
use aiy_types::transaction::{SenderSignedData, Transaction};
...
pub enum ToolCommand {
    ...
    FetchObject {
        ...
        /// prints tabular output aiytable for processing with unix tools. For
        /// instance, to quickly check that all validators agree on the history of an object:
        /// ```text
        /// $ aiy-tool fetch-object --id 0x260efde76ebccf57f4c5e951157f5c361cde822c \
        ///      --genesis $HOME/.aiy/aiy_config/genesis.blob \
    ...
    /// contains Move bytecode (aiytable for passing into a disassembler).
    ...
    Replay {
        ...
        #[arg(
            ...
            If not specified we will use the default network config file at ~/.aiy-replay/network-config.yaml"
        )]
...
async fn check_locked_object(
    ...
    let clients = Arc::new(make_clients(aiy_client).await?);
    ...
    pub async fn execute(self, tracing_handle: TracingHandle) -> Result<(), anyhow::Error> {
        match self {
            ...
            ToolCommand::LockedObject {
                ...
                let aiy_client =
                    Arc::new(AiyClientBuilder::default().build(fullnode_rpc_url).await?);
            ...
            ToolCommand::DumpPackages {
                ...
                aiy_package_dump::dump(rpc_url, output_dir, before_checkpoint).await?;
            }
                ...
                if !concise {
                    ...
                    for (i, val_info) in genesis.validator_set_for_tooling().iter().enumerate() {
                        ...
                        println!(
                            ...
                            i,
                            metadata.name,
                            metadata.aiy_pubkey_bytes().concise(),
            ...
            ToolCommand::RestoreFromDBCheckpoint {
                ...
                let config = aiy_config::NodeConfig::load(config_path)?;
