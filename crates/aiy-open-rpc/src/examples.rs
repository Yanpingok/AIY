...
use aiy_json_rpc::error::Error;
use aiy_json_rpc_types::BcsEvent;
use aiy_json_rpc_types::DevInspectArgs;
use aiy_json_rpc_types::{
    ...
    Page, ProtocolConfigResponse, RPCTransactionRequestParams, Stake, StakeStatus, AiyCoinMetadata,
    AiyCommittee, AiyData, AiyEvent, AiyExecutionStatus, AiyGetPastObjectRequest, AiyMoveAbility,
    AiyMoveAbilitySet, AiyMoveNormalizedFunction, AiyMoveNormalizedModule, AiyMoveNormalizedStruct,
    AiyMoveNormalizedType, AiyMoveVisibility, AiyObjectData, AiyObjectDataFilter,
    AiyObjectDataOptions, AiyObjectRef, AiyObjectResponse, AiyObjectResponseQuery, AiyParsedData,
    AiyPastObjectResponse, AiyTransactionBlock, AiyTransactionBlockData,
    AiyTransactionBlockEffects, AiyTransactionBlockEffectsV1, AiyTransactionBlockEvents,
    AiyTransactionBlockResponse, AiyTransactionBlockResponseOptions,
    AiyTransactionBlockResponseQuery, TransactionBlockBytes, TransactionBlocksPage,
    TransactionFilter, TransferObjectParams,
};
use aiy_json_rpc_types::{AiyTypeTag, ValidatorApy, ValidatorApys};
use aiy_open_rpc::ExamplePairing;
use aiy_protocol_config::Chain;
use aiy_protocol_config::ProtocolConfig;
use aiy_types::balance::Supply;
use aiy_types::base_types::random_object_ref;
use aiy_types::base_types::{
    FullObjectRef, MoveObjectType, ObjectDigest, ObjectID, ObjectType, SequenceNumber, AiyAddress,
    TransactionDigest,
};
use aiy_types::committee::Committee;
use aiy_types::crypto::{get_key_pair_from_rng, AccountKeyPair, AggregateAuthoritySignature};
use aiy_types::digests::TransactionEventsDigest;
use aiy_types::dynamic_field::{DynamicFieldInfo, DynamicFieldName, DynamicFieldType};
...
use aiy_types::object::MoveObject;
...
use aiy_types::transaction::ObjectArg;
use aiy_types::transaction::TEST_ONLY_GAS_UNIT_FOR_TRANSFER;
use aiy_types::transaction::{CallArg, TransactionData};
use aiy_types::utils::to_sender_signed_transaction;
use aiy_types::{parse_aiy_struct_tag, AIY_FRAMEWORK_PACKAGE_ID};
...
impl RpcExampleProvider {
    ...
    pub fn examples(&mut self) -> BTreeMap<String, Vec<ExamplePairing>> {
        [
            ...
            self.aiy_get_committee_info(),
            self.aiy_get_reference_gas_price(),
            self.aiyx_get_all_balances(),
            self.aiyx_get_all_coins(),
            self.aiyx_get_balance(),
            self.aiyx_get_coin_metadata(),
            self.aiy_get_latest_checkpoint_sequence_number(),
            self.aiyx_get_coins(),
            self.aiyx_get_total_supply(),
            self.aiyx_get_dynamic_fields(),
            self.aiyx_get_dynamic_field_object(),
            self.aiyx_get_owned_objects(),
            self.aiy_get_move_function_arg_types(),
            self.aiy_get_normalized_move_function(),
            self.aiy_get_normalized_move_module(),
            self.aiy_get_normalized_move_modules_by_package(),
            self.aiy_get_normalized_move_struct(),
            self.multi_get_objects_example(),
            self.multi_get_transaction_blocks(),
            self.aiyx_get_validators_apy(),
            self.aiyx_get_dynamic_fields(),
            self.aiyx_get_dynamic_field_object(),
            self.aiyx_get_owned_objects(),
            self.aiyx_query_events(),
            self.aiyx_get_latest_aiy_system_state(),
            self.get_protocol_config(),
            self.aiy_get_chain_identifier(),
            self.aiyx_get_stakes(),
            self.aiyx_get_stakes_by_ids(),
            self.aiyx_resolve_name_service_address(),
            self.aiyx_resolve_name_service_names(),
            self.aiy_try_multi_get_past_objects(),
        ]
        .into_iter()
    ...
    fn batch_transaction_examples(&mut self) -> Examples {
        let signer = AiyAddress::from(ObjectID::new(self.rng.gen()));
        let recipient = AiyAddress::from(ObjectID::new(self.rng.gen()));
        ...
        let tx_params = vec![
            RPCTransactionRequestParams::MoveCallRequestParams(MoveCallParams {
                package_object_id: AIY_FRAMEWORK_PACKAGE_ID,
                module: "pay".to_string(),
                function: "split".to_string(),
                type_arguments: vec![AiyTypeTag::new("0x2::aiy::AIY".to_string())],
                arguments: vec![
                    AiyJsonValue::new(json!(coin_ref.0)).unwrap(),
                    AiyJsonValue::new(json!(random_amount)).unwrap(),
                ],
            }),
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "options",
                        json!(AiyTransactionBlockResponseOptions::full_content()),
                    ),
                    (
                        "request_type",
    ...
    fn dev_inspect_transaction_block(&mut self) -> Examples {
        ...
        let dev_inspect_results = DevInspectResults {
            ...
            events: AiyTransactionBlockEvents { data: vec![] },
            results: None,
            error: None,
            raw_txn_data: vec![],
            raw_effects: vec![],
        ...
        Examples::new(
            "aiy_devInspectTransactionBlock",
            vec![ExamplePairing::new(
                "Runs the transaction in dev-inspect mode. Which allows for nearly any transaction (or Move call) with any arguments. Detailed results are provided, including both the transaction effects and any return values.",
                vec![
                    ("sender_address", json!(AiyAddress::from(ObjectID::new(self.rng.gen())))),
                    ("tx_bytes", json!(tx_bytes.tx_bytes)),
                    ("gas_price", json!(1000)),
                    ("epoch", json!(8888)),
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    ("options", json!(AiyObjectDataOptions::full_content())),
                ],
                json!(objects),
            )],
        )
    ...
    fn get_object_responses(&mut self, object_count: usize) -> Vec<AiyObjectResponse> {
        (0..object_count)
            .map(|_| {
                let object_id = ObjectID::new(self.rng.gen());
                let coin = GasCoin::new(object_id, 100000000);

                AiyObjectResponse::new_with_data(AiyObjectData {
                    content: Some(
                        AiyParsedData::try_from_object(
                            coin.to_object(SequenceNumber::from_u64(1)),
                            GasCoin::layout(),
                        )
                        .unwrap(),
                    ),
                    owner: Some(Owner::AddressOwner(AiyAddress::from(ObjectID::new(
                        self.rng.gen(),
                    )))),
    ...
    fn get_past_object_example(&mut self) -> Examples {
        ...
        let result = AiyPastObjectResponse::VersionFound(AiyObjectData {
            content: Some(
                AiyParsedData::try_from_object(
                    coin.to_object(SequenceNumber::from_u64(1)),
                    GasCoin::layout(),
    ...
    fn get_owned_objects(&mut self) -> Examples {
        let owner = AiyAddress::from(ObjectID::new(self.rng.gen()));
        let result = (0..4)
            .map(|_| AiyObjectData {
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "query",
                        json!(AiyObjectResponseQuery {
                            filter: Some(AiyObjectDataFilter::StructType(
                                StructTag::from_str("0x2::coin::Coin<0x2::aiy::AIY>").unwrap()
                            )),
                            options: Some(
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "options",
                        json!(AiyTransactionBlockResponseOptions::new()
                            .with_input()
                            .with_effects()
                            .with_events()),
                    ),
    ...
    fn query_transaction_blocks(&mut self) -> Examples {
        ...
        let data = data
            .into_iter()
            .map(AiyTransactionBlockResponse::new)
            .collect();
        ...
        Examples::new(
            "aiyx_queryTransactionBlocks",
            vec![ExamplePairing::new(
                "Returns the transaction digest for specified query criteria.",
                vec![
                    (
                        "query",
                        json!(AiyTransactionBlockResponseQuery {
    ...
    fn get_transfer_data_response(
        ...
        let object_change = ObjectChange::Transferred {
            ...
            object_type: parse_aiy_struct_tag("0x2::example::Object").unwrap(),
        ...
        let result = AiyTransactionBlockResponse {
            digest: *tx_digest,
            effects: Some(AiyTransactionBlockEffects::V1(
                AiyTransactionBlockEffectsV1 {
                    status: AiyExecutionStatus::Success,
                    executed_epoch: 0,
                    ...
                    gas_object: OwnedObjectRef {
                        ...
                        reference: AiyObjectRef::from(gas_ref),
                    },
            ...
            timestamp_ms: None,
            transaction: Some(AiyTransactionBlock {
    ...
    fn get_events(&mut self) -> Examples {
        ...
        let event = AiyEvent {
            ...
            sender: AiyAddress::from(ObjectID::new(self.rng.gen())),
            type_: parse_aiy_struct_tag("0x9::test::TestEvent").unwrap(),
    ...
    fn aiy_get_reference_gas_price(&mut self) -> Examples {
        let result = 1000;
        Examples::new(
            "aiyx_getReferenceGasPrice",
            vec![ExamplePairing::new(
    ...
    fn aiyx_get_all_balances(&mut self) -> Examples {
        let address = AiyAddress::from(ObjectID::new(self.rng.gen()));

        let result = Balance {
            coin_type: "0x2::aiy::AIY".to_string(),
    ...
    fn aiyx_get_all_coins(&mut self) -> Examples {
        let limit = 3;
        let owner = AiyAddress::from(ObjectID::new(self.rng.gen()));
    ...
    fn aiyx_get_balance(&mut self) -> Examples {
        let owner = AiyAddress::from(ObjectID::new(self.rng.gen()));
    ...
    fn aiyx_get_coin_metadata(&mut self) -> Examples {
        let result = AiyCoinMetadata {
            decimals: 9,
            name: "Usdc".to_string(),
    ...
    fn aiy_get_latest_checkpoint_sequence_number(&mut self) -> Examples {
        let result = "507021";
        Examples::new(
            "aiy_getLatestCheckpointSequenceNumber",
            vec![ExamplePairing::new(
    ...
    fn aiyx_get_coins(&mut self) -> Examples {
        let coin_type = "0x2::aiy::AIY".to_string();
        let owner = AiyAddress::from(ObjectID::new(self.rng.gen()));
        let coins = (0..3)
            .map(|_| Coin {
                coin_type: coin_type.clone(),
        ...
        Examples::new(
            "aiyx_getCoins",
            vec![ExamplePairing::new(
                "Gets all AIY coins owned by the address provided. Return a paginated list of `limit` results per page. Similar to `aiyx_getAllCoins`, but provides a way to filter by coin type.",
                vec![
                    ("owner", json!(owner)),
    ...
    fn aiyx_get_total_supply(&mut self) -> Examples {
    ...
    fn aiy_get_move_function_arg_types(&mut self) -> Examples {
        let result = vec![
    ...
    fn aiy_get_normalized_move_function(&mut self) -> Examples {
        let ability_set = AiyMoveAbilitySet {
            abilities: vec![AiyMoveAbility::Store, AiyMoveAbility::Key],
        };

        let result = AiyMoveNormalizedFunction {
            is_entry: false,
            type_parameters: vec![ability_set],
            parameters: vec![AiyMoveNormalizedType::U64],
            visibility: AiyMoveVisibility::Public,
            return_: vec![AiyMoveNormalizedType::U64],
        };

        Examples::new(
            "aiy_getNormalizedMoveFunction",
            vec![ExamplePairing::new(
    ...
    fn aiy_get_normalized_move_module(&mut self) -> Examples {
        let result = AiyMoveNormalizedModule {
    ...
    fn aiy_get_normalized_move_modules_by_package(&mut self) -> Examples {
        let result = AiyMoveNormalizedModule {
            address: ObjectID::new(self.rng.gen()).to_string(),
            exposed_functions: BTreeMap::new(),
            file_format_version: 6,
        ...
        Examples::new(
            "aiy_getNormalizedMoveModulesByPackage",
            vec![ExamplePairing::new(
    ...
    fn aiy_get_normalized_move_struct(&mut self) -> Examples {
        let abilities = AiyMoveAbilitySet {
            abilities: vec![AiyMoveAbility::Store, AiyMoveAbility::Key],
        };
        let fields = vec![].into_iter().collect::<Vec<_>>();
        let type_parameters = vec![].into_iter().collect::<Vec<_>>();
        let result = AiyMoveNormalizedStruct {
            abilities,
            fields,
            type_parameters,
        };
    ...
    fn aiyx_get_validators_apy(&mut self) -> Examples {
        let result = vec![
            ValidatorApy {
                address: AiyAddress::from(ObjectID::new(self.rng.gen())),
                apy: 0.06,
            },
            ValidatorApy {
                address: AiyAddress::from(ObjectID::new(self.rng.gen())),
    ...
    fn aiyx_get_dynamic_fields(&mut self) -> Examples {
        let object_id = ObjectID::new(self.rng.gen());
        ...
        Examples::new("aiyx_getDynamicFields",
        vec![ExamplePairing::new(
    ...
    fn aiyx_get_dynamic_field_object(&mut self) -> Examples {
        ...
        let struct_tag = parse_aiy_struct_tag("0x9::test::TestField").unwrap();
        let resp = AiyObjectResponse::new_with_data(AiyObjectData {
            content: Some(
                AiyParsedData::try_from_object(
                    unsafe {
            ...
            type_: Some(ObjectType::Struct(MoveObjectType::from(
                parse_aiy_struct_tag("0x9::test::TestField").unwrap(),
            ))),
            bcs: None,
            display: None,
        });
        Examples::new(
            "aiyx_getDynamicFieldObject",
            vec![ExamplePairing::new(
    ...
    fn aiyx_get_owned_objects(&mut self) -> Examples {
        let owner = AiyAddress::from(ObjectID::new(self.rng.gen()));
        ...
        let filter = Some(AiyObjectDataFilter::MatchAll(vec![
            AiyObjectDataFilter::StructType(
                StructTag::from_str("0x2::coin::Coin<0x2::aiy::AIY>").unwrap(),
            ),
            AiyObjectDataFilter::AddressOwner(owner),
            AiyObjectDataFilter::Version(version),
        ]));
        let query = json!(AiyObjectResponseQuery { filter, options });
        let object_id = ObjectID::new(self.rng.gen());
    ...
    fn aiyx_query_events(&mut self) -> Examples {
    ...
    fn aiyx_get_latest_aiy_system_state(&mut self) -> Examples {
        let result = "some_system_state";
        Examples::new(
            "aiyx_getLatestAiySystemState",
            vec![ExamplePairing::new(
                "Gets objects owned by the address in the request.",
                vec![],
                json!(result),
    ...
    fn aiy_get_chain_identifier(&mut self) -> Examples {
        let result = "4c78adac".to_string();
        Examples::new(
            "aiy_getChainIdentifier",
            vec![ExamplePairing::new(
    ...
    fn aiyx_get_stakes(&mut self) -> Examples {
        let principal = 200000000000;
        let owner = AiyAddress::from(ObjectID::new(self.rng.gen()));
        let result = vec![
            DelegatedStake {
                validator_address: AiyAddress::from(ObjectID::new(self.rng.gen())),
                staking_pool: ObjectID::new(self.rng.gen()),
                stakes: vec![
                    Stake {
                        staked_aiy_id: ObjectID::new(self.rng.gen()),
    ...
    fn aiyx_get_stakes_by_ids(&mut self) -> Examples {
        let principal = 200000000000;
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                "Returns the staking information for the address the request provides.",
                vec![("staked_aiy_ids", json!(vec![stake1, stake2]))],
                json!(result),
            )],
        )
    }

    fn aiyx_resolve_name_service_address(&mut self) -> Examples {
        let result = ObjectID::new(self.rng.gen());
        Examples::new(
            "aiyx_resolveNameServiceAddress",
            vec![ExamplePairing::new(
                "Returns the resolved address for the name the request provides.",
                vec![("name", json!("example.aiy".to_string()))],
                json!(result),
            )],
        )
    }

    fn aiyx_resolve_name_service_names(&mut self) -> Examples {
        let next_cursor = Some(ObjectID::new(self.rng.gen()));
        let object_id = ObjectID::new(self.rng.gen());
        let result = Page {
            data: vec!["example.aiy".to_string()],
            next_cursor,
            has_next_page: false,
        };
        Examples::new(
            "aiyx_resolveNameServiceNames",
            vec![ExamplePairing::new(
                "Returns the AiyNS name for the address the request provides. Currently, the API returns only the first name in cases where there are multiple. Future support will use the cursor ID and limit values in the request to control pagination of the response for addresses with multiple names.",
                vec![
    ...
    fn aiy_try_multi_get_past_objects(&mut self) -> Examples {
        ...
        let result = vec![
            AiyPastObjectResponse::VersionFound(AiyObjectData {
                content: Some(
                    AiyParsedData::try_from_object(
                        coin.to_object(SequenceNumber::from_u64(1)),
                        GasCoin::layout(),
