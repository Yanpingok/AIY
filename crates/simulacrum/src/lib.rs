...
//! provides the [`Simulacrum`] type which is a implementation or instantiation of a aiy
...
use aiy_config::{genesis, transaction_deny_config::TransactionDenyConfig};
use aiy_framework_snapshot::load_bytecode_snapshot;
use aiy_protocol_config::ProtocolVersion;
use aiy_storage::blob::{Blob, BlobEncoding};
...
use aiy_types::base_types::{AuthorityName, ObjectID, ObjectRef, SequenceNumber, VersionNumber};
use aiy_types::crypto::{get_account_key_pair, AccountKeyPair, AuthoritySignature};
use aiy_types::digests::{ChainIdentifier, ConsensusCommitDigest};
...
use aiy_types::storage::{ObjectStore, ReadStore, RpcStateReader};
use aiy_types::aiy_system_state::epoch_start_aiy_system_state::EpochStartSystemState;
use aiy_types::transaction::EndOfEpochTransactionKind;
use aiy_types::{
    base_types::{EpochId, AiyAddress},
    committee::Committee,
    effects::TransactionEffects,
    error::ExecutionError,
    gas_coin::MIST_PER_AIY,
...
use aiy_core::mock_checkpoint_builder::{MockCheckpointBuilder, ValidatorKeypairProvider};
use aiy_types::messages_checkpoint::{CheckpointContents, CheckpointSequenceNumber};
use aiy_types::{
    gas_coin::GasCoin,
...
/// A `Simulacrum` of Aiy.
///
/// This type represents a simulated instantiation of a Aiy blockchain that needs to be driven
...
impl<R, S: store::SimulatorStore> Simulacrum<R, S> {
    pub fn new_with_network_config_store(config: &NetworkConfig, rng: R, store: S) -> Self {
        ...
        let epoch_state = EpochState::new(genesis.aiy_system_object());
    ...
    /// use aiy_types::gas_coin::MIST_PER_AIY;
    ///
    /// # fn main() {
    /// let mut simulacrum = Simulacrum::new();
    /// let (account, kp, gas) = simulacrum.funded_account(MIST_PER_AIY).unwrap();
    ///
    /// // `account` is a fresh AiyAddress that owns a Coin<AIY> object with single AIY in it,
    /// // referred to by `gas`.
    /// // ...
    /// # }
    /// ```
    pub fn funded_account(
        &mut self,
        amount: u64,
    ) -> Result<(AiyAddress, AccountKeyPair, ObjectRef)> {
    ...
    /// # fn main() {
    /// let mut simulacrum = Simulacrum::new();
    /// let address = AiyAddress::generate(simulacrum.rng());
    /// simulacrum.request_gas(address, MIST_PER_AIY).unwrap();
    ///
    /// // `account` now has a Coin<AIY> object with single AIY in it.
    /// // ...
    /// # }
    /// ```
    pub fn request_gas(&mut self, address: AiyAddress, amount: u64) -> Result<TransactionEffects> {
        ...
        let object = self
            ...
            .find(|object| {
                object.is_gas_coin() && object.get_coin_value_unsafe() > amount + MIST_PER_AIY
            })
            .ok_or_else(|| {
                anyhow!("unable to find a coin with enough to satisfy request for {amount} Mist")
            })?;

        let gas_data = aiy_types::transaction::GasData {
        ...
        let pt = {
            let mut builder =
                aiy_types::programmable_transaction_builder::ProgrammableTransactionBuilder::new();
            builder.transfer_aiy(address, Some(amount));
            builder.finish()
        };

        let kind = aiy_types::transaction::TransactionKind::ProgrammableTransaction(pt);
        let tx_data =
            aiy_types::transaction::TransactionData::new_with_gas_data(kind, *sender, gas_data);
    ...
    fn process_data_ingestion(
        ...
        if let Some(path) = &self.data_ingestion_path {
            ...
            let checkpoint_data: aiy_types::full_checkpoint_content::CheckpointData = self
...
impl<T, V: store::SimulatorStore> ReadStore for Simulacrum<T, V> {
    ...
    fn get_checkpoint_contents_by_digest(
        &self,
        digest: &aiy_types::messages_checkpoint::CheckpointContentsDigest,
    ...
    fn get_transaction(
        &self,
        tx_digest: &aiy_types::digests::TransactionDigest,
    ...
    fn get_events(
        &self,
        event_digest: &aiy_types::digests::TransactionDigest,
    ) -> Option<aiy_types::effects::TransactionEvents> {
    ...
    fn get_full_checkpoint_contents(
        ...
        _digest: &aiy_types::messages_checkpoint::CheckpointContentsDigest,
    ...
    fn get_unchanged_loaded_runtime_objects(
        &self,
        _digest: &aiy_types::digests::TransactionDigest,
...
impl<T: Send + Sync, V: store::SimulatorStore + Send + Sync> RpcStateReader for Simulacrum<T, V> {
    ...
    fn get_chain_identifier(
        &self,
    ) -> aiy_types::storage::error::Result<aiy_types::digests::ChainIdentifier> {
        Ok(self
            .store()
    ...
    fn indexes(&self) -> Option<&dyn aiy_types::storage::RpcIndexes> {
        None
    }
    ...
    ) -> aiy_types::storage::error::Result<Option<move_core_types::annotated_value::MoveTypeLayout>>
    {
        Ok(None)
    }
}
...
impl Simulacrum {
    ...
    /// existing code for generating transactions in aiy-test-transaction-builder by defining a trait
    /// that both WalletContext and Simulacrum implement. Then we can remove this function.
    pub fn transfer_txn(&mut self, recipient: AiyAddress) -> (Transaction, u64) {
        let (sender, key) = self.keystore().accounts().next().unwrap();
        let sender = *sender;
        ...
        let pt = {
            ...
            builder.transfer_aiy(recipient, Some(transfer_amount));
            builder.finish()
        };
...
mod tests {
    ...
    use aiy_types::{
        base_types::AiyAddress, effects::TransactionEffectsAPI, gas_coin::GasCoin,
