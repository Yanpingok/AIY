...
/// (i.e., Narwhal, Aiy, Ethereum etc) should never collide, so that even when a signing
...
impl Intent {
    pub fn aiy_app(scope: IntentScope) -> Self {
        Self {
...
/// be any type that implements [trait Serialize]. *ALL* signatures in Aiy must commits
...
/// A 1-byte domain separator for hashing Object ID in Aiy. It is starting from 0xf0
/// to ensure no hashing collision for any ObjectID vs AiyAddress which is derived
/// as the hash of `flag || pubkey`. See `aiy_types::crypto::SignatureScheme::flag()`.
#[derive(Serialize_repr, Deserialize_repr, Copy, Clone, PartialEq, Eq, Debug, Hash)]
#[repr(u8)]
