...
use aiy_json::{is_receiving_argument, resolve_move_function_args, ResolvedCallArg, AiyJsonValue};
use aiy_json_rpc_types::{
    RPCTransactionRequestParams, AiyData, AiyObjectDataOptions, AiyObjectResponse, AiyRawData,
    AiyTypeTag,
};
use aiy_protocol_config::ProtocolConfig;
use aiy_types::base_types::{
    FullObjectRef, ObjectID, ObjectInfo, ObjectRef, ObjectType, AiyAddress,
};
use aiy_types::error::UserInputError;
use aiy_types::gas_coin::GasCoin;
use aiy_types::governance::{ADD_STAKE_MUL_COIN_FUN_NAME, WITHDRAW_STAKE_FUN_NAME};
...
use aiy_types::{coin, fp_ensure, AIY_FRAMEWORK_PACKAGE_ID, AIY_SYSTEM_PACKAGE_ID};
...
impl TransactionBuilder {
    ...
    pub async fn select_gas(
        ...
        if let Some(gas) = input_gas {
            ...
            for obj in gas_objs {
                let response = self
                    .0
                    .get_object_with_options(obj.object_id, AiyObjectDataOptions::new().with_bcs())
                    .await?;
            ...
            Err(anyhow!("Cannot find gas coin for signer address {signer} with amount sufficient for the required gas budget {gas_budget}. If you are using the pay or transfer commands, you can use pay-aiy or transfer-aiy commands instead, which will use the only object as gas payment."))
        }
    }
    ...
    pub async fn transfer_aiy(
        &self,
        signer: AiyAddress,
        aiy_object_id: ObjectID,
        gas_budget: u64,
        recipient: AiyAddress,
        amount: Option<u64>,
    ) -> anyhow::Result<TransactionData> {
        let object = self.get_object_ref(aiy_object_id).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_transfer_aiy(
    ...
    pub async fn pay(
        ...
        if let Some(gas) = gas {
            if input_coins.contains(&gas) {
                return Err(anyhow!("Gas coin is in input coins of Pay transaction, use PayAiy transaction instead!"));
            }
        }
    ...
    /// Construct a transaction kind for the PayAiy transaction type
    pub fn pay_aiy_tx_kind(
        &self,
        recipients: Vec<AiyAddress>,
        amounts: Vec<u64>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_aiy(recipients.clone(), amounts.clone())?;
        let pt = builder.finish();
        let tx_kind = TransactionKind::programmable(pt);
        Ok(tx_kind)
    }

    pub async fn pay_aiy(
        &self,
        signer: AiyAddress,
        input_coins: Vec<ObjectID>,
        recipients: Vec<AiyAddress>,
    ...
    pub fn pay_all_aiy_tx_kind(&self, recipient: AiyAddress) -> TransactionKind {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn pay_all_aiy(
        ...
        let gas_object_ref = coin_refs.remove(0);
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_pay_all_aiy(
            signer,
            coin_refs,
            recipient,
            gas_object_ref,
    ...
    pub async fn move_call_tx_kind(
        ...
        module: &str,
        function: &str,
        type_args: Vec<AiyTypeTag>,
        call_args: Vec<AiyJsonValue>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn upgrade_tx_kind(
        ...
        let upgrade_capability = self
            .0
            .get_object_with_options(upgrade_capability, AiyObjectDataOptions::new().with_owner())
            .await?
            .into_object()?;
    ...
    pub async fn merge_coins_tx_kind(
        ...
        let coin = self
            .0
            .get_object_with_options(primary_coin, AiyObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
    ...
    pub async fn request_add_stake(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            let arguments = vec![
                builder.input(CallArg::AIY_SYSTEM_MUT).unwrap(),
    ...
    pub async fn request_withdraw_stake(
        ...
        let staked_aiy = self.get_object_ref(staked_aiy).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        let gas = self
        ...
        TransactionData::new_move_call(
            ...
            vec![
                ...
                CallArg::Object(ObjectArg::ImmOrOwnedObject(staked_aiy)),
            ],
            gas_budget,
            gas_price,
        )
    ...
    pub async fn get_full_object_ref(&self, object_id: ObjectID) -> anyhow::Result<FullObjectRef> {
        let object_data = self
            .0
            .get_object_with_options(object_id, AiyObjectDataOptions::new().with_owner())
            .await?
            .into_object()?;
    ...
    async fn get_object_ref_and_type(
        ...
        let object = self
            .0
            .get_object_with_options(object_id, AiyObjectDataOptions::new().with_type())
            .await?
            .into_object()?;
    ...
    pub async fn get_full_object_ref_and_type(
        ...
        let object_data = self
            ...
            .get_object_with_options(
                object_id,
                AiyObjectDataOptions::new().with_owner().with_type(),
            )
            .await?
            .into_object()?;
