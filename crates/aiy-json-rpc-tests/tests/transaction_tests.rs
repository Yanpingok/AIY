...
use aiy_json_rpc_types::AiyTransactionBlockDataAPI;
use aiy_json_rpc_types::AiyTransactionBlockResponseQuery;
use aiy_json_rpc_types::TransactionFilter;
use aiy_json_rpc_types::{
    AiyObjectDataOptions, AiyObjectResponseQuery, AiyTransactionBlockResponse,
    AiyTransactionBlockResponseOptions, TransactionBlockBytes,
};
...
use aiy_types::transaction::SenderSignedData;
use aiy_types::transaction::TransactionData;
use aiy_types::AIY_FRAMEWORK_ADDRESS;
use test_cluster::TestClusterBuilder;

use aiy_json_rpc_api::{IndexerApiClient, TransactionBuilderClient, WriteApiClient};
...
async fn test_get_transaction_block() -> Result<(), anyhow::Error> {
    ...
    let mut tx_responses: Vec<AiyTransactionBlockResponse> = Vec::new();
    ...
    // let batch_responses: Vec<AiyTransactionBlockResponse> = http_client
    //     .multi_get_transaction_blocks(tx, Some(AiyTransactionBlockResponseOptions::new()))
    //     .await?;
    ...
    //     assert!(tx_responses
    //         .iter()
    //         .any(|resp| matches!(resp, AiyTransactionBlockResponse {digest, ..} if *digest == r.digest)))
    // }

    // // test get_transaction
    // for tx_digest in tx {
    //     let response: AiyTransactionBlockResponse = http_client
    //         .get_transaction_block(
    //             tx_digest,
    //             Some(AiyTransactionBlockResponseOptions::new().with_raw_input()),
    //         )
    //         .await?;
    //     assert!(tx_responses.iter().any(
    //         |resp| matches!(resp, AiyTransactionBlockResponse {digest, ..} if *digest == response.digest)
    //     ));
...
async fn test_get_fullnode_transaction() -> Result<(), anyhow::Error> {
    ...
    let query = AiyTransactionBlockResponseQuery {
        options: Some(AiyTransactionBlockResponseOptions {
            show_input: true,
            show_effects: true,
            show_events: true,
            ..Default::default()
    ...
    let first_page = client
        ...
        .query_transaction_blocks(
            AiyTransactionBlockResponseQuery::default(),
            None,
            Some(5),
            false,
        )
    ...
    let address_txs_asc = client
        .read_api()
        .query_transaction_blocks(
            AiyTransactionBlockResponseQuery::new_with_filter(TransactionFilter::FromAddress(
    ...
    for tx_resp in tx.data {
        let response: AiyTransactionBlockResponse = client
            .read_api()
            .get_transaction_with_options(tx_resp.digest, AiyTransactionBlockResponseOptions::new())
            .await
            .unwrap();
...
async fn test_query_transaction_blocks() -> Result<(), anyhow::Error> {
    ...
    let aiy_type_args = type_args![GAS::type_tag()]?;
    let type_args = aiy_type_args
        .into_iter()
        .map(|ty| ty.try_into())
        .collect::<Result<Vec<_>, _>>()?;

    let aiy_call_args_1 = call_args!(coin.data.clone().unwrap().object_id, 10)?;
    ...
    let aiy_call_args_2 = call_args!(coin_2.data.clone().unwrap().object_id, 10)?;
    ...
    let move_call_query = AiyTransactionBlockResponseQuery::new_with_filter(filter);
    let tx = client
        .read_api()
        .query_transaction_blocks(move_call_query, None, Some(20), true)
        .await
        .unwrap();
    // verify that only 1 tx is returned and no AiyRpcInputError::ContainsDuplicates error
...
async fn test_display_transaction_block_with_empty_balance_changes() {
    ...
    let tx_block = client
        .get_transaction_block(
            *digest,
            Some(AiyTransactionBlockResponseOptions::full_content()),
        )
        .await
        .unwrap();
