...
mod test {
    ...
    use aiy_benchmark::bank::BenchmarkBank;
    use aiy_benchmark::system_state_observer::SystemStateObserver;
    use aiy_benchmark::workloads::adversarial::AdversarialPayloadCfg;
    use aiy_benchmark::workloads::benchmark_move_base_dir;
    use aiy_benchmark::workloads::expected_failure::ExpectedFailurePayloadCfg;
    use aiy_benchmark::workloads::workload::ExpectedFailureType;
    use aiy_benchmark::workloads::workload_configuration::{
    ...
    use aiy_config::node::{AuthorityOverloadConfig, ForkCrashBehavior, ForkRecoveryConfig};
    use aiy_config::ExecutionCacheConfig;
    use aiy_config::{AUTHORITIES_DB_NAME, AIY_KEYSTORE_FILENAME};
    ...
    use aiy_core::checkpoints::{CheckpointStore, CheckpointWatermark};
    ...
    use aiy_simulator::tempfile::TempDir;
    use aiy_simulator::{configs::*, SimConfig};
    use aiy_storage::blob::Blob;
    use aiy_surfer::surf_strategy::SurfStrategy;
    use aiy_swarm_config::network_config_builder::ConfigBuilder;
    use aiy_types::base_types::{AuthorityName, ConciseableName, ObjectID, SequenceNumber};
    use aiy_types::digests::TransactionDigest;
    use aiy_types::full_checkpoint_content::CheckpointData;
    use aiy_types::messages_checkpoint::VerifiedCheckpoint;
    use aiy_types::supported_protocol_versions::SupportedProtocolVersions;
    use aiy_types::traffic_control::{FreqThresholdConfig, PolicyConfig, PolicyType};
    ...
    struct DeadValidator {
        node_id: aiy_simulator::task::NodeId,
    ...
    fn get_keep_alive_nodes(cluster: &TestCluster) -> HashSet<aiy_simulator::task::NodeId> {
        ...
        keep_alive_nodes.insert(aiy_simulator::current_simnode_id());
        keep_alive_nodes
    }

    fn handle_failpoint(
        dead_validator: Arc<Mutex<Option<DeadValidator>>>,
        keep_alive_nodes: HashSet<aiy_simulator::task::NodeId>,
        ...
        if rng.gen_range(0.0..1.0) < probability {
            ...
            *dead_validator = Some(DeadValidator {
            ...
            aiy_simulator::task::kill_current_node(Some(restart_after));
        }
    }
    ...
    async fn test_simulated_load_reconfig_with_prune_and_compact() {
        ...
        let node_state = test_cluster.fullnode_handle.aiy_node.clone().state();
    ...
    async fn test_protocol_upgrade_compatibility_impl() {
        ...
        let manifest = aiy_framework_snapshot::load_bytecode_snapshot_manifest();
        ...
        let init_framework =
            aiy_framework_snapshot::load_bytecode_snapshot(starting_version).unwrap();
        let test_cluster = Arc::new(
        ...
        let _handle = tokio::task::spawn(async move {
            ...
            for version in starting_version..=max_ver {
                ...
                let new_framework = aiy_framework_snapshot::load_bytecode_snapshot(next_version);
    ...
    fn handle_bool_failpoint(
        eligible_nodes: &HashSet<aiy_simulator::task::NodeId>, // only given eligible nodes may fail
        probability: f64,
    ) -> bool {
        if !eligible_nodes.contains(&aiy_simulator::current_simnode_id()) {
        ...
        Fut: std::future::Future<Output = ()> + Send,
    {
        ...
        let keystore_path = test_cluster.swarm.dir().join(AIY_KEYSTORE_FILENAME);
        ...
        if enable_surfer {
            let surfer_task = tokio::spawn(async move {
                // now do a aiy-surfer test
                let mut test_packages_dir = benchmark_move_base_dir();
                test_packages_dir.extend(["..", "..", "crates", "aiy-surfer", "tests"]);
                let test_package_paths: Vec<PathBuf> = std::fs::read_dir(test_packages_dir)
                    ...
                    .collect();
                info!("using aiy_surfer test packages: {test_package_paths:?}");

                let surf_strategy = SurfStrategy::new(Duration::from_millis(400));
                let results = aiy_surfer::run_with_test_cluster_and_strategy(
                    surf_strategy,
                    test_duration,
                    test_package_paths,
                    test_cluster,
                ...
                .await;
                info!("aiy_surfer test complete with results: {results:?}");
                ...
                move |_cluster: Arc<TestCluster>| async move {
                    ...
                    register_fail_point_arg("kill_checkpoint_fork_node", {
                        let forked_validators: Arc<
                            Mutex<HashSet<aiy_types::crypto::AuthorityPublicKeyBytes>>,
                        > = forked_validators.clone();
                        let checkpoint_overrides = checkpoint_overrides.clone();
                        let node_to_authority_map = node_to_authority_map.clone();
                        move || {
                            let current_node_id = aiy_simulator::current_simnode_id();
                            let authority_name =
                        ...
                        move || {
                            forked_validators.lock().unwrap().contains(
                                node_to_authority_map
                                    .get(&aiy_simulator::current_simnode_id())
                                    .unwrap(),
                            )
                        }
                    });
