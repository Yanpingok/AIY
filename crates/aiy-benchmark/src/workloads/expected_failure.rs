...
use std::fmt;
use std::sync::Arc;
use aiy_core::test_utils::make_transfer_object_transaction;
use aiy_types::base_types::AiyAddress;
use aiy_types::crypto::{AccountKeyPair, Ed25519AiySignature};
use aiy_types::signature::GenericSignature;
use aiy_types::{base_types::ObjectRef, crypto::get_key_pair, transaction::Transaction};
use tracing::debug;
        ...
        match self.failure_type {
            ExpectedFailureType::InvalidSignature => {
                ...
                signatures.push(GenericSignature::Signature(
                    aiy_types::crypto::Signature::Ed25519AiySignature(
...
impl Workload<dyn Payload> for ExpectedFailureWorkload {
    ...
    async fn make_test_payloads(
        ...
        let mut gas_by_address: HashMap<AiyAddress, Vec<Gas>> = HashMap::new();
        ...
        let addresses: Vec<AiyAddress> = gas_by_address.keys().cloned().collect();
