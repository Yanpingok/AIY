...
/// as stress, but may not be aiytable in some other cases.
#[derive(Clone)]
pub struct FullNodeReconfigObserver {
    pub fullnode_client: AiyClient,
...
impl FullNodeReconfigObserver {
    pub async fn new(
        ...
        Self {
            fullnode_client: AiyClientBuilder::default()
                .build(fullnode_rpc_url)
                .await
                .unwrap_or_else(|e| {
                    panic!(
                        "Can't create AiyClient with rpc url {fullnode_rpc_url}: {:?}",
                        e
                    )
                }),
            committee_store,
...
impl ReconfigObserver<NetworkAuthorityClient> for FullNodeReconfigObserver {
    ...
    async fn run(&mut self, driver: Arc<dyn AuthorityAggregatorUpdatable<NetworkAuthorityClient>>) {
        loop {
            ...
            {
                Ok(aiy_system_state) => {
                    let epoch_id = aiy_system_state.epoch;
                    if epoch_id > driver.epoch() {
                        debug!(epoch_id, "Got AiySystemState in newer epoch");
                        let new_committee = aiy_system_state.get_aiy_committee_for_benchmarking();
                        let _ = self
                            .committee_store
                            .insert_new_committee(new_committee.committee());
                        let auth_agg = AuthorityAggregator::new_from_committee(
                            new_committee,
                            Arc::new(aiy_system_state.get_committee_authority_names_to_hostnames()),
                            aiy_system_state.reference_gas_price,
                ...
                Err(err) => error!("Can't get AiySystemState from Full Node: {:?}", err,),
            }
        }
    }
}
