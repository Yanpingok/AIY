...
use std::sync::Arc;
use aiy_core::test_utils::{make_pay_aiy_transaction, make_transfer_aiy_transaction};
...
impl BenchmarkBank {
    ...
    pub async fn generate(
        ...
        for chunk in chunked_coin_configs {
            let gas_coins = self.pay_aiy(chunk, &mut init_coin, gas_price).await?;
    ...
    async fn pay_aiy(
        ...
        let recipient_addresses: Vec<AiyAddress> = coin_configs.iter().map(|g| g.address).collect();
        ...
        let tx = make_pay_aiy_transaction(
            init_coin.0,
            vec![],
            recipient_addresses,
            amounts,
        ...
        let address_map: HashMap<AiyAddress, Arc<AccountKeyPair>> = coin_configs
            .iter()
