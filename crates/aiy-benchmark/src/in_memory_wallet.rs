...
/// A Aiy account and all of the objects it owns
#[derive(Debug)]
pub struct AiyAccount {
...
pub struct InMemoryWallet {
    accounts: BTreeMap<AiyAddress, AiyAccount>, // TODO: track shared and immutable objects as well
}
...
impl InMemoryWallet {
    ...
    pub fn add_account(
        ...
        self.accounts.insert(addr, AiyAccount::new(key, gas, objs));
    }
    ...
    pub fn account_mut(&mut self, addr: &AiyAddress) -> Option<&mut AiyAccount> {
        self.accounts.get_mut(addr)
    }

    pub fn account(&self, addr: &AiyAddress) -> Option<&AiyAccount> {
        self.accounts.get(addr)
    }

    pub fn accounts(&self) -> impl Iterator<Item = &AiyAddress> {
        self.accounts.keys()
    }

    pub fn gas(&self, addr: &AiyAddress) -> Option<&ObjectRef> {
        self.accounts.get(addr).map(|a| &a.gas)
    }

    pub fn owned_object(&self, addr: &AiyAddress, id: &ObjectID) -> Option<&ObjectRef> {
        self.accounts.get(addr).and_then(|a| a.owned.get(id))
    }

    pub fn owned_objects(&self, addr: &AiyAddress) -> Option<impl Iterator<Item = &ObjectRef>> {
    ...
    pub fn keypair(&self, addr: &AiyAddress) -> Option<Arc<AccountKeyPair>> {
    ...
    pub fn addresses(&self) -> impl Iterator<Item = &AiyAddress> {
        self.accounts.keys()
    }
