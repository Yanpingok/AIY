...
use aiy_types::{
    ...
    BRIDGE_ADDRESS, DEEPBOOK_ADDRESS, MOVE_STDLIB_ADDRESS, AIY_FRAMEWORK_ADDRESS,
    AIY_SYSTEM_ADDRESS,
};
use aiy_verifier::verifier as aiy_bytecode_verifier;
...
pub mod test_utils {
    use crate::{BuildConfig, CompiledPackage, AiyPackageHooks};
    ...
    pub fn compile_managed_coin_package() -> CompiledPackage {
        compile_example_package("../../crates/aiy-core/src/unit_tests/data/managed_coin")
    }
...
/// Wrapper around the core Move `CompiledPackage` with some Aiy-specific traits and info
#[derive(Debug, Clone)]
...
/// Wrapper around the core Move `BuildConfig` with some Aiy-specific info
#[derive(Clone)]
...
impl BuildConfig {
    pub fn new_for_testing() -> Self {
        ...
        let config = MoveBuildConfig {
            default_flavor: Some(move_compiler::editions::Flavor::Aiy),
    ...
    /// If we are building the Aiy framework, we skip the check that the addresses should be 0
    pub fn build(self, path: &Path) -> AiyResult<CompiledPackage> {
    ...
    pub fn resolution_graph(
    ...
    ) -> AiyResult<ResolvedGraph> {
        if let Some(err_msg) = set_aiy_flavor(&mut self.config) {
            return Err(AiyError::ModuleBuildFailure { error: err_msg });
        }
        ...
        .map_err(|err| AiyError::ModuleBuildFailure {
            error: format!("{:?}", err),
        })
    }
}
...
/// Sets build config's default flavor to `Flavor::Aiy`. Returns error message if the flavor was
/// previously set to something else than `Flavor::Aiy`.
pub fn set_aiy_flavor(build_config: &mut MoveBuildConfig) -> Option<String> {
    use move_compiler::editions::Flavor;

    let flavor = build_config.default_flavor.get_or_insert(Flavor::Aiy);
    if flavor != &Flavor::Aiy {
        return Some(format!(
...
pub fn build_from_resolution_graph(
    ...
    let (package, fn_info) = result.map_err(|error| AiyError::ModuleBuildFailure {
...
fn collect_bytecode_deps(
    resolution_graph: &ResolvedGraph,
) -> AiyResult<Vec<(Symbol, CompiledModule)>> {
        ...
        for module in modules {
            let module =
                CompiledModule::deserialize_with_defaults(module.as_ref()).map_err(|error| {
                    AiyError::ModuleDeserializationFailure {
                        error: format!(
                            "Deserializing bytecode dependency for package {}: {:?}",
                            name, error
                        ),
...
fn verify_bytecode(package: &MoveCompiledPackage, fn_info: &FnInfoMap) -> AiyResult<()> {
    ...
    for m in compiled_modules.iter_modules() {
        move_bytecode_verifier::verify_module_unmetered(m).map_err(|err| {
            AiyError::ModuleVerificationFailure {
                error: err.to_string(),
            }
        })?;
        aiy_bytecode_verifier::aiy_verify_module_unmetered(m, fn_info, &verifier_config)?;
    }
...
impl CompiledPackage {
    ...
    /// Note: these are not topologically sorted by dependency--use `get_dependency_sorted_modules` to produce a list of modules aiytable
    ...
    /// Get bytecode modules from the Aiy System that are used by this package
    pub fn get_aiy_system_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == AIY_SYSTEM_ADDRESS)
    }

    /// Get bytecode modules from the Aiy Framework that are used by this package
    pub fn get_aiy_framework_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == AIY_FRAMEWORK_ADDRESS)
    }
    ...
    pub fn verify_unpublished_dependencies(
        ...
        Err(AiyError::ModulePublishFailure {
...
pub struct AiyPackageHooks;

impl PackageHooks for AiyPackageHooks {
...
pub fn check_unpublished_dependencies(unpublished: &BTreeSet<Symbol>) -> Result<(), AiyError> {
...
pub fn check_invalid_dependencies(invalid: &BTreeMap<Symbol, String>) -> Result<(), AiyError> {
    if invalid.is_empty() {
        return Ok(());
    }
...
pub fn check_conflicting_addresses(
 ...
 - check that your `aiy active-env` corresponds to the chain on which the package is published (i.e., devnet, testnet, mainnet); OR
 - contact the maintainer if this package is a dependency and request resolving the conflict.";
    ...
    let err = if dump_bytecode_base64 {
        AiyError::ModuleBuildFailure { error }
    } else {
        AiyError::ModulePublishFailure { error }
    };
