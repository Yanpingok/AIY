...
use aiy_config::{Config, NodeConfig};
use aiy_core::runtime::AiyRuntimes;
use aiy_telemetry::send_telemetry_event;
...
fn main() {
    ...
    let runtimes = AiyRuntimes::new(&config);
...

    {
        ...
        if let Some(metrics_config) = &config.metrics {
            if let Some(push_url) = &metrics_config.push_url {
                aiy_metrics_push_client::start_metrics_push_task(
    ...
    // if it deadlocks.
    let node_once_cell = Arc::new(AsyncOnceCell::<Arc<aiy_node::AiyNode>>::new());
    let node_once_cell_clone = node_once_cell.clone();

    // let aiy-node signal main to shutdown runtimes
    ...
    runtimes.aiy_node.spawn(async move {
        match aiy_node::AiyNode::start_async(config, registry_service, server_version).await {
            Ok(aiy_node) => node_once_cell_clone
                .set(aiy_node)
        ...
        // when we get a shutdown signal from aiy-node, forward it on to the runtime_shutdown_channel here in
        // main to signal runtimes to all shutdown.
        tokio::select! {
           _ = shutdown_rx.recv() => {
                runtime_shutdown_tx.send(()).expect("failed to forward shutdown signal from aiy-node to aiy-node main");
            }
        }
    ...
    runtimes.metrics.spawn(async move {
        ...
        info!("Aiy chain identifier: {chain_identifier}");
        prometheus_registry
        ...
        aiy_node::admin::run_admin_server(node, admin_interface_port, filter_handle).await
    });
