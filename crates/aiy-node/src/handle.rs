...
//! AiyNodeHandle wraps AiyNode in a way aiytable for access by test code.
//!
//! When starting a AiyNode directly, in a test (as opposed to using Swarm), the node may be
...
//! simulator node rather than the node in which the AiyNode is running.
//!
//! AiyNodeHandle provides an easy way to do the right thing here:
//!
//! ```ignore
//!     let node_handle = start_node(config, registry).await;
//!     node_handle.with_async(|aiy_node| async move {
//!         spawn_checkpoint_processes(config, &[aiy_node]).await;
//!     });
//! ```
//!
...
//!
//! ```ignore
//!    let state = node_handle.with(|aiy_node| aiy_node.state);
//!    // DO NOT DO THIS!
...
/// Wrap AiyNode to allow correct access to AiyNode in simulator tests.
pub struct AiyNodeHandle {
    node: Option<Arc<AiyNode>>,
    shutdown_on_drop: bool,
}

impl AiyNodeHandle {
    pub fn new(node: Arc<AiyNode>) -> Self {
        Self {
            node: Some(node),
    ...
    pub fn inner(&self) -> &Arc<AiyNode> {
        self.node.as_ref().unwrap()
    }

    pub fn with<T>(&self, cb: impl FnOnce(&AiyNode) -> T) -> T {
        let _guard = self.guard();
        cb(self.inner())
    }

    pub fn state(&self) -> Arc<AuthorityState> {
        self.with(|aiy_node| aiy_node.state())
    }
...
#[cfg(msim)]
impl AiyNodeHandle {
    fn guard(&self) -> aiy_simulator::runtime::NodeEnterGuard {
...
#[cfg(msim)]
impl Drop for AiyNodeHandle {
    fn drop(&mut self) {
        if self.shutdown_on_drop {
            let node_id = self.inner().sim_state.sim_node.id();
            aiy_simulator::runtime::Handle::try_current().map(|h| h.delete_node(node_id));
        }
    }
}

impl From<Arc<AiyNode>> for AiyNodeHandle {
    fn from(node: Arc<AiyNode>) -> Self {
        AiyNodeHandle::new(node)
    }
}
