...
use aiy_core::authority::authority_store_tables::{
    AuthorityPerpetualTablesOptions, AuthorityPrunerTables,
};
use aiy_core::authority::backpressure::BackpressureManager;
use aiy_core::authority::epoch_start_configuration::EpochFlag;
use aiy_core::authority::execution_time_estimator::ExecutionTimeObserver;
use aiy_core::authority::shared_object_version_manager::Schedulable;
use aiy_core::authority::ExecutionEnv;
use aiy_core::authority::RandomnessRoundReceiver;
use aiy_core::consensus_adapter::ConsensusClient;
use aiy_core::consensus_manager::UpdatableConsensusClient;
use aiy_core::epoch::randomness::RandomnessManager;
use aiy_core::execution_cache::build_execution_cache;
use aiy_network::validator::server::AIY_TLS_SERVER_NAME;
use aiy_types::full_checkpoint_content::Checkpoint;

use aiy_core::execution_scheduler::SchedulingSource;
use aiy_core::global_state_hasher::GlobalStateHashMetrics;
use aiy_core::storage::RestReadStore;
use aiy_json_rpc::bridge_api::BridgeReadApi;
use aiy_json_rpc_api::JsonRpcMetrics;
use aiy_network::randomness;
use aiy_rpc_api::subscription::SubscriptionService;
use aiy_rpc_api::RpcMetrics;
use aiy_rpc_api::ServerVersion;
use aiy_types::base_types::ConciseableName;
...
use aiy_types::messages_consensus::AuthorityCapabilitiesV2;
use aiy_types::messages_consensus::ConsensusTransactionKind;
use aiy_types::aiy_system_state::AiySystemState;
use aiy_types::transaction::VerifiedCertificate;
...
pub use handle::AiyNodeHandle;
use mysten_metrics::{spawn_monitored_task, RegistryService};
use mysten_service::server_timing::server_timing_middleware;
use aiy_config::node::{DBCheckpointConfig, RunWithRange};
use aiy_config::node::{ForkCrashBehavior, ForkRecoveryConfig};
use aiy_config::node_config_metrics::NodeConfigMetrics;
use aiy_config::{ConsensusConfig, NodeConfig};
use aiy_core::authority::authority_per_epoch_store::AuthorityPerEpochStore;
use aiy_core::authority::authority_store_tables::AuthorityPerpetualTables;
use aiy_core::authority::epoch_start_configuration::EpochStartConfigTrait;
use aiy_core::authority::epoch_start_configuration::EpochStartConfiguration;
use aiy_core::authority::submitted_transaction_cache::SubmittedTransactionCacheMetrics;
use aiy_core::authority_aggregator::{AuthAggMetrics, AuthorityAggregator};
use aiy_core::authority_server::{ValidatorService, ValidatorServiceMetrics};
use aiy_core::checkpoints::checkpoint_executor::metrics::CheckpointExecutorMetrics;
use aiy_core::checkpoints::checkpoint_executor::{CheckpointExecutor, StopReason};
use aiy_core::checkpoints::{
    CheckpointMetrics, CheckpointService, CheckpointStore, SendCheckpointToStateSync,
    SubmitCheckpointToConsensus,
};
use aiy_core::consensus_adapter::{
    CheckConnection, ConnectionMonitorStatus, ConsensusAdapter, ConsensusAdapterMetrics,
};
use aiy_core::consensus_manager::ConsensusManager;
use aiy_core::consensus_throughput_calculator::{
    ConsensusThroughputCalculator, ConsensusThroughputProfiler, ThroughputProfileRanges,
};
use aiy_core::consensus_validator::{AiyTxValidator, AiyTxValidatorMetrics};
use aiy_core::db_checkpoint_handler::DBCheckpointHandler;
use aiy_core::epoch::committee_store::CommitteeStore;
use aiy_core::epoch::consensus_store_pruner::ConsensusStorePruner;
use aiy_core::epoch::epoch_metrics::EpochMetrics;
use aiy_core::epoch::reconfiguration::ReconfigurationInitiator;
use aiy_core::global_state_hasher::GlobalStateHasher;
use aiy_core::jsonrpc_index::IndexStore;
use aiy_core::module_cache_metrics::ResolverMetrics;
use aiy_core::overload_monitor::overload_monitor;
use aiy_core::rpc_index::RpcIndexStore;
use aiy_core::signature_verifier::SignatureVerifierMetrics;
use aiy_core::storage::RocksDbStore;
use aiy_core::transaction_orchestrator::TransactionOrchestrator;
use aiy_core::{
    authority::{AuthorityState, AuthorityStore},
    authority_client::NetworkAuthorityClient,
};
use aiy_json_rpc::coin_api::CoinReadApi;
use aiy_json_rpc::governance_api::GovernanceReadApi;
use aiy_json_rpc::indexer_api::IndexerApi;
use aiy_json_rpc::move_utils::MoveUtils;
use aiy_json_rpc::read_api::ReadApi;
use aiy_json_rpc::transaction_builder_api::TransactionBuilderApi;
use aiy_json_rpc::transaction_execution_api::TransactionExecutionApi;
use aiy_json_rpc::JsonRpcServerBuilder;
use aiy_macros::fail_point;
use aiy_macros::{fail_point_async, replay_log};
use aiy_network::api::ValidatorServer;
use aiy_network::discovery;
use aiy_network::discovery::TrustedPeerChangeEvent;
use aiy_network::state_sync;
use aiy_network::validator::server::ServerBuilder;
use aiy_protocol_config::{Chain, ProtocolConfig, ProtocolVersion};
use aiy_snapshot::uploader::StateSnapshotUploader;
use aiy_storage::{
    http_key_value_store::HttpKVStore,
    key_value_store::{FallbackTransactionKVStore, TransactionKeyValueStore},
    key_value_store_metrics::KeyValueStoreMetrics,
};
use aiy_types::base_types::{AuthorityName, EpochId};
use aiy_types::committee::Committee;
use aiy_types::crypto::KeypairTraits;
use aiy_types::error::{AiyError, AiyResult};
use aiy_types::messages_consensus::{
    check_total_jwk_size, AuthorityCapabilitiesV1, ConsensusTransaction,
};
use aiy_types::aiy_system_state::epoch_start_aiy_system_state::EpochStartSystemState;
...
use aiy_types::supported_protocol_versions::SupportedProtocolVersions;
use typed_store::rocks::default_db_options;
use typed_store::DBMetrics;

use crate::metrics::{GrpcMetrics, AiyNodeMetrics};
...
pub struct ValidatorComponents {
    ...
    aiy_tx_validator_metrics: Arc<AiyTxValidatorMetrics>,
}
...
mod simulator {
    ...
    pub(super) struct SimState {
        pub sim_node: aiy_simulator::runtime::NodeHandle,
        pub sim_safe_mode_expected: AtomicBool,
        _leak_detector: aiy_simulator::NodeLeakDetector,
    }

    impl Default for SimState {
        fn default() -> Self {
            Self {
                sim_node: aiy_simulator::runtime::NodeHandle::current(),
                sim_safe_mode_expected: AtomicBool::new(false),
                _leak_detector: aiy_simulator::NodeLeakDetector::new(),
            }
        }
    }

    type JwkInjector = dyn Fn(AuthorityName, &OIDCProvider) -> AiyResult<Vec<(JwkId, JWK)>>
        + Send
        + Sync
        + 'static;
    ...
    ) -> AiyResult<Vec<(JwkId, JWK)>> {
        use fastcrypto_zkp::bn254::zk_login::parse_jwks;
        // Just load a default Twitch jwk for testing.
        parse_jwks(
            aiy_types::zk_login_util::DEFAULT_JWK_BYTES,
            &OIDCProvider::Twitch,
            true,
        )
        .map_err(|_| AiyError::JWKRetrievalError)
    }
...
#[cfg(msim)]
use simulator::*;
use aiy_core::authority::authority_store_pruner::{ObjectsCompactionFilter, PrunerWatermarks};
use aiy_core::{
...
pub struct AiyNode {
    ...
    end_of_epoch_channel: broadcast::Sender<AiySystemState>,
    ...
    // Channel to allow signaling upstream to shutdown aiy-node
...
impl fmt::Debug for AiyNode {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("AiyNode")
...
impl AiyNode {
    pub async fn start(
        ...
        Self::start_async(
            config,
            registry_service,
            ServerVersion::new("aiy-node", "unknown"),
        )
        .await
    }
    ...
    pub async fn start_async(
        ...
        info!(node =? config.protocol_public_key(),
            "Initializing aiy-node listening on {}", config.network_address
        );
        ...
        if is_genesis {
            info!("checking AIY conservation at genesis");
            // When we are opening the db table, the only time when it's safe to
            // check AIY conservation is at genesis. Otherwise we may be in the middle of
            // an epoch and the AIY conservation check will fail. This also initialize
            // the expected_network_aiy_amount table.
            cache_traits
                .reconfig_api
                .expensive_check_aiy_conservation(&epoch_store)
                .expect("AIY conservation check cannot fail at genesis");
        }
        ...
        if epoch_store.epoch() == 0 {
            ...
            let transaction =
                aiy_types::executable_transaction::VerifiedExecutableTransaction::new_unchecked(
                    aiy_types::executable_transaction::ExecutableTransaction::new_from_data_and_sig(
            ...
            .enable_secondary_index_checks()
        {
            if let Some(indexes) = state.indexes.clone() {
                aiy_core::verify_indexes::verify_indexes(
        ...
        let aiy_node_metrics = Arc::new(AiyNodeMetrics::new(&registry_service.default_registry()));
    ...
    pub fn subscribe_to_epoch_change(&self) -> broadcast::Receiver<AiySystemState> {
    ...
    pub async fn close_epoch(&self, epoch_store: &Arc<AuthorityPerEpochStore>) -> AiyResult {
        info!("close_epoch (current epoch = {})", epoch_store.epoch());
        self.validator_components
            .lock()
            .await
            .as_ref()
            .ok_or_else(|| AiyError::from("Node is not a validator"))?
            .consensus_adapter
            .close_epoch(epoch_store);
        Ok(())
    }

    pub fn clear_override_protocol_upgrade_buffer_stake(&self, epoch: EpochId) -> AiyResult {
        self.state
    ...
    pub async fn close_epoch_for_testing(&self) -> AiyResult {
    ...
    fn create_p2p_network(
        ...
        let p2p_network = {
            ...
            let inbound_network_metrics =
                consensus_core::NetworkRouteMetrics::new("aiy", "inbound", prometheus_registry);
            let outbound_network_metrics =
                consensus_core::NetworkRouteMetrics::new("aiy", "outbound", prometheus_registry);
            ...
            let server_name = format!("aiy-{}", chain_identifier);
    ...
    async fn construct_validator_components(
        ...
        connection_monitor_status: Arc<ConnectionMonitorStatus>,
        registry_service: &RegistryService,
        aiy_node_metrics: Arc<AiyNodeMetrics>,
        checkpoint_metrics: Arc<CheckpointMetrics>,
        ...
        let aiy_tx_validator_metrics =
            AiyTxValidatorMetrics::new(&registry_service.default_registry());
    ...
    async fn start_epoch_specific_validator_components(
        ...
        tokio::spawn({
            ...
            let epoch_store = epoch_store.clone();
            let aiy_tx_validator = AiyTxValidator::new(
                state.clone(),
                consensus_adapter.clone(),
                checkpoint_service.clone(),
                aiy_tx_validator_metrics.clone(),
            );
    ...
    async fn start_grpc_validator_service(
        ...
        let tls_config = aiy_tls::create_rustls_server_config(
            config.network_key_pair().copy().private(),
            AIY_TLS_SERVER_NAME.to_string(),
        );
        ...
        if tokio::time::timeout(
            ...
            state
                ...
                .notify_read_executed_effects_digests(
                    "AiyNode::notify_read_executed_effects_digests",
                    &digests,
                ),
        )
        .await
    ...
    pub async fn monitor_reconfiguration(
        ...
        loop {
            ...
            let latest_system_state = self
                ...
                .expect("Read Aiy System State object cannot fail");
            ...
            let next_epoch_committee = new_epoch_start_state.get_aiy_committee();
                ...
                )
            {
                self.state
                    .prune_checkpoints_for_eligible_epochs_for_testing(
                        self.config.clone(),
                        aiy_core::authority::authority_store_pruner::AuthorityStorePruningMetrics::new_for_test(),
                    )
                    .await?;
            }
    ...
    fn try_recover_checkpoint_fork(
        ...
        for (seq, expected_digest_str) in &recovery.checkpoint_overrides {
            ...
            if let Some(local_summary) = checkpoint_store.get_locally_computed_checkpoint(*seq)? {
                let local_digest = aiy_types::message_envelope::Message::digest(&local_summary);
...
#[cfg(msim)]
impl AiyNode {
    pub fn get_sim_node_id(&self) -> aiy_simulator::task::NodeId {
...
async fn build_http_servers(
    ...
    let json_rpc_router = {
        ...
        let name_service_config =
            if let (Some(package_address), Some(registry_id), Some(reverse_registry_id)) = (
                ...
                aiy_name_service::NameServiceConfig::new(
                    package_address,
                    registry_id,
                    reverse_registry_id,
                )
                ...
                match state.get_chain_identifier().chain() {
                    Chain::Mainnet => aiy_name_service::NameServiceConfig::mainnet(),
                    Chain::Testnet => aiy_name_service::NameServiceConfig::testnet(),
                    Chain::Unknown => aiy_name_service::NameServiceConfig::default(),
                }
            };
    ...
    let rpc_router = {
        let mut rpc_service =
            aiy_rpc_api::RpcService::new(Arc::new(RestReadStore::new(state.clone(), store)));
    ...
    let layers = ServiceBuilder::new()
        .map_request(|mut request: axum::http::Request<_>| {
            if let Some(connect_info) = request.extensions().get::<aiy_http::ConnectInfo>() {
    ...
    {
        let https = aiy_http::Builder::new()
...
struct HttpServers {
    #[allow(unused)]
    http: Option<aiy_http::ServerHandle>,
    #[allow(unused)]
    https: Option<aiy_http::ServerHandle>,
}
...
mod tests {
    ...
    use aiy_config::node::{ForkCrashBehavior, ForkRecoveryConfig};
    use aiy_core::checkpoints::{CheckpointMetrics, CheckpointStore};
    use aiy_types::digests::{CheckpointDigest, TransactionDigest, TransactionEffectsDigest};
    ...
    async fn test_fork_error_and_recovery_both_paths() {
        ...
        let r = AiyNode::check_and_recover_forks(
            &checkpoint_store,
            &checkpoint_metrics,
            true,
            Some(&fork_recovery),
