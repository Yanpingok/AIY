# ClickHouse Aiy Indexer

A simple example of how to build a custom Aiy indexer that writes transaction data to ClickHouse.
...
- Fetch checkpoints from the Aiy testnet
...
```
Aiy Network → Checkpoints → Concurrent Pipeline → ClickHouse Store → ClickHouse DB
```
