...
use aiy_config::{aiy_config_dir, AIY_CLIENT_CONFIG};
...
enum CliCommand {
    ...
    /// Transfer coin from the aiy client's active address
    Transfer {
    ...
    /// Burn coin inside the aiy client's active address
    Burn {
        /// The Coin to burn
        #[arg(value_parser)]
        coin: String,
...
async fn cli_parse() -> Result<(AppConfig, AppCommand)> {
    ...
    let command = match command {
        CliCommand::DenyListAdd { address } => {
            AppCommand::DenyListAdd(AiyAddress::from_str(&address)?)
        }
        CliCommand::DenyListRemove { address } => {
            AppCommand::DenyListRemove(AiyAddress::from_str(&address)?)
        }
        CliCommand::MintAndTransfer { balance, address } => {
            AppCommand::MintAndTransfer(balance, AiyAddress::from_str(&address)?)
        }
        CliCommand::Transfer { coin, address } => AppCommand::Transfer(
            ObjectID::from_hex_literal(&coin)?,
            AiyAddress::from_str(&address)?,
        ),
