...
/// Basic raffle game that depends on Aiy randomness.
///
...
public fun redeem(ticket: Ticket, game: Game, ctx: &mut TxContext): Coin<AIY> {
    assert!(object::id(&game) == ticket.game_id, EGameMismatch);
    assert!(game.winner.contains(&ticket.participant_index), ENotWinner);
    destroy_ticket(ticket);

    let Game { id, cost_in_aiy: _, participants: _, end_time: _, winner: _, balance } = game;
    object::delete(id);
