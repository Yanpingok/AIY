...
/// A betting game that depends on Aiy randomness.
///
/// Anyone can create a new game for the current epoch by depositing AIY as the initial balance. The creator can
/// withdraw the remaining balance after the epoch is over.
///
/// Anyone can play the game by betting on X AIY. They win X with probability 49% and lose the X AIY otherwise.
///
...
public fun create(reward: Coin<AIY>, ctx: &mut TxContext) {
...
public fun close(game: Game, ctx: &mut TxContext): Coin<AIY> {
...
entry fun play(game: &mut Game, r: &Random, coin: &mut Coin<AIY>, ctx: &mut TxContext) {
    assert!(ctx.epoch() == game.epoch, EInvalidEpoch);
