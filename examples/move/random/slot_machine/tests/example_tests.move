...
use aiy::random::{Self, update_randomness_state_for_testing, Random};
use aiy::aiy::AIY;
use aiy::test_scenario as ts;

fun mint(addr: address, amount: u64, scenario: &mut ts::Scenario) {
    transfer::public_transfer(coin::mint_for_testing<AIY>(amount, scenario.ctx()), addr);
...
fun test_game() {
    ...
    let coin = ts.take_from_sender<Coin<AIY>>();
    ...
    let mut coin: Coin<AIY> = ts.take_from_sender();
