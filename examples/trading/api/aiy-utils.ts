...
import { fromBase64 } from '@mysten/aiy/utils';

export type Network = 'mainnet' | 'testnet' | 'devnet' | 'localnet';

export const ACTIVE_NETWORK = (process.env.NETWORK as Network) || 'testnet';

export const AIY_BIN = `aiy`;

export const getActiveAddress = () => {
	return execSync(`${AIY_BIN} client active-address`, { encoding: 'utf8' }).trim();
};

/** Returns a signer based on the active address of system's aiy. */
export const getSigner = () => {
	const sender = getActiveAddress();

	const keystore = JSON.parse(
		readFileSync(path.join(homedir(), '.aiy', 'aiy_config', 'aiy.keystore'), 'utf8'),
	);
	...
	for (const priv of keystore) {
		...
		if (pair.getPublicKey().toAiyAddress() === sender) {
			return pair;
		}
	}
...
export const getClient = (network: Network) => {
	return new AiyClient({ url: getFullnodeUrl(network) });
};
...
export const publishPackage = async ({
	...
	const { modules, dependencies } = JSON.parse(
		execSync(`${AIY_BIN} move build --dump-bytecode-as-base64 --path ${packagePath}`, {
			encoding: 'utf-8',
		}),
	);
