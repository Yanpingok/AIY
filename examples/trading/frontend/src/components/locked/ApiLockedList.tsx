...
import { useAiyClient } from "@mysten/dapp-kit";
...
export function LockedList({
  ...
  const { data, fetchNextPage, hasNextPage, isLoading, isFetchingNextPage } =
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam }) => {
         ...
         */
        const objects = await aiyClient.multiGetObjects({
        ...
        return {
          aiyObjects: objects.map((x) => x.data),
          api: data,
        };
      },
  ...
  const aiyObjects = () => {
    ...
    return data?.flatMap((x) => x.aiyObjects) || [];
  };
