...
import { useCurrentAccount, useAiyClientQuery } from "@mysten/dapp-kit";
import { AiyObjectDisplay } from "@/components/AiyObjectDisplay";
...
export function Escrow({ escrow }: { escrow: ApiEscrowObject }) {
  ...
  const aiyObject = useAiyClientQuery("getObject", {
    id: escrow?.itemId,
    options: {
      showDisplay: true,
      showType: true,
  ...
  const { data: aiyLockedObject } = useGetLockedObject({
      ...
      <div className="p-4 flex gap-3 flex-wrap">
        ...
        {isToggled && lockedData.data && (
          <div className="min-w-[340px] w-full justify-self-start text-left">
            {aiyLockedObject?.data && (
              <LockedObject
                object={aiyLockedObject.data}
