...
use aiy_sdk::{
    rpc_types::{
        DevInspectArgs, DevInspectResults, DryRunTransactionBlockResponse, ObjectChange, AiyData,
        AiyExecutionStatus, AiyObjectData, AiyObjectDataFilter, AiyObjectDataOptions,
        AiyObjectResponse, AiyObjectResponseQuery, AiyProtocolConfigValue,
        AiyTransactionBlockEffectsAPI, AiyTransactionBlockResponse,
    },
...
pub struct Connection {
    /// The Aiy CLI config file, (default: ~/.aiy/aiy_config/client.yaml)
    #[clap(long)]
...
impl Client {
    ...
    pub(crate) fn new(conn: Connection) -> Result<Self> {
        let Some(config) = conn.config.or_else(|| {
            ...
            default.extend([".aiy", "aiy_config", "client.yaml"]);
            Some(default)
        }) else {
            bail!(
                "Cannot find wallet config. No config was supplied, and the default path \
                 (~/.aiy/aiy_config/client.yaml) does not exist.",
            );
        };
    ...
    pub(crate) async fn turn_cap(&mut self, game: &Game) -> Result<ObjectRef> {
        ...
        let query = Some(AiyObjectResponseQuery::new(
            Some(AiyObjectDataFilter::StructType(turn_cap_type.clone())),
            Some(AiyObjectDataOptions::new().with_bcs()),
        ));
        ...
        loop {
            ...
            for AiyObjectResponse { data, error } in response.data {
    ...
    pub(crate) async fn new_shared_game(&mut self, opponent: AiyAddress) -> Result<ObjectID> {
        let player = self.wallet.active_address()?;
    ...
    pub async fn new_owned_game(&mut self, opponent_key: PublicKey) -> Result<ObjectID> {
        ...
        let opponent = AiyAddress::from(&opponent_key);
        ...
        let admin = AiyAddress::from(&admin_key);
        let admin_bytes =
    ...
    async fn max_gas_budget(&self) -> Result<u64> {
        ...
        let Some(Some(AiyProtocolConfigValue::U64(max))) = cfg.attributes.get("max_tx_gas") else {
    ...
    async fn execute_transaction(&self, tx: Transaction) -> Result<AiyTransactionBlockResponse> {
        let response = self
            .wallet
    ...
    async fn client(&self) -> Result<AiyClient> {
        self.wallet
            .get_client()
            .await
