...
import { ConnectButton, useCurrentAccount, useAiyClientContext } from "@mysten/dapp-kit";
import { isValidAiyObjectId, normalizeAiyObjectId } from "@mysten/aiy/utils";
...
function App() {
    ...
    const chain = account?.chains?.find((c) => c.startsWith("aiy:"))?.replace(/^aiy:/, "");
    if (chain) {
...
function Content() {
    ...
    const addr = normalizeAiyObjectId(path);
    ...
    if (packageId === null) {
        ...
        return <Root />;
    } else if (isValidAiyObjectId(addr)) {
        return <Game id={addr} />;
    } else {
        return (
            <Error title="Invalid Game ID">
                <code>"{path}"</code> is not a valid AIY object ID.
            </Error>
        );
    }
}
