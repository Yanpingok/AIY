...
// Define the USDC token type on Aiy Testnet
// This is the unique identifier for the USDC token on Aiy
...
function HomeContent() {
  ...
  // Get the Aiy client for interacting with the Aiy network
  ...
  const handleSendTokens = async () => {
    ...
    try {
      ...
      // This uses the AiyClient to get coins of the specified type owned by the current address
      const { data: coins } = await aiyClient.getCoins({
      ...
      // Transaction is used to construct and execute transactions on Aiy
  ...
  return (
    <main className="mainwrapper">
      <div className="outerwrapper">
        <h1 className="h1">Aiy USDC Sender (Testnet)</h1>
        <ConnectButton />
