...
import "./interfaces/IAiyBridge.sol";
...
/// tokens. Bridge operations are managed by a committee of Aiy validators that are responsible
...
contract AiyBridge is IAiyBridge, CommitteeUpgradeable, PausableUpgradeable {
    /* ========== STATE VARIABLES ========== */
    ...
    uint8 constant AIY_ADDRESS_LENGTH = 32;

    /* ========== INITIALIZER ========== */

    /// @notice Initializes the AiyBridge contract with the provided parameters.
        ...
        onlySupportedChain(message.chainID)
    {
        ...
        require(!isTransferProcessed[message.nonce], "AiyBridge: Message already processed");
        ...
        require(
            tokenTransferPayload.targetChain == config.chainID(), "AiyBridge: Invalid target chain"
        );

        // convert amount to ERC20 token decimals
        uint256 erc20AdjustedAmount = BridgeUtils.convertAiyToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(tokenTransferPayload.tokenID)).decimals(),
            config.tokenAiyDecimalOf(tokenTransferPayload.tokenID),
    ...
    /// @param recipientAddress The address on the Aiy chain where the tokens will be sent.
    ...
    function bridgeERC20(
        ...
        require(
            recipientAddress.length == AIY_ADDRESS_LENGTH,
            "AiyBridge: Invalid recipient address length"
        );

        IBridgeConfig config = committee.config();

        require(config.isTokenSupported(tokenID), "AiyBridge: Unsupported token");
        ...
        require(
            ...
            "AiyBridge: Insufficient allowance"
        );
        ...
        // Adjust the amount
        uint64 aiyAdjustedAmount = BridgeUtils.convertERC20ToAiyDecimal(
            IERC20Metadata(tokenAddress).decimals(),
            config.tokenAiyDecimalOf(tokenID),
            amountTransfered
        );
        ...
        onlySupportedChain(destinationChainID)
    {
        ...
        require(success, "AiyBridge: Failed to transfer ETH to vault");
        ...
        uint64 aiyAdjustedAmount = BridgeUtils.convertERC20ToAiyDecimal(
            ...
            config.tokenAiyDecimalOf(BridgeUtils.ETH),
            amount
        );
    ...
    function _transferTokensFromVault(
        ...
        require(tokenAddress != address(0), "AiyBridge: Unsupported token");
    ...
    modifier limitNotExceeded(uint8 chainID, uint8 tokenID, uint256 amount) {
        require(
            ...
            "AiyBridge: Amount exceeds bridge limit"
        );
        _;
    }
    ...
    modifier onlySupportedChain(uint8 targetChainID) {
        require(
            ...
            "AiyBridge: Target chain not supported"
        );
        _;
    }
}
