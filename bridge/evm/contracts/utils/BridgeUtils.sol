...
/// @title BridgeUtils
/// @notice This library defines the message format and constants for the Aiy native bridge. It also
...
library BridgeUtils {
    ...
    /// @notice Converts the provided token amount to the Aiy decimal adjusted amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param aiyDecimal The aiy decimal value for the token.
    /// @param amount The ERC20 amount of the tokens to convert to Aiy.
    /// @return Aiy converted amount.
    function convertERC20ToAiyDecimal(uint8 erc20Decimal, uint8 aiyDecimal, uint256 amount)
        internal
        pure
        returns (uint64)
    {
        if (erc20Decimal == aiyDecimal) {
        ...
        require(erc20Decimal > aiyDecimal, "BridgeUtils: Invalid Aiy decimal");

        // Difference in decimal places
        uint256 factor = 10 ** (erc20Decimal - aiyDecimal);
    ...
    /// @notice Converts the provided Aiy decimal adjusted amount to the ERC20 token amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param aiyDecimal The aiy decimal value for the token.
    /// @param amount The Aiy amount of the tokens to convert to ERC20.
    /// @return ERC20 converted amount.
    function convertAiyToERC20Decimal(uint8 erc20Decimal, uint8 aiyDecimal, uint64 amount)
        internal
        pure
        returns (uint256)
    {
        if (aiyDecimal == erc20Decimal) {
    ...
    ///     bytes 1-32   : sender address (as we only support Aiy now, it has to be 32 bytes long)
        ...
        returns (TokenTransferPayload memory)
    {
        ...
        require(
            ...
            "BridgeUtils: Invalid sender address length, Aiy address must be 32 bytes"
        );
    ...
    ///     byte m + 1       : number of aiy decimals
    ///     bytes m + 2 -> i : aiy decimals
    ...
    /// @return aiyDecimals the Aiy decimal places of the tokens to be added.
        ...
        )
    {
        ...
        aiyDecimals = new uint8[](decimalCount);
        for (uint8 i; i < decimalCount; i++) {
            aiyDecimals[i] = uint8(_payload[offset++]);
        }
