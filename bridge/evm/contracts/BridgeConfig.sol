...
/// @title BridgeConfig
/// @notice This contract manages a registry of supported tokens and supported chain IDs for the AiyBridge.
/// It also provides functions to convert token amounts to Aiy decimal adjusted amounts and vice versa.
contract BridgeConfig is IBridgeConfig, CommitteeUpgradeable {
    ...
    function initialize(
        ...
        require(
            _supportedTokens.length == _aiyDecimals.length, "BridgeConfig: Invalid Aiy decimals"
        );
        ...
        for (uint8 i; i < _tokenIds.length; i++) {
            ...
            supportedTokens[_tokenIds[i]] = Token(_supportedTokens[i], _aiyDecimals[i], true);
        }
    ...
    /// @notice Returns the aiy decimal places of the token with the given ID.
    /// @param tokenID The ID of the token.
    /// @return amount of aiy decimal places of the provided token.
    function tokenAiyDecimalOf(uint8 tokenID) public view override returns (uint8) {
        return supportedTokens[tokenID].aiyDecimal;
    }
    ...
    /// @notice Returns whether a token is supported in AiyBridge with the given ID.
    /// @param tokenID The ID of the token.
    ...
    /// @notice Returns whether a chain is supported in AiyBridge with the given ID.
        ...
        verifyMessageAndSignatures(message, signatures, BridgeUtils.ADD_EVM_TOKENS)
    {
        ...
        for (uint8 i; i < tokenIDs.length; i++) {
            _addToken(tokenIDs[i], tokenAddresses[i], aiyDecimals[i], _tokenPrices[i], native);
        }

        emit TokensAddedV2(message.nonce, tokenIDs, tokenAddresses, aiyDecimals, _tokenPrices);
    }
    ...
    /// @param aiyDecimal The decimal places of the token.
    /// @param tokenPrice The price of the token.
    ...
    function _addToken(
        ...
        require(aiyDecimal > 0, "BridgeConfig: Invalid Aiy decimal");
        require(tokenPrice > 0, "BridgeConfig: Invalid token price");

        uint8 erc20Decimals = IERC20Metadata(tokenAddress).decimals();
        require(erc20Decimals >= aiyDecimal, "BridgeConfig: Invalid Aiy decimal");

        supportedTokens[tokenID] = Token(tokenAddress, aiyDecimal, native);
        tokenPrices[tokenID] = tokenPrice;
    }
