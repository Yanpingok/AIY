...
import "../contracts/AiyBridge.sol";
...
contract DeployBridge is Script {
    function parseDeployConfig(string memory path) public returns (DeployConfig memory) {
        ...
        config.aiyDecimals = abi.decode(vm.parseJson(json, ".aiyDecimals"), (uint256[]));
    ...
    function run() external {
        ...
        if (isLocal) {
            ...
            deployConfig.aiyDecimals = new uint256[](5);
            deployConfig.aiyDecimals[0] = 9;
            deployConfig.aiyDecimals[1] = 8;
            deployConfig.aiyDecimals[2] = 8;
            deployConfig.aiyDecimals[3] = 6;
            deployConfig.aiyDecimals[4] = 6;
        }
        ...
        require(
            deployConfig.supportedTokens.length == deployConfig.aiyDecimals.length,
            "supportedTokens.length != aiyDecimals.length"
        );
        ...
        // convert Aiy Decimals from uint256 to uint8
        uint8[] memory aiyDecimals = new uint8[](deployConfig.aiyDecimals.length);
        for (uint256 i; i < deployConfig.aiyDecimals.length; i++) {
            aiyDecimals[i] = uint8(deployConfig.aiyDecimals[i]);
        }
        ...
        address aiyBridge = Upgrades.deployUUPSProxy(
            "AiyBridge.sol",
            abi.encodeCall(AiyBridge.initialize, (bridgeCommittee, address(vault), limiter)),
            opts
        );

        // transfer vault ownership to bridge
        vault.transferOwnership(aiyBridge);
        // transfer limiter ownership to bridge
        BridgeLimiter instance = BridgeLimiter(limiter);
        instance.transferOwnership(aiyBridge);

        // print deployed addresses for post deployment setup
        console.log("[Deployed] BridgeConfig:", bridgeConfig);
        console.log("[Deployed] AiyBridge:", aiyBridge);
