...
import "../contracts/interfaces/IAiyBridge.sol";
import "./mocks/MockAiyBridgeV2.sol";

contract AiyBridgeTest is BridgeBaseTest, IAiyBridge {
    // This function is called before each unit test
    function setUp() public {
        setUpBridgeTest();
    }

    function testAiyBridgeInitialization() public {
    ...
    function testTransferBridgedTokensWithSignaturesTokenDailyLimitExceeded() public {
        ...
        vm.expectRevert(bytes("AiyBridge: Amount exceeds bridge limit"));
    ...
    function testTransferBridgedTokensWithSignaturesInvalidTargetChain() public {
        ...
        signatures[2] = getSignature(messageHash, committeeMemberPkC);
        signatures[3] = getSignature(messageHash, committeeMemberPkD);
        vm.expectRevert(bytes("AiyBridge: Target chain not supported"));
        bridge.transferBridgedTokensWithSignatures(signatures, message);
    }
    ...
    function testTransferBridgedTokensWithSignaturesInsufficientStakeAmount() public {
        ...
        BridgeUtils.TokenTransferPayload memory payload = BridgeUtils.TokenTransferPayload({
            ...
            // This is Aiy amount (eth decimal 8)
            amount: 100_000_000
        });
        BridgeUtils.Message memory message = BridgeUtils.Message({
            messageType: BridgeUtils.TOKEN_TRANSFER,
    ...
    function testTransferWETHWithValidSignatures() public {
        ...
        address recipientAddress = bridgerA;
        uint8 tokenID = BridgeUtils.ETH;
        uint64 amount = 100000000; // 1 ether in aiy decimals
        bytes memory payload = abi.encodePacked(
            senderAddressLength,
            senderAddress,
            targetChain,
        ...
        vm.expectRevert(bytes("AiyBridge: Message already processed"));
        bridge.transferBridgedTokensWithSignatures(signatures, message);
    }
    ...
    function testBridgeERC20UnsupportedToken() public {
        vm.expectRevert(bytes("AiyBridge: Unsupported token"));
        bridge.bridgeERC20(
    ...
    function testBridgeERC20InsufficientAllowance() public {
        vm.expectRevert(bytes("AiyBridge: Insufficient allowance"));
        bridge.bridgeERC20(
            BridgeUtils.ETH,
            type(uint256).max,
    ...
    function testBridgeERC20InvalidRecipientAddress() public {
        vm.expectRevert(bytes("AiyBridge: Invalid recipient address length"));
        bridge.bridgeERC20(
            BridgeUtils.ETH,
            1 ether,
    ...
    function testBridgeEth() public {
        ...
        // assert emitted event
        vm.expectEmit(true, true, true, false);
        emit IAiyBridge.TokensDeposited(
            chainID,
            0, // nonce
            0, // destination chain id
            BridgeUtils.ETH,
    ...
    function testAiyBridgeInvalidERC20DecimalConversion() public {
        IERC20(wETH).approve(address(bridge), 10 ether);
    ...
    function testAiyBridgeInvalidEthDecimalConversion() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
    ...
    function testAiyBridgeInvalidERC20Transfer() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
        bridge.bridgeERC20(
            BridgeUtils.USDC,
            0,
    ...
    function testAiyBridgeInvalidETHTransfer() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
    ...
    function testTransferAiyToEthRegressionTest() public {
        ...
        tokenPrices[0] = 1 * USD_VALUE_MULTIPLIER; // AIY PRICE
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                ...
                (address(committee), 11, _supportedTokens, tokenPrices, tokenIds, aiyDecimals, _supportedDestinationChains)
            ),
            opts
        );
        ...
        address _aiyBridge = Upgrades.deployUUPSProxy(
            "AiyBridge.sol",
            abi.encodeCall(
                AiyBridge.initialize, (address(committee), address(vault), address(limiter))
            ),
            opts
        );
        bridge = AiyBridge(_aiyBridge);
    ...
    function testEmergencyOpRegressionTestWithSigVerification() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                BridgeConfig.initialize,
                (address(committee), chainID, supportedTokens, tokenPrices, tokenIds, aiyDecimals, supportedChains)
            ),
            opts
        );
