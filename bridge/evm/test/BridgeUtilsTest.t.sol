...
contract BridgeUtilsTest is BridgeBaseTest {
    ...
    function testConvertERC20ToAiyDecimalAmountTooLargeForUint64() public {
        vm.expectRevert(bytes("BridgeUtils: Amount too large for uint64"));
        BridgeUtils.convertERC20ToAiyDecimal(18, 8, type(uint256).max);
    }

    function testConvertERC20ToAiyDecimalInvalidAiyDecimal() public {
        vm.expectRevert(bytes("BridgeUtils: Invalid Aiy decimal"));
        BridgeUtils.convertERC20ToAiyDecimal(10, 11, 100);
    }

    function testconvertAiyToERC20DecimalInvalidAiyDecimal() public {
        vm.expectRevert(bytes("BridgeUtils: Invalid Aiy decimal"));
        BridgeUtils.convertAiyToERC20Decimal(10, 11, 100);
    }

    function testConvertERC20ToAiyDecimal() public {
        // ETH
        assertEq(IERC20Metadata(wETH).decimals(), 18);
        uint256 ethAmount = 10 ether;
        uint64 aiyAmount = BridgeUtils.convertERC20ToAiyDecimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.ETH)).decimals(),
            config.tokenAiyDecimalOf(BridgeUtils.ETH),
            ethAmount
        );
        assertEq(aiyAmount, 10_000_000_00); // 10 * 10 ^ 8

        // USDC
        assertEq(IERC20Metadata(USDC).decimals(), 6);
        ethAmount = 50_000_000; // 50 USDC
        aiyAmount = BridgeUtils.convertERC20ToAiyDecimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDC)).decimals(),
            config.tokenAiyDecimalOf(BridgeUtils.USDC),
            ethAmount
        );
        assertEq(aiyAmount, ethAmount);
        ...
        aiyAmount = BridgeUtils.convertERC20ToAiyDecimal(
            ...
            config.tokenAiyDecimalOf(BridgeUtils.USDT),
            ethAmount
        );
        assertEq(aiyAmount, ethAmount);
        ...
        aiyAmount = BridgeUtils.convertERC20ToAiyDecimal(
            ...
            config.tokenAiyDecimalOf(BridgeUtils.BTC),
            ethAmount
        );
        assertEq(aiyAmount, ethAmount);
    }

    function testconvertAiyToERC20Decimal() public {
        // ETH
        assertEq(IERC20Metadata(wETH).decimals(), 18);
        uint64 aiyAmount = 11_000_000_00; // 11 eth
        uint256 ethAmount = BridgeUtils.convertAiyToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.ETH)).decimals(),
            config.tokenAiyDecimalOf(BridgeUtils.ETH),
            aiyAmount
        );
        ...
        // USDC
        assertEq(IERC20Metadata(USDC).decimals(), 6);
        aiyAmount = 50_000_000; // 50 USDC
        ethAmount = BridgeUtils.convertAiyToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDC)).decimals(),
            config.tokenAiyDecimalOf(BridgeUtils.USDC),
            aiyAmount
        );
