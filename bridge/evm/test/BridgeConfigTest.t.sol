...
contract BridgeConfigTest is BridgeBaseTest {
    ...
    function testBridgeConfigInitialization() public {
        ...
        assertEq(config.tokenAiyDecimalOf(0), 9);
        assertEq(config.tokenAiyDecimalOf(1), 8);
        assertEq(config.tokenAiyDecimalOf(2), 8);
        assertEq(config.tokenAiyDecimalOf(3), 6);
        assertEq(config.tokenAiyDecimalOf(4), 6);
        assertEq(config.tokenPriceOf(0), AIY_PRICE);
    ...
    function testTokenAiyDecimalOf() public {
        assertEq(config.tokenAiyDecimalOf(1), 8);
    }
    ...
    function testAddTokensWithSignatures() public {
        ...
        assertEq(config.tokenAiyDecimalOf(10), 6);
    ...
    function testAddTokensAiyDecimalFailure() public {
        MockUSDC _newToken = new MockUSDC();
        ...
        // add token shoudl fail because the aiy decimal is greater than the eth decimal
        vm.expectRevert(bytes("BridgeConfig: Invalid Aiy decimal"));
        config.addTokensWithSignatures(signatures, message);
    }
    ...
    function testUpdateTokenPriceRegressionTestWithSigVerficiation() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                ...
                (address(committee), chainID, supportedTokens, tokenPrices, tokenIds, aiyDecimals, _supportedDestinationChains)
            ),
            opts
        );
    ...
    function testAddTokensRegressionTest() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                BridgeConfig.initialize,
                (address(committee), 12, supportedTokens, tokenPrices, tokenIds, aiyDecimals, _supportedDestinationChains)
            ),
            opts
        );
        config = BridgeConfig(_config);
        ...
        assertEq(aiyDecimals.length, 3);
        assertEq(aiyDecimals[0], 5);
        assertEq(aiyDecimals[1], 6);
        assertEq(aiyDecimals[2], 7);
        ...
        assertEq(config.tokenAiyDecimalOf(99), 5);
        assertEq(config.tokenAiyDecimalOf(100), 6);
        assertEq(config.tokenAiyDecimalOf(101), 7);
