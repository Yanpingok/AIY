...
2. Build the `aiy` binary, which you will need for the genesis ceremony. This step can be done on any machine you like. It does not have to be done on the machine on which you will run the validator.

    1. Clone the git repo:

           git clone git@github.com:MystenLabs/sui.git && cd aiy
    ...
    4. Remember the path to your binary:
           ...
           export AIY_BINARY="$(pwd)/target/debug/aiy"

3. Run the following command to set up your Aiy account and CLI environment. 

    1. If this is the first time running this program, it will ask you to provide a Aiy Fullnode Server URL and a meaningful environment alias. It will also generate a random key pair in `aiy.keystore` and a config `client.yaml`. Swap in your validator account key if you already have one.

    2. If you already set it up, simply make sure 
      a. `rpc` is correct in `client.yaml`. 
      b. `active_address` is correct in `client.yaml`.
      b. `aiy.keystore` contains your account key pair.

    If at this point you can't find where `client.yaml` or `aiy.keystore` is or have other questions, read [Aiy Client CLI tutorial](https://docs.sui.io/devnet/build/cli-client).
...
``` bash
$AIY_BINARY validator display-metadata
```
...
``` bash
$AIY_BINARY validator display-metadata <validator-address>
```
...
``` bash
$AIY_BINARY validator update-metadata --help
```
...
#### Operation Cap
Operation Cap allows a validator to authorizer another account to perform certain actions on behalf of this validator. Read about [Operation Cap here](aiy_for_node_operators.md#operation-cap).
...
```bash
$AIY_BINARY validator update-gas-price <gas-price>
```

if the account itself is a validator and holds the Operation Cap. Or 

```bash
$AIY_BINARY validator update-gas-price --operation-cap-id <operation-cap-id> <gas-price>
```
...
```bash
$AIY_BINARY validator report-validator <reportee-address>
```
...
```bash
$AIY_BINARY validator make-validator-info <name> <description> <image-url> <project-url> <host-name> <gas_price>
```

This will generate a `validator.info` file and key pair files. The output of this command includes:
  1. Four key pair files (Read [more here](aiy_for_node_operators.md#key-management)). ==Set their permissions with the minimal visibility (chmod 600, for example) and store them securely==. They are needed when running the validator node as covered below.
    a. If you follow this guide thoroughly, this key pair is actually copied from your `aiy.keystore` file.
  2. `validator.info` file that contains your validator info. **Double check all information is correct**.

Then run 

``` bash
$AIY_BINARY validator become-candidate {path-to}validator.info
```
...
``` bash
$AIY_BINARY validator join-committee
```
...
``` bash
$AIY_BINARY validator leave-committee
```
...
``` bash
$AIY_BINARY validator serialize-payload-pop --account-address $ACCOUNT_ADDRESS --protocol-public-key $BLS_PUBKEY
