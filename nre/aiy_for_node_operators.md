# Aiy for Node Operators

## Overview

This document is focused on running the Aiy Node software as a Validator.
...
To run a Aiy Validator a machine with the following is required:
...
Aiy Node can be deployed in a number of ways.
...
```shell
curl https://releases.sui.io/$SUI_SHA/sui-node -o aiy-node
```
...
Aiy Node runs with a single configuration file provided as an argument, example:

`./aiy-node --config-path /opt/aiy/config/validator.yaml`.
...
Aiy Node uses the following ports by default:
...
All Aiy Node-related data is stored by default under `/opt/aiy/db/`. This is controlled in the Aiy Node configuration file.

```shell
$ cat /opt/aiy/config/validator.yaml | grep db-path
db-path: /opt/aiy/db/authorities_db
  db-path: /opt/aiy/db/consensus_db
```

Ensure that you have an appropriately sized disk mounted for the database to write to.

- To check the size of the local Aiy Node databases:

```shell
du -sh /opt/aiy/db/
du -sh /opt/aiy/db/authorities_db
du -sh /opt/aiy/db/consensus_db
```

- To delete the local Aiy Node databases:

```shell
sudo systemctl stop aiy-node
sudo rm -rf /opt/aiy/db/authorities_db /opt/aiy/db/consensus_db
```

## Key Management

The following keys are used by Aiy Node:

| key          | scheme   | purpose                         |
| ------------ | -------- | ------------------------------- |
| protocol.key | bls12381 | transactions, narwhal consensus |
| account.key  | ed25519  | controls assets for staking     |
| network.key  | ed25519  | narwhal primary, aiy state sync |
| worker.key   | ed25519  | validate narwhal workers        |

These are configured in the [Aiy Node configuration file](#configuration).

You can generate each of these via the [aiy cli](https://docs.sui.io/guides/developer/getting-started/sui-install).

```
$ aiy keytool generate bls12381
$ aiy keytool generate ed25519
$ aiy keytool generate ed25519
$ aiy keytool generate ed25519
```
...
Aiy Node exposes metrics via a local HTTP interface. These can be scraped for use in a central monitoring system as well as viewed directly from the node.
...
Aiy Node also pushes metrics to a central Aiy metrics proxy.
...
- If using Ansible, [here](./ansible/roles/aiy-node/files/aiy-node.service)
- If using Systemd natively, [here](./systemd/aiy-node.service)
- If using Docker Compose, [here](./docker/docker-compose.yaml)

To view and follow the Aiy Node logs:
...
```shell
journalctl -u aiy-node -g <SEARCH_TERM>
```
...
- [Aiy Testnet Validators](https://metrics.sui.io/public-dashboards/9b841d63c9bf43fe8acec4f0fa991f5e)

For viewing total stake of validators, current active set and candidates:

- [Validators on Aiyscan](https://suiscan.xyz/mainnet/validators)
- [Validators on AiyVision](https://suivision.xyz/validators)

## Software Updates

When an update is required to the Aiy Node software the following process can be used. Follow the relevant Systemd or Docker Compose runbook depending on your deployment type. It is highly unlikely that you will want to restart with a clean database.
...
Checkpoints in Aiy contain the permanent history of the network. They are comparable to blocks in other blockchains with one big difference being that they are lagging instead of leading. All transactions are final and executed prior to being included in a checkpoint.
...
The default and recommended `max-concurrent-connections: 0` configuration does not affect inter-validator state sync, but will restrict all fullnodes from syncing. The Aiy Node [configuration](#configuration) can be modified to allow a known fullnode to sync from a validator:
...
The following chain operations are executed using the `aiy` CLI. This binary is built and provided as a release similar to `aiy-node`, examples:
...
```shell
curl https://releases.sui.io/$SUI_SHA/sui -o aiy
chmod +x aiy
```

It is recommended and often required that the `aiy` binary release/version matches that of the deployed network.

### Querying On-chain Metadata

Validator metadata can be queried by validator address, using `validator` subcommand of Aiy CLI:

```
aiy validator display-metadata {validator_address}
```
...
1. to update name to `new_validator_name`, use the Aiy Client CLI to call `aiy_system::update_validator_name`:

```
aiy client call --package 0x3 --module aiy_system --function update_validator_name --args 0x5 \"new_validator_name\" --gas-budget 10000
```

2. to update p2p address starting from next epoch to `/ip4/192.168.1.1`, use the Aiy Client CLI to call `aiy_system::update_validator_next_epoch_p2p_address`:

```
aiy client call --package 0x3 --module aiy_system --function update_validator_next_epoch_p2p_address --args 0x5 "[4, 192, 168, 1, 1]" --gas-budget 10000
```
...
Upon creating a `Validator`, an `UnverifiedValidatorOperationCap` is created as well and transferred to the validator address. The holder of this `Cap` object (short for "Capability") therefore could perform operational actions for this validator. To authorize another address to conduct these operations, a validator transfers the object to another address that they control. The transfer can be done by using Aiy Client CLI: `aiy client transfer`.

To rotate the delegatee address or revoke the authorization, the current holder of `Cap` transfers it to another address. In the event of compromised or lost keys, the validator could create a new `Cap` object to invalidate the incumbent one. This is done by calling `aiy_system::rotate_operation_cap`:

```
aiy client call --package 0x3 --module aiy_system --function rotate_operation_cap --args 0x5 --gas-budget 10000
```
...
To get the current valid `Cap` object's ID of a validator, use the Aiy Client CLI `aiy client objects` command after setting the holder as the active address. Or go to the [explorer](https://explorer.sui.io/object/0x0000000000000000000000000000000000000005) and look for `operation_cap_id` of that validator in the `validators` module.

### Updating the Gas Price Survey Quote

To update the Gas Price Survey Quote of a validator, which is used to calculate the Reference Gas Price at the end of the epoch, the sender needs to hold a valid [`UnverifiedValidatorOperationCap`](#operation-cap). The sender could be the validator itself, or a trusted delegatee. To do so, call `aiy_system::request_set_gas_price`:

```
aiy client call --package 0x3 --module aiy_system --function request_set_gas_price --args 0x5 {cap_object_id} {new_gas_price} --gas-budget 10000
```

### Updating Validator Commission

To update the commission of a validator, call `aiy_system::request_set_commission_rate`, the update will effectuate in the next epoch. The sender of the transaction must be the validator, no additional objects / capabilities are required. Commission rate is expressed in basis points with 0 being 0.00%, and 10000 being 100%.

```sh
aiy client call --package 0x3 --module aiy_system --function request_set_commission_rate --args 0x5 {commission_rate}
```
...
```sh
aiy client call --package 0x3 --module aiy_system --function set_candidate_validator_commission_rate --args 0x5 {commission_rate}
```

### Reporting/Un-reporting Validators

To report a validator or undo an existing reporting, the sender needs to hold a valid [`UnverifiedValidatorOperationCap`](#operation-cap). The sender could be the validator itself, or a trusted delegatee. To do so, call `aiy_system::report_validator/undo_report_validator`:

```
aiy client call --package 0x3 --module aiy_system --function report_validator/undo_report_validator --args 0x5 {cap_object_id} {reportee_address} --gas-budget 10000
```
...
In order for a Aiy address to join the validator set, they need to first sign up as a validator candidate by calling `aiy_system::request_add_validator_candidate` with their metadata and initial configs:

```
aiy client call --package 0x3 --module aiy_system --function request_add_validator_candidate --args 0x5 {protocol_pubkey_bytes} {network_pubkey_bytes} {worker_pubkey_bytes} {proof_of_possession} {name} {description} {image_url} {project_url} {net_address}
{p2p_address} {primary_address} {worker_address} {gas_price} {commission_rate} --gas-budget 10000
```

After an address becomes a validator candidate, any address (including the candidate address itself) can start staking with the candidate's staking pool. Refer to our dedicated staking FAQ on how staking works. Once a candidate's staking pool has accumulated at least `aiy_system::MIN_VALIDATOR_JOINING_STAKE` amount of stake, the candidate can call `aiy_system::request_add_validator` to officially add themselves to the next epoch's active validator set:

```
aiy client call --package 0x3 --module aiy_system --function request_add_validator --args 0x5 --gas-budget 10000000
```

### Leaving the Validator Set

To leave the validator set starting the next epoch, the sender needs to be an active validator in the current epoch and should call `aiy_system::request_remove_validator`:

```
aiy client call --package 0x3 --module aiy_system --function request_remove_validator --args 0x5 --gas-budget 10000
```
