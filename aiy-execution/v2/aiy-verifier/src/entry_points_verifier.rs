...
use aiy_types::randomness_state::is_mutable_random;
use aiy_types::{
...
fn verify_param_type(
    ...
    // Only `aiy::aiy_system` is allowed to expose entry functions that accept a mutable clock
    // parameter.
    ...
    // Only `aiy::aiy_system` is allowed to expose entry functions that accept a mutable Random
    // parameter.
