...
mod checked {
    ...
    use crate::{adapter::new_native_extensions, gas_meter::AiyGasMeter};
    ...
    use aiy_types::execution::ExecutionResults;
    use aiy_types::storage::PackageObject;
    use aiy_types::{
        balance::Balance,
    ...
    use aiy_types::{error::command_argument_error, execution_status::CommandArgumentError};
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        ...
        pub fn new(
            ...
            let mut linkage_view = LinkageView::new(Box::new(state_view.as_aiy_resolver()));
    ...
    pub(crate) struct AiyDataStore<'state, 'a> {
        linkage_view: &'a LinkageView<'state>,
        new_packages: &'a [MovePackage],
    }

    impl<'state, 'a> AiyDataStore<'state, 'a> {
        pub(crate) fn new(
            linkage_view: &'a LinkageView<'state>,
            new_packages: &'a [MovePackage],
        ) -> Self {
    ...
    impl DataStore for AiyDataStore<'_, '_> {
