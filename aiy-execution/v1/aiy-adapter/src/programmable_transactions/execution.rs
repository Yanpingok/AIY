...
mod checked {
    ...
    use aiy_move_natives::object_runtime::ObjectRuntime;
    use aiy_protocol_config::ProtocolConfig;
    use aiy_types::execution_config_utils::to_binary_config;
    use aiy_types::execution_status::{CommandArgumentError, PackageUpgradeError};
    use aiy_types::storage::{get_package_objects, PackageObject};
    use aiy_types::{
        base_types::{
            MoveLegacyTxContext, MoveObjectType, ObjectID, AiyAddress, TxContext, TxContextKind,
    ...
    fn publish_and_verify_modules(
        ...
        // run the Aiy verifier
        for module in modules {
            // Run Aiy bytecode verifier, which runs some additional checks that assume the Move
            // bytecode verifier has passed.
            aiy_verifier::verifier::aiy_verify_module_unmetered(module, &BTreeMap::new())?;
        }
    ...
    fn check_visibility_and_signature<Mode: ExecutionMode>(
        ...
        let data_store = AiyDataStore::new(&context.linkage_view, &no_new_packages);
        let module = context
            .vm
            .get_runtime()
    ...
    fn check_private_generics(
        ...
        if module_ident == (&AIY_FRAMEWORK_ADDRESS, EVENT_MODULE) {
            return Err(ExecutionError::new_with_source(
                ExecutionErrorKind::NonEntryFunctionInvoked,
                format!("Cannot directly call functions in aiy::{}", EVENT_MODULE),
            ));
        }

        if module_ident == (&AIY_FRAMEWORK_ADDRESS, TRANSFER_MODULE)
            && PRIVATE_TRANSFER_FUNCTIONS.contains(&function)
        {
            let msg = format!(
                "Cannot directly call aiy::{m}::{f}. \
                Use the public variant instead, aiy::{m}::public_{f}",
                m = TRANSFER_MODULE,
                f = function
            );
            return Err(ExecutionError::new_with_source(
        ...
        match value {
            ...
            Value::Receiving(_, _, assigned_type) => {
                ...
                let Some(s) = context.vm.get_runtime().get_type(*sidx) else {
                    invariant_violation!("aiy::transfer::Receiving struct not found in session")
                };
    ...
    pub fn is_tx_context(
        ...
        let is_tx_context_type = module_addr == &AIY_FRAMEWORK_ADDRESS
        ...
        fn deserialize<D: serde::de::Deserializer<'d>>(
            ...
            match self {
                ...
                PrimitiveArgumentLayout::Address => {
                    AiyAddress::deserialize(deserializer)?;
                    Ok(())
                }
            }
        }
