...
use aiy_types::storage::{BackingPackageStore, PackageObject};
use aiy_types::{error::AiyError, layout_resolver::LayoutResolver};
...
/// Implements AiyResolver traits by providing null implementations for module and resource
/// resolution and delegating backing package resolution to the trait object.
struct NullAiyResolver<'state>(Box<dyn TypeLayoutStore + 'state>);

impl<'state, 'vm> TypeLayoutResolver<'state, 'vm> {
    pub fn new(vm: &'vm MoveVM, state_view: Box<dyn TypeLayoutStore + 'state>) -> Self {
        let session = new_session_for_linkage(
            vm,
            LinkageView::new(Box::new(NullAiyResolver(state_view)), LinkageInfo::Unset),
        );
        Self { session }
    }
}
...
impl BackingPackageStore for NullAiyResolver<'_> {
