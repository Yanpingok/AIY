...
use aiy_types::execution::{DynamicallyLoadedObjectMetadata, ExecutionResults, SharedInput};
...
use aiy_types::storage::{BackingStore, DeleteKindWithOldVersion, DenyListResult, PackageObject};
use aiy_types::aiy_system_state::{get_aiy_system_state_wrapper, AdvanceEpochParams};
use aiy_types::{
    base_types::{
        ObjectDigest, ObjectID, ObjectRef, SequenceNumber, AiyAddress, TransactionDigest,
        VersionDigest,
    },
...
impl TemporaryStore<'_> {
    ...
    ///
    /// 3. all AIY in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// amount of AIY. We need these information for conservation check.
    pub fn check_aiy_conserved(
        ...
        for (id, input, output) in self.get_modified_objects() {
            if let Some((version, storage_rebate)) = input {
                ...
                if do_expensive_checks {
                    total_input_aiy += self.get_input_aiy(&id, version, layout_resolver)?;
                }
            }
        ...
        if do_expensive_checks {
            ...
            if total_input_aiy != total_output_aiy {
                ...
                format!("AIY conservation failed: input={}, output={}, this transaction either mints or burns AIY",
                total_input_aiy,
                total_output_aiy))
            );
            }
        ...
        // all AIY in storage rebate fields of input objects should flow either to the transaction storage rebate, or the non-refundable
        // storage rebate pool
        if total_input_rebate != gas_summary.storage_rebate + gas_summary.non_refundable_storage_fee
        {
            ...
            /*return Err(ExecutionError::invariant_violation(
                format!("AIY conservation failed--{} AIY in storage rebate field of input objects, {} AIY in tx storage rebate or tx non-refundable storage rebate",
                total_input_rebate,
                gas_summary.non_refundable_storage_fee))
            );*/
        }

        // all AIY charged for storage should flow into the storage rebate field of some output object
        if gas_summary.storage_cost != total_output_rebate {
            // TODO: re-enable once we fix the edge case with OOG, gas smashing, and storage rebate
            /*return Err(ExecutionError::invariant_violation(
                format!("AIY conservation failed--{} AIY charged for storage, {} AIY in storage rebate field of output objects",
...
impl Storage for TemporaryStore<'_> {
    ...
    fn record_execution_results(&mut self, results: ExecutionResults) {
        let ExecutionResults::V1(results) = results else {
            panic!("ExecutionResults::V1 expected in aiy-execution v0");
        };
    ...
    fn check_coin_deny_list(
        ...
        unreachable!("Coin denylist v2 is not supported in aiy-execution v0");
    }

    fn record_generated_object_ids(&mut self, _generated_ids: BTreeSet<ObjectID>) {
        unreachable!(
            "Generated object IDs are not recorded in ExecutionResults in aiy-execution v0"
        );
    }
}
