...
#[test]
/// Make sure that all accesses to execution layer crates in the `aiy-node` and `aiy-replay` crates
/// go via the `aiy-execution` crate (in other words, the `aiy-execution` crate dominates execution
/// layer crates in the dependency graphs of `aiy-node` and `aiy-replay`).
///
...
fn test_encapsulation() {
    ...
    let mut exec_crates: BTreeSet<_> = packages.normal_deps("aiy-execution").collect();

    // Remove the crates that the execution layer depends on but which are not directly part of the
    // execution layer -- these don't need to be accessed exclusively via `aiy-execution`.
    exec_crates.remove("aiy-protocol-config");
    exec_crates.remove("aiy-types");
    ...
    panic!(
        "protocol-sensitive binaries depend on execution crates outside of 'aiy-execution', e.g.:\n\
         \n  {}\n\
         \n\
         This can cause execution to fork by not respecting the execution layer version set in the \
         protocol config.  Fix this by depending on these crates via 'aiy-execution'.\n\
         \n\
         P.S. if you believe one of these crates should not be part of 'aiy-execution' then update \
         the test to exclude this crate.",
        examples.join("\n  "),
    );
}

/// Parse `cargo metadata` for the `aiy` repo.
fn cargo_metadata() -> cargo_metadata::Result<Metadata> {
    let aiy_execution = PathBuf::from(env!("CARGO_MANIFEST_DIR"));

    MetadataCommand::new()
        .manifest_path(aiy_execution.join("../Cargo.toml"))
        .current_dir(aiy_execution.join(".."))
        .no_deps()
        .exec()
}
