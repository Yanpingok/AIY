...
mod checked {
    use crate::{
        ...
        data_store::{
            ...
            legacy::{linkage_view::LinkageView, aiy_data_store::AiyDataStore},
        },
    ...
    use aiy_move_natives::object_runtime::{
    ...
    use aiy_types::{
        ...
        balance::Balance,
        base_types::{MoveObjectType, ObjectID, AiyAddress, TxContext},
        coin::Coin,
        effects::{AccumulatorAddress, AccumulatorValue, AccumulatorWriteV1},
        error::{ExecutionError, ExecutionErrorKind, AiyError, command_argument_error},
        event::Event,
    ...
    pub struct ExecutionContext<'vm, 'state, 'a> {
        ...
        additional_transfers: Vec<(/* new owner */ AiyAddress, ObjectValue)>,
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        ...
        pub(crate) fn execute_function_bypass_visibility(
            ...
            let mut data_store = AiyDataStore::new(&self.linkage_view, &self.new_packages);
        ...
        pub fn publish_module_bundle(
            ...
            self.vm.get_runtime().publish_module_bundle(
                ...
                sender,
                &mut data_store,
                &mut AiyGasMeter(self.gas_charger.move_gas_status_mut()),
            )
        }
        ...
        let accumulator_events = accumulator_events
            ...
            .map(|accum_event| {
                ...
                let write = AccumulatorWriteV1 {
                    address,
                    operation: accum_event.action.into_aiy_accumulator_action(),
                    value,
                };
    ...
    pub fn load_type_from_struct(
        ...
        let data_store = AiyDataStore::new(linkage_view, new_packages);
        ...
        Ok(match call_arg {
            ...
            CallArg::FundsWithdrawal(FundsWithdrawalArg {
                ...
                let limit = match reservation {
                    aiy_types::transaction::Reservation::EntireBalance => {
                        // TODO(address-balances): support entire balance withdrawal
                        todo!("Entire balance withdrawal not yet supported")
                    }
                    aiy_types::transaction::Reservation::MaxAmountU64(u) => U256::from(u),
                };
    ...
    fn convert_vm_error<S: MoveResolver<Err = AiyError>>(
        error: VMError,
        vm: &MoveVM,
        state_view: &S,
    ...
    /// Special case errors for type arguments to Move functions
    fn convert_type_argument_error<S: MoveResolver<Err = AiyError>>(
        idx: usize,
        error: VMError,
        vm: &MoveVM,
        state_view: &S,
        ...
        use aiy_types::execution_status::TypeArgumentError;
