...
use std::rc::Rc;
use aiy_types::{base_types::ObjectID, error::AiyResult, move_package::MovePackage};
...
impl PackageStore for AiyDataStore<'_, '_> {
    fn get_package(&self, id: &ObjectID) -> AiyResult<Option<Rc<MovePackage>>> {
        for package in self.new_packages {
            if package.id() == *id {
    ...
    fn resolve_type_to_defining_id(
        ...
        unimplemented!(
            "resolve_type_to_defining_id is not implemented for legacy::AiyDataStore and should never be called"
        )
    }
}
