...
macro_rules! charge_gas_ {
    ($gas_charger:expr, $env:expr, $case:ident, $value_view:expr) => {{
        AiyGasMeter($gas_charger.move_gas_status_mut())
            .$case($value_view)
...
impl<'env, 'pc, 'vm, 'state, 'linkage, 'gas> Context<'env, 'pc, 'vm, 'state, 'linkage, 'gas> {
    ...
    pub fn argument_updates(
        &mut self,
        args: Vec<T::Argument>,
    ) -> Result<Vec<(aiy_types::transaction::Argument, Vec<u8>, TypeTag)>, ExecutionError> {
        args.into_iter()
    ...
    ) -> Result<Option<(aiy_types::transaction::Argument, Vec<u8>, TypeTag)>, ExecutionError> {
        use aiy_types::transaction::Argument as TxArgument;
        let ty = match ty {
...
fn load_object_arg_impl(
    ...
    let aiy_types::object::ObjectInner {
        data: aiy_types::object::Data::Move(move_obj),
        ..
    } = obj.as_inner()
    else {
