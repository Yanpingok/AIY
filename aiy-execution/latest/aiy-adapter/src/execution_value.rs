...
pub trait AiyResolver: BackingPackageStore {
    fn as_backing_package_store(&self) -> &dyn BackingPackageStore;
}

impl<T> AiyResolver for T
where
...
pub trait ExecutionState: StorageView + AiyResolver {
    fn as_aiy_resolver(&self) -> &dyn AiyResolver;
...
impl InputValue {
    ...
    pub fn withdrawal(withdrawal_ty: RawValueType, owner: AiyAddress, limit: U256) -> Self {
        let value = Value::Raw(
            withdrawal_ty,
...
impl TryFromValue for AiyAddress {
    fn try_from_value(value: Value) -> Result<Self, CommandArgumentError> {
