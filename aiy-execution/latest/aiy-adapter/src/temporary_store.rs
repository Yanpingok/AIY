...
use aiy_types::base_types::VersionDigest;
use aiy_types::committee::EpochId;
use aiy_types::deny_list_v2::check_coin_deny_list_v2_during_execution;
use aiy_types::effects::{TransactionEffects, TransactionEvents};
use aiy_types::execution::{
    DynamicallyLoadedObjectMetadata, ExecutionResults, ExecutionResultsV2, SharedInput,
};
use aiy_types::execution_config_utils::to_binary_config;
use aiy_types::execution_status::ExecutionStatus;
use aiy_types::inner_temporary_store::InnerTemporaryStore;
use aiy_types::layout_resolver::LayoutResolver;
use aiy_types::storage::{BackingStore, DenyListResult, PackageObject};
use aiy_types::aiy_system_state::{AdvanceEpochParams, get_aiy_system_state_wrapper};
use aiy_types::{
    AIY_DENY_LIST_OBJECT_ID,
    base_types::{ObjectID, ObjectRef, SequenceNumber, AiyAddress, TransactionDigest},
    effects::EffectsObjectChange,
    error::{ExecutionError, AiyResult},
    gas::GasCostSummary,
    object::Object,
    object::Owner,
...
use aiy_types::{AIY_SYSTEM_STATE_OBJECT_ID, TypeTag, is_system_package};
...
impl<'backing> TemporaryStore<'backing> {
    ...
    pub fn conserve_unmetered_storage_rebate(&mut self, unmetered_storage_rebate: u64) {
        ...
        let mut system_state_wrapper = self
            .read_object(&AIY_SYSTEM_STATE_OBJECT_ID)
        ...
        // If not, we would be dropping AIY on the floor by overriding it.
                    ...
                    Owner::Immutable => {
                        assert!(
                            ...
                            "Immutable objects cannot be written, except for \
                             Aiy Framework/Move stdlib upgrades at epoch change boundaries"
                        );
...
impl TemporaryStore<'_> {
    ...
    /// `AiyGasStatus` `storage_rebate` and `storage_gas_units` track the transaction
...
impl TemporaryStore<'_> {
    pub fn advance_epoch_safe_mode(
        ...
        let wrapper = get_aiy_system_state_wrapper(self.store.as_object_store())
...
impl TemporaryStore<'_> {
    fn get_input_aiy(
        ...
        if let Some(obj) = self.input_objects.get(id) {
            ...
            obj.get_total_aiy(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input AIY in AIY conservation checking for input with \
                         type {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
            ...
            let Some(obj) = self.store.get_object_by_key(id, expected_version) else {
                invariant_violation!(
                    "Failed looking up dynamic field {id} in AIY conservation checking"
                );
            };
            obj.get_total_aiy(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input AIY in AIY conservation checking for type \
                         {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
    ...
    /// Check that this transaction neither creates nor destroys AIY. This should hold for all txes
    ...
    ///
    /// 1. all AIY in storage rebate fields of input objects should flow either to the transaction
    ///    storage rebate, or the transaction non-refundable storage rebate
    /// 2. all AIY charged for storage should flow into the storage rebate field of some output
    ///    object
    ///
    ...
    pub fn check_aiy_conserved(
        ...
        // total amount of AIY in storage rebate of input objects
        let mut total_input_rebate = 0;
        // total amount of AIY in storage rebate of output objects
        ...
        if gas_summary.storage_cost == 0 {
                    ...
                    + gas_summary.non_refundable_storage_fee
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "AIY conservation failed -- no storage charges in gas summary \
            ...
            // all AIY in storage rebate fields of input objects should flow either to
            ...
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "AIY conservation failed -- {} AIY in storage rebate field of input objects, \
                        {} AIY in tx storage rebate or tx non-refundable storage rebate",
                    total_input_rebate, gas_summary.non_refundable_storage_fee,
                )));
            }

            // all AIY charged for storage should flow into the storage rebate field
            // of some output object
            if gas_summary.storage_cost != total_output_rebate {
                return Err(ExecutionError::invariant_violation(format!(
                    "AIY conservation failed -- {} AIY charged for storage, \
                        {} AIY in storage rebate field of output objects",
    ...
    /// Check that this transaction neither creates nor destroys AIY.
    /// This more expensive check will check a third invariant on top of the 2 performed
    /// by `check_aiy_conserved` above:
    ///
    /// * all AIY in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// advance epoch transaction would mint `epoch_fees` amount of AIY, and burn `epoch_rebates`
    /// amount of AIY. We need these information for this check.
    pub fn check_aiy_conserved_expensive(
        &self,
        gas_summary: &GasCostSummary,
        advance_epoch_gas_summary: Option<(u64, u64)>,
        layout_resolver: &mut impl LayoutResolver,
    ) -> Result<(), ExecutionError> {
        // total amount of AIY in input objects, including both coins and storage rebates
        let mut total_input_aiy = 0;
        // total amount of AIY in output objects, including both coins and storage rebates
        let mut total_output_aiy = 0;

        // settlement input/output aiy is used by the settlement transactions to account for
        // Aiy that has been gathered from the accumulator writes of transactions which it is
        // settling.
        total_input_aiy += self.execution_results.settlement_input_aiy;
        total_output_aiy += self.execution_results.settlement_output_aiy;

        for (id, input, output) in self.get_modified_objects() {
            if let Some(input) = input {
                total_input_aiy += self.get_input_aiy(&id, input.version, layout_resolver)?;
            }
            if let Some(object) = output {
                total_output_aiy += object.get_total_aiy(layout_resolver).map_err(|e| {
                    make_invariant_violation!(
                        "Failed looking up output AIY in AIY conservation checking for \
                         mutated type {:?}: {e:#?}",
                        object.struct_tag(),
                    )
                })?;
        ...
        for event in &self.execution_results.accumulator_events {
            let (input, output) = event.total_aiy_in_event();
            total_input_aiy += input;
            total_output_aiy += output;
        }
        ...
        total_output_aiy += gas_summary.computation_cost + gas_summary.non_refundable_storage_fee;
        if let Some((epoch_fees, epoch_rebates)) = advance_epoch_gas_summary {
            total_input_aiy += epoch_fees;
            total_output_aiy += epoch_rebates;
        }
        if total_input_aiy != total_output_aiy {
            return Err(ExecutionError::invariant_violation(format!(
                "AIY conservation failed: input={}, output={}, \
                    this transaction either mints or burns AIY",
                total_input_aiy, total_output_aiy,
            )));
        }
        Ok(())
    }
...
impl Storage for TemporaryStore<'_> {
    ...
    fn record_execution_results(&mut self, results: ExecutionResults) {
        let ExecutionResults::V2(results) = results else {
            panic!("ExecutionResults::V2 expected in aiy-execution v1 and above");
        };
    ...
    fn check_coin_deny_list(
        &self,
        receiving_funds_type_and_owners: BTreeMap<TypeTag, BTreeSet<AiyAddress>>,
    ) -> DenyListResult {
        ...
        if result.num_non_gas_coin_owners > 0
            && !self.input_objects.contains_key(&AIY_DENY_LIST_OBJECT_ID)
        {
            self.loaded_per_epoch_config_objects
                .write()
                .insert(AIY_DENY_LIST_OBJECT_ID);
        }
        result
    }
...
impl BackingPackageStore for TemporaryStore<'_> {
    fn get_package_object(&self, package_id: &ObjectID) -> AiyResult<Option<PackageObject>> {
