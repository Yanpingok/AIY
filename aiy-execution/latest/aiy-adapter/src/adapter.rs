...
mod checked {
    ...
    use aiy_move_natives::{object_runtime, transaction_context::TransactionContext};
    use aiy_types::metrics::BytecodeVerifierMetrics;
    use aiy_verifier::check_for_verifier_timeout;
    use tracing::instrument;

    use aiy_move_natives::{NativesCostTable, object_runtime::ObjectRuntime};
    use aiy_protocol_config::ProtocolConfig;
    use aiy_types::{
        base_types::*,
        error::ExecutionError,
        error::{ExecutionErrorKind, AiyError},
        execution_config_utils::to_binary_config,
        metrics::LimitsMetrics,
        storage::ChildObjectResolver,
    };
    use aiy_verifier::verifier::aiy_verify_module_metered_check_timeout_only;

    pub fn new_move_vm(
        natives: NativeFunctionTable,
        protocol_config: &ProtocolConfig,
    ) -> Result<MoveVM, AiyError> {
        ...
        .map_err(|_| AiyError::ExecutionInvariantViolation)
    }
    ...
    /// Run both the Move verifier and the Aiy verifier, checking just for timeouts. Returns Ok(())
    ...
    fn verify_module_timeout_only(
        ...
        if let Err(e) = verify_module_with_config_metered(verifier_config, module, meter) {
            ...
            if check_for_verifier_timeout(&e.major_status()) {
                return Err(AiyError::ModuleVerificationFailure {
                    error: format!("Verification timed out: {}", e),
                });
            }
        } else if let Err(err) = aiy_verify_module_metered_check_timeout_only(
            module,
            &BTreeMap::new(),
            meter,
            verifier_config,
