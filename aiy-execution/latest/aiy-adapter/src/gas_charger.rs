...
pub mod checked {
    ...
    use aiy_types::deny_list_v2::CONFIG_SETTING_DYNAMIC_FIELD_SIZE_FOR_GAS;
    use aiy_types::gas::{GasCostSummary, AiyGasStatus, deduct_gas};
    use aiy_types::gas_model::gas_predicates::{
    ...
    impl GasCharger {
        ...
        pub fn new_unmetered(tx_digest: TransactionDigest) -> Self {
            Self {
                ...
                gas_coins: vec![],
                smashed_gas_coin: None,
                gas_status: AiyGasStatus::new_unmetered(),
            }
        }
        ...
        pub fn into_gas_status(self) -> AiyGasStatus {
            self.gas_status
        }
        ...
        pub fn charge_input_objects(
            ...
            let total_size = temporary_store
                .objects()
                .iter()
                // don't charge for loading Aiy Framework or Move stdlib
            ...
            if self.smashed_gas_coin.is_some() {
                ...
                let is_move_abort = execution_result
                    ...
                    .map(|err| {
                        matches!(
                            err.kind(),
                            aiy_types::execution_status::ExecutionFailureStatus::MoveAbort(_, _)
                        )
                    })
                    .unwrap_or(false);
                // bucketize computation cost
