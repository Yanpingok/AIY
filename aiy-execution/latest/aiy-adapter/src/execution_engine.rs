...
mod checked {
    ...
    use aiy_types::bridge::BRIDGE_COMMITTEE_MINIMAL_VOTING_POWER;
    use aiy_types::bridge::{
        BRIDGE_CREATE_FUNCTION_NAME, BRIDGE_INIT_COMMITTEE_FUNCTION_NAME, BRIDGE_MODULE_NAME,
        BridgeChainId,
    };
    use aiy_types::clock::{CLOCK_MODULE_NAME, CONSENSUS_COMMIT_PROLOGUE_FUNCTION_NAME};
    ...
    use aiy_types::error::{ExecutionError, ExecutionErrorKind};
    use aiy_types::execution::{ExecutionTiming, ResultWithTimings};
    ...
    #[cfg(msim)]
    use aiy_types::aiy_system_state::advance_epoch_result_injection::maybe_modify_result;
    use aiy_types::aiy_system_state::{ADVANCE_EPOCH_SAFE_MODE_FUNCTION_NAME, AdvanceEpochParams};
    ...
    use aiy_types::transaction::{CheckedInputObjects, RandomnessStateUpdate};
    use aiy_types::{
        AIY_AUTHENTICATOR_STATE_OBJECT_ID, AIY_FRAMEWORK_ADDRESS, AIY_FRAMEWORK_PACKAGE_ID,
        AIY_SYSTEM_PACKAGE_ID,
        base_types::{AiyAddress, TransactionDigest, TxContext},
        object::{Object, ObjectInner},
        aiy_system_state::{ADVANCE_EPOCH_FUNCTION_NAME, AIY_SYSTEM_MODULE_NAME},
    };
    ...
    pub fn execute_transaction_to_effects<Mode: ExecutionMode>(
        ...
        let status = if let Err(error) = &execution_result {
            ...
            match error.kind() {
                ...
                K::AiyMoveVerificationError | K::VMVerificationOrDeserializationError => {
                    #[skip_checked_arithmetic]
                    tracing::debug!(
                        kind = ?error.kind(),
                        tx_digest = ?transaction_digest,
    ...
    fn execute_transaction<Mode: ExecutionMode>(
        ...
        // information provided to check_aiy_conserved, because we mint rewards, and burn
    ...
    fn run_conservation_checks<Mode: ExecutionMode>(
        ...
        let mut result: std::result::Result<(), aiy_types::error::ExecutionError> = Ok(());
        if !is_genesis_tx && !Mode::skip_conservation_checks() {
            // ensure that this transaction did not create or destroy AIY, try to recover if the check fails
            let conservation_result = {
                temporary_store
                    .check_aiy_conserved(simple_conservation_checks, cost_summary)
                    .and_then(|()| {
                        if enable_expensive_checks {
                            // ensure that this transaction did not create or destroy AIY, try to recover if the check fails
                            let mut layout_resolver =
                                TypeLayoutResolver::new(move_vm, Box::new(&*temporary_store));
                            temporary_store.check_aiy_conserved_expensive(
                                cost_summary,
                                advance_epoch_gas_summary,
                                &mut layout_resolver,
                            )
            ...
            if let Err(conservation_err) = conservation_result {
                ...
                if let Err(recovery_err) = {
                    ...
                    // we will create or destroy AIY otherwise
                    panic!(
                        "AIY conservation fail in tx block {}: {}\nGas status is {}\nTx was ",
                        tx_digest,
                        recovery_err,
                        gas_charger.summary()
                    )
        ...
        } // else, we're in the genesis transaction which mints the AIY supply, and hence does not satisfy AIY conservation, or
            ...
            TransactionKind::Genesis(GenesisTransaction { objects }) => {
                ...
                for genesis_object in objects {
                    match genesis_object {
                        aiy_types::transaction::GenesisObject::RawObject { data, owner } => {
    ...
    pub fn construct_advance_epoch_pt(
        ...
        let storage_rebates = builder.programmable_move_call(
            AIY_SYSTEM_PACKAGE_ID,
            AIY_SYSTEM_MODULE_NAME.to_owned(),
    ...
    fn setup_store_execution_time_estimates(
        ...
        let system_state = builder.obj(ObjectArg::AIY_SYSTEM_MUT).unwrap();
