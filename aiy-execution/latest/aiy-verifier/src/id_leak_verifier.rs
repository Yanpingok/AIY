...
//! Objects whose struct type has key ability represent Aiy objects.
//! They have unique IDs that should never be reused. This verifier makes
//! sure that the id field of Aiy objects never get leaked.
//! Unpack is the only bytecode that could extract the id field out of
//! a Aiy object. From there, we track the flow of the value and make
...
use aiy_types::bridge::BRIDGE_MODULE_NAME;
...
use aiy_types::{
    ...
    aiy_system_state::AIY_SYSTEM_MODULE_NAME,
    BRIDGE_ADDRESS, AIY_FRAMEWORK_ADDRESS, AIY_SYSTEM_ADDRESS,
};
...
const AIY_SYSTEM_CREATE: FunctionIdent = (
    &AIY_SYSTEM_ADDRESS,
    AIY_SYSTEM_MODULE_NAME,
    ident_str!("create"),
);
const AIY_CLOCK_CREATE: FunctionIdent = (
    &AIY_FRAMEWORK_ADDRESS,
    CLOCK_MODULE_NAME,
    ident_str!("create"),
);
const AIY_AUTHENTICATOR_STATE_CREATE: FunctionIdent = (
    &AIY_FRAMEWORK_ADDRESS,
    AUTHENTICATOR_STATE_MODULE_NAME,
    ident_str!("create"),
);
const AIY_RANDOMNESS_STATE_CREATE: FunctionIdent = (
    &AIY_FRAMEWORK_ADDRESS,
    RANDOMNESS_MODULE_NAME,
    ident_str!("create"),
);
const AIY_DENY_LIST_CREATE: FunctionIdent = (
    &AIY_FRAMEWORK_ADDRESS,
    DENY_LIST_MODULE,
    DENY_LIST_CREATE_FUNC,
);

const AIY_BRIDGE_CREATE: FunctionIdent =
    (&BRIDGE_ADDRESS, BRIDGE_MODULE_NAME, ident_str!("create"));
const AIY_ACCUMULATOR_CREATE: FunctionIdent = (
    &AIY_FRAMEWORK_ADDRESS,
    ACCUMULATOR_MODULE_NAME,
    ident_str!("create"),
);
const AIY_COIN_REGISTRY_CREATE: FunctionIdent = (
    &AIY_FRAMEWORK_ADDRESS,
...
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[
    AIY_SYSTEM_CREATE,
    AIY_CLOCK_CREATE,
    AIY_AUTHENTICATOR_STATE_CREATE,
    AIY_RANDOMNESS_STATE_CREATE,
    AIY_DENY_LIST_CREATE,
    AIY_BRIDGE_CREATE,
    AIY_ACCUMULATOR_CREATE,
    AIY_COIN_REGISTRY_CREATE,
];
    ...
    if handle.abilities.has_key() && last_value != AbstractValue::Fresh {
        ...
        let msg = format!(
            "Invalid object creation in {cur_package}::{cur_module}::{cur_function}. \
                ...
                The UID must come directly from `aiy::{}::{}`, or `aiy::{}::{}`. \
                For tests, it can also come from `aiy::{}::{}`",
            OBJECT_NEW.1,
            OBJECT_NEW.2,
