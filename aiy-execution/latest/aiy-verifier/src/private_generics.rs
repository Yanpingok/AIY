...
use aiy_types::{error::ExecutionError, AIY_FRAMEWORK_ADDRESS};
...
/// All transfer functions (the functions in `aiy::transfer`) are "private" in that they are
...
pub fn verify_module(
    ...
    if *module.address() == AIY_FRAMEWORK_ADDRESS
        && module.name() == IdentStr::new(TEST_SCENARIO_MODULE_NAME).unwrap()
    {
        // exclude test_module which is a test-only module in the Aiy framework which "emulates"
        // transactional execution and needs to allow test code to bypass private generics
        return Ok(());
    }
    // do not need to check the aiy::transfer module itself
...
fn verify_function(
    ...
    for instr in &code.code {
        if let Bytecode::CallGeneric(finst_idx) = instr {
            ...
            if ident == (AIY_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
                verify_private_transfer(view, fhandle, type_arguments, allow_receiving_object_id)?
            } else if ident == (AIY_FRAMEWORK_ADDRESS, EVENT_MODULE) {
                verify_private_event_emit(view, fhandle, type_arguments)?
            } else if ident == (AIY_FRAMEWORK_ADDRESS, COIN_REGISTRY_MODULE) {
...
fn verify_private_transfer(
    ...
    if addr_module(view, self_handle) == (AIY_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
        return Ok(());
    }
    ...
    if !is_defined_in_current_module(view, type_arg) {
        return Err(format!(
            "Invalid call to '{aiy}::transfer::{f}' on an object of type '{t}'. \
            ...
            instead, i.e. '{aiy}::transfer::public_{f}'",
            aiy = AIY_FRAMEWORK_ADDRESS,
            f = fident,
...
fn verify_private_event_emit(
    ...
    if fident == EMIT_AUTHENTICATED_IMPL_FUNCTION {
        ...
        if (module_id.address(), module_id.name()) != (&AIY_FRAMEWORK_ADDRESS, EVENT_MODULE) {
            debug_assert!(
                false,
