...
pub fn verify_module(
    ...
    // In Aiy's framework code there is an exception to the one-time witness type rule - we have a
    // AIY type in the aiy module but it is instantiated outside of the module initializer (in fact,
    // the module has no initializer). The reason for it is that the AIY coin is only instantiated
    ...
    if ModuleId::new(AIY_FRAMEWORK_ADDRESS, ident_str!("aiy").to_owned()) == self_id {
        return Ok(());
    }
