...
use aiy_types::{
    base_types::{MoveObjectType, ObjectID, SequenceNumber, AiyAddress},
    committee::EpochId,
    ...
    TypeTag, AIY_ACCUMULATOR_ROOT_OBJECT_ID, AIY_AUTHENTICATOR_STATE_OBJECT_ID,
    AIY_BRIDGE_OBJECT_ID, AIY_CLOCK_OBJECT_ID, AIY_COIN_REGISTRY_OBJECT_ID,
    AIY_DENY_LIST_OBJECT_ID, AIY_RANDOMNESS_STATE_OBJECT_ID, AIY_SYSTEM_STATE_OBJECT_ID,
};
use tracing::error;
...
pub(crate) struct TestInventories {
    ...
    pub(crate) address_inventories: BTreeMap<AiyAddress, BTreeMap<MoveObjectType, Set<ObjectID>>>,
...
pub struct RuntimeResults {
    ...
    pub settlement_input_aiy: u64,
    pub settlement_output_aiy: u64,
}
...
pub(crate) struct ObjectRuntimeState {
    ...
    // Used to track AIY conservation in settlement transactions. Settlement transactions
    // gather up withdraws and deposits from other transactions, and record them to accumulator
    // fields. The settlement transaction records the total amount of AIY being disbursed here,
    // so that we can verify that the amount stored in the fields at the end of the transaction
    // is correct.
    settlement_input_aiy: u64,
    settlement_output_aiy: u64,
}
...
impl<'a> ObjectRuntime<'a> {
    pub fn new(
        ...
        Self {
            ...
            state: ObjectRuntimeState {
                ...
                settlement_input_aiy: 0,
                settlement_output_aiy: 0,
            },
            is_metered,
            protocol_config,
            metrics,
    ...
    pub fn record_settlement_aiy_conservation(&mut self, input_aiy: u64, output_aiy: u64) {
        self.state.settlement_input_aiy += input_aiy;
        self.state.settlement_output_aiy += output_aiy;
    }
...
impl ObjectRuntimeState {
    ...
    pub(crate) fn finish(
        ...
        let ObjectRuntimeState {
            ...
            received,
            accumulator_events,
            settlement_input_aiy,
            settlement_output_aiy,
        } = self;
