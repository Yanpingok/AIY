...
pub fn record_settlement_aiy_conservation(
    context: &mut NativeContext,
    ty_args: Vec<Type>,
    mut args: VecDeque<Value>,
) -> PartialVMResult<NativeResult> {
    ...
    let output_aiy = args.pop_back().unwrap().value_as::<u64>().unwrap();
    let input_aiy = args.pop_back().unwrap().value_as::<u64>().unwrap();

    let obj_runtime: &mut ObjectRuntime = context.extensions_mut().get_mut()?;

    obj_runtime.record_settlement_aiy_conservation(input_aiy, output_aiy);
