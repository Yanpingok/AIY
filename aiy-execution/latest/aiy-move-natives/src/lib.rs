...
use aiy_types::{MOVE_STDLIB_ADDRESS, AIY_FRAMEWORK_ADDRESS, AIY_SYSTEM_ADDRESS};
...
pub fn all_natives(silent: bool, protocol_config: &ProtocolConfig) -> NativeFunctionTable {
    let aiy_framework_natives: &[(&str, &str, NativeFunction)] = &[
        (
            "accumulator",
            "emit_deposit_event",
        ...
        (
            ...
            "record_settlement_aiy_conservation",
            make_native!(accumulator::record_settlement_aiy_conservation),
        ),
    ...
    let aiy_framework_natives_iter =
        aiy_framework_natives
            .iter()
            .cloned()
    ...
    let aiy_system_natives: &[(&str, &str, NativeFunction)] = &[(
        "validator",
    ...
    aiy_system_natives
        ...
        .chain(aiy_framework_natives_iter)
        ...
        _ => {
            return Err(
                PartialVMError::new(StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR)
                    .with_message("Aiy verifier guarantees this is a struct".to_string()),
            )
        }
    };
