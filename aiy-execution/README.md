...
The `aiy-execution` crate is responsible for abstracting access to the
...
- The adapter, that integrates Move into Aiy.
...
- `./aiy-execution`, (latest version and cuts/copies of aiy-specific
  crates).
...
code must be via `aiy-execution`, and not by directly depending a
constituent crate.
...
and elsewhere, **default to accessing execution via `aiy-execution`.**
...
`aiy-execution tests::test_encapsulation` is a test that detects
...
`./crates/aiy-types` are shared across all versions.  Changes to
...
## `aiy-execution/src/lib.rs`

The entry-point to the execution crate -- `aiy-execution/src/lib.rs`
...
- `aiy-execution/src/lib.template.rs` -- a template file with
