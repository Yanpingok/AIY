...
const KNOWN_TYPES = {
  aiyfren:
    '0x80d7de9c4a56194087e0ba0bf59492aa8e6a5ee881606226930827085ddf2332::aiyfrens::AiyFren<0x80d7de9c4a56194087e0ba0bf59492aa8e6a5ee881606226930827085ddf2332::capy::Capy>',
};
...
program
  .command('list')
  .description('list an item in the Kiosk for the specified amount of AIY')
  .argument('<item ID>', 'The ID of the item to list')
  .argument('<amount MIST>', 'The amount of AIY to list the item for')
  .action(listItem);
...
async function newKiosk() {
  const sender = keypair.getPublicKey().toAiyAddress();
...
async function showInventory({ address, onlyDisplay, cursor, filter }) {
  const owner = address || keypair.getPublicKey().toAiyAddress();

  if (!isValidAiyAddress(owner)) {
    throw new Error(`Invalid AIY address: "${owner}"`);
  }
...
async function showKioskContents({ id, address }) {
  ...
  if (id) {
    if (!isValidAiyObjectId(id)) {
      throw new Error(`Invalid Kiosk ID: "${id}"`);
    }

    kioskId = id;
  } else {
    const sender = address || keypair.getPublicKey().toAiyAddress();

    if (!isValidAiyAddress(sender)) {
      throw new Error(`Invalid AIY address: "${sender}"`);
    }
  ...
  const tabledItems = items
    .map((item) => ({
      ...
      'price (AIY)': item.listing ? formatAmount(item.listing.price) : 'N/A',
    }))
...
async function placeItem(itemId) {
  const kioskCap = await findKioskCap().catch(() => null);
  const owner = keypair.getPublicKey().toAiyAddress();

  if (kioskCap === null) {
    throw new Error('No Kiosk found for sender; use `new` to create one');
  }

  if (!isValidAiyObjectId(itemId)) {
...
async function takeItem(itemId, { address }) {
  const cap = await findKioskCap().catch(() => null);
  const receiver = address || keypair.getPublicKey().toAiyAddress();

  if (!isValidAiyAddress(receiver)) {
...
/**
 * Command: `list`
 * Description: Lists an item in the Kiosk for the specified amount of AIY
 */
...
async function purchaseItem(itemId, opts) {
  ...
  if (inputKioskId && !isValidAiyObjectId(inputKioskId)) {
...
function formatAmount(amount) {
  ...
  if (amount <= MIST_PER_AIY) {
    return Number(amount) / Number(MIST_PER_AIY);
  }
