...
import { AiyArgument, AiyCallArg, AiyTransaction, TransactionBlockData } from '@mysten/aiy/client';
...
const getCallArgDisplay = (argument: AiyCallArg | undefined) => {
...
const getAiyArgumentDisplay = (argument: AiyArgument, inputs: AiyCallArg[]) => {
	if (typeof argument === 'string') return argument;
...
const renderArguments = (callArgs: AiyArgument[], inputs: AiyCallArg[]) => {
	return (
		<div className="flex overflow-x-auto gap-3 my-3">
			{callArgs.map((arg, index) => (
				<div key={index} className="flex-shrink-0">
					{getAiyArgumentDisplay(arg, inputs)}
				</div>
			))}
		</div>
	);
