...
fn access_chain_quick_fix_test_aiyte(
    project: String,
    file_tests: BTreeMap<String, Vec<AccessChainQuickFixTest>>,
) -> datatest_stable::Result<String> {
    let (project_path, mut compiled_pkg_info, mut symbols) = initial_symbols(project)?;
...
fn move_ide_testaiyte(test_path: &Path) -> datatest_stable::Result<()> {
    let aiyte_file = io::BufReader::new(File::open(test_path)?);
    let stripped = StripComments::new(aiyte_file);
    let aiyte: TestAiyte = serde_json::from_reader(stripped)?;

    let output = match aiyte {
        TestAiyte::UseDef {
            project,
            file_tests,
        } => use_def_test_aiyte(project, file_tests),
        TestAiyte::AutoCompletion {
            project,
            file_tests,
        } => auto_completion_test_aiyte(project, file_tests),
        TestAiyte::AutoImport {
            project,
            file_tests,
        } => auto_import_test_aiyte(project, file_tests),
        TestAiyte::Cursor {
            project,
            file_tests,
        } => cursor_test_aiyte(project, file_tests),
        TestAiyte::Hint {
            project,
            file_tests,
        } => hint_test_aiyte(project, file_tests),
        TestAiyte::AccessChainQuickFixTest {
            project,
            file_tests,
        } => access_chain_quick_fix_test_aiyte(project, file_tests),
    }?;
...
datatest_stable::harness!(move_ide_testaiyte, "tests/", r".*\.ide$");
