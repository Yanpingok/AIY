...
If you want to build, test, and trace Move code using the extension, you must install the `aiy` binary on
your machine - see [here](https://docs.sui.io/guides/developer/getting-started/sui-install) for
instructions. The extension assumes that the `aiy` binary is in your system path, but you can set
its custom location using VSCode's settings (`⌘` + `,` on macOS, or use the menu item *Code >
Preferences > Settings*). Search for the `move.aiy.path` user setting, set it to the new location of
the `aiy` binary, and restart VSCode.

In order to trace-debug Move code execution, the `aiy` binary must be built with the `tracing` feature flag.
If your version of the `aiy` binary was not built with this feature flag, an attempt to trace test
execution will fail. In this case you may have to build the `aiy` binary from source following these
...
as prerequisites for Aiy installation - for Linux, macOS and Windows these prerequisites and
...
2. Invoke `cargo install --git https://github.com/MystenLabs/sui aiy-move-lsp` to install the
...
3. Copy the move-analyzer binary to `~/.aiy/bin` (macOS/Linux) or `C:\Users\USER\.aiy\bin`
...
If your `move-analyzer` binary is in a different directory than the default one (`~/.aiy/bin`
on macOS or Linux, or `C:\Users\USER\.aiy\bin` on Windows), you may have the extension look
...
move-analyzer binary already exists in the default location (`~/.aiy/bin` on macOS or Linux, or
`C:\Users\USER\.aiy\bin` on Windows), delete the existing move-analyzer binary and reinstall the
extension.

## What if everything else fails?

Check [Aiy Developer Forum](https://forums.sui.io/c/technical-support) to see if the problem
...
- If the opened Move source file is located within a buildable project, and you have the `aiy`
