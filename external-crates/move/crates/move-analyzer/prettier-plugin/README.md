...
Clone Aiy repository into `$AIY` directory:

```bash
git clone https://github.com/MystenLabs/sui.git "$AIY"
```

Go to `"$AIY"/external-crates/move/crates/move-analyzer/prettier-plugin` and run the following
command:
...
``` bash
npm install prettier@3.1.1 "$AIY"/external-crates/move/crates/move-analyzer/prettier-plugin
```
...
implement formatting for structs) and file an issue in the Aiy
