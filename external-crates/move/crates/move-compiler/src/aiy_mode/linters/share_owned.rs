...
use crate::{
    ...
    aiy_mode::{
        AIY_ADDR_VALUE, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_TYPE_NAME,
        info::TransferKind,
        linters::{
...
const SHARE_FUNCTIONS: &[(AccountAddress, &str, &str)] = &[
    (AIY_ADDR_VALUE, TRANSFER_MOD_NAME, PUBLIC_SHARE_FUN),
    (AIY_ADDR_VALUE, TRANSFER_MOD_NAME, SHARE_FUN),
];

const SHARE_OWNED_DIAG: DiagnosticInfo = custom(
    LINT_WARNING_PREFIX,
    Severity::Warning,
    LinterDiagnosticCategory::Aiy as u8,
...
impl ShareOwnedVerifierAI<'_> {
    ...
    /// Gets AIY info from either the current program or precompiled library
    fn get_aiy_info(&self, mident: &ModuleIdent) -> &crate::aiy_mode::info::AiyModInfo {
        self.info
            .modules
            .get(mident)
            .or_else(|| {
            ...
            .aiy_info
            .as_ref()
            .expect("ICE typing module info expected to contain Aiy info")
    }
    ...
    fn transfer_kind(
        ...
        self.get_aiy_info(mident).transferred.get(datatype_name)
    }
}
