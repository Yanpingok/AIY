...
//! ProgramInfo extension for Aiy Flavor
//! Contains information that may be expensive to compute and is needed only for Aiy
...
use crate::{
    ...
    aiy_mode::{
        OBJECT_MODULE_NAME, AIY_ADDR_VALUE, TRANSFER_FUNCTION_NAME, TRANSFER_MODULE_NAME,
        UID_TYPE_NAME,
    },
...
pub enum TransferKind {
    ...
    PublicTransfer(Loc),
    /// transferred within the module to an address vis `aiy::transfer::transfer`
    PrivateTransfer(Loc),
}

/// Aiy info for the entire program (mostly used during translation ).
#[derive(Debug, Clone)]
pub struct AiyInfo {
...
/// Same as `AiyInfo` but for a given module (store per-module in `ProgramInfo`).
#[derive(Debug, Clone)]
pub struct AiyModInfo {
    ...
    fn visit_ty(
        ...
        match ty_ {
            ...
            N::Type_::Apply(_, sp!(_, tn_), _)
                if tn_.is(&AIY_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME) =>
            {
