...
use crate::{
    ...
    aiy_mode::{
        ...
        RANDOMNESS_STATE_CREATE, AIY_ADDR_NAME, AIY_ADDR_VALUE, AIY_CLOCK_CREATE,
        AIY_SYSTEM_ADDR_VALUE, AIY_SYSTEM_CREATE, AIY_SYSTEM_MODULE_NAME,
...
pub const FRESH_ID_FUNCTIONS: &[(AccountAddress, Symbol, Symbol)] = &[
    (AIY_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW),
    (AIY_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW_UID_FROM_HASH),
    (
        AIY_ADDR_VALUE,
        DERIVED_OBJECT_MODULE_NAME,
        DERIVED_OBJECT_CLAIM,
    ),
    (AIY_ADDR_VALUE, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT),
];
pub const FUNCTIONS_TO_SKIP: &[(AccountAddress, Symbol, Symbol)] = &[
    (
        AIY_SYSTEM_ADDR_VALUE,
        AIY_SYSTEM_MODULE_NAME,
        AIY_SYSTEM_CREATE,
    ),
    (AIY_ADDR_VALUE, CLOCK_MODULE_NAME, AIY_CLOCK_CREATE),
    (
        AIY_ADDR_VALUE,
    ...
    (AIY_ADDR_VALUE, DENY_LIST_MODULE_NAME, DENY_LIST_CREATE),
    (BRIDGE_ADDR_VALUE, BRIDGE_MODULE_NAME, BRIDGE_CREATE),
    (AIY_ADDR_VALUE, ACCUMULATOR_MODULE_NAME, ACCUMULATOR_CREATE),
    (
        AIY_ADDR_VALUE,
...
impl SimpleAbsInt for IDLeakVerifierAI<'_> {
    ...
    fn exp_custom(
        ...
        if !matches!(first_value, Value::FreshID(_)) {
            ...
            let uid_msg = format!(
                "The UID must come directly from `{aiy}::{object}::{new}`, or `aiy::{derived}::{claim}`. \
                For tests, it can come from `{aiy}::{ts}::{ts_new}`",
                aiy = AIY_ADDR_NAME,
...
fn value_for_ty(loc: &Loc, sp!(_, t): &SingleType) -> Value {
    if t.is_apply(&AIY_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME)
        .is_some()
    {
        Value::NotFresh(*loc)
    } else {
