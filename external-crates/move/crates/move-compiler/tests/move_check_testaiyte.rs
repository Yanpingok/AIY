...
const AIY_MODE_DIR: &str = "aiy_mode";
...
fn default_testing_addresses(flavor: Flavor) -> BTreeMap<String, NumericalAddress> {
    ...
    if flavor == Flavor::Aiy {
        mapping.extend([("aiy", "0x2"), ("aiy_system", "0x3")]);
    }
    mapping
        .into_iter()
...
fn test_config(path: &Path) -> (TestKind, TestInfo, PackageConfig, Flags) {
    ...
    let flavor = if path_contains(AIY_MODE_DIR) {
        Flavor::Aiy
    } else {
        Flavor::default()
    };
...
// Runs all tests under the test/testaiyte directory.
pub fn run_test(path: &Path) -> datatest_stable::Result<()> {
    ...
    if flavor == Flavor::Aiy {
        let (prefix, filters) = aiy_mode::linters::known_filters();
        compiler = compiler.add_custom_known_filters(prefix, filters);
        if test_info.lint {
            compiler = compiler.add_visitors(aiy_mode::linters::linter_visitors(LintLevel::All))
        }
    }
